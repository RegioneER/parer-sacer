...
public class AnnulVersEjb {
     ...
     *
     * @param cdRichAnnulVers codice richiesta annullamento
     * @param idStrut         id struttura
     *
     * @return true se esiste gi\u00E0 una richiesta di annullamento con codice
     *         <code>cdRichAnnulVers</code>
     */
    ...
    public Long saveRichAnnulVers(long idUserIam, String cdRichAnnulVers, String dsRichAnnulVers,
            String ntRichAnnulVers, String flImmediata, byte[] fileByteArray, BigDecimal idStrut,
        ...
        try {
            ...
            if (tiRichAnnulVers.equals("UNITA_DOC")) {
                if (fileByteArray != null) {
                    ...
                    context.getBusinessObject(AnnulVersEjb.class).handleCsvRecords(rich,
                ...
                if (fileByteArray != null) {
                    ...
                    // Preparo gli item e gli eventuali errori in ARO_ITEM_RICH_ANNUL_VERS e
                    // ARO_ERR_RICH_ANNUL_VERS
                    context.getBusinessObject(AnnulVersEjb.class).handleCsvRecordsFasc(rich,
                            fileByteArray, idUserIam);
                }
            }
     ...
     * @param dsRichAnnulVers      descrizione richiesta
     * @param ntRichAnnulVers      nota richiesta
     * @param tiRichAnnulVers      tipo richiesta
     * @param flImmediata          indicatore di annullamento immediato
     * @param dtCreazione          data creazione
     * @param idStrut              identificativo della struttura corrente
     * @param flForzaAnnul         indicatore di forzatura annullamento
     * @param flRichiestaPing      indicatore di richiesta annullamento da PreIngest
     *
     * @param tiAnnulRichAnnulVers tipo annullamento
     * @param ravExt               invio richiesta annullamento
     *
     * @return id richiesta
     *
     * @throws ParerUserError errore generico
    ...
    public AroRichAnnulVers insertRichAnnulVers(long idUserIam, Long idStrut,
            String cdRichAnnulVers, String dsRichAnnulVers, String ntRichAnnulVers,
            String tiRichAnnulVers, Date dtCreazione, boolean flImmediata, boolean flForzaAnnul,
            boolean flRichiestaPing, String tiAnnulRichAnnulVers,
        ...
        try {
            ...
            rich = initAroRichAnnulVers(cdRichAnnulVers, dsRichAnnulVers, ntRichAnnulVers,
                    tiRichAnnulVers, dtCreazione,
                    CostantiDB.TipoCreazioneRichAnnulVers.WEB_SERVICE.name(), flImmediata,
                    flForzaAnnul, flRichiestaPing, tiAnnulRichAnnulVers, strut);

            // Persisto la richiesta
            helper.insertEntity(rich, true);

            // Popolo gli item
            handleXmlRecords(rich,
                    ravExt.getRichiestaAnnullamentoVersamenti().getVersamentiDaAnnullare(),
                    idUserIam);
    ...
    private AroRichAnnulVers initAroRichAnnulVers(String cdRichAnnulVers, String dsRichAnnulVers,
            String ntRichAnnulVers, String tiRichAnnulVers, Date now, String tiCreazione,
            boolean flImmediata, boolean flForzaAnnul, boolean flRichiestaPing,
            String tiAnnulRichAnnulVers, OrgStrut strut) {
        AroRichAnnulVers rich = new AroRichAnnulVers();
        rich.setCdRichAnnulVers(cdRichAnnulVers);
        rich.setDsRichAnnulVers(dsRichAnnulVers);
        rich.setNtRichAnnulVers(ntRichAnnulVers);
    ...
    /**
     * Legge il csv e crea gli oggetti AroItemRichAnnulVers da salvare alla richiesta di
     * annullamento e gli eventuali errori in AroErrRichAnnulVers
     *
     * @param rich          la richiesta di annullamento
     ...
     *
     * @throws IOException    eccezione di tipo IO
     * @throws ParerUserError Errore gestito annullamento a ping
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
            ...
            if (csvReader.readHeaders()) {
                ...
                while (csvReader.readRecord()) {
                    ...
                    if (udsInRich.add(new UnitaDocBean(idStrut, registro, anno, numero))) {
                        ...
                        AroItemRichAnnulVers item = createAroItemRichAnnulVers(rich, registro, anno,
                                numero, progressivoItem);
                        // Ricavo l'ud non annullata
                        Long idUnitaDoc = udHelper.getIdUnitaDocVersataNoAnnul(idStrut, registro,
                                anno, numero);
                        ...
                        if (idUnitaDoc != null) {
                            ...
                            if (item.getTiStatoItem()
                                    .equals(CostantiDB.StatoItemRichAnnulVers.DA_ANNULLARE_IN_PING
                                            .name())) {
                                throw new ParerUserError(
                            ...
                            // Se non esiste una ud non annullata, allora controllo se esiste in
                            // generale
                            if (udHelper.existAroUnitaDoc(idStrut, registro, anno, numero)) {
                                ...
                                createAroErrRichAnnulVers(item, BigDecimal.ONE,
                                        CostantiDB.TipoErrRichAnnulVers.ITEM_GIA_ANNULLATO.name(),
                                ...
                                createAroErrRichAnnulVers(item, BigDecimal.ONE,
                                        CostantiDB.TipoErrRichAnnulVers.ITEM_NON_ESISTE.name(),
                                        dsErr, CostantiDB.TipoGravitaErrore.ERRORE.name());
                            }
                        }
                        progressivoItem++;
                ...
                if (progressivoItem == 1) {
                    ...
                    throw new ParerUserError(
                            "Il file non contiene alcuna unit\u00E0 documentaria da annullare");
                }
            } else {
    ...
    /**
     * Legge il csv e crea gli oggetti AroItemRichAnnulVers da salvare alla richiesta di
     * annullamento e gli eventuali errori in AroErrRichAnnulVers riguardante i fascicoli
     *
     * @param rich          la richiesta di annullamento
     * @param fileByteArray il file csv in byte[]
     * @param idUserIam     id utente che ha creato la richiesta (ovvero colui che ha definito il
            ...
            if (csvReader.readHeaders()) {
                ...
                while (csvReader.readRecord()) {
                    ...
                    if (fascInRich.add(new FascBean(idStrut, anno, numero))) {
                        ...
                        // registrare in
                        // ARO_ITEM_RICH_ANNUL_VERS
                        AroItemRichAnnulVers item = createAroItemRichAnnulVersFasc(rich, anno,
                                numero, progressivoItem);
                        // Ricavo il fasicolo non annullato
                        Long idFascicolo = fascicoliHelper.getIdFascVersatoNoAnnul(idStrut, anno,
                                numero);
                        logger.debug("Controlli item da annullare");
                        if (idFascicolo != null) {
                            ...
                            // Se non esiste un fascicolo non annullato, allora controllo se esiste
                            // in
                            // generale
    ...
    /**
     * Legge la richiesta ottenuta dall'XML e crea gli oggetti AroItemRichAnnulVers da salvare alla
     * richiesta di annullamento insieme agli eventuali AroErrRichAnnulVers. Infine controlla le ud
     *
     * @param rich                  la richiesta di annullamento
     * @param versamentiDaAnnullare versamenti da annullare
     * @param idUserIam             id user IAM
     *
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void handleXmlRecords(AroRichAnnulVers rich,
            RichiestaAnnullamentoVersamenti.VersamentiDaAnnullare versamentiDaAnnullare,
            long idUserIam) {
        int progressivoItem = 1;
        Set<UnitaDocBean> udsInRich = new HashSet<>();
        ...
        for (VersamentoDaAnnullareType versamentoDaAnnullare : versamentiDaAnnullare
                .getVersamentoDaAnnullare()) {
            entityManager.flush();
            entityManager.clear();
            ...
            if (TipoVersamentoType.UNITA_DOCUMENTARIA.value().equals(tipoVersamento.value())) {
                /*
                 * Trucchetto bonnico che "sfrutta" una collection di natura "set" per verificare se
                 * un record è già presente nella richiesta. Se così fosse, il metodo add non
                 * aggiungerebbe il nuovo record e, restituendo esso false, verrebbe eseguita la
                 * porzione di codice presente nell'else
                 */
                if (udsInRich.add(new UnitaDocBean(idStrut, registro, anno, numero))) {
                    // Creo la AroItemRichAnnulVers figlia della AroRichAnnulVers
                    AroItemRichAnnulVers item = createAroItemRichAnnulVers(rich, registro, anno,
                            numero, progressivoItem, CostantiDB.TipiEntitaSacer.UNI_DOC.name(),
                            CostantiDB.StatoItemRichAnnulVers.NON_ANNULLABILE.name());
                    Long idUnitaDoc = udHelper.getIdUnitaDocVersataNoAnnul(idStrut, registro, anno,
                            numero);
                    logger.debug("Controlli item da annullare");
                    if (idUnitaDoc != null) {
                        // UD esistente
                        AroUnitaDoc ud = helper.findById(AroUnitaDoc.class, idUnitaDoc);
                        item.setAroUnitaDoc(ud);
                        // Già qui, controllo l'unità doc definita nella richiesta e che sto
                        // trattando
                        controlloItemDaAnnullare(item, idUserIam);
                    } else {
                        // Se non esiste una ud non annullata, allora controllo se esiste in
                        ...
                        if (udHelper.existAroUnitaDoc(idStrut, registro, anno, numero)) {
                            // UD esistente - creo record di errore
                            String dsErr = "L'unit\u00E0 documentaria " + registro + "-" + annoInt
                                    + "-" + numero + " \u00E8 gi\u00E0 stata annullata";
                            createAroErrRichAnnulVers(item, BigDecimal.ONE,
                                    CostantiDB.TipoErrRichAnnulVers.ITEM_GIA_ANNULLATO.name(),
                                    dsErr, CostantiDB.TipoGravitaErrore.ERRORE.name());
                        } else {
                    ...
                    AroItemRichAnnulVers item = createAroItemRichAnnulVers(rich, registro, anno,
                            ...
                            CostantiDB.StatoItemRichAnnulVers.NON_ANNULLABILE.name());
                    String dsErr = "L'unit\u00E0 documentaria " + registro + "-" + annoInt + "-"
                            + numero + " \u00E8 gi\u00E0 presente nella richiesta";
                    createAroErrRichAnnulVers(item, BigDecimal.ONE,
                ...
                if (fascsInRich.add(new FascBean(idStrut, anno, numero))) {
                    AroItemRichAnnulVers item = createAroItemRichAnnulVersFasc(rich, anno, numero,
                            progressivoItem);
                    ...
                    if (idFasc != null) {
                        ...
                        item.setFasFascicolo(fasc);
                        // Già qui, controllo il fascicolo definito nella richiesta e che sto
                        // trattando
                        controlloItemDaAnnullare(item, idUserIam);
                    } else {
    ...
    /**
     * Controlli sull'item candidato ad essere annullato tenendo conto delle abilitazioni
     * dell'utente passato in ingresso
     *
     * @param item
     */
    private void controlloItemDaAnnullare(AroItemRichAnnulVers item, long idUserIam) {
        ...
        if (ud != null && item.getTiItemRichAnnulVers()
            ...
            AroRichAnnulVers existingRich = helper.getAroRichAnnulVersContainingUd(
                    ud.getIdUnitaDoc(), item.getAroRichAnnulVers().getIdRichAnnulVers());
            if (existingRich != null) {
                ...
                String dsErr = "L'unit\u00E0 documentaria " + ud.getCdRegistroKeyUnitaDoc() + "-"
                        ...
                        + " \u00E8 in corso di annullamento nella richiesta "
                        + existingRich.getCdRichAnnulVers();
            ...
            if (helper.isUdAnnullata(ud.getIdUnitaDoc())) {
                ...
                && item.getTiItemRichAnnulVers().equals(Constants.TipoEntitaSacer.FASC.name())) {
            // Controllo se esiste già un'altra richiesta, diversa da quella corrente, con
            // stato APERTA o CHIUSA che
            // contiene quel fascicolo
            AroRichAnnulVers existingRich = helper.getAroRichAnnulVersContainingFasc(
                    fascicolo.getIdFascicolo(), item.getAroRichAnnulVers().getIdRichAnnulVers());
            if (existingRich != null) {
            ...
            if ((!forzaAnnul
                ...
                String dsErr = "Il fascicolo " + fascicolo.getAaFascicolo() + "-"
                        ...
                        + fascicolo.getTiStatoConservazione()
            ...
            if (!helper.isUserAbilitatoToTipoDato(idUserIam,
                    fascicolo.getDecTipoFascicolo().getIdTipoFascicolo(),
            ...
            if (helper.isFascicoloAnnullato(fascicolo.getIdFascicolo())) {
                String dsErr = "Il fascicolo " + fascicolo.getAaFascicolo() + "-"
                        + fascicolo.getCdKeyFascicolo() + " \u00E8 gi\u00E0 stato annullato";
                createAroErrRichAnnulVers(item, new BigDecimal(progressivoErr++),
                        CostantiDB.TipoErrRichAnnulVers.ITEM_GIA_ANNULLATO.name(), dsErr,
                        CostantiDB.TipoGravitaErrore.ERRORE.name());
            }
    ...
    private void createAroErrRichAnnulVers(AroItemRichAnnulVers item, BigDecimal pgErr,
    ...
    private AroItemRichAnnulVers createAroItemRichAnnulVers(AroRichAnnulVers rich, String registro,
            BigDecimal anno, String numero, int progressivo) {
    ...
    private AroItemRichAnnulVers createAroItemRichAnnulVersFasc(AroRichAnnulVers rich,
            BigDecimal anno, String numero, int progressivo) {
        // Dovrei aver ottenuto tutti i campi necessari per creare il nuovo record
        AroItemRichAnnulVers item = new AroItemRichAnnulVers();
    ...
    private AroItemRichAnnulVers createAroItemRichAnnulVers(AroRichAnnulVers rich, String registro,
            BigDecimal anno, String numero, int progressivo, String tiItemRichAnnulVers,
            String tiStatoItem) {
        // Dovrei aver ottenuto tutti i campi necessari per creare il nuovo record
        AroItemRichAnnulVers item = new AroItemRichAnnulVers();
        item.setCdRegistroKeyUnitaDoc(registro);
    ...
    public void annullamentoVersamenti() {
        /*
         * Determino le richieste con stato CHIUSA per le quali non sia definito un item con stato
         * DA_ANNULLARE_IN_PING oppure le richieste con stato COMUNICATA A SACER, purché non
         * immediate
         */
        List<AroRichAnnulVers> richiesteAnnullamento = helper
                .getRichiesteAnnullamentoVersamentoDaElab();
    ...
    public void elaboraRichiestaAnnullamento(long idRichAnnulVers, long idUserIam,
        ...
        AroRichAnnulVers richiestaAnnullamento = helper.findByIdWithLock(AroRichAnnulVers.class,
                idRichAnnulVers);
        ...
        if (richiestaAnnullamento.getTiRichAnnulVers()
                .equals(CostantiDB.TiRichAnnulVers.UNITA_DOC.name())) {
            for (AroItemRichAnnulVers itemRichAnnulVers : richiestaAnnullamento
                    .getAroItemRichAnnulVers()) {
        ...
        } else if (richiestaAnnullamento.getTiRichAnnulVers()
                .equals(CostantiDB.TiRichAnnulVers.FASCICOLI.name())) {
            for (AroItemRichAnnulVers itemRichAnnulVers : richiestaAnnullamento
                    .getAroItemRichAnnulVers()) {
                // Controllo che l'item esista per loccarlo
                if (itemRichAnnulVers.getFasFascicolo() != null) {
                    helper.findByIdWithLock(FasFascicolo.class,
        ...
        if (statoRichAnnulVers.getTiStatoRichAnnulVers()
            ...
            helper.deleteAroErrRichAnnulVers(idRichAnnulVers,
                    CostantiDB.TipoErrRichAnnulVers.getStatiControlloItem());

            // Controllo gli item
            List<AroItemRichAnnulVers> itemRichAnnulVersList = richiestaAnnullamento
                    .getAroItemRichAnnulVers();
            for (AroItemRichAnnulVers item : itemRichAnnulVersList) {
                ...
                /*
                 * CONTROLLA ITEM DA ANNULLARE (senza cancellazione errori e assegnazione stato
                 * NON_ANNULLABILE)
                 */
                controlloItemDaAnnullare(item, idUserIam);
        ...
        if (proseguiAnnullamento && !richAnnulInPing) {
            context.getBusinessObject(AnnulVersEjb.class)
                    .evasioneRichiestaAnnullamento(richiestaAnnullamento, idUserIam, modalita);
        }
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void evasioneRichiestaAnnullamento(AroRichAnnulVers richiestaAnnullamento,
            long idUserIam, String modalita) {
        richiestaAnnullamento = entityManager.find(AroRichAnnulVers.class,
                richiestaAnnullamento.getIdRichAnnulVers(), LockModeType.PESSIMISTIC_WRITE);
        if (richiestaAnnullamento.getTiRichAnnulVers()
                .equals(CostantiDB.TiRichAnnulVers.UNITA_DOC.name()))

        {
            evadiAnnullamentoVersamentiUnitaDoc(richiestaAnnullamento, idUserIam, modalita);
        } else if (richiestaAnnullamento.getTiRichAnnulVers()
                .equals(CostantiDB.TiRichAnnulVers.FASCICOLI.name())) {
            evadiAnnullamentoVersamentiFascicoli(richiestaAnnullamento, idUserIam, modalita);
        }
    }
    ...
    public void evadiAnnullamentoVersamentiUnitaDoc(AroRichAnnulVers richiestaAnnullamento,
        ...
        logger.debug("{} Procedo ad evadere la richiesta avente id:{}", LOG_MESSAGE_ANNULLA_UD,
                idRichAnnulVers);
        logger.debug("{} tiStatoRichAnnulVers={} dtRegStatoRichAnnulVers={}",
                LOG_MESSAGE_ANNULLA_UD, statoRichAnnulVers.getTiStatoRichAnnulVers(),
                statoRichAnnulVers.getDtRegStatoRichAnnulVers());
        ...
        logger.debug("{} Gestione dei volumi di conservazione della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        List<VolVLisVolumeUdAnnul> volumiList = helper
                .retrieveVolVLisVolumeUdAnnul(idRichAnnulVers);
        ...
        for (VolVLisVolumeUdAnnul vols : volumiList) {
            logger.debug("{} Elaboro il volume con idRichAnnulVers={} idVolumeConserv={}",
            ...
            /*
             * Determina le unità doc appartenenti al volume (sia come ud versate che come doc
             * aggiunti) presenti come item da annullare nella richiesta il sistema registra in
             * VOL_VOLUME_VERS_UD_ANNUL l'insieme delle unità doc in esso presenti ed annullate
             */
            List<VolVLisUdAnnulByVolume> uds = helper.retrieveVolVLisUdAnnulByVolume(
                    idRichAnnulVers,
            ...
            for (VolVLisUdAnnulByVolume udAnnulInVolume : uds) {
                ...
                AroUnitaDoc ud = helper.findById(AroUnitaDoc.class,
                        udAnnulInVolume.getIdUnitaDoc());
            ...
            for (VolAppartUnitaDocVolume volAppart : volume.getVolAppartUnitaDocVolumes()) {
                ...
                for (VolAppartDocVolume appartDocVolume : appartDocVolumeList) {
                    appartDocVolume.setNtGenericheDoc(
                            ((StringUtils.isNotBlank(appartDocVolume.getNtGenericheDoc())
                                    ? appartDocVolume.getNtGenericheDoc() + " "
                                    : "") + "Versamento del documento annullato in data "
                                    + dtString).trim());
                }
            }
            logger.debug(
                    "{} Fine elaborazione del volume con idRichAnnulVers={} idVolumeConserv={}",
                    LOG_MESSAGE_ANNULLA_UD, vols.getVolVLisVolumeUdAnnulId().getIdRichAnnulVers(),
                    vols.getVolVLisVolumeUdAnnulId().getIdVolumeConserv());

        }
        logger.debug(
                "{} Fine della gestione dei volumi di conservazione della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);

        logger.debug("{} Gestione degli item della richiesta avente id: {}", LOG_MESSAGE_ANNULLA_UD,
                idRichAnnulVers);
        /*
         * Ricavo gli item di tipo UNI_DOC con stato DA_ANNULLARE_IN_SACER il cui stato relativo al
         * processo di inclusione in un elenco vale IN_ATTESA_MEMORIZZAZIONE, IN_ATTESA_SCHED,
         * NON_SELEZ_SCHED
         */
        List<AroItemRichAnnulVers> aroItemRichAnnulVersList = helper.getItem(idRichAnnulVers,
                ...
                ElencoEnums.UdDocStatusEnum.IN_ATTESA_SCHED.name(),
                ElencoEnums.UdDocStatusEnum.NON_SELEZ_SCHED.name());
        for (AroItemRichAnnulVers aroItemRichAnnulVers : aroItemRichAnnulVersList) {
            // Cancello l'ud dalla coda di costruzione elenchi
            logger.debug("{} AroItemRichAnnulVers con idItemRichAnnulVers={}",
                    LOG_MESSAGE_ANNULLA_UD, aroItemRichAnnulVers.getIdItemRichAnnulVers());
            logger.debug(
                    "{} Procedo alla cancellazione dalla coda di costruzione elenchi di idUnitaDoc={}",
                    LOG_MESSAGE_ANNULLA_UD, aroItemRichAnnulVers.getAroUnitaDoc().getIdUnitaDoc());
            helper.deleteElvUdVersDaElabElenco(
                    aroItemRichAnnulVers.getAroUnitaDoc().getIdUnitaDoc());
            // Annullo lo stato relativo al processo di inclusione in un elenco dell'ud
            logger.debug("{} Metto a null tiStatoUdElencoVers di idItemRichAnnulVers={}",
                    LOG_MESSAGE_ANNULLA_UD, aroItemRichAnnulVers.getIdItemRichAnnulVers());
        ...
        logger.debug("{} Gestione dei documenti aggiunti degli item della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        /*
         * Ricavo i documenti aggiunti degli item di tipo UNI_DOC (quindi sto considerando l'item
         * essere un'unit\u00E0 doc e ne cerco i documenti aggiunti) con stato DA_ANNULLARE_IN_SACER
         * il cui stato relativo al processo di inclusione in un elenco vale
         * IN_ATTESA_MEMORIZZAZIONE, IN_ATTESA_SCHED, NON_SELEZ_SCHED
         */
        List<AroDoc> docList = helper.getDocAggiunti(idRichAnnulVers,
                ElencoEnums.UdDocStatusEnum.IN_ATTESA_MEMORIZZAZIONE.name(),
                ElencoEnums.UdDocStatusEnum.IN_ATTESA_SCHED.name(),
                ElencoEnums.UdDocStatusEnum.NON_SELEZ_SCHED.name());
        logger.debug("{} - trovati {} AroDoc di documenti aggiunti ", LOG_MESSAGE_ANNULLA_UD,
                docList.size());
        for (AroDoc doc : docList) {
            ...
            logger.debug(
                    "{} Procedo alla cancellazione dalla coda di costruzione elenchi di idDoc={}",
            ...
            // documento
            logger.debug("{} Metto a null tiStatoUdElencoVers idDoc={}", LOG_MESSAGE_ANNULLA_UD,
                    doc.getIdDoc());
        ...
        /*
         * il sistema determina gli aggiornamenti unità doc degli item (della richiesta corrente) di
         * tipo UNI_DOC con stato DA_ANNULLARE_IN_SACER, il cui stato relativo al processo di
         * inclusione in un elenco vale IN_ATTESA_SCHED, NON_SELEZ_SCHED
         */
        ...
        logger.debug("{} - trovati {} AroUpdUnitaDoc", LOG_MESSAGE_ANNULLA_UD,
                aroUpdUnitaDocList.size());
        for (AroUpdUnitaDoc aroUpdUnitaDoc : aroUpdUnitaDocList) {
            ...
            // Cancello l'ud dalla coda di costruzione elenchi
            logger.debug(
                    "{} Procedo alla cancellazione dalla coda di costruzione elenchi di idUpdUnitaDoc={}",
                    LOG_MESSAGE_ANNULLA_UD, aroUpdUnitaDoc.getIdUpdUnitaDoc());
            helper.deleteElvUpdUdDaElabElenco(aroUpdUnitaDoc.getIdUpdUnitaDoc());
            // Annullo lo stato relativo al processo di inclusione in un elenco dell'ud
            logger.debug("{} Metto a null tiStatoUpdElencoVers idUpdUnitaDoc={}",
                    LOG_MESSAGE_ANNULLA_UD, aroUpdUnitaDoc.getIdUpdUnitaDoc());
            aroUpdUnitaDoc.setTiStatoUpdElencoVers(null);
        }

        logger.debug("{} - Gestione degli item della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        /*
         * Ricavo gli item di tipo UNI_DOC con stato DA_ANNULLARE_IN_SACER il cui stato relativo al
         * processo di inclusione in un elenco vale IN_ELENCO_APERTO o IN_ELENCO_DA_CHIUDERE
         */
        ...
        logger.debug("{} - trovati {} AroItemRichAnnulVers", LOG_MESSAGE_ANNULLA_UD,
                item2List.size());
        for (AroItemRichAnnulVers aroItemRichAnnulVers : item2List) {
            ...
            if (aroItemRichAnnulVers.getAroUnitaDoc().getElvElencoVer() == null) {
                ...
                idElencoVersApertiODaChiudereSet.add(
                        aroItemRichAnnulVers.getAroUnitaDoc().getElvElencoVer().getIdElencoVers());
                ...
                logger.debug(
                        "{} - cancello idUnitaDoc {} dall'elenco: metto a null ElvElencoVer e tiStatoUdElencoVers",
                        LOG_MESSAGE_ANNULLA_UD,
                        aroItemRichAnnulVers.getAroUnitaDoc().getIdUnitaDoc());
        ...
        logger.debug(
                "{} - Gestione dei documenti aggiunti degli item della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        /*
         * Ricavo i documenti aggiunti degli item di tipo UNI_DOC (quindi sto considerando l'item
         * essere un'unit\u00E0 doc e ne cerco i documenti aggiunti) con stato DA_ANNULLARE_IN_SACER
         * il cui stato relativo al processo di inclusione in un elenco vale IN_ELENCO_APÈRTO
         */
        docList = helper.getDocAggiunti(idRichAnnulVers,
                ElencoEnums.DocStatusEnum.IN_ELENCO_APERTO.name(),
        ...
        logger.debug("{} - Ricalcolo totali elenco della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        ...
        logger.debug("{} - Gestione elenchi della richiesta avente id: {}", LOG_MESSAGE_ANNULLA_UD,
                idRichAnnulVers);
        /*
         * Il sistema determina gli elenchi con stato = CHIUSO o VALIDATO a cui appartengono gli
         * item (della richiesta corrente) di tipo UNI_DOC con stato DA_ANNULLARE_IN_SACER, il cui
         * stato relativo al processo di inclusione in un elenco vale IN_ELENCO_CHIUSO o
         * IN_ELENCO_VALIDATO, uniti agli elenchi a cui appartengono i documenti aggiunti degli item
         * (della richiesta corrente) di tipo UNI_DOC con stato DA_ANNULLARE_IN_SACER, il cui stato
         * relativo al processo di inclusione in un elenco vale IN_ELENCO_CHIUSO o
         * IN_ELENCO_VALIDATO
         */
        List<ElvVLisElencoUdAnnul> elenchiItem = helper
                .retrieveElvVLisElencoUdAnnuls(idRichAnnulVers);
        for (ElvVLisElencoUdAnnul elencoItem : elenchiItem) {
            ...
            if (StringUtils.isBlank(elencoItem.getNtElencoChiuso())
                    || !elencoItem.getNtElencoChiuso().contains(
            ...
            List<ElvVLisUdAnnulByElenco> uds = helper.retrieveElvVLisUdAnnulByElenco(
                    idRichAnnulVers,
        ...
        logger.debug("{} - Modifica ud, doc e collegamenti della richiesta avente id: {}",
                LOG_MESSAGE_ANNULLA_UD, idRichAnnulVers);
        ...
        for (Long idUnitaDoc : idUnitaDocList) {
            udEjb.insertLogStatoConservUd(idUnitaDoc, utente.getNmUserid(),
                    ...
                    CostantiDB.StatoConservazioneUnitaDoc.ANNULLATA.name(), modalita);
        }
        // end MEV #31162
        ...
        AroStatoRichAnnulVers statoRichAnnulVersNew = context.getBusinessObject(AnnulVersEjb.class)
                .createAroStatoRichAnnulVers(richiestaAnnullamento,
        ...
        /*
         * Aggiorna il fascicolo settando l’indicatore che segnala che il fascicolo deve essere
         * aggiornato a causa di annullamento di almeno una unità documentaria
         */
        List<BigDecimal> idFascicoli = fascicoliHelper
                .retrieveFasVLisFascByRichann(idRichAnnulVers);
    ...
    public void evadiAnnullamentoVersamentiFascicoli(AroRichAnnulVers richiestaAnnullamento,
        ...
        logger.debug(
                ...
                idRichAnnulVers);
        /*
         * Ricavo gli item (della richiesta corrente) di tipo FASC con stato DA_ANNULLARE_IN_SACER,
         * il cui stato relativo al processo di inclusione in un elenco vale IN_ATTESA_SCHED,
         * NON_SELEZ_SCHED
         */
        ...
        logger.debug(
                ...
                idRichAnnulVers);
        /*
         * Ricavo gli item di tipo FASC con stato DA_ANNULLARE_IN_SACER il cui stato relativo al
         * processo di inclusione in un elenco vale IN_ELENCO_APERTO o IN_ELENCO_DA_CHIUDERE
         */
        List<AroItemRichAnnulVers> item2List = helper.getItemFasc(idRichAnnulVers,
                TiStatoFascElencoVers.IN_ELENCO_APERTO,
        ...
        for (Long idElencoVersFascAperto : idElencoVersFascApertiODaChiudereSet) {
            context.getBusinessObject(AnnulVersEjb.class).updateTotaliElencoFasc(
                    idElencoVersFascAperto, statoRichAnnulVers.getIamUser().getIdUserIam(),
                    dtString);
        }
        ...
        logger.debug(
                ...
                idRichAnnulVers);
        /*
         * Il sistema determina gli elenchi con stato = CHIUSO o FIRMA_IN_CORSO o FIRMATO o
         * IN_CODA_CREAZIONE_AIP o AIP_CREATI o ELENCO_INDICI_AIP_CREATO o
         * ELENCO_INDICI_AIP_FIRMA_IN_CORSO o COMPLETATO, a cui appartengono gli item (della
         * richiesta corrente) di tipo FASC con stato DA_ANNULLARE_IN_SACER, il cui stato relativo
         * al processo di inclusione in un elenco vale IN_ELENCO_CHIUSO o IN_ELENCO_FIRMATO o
         * IN_ELENCO_IN_CODA_CREAZIONE_AIP o IN_ELENCO_CON_AIP_CREATO o
         * IN_ELENCO_CON_ELENCO_INDICI_AIP_CREATO o IN_ELENCO_COMPLETATO (vedi vista
         * ELV_V_LIS_ELENCO_FASC_ANNUL)
         */
        List<ElvVLisElencoFascAnnul> elenchiItem = helper
                .retrieveElvVLisElencoFascAnnul(idRichAnnulVers);
        for (ElvVLisElencoFascAnnul elencoItem : elenchiItem) {
            ...
            if (StringUtils.isBlank(elencoItem.getNtElencoChiuso())
                    || !elencoItem.getNtElencoChiuso().contains(
                            "Nell'elenco sono presenti fascicoli di cui \u00E8 stato annullato il versamento")) {
                elenco.setNtElencoChiuso(((StringUtils.isNotBlank(elencoItem.getNtElencoChiuso())
                        ? elencoItem.getNtElencoChiuso() + " "
                        : "")
                        + "Nell'elenco sono presenti fascicoli di cui \u00E8 stato annullato il versamento")
            ...
            List<ElvVLisFascAnnulByElenco> fascAnnulInElenco = helper
                    .retrieveElvVLisFascAnnulByElenco(idRichAnnulVers, elencoItem
                            .getElvVLisElencoFascAnnulId().getIdElencoVersFasc().longValue());
        ...
        /*
         * Il sistema registra un record in FAS_STATO_CONSERV_FASCICOLO per ogni fascicolo
         * corrispondente agli item (della richiesta corrente) con stato = DA_ANNULLARE_IN_SACER
         */
        List<AroItemRichAnnulVers> item3List = helper.getItemFasc(idRichAnnulVers);
        for (AroItemRichAnnulVers item : item3List) {
            context.getBusinessObject(AnnulVersEjb.class).createFasStatoConservFascicolo(
                    item.getFasFascicolo(), dataAnnullamento,
                    it.eng.parer.entity.constraint.FasStatoConservFascicolo.TiStatoConservazione.ANNULLATO,
                    statoRichAnnulVers.getIamUser());
        }

        /*
         * Modifico i fascicoli corrispondenti agli item (della richiesta corrente) con stato =
         * DA_ANNULLARE_IN_SACER, assegnando data di annullamento, stato conservazione = ANNULLATA,
         * note su annullamento = causale definita dalla richiesta
         */
        logger.debug(
        ...
        logger.debug(
                "Annullamento Versamenti Fascicoli - richiesta avente id: {} elaborata con successo!",
                idRichAnnulVers);
        logger.info("Fine evasione richiesta ID: {}", idRichAnnulVers);
    }
    ...
    public void updateTotaliElenco(long idElencoVers, long idUserIam, String dtString) {
        ...
        if (niUnitaDocVersElenco == 0 && niDocAggElenco == 0 && niUpdUnitaDoc == 0) {
            ...
            elenco.setNiSizeVersElenco(
                    objVers[1] != null ? (BigDecimal) objVers[1] : BigDecimal.ZERO);
    ...
    public AroStatoRichAnnulVers createAroStatoRichAnnulVers(AroRichAnnulVers richAnnulVers,
            String tiStatoRichAnnulVers, Date dtRegStatoRichAnnulVers, String dsNotaRichAnnulVers,
            IamUser iamUser) {
        AroStatoRichAnnulVers statoRichAnnulVers = new AroStatoRichAnnulVers();
        statoRichAnnulVers.setPgStatoRichAnnulVers(
                helper.getUltimoProgressivoStatoRichiesta(richAnnulVers.getIdRichAnnulVers())
                        .add(BigDecimal.ONE));
    ...
    public AroStatoRichAnnulVers insertAroStatoRichAnnulVers(AroRichAnnulVers richAnnulVers,
            ...
            long idUser) {
        AroStatoRichAnnulVers statoRichAnnulVers = new AroStatoRichAnnulVers();
        logger.info("Eseguo il salvataggio dello stato richiesta annullamento pari a {}",
                tiStatoRichAnnulVers);
        statoRichAnnulVers.setPgStatoRichAnnulVers(
                helper.getUltimoProgressivoStatoRichiesta(richAnnulVers.getIdRichAnnulVers())
                        .add(BigDecimal.ONE));
        statoRichAnnulVers.setTiStatoRichAnnulVers(tiStatoRichAnnulVers);
        ...
        richAnnulVers.addAroStatoRichAnnulVers(statoRichAnnulVers);
        helper.insertEntity(statoRichAnnulVers, true);
        richAnnulVers.setIdStatoRichAnnulVersCor(
                BigDecimal.valueOf(statoRichAnnulVers.getIdStatoRichAnnulVers()));
     ...
     * @return tablebean
     */
    public AroVRicRichAnnvrsTableBean getAroVRicRichAnnvrsTableBean(long idUser,
            RicercaRichAnnulVersBean filtri) {
            ...
            try {
                for (AroVRicRichAnnvrs aroVRicRichAnnvrs : list) {
                    ...
                    row.setString("amb_ente_strut",
                            aroVRicRichAnnvrs.getNmAmbiente() + " - "
                                    + aroVRicRichAnnvrs.getNmEnte() + " - "
                                    + aroVRicRichAnnvrs.getNmStrut());
                    table.add(row);
                }
                ...
                logger.error(
                        "Errore durante il recupero delle richieste di annullamento versamento "
                                + ExceptionUtils.getRootCauseMessage(ex),
                        ex);
            }
        }
     ...
     *
     * @param idRichAnnulVers id della richiesta
     *
    ...
    public AroVLisItemRichAnnvrsTableBean getAroVLisItemRichAnnvrsTableBean(
            BigDecimal idRichAnnulVers) {
    ...
    public AroVLisStatoRichAnnvrsTableBean getAroVLisStatoRichAnnvrsTableBean(
            BigDecimal idRichAnnulVers) {
     ...
     *
     * @param idRichAnnulVers id della richiesta
     * @param idUserIam       utente corrente
     *
     * @throws ParerUserError Errore imprevisto
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void controlloItemOnline(BigDecimal idRichAnnulVers, long idUserIam)
            throws ParerUserError {
     ...
     * Esegue il controllo degli item della richiesta data come parametro
     *
     * @param richAnnulVers richiesta annullamento versamento
     * @param idUserIam     utente corrente
     *
     ...
     *
     * @param idRichAnnulVers id della richiesta
     * @param idUserIam       id utente presente nell'xml di richiesta
     * @param isFromPreIngest true o false a seconda che la richiesta provenga o meno da PreIngest
     *
     * @throws it.eng.parer.exception.ParerUserError Errore imprevisto
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void controlloItemWsRichiestaAnnul(BigDecimal idRichAnnulVers, long idUserIam,
            boolean isFromPreIngest) throws ParerUserError {
        // Assumo lock esclusivo sulla richiesta
     ...
     *
     * @param idRichAnnulVers id della richiesta
     * @param cdRichAnnulVers nuovo valore del campo cdRichAnnulVers
     * @param dsRichAnnulVers nuovo valore del campo dsRichAnnulVers
     * @param ntRichAnnulVers nuovo valore del campo ntRichAnnulVers
     * @param flForzaAnnul    flag 1/0 (true/false)
     * @param idStrut         struttura di riferimento della richiesta
     *
     * @throws ParerUserError eccezione generazione
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveRichAnnulVers(BigDecimal idRichAnnulVers, String cdRichAnnulVers,
            String dsRichAnnulVers, String ntRichAnnulVers, String flForzaAnnul, BigDecimal idStrut)
            throws ParerUserError {
        try {
            AroRichAnnulVers richAnnulVers = helper.findByIdWithLock(AroRichAnnulVers.class,
                    idRichAnnulVers);
            ...
            if (!statoCorrente.getTiStatoRichAnnulVers()
                    .equals(CostantiDB.StatoRichAnnulVers.APERTA.name())) {
                throw new ParerUserError(
    ...
    /**
     * Esegue la modifica della richiesta di annullamento versamenti, aggiungendo le ud da un file
     * caricato
     *
     * @param idRichAnnulVers id richiesta annullamento
    ...
    public void saveRichAnnulVers(BigDecimal idRichAnnulVers, byte[] fileByteArray, long idUserIam,
        ...
        try {
            AroRichAnnulVers richAnnulVers = helper.findByIdWithLock(AroRichAnnulVers.class,
                    idRichAnnulVers);
            richAnnulVers.setTiCreazioneRichAnnulVers(
                    CostantiDB.TipoCreazioneRichAnnulVers.UPLOAD_FILE.name());
            if (fileByteArray != null) {
                if (tiRichAnnulVers.equals("UNITA_DOC")) {
                    AroFileRichAnnulVers fileRich = new AroFileRichAnnulVers();
                    fileRich.setTiFile(CostantiDB.TipoFileRichAnnulVers.FILE_UD_ANNUL.name());
                    ...
                    context.getBusinessObject(AnnulVersEjb.class).handleCsvRecords(richAnnulVers,
                            fileByteArray, idUserIam);
                } else if (tiRichAnnulVers.equals("FASCICOLI")) {
                    AroFileRichAnnulVers fileRich = new AroFileRichAnnulVers();
                    fileRich.setTiFile(CostantiDB.TipoFileRichAnnulVers.FILE_FASC_ANNUL.name());
                    ...
                    context.getBusinessObject(AnnulVersEjb.class)
                            .handleCsvRecordsFasc(richAnnulVers, fileByteArray, idUserIam);
                }
            }
        } catch (ParerUserError ex) {
            throw ex;
    ...
    public void deleteRichAnnulVers(BigDecimal idRichAnnulVers) throws ParerUserError {
        ...
        if (!statoCorrente.getTiStatoRichAnnulVers()
                .equals(CostantiDB.StatoRichAnnulVers.APERTA.name())
                && !statoCorrente.getTiStatoRichAnnulVers()
                        .equals(CostantiDB.StatoRichAnnulVers.CHIUSA.name())) {
            throw new ParerUserError(
     ...
     *
     * @param idRichAnnulVers     id della richiesta
     * @param idItemRichAnnulVers id dell'item da eliminare
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteItemRichAnnulVers(BigDecimal idRichAnnulVers, BigDecimal idItemRichAnnulVers)
        ...
        AroItemRichAnnulVers item = helper.findById(AroItemRichAnnulVers.class,
                idItemRichAnnulVers);
     ...
     * @param idRichAnnulVers         id della richiesta
     * @param tiStatoRichAnnulVersOld stato attuale della richiesta
     * @param tiStatoRichAnnulVersNew stato da assumere
     * @param dsNotaRichAnnulVers     nota utente
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void cambiaStato(long idUserIam, BigDecimal idRichAnnulVers,
            String tiStatoRichAnnulVersOld, String tiStatoRichAnnulVersNew,
            String dsNotaRichAnnulVers) throws ParerUserError {
        AroRichAnnulVers richAnnulVers = helper.findByIdWithLock(AroRichAnnulVers.class,
                idRichAnnulVers);
        AroStatoRichAnnulVers statoCorrente = helper.findById(AroStatoRichAnnulVers.class,
                richAnnulVers.getIdStatoRichAnnulVersCor());
        ...
        AroStatoRichAnnulVers statoRichAnnulVers = context.getBusinessObject(AnnulVersEjb.class)
                .createAroStatoRichAnnulVers(richAnnulVers, tiStatoRichAnnulVersNew,
                        Calendar.getInstance().getTime(), dsNotaRichAnnulVers, iamUser);
    ...
    public boolean isUdInRichAnnulVers(BigDecimal idUnitaDoc) {
        boolean result = false;
        AroRichAnnulVers existingRich = helper
                .getAroRichAnnulVersContainingUd(idUnitaDoc.longValue(), null);
        if (existingRich != null) {
            result = true;
        }
        return result;
    ...
    public boolean isFascInRichAnnulVers(BigDecimal idFascicolo) {
        boolean result = false;
        AroRichAnnulVers existingRich = helper
                .getAroRichAnnulVersContainingFasc(idFascicolo.longValue(), null);
        if (existingRich != null) {
            result = true;
        }
        return result;
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void addUnitaDocToRichAnnulVers(BigDecimal idRichAnnulVers, String registro,
            BigDecimal anno, String numero, int progressivo, long idUserIam) throws ParerUserError {
        AroRichAnnulVers rich = helper.findByIdWithLock(AroRichAnnulVers.class, idRichAnnulVers);
        BigDecimal idStrut = new BigDecimal(rich.getOrgStrut().getIdStrut());
        ...
        if (idUnitaDoc != null) {
            ...
            throw new ParerUserError(
                    "Errore inaspettato nell'inserimento dell'unit\u00E0 documentaria alla richiesta: Impossibile recuperare l'unit\u00E0 documentaria "
                            + registro + "-" + anno.toPlainString() + "-" + numero
                            + " per la struttura " + rich.getOrgStrut().getNmStrut());
        }
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void addFascicoloToRichAnnulVers(BigDecimal idRichAnnulVers, BigDecimal anno,
            String numero, int progressivo, long idUserIam) throws ParerUserError {
        AroRichAnnulVers rich = helper.findByIdWithLock(AroRichAnnulVers.class, idRichAnnulVers);
        BigDecimal idStrut = new BigDecimal(rich.getOrgStrut().getIdStrut());
    ...
    public void saveStatoRichAnnulVers(BigDecimal idStatoRichAnnulVers, String dsNotaRichAnnulVers)
        ...
        try {
            AroStatoRichAnnulVers stato = helper.findById(AroStatoRichAnnulVers.class,
                    idStatoRichAnnulVers);
    ...
    public String marshallaEsitoRichiestaAnnullamentoVersamenti(
            EsitoRichiestaAnnullamentoVersamenti esito) throws JAXBException {
        StringWriter sw = new StringWriter();
        Marshaller marshaller = xmlContextCache
                .getEsitoAnnVersCtx_EsitoRichiestaAnnullamentoVersamenti().createMarshaller();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public FasStatoConservFascicolo createFasStatoConservFascicolo(FasFascicolo fascicolo,
            Date data, TiStatoConservazione tiStatoConservazione, IamUser iamUser) {

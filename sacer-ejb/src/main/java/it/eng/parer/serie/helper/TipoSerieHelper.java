...
public class TipoSerieHelper extends GenericHelper {
	...
	if (filterValid) {
	    queryStr.append(
		    "AND tipoSerie.dtIstituz <= :filterDate AND tipoSerie.dtSoppres >= :filterDate ");
	}
	...
	query.setParameter("idStrut", longFromBigDecimal(idStrut));
	query.setParameter("flTipoSeriePadre",
		flTipoSeriePadre ? JobConstants.DB_TRUE : JobConstants.DB_FALSE);
	if (filterValids) {
	    query.setParameter("filterDate", Calendar.getInstance().getTime());
	}
	if (StringUtils.isNotBlank(tipoContenSerie)) {
    ...
    public List<DecTipoSerie> getDecTipoSerie(long idUser, BigDecimal idAmbiente, BigDecimal idEnte,
	    BigDecimal idStrut, String isAttivo, String tipiSerieNoGenModello,
	...
	StringBuilder queryStr = new StringBuilder("SELECT tipoSerie FROM DecTipoSerie tipoSerie "
		...
		+ "WHERE strut.idStrut = abilOrganiz.idOrganizApplic "
	...
	if (isAttivo != null) {
		...
		queryStr.append(
			"AND (tipoSerie.dtIstituz <= :dataOdierna AND tipoSerie.dtSoppres >= :dataOdierna) ");
	    } else {
		queryStr.append(
			"AND (tipoSerie.dtIstituz > :dataOdierna OR tipoSerie.dtSoppres < :dataOdierna) ");
	    }
	}
	...
	if (idModelloTipoSerie != null) {
	    queryStr.append(
		    "AND tipoSerie.decModelloTipoSerie.idModelloTipoSerie = :idModelloTipoSerie ");
	}
    ...
    public DecTipoUnitaDoc getDecTipoUnitaDocByIdRegistroIdSerie(long idRegistroUnitaDoc,
    ...
    public List<DecCampoInpUd> getDecCampoInpUdPerTipoSerie(BigDecimal idTipoSerie,
	    String ti_campo) {
	List<DecCampoInpUd> result = null;
	StringBuilder sQuery = new StringBuilder();
	sQuery.append(
		"Select inp from DecCampoInpUd inp where inp.decTipoSerie.idTipoSerie=:idTipoSerie");
	if (ti_campo != null) {
	...
	Query query = getEntityManager().createQuery(
		"Select sser from SerSerie sser where sser.decTipoSerie.idTipoSerie=:idTipoSerie ");
	query.setParameter("idTipoSerie", longFromBigDecimal(idTipoSerie));
	return query.getResultList();
    }
    ...
    public List<DecTipoSerieUd> getDecTipoSerieUd(BigDecimal idTipoSerie,
    ...
    public List<DecFiltroSelUd> getDecFiltroSelUdList(BigDecimal idTipoSerie,
	...
	DecTipoSerieUd decTipoSerieUd = getEntityManager().find(DecTipoSerieUd.class,
		idTipoSerieUd);

	// Se il criterio già esisteva, aggiorno i dati specifici che potrebbero essere stati
	// modificati
	...
	try {
		...
		DecFiltroSelUdAttb dcds = (DecFiltroSelUdAttb) Transform
			.rowBean2Entity(rigaDecFiltroSelUdAttb);
		...
		for (DecFiltroSelUdAttbBean datoSpecBean : listaDatiSpecOnLine) {
			...
			for (DecFiltroSelUdDatoBean dcab : dcabList) {
			    ...
			    DecAttribDatiSpec dads = getEntityManager()
				    .find(DecAttribDatiSpec.class, id);
	...
	String sQuery = "Select u FROM DecFiltroSelUdAttb u "
		+ "WHERE u.decTipoSerieUd.idTipoSerieUd = :idTipoSerieUd";
	Query query = getEntityManager().createQuery(sQuery);
    ...
    public List<DecCampoOutSelUd> getDecCampoOutSelUdPerDecOutSelUd(BigDecimal idOutSelUd,
	    String ti_campo) {
	List<DecCampoOutSelUd> result = null;
	StringBuilder sQuery = new StringBuilder();
	sQuery.append(
		"Select out from DecCampoOutSelUd out where out.decOutSelUd.idOutSelUd=:idOutSelUd");
	if (ti_campo != null) {
    ...
    public List<DecCampoOutSelUd> getDecCampoOutSelUd(BigDecimal idOutSelUd, String nmCampo,
	    String tiCampo) {
	...
	if (idOutSelUd != null && nmCampo != null && tiCampo != null) {
	    ...
	    sQuery.append("SELECT campoOutSelUd FROM DecCampoOutSelUd campoOutSelUd "
		    ...
		    + "AND campoOutSelUd.nmCampo = :nmCampo "
		    + "AND campoOutSelUd.tiCampo = :tiCampo ");
    ...
    public DecAttribDatiSpec getDecAttribDatiSpecByName(String nmAttribDatiSpec,
	...
	try {
		...
		for (DecAttribDatiSpecRowBean row : listaDatiSpecTableBean) {
		    String versioniXsd = getVersioniXsd(row.getIdAttribDatiSpec(), id,
			    tipoEntitaSacer);
    ...
    public List<DecAttribDatiSpec> getDecAttribDatiSpec(BigDecimal id,
	    Constants.TipoEntitaSacer tipoEntitaSacer) {
	...
	Query query = getEntityManager()
		.createQuery("delete from DecOutSelUd u where u.idOutSelUd=:idOutSelUd");
	query.setParameter("idOutSelUd", longFromBigDecimal(idOutSelUd));
    ...
    /**
     * Questa versione del metodo di cancellazione campo nelle regole di rappresentazione necessita
     * anche di "aggiornare" il campo stringa "dl_formato_out" della regola di rappresentazione
     * stessa. Se non ho più valori nel suddetto campo, elimino anche il record della regola di
     * rappresentazione.
     *
     * @param idOutSelUd id unita doc
     * @param tipoCampo  tipo camp
     *
	...
	// Cancello il campo
	int deleted = -1;
	String queryStr = "DELETE FROM DecCampoOutSelUd u "
		+ "WHERE u.decOutSelUd.idOutSelUd = :idOutSelUd ";
	if (tipoCampo != null) {
	...
	Query query = getEntityManager().createQuery(
		"Select out from DecOutSelUd out where out.idOutSelUd<>:idOutSelUd and out.tiOut=:tiOut");
	query.setParameter("idOutSelUd", longFromBigDecimal(idOutSelUd));
    ...
    public DecAttribDatiSpecTableBean getDecAttribDatiSpecTableBean(
	    Constants.TipoEntitaSacer tipoEntitaSacer, List<Long> id) {
    ...
    public List<DecAttribDatiSpec> getDecAttribDatiSpecs(Constants.TipoEntitaSacer tipoEntitaSacer,
	    List<Long> id) {
	String tmpTipoEntita = null;
    ...
    public BigDecimal getMaxPgPerTipoFiltro(CostantiDB.TipoFiltroSerieUd tipoFiltro,
	...
	int deleted = -1;
	Query query = getEntityManager().createQuery(
		"delete from DecCampoInpUd u where u.decTipoSerie.idTipoSerie=:idTipoSerie");
	query.setParameter("idTipoSerie", longFromBigDecimal(idTipoSerie));
	deleted = query.executeUpdate();
	getEntityManager().flush();
	return deleted;
	...
	Query query = getEntityManager().createQuery(
		"Select u from DecNotaTipoSerie u where u.decTipoSerie.idTipoSerie=:idTipoSerie");
	query.setParameter("idTipoSerie", longFromBigDecimal(idTipoSerie));
	result = query.getResultList();
	return result;
    }
	...
	int deleted = -1;
	Query query = getEntityManager().createQuery(
		"Delete from DecNotaTipoSerie u where u.idNotaTipoSerie=:idNotaTipoSerie");
	...
	Query query = getEntityManager().createQuery(
		"Select tns from DecTipoNotaSerie tns where tns.flMolt='1' order by tns.cdTipoNotaSerie");
	result = query.getResultList();
	return result;
    }
    ...
    public List<DecTipoNotaSerie> getDecTipoNotaSerieNotInModelloSerie(
	    BigDecimal idModelloTipoSerie) {
	Query query = getEntityManager().createQuery(
	...
	List<DecNotaTipoSerie> result = null;
	Query query = getEntityManager().createQuery("SELECT tns FROM DecNotaTipoSerie tns "
		+ "WHERE tns.decTipoSerie.idTipoSerie = :idTipoSerie "
	...
	String queryStr = String.format(
		"SELECT DISTINCT dec.cdVersioneXsd FROM DecXsdAttribDatiSpec decXsd "
			+ " JOIN decXsd.decXsdDatiSpec dec "
			+ " JOIN decXsd.decAttribDatiSpec decAttrib " + " WHERE %s = :id",
		tmpTipoEntita);
    ...
    public DecFiltroSelUd getFiltroSelUdByIdTipoDoc(BigDecimal idTipoDoc,
	    BigDecimal idTipoSerieUd) {
    ...
    public DecTipoDoc getDecTipoDocFromFiltroSelUdByIdTipoDoc(BigDecimal idTipoDoc,
	    BigDecimal idTipoSerieUd) {
	DecTipoDoc result = null;
	Query query = getEntityManager().createQuery("select f.decTipoDoc from DecFiltroSelUd f "
		+ "where f.decTipoDoc.idTipoDoc=:idTipoDoc "
		+ "and f.decTipoSerieUd.idTipoSerieUd=:idTipoSerieUd ");
	query.setParameter("idTipoDoc", longFromBigDecimal(idTipoDoc));
	query.setParameter("idTipoSerieUd", longFromBigDecimal(idTipoSerieUd));
	...
	Query q = getEntityManager().createQuery("DELETE FROM DecFiltroSelUdAttb u "
		+ "WHERE u.idFiltroSelUdAttb = :idFiltroSelUdAttb ");
	...
	Query q = getEntityManager().createQuery(
		"DELETE FROM DecCampoInpUd u " + "WHERE u.idCampoInpUd = :idCampoInpUd ");
	...
	Query query = getEntityManager()
		.createQuery("SELECT registro FROM DecRegistroUnitaDoc registro "
    ...
    public List<DecTipoSerie> retrieveDecTipoSerieForRegistro(BigDecimal idStrut,
    ...
    /**
     * Controlla se un determinato registro è presente, più di una volta, in qualunque tipo serie
     * attraverso la relazione col tipo ud. Verifica dunque che l'associazione Registro-TipoUd
     * (DecTipoSerieUd) sia presente, divisa per tipi serie, più di una volta.
     *
	...
	Query query = getEntityManager().createQuery("SELECT tipoSerie.idTipoSerie "
		...
		+ "WHERE registroUnitaDoc.idRegistroUnitaDoc = :idRegistroUnitaDoc "
		+ "GROUP BY tipoSerie " + "HAVING COUNT(registroUnitaDoc) > 1 ");
	query.setParameter("idRegistroUnitaDoc", longFromBigDecimal(idRegistroUnitaDoc));
	return !query.getResultList().isEmpty();
    }
	...
	StringBuilder queryStr = new StringBuilder(
		"SELECT COUNT(tipoSerieUd) FROM DecTipoSerieUd tipoSerieUd ");
	boolean isTipoDoc = false;
	switch (tipoDato) {
	case REGISTRO:
	    queryStr.append(
		    "WHERE tipoSerieUd.decRegistroUnitaDoc.idRegistroUnitaDoc = :idTipoDato ");
	    break;
	case TIPO_UNITA_DOC:
	...
	case TIPO_DOC:
	    queryStr = new StringBuilder(
		    "SELECT COUNT(filtroSelUd) FROM DecFiltroSelUd filtroSelUd ");

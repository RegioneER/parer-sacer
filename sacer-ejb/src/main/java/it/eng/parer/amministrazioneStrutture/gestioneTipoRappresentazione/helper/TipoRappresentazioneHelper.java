...
public class TipoRappresentazioneHelper extends GenericHelper {
    ...
    public List<DecTipoRapprComp> retrieveDecRapprCompList(BigDecimal idStrut,
            boolean filterValid) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT tipoRapprComp FROM DecTipoRapprComp tipoRapprComp ");
        String clause = " WHERE ";
        if (idStrut != null) {
        ...
        if (filterValid) {
            queryStr.append(clause).append(
                    " tipoRapprComp.dtIstituz <= :filterDate AND tipoRapprComp.dtSoppres >= :filterDate ");
        }
    ...
    public CSVersatore getCSVersatoreForImageTrasform(BigDecimal idImageTrasform) {
        ...
        String sQuery = "Select strut.nmStrut, ente.nmEnte, amb.nmAmbiente from DecImageTrasform dit "
                + " join dit.decTrasformTipoRappr ttr" + " join ttr.decTipoRapprComp dtrc"
                + " join dtrc.orgStrut strut" + " join strut.orgEnte ente"
                + " join ente.orgAmbiente amb" + " where dit.idImageTrasform=:idImageTrasform";
        Query query = getEntityManager().createQuery(sQuery);
    ...
    public CSVersatore getCSVersatoreForTrasformTipoRappr(BigDecimal idTrasformTipoRappr) {
        ...
        String sQuery = "Select strut.nmStrut, ente.nmEnte, amb.nmAmbiente from DecTrasformTipoRappr ttr"
                + " join ttr.decTipoRapprComp dtrc" + " join dtrc.orgStrut strut"
                + " join strut.orgEnte ente" + " join ente.orgAmbiente amb"
                + " where ttr.idTrasformTipoRappr=:idTrasformTipoRappr";
        Query query = getEntityManager().createQuery(sQuery);
        query.setParameter("idTrasformTipoRappr", longFromBigDecimal(idTrasformTipoRappr));
        List<Object[]> list = query.getResultList();
        if (list != null && !list.isEmpty()) {
    ...
    public DecImageTrasform getDecImageTrasformByName(BigDecimal idTrasformTipoRappr,
            String nmImageTrasform) {

...
public class TipoDocumentoEjb {
    ...
    public DecTipoDocTableBean getTipiDocPrincipaliAbilitati(long idUtente, BigDecimal idStruttura) {
        DecTipoDocTableBean table = new DecTipoDocTableBean();
    ...
    private DecTipoDocRowBean getDecTipoDoc(BigDecimal idTipoDoc, String nmTipoDoc, BigDecimal idStrut) {
    ...
    public DecTipoDocTableBean getDecTipoDocTableBean(BigDecimal idStrut, boolean flPrinc, Date dtSoppres) {
        ...
        List<DecTipoDoc> tipoDocList = helper.retrieveDecTipoDocList(idStrut, flPrinc, dtSoppres, false);
        if (tipoDocList != null && !tipoDocList.isEmpty()) {
            try {
                tipoDocTableBean = (DecTipoDocTableBean) Transform.entities2TableBean(tipoDocList);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException | IllegalAccessException
                    | IllegalArgumentException | InvocationTargetException ex) {
                logger.error("Errore durante il recupero dei tipi documento " + ExceptionUtils.getRootCauseMessage(ex),
                        ex);
    ...
    public DecTipoDocTableBean getDecTipoDocTableBean(BigDecimal idStrut, boolean flPrinc, boolean isFilterValid) {
        DecTipoDocTableBean tipoDocTableBean = new DecTipoDocTableBean();
        List<DecTipoDoc> tipoDocList = helper.retrieveDecTipoDocList(idStrut, flPrinc, null, isFilterValid);
        if (tipoDocList != null && !tipoDocList.isEmpty()) {
            try {
                tipoDocTableBean = (DecTipoDocTableBean) Transform.entities2TableBean(tipoDocList);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException | IllegalAccessException
    ...
    public DecTipoDocTableBean getDecTipoDocTableBean(BigDecimal idStrut, Date dtSoppres) {
        DecTipoDocTableBean tipoDocTableBean = new DecTipoDocTableBean();
        List<DecTipoDoc> tipoDocList = helper.retrieveDecTipoDocList(idStrut, false, dtSoppres, false);
        if (tipoDocList != null && !tipoDocList.isEmpty()) {
            try {
                tipoDocTableBean = (DecTipoDocTableBean) Transform.entities2TableBean(tipoDocList);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException | IllegalAccessException
        ...
        try {
            if (!list.isEmpty()) {
                for (DecTipoDoc tipo : list) {
                    DecTipoDocRowBean tipoDocRow = (DecTipoDocRowBean) Transform.entity2RowBean(tipo);
                    ...
                    DecXsdDatiSpec lastXsd = datiSpecHelper
                            .getLastDecXsdDatiSpecForTipoDoc(tipo.getOrgStrut().getIdStrut(), tipo.getIdTipoDoc());
                    if (lastXsd != null) {
                        tipoDocRow.setObject("cd_versione_xsd", lastXsd.getCdVersioneXsd());
                    }

                    String flagStandard = crHelper.getCriterioStandardPerTipoDatoAnno(tipo.getIdTipoDoc(),
                            TipoDato.TIPO_DOC);
    ...
    public void insertDecTipoDoc(LogParam param, DecTipoDocRowBean tipoDocRowBean, String criterioAutomTipoDoc)
            throws ParerUserError {
        TipoDocumentoEjb me = context.getBusinessObject(TipoDocumentoEjb.class);
        IamOrganizDaReplic replic = me.saveDecTipoDoc(param, null, tipoDocRowBean, StruttureEjb.TipoOper.INS,
                criterioAutomTipoDoc);
        if (replic != null) {
            struttureEjb.replicateToIam(replic);
        }
    }

    public void updateDecTipoDoc(LogParam param, BigDecimal idTipoDoc, DecTipoDocRowBean tipoDocRowBean,
            String criterioAutomTipoDoc) throws ParerUserError {
        TipoDocumentoEjb me = context.getBusinessObject(TipoDocumentoEjb.class);
        IamOrganizDaReplic replic = me.saveDecTipoDoc(param, idTipoDoc, tipoDocRowBean, StruttureEjb.TipoOper.MOD,
                criterioAutomTipoDoc);
        if (replic != null) {
            struttureEjb.replicateToIam(replic);
        }
    ...
    public IamOrganizDaReplic saveDecTipoDoc(LogParam param, BigDecimal idTipoDoc, DecTipoDocRowBean tipoDocRowBean,
            StruttureEjb.TipoOper tipoOper, String criterioAutomTipoDoc) throws ParerUserError {
        ...
        if (tipoOper.name().equals((StruttureEjb.TipoOper.INS.name()))) {
            ...
            if (helper.getDecTipoDocByName((tipoDocRowBean.getNmTipoDoc()), tipoDocRowBean.getIdStrut()) != null) {
                throw new ParerUserError("Tipo Documento gi\u00E0 esistente all'interno della struttura</br>");
            }
        ...
        } else /* MODIFICA TIPO DOCUMENTO */ if (tipoOper.name().equals((StruttureEjb.TipoOper.MOD.name()))) {
            ...
            if (tipoDocRowBean.getFlTipoDocPrincipale() == null || (tipoDocRowBean.getFlTipoDocPrincipale() != null
        ...
        if (criterioAutomTipoDoc != null && criterioAutomTipoDoc.equals("1")) {
            DecVCreaCritRaggrTipoDoc creaCritTipoDoc = crHelper.getDecVCreaCritRaggrTipoDoc(tipoDoc.getIdTipoDoc());
        ...
        /*
         * Il Tipo Documento va replicato solo se appartiere ad una struttura non appartenente ad un ente di tipo
         * template e sono stati modificati il nome e/o la descrizione
         */
    ...
    public IamOrganizDaReplic deleteTipoDocFromStruttura(LogParam param, long idTipoDoc) throws ParerUserError {
        return eseguiDeleteTipoDoc(param, idTipoDoc, true);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamOrganizDaReplic deleteTipoDocPuntuale(LogParam param, long idTipoDoc) throws ParerUserError {
        return eseguiDeleteTipoDoc(param, idTipoDoc, false);
    }

    /**
     * Business method per la cancellazione di un tipo documento di una determinata struttura. Il metodo esegue le
     * seguenti istruzioni: 1) Controlla i legami con Tipologia Serie e Unità documentarie; 2) Controllo i legami con
     * criteri di raggruppamento e, se può, li elimina 3) Esegue la cancellazione del Tipo Doc con cascade 4) Scrive il
     ...
     *             errore generico
     */
    private IamOrganizDaReplic eseguiDeleteTipoDoc(LogParam param, long idTipoDoc, boolean isFromDeleteStruttura)
            throws ParerUserError {
        ...
        boolean existsRelationsWithUnitaDoc = unitaDocHelper.existsRelationsWithUnitaDoc(tipoDoc.getIdTipoDoc(),
                Constants.TipoDato.TIPO_DOC);
        ...
        boolean existsRelationsWithModello = modelliSerieHelper.existsRelationsWithModello(tipoDoc.getIdTipoDoc(),
                Constants.TipoDato.TIPO_DOC);
        if (existsRelationsWithModello) {
            throw new ParerUserError("Il tipo di documento è definito come filtro su almeno un modello</br>");
        }
        ...
        List<ObjectsToLogBefore> listaBefore = sacerLogEjb.logBefore(param.getTransactionLogContext(),
                ...
                SacerLogConstants.TIPO_OGGETTO_TIPO_DOCUMENTO, new BigDecimal(idTipoDoc), param.getNomePagina());
        ...
        List<DecCriterioFiltroMultiplo> criteriAssociati = struttureHelper
                .getRelationsWithCriteriRaggruppamento(tipoDoc.getIdTipoDoc(), Constants.TipoDato.TIPO_DOC);
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_TIPO_DOCUMENTO, new BigDecimal(idTipoDoc),
                param.getNomePagina());
    ...
    public DecTipoStrutDocAmmessoTableBean getDecTipoStrutDocAmmessoTableBeanByIdTipoDoc(BigDecimal idTipoDoc) {
    ...
    public DecTipoStrutDocTableBean getDecTipoStrutDocTableBeanByIdStrut(BigDecimal idStrut, Date data) {
        DecTipoStrutDocTableBean tipoStrutDocTableBean = new DecTipoStrutDocTableBean();
        List<DecTipoStrutDoc> tipoStrutDocList = helper.getDecTipoStrutDocListByIdStrut(idStrut.longValue(), data);
        if (tipoStrutDocList != null && !tipoStrutDocList.isEmpty()) {
            try {
                tipoStrutDocTableBean = (DecTipoStrutDocTableBean) Transform.entities2TableBean(tipoStrutDocList);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException | IllegalAccessException
                    | IllegalArgumentException | InvocationTargetException ex) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void insertDecTipoStrutDocAmmesso(LogParam param, BigDecimal idTipoDoc, BigDecimal idTipoStrutDoc)
            throws ParerUserError {
        if (helper.getDecTipoStrutDocAmmessoByParentId(idTipoDoc.longValue(), idTipoStrutDoc.longValue()) != null) {
        ...
        try {
            ...
            DecTipoStrutDoc tipoStrutDoc = helper.findById(DecTipoStrutDoc.class, idTipoStrutDoc.longValue());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateDecTipoStrutDocAmmessoFromTipoDoc(LogParam param, BigDecimal idTipoStrutDocAmmesso,
            BigDecimal idTipoDoc, BigDecimal idTipoStrutDoc) throws ParerUserError {
        updateDecTipoStrutDocAmmesso(param, idTipoStrutDocAmmesso, idTipoDoc, idTipoStrutDoc,
                "Nome tipo struttura documento ammesso");
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateDecTipoStrutDocAmmessoFromTipoStrutDoc(LogParam param, BigDecimal idTipoStrutDocAmmesso,
            BigDecimal idTipoDoc, BigDecimal idTipoStrutDoc) throws ParerUserError {
        updateDecTipoStrutDocAmmesso(param, idTipoStrutDocAmmesso, idTipoDoc, idTipoStrutDoc,
                "Nome tipo documento ammesso");
    }

    private void updateDecTipoStrutDocAmmesso(LogParam param, BigDecimal idTipoStrutDocAmmesso, BigDecimal idTipoDoc,
            BigDecimal idTipoStrutDoc, String origin) throws ParerUserError {
        DecTipoStrutDocAmmesso dbTipoStrutDocAmmesso = helper.getDecTipoStrutDocAmmessoByParentId(idTipoDoc.longValue(),
        ...
        if (dbTipoStrutDocAmmesso != null) {
            ...
            if (dbTipoStrutDocAmmesso.getIdTipoStrutDocAmmesso() != idTipoStrutDocAmmesso.longValue()) {
                throw new ParerUserError(
                        origin + " gi\u00E0 associato a questo documento all'interno del database</br>");
            }
        } else {
            dbTipoStrutDocAmmesso = helper.findById(DecTipoStrutDocAmmesso.class, idTipoStrutDocAmmesso);
        }
        boolean update = false;
        if (dbTipoStrutDocAmmesso.getDecTipoDoc().getIdTipoDoc() != idTipoDoc.longValue()
                || dbTipoStrutDocAmmesso.getDecTipoStrutDoc().getIdTipoStrutDoc() != idTipoStrutDoc.longValue()) {
            update = true;
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteDecTipoStrutDocAmmesso(LogParam param, BigDecimal idTipoStrutDocAmmesso, BigDecimal idTipoDoc) {
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_TIPO_DOCUMENTO, idTipoDoc, param.getNomePagina());
    }

    public DecTipoStrutDocAmmessoRowBean getDecTipoStrutDocAmmessoRowBean(BigDecimal idTipoDoc,
            BigDecimal idTipoStrutDoc) {
        DecTipoStrutDocAmmesso tipoStrutDocAmmesso = helper.getDecTipoStrutDocAmmessoByParentId(idTipoDoc.longValue(),
                idTipoStrutDoc.longValue());
    ...
    public DecTipoStrutDocAmmessoTableBean getDecTipoDocAmmessoTableBeanByIdTipoStrutDoc(BigDecimal idTipoStrutDoc) {
        DecTipoStrutDocAmmessoTableBean tipoStrutDocAmmessoTableBean = new DecTipoStrutDocAmmessoTableBean();
        List<DecTipoStrutDocAmmesso> tipoStrutDocAmmessoList = helper
                .getDecTipoStrutDocAmmessoListByIdTipoStrutDoc(idTipoStrutDoc.longValue());
        if (tipoStrutDocAmmessoList != null && !tipoStrutDocAmmessoList.isEmpty()) {

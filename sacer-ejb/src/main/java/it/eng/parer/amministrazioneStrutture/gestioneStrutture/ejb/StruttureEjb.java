...
public class StruttureEjb {
    ...
    public byte[] getOrgStrutXml(BaseRowInterface orgStrutRowBean)
        ...
        OrgStrut struttura = struttureHelper.findById(OrgStrut.class,
                orgStrutRowBean.getBigDecimal("id_strut"));

        StringWriter sw = new StringWriter();

        JAXBContext jaxbCtx = JAXBContext.newInstance(OrgStrut.class);
        // Nota: al fine di evitare problemi di classloading e "override" del parser (vedi libreria
        // Saxon-HE)
        ...
        try {
            handler = ((SAXTransformerFactory) transFact)
                    .newTransformerHandler(new StreamSource(this.getClass().getClassLoader()
                            .getResourceAsStream("/xsl/sortExportXml.xsl")));
    ...
    public OrgAmbienteRowBean getOrgAmbienteRowBeanByIdStrut(BigDecimal idStrut) {
        ...
        try {
            ambienteRowBean = (OrgAmbienteRowBean) Transform
                    .entity2RowBean(struttura.getOrgEnte().getOrgAmbiente());
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
        }
            ...
            try {
                ...
                if (idAmbienteEnteConvenz != null) {
                    strutRowBean.setBigDecimal("id_ambiente_ente_convenz",
                            new BigDecimal(idAmbienteEnteConvenz));
                }
            } catch (Exception e) {
                logger.error(e.getMessage(), e);
            }
    ...
    public OrgStrutRowBean getOrgStrutRowBean(BigDecimal idStrut) {
        ...
        if (strut != null) {
            try {
                ...
                strutRowBean.setBigDecimal("id_ambiente",
                        new BigDecimal(orgAmbiente.getIdAmbiente()));
                strutRowBean.setString("nm_ambiente", orgAmbiente.getNmAmbiente());
                // Se è presente l'ente convenzionato, allora ricavo anche il padre, vale a dire
                // l'ambiente ente
    ...
    public OrgStrutTableBean getOrgStrutTableBean(String nmStrut, BigDecimal idEnte,
            BigDecimal idAmbiente, Boolean isTemplate) {
        OrgStrutTableBean strutTableBean = new OrgStrutTableBean();
        List<OrgStrut> strutList = struttureHelper.retrieveOrgStrutList(nmStrut, idEnte, idAmbiente,
                isTemplate);
    ...
    public OrgStrutTableBean getOrgStrutTableBean(long idUtente, List<BigDecimal> idEntiList,
        ...
        OrgStrutTableBean strutTableBean = new OrgStrutTableBean();
        List<OrgStrut> strutList = struttureHelper.retrieveOrgStrutList(idUtente, idEntiList,
                idCategStrutList);
        try {
            if (!strutList.isEmpty()) {
                strutTableBean = (OrgStrutTableBean) Transform.entities2TableBean(strutList);
    ...
    public OrgStrutTableBean getOrgStrutTableBean(long idUtente, BigDecimal idEnte,
            Boolean filterValid) {
        OrgStrutTableBean strutTableBean = new OrgStrutTableBean();
        List<OrgStrut> strutList = struttureHelper.retrieveOrgStrutList(idUtente, idEnte,
                filterValid);
        try {
            if (!strutList.isEmpty()) {
                strutTableBean = (OrgStrutTableBean) Transform.entities2TableBean(strutList);
    ...
    //
    public OrgVRicStrutTableBean getOrgVRicStrutTableBean(String nmStrut, BigDecimal idEnte,
            BigDecimal idAmbiente, Boolean isTemplate, String partizionata,
            String nmSistemaVersante, BigDecimal idAmbitoTerrit, BigDecimal idCategEnte,
            BigDecimal idAmbienteEnteConvenz, BigDecimal idEnteConvenz, String flParametriSpecifici,
            long idUtente) {

        OrgVRicStrutTableBean strutTableBean = new OrgVRicStrutTableBean();

        List<OrgVRicStrut> strutList = struttureHelper.retrieveOrgVRicStrutList(nmStrut, idEnte,
                idAmbiente, isTemplate, partizionata, nmSistemaVersante, idAmbitoTerrit,
                idCategEnte, idAmbienteEnteConvenz, idEnteConvenz, flParametriSpecifici, idUtente);

        try {
            if (!strutList.isEmpty()) {
                for (OrgVRicStrut strut : strutList) {
                    OrgVRicStrutRowBean strutRow = (OrgVRicStrutRowBean) Transform
                            .entity2RowBean(strut);
                    strutRow.setString("nm_ente",
                            strut.getNmEnte() + " (" + (strut.getNmAmbiente()) + ")");
    ...
    public CopiaStruttureEjb.OrgStrutCopyResult insertOrgStrutImp(LogParam param,
            OrgStrutRowBean strutRowBean, UUID uuid, SalvaStrutturaDto salva,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura) throws ParerUserError {
        StruttureEjb me = context.getBusinessObject(StruttureEjb.class);

        // IMPORTA NON STANDARD
        CopiaStruttureEjb.OrgStrutCopyResult result = me.otherTypeSaveStruttura(param, uuid, null,
                strutRowBean, TipoOper.IMPORTA_NON_STANDARD, salva,
            ...
            for (DecTipoUnitaDocAmmesso associazione : registro.getDecTipoUnitaDocAmmessos()) {
                try {
                    BigDecimal idRegistroUnitaDoc = new BigDecimal(
                            registro.getIdRegistroUnitaDoc());
                    BigDecimal idTipoUnitaDoc = new BigDecimal(
                            associazione.getDecTipoUnitaDoc().getIdTipoUnitaDoc());
                    tipoSerieEjb.createTipoSerieStandardDaRegistroOTipoUdNewTx(param,
        ...
        // Aggiorno i servizi di conservazione e attivazione presenti sui tipi ud della struttura
        // considerata
        updateTipiServizioOnTipiUdInStruttura(replic.getIdOrganizApplic());
        struttureHelper.getEntityManager().flush();
        // Log applicativo
        sacerLogEjb.logInNewTx(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_STRUTTURA,
                new BigDecimal(replic.getIdOrganizApplic().longValueExact()),
                param.getNomePagina());
    ...
    public CopiaStruttureEjb.OrgStrutCopyResult copyOrgStrutRowBean(LogParam param,
            OrgStrutRowBean strutRowBean, BigDecimal idStrut, SalvaStrutturaDto salva,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura) throws ParerUserError {
        String errorMessageTipiSerie = "TIPISERIE;";
        ...
        CopiaStruttureEjb.OrgStrutCopyResult result = me.otherTypeSaveStruttura(param, null,
                idStrut, strutRowBean, TipoOper.DUPLICA_NON_STANDARD, salva,
                parametriAmministrazioneStruttura, parametriConservazioneStruttura,
                parametriGestioneStruttura);
        IamOrganizDaReplic replic = result.getIamOrganizDaReplic();
        // Eseguo la creazione dei tipi serie standard, se possibile
    ...
    public CopiaStruttureEjb.OrgStrutCopyResult overwriteOrgStrutImp(LogParam param,
            OrgStrutRowBean strutRowBean, UUID uuid, SalvaStrutturaDto salva,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura) throws ParerUserError {
        ...
        CopiaStruttureEjb.OrgStrutCopyResult result = me.otherTypeSaveStruttura(param, uuid, null,
                strutRowBean, TipoOper.IMPORTA_STANDARD, salva, parametriAmministrazioneStruttura,
    ...
    public CopiaStruttureEjb.OrgStrutCopyResult overwriteStrut(LogParam param, BigDecimal idStrut,
        ...
        // DUPLICA STANDARD
        CopiaStruttureEjb.OrgStrutCopyResult result = me.otherTypeSaveStruttura(param, null,
                idStrut, strutRowBean, TipoOper.DUPLICA_STANDARD, salva,
                parametriAmministrazioneStruttura, parametriConservazioneStruttura,
                parametriGestioneStruttura);
        IamOrganizDaReplic replic = result.getIamOrganizDaReplic();
        // Eseguo la creazione dei tipi serie standard, se possibile
    ...
    public void updateOrgStruttura(LogParam param, BigDecimal idStrut, OrgStrutRowBean strutRowBean)
        ...
        StruttureEjb me = context.getBusinessObject(StruttureEjb.class);
        IamOrganizDaReplic replic = me.saveStruttura(param, idStrut, strutRowBean, TipoOper.MOD,
                false);
        if (replic != null) {
            me.replicateToIam(replic);
        }
    ...
    public void insertOrgStruttura(LogParam param, OrgStrutRowBean orgStrutRowBean,
            boolean isStruttureTemplate) throws ParerUserError {
        StruttureEjb me = context.getBusinessObject(StruttureEjb.class);
        IamOrganizDaReplic replic = me.saveStruttura(param, null, orgStrutRowBean, TipoOper.INS,
                isStruttureTemplate);
        if (replic != null) {
            me.replicateToIam(replic);
        }
    ...
    public IamOrganizDaReplic saveStruttura(LogParam param, BigDecimal idStrut,
            OrgStrutRowBean strutRowBean, TipoOper tipoOper, boolean isStruttureTemplate)
            throws ParerUserError {
        ...
        if (tipoOper.name().equals(TipoOper.MOD.name())) {
            ...
            // Se la struttura di un certo ente che sto modificando è già presente su DB (ovviamente
            // controllo solo se
            // il nome è cambiato...)
            if (struttureHelper.getOrgStrutByName(strutRowBean.getNmStrut(),
                    strutRowBean.getIdEnte()) != null
                    && !strut.getNmStrut().equals(strutRowBean.getNmStrut())) {
                throw new ParerUserError(
                        "Struttura gi\u00E0 associata a questo ente all'interno del database</br>");
            }

            // Se la struttura NON è template
            if (!strut.getFlTemplate().equals("1")) {
                if (strutRowBean.getCdStrutNormaliz() == null
                        || struttureHelper.existsCdStrutNormaliz(strutRowBean.getCdStrutNormaliz(),
                                strutRowBean.getIdEnte(), idStrut)) {
                    throw new ParerUserError(
            ...
            if (!strutRowBean.getFlTemplate().equals("1")) {
                ...
                formatoFileDocHelper
                        .insertDecFormatoFileDocStrutturaSpecificaNative(strut.getIdStrut());
            }
        }

        IamOrganizDaReplic replic = null;
        if (modificatiNomeDescrizioneEntePapi) {
            replic = context.getBusinessObject(StruttureEjb.class)
                    .insertStrutIamOrganizDaReplic(strut, tiOper);
        }
        return replic;
    }

    /**
     * Controlli sul salvataggio associazione struttura/ente convenzionato in fase di
     * inserimento/importa/duplica struttura come riportato in MAC #26960 (primo punto riferito
     * all'inserimento di una nuova struttura)
     *
     * @param strutRowBean
     * @param ente
     *
     * @throws ParerUserError
     */
    private void controlloAccordoEnteConvenzionatoSuInsStruttura(OrgStrutRowBean strutRowBean)
            throws ParerUserError {
        if (strutRowBean.getIdEnteConvenz() != null) {
            ...
            // Controllo l'intervallo dell'associazione rientri nell'intervallo di decorrenza
            // accordo valido e fine
            // validità ente siam
            if (!struttureHelper.existsIntervalloValiditaPerAssociazione(
                    strutRowBean.getIdEnteConvenz(), strutRowBean.getDtIniVal(),
                    strutRowBean.getDtFineVal())) {
                throw new ParerUserError(
    ...
    public void cessazioneStruttura(LogParam param, long idStrut) throws ParerUserError {
        ...
        if (struttura.getIdEnteConvenz() != null) {
            // Controllo che l’ente siam cui la struttura appartiene alla data corrente NON abbia
            // accordi validi alla
            // data corrente
            if (struttureHelper.checkEsistenzaAccordoValidoEnteConvenzStrutVers(
                    struttura.getIdEnteConvenz())) {
                throw new ParerUserError(
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_STRUTTURA, new BigDecimal(idStrut),
                param.getNomePagina());
    ...
    public void saveParametriStruttura(LogParam param,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura, BigDecimal idStrut) {
        OrgStrut strut = struttureHelper.findById(OrgStrut.class, idStrut);
        gestioneParametriStruttura(parametriAmministrazioneStruttura,
                parametriConservazioneStruttura, parametriGestioneStruttura, strut);
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_STRUTTURA, new BigDecimal(strut.getIdStrut()),
                param.getNomePagina());
    }

    private void gestioneParametriStruttura(
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura, OrgStrut strut) {
        // Gestione parametri amministrazione
        manageParametriPerStruttura(parametriAmministrazioneStruttura,
                "ds_valore_param_applic_strut_amm", strut);
        // Gestione parametri conservazione
        manageParametriPerStruttura(parametriConservazioneStruttura,
                "ds_valore_param_applic_strut_cons", strut);
        // Gestione parametri gestione
        manageParametriPerStruttura(parametriGestioneStruttura, "ds_valore_param_applic_strut_gest",
                strut);
    }
    ...
    public void saveDefaultSubStrut(OrgStrut strut, boolean isTemplate) {
        ...
        subStrut.setDsSubStrut(isTemplate ? CostantiDB.SubStruttura.DEFAULT_TEMPLATE_DESC
                : CostantiDB.SubStruttura.DEFAULT_DESC + strut.getNmStrut() + " - "
                        + strut.getDsStrut());
        struttureHelper.insertEntity(subStrut, true);
        strut.getOrgSubStruts().add(subStrut);
    }

    public long insertOrgEnteConvezOrg(BigDecimal idStrut, BigDecimal idEnteConvenz, Date dtIniVal,
            Date dtFineVal) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public CopiaStruttureEjb.OrgStrutCopyResult otherTypeSaveStruttura(LogParam param, UUID uuid,
            BigDecimal idStrut, OrgStrutRowBean strutRowBean, TipoOper tipoOper,
            SalvaStrutturaDto salva, AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura) throws ParerUserError {
        ...
        if (tipoOper.name().equals(TipoOper.DUPLICA_STANDARD.name())) {
            ...
            /*
             * Eseguo la copia dei record in orgCampoValSubStrut manualmente Per farlo devo eseguire
             * le seguenti operazioni nella copia xml: - scorrere i tipiUd - per ogni tipoUd,
             * scorrermi ogni regola - per ogni regola, scorrere i campi e copiarli, ricercando la
             * regola in base ai suoi parametri su db
             *
             * Per copiare i campi, bisogna eseguire le seguenti operazioni: In base al tipo campo:
             * + SUB_STRUTTURA: riprendo dalla struttura nuova la sottostruttura creata (l'unica) e
             * l'associo al nuovo campo + DATI_SPEC : ricerco in base ai dati il nuovo record di
             * DecAttribDatiSpec e glielo associo + DATO_PROFILO : non ha bisogno di altre ricerche
             */
            ...
            for (DecTipoUnitaDoc tipoUd : duplicante.getDecTipoUnitaDocs()) {
                for (OrgRegolaValSubStrut regola : tipoUd.getOrgRegolaValSubStruts()) {
                    OrgRegolaValSubStrut regolaDb = subStrutHelper.getOrgRegolaSubStrut(
                            strut.getIdStrut(), regola.getDecTipoUnitaDoc().getNmTipoUnitaDoc(),
                            regola.getDecTipoDoc().getNmTipoDoc(), regola.getDtIstituz(),
                            regola.getDtSoppres());
                    if (regolaDb != null) {
                        for (OrgCampoValSubStrut campo : regola.getOrgCampoValSubStruts()) {
                            OrgCampoValSubStrut copiaCampo = new OrgCampoValSubStrut();
                            CostantiDB.TipoCampo campoEnum = CostantiDB.TipoCampo
                                    .valueOf(campo.getTiCampo());
                            ...
                            case DATO_SPEC_DOC_PRINC:
                                ...
                                try {
                                    attribDb = datiSpecHelper.getDecAttribDatiSpecUniDocAndDoc(
                                            strut.getIdStrut(), attrib.getNmAttribDatiSpec(),
                                            attrib.getTiEntitaSacer(), attrib.getTiUsoAttrib(),
                                            null, attrib.getDecTipoDoc().getNmTipoDoc(), null);
                            ...
                            case DATO_SPEC_UNI_DOC:
                                ...
                                try {
                                    attribDb = datiSpecHelper.getDecAttribDatiSpecUniDocAndDoc(
                                            strut.getIdStrut(), attrib.getNmAttribDatiSpec(),
                                            attrib.getTiEntitaSacer(), attrib.getTiUsoAttrib(),
                                            attrib.getDecTipoUnitaDoc().getNmTipoUnitaDoc(), null,
                                            null);
                                    copiaCampo.setDecAttribDatiSpec(attribDb);
                                } catch (Exception e) {
                                    throw new ParerUserError(
                            ...
                            case SUB_STRUT:
                                List<OrgSubStrut> subStruts = subStrutHelper.getOrgSubStrut(
                                        campo.getOrgSubStrut().getNmSubStrut(),
                                        new BigDecimal(strut.getIdStrut()));
            ...
            // Verifico se questo ente, che deve avere strutture template appartenenti ad esso,
            // effettivamente ne ha
            if (orgEnte.getTipoDefTemplateEnte()
                ...
                if (template == null) {
                    throw new ParerUserError("Nell\u0027 ente " + orgEnte.getNmEnte()
                            + " non sono presenti strutture template");
                }
            } // Questo ente non è un template, di conseguenza non può avere al suo cospetto
              // strutture template.
              // Allora verifico se l'ambiente di cui fa parte, ha altri enti con strutture template
            else if (orgEnte.getTipoDefTemplateEnte()
                    .equals(CostantiDB.TipoDefTemplateEnte.NO_TEMPLATE.name())) {
                template = struttureHelper
                        .getFirstOrgStrutTemplatePerAmbienteAndTipoDefTemplateEntePartizionata(
                                idAmbiente,
                                CostantiDB.TipoDefTemplateEnte.TEMPLATE_DEF_AMBIENTE.name());
                if (template == null) {
                    String nmAmbiente = (struttureHelper.findById(OrgAmbiente.class, idAmbiente))
                            .getNmAmbiente();
                    throw new ParerUserError("Nell\u0027 ambiente " + nmAmbiente
                            + " non sono presenti strutture template");
                }
            } else // Controllo se il flag di "Struttura Template" è uguale a 0, in caso segnalo
            // errore
            {
                if (strutRowBean.getFlTemplate().equals("0")) {
                    throw new ParerUserError(
                            "L'ente template può contenere solo strutture di tipo template");
                }
            }
            ...
            result = impostaDatiStrutturaImpStd(struttura, template, orgEnte, strutRowBean, salva,
                    true, true, parametriAmministrazioneStruttura, parametriConservazioneStruttura,
                    parametriGestioneStruttura);
            strut = result.getOrgStrut();
            ...
            for (DecTipoUnitaDoc tipoUd : struttura.getDecTipoUnitaDocs()) {
                for (OrgRegolaValSubStrut regola : tipoUd.getOrgRegolaValSubStruts()) {
                    EntitaValida entita = new EntitaValida(salva.isCheckIncludiElementiDisattivi(),
                            salva.isCheckMantieniDateFineValidita(), regola.getDtIstituz(),
                            regola.getDtSoppres(), salva.getDataAttuale());
                    if (entita.isValida()) {
                        OrgRegolaValSubStrut regolaDb = subStrutHelper.getOrgRegolaSubStrut(
                                strut.getIdStrut(), regola.getDecTipoUnitaDoc().getNmTipoUnitaDoc(),
                                regola.getDecTipoDoc().getNmTipoDoc(), entita.getDataInizio(),
                                entita.getDataFine());
                        if (regolaDb != null) {
                            for (OrgCampoValSubStrut campo : regola.getOrgCampoValSubStruts()) {
                                OrgCampoValSubStrut copiaCampo = new OrgCampoValSubStrut();
                                ...
                                case DATO_SPEC_UNI_DOC:
                                    ...
                                    try {
                                        attribDb = datiSpecHelper.getDecAttribDatiSpecUniDocAndDoc(
                                                strut.getIdStrut(), attrib.getNmAttribDatiSpec(),
                                                attrib.getTiEntitaSacer(), attrib.getTiUsoAttrib(),
                                                attrib.getDecTipoUnitaDoc().getNmTipoUnitaDoc(),
                                                null, null);
                                        copiaCampo.setDecAttribDatiSpec(attribDb);
                                    } catch (Exception e) {
                                        throw new ParerUserError(
            ...
            result = impostaDatiStruttura(struttura, new OrgStrut(), newOrgEnte, strutRowBean,
                    salva, false, false, parametriAmministrazioneStruttura,
                    parametriConservazioneStruttura, parametriGestioneStruttura,
            ...
            // controllo che siano presenti nel DB tutte le categorie tipi unita doc che devo
            // associare alla struttura
            // che importo
            if (struttura.getDecTipoUnitaDocs() != null) {
                for (DecTipoUnitaDoc tipoUd : struttura.getDecTipoUnitaDocs()) {
                    ...
                    if ((categ = tipoUnitaDocHelper.getDecCategTipoUnitaDocByCode(
                        ...
                        throw new ParerUserError(
                                "La categoria della tipologia unit\u00E0 documentaria "
            ...
            // Nuovo codice a fattor comune con l'importa standard/non standard
            result = impostaDatiStruttura(struttura, new OrgStrut(), orgEnte, strutRowBean, salva,
                    true, false, parametriAmministrazioneStruttura, parametriConservazioneStruttura,
                    parametriGestioneStruttura, "0");
            strut = result.getOrgStrut();
            struttureHelper.getEntityManager().flush();
    ...
    private CopiaStruttureEjb.OrgStrutCopyResult gestisciImpDupStandard(LogParam param,
            OrgStrut strutturaDaCopiare, OrgStrutRowBean strutRowBean, SalvaStrutturaDto salva,
            boolean isImport, AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura) throws ParerUserError {
        CopiaStruttureEjb.OrgStrutCopyResult result = null;
        /* CONTROLLI PRELIMINARI */
        ...
        // Verifico se questo ente, che deve avere strutture template appartenenti ad esso,
        // effettivamente ne ha
        if (ente.getTipoDefTemplateEnte()
                .equals(CostantiDB.TipoDefTemplateEnte.TEMPLATE_DEF_ENTE.name())) {
            template = struttureHelper.getFirtsOrgStrutTemplatePerEntePartizionata(idEnte);
            if (template == null) {
                throw new ParerUserError("Nell\u0027 ente " + ente.getNmEnte()
                        + " non sono presenti strutture template");
            }
        } // Questo ente non è un template, di conseguenza non può avere al suo cospetto strutture
          // template.
          // Allora verifico se l'ambiente di cui fa parte, ha altri enti con strutture template
        else if (ente.getTipoDefTemplateEnte()
                .equals(CostantiDB.TipoDefTemplateEnte.NO_TEMPLATE.name())) {
            template = struttureHelper
                    .getFirstOrgStrutTemplatePerAmbienteAndTipoDefTemplateEntePartizionata(
                            idAmbiente,
        ...
        struttureHelper.deleteOrgStrutRelations(template);
        struttureHelper.getEntityManager().flush();
        result = impostaDatiStrutturaImpStd(strutturaDaCopiare, template, ente, strutRowBean, salva,
                isImport, true, parametriAmministrazioneStruttura, parametriConservazioneStruttura,
                parametriGestioneStruttura);
    ...
    private CopiaStruttureEjb.OrgStrutCopyResult impostaDatiStruttura(OrgStrut oldStrut,
            OrgStrut newStrut, OrgEnte orgEnte, OrgStrutRowBean strutRowBean,
            SalvaStrutturaDto salva, boolean isImport, boolean isStandard,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
            AplParamApplicTableBean parametriGestioneStruttura, String flTemplate)
            throws ParerUserError {
        // Se la struttura NON è template
        if (!strutRowBean.getFlTemplate().equals("1")) {
            if (strutRowBean.getCdStrutNormaliz() == null || struttureHelper.existsCdStrutNormaliz(
                    strutRowBean.getCdStrutNormaliz(), strutRowBean.getIdEnte(), null)) {
                throw new ParerUserError(
                        "Il nome normalizzato della struttura non è stato indicato o non è univoco</br>");
            }
        }
        ...
        if (strutRowBean.getIdCategStrut() != null) {
            newStrut.setOrgCategStrut(
                    struttureHelper.findById(OrgCategStrut.class, strutRowBean.getIdCategStrut()));
        } else {
        ...
        if (salva.isCheckIncludiTipiFascicolo()) {
            // Controllo i modelli xsd periodo tipi fascicolo in base all'ambiente di questa nuova
            // struttura
            checkAndSetModelliXsdTipiFascicoloStruttura(oldStrut,
                    new BigDecimal(orgEnte.getOrgAmbiente().getIdAmbiente()));
        }

        CopiaStruttureEjb.OrgStrutCopyResult result = copiaStruttureEjb
                .getOrgStrutCopyFromStrut(oldStrut, newStrut, salva, isStandard);
        ...
        checkAndSetModelliTipiSerieStruttura(newStrut,
                new BigDecimal(orgEnte.getOrgAmbiente().getIdAmbiente()),
    ...
    private CopiaStruttureEjb.OrgStrutCopyResult impostaDatiStrutturaImpStd(OrgStrut oldStrut,
            OrgStrut newStrut, OrgEnte orgEnte, OrgStrutRowBean strutRowBean,
            SalvaStrutturaDto salva, boolean isImport, boolean isStandard,
            AplParamApplicTableBean parametriAmministrazioneStruttura,
            AplParamApplicTableBean parametriConservazioneStruttura,
        ...
        CopiaStruttureEjb.OrgStrutCopyResult result = copiaStruttureEjb
                .getOrgStrutCopyFromStrutImpStd(oldStrut, newStrut, salva, isStandard);
        newStrut = result.getOrgStrut();
    ...
    /*
     * Popola i valori param applic della struttura partendo da liste di rowBean separati per
     * gestione, amministrazione e conservazione
     */
    ...
    /**
     * Controlla se registri e tipi Ud della nuova struttura hanno associati dei modelli. Nel qual
     * caso verifica che gli stessi siano presenti nel nuovo ambiente della nuova struttura. Se
     * anche un solo modello non è presente per il nuovo ambiente, viene segnalato errore, al
     * contrario, con tutti i modelli presenti nella nuova struttura (verificata l'uguaglianza
     * tramite il nome) si procede al recupero da DB degli stessi ed associati a registri e tipi ud
     *
     * @param strutturaNuova  struttura
     * @param idAmbienteNuovo id ambiente nuovo
     * @param infisso         fisso
     *
     * @throws ParerUserError errore generico
     */
    private void checkAndSetModelliTipiSerieStruttura(OrgStrut strutturaNuova,
            BigDecimal idAmbienteNuovo, String infisso) throws ParerUserError {
        ...
        String errorMessage = "Nella struttura " + infisso + "ta "
                ...
                + "a un modello che non è definito nell'ambiente di destinazione. "
                + "Prima di eseguire " + articoloDeterminativo + infisso
                + "zione occorre definire il modello";
        if (registriNuovi != null) {
            for (DecRegistroUnitaDoc registroNuovo : registriNuovi) {
                if (registroNuovo.getDecModelloTipoSerie() != null) {
                    DecModelloTipoSerie modelloTipoSerieNuovoAmbiente = modelliSerieHelper
                            .getDecModelloTipoSerie(
                                    registroNuovo.getDecModelloTipoSerie().getNmModelloTipoSerie(),
                                    idAmbienteNuovo);
        ...
        if (tipiUdNuovi != null) {
            for (DecTipoUnitaDoc tipoUdNuovo : tipiUdNuovi) {
                if (tipoUdNuovo.getDecModelloTipoSerie() != null) {
                    DecModelloTipoSerie modelloTipoSerieNuovoAmbiente = modelliSerieHelper
                            .getDecModelloTipoSerie(
                                    tipoUdNuovo.getDecModelloTipoSerie().getNmModelloTipoSerie(),
                                    idAmbienteNuovo);
                    if (modelloTipoSerieNuovoAmbiente != null) {
    ...
    /**
     * Controlla se i periodi di validità dei tipi fascicolo della nuova struttura hanno associati
     * dei modelli xsd tipo fascicolo. Nel qual caso verifica che gli stessi siano presenti nel
     * nuovo ambiente della nuova struttura. Se anche un solo modello xsd non è presente per il
     * nuovo ambiente, viene segnalato errore, al contrario, con tutti i modelli presenti nella
     * nuova struttura (verificata l'uguaglianza tramite il il tipo modello e la versione codice
     * xsd) si procede al recupero da DB degli stessi ed associati al periodo tipo fascicolo
     *
     * @param strutturaOld    strutturaOld
     * @param idAmbienteNuovo id ambiente nuovo
     *
     * @throws ParerUserError errore generico
     */
    public void checkAndSetModelliXsdTipiFascicoloStruttura(OrgStrut strutturaOld,
            BigDecimal idAmbienteNuovo) throws ParerUserError {
        ...
        if (tipiFascicoloOld != null) {
            for (DecTipoFascicolo tipoFascicoloNuovo : tipiFascicoloOld) {
                if (tipoFascicoloNuovo.getDecAaTipoFascicolos() != null) {
                    for (DecAaTipoFascicolo aaTipoFascicoloNuovo : tipoFascicoloNuovo
                            .getDecAaTipoFascicolos()) {
                                ...
                                if (modelloXsdFascicoloNuovo != null) {
                                    DecModelloXsdFascicolo modelloXsdFascicoloNuovoAmbiente = modelliFascicoliHelper
                                            .getDecModelloXsdFascicolo(idAmbienteNuovo,
                                                    modelloXsdFascicoloNuovo.getTiModelloXsd()
                                                            .name(),
                                                    modelloXsdFascicoloNuovo.getTiUsoModelloXsd()
                                                            .name(),
                                                    modelloXsdFascicoloNuovo.getCdXsd());
                                    if (modelloXsdFascicoloNuovoAmbiente == null) {
                                        eccezione = true;
                                        String[] datiErroreModelli = new String[3];
                                        datiErroreModelli[0] = tipoFascicoloNuovo
                                                .getNmTipoFascicolo();
                                        datiErroreModelli[1] = modelloXsdFascicoloNuovo
                                                .getTiModelloXsd().name();
                                        datiErroreModelli[2] = modelloXsdFascicoloNuovo.getCdXsd();
                                        String modelloDaValutare = modelloXsdFascicoloNuovo
                                                .getTiModelloXsd().name() + "_v"
                                                + modelloXsdFascicoloNuovo.getCdXsd();
        ...
        if (eccezione) {
            ...
            for (String[] datiErroreModelli : listaDatiErroreModelli) {
                errorMessage.append("Il Tipo fascicolo ").append(datiErroreModelli[0])
                        .append(" referenzia il modello ").append(datiErroreModelli[1]).append("_v")
                        .append(datiErroreModelli[2]);
            }
    ...
    public void checkAndSetModelliXsdTipiFascicoloStrutturaImpParam(OrgStrut strutturaOld,
            BigDecimal idAmbienteNuovo, String tipoFascicoloDaImp) throws ParerUserError {
        List<DecTipoFascicolo> tipiFascicoloOld = strutturaOld.getDecTipoFascicolos();
        boolean eccezione = false;
                                    ...
                                    if (modelloXsdFascicoloNuovo != null) {
                                        DecModelloXsdFascicolo modelloXsdFascicoloNuovoAmbiente = modelliFascicoliHelper
                                                .getDecModelloXsdFascicolo(idAmbienteNuovo,
                                                        modelloXsdFascicoloNuovo.getTiModelloXsd()
                                                                .name(),
                                                        modelloXsdFascicoloNuovo
                                                                .getTiUsoModelloXsd().name(),
                                                        modelloXsdFascicoloNuovo.getCdXsd());
                                        if (modelloXsdFascicoloNuovoAmbiente == null) {
                                            eccezione = true;
                                            String[] datiErroreModelli = new String[3];
                                            ...
                                            datiErroreModelli[1] = modelloXsdFascicoloNuovo
                                                    .getTiModelloXsd().name();
                                            datiErroreModelli[2] = modelloXsdFascicoloNuovo
                                                    .getCdXsd();
                                            String modelloDaValutare = modelloXsdFascicoloNuovo
                                                    .getTiModelloXsd().name() + "_v"
                                                    + modelloXsdFascicoloNuovo.getCdXsd();
    ...
    public void checkAndSetModelliXsdTipiFascicoloStrutturaImpParam(UUID uuid,
            BigDecimal idStrutCorrente, String tipoFascicoloDaImp) throws ParerUserError {
        // Ricavo la struttura presente nell'XML
        OrgStrut strutExp = strutCache.getOrgStrut(uuid);
        OrgStrut strutturaCorrente = struttureHelper.findById(OrgStrut.class, idStrutCorrente);
    ...
    /**
     *
     * @param struttureDaElaborare le strutture da elaborare nell'importa parametri massivo
     *
     * @return la stringa con eventuale errore per presenza di strutture appartenenti ad ambienti
     *         diversi nell'import parametri massivo
     *
     * @throws ParerUserError errore generico
     */
    public String checkAppartenenzaAmbiente(Map<BigDecimal, String> struttureDaElaborare)
            throws ParerUserError {
        // Scorro le strutture coinvolte nell'importa parametri per verificare se appartengono tutte
        // allo stesso
        // ambiente
     ...
     *
     * @param idStrutturaCorrente id struttura corrente
     * @param tipoUnitaDocExp     tipo unita doc
     * @param tipoUnitaDoc        tipo unita doc
     * @param registriImportati   registri
     *
     * @throws ParerUserError errore generico
     */
    private void checkAndSetModelliTipiSerieStrutturaPerImportaParametri(
            BigDecimal idStrutturaCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
            DecTipoUnitaDoc tipoUnitaDoc, List<CoppiaRegistri> registriImportati)
            throws ParerUserError {
        // Ricavo la struttura corrente, quella dove mi trovo e sto eseguendo l'importazione dei
        // parametri
        OrgStrut strutturaCorrente = struttureHelper.findById(OrgStrut.class,
                idStrutturaCorrente.longValue());
        ...
        // Verifico se sui registri o tipi di unità documentaria che sto importando ci siano dei tag
        // DecModelloTipoSerie
        if (registriImportati != null) {
            for (CoppiaRegistri registroNuovo : registriImportati) {
                if (registroNuovo.getRegistroUnitaDocExp().getDecModelloTipoSerie() != null) {
                    DecModelloTipoSerie modelloTipoSerieNuovoAmbiente = modelliSerieHelper
                            .getDecModelloTipoSerie(
                                    registroNuovo.getRegistroUnitaDocExp().getDecModelloTipoSerie()
                                            .getNmModelloTipoSerie(),
                                    new BigDecimal(idAmbienteNuovo));
                    if (modelloTipoSerieNuovoAmbiente != null) {
                        registroNuovo.getRegistroUnitaDoc()
                                .setDecModelloTipoSerie(modelloTipoSerieNuovoAmbiente);
                    } else {
                        throw new ParerUserError(errorMessage);
                    }
                }
        ...
        if (tipoUnitaDocExp.getDecModelloTipoSerie() != null) {
            DecModelloTipoSerie modelloTipoSerieNuovoAmbiente = modelliSerieHelper
                    .getDecModelloTipoSerie(
                            tipoUnitaDocExp.getDecModelloTipoSerie().getNmModelloTipoSerie(),
                            new BigDecimal(idAmbienteNuovo));
            if (modelloTipoSerieNuovoAmbiente != null) {
    ...
    /**
     * Verifica la presenza dei sistemi versanti in fase di import
     *
     * @param tipoStrutUnitaDocExpList lista dei tipi struttura ud per i quali controllare il
     *                                 sistema versante
     *
     * @throws ParerUserError errore che segnala l'assenza del sistema versante
     */
    private void checkPresenzaSistemiVersanti(List<DecTipoStrutUnitaDoc> tipoStrutUnitaDocExpList)
            throws ParerUserError {
        String errorMessage = "";
        ...
        for (DecTipoStrutUnitaDoc tipoStrutUnitaDocExp : tipoStrutUnitaDocExpList) {
            ...
            for (DecTipoStrutUdSisVer tipoStrutUdSisVersExp : tipoStrutUnitaDocExp
                    .getDecTipoStrutUdSisVers()) {
                ...
                if (sistemaVersante == null) {
                    errorMessage = "Il sistema versante "
                            + tipoStrutUdSisVersExp.getAplSistemaVersante().getNmSistemaVersante()
                            + " presente nel tipo struttura unità documentaria "
                            + tipoStrutUnitaDocExp.getNmTipoStrutUnitaDoc() + " del tipo ud "
    ...
    // MAC #33057
    /**
     * Controlla i modelli xsd ud del tipo UD da importare verificando che siano presenti
     * nell'ambiente della struttura
     *
     * @param idStrutturaCorrente la struttura corrente nella quale si sta importando il tipo ud
     * @param tipoUnitaDocExp     il tipo ud dell'XML da importare
     * @param tipoUnitaDoc        il tipo ud che si sta importando
     *
     * @throws ParerUserError errore restituito in caso di mancanza modello
     */
    public void checkAndSetModelliXsdTipiUdStrutturaPerImportaParametri(
            BigDecimal idStrutturaCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
        ...
        if (tipoUnitaDocExp.getDecUsoModelloXsdUniDocs() != null) {
            for (DecUsoModelloXsdUniDoc usoModelloXsdUniDocNuovo : tipoUnitaDocExp
                    .getDecUsoModelloXsdUniDocs()) {
                DecModelloXsdUd modelloXsdUdNuovo = usoModelloXsdUniDocNuovo.getDecModelloXsdUd();
                if (modelloXsdUdNuovo != null) {
                    DecModelloXsdUd modelloXsdUdNuovoAmbiente = modelliXsdUdHelper
                            .getDecModelloXsdUd(idAmbienteNuovo,
                                    modelloXsdUdNuovo.getTiModelloXsd().name(),
                                    modelloXsdUdNuovo.getTiUsoModelloXsd(),
        ...
        if (eccezione) {
            ...
            for (String[] datiErroreModelli : listaDatiErroreModelli) {
                ...
                errorMessage.append("Il Tipo unità documentaria ").append(datiErroreModelli[0])
                        .append(" per la struttura ").append(strut.getNmStrut())
                        .append(" referenzia il modello ").append(datiErroreModelli[1]).append("_v")
                        .append(datiErroreModelli[2])
                        .append(" che non è presente in questo ambiente;<br>");
            }
    ...
    public void importaUsoModelloXsdUd(BigDecimal idStrutCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
            DecTipoUnitaDoc tipoUnitaDoc) throws ParerUserError {
        // Importa Modello
        List<DecUsoModelloXsdUniDoc> usoModelloXsdUniDocExpList = tipoUnitaDocExp
                .getDecUsoModelloXsdUniDocs();
        ...
        for (DecUsoModelloXsdUniDoc usoModelloXsdUniDocExp : usoModelloXsdUniDocExpList) {
            ...
            if (modelloXsdUd != null) {
                // Controlla se esiste uso modello del tipo unit\u00E0 doc su DB per l'ambiente
                // della struttura del tipo
                // ud
                // che sto importando
                DecUsoModelloXsdUniDoc usoModelloXsdUniDoc = modelliXsdUdHelper
                        .getDecUsoModelloXsdUniDoc(idStrutCorrente,
                                tipoUnitaDocExp.getNmTipoUnitaDoc(),
                                modelloXsdUd.getIdModelloXsdUd());

                // Se usoModelloXsdUniDoc non \u00E8 presente, lo creo e lo salvo su DB
                if (usoModelloXsdUniDoc == null) {

                    inserisciUsoModelloXsdUniDoc(tipoUnitaDoc, usoModelloXsdUniDocExp,
                            modelloXsdUd);
                }
            }
     ...
     *
     * @param tipoUnitaDoc        tipo unita doc
     * @param usoModelloXsdUniDoc uso modello xsd uni doc
     * @param modelloXsdUd        modello xsd uni doc
     *
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamOrganizDaReplic executeDeleteStruttura(LogParam param, long idStrut)
            throws ParerUserError {
        OrgStrut struttura = struttureHelper.findById(OrgStrut.class, idStrut);
        ...
        try {
            ...
            for (DecRegistroUnitaDoc registroUnitaDoc : struttura.getDecRegistroUnitaDocs()) {
                registroEjb.deleteRegistroUnitaDocFromStruttura(param,
                        registroUnitaDoc.getIdRegistroUnitaDoc());
            }

            // Elimino tutte le tipologie unità documentaria
            for (DecTipoUnitaDoc tipoUnitaDoc : struttura.getDecTipoUnitaDocs()) {
                tipoUnitaDocEjb.deleteTipoUnitaDocFromStruttura(param,
            ...
            for (DecTipoFascicolo tipoFascicolo : struttura.getDecTipoFascicolos()) {
                tipoFascicoloEjb.deleteDecTipoFascicolo(param,
            ...
            for (DecCriterioRaggr criterioRaggr : struttura.getDecCriterioRaggrs()) {
                crHelper.deleteDecCriterioRaggr(param,
                        new BigDecimal(criterioRaggr.getOrgStrut().getIdStrut()),
     ...
     */
    // -------------------------------------
    public boolean existAroUnitaDocByIdRegistroUnitaDoc(BigDecimal idRegistroUnitaDoc,
            BigDecimal idStrut) {
    ...
    public boolean isStrutUsedForVers(BaseRowInterface orgStrutRowBean) {
        OrgStrut strut = struttureHelper.findById(OrgStrut.class,
                orgStrutRowBean.getBigDecimal("id_strut"));
    ...
    public String countOrgStrutTemplateRaggruppati(long idUserIam) {
        List<Object[]> numStrutTemplatePerAmbiente = struttureHelper
                .countOrgStrutTemplateRaggruppati(idUserIam);
    ...
    public long countOrgStrutTemplatePerAmbienteEnte(Long idAmbiente, Long idEnte,
            String tipoDefTemplateEnte) {
        return struttureHelper.countOrgStrutTemplatePerAmbienteEnte(idAmbiente, idEnte,
                tipoDefTemplateEnte);
    }

    private String getConteggioAmbientiPerStrutturaFormattati(
            List<Object[]> numStrutTemplatePerAmbiente) {
    ...
    public OrgCategStrutTableBean getOrgCategStrutTableBean(
            OrgCategStrutRowBean categStrutRowBean) {
        ...
        if (list != null) {
            try {
                orgCategStrutTableBean = (OrgCategStrutTableBean) Transform
    ...
    public void insertOrgCategStrut(OrgCategStrutRowBean categStrutRowBean) throws ParerUserError {
        OrgCategStrut categStrutDB = struttureHelper
                .getOrgCategStrutByCd(categStrutRowBean.getCdCategStrut());
        if (categStrutDB != null) {
            throw new ParerUserError(
                    "Codice categoria gi\u00E0 utilizzato nel database, scegliere altro codice ");
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteOrgCategStrut(OrgCategStrutRowBean orgCategStrutRowBean)
            throws ParerUserError {
     ...
     *
     * @throws ParerUserError Eccezione con rollback in caso di entity diversa dalle suddette o di
     *                        errore imprevisto da parte dell'allineamento
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    ...
    public IamOrganizDaReplic insertStrutIamOrganizDaReplic(OrgStrut orgStrut,
            ApplEnum.TiOperReplic tipoOperazione) {
     ...
     *
     * @param enteConvenzDaAllineaList lista IamEnteConvenzDaAllinea
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void alignsEnteConvenzToIam(List<IamEnteConvenzDaAllinea> enteConvenzDaAllineaList)
            throws ParerUserError {
        try {
            aecEjb.allineaEntiConvenzionati(enteConvenzDaAllineaList);
        } catch (Exception ex) {
            logger.error("Errore imprevisto del servizio di allineamento ente convenzionato : "
                    + ex.getMessage(), ex);
     ...
     * convenzionato associato alla struttura del tipo ud
     *
    ...
    public boolean calcoloServiziErogati(BigDecimal idEnteConvenz) {
        ...
        String cdPsw = configurationHelper
                .getValoreParamApplicByApplic(CostantiDB.ParametroAppl.PSW_REPLICA_ORG);
        String timeoutString = configurationHelper.getValoreParamApplicByApplic(
                CostantiDB.ParametroAppl.TIMEOUT_CALCOLO_SERVIZI_EROGATI);

        try {

            /* Recupero il client per chiamata al WS */
            CalcoloServiziErogati client = IAMSoapClients.calcoloServiziErogatiClient(nmUserid,
                    cdPsw, url);
            ...
            if (client != null) {
                logger.info(
                        "Calcolo servizi erogati - Preparazione attivazione servizio per l'ente convenzionato {}",
                        idEnteConvenz);
                ...
                esito = false;
                logger.error("{} {}", CALCOLO_SERVIZI_EROGATI_RISPOSTA_NEGATIVA_ENTE_CONVENZIONATO,
                        idEnteConvenz);
            }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public IamEnteConvenzDaAllinea insertIamEnteConvenzDaAllinea(BigDecimal idEnteConvenz,
    ...
    /**
     * Data una struttura importata da XML, restituisce tutti i Tipi Unit\u00E0 Documentaria
     * associati ad essa nell'XML stesso
     *
    ...
    public DecTipoUnitaDocTableBean getTipiUdDaXmlImportato(UUID uuid) {
        ...
        try {
            if (!decTipoUnitaDocList.isEmpty()) {
                decTipoUnitaDocTableBean = (DecTipoUnitaDocTableBean) Transform
                        .entities2TableBean(decTipoUnitaDocList);
            }
        } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
     ...
     *
     * @param uuid           l'oggetto contenente la struttura importata da XML
     * @param nmTipoUnitaDoc il tipo unit\u00E0 documentaria scelto tra quelli presenti nell'XML
     *
     * @return il table bean rappresentante i Tipi Struttura Unit\u00E0 Documentaria trovati
     */
    public DecTipoStrutUnitaDocTableBean getTipiStrutUdDaXmlImportato(UUID uuid,
    ...
    /**
     * Data una struttura importata da XML, restituisce tutti i Tipi Fascicolo associati ad essa
     * nell'XML stesso
     *
     * @param uuid l'oggetto contenente la struttura importata da XML
     *
    ...
    public DecTipoFascicoloTableBean getTipiFascicoloDaXmlImportato(UUID uuid) {
        ...
        try {
            if (!tipoFascicoloList.isEmpty()) {
                tipoFascicoloTableBean = (DecTipoFascicoloTableBean) Transform
                        .entities2TableBean(tipoFascicoloList);
            }
        } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
                | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
            logger.error(e.getMessage(), e);
    ...
    /**
     * Data una struttura importata da XML e uno dei suoi Tipi Fascicolo selezionato tra quelli
     * presenti, restituisce tutti i Periodi tipo fascicolo associati al tipo fascicolo dell'XML
     * stesso
     *
     * @param uuid            l'oggetto contenente la struttura importata da XML
     * @param nmTipoFascicolo il tipo fascicolo scelto tra quelli presenti nell'XML
     *
     * @return il table bean rappresentante i Periodi tipo fascicolo trovati
     */
    public DecAaTipoFascicoloTableBean getAaTipoFascicoloDaXmlImportato(UUID uuid,
     ...
     * @param struttureDaElaborare                             l'insieme delle strutture su cui
     *                                                         eseguire l'importa parametri
     * @param nmTipoUnitaDocSelezionataDaCombo                 il tipo unit\u00E0 documentaria
     *                                                         selezionato nella combo
     * @param uuid                                             l'oggetto contenente la struttura
     *                                                         importata da XML
     ...
     *                                                         anche i criteri di raggruppamento
     * @param importareSistemiMigraz                           flag 1/0
     * @param existRegistriDaImportareConFlTipoSerieMultAlzato esiste registro da importare
     *
     * @param importareFormatiComponente                       flag
     *
     *
    ...
    public Object[] eseguiImportTipoUd(LogParam param, Map<BigDecimal, String> struttureDaElaborare,
        ...
        Map<String, String> strutErrorSisVers = new HashMap<>();
        ...
        for (Map.Entry<BigDecimal, String> entry : struttureDaElaborare.entrySet()) {
            ...
            try {
                ...
                if (e.getDescription()
                        .contains(" iniziare o terminare con caratteri di spaziatura")) {
                    ...
                    continue;
                } else if (e.getDescription().contains(" censito nel sistema")) {
                    strutErrorSisVers.put(nmStrutCorrente, e.getDescription());
                    // passa alla struttura successiva
                    continue;
                } else {
            ...
            DecTipoUnitaDoc tipoUnitaDoc = tipoUnitaDocHelper
                    .getDecTipoUnitaDocByName(nmTipoUnitaDocSelezionataDaCombo, idStrutCorrente);

            String errorMessageTipiSerie = "TIPISERIE;";
            if (!existRegistriDaImportareConFlTipoSerieMultAlzato) {
                // Eseguo, se esistono associazioni registro/tipo ud, la creazione dei tipi serie
                // standard
                if (tipoUnitaDoc.getDecTipoUnitaDocAmmessos() != null) {
                    for (DecTipoUnitaDocAmmesso decTipoUnitaDocAmmesso : tipoUnitaDoc
                            .getDecTipoUnitaDocAmmessos()) {
                        try {
                            BigDecimal idRegistroUnitaDoc = new BigDecimal(decTipoUnitaDocAmmesso
                                    .getDecRegistroUnitaDoc().getIdRegistroUnitaDoc());
                            BigDecimal idTipoUnitaDoc = new BigDecimal(decTipoUnitaDocAmmesso
                                    .getDecTipoUnitaDoc().getIdTipoUnitaDoc());
                            tipoSerieEjb.createTipoSerieStandardDaRegistroOTipoUdNewTx(param,
                                    idRegistroUnitaDoc, idTipoUnitaDoc);
                        } catch (ParerUserError e) {
                            if (!errorMessageTipiSerie.contains(e.getDescription())) {
            ...
            List<DecTipoUnitaDocAmmesso> associazioniRegistroTipoUd = tipoUnitaDoc
                    .getDecTipoUnitaDocAmmessos();
            for (DecTipoUnitaDocAmmesso associazione : associazioniRegistroTipoUd) {
                if (tipoSerieEjb.multipleDecRegistroUnitaDocInTipiSerie(new BigDecimal(
                        associazione.getDecRegistroUnitaDoc().getIdRegistroUnitaDoc()))) {
    ...
    public Object[] confermaImportazione(LogParam param, BigDecimal idStrutCorrente, UUID uuid,
        ...
        DecTipoUnitaDoc tipoUnitaDocExp = getDecTipoUnitaDocExpSelected(strutExp,
                nmTipoUnitaDocSelezionataDaCombo);

        /*
         * Dichiaro il tipo unità documentaria che verrà "decorato" e quindi importato a seguito dei
         * vari controlli
         */
        /////////////////////////////
        ...
        if (importareRegistri != null && importareRegistri.equals("1")) {
            registriImportati = importaRegistro(idStrutCorrente, tipoUnitaDocExp, tipoUnitaDoc,
                    registriDaLoggare);
        }

        // Controllo sui ModelliTipiSerie
        checkAndSetModelliTipiSerieStrutturaPerImportaParametri(idStrutCorrente, tipoUnitaDocExp,
                tipoUnitaDoc, registriImportati);

        // MAC #33057
        // Controllo sui ModelliXsdTipoUd
        checkAndSetModelliXsdTipiUdStrutturaPerImportaParametri(idStrutCorrente, tipoUnitaDocExp,
                tipoUnitaDoc);
        ...
        if (importareFormatiComponente != null && importareFormatiComponente.equals("1")) {
            ...
            strut.setDecTipoStrutDocs(
                    copiaStruttureEjb.determinaDecTipoStrutDocsPerImportaParametri(strutExp,
                            new Date(), true, true, strut, true));

            // EVO 27925: Allineo nei tipi componente (flag gestiti, idonei, deprecati)
            // eventualmente anche eventuali
            // formati che non erano presenti nella struttura dell'XML di import
            /* In base ai flag spuntati, inserisco i formati ammessi */
            List<DecTipoStrutDoc> tipoStrutDocList = tipoStrutDocHelper
                    .getDecTipoStrutDocList(idStrutCorrente, true);
        ...
        if (nmTipoStrutUnitaDocSelezionataDaCombo != null) {
            decTipoStrutUnitaDocExpList.add(getDecTipoStrutUnitaDocExpSelected(tipoUnitaDocExp,
                    nmTipoStrutUnitaDocSelezionataDaCombo));
        } // Altrimenti importo tutte quelle dell'XML
        else {
            decTipoStrutUnitaDocExpList = tipoUnitaDocExp.getDecTipoStrutUnitaDocs();
        }

        // MAC #38971: Controllo sui sistemi versanti
        checkPresenzaSistemiVersanti(decTipoStrutUnitaDocExpList);
        ...
        if (importareCriteri != null && importareCriteri.equals("1")) {
            importaCriteriTipoUd(idStrutCorrente, tipoUnitaDocExp, registriImportati,
                    tipiDocImportati, criteriRaggruppamentoDaLoggare);
        }
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_REGISTRO, registriDaLoggare, param.getNomePagina());
        // Log di tutti i Tipi documento
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_TIPO_DOCUMENTO, tipiDocumentoDaLoggare,
                param.getNomePagina());
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_FORMATO_AMMESSO, formatiDaLoggare,
                param.getNomePagina());
        // Logging dei Tipi Rappresentazioen Componente
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                param.getNomeUtente(), param.getNomeAzione(),
                SacerLogConstants.TIPO_OGGETTO_TIPO_RAPPRESENTAZIONE_COMPONENTE,
        ...
        /*
         * Eseguo la replica organizzazioni in ogni caso perché posso o meno aver inserito nuovi
         * registri e/o tipiDoc ma di sicuro ho inserito un nuovo tipo ud
         */
        IamOrganizDaReplic organizDaReplic = context.getBusinessObject(StruttureEjb.class)
                .insertStrutIamOrganizDaReplic(tipoUnitaDoc.getOrgStrut(),
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Object[] eseguiImportTipoFascicolo(LogParam param,
            Map<BigDecimal, String> struttureDaElaborare, UUID uuid,
        ...
        for (Map.Entry<BigDecimal, String> entry : struttureDaElaborare.entrySet()) {
            ...
            try {
                // L'import viene eseguito in una nuova transazione
                me.confermaImportazioneTipoFascicolo(param, idStrutCorrente, uuid,
                        nmTipoFascicoloSelezionatoDaMulti, aaTipoFascicoloSelezionatoDaCombo,
                        sovrascriviPeriodi.equals("1"));
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Object[] confermaImportazioneTipoFascicolo(LogParam param, BigDecimal idStrutCorrente,
            UUID uuid, String nmTipoFascicoloSelezionatoDaMulti,
            BigDecimal aaTipoFascicoloSelezionatoDaCombo, boolean sovrascriviPeriodo)
            throws ParerUserError {
        ...
        DecTipoFascicolo tipoFascicoloExp = getDecTipoFascicoloExpSelected(strutExp,
                nmTipoFascicoloSelezionatoDaMulti);
        ...
        if (tipoFascicoloDB == null) {
            ...
            /*
             * Dichiaro il tipo fascicolo che verrà "decorato" e quindi importato a seguito dei vari
             * controlli
             */
            ////////////////////////////
            ...
            if (aaTipoFascicoloSelezionatoDaCombo != null) {
                aaTipoFascicoloExpList.add(getDecAaTipoFascicoloExpSelected(tipoFascicoloExp,
                        aaTipoFascicoloSelezionatoDaCombo));
            } // Altrimenti importo tutti quelli dell'XML relativi al tipo fascicolo che sto
              // considerando
            else {
                aaTipoFascicoloExpList = tipoFascicoloExp.getDecAaTipoFascicolos();
            }
            importaAaTipoFascicolo(idStrutCorrente, aaTipoFascicoloExpList, tipoFascicolo,
                    (boolean) obj[1], sovrascriviPeriodo);
            ...
            /*
             * Eseguo la replica organizzazioni in ogni caso perché ho inserito un nuovo tipo
             * fascicolo
             */
            IamOrganizDaReplic organizDaReplic = context.getBusinessObject(StruttureEjb.class)
                    .insertStrutIamOrganizDaReplic(tipoFascicolo.getOrgStrut(),
                            ApplEnum.TiOperReplic.MOD);
    ...
    /**
     * Esegue l'importazione del solo Tipo fascicolo. Viene verificata l'esistenza dello stesso su
     * DB. Infine viene eseguito l'import dell'XSD relativo al tipoFascicolo
     *
     * @param idStrutCorrente  la struttura in cui mi trovo e nella quale sto facendo l'import
     * @param tipoFascicoloExp il tipo fascicolo selezionato nella combo
     *
     * @return un array di object di lunghezza 2 contenente nel primo campo l'oggetto tipoFascicolo
     *         "decorato" dopo la fase di importaTipoFascicolo e nel secondo il valore booleano
     *         relativo al fatto che il tipoFascicolo fosse gi\u00E0 presente su DB
     *
     * @throws ParerUserError errore generico
     */
    public Object[] importaTipoFascicolo(BigDecimal idStrutCorrente,
            DecTipoFascicolo tipoFascicoloExp) throws ParerUserError {
    ...
    /**
     * Esegue l'importazione della sola tipologia unit\u00E0 documentaria. Viene verificata
     * l'esistenza della stessa su DB e della categoria associata. Infine viene eseguito l'import
     * dell'XSD relativo al tipoUd
     *
     * @param idStrutCorrente la struttura in cui mi trovo e nella quale sto facendo l'import
     * @param tipoUnitaDocExp il tipo unit\u00E0 documentaria selezionato nella combo
     *
     * @return un array di object di lunghezza 2 contenente nel primo campo l'oggetto tipoUnitaDoc
     *         "decorato" dopo la fase di importaTipoUd e nel secondo il valore booleano relativo al
     *         fatto che il tipoUnitaDoc fosse gi\u00E0 presente su DB
     *
     * @throws ParerUserError errore generico
     */
    public Object[] importaTipoUd(BigDecimal idStrutCorrente, DecTipoUnitaDoc tipoUnitaDocExp)
            throws ParerUserError {
        Object[] obj = new Object[2];
        ...
        DecTipoUnitaDoc tipoUnitaDoc = tipoUnitaDocHelper
                .getDecTipoUnitaDocByName(tipoUnitaDocExp.getNmTipoUnitaDoc(), idStrutCorrente);
        ...
        if (tipoUnitaDoc == null) {
            ...
            DecVCalcTiServOnTipoUd calcTiServOnTipoUd = tipoUnitaDocHelper
                    .getDecVCalcTiServOnTipoUd(idStrutCorrente,
                            new BigDecimal(categTipoUnitaDoc.getIdCategTipoUnitaDoc()),
                            "CLASSE_ENTE");

            DecVCalcTiServOnTipoUd calcTiServOnTipoUd2 = tipoUnitaDocHelper
                    .getDecVCalcTiServOnTipoUd(idStrutCorrente,
                            new BigDecimal(categTipoUnitaDoc.getIdCategTipoUnitaDoc()),
                            "NO_CLASSE_ENTE");
            // Inserisco il tipo ud "nuovo"
            tipoUnitaDoc = inserisciTipoUd(idStrutCorrente, tipoUnitaDocExp, categTipoUnitaDoc,
                    calcTiServOnTipoUd, calcTiServOnTipoUd2);
        }
    ...
    public void importaValoreParamApplic(DecTipoUnitaDoc tipoUnitaDocExp,
            DecTipoUnitaDoc tipoUnitaDoc) {
        // Recupero la lista dei valori dei parametri sul tipo ud
        List<AplValoreParamApplic> valoreParamApplicExpList = tipoUnitaDocExp
                .getAplValoreParamApplics();
        ...
        for (AplValoreParamApplic valoreParamApplicExp : valoreParamApplicExpList) {
            ...
            String dsValoreParamApplic = configurationHelper.getAplValoreParamApplic(
                    valoreParamApplicExp.getAplParamApplic().getNmParamApplic(),
                    TiApparType.TIPO_UNITA_DOC.name(), null, null,
                    BigDecimal.valueOf(tipoUnitaDoc.getIdTipoUnitaDoc()), null);
    ...
    /**
     * Esegue l'importazione del registro. Viene verificata l'esistenza dello stesso su DB e della
     * relazione con il tipo UD (mediante la tabella DEC_TIPO_UNITA_DOC_AMMESSO)
     *
     * @param idStrutCorrente   la struttura in cui mi trovo e nella quale sto facendo l'import
     * @param tipoUnitaDoc      l'oggetto tipoUnitaDoc prima della fase di importaRegistro
     * @param tipoUnitaDocExp   i tipiUnitaDoc preso da XML @return, l'elenco dei registri importati
     *                          ex novo
     * @param registriDaLoggare i registri da loggare
     *
     * @return lista elementi di tipo CoppiaRegistri
     */
    public List<CoppiaRegistri> importaRegistro(BigDecimal idStrutCorrente,
            DecTipoUnitaDoc tipoUnitaDocExp, DecTipoUnitaDoc tipoUnitaDoc,
            Set<BigDecimal> registriDaLoggare) {
        List<DecTipoUnitaDocAmmesso> tipoUnitaDocAmmmessoExpList = tipoUnitaDocExp
                .getDecTipoUnitaDocAmmessos();
        List<CoppiaRegistri> registriImportati = new ArrayList<>();
        ...
        for (DecTipoUnitaDocAmmesso decTipoUnitaDocAmmessoExp : tipoUnitaDocAmmmessoExpList) {
            ...
            DecRegistroUnitaDoc registroUnitaDocExp = decTipoUnitaDocAmmessoExp
                    .getDecRegistroUnitaDoc();
            String cdRegistroUnitaDocExp = registroUnitaDocExp.getCdRegistroUnitaDoc();

            boolean esisteRegistroUd = registroHelper.existsRegistroUnitaDoc(idStrutCorrente,
                    cdRegistroUnitaDocExp);

            // Se il registro NON \u00E8 presente, lo creo e lo salvo su DB con relativi anni e
            // parti numero
            if (!esisteRegistroUd) {
            ...
            // Verifico se il registro \u00E8 inserito anche in DEC_TIPO_UNITA_DOC_AMMESSO
            // (relazione tipoUd-registro)
            String nmTipoUnitaDocExp = tipoUnitaDoc.getNmTipoUnitaDoc();
            DecTipoUnitaDocAmmesso tipoUnitaDocAmmesso = tipoUnitaDocHelper
                    .getDecTipoUnitaDocAmmessoByName(idStrutCorrente, nmTipoUnitaDocExp,
                            cdRegistroUnitaDocExp);
    ...
    public List<String> importaSistemiMigraz(BigDecimal idStrutCorrente, OrgStrut strutExp) {
        ...
        for (OrgUsoSistemaMigraz usoSistemaMigrazExp : usoSistemaMigrazExpList) {
            ...
            // ... recupero ogni sistema di migrazione presente nell'XML (tramite il tag
            // nmSistemaMigraz)
            ...
            AplSistemaMigraz sistemaMigraz = sistemaMigrazioneHelper
                    .getAplSistemaMigrazByName(nmSistemaMigrazExp);
            ...
            if (sistemaMigraz == null) {
                ...
                OrgStrut strutturaCorrente = tipoStrutDocHelper.findById(OrgStrut.class,
                        idStrutCorrente);
                usoSis = copiaStruttureEjb.cercaOrgUsoSistemaMigraz(sistemaMigraz,
                        strutturaCorrente);
                ...
                if (usoSis == null) {
                    ...
                    // Se ho inserito un'associazione struttura-sistema di migrazione, inserisco i
                    // dati spec
                    // UD
                    List<DecXsdDatiSpec> listUd = datiSpecHelper.retrieveDecXsdDatiSpecList(
                            BigDecimal.valueOf(strutExp.getIdStrut()),
                    ...
                    for (DecXsdDatiSpec ds : listUd) {
                        ...
                        DecXsdDatiSpec dsNew = copiaStruttureEjb.createDecXsdDatiSpec(ds,
                                strutturaCorrente);
    ...
    private class CoppiaRegistri {
        ...
        public CoppiaRegistri(DecRegistroUnitaDoc registroUnitaDoc,
                DecRegistroUnitaDoc registroUnitaDocExp) {
     ...
     *
     * @param idStrutCorrente                         la struttura in cui mi trovo e nella quale sto
     *                                                facendo l'import
     * @param tipoStrutUnitaDocExpList                la lista di tipi struttura ud da importare sui
     *                                                quali effettuare i controlli
     * @param tipoUnitaDoc                            l'oggetto tipo ud in fase di decorazione
     * @param tipoUnitaDocGiaPresente                 l'informazione se il tipoUnitaDoc fosse
     *                                                gi\u00E0 presente su DB
     * @param tipiDocumetoDaLoggare                   tipo documento da loggare
     * @param tipiStrutturaDocumetoDaLoggare          tipo struttura documento da loggare
     * @param formatiDaLoggare                        formati da loggare
    ...
    public List<CoppiaTipiDoc> importaTipoStrutUnitaDoc(BigDecimal idStrutCorrente,
        ...
        for (DecTipoStrutUnitaDoc tipoStrutUnitaDocExp : tipoStrutUnitaDocExpList) {
            ...
            } // Se invece il tipo unita doc NON \u00E8 stato importato (in quanto gi\u00E0 presente
              // su DB)
              // ricavo, se presente, il relativo tipoStrutUnitaDoc
            else {
                tipoStrutUnitaDoc = tipoStrutDocHelper.getDecTipoStrutUnitaDocByName(
                        idStrutCorrente, tipoUnitaDoc.getNmTipoUnitaDoc(),
                        tipoStrutUnitaDocExp.getNmTipoStrutUnitaDoc());
                // Se non presente, lo importo
                if (tipoStrutUnitaDoc == null) {
                    // Importo il tipo struttura unita doc
                    tipoStrutUnitaDoc = inserisciTipoStrutUnitaDoc(tipoStrutUnitaDocExp,
                            tipoUnitaDoc);
                }
            }
            ...
            for (DecTipoDocAmmesso tipoDocAmmessoExp : tipoStrutUnitaDocExp
                ...
                for (DecTipoStrutDocAmmesso tipoStrutDocAmmessoExp : tipoDocAmmessoExp
                        .getDecTipoDoc().getDecTipoStrutDocAmmessos()) {
                    gestisciTipoStrutDoc(idStrutCorrente, tipoStrutDocAmmessoExp, tipoDoc,
                            tipiStrutturaDocumetoDaLoggare,
            ...
            for (DecTipoStrutUdXsd tipoStrutUdXsdExp : tipoStrutUnitaDocExp
                    .getDecTipoStrutUdXsds()) {
                ...
                DecXsdDatiSpec xsdDatiSpec = datiSpecHelper.getDecXsdDatiSpec(idStrutCorrente,
                        ...
                        tipoStrutUnitaDocExp.getDecTipoUnitaDoc().getNmTipoUnitaDoc(), null, null,
                        null, xsdDatiSpecExp.getNmSistemaMigraz(),
                        xsdDatiSpecExp.getCdVersioneXsd());

                // Se il metadato NON è presente nella struttura: il record è importato.
                if (xsdDatiSpec == null) {
                    xsdDatiSpec = inserisciXsdDatiSpec(idStrutCorrente, xsdDatiSpecExp,
                            tipoUnitaDoc, null, null);
                }

                // Controllo se esiste già il record nella tabella DEC_TIPO_STRT_UD_SIS_VERS, ed in
                // caso lo inserisco
                String nmTipoStrutUnitaDoc = tipoStrutUdXsdExp.getDecTipoStrutUnitaDoc()
                        .getNmTipoStrutUnitaDoc();
                DecTipoStrutUdXsd tipoStrutUdXsd = datiSpecHelper.getDecTipoStrutUdXsdByName(
                        idStrutCorrente, nmTipoStrutUnitaDoc, xsdDatiSpecExp.getTiUsoXsd(),
                        xsdDatiSpecExp.getTiEntitaSacer(),
                        tipoStrutUnitaDocExp.getDecTipoUnitaDoc().getNmTipoUnitaDoc(),
            ...
            for (DecTipoStrutUdSisVer tipoStrutUdSisVersExp : tipoStrutUnitaDocExp
                ...
                // MAC #38971 --> non dovrebbe mai esserci il sistemaVersanete null per controllo
                // precedente
                // Se il sistema versante NON è presente nella struttura: il record è importato.
                // if (sistemaVersante == null) {
                // sistemaVersante = inserisciSistemaVersante(
                // tipoStrutUdSisVersExp.getAplSistemaVersante());
                // }

                // Controllo se esiste già il record nella tabella DEC_TIPO_STRT_UD_SIS_VERS, ed in
                // caso lo inserisco
                String nmTipoUnitaDoc = tipoStrutUdSisVersExp.getDecTipoStrutUnitaDoc()
                        .getDecTipoUnitaDoc().getNmTipoUnitaDoc();
                String nmTipoStrutUnitaDoc = tipoStrutUdSisVersExp.getDecTipoStrutUnitaDoc()
                        .getNmTipoStrutUnitaDoc();
                String nmSistemaVersante = tipoStrutUdSisVersExp.getAplSistemaVersante()
                        .getNmSistemaVersante();

                DecTipoStrutUdSisVer tipoStrutUdSisVers = tipoUnitaDocHelper
                        .getDecTipoStrutUdSisVersByName(idStrutCorrente, nmTipoUnitaDoc,
                                nmTipoStrutUnitaDoc, nmSistemaVersante);
            ...
            for (DecTipoStrutUdReg tipoStrutUdRegExp : tipoStrutUnitaDocExp
                    .getDecTipoStrutUdRegs()) {
                ...
                DecRegistroUnitaDoc registroUnitaDoc = registroHelper.getDecRegistroUnitaDocByName(
                        tipoStrutUdRegExp.getDecRegistroUnitaDoc().getCdRegistroUnitaDoc(),
                        idStrutCorrente);

                // Se il sistema versante NON è presente nella struttura: il record è importato.
                if (registroUnitaDoc == null) {
                    registroUnitaDoc = inserisciRegistro(idStrutCorrente,
                            tipoStrutUdRegExp.getDecRegistroUnitaDoc());
                }

                // Controllo se esiste già il record nella tabella, ed in caso lo inserisco
                String nmTipoUnitaDoc = tipoStrutUdRegExp.getDecTipoStrutUnitaDoc()
                        .getDecTipoUnitaDoc().getNmTipoUnitaDoc();
                String nmTipoStrutUnitaDoc = tipoStrutUdRegExp.getDecTipoStrutUnitaDoc()
                        .getNmTipoStrutUnitaDoc();
                String cdRegistroUnitaDoc = tipoStrutUdRegExp.getDecRegistroUnitaDoc()
                        .getCdRegistroUnitaDoc();

                DecTipoStrutUdReg tipoStrutUdReg = registroHelper.getDecTipoStrutUdRegByName(
                        idStrutCorrente, nmTipoUnitaDoc, nmTipoStrutUnitaDoc, cdRegistroUnitaDoc);
     ...
     *
     * @param idStrutCorrente          la struttura in cui mi trovo e nella quale sto facendo
     *                                 l'import
     * @param aaTipoFascicoloExpList   la lista di periodi tipo fascicolo da importare dal file XML
     *                                 sui quali effettuare i controlli
     * @param tipoFascicolo            l'oggetto tipo fascicolo in fase di decorazione
     * @param tipoFascicoloGiaPresente l'informazione se il tipoFascicolo sia gi\u00E0 presente su
     *                                 DB
     * @param sovrascriviPeriodo       flag per richiedere la sovrascrittura dei periodi tipo
     *                                 fascicolo
     */
    public void importaAaTipoFascicolo(BigDecimal idStrutCorrente,
            List<DecAaTipoFascicolo> aaTipoFascicoloExpList, DecTipoFascicolo tipoFascicolo,
            boolean tipoFascicoloGiaPresente, boolean sovrascriviPeriodo) {

        // Se il tipoFascicolo era già presente (quindi non lo sto inserendo da zero) e ho spuntato
        // sovrascriviPeriodo,
        ...
        for (DecAaTipoFascicolo aaTipoFascicoloExp : aaTipoFascicoloExpList) {
            ...
            // Se il tipo fascicolo da importare NON era presente nella struttura, importo tutto
            // RELATIVAMENTE AL
            // PERIODO DI VALIDITA':
            ...
            } // Se invece il tipo fascicolo NON \u00E8 stato importato (in quanto gi\u00E0 presente
              // su DB) ricavo, se
              // presente, il relativo aaTipoFascicolo dall'XML E LO INSERISCO SOLO SE IL FLAG
              // SOVRASCRIVI E' STATO
              // SETTATO A TRUE
            else {
    ...
    public DecUsoModelloXsdFasc inserisciUsoModelloXsdFasc(
            DecUsoModelloXsdFasc usoModelloXsdFascExp, DecAaTipoFascicolo aaTipoFascicolo) {
        ...
        DecModelloXsdFascicolo modelloXsdFascicoloExp = usoModelloXsdFascExp
                .getDecModelloXsdFascicolo();
        if (modelloXsdFascicoloExp != null) {
            long idAmbienteCorrente = aaTipoFascicolo.getDecTipoFascicolo().getOrgStrut()
                    .getOrgEnte().getOrgAmbiente().getIdAmbiente();

            DecModelloXsdFascicolo modelloXsdFascicolo = modelliFascicoliHelper
                    .getDecModelloXsdFascicolo(BigDecimal.valueOf(idAmbienteCorrente),
                            modelloXsdFascicoloExp.getTiModelloXsd().name(),
                            modelloXsdFascicoloExp.getTiUsoModelloXsd().name(),
                            modelloXsdFascicoloExp.getCdXsd());
    ...
    public DecTipoStrutUdXsd inserisciTipoStrutUdXsd(DecTipoStrutUnitaDoc tipoStrutUd,
            DecXsdDatiSpec xsdDatiSpec) {
    ...
    public boolean checkAnniSovrapposti(DecAaRegistroUnitaDoc decAaRegistroUnitaDocExp,
        ...
        int maxExp = decAaRegistroUnitaDocExp.getAaMaxRegistroUnitaDoc() != null
                ? decAaRegistroUnitaDocExp.getAaMaxRegistroUnitaDoc().intValue()
                : 2444;
        ...
        int maxDB = decAaRegistroUnitaDocDB.getAaMaxRegistroUnitaDoc() != null
                ? decAaRegistroUnitaDocDB.getAaMaxRegistroUnitaDoc().intValue()
                : 2444;
        return maxExp >= minDB && minExp <= maxDB;
    }

    public DecTipoUnitaDoc getDecTipoUnitaDocExpSelected(OrgStrut strutExp,
            String nmTipoUnitaDocSelezionataDaCombo) {
    ...
    public DecTipoStrutUnitaDoc getDecTipoStrutUnitaDocExpSelected(DecTipoUnitaDoc tipoUnitaDocExp,
        ...
        List<DecTipoStrutUnitaDoc> tipoStrutUnitaDocExpList = tipoUnitaDocExp
                .getDecTipoStrutUnitaDocs();
        for (DecTipoStrutUnitaDoc tipoStrutUnitaDocExp : tipoStrutUnitaDocExpList) {
            if (tipoStrutUnitaDocExp.getNmTipoStrutUnitaDoc()
                    .equals(nmTipoStrutUnitaDocSelezionataDaCombo)) {
    ...
    public DecAaTipoFascicolo getDecAaTipoFascicoloExpSelected(DecTipoFascicolo tipoFascicoloExp,
        ...
        for (DecAaTipoFascicolo aaTipoFascicoloExp : aaTipoFascicoloExpList) {
            if (aaTipoFascicoloExp.getAaIniTipoFascicolo()
                    .compareTo(aaTipoFascicoloSelezionatoDaCombo) == 0) {
    ...
    /**
     * Esegue il salvataggio su DB di DecAaRegistroUnitaDoc. Recupera l'anno dall'XML e crea il
     * record su DB
     *
     * @param aaRegistroUnitaDocExp l'anno registro presente nell'XML
     * @param registroUnitaDoc      il record del registro, gi\u00E0 presente su DB, nel quale
     *                              associare l'anno @return, il record relativo all'anno appena
     *                              creato
     *
     * @return DecAaRegistroUnitaDoc
     */
    public DecAaRegistroUnitaDoc inserisciAaRegistroUnitaDoc(
            DecAaRegistroUnitaDoc aaRegistroUnitaDocExp, DecRegistroUnitaDoc registroUnitaDoc) {
        DecAaRegistroUnitaDoc aaRegistroUnitaDoc = new DecAaRegistroUnitaDoc();
        aaRegistroUnitaDoc
                .setAaMinRegistroUnitaDoc(aaRegistroUnitaDocExp.getAaMinRegistroUnitaDoc());
        aaRegistroUnitaDoc
                .setAaMaxRegistroUnitaDoc(aaRegistroUnitaDocExp.getAaMaxRegistroUnitaDoc());
    ...
    /**
     * Esegue il salvataggio su DB di DecParteNumeroRegistro. Recupera l'anno dall'XML e crea il
     * record su DB
     *
     * @param parteNumeroRegistroExp la parte numero presente nell'XML
     * @param aaRegistroUnitaDoc     il record dell'anno, gi\u00E0 presente su DB, nel quale
     *                               associare la parte numero @return, il record relativo alla
     *                               parte numero appena creata
     *
     * @return DecParteNumeroRegistro
     */
    public DecParteNumeroRegistro inserisciParteNumeroRegistro(
            DecParteNumeroRegistro parteNumeroRegistroExp,
            DecAaRegistroUnitaDoc aaRegistroUnitaDoc) {
        DecParteNumeroRegistro parteNumeroRegistro = new DecParteNumeroRegistro();
        parteNumeroRegistro
                .setNmParteNumeroRegistro(parteNumeroRegistroExp.getNmParteNumeroRegistro());
        parteNumeroRegistro
                .setDsParteNumeroRegistro(parteNumeroRegistroExp.getDsParteNumeroRegistro());
        parteNumeroRegistro
                .setNiParteNumeroRegistro(parteNumeroRegistroExp.getNiParteNumeroRegistro());
    ...
    /**
     * Esegue il salvataggio su DB di DecParteNumeroFascicolo. Recupera l'anno dall'XML e crea il
     * record su DB
     *
     * @param parteNumeroFascicoloExp la parte numero presente nell'XML
     * @param aaTipoFascicolo         il record dell'anno, gi\u00E0 presente su DB, nel quale
     *                                associare la parte numero
     *
     * @return DecParteNumeroFascicolo, il record relativo alla parte numero appena creata
     */
    public DecParteNumeroFascicolo inserisciParteNumeroFascicolo(
            DecParteNumeroFascicolo parteNumeroFascicoloExp, DecAaTipoFascicolo aaTipoFascicolo) {
    ...
    /**
     * Inserisce su DB il record relativo al Tipo Struttura Unit\u00E0 Documentaria partendo da
     * quella passata come parametro dell'oggetto tipoStrutUd recuperato dall'XML
     *
     * @param tipoStrutUnitaDocExp il tipo struttura unit\u00E0 doc recuperato dall'XML
     * @param tipoUnitaDoc         il tipo unità doc in fase di "decorazione" al quale associare il
     *                             tipo struttura ud
     *
     * @return il tipo struttura unit\u00E0 documentaria inserito
     */
    public DecTipoStrutUnitaDoc inserisciTipoStrutUnitaDoc(
            DecTipoStrutUnitaDoc tipoStrutUnitaDocExp, DecTipoUnitaDoc tipoUnitaDoc) {
        ...
        tipoStrutUnitaDoc
                .setAaMaxTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getAaMaxTipoStrutUnitaDoc());
        tipoStrutUnitaDoc
                .setAaMinTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getAaMinTipoStrutUnitaDoc());
        tipoStrutUnitaDoc
                .setDsAnnoTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getDsAnnoTipoStrutUnitaDoc());
        tipoStrutUnitaDoc
                .setDsDataTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getDsDataTipoStrutUnitaDoc());
        tipoStrutUnitaDoc
                .setDsNumeroTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getDsNumeroTipoStrutUnitaDoc());
        tipoStrutUnitaDoc
                .setDsOggTipoStrutUnitaDoc(tipoStrutUnitaDocExp.getDsOggTipoStrutUnitaDoc());
    ...
    /**
     * Inserisce su DB il record relativo al Periodo Tipo Fascicolo partendo da quello passato come
     * parametro dell'oggetto aaTipoFascicolo recuperato dall'XML
     *
     * @param aaTipoFascicoloExp il periodo tipo fascicolo recuperato dall'XML
     * @param tipoFascicolo      il tipo fascicolo in fase di "decorazione" al quale associare il
     *                           periodo tipo fascicolo
     *
    ...
    /**
     * Inserisce su DB una nuova categoria tipo unit\u00E0 documentaria partendo da quella passata
     * come parametro dell'oggetto categoria recuperato dall'XML
     *
     * @param categTipoUnitaDocExp l'oggetto recuperato dall'XML
     *
     * @return la nuova categoria inserita su DB
     */
    public DecCategTipoUnitaDoc inserisciCategTipoUnitaDoc(
            DecCategTipoUnitaDoc categTipoUnitaDocExp) {
     ...
     *
     * @param tipoUnitaDoc     tipo unita doc
     * @param registroUnitaDoc registro unita doc
     *
     * @return il record relativo al tipoUnitaDocAmmesso inserito
     */
    ...
    public DecXsdDatiSpec inserisciXsdDatiSpec(BigDecimal idStrutCorrente,
            DecXsdDatiSpec xsdDatiSpecExp, DecTipoUnitaDoc tipoUnitaDoc, DecTipoDoc tipoDoc,
    ...
    public DecTipoStrutDoc inserisciTipoStrutDoc(BigDecimal idStrutCorrente,
            DecTipoStrutDoc tipoStrutDocExp) {
    ...
    public DecTipoStrutDocAmmesso inserisciTipoStrutDocAmmesso(DecTipoDoc tipoDoc,
            DecTipoStrutDoc tipoStrutDoc) {
    ...
    public DecTipoCompDoc inserisciTipoCompDoc(BigDecimal idStrutCorrente, String nmTipoStrutDoc,
        ...
        DecTipoStrutDoc tipoStrutDoc = tipoStrutDocHelper.getDecTipoStrutDocByName(nmTipoStrutDoc,
                idStrutCorrente);
    ...
    public DecFormatoFileDoc inserisciFormatoFileDoc(BigDecimal idStrutCorrente,
            DecFormatoFileDoc formatoFileDocExp) {
    ...
    public DecTipoRapprComp inserisciTipoRapprComp(BigDecimal idStrutCorrente,
            DecTipoRapprComp tipoRapprCompExp) {
        ...
        if (tipoRapprCompExp.getDecFormatoFileStandard() != null) {
            if (copiaStruttureEjb.existsAllFormatiStandard(
                ...
                formatoFileStandard = gestisciFormatoFileStandard(
        ...
        if (tipoRapprCompExp.getDecFormatoFileDocCont() != null) {
            if (copiaStruttureEjb.existsAllFormatiStandard(
                    tipoRapprCompExp.getDecFormatoFileDocCont().getNmFormatoFileDoc())) {
        ...
        if (tipoRapprCompExp.getDecFormatoFileDocConv() != null) {
            if (copiaStruttureEjb.existsAllFormatiStandard(
                    tipoRapprCompExp.getDecFormatoFileDocConv().getNmFormatoFileDoc())) {
        ...
        if (tipoRapprCompExp.getDecTrasformTipoRapprs() != null) {
            for (DecTrasformTipoRappr trasformTipoRapprExp : tipoRapprCompExp
                    .getDecTrasformTipoRapprs()) {
                ...
                trasformTipoRappr
                        .setCdVersioneTrasform(trasformTipoRapprExp.getCdVersioneTrasform());
                trasformTipoRappr.setDecImageTrasforms(new ArrayList<>());
                trasformTipoRappr.setDecTipoRapprComp(tipoRapprComp);
                trasformTipoRappr
                        .setDsHashFileTrasform(trasformTipoRapprExp.getDsHashFileTrasform());
                ...
                trasformTipoRappr
                        .setIdTrasformTipoRappr(trasformTipoRapprExp.getIdTrasformTipoRappr());
                trasformTipoRappr.setNmTrasform(trasformTipoRapprExp.getNmTrasform());
                trasformTipoRappr
                        .setTiStatoFileTrasform(trasformTipoRapprExp.getTiStatoFileTrasform());
    ...
    public DecFormatoFileDoc gestisciFormatoFileDoc(BigDecimal idStrutCorrente,
            DecFormatoFileDoc formatoFileDocExp) {
        DecFormatoFileDoc formatoFileDoc = formatoFileDocHelper.getDecFormatoFileDocByName(
                formatoFileDocExp.getNmFormatoFileDoc(), idStrutCorrente);
        ...
        if (formatoFileDoc == null) {
            ...
            for (DecUsoFormatoFileStandard usoFormatoFileStandardExp : usoFormatoFileStandardExpList) {
                ...
                inserisciUsoFormatoFileStandard(usoFormatoFileStandardExp, formatoFileDoc,
                        formatoFileStandard);
            }
        }
        return formatoFileDoc;
    ...
    public DecFormatoFileStandard gestisciFormatoFileStandard(
    ...
    public DecTipoRapprAmmesso inserisciTipoRapprAmmesso(DecTipoCompDoc tipoCompDoc,
            DecTipoRapprComp tipoRapprComp) {
    ...
    public DecFormatoFileStandard inserisciFormatoFileStandard(
            DecFormatoFileStandard formatoFileStandardExp) {
        DecFormatoFileStandard formatoFileStandard = new DecFormatoFileStandard();
        formatoFileStandard
                .setNmFormatoFileStandard(formatoFileStandardExp.getNmFormatoFileStandard());
        formatoFileStandard
                .setDsFormatoFileStandard(formatoFileStandardExp.getDsFormatoFileStandard());
    ...
    public void inserisciRegola(BigDecimal idStrutCorrente,
            OrgRegolaValSubStrut regolaValSubStrutExp, DecTipoUnitaDoc tipoUnitaDoc,
            DecTipoDoc tipoDoc) throws ParerUserError {
        Long idRegolaValSubStrut = subStrutEjb.saveRegolaSubStrut(null,
                new BigDecimal(tipoUnitaDoc.getIdTipoUnitaDoc()),
                new BigDecimal(tipoDoc.getIdTipoDoc()), new Date(),
                regolaValSubStrutExp.getDtSoppres());
        OrgRegolaValSubStrut regolaValSubStrut = struttureHelper
                .findById(OrgRegolaValSubStrut.class, idRegolaValSubStrut);
        // Inserisci i campi
        for (OrgCampoValSubStrut campoValSubStrutExp : regolaValSubStrutExp
                .getOrgCampoValSubStruts()) {

            BigDecimal idRecord = null;
            boolean skip = false;
            CostantiDB.TipoCampo campo = CostantiDB.TipoCampo
                    .valueOf(campoValSubStrutExp.getTiCampo());
            switch (campo) {
            case DATO_SPEC_DOC_PRINC:
            case DATO_SPEC_UNI_DOC:
                DecAttribDatiSpec attribDatiSpec = datiSpecHelper.getDecAttribDatiSpecByName(
                        idStrutCorrente,
                        campoValSubStrutExp.getDecAttribDatiSpec().getTiUsoAttrib(),
                        campoValSubStrutExp.getDecAttribDatiSpec().getTiEntitaSacer(),
                        campoValSubStrutExp.getDecAttribDatiSpec().getDecTipoUnitaDoc()
                                .getNmTipoUnitaDoc(),
                        null, null, null,
                ...
                break;
            case SUB_STRUT:
                List<OrgSubStrut> subStrut = subStrutHelper.getOrgSubStrut(
                        campoValSubStrutExp.getOrgSubStrut().getNmSubStrut(), idStrutCorrente);
                if (!subStrut.isEmpty()) {
            ...
            if (!skip) {
                subStrutEjb.saveCampoSubStrut(null, regolaValSubStrut,
                        campoValSubStrutExp.getTiCampo(), campoValSubStrutExp.getNmCampo(),
                        idRecord);
            }
        }
    }

    public DecCriterioRaggr inserisciCriterio(BigDecimal idStrutCorrente,
            DecCriterioRaggr criterioRaggrExp, Set<BigDecimal> criteriRaggruppamentoDaLoggare) {
    ...
    public DecCriterioRaggrFasc inserisciCriterioFasc(BigDecimal idStrutCorrente,
            DecCriterioRaggrFasc criterioRaggrFascExp,
            Set<BigDecimal> criteriRaggruppamentoFascDaLoggare) {
        ...
        criterioRaggrFasc
                .setFlCriterioRaggrStandard(criterioRaggrFascExp.getFlCriterioRaggrStandard());
        ...
        criteriRaggruppamentoFascDaLoggare
                .add(new BigDecimal(criterioRaggrFasc.getIdCriterioRaggrFasc()));
    ...
    private void inserisciFiltroMultiploTipoUd(BigDecimal idStrutCorrente,
        ...
        criterioFiltroMultiplo
                .setTiEsitoVerifFirme(criterioFiltroMultiploExp.getTiEsitoVerifFirme());
        DecTipoUnitaDoc tipoUnitaDoc = tipoUnitaDocHelper.getDecTipoUnitaDocByName(
                criterioFiltroMultiploExp.getDecTipoUnitaDoc().getNmTipoUnitaDoc(),
                idStrutCorrente);
    ...
    private void inserisciFiltroMultiploTipoDoc(BigDecimal idStrutCorrente,
        ...
        DecTipoDoc tipoDoc = tipoDocHelper.getDecTipoDocByName(
                criterioFiltroMultiploExp.getDecTipoDoc().getNmTipoDoc(), idStrutCorrente);
    ...
    private void inserisciFiltroMultiploRegistroUnitaDoc(BigDecimal idStrutCorrente,
        ...
        criterioFiltroMultiplo
                .setTiEsitoVerifFirme(criterioFiltroMultiploExp.getTiEsitoVerifFirme());
        DecRegistroUnitaDoc registroUnitaDoc = registroHelper.getDecRegistroUnitaDocByName(
                criterioFiltroMultiploExp.getDecRegistroUnitaDoc().getCdRegistroUnitaDoc(),
                idStrutCorrente);
    ...
    public DecAttribDatiSpec inserisciAttribDatiSpec(BigDecimal idStrutCorrente,
            DecAttribDatiSpec attribDatiSpecExp, DecTipoUnitaDoc tipoUnitaDoc, DecTipoDoc tipoDoc,
            DecTipoCompDoc tipoCompDoc, Constants.TipoEntitaSacer tiEntitaSacer) {
    ...
    public DecXsdAttribDatiSpec inserisciXsdAttribDatiSpec(DecXsdDatiSpec xsdDatiSpec,
            DecAttribDatiSpec attribDatiSpec, BigDecimal niOrdAttrib, String dsAttribDatiSpec) {
    ...
    /**
     * Creo un nuovo oggetto tipo unit\u00E0 documentaria partendo dallo stesso ricavato dall'XML e
     * aggiungendo la categoria e i valori sui parametri
     *
     * @param idStrutCorrente     id struttura corrente
     * @param tipoUnitaDocExp     tipo unita doc
     * @param categTipoUnitaDoc   categoria tipo unita doc
     * @param calcTiServOnTipoUd  calcolo tipo servizio
     * @param calcTiServOnTipoUd2 calcolo tipo servizio
     *
     * @return DecTipoUnitaDoc decodifica tipo unita doc
     */
    public DecTipoUnitaDoc inserisciTipoUd(BigDecimal idStrutCorrente,
            DecTipoUnitaDoc tipoUnitaDocExp, DecCategTipoUnitaDoc categTipoUnitaDoc,
            DecVCalcTiServOnTipoUd calcTiServOnTipoUd, DecVCalcTiServOnTipoUd calcTiServOnTipoUd2) {
        ...
        if (calcTiServOnTipoUd != null) {
            OrgTipoServizio tipoServizioConserv = calcTiServOnTipoUd
                    .getIdTipoServizioConserv() != null
                            ? struttureHelper.findById(OrgTipoServizio.class,
                                    calcTiServOnTipoUd.getIdTipoServizioConserv())
                            : null;
            OrgTipoServizio tipoServizioAttiv = calcTiServOnTipoUd.getIdTipoServizioAttiv() != null
                    ? struttureHelper.findById(OrgTipoServizio.class,
                            calcTiServOnTipoUd.getIdTipoServizioAttiv())
                    : null;
        ...
        if (calcTiServOnTipoUd2 != null) {
            OrgTipoServizio tipoServizioConserv = calcTiServOnTipoUd2
                    .getIdTipoServizioConserv() != null
                            ? struttureHelper.findById(OrgTipoServizio.class,
            ...
            OrgTipoServizio tipoServizioAttiv = calcTiServOnTipoUd2.getIdTipoServizioAttiv() != null
                    ? struttureHelper.findById(OrgTipoServizio.class,
                            calcTiServOnTipoUd2.getIdTipoServizioAttiv())
                    : null;
     ...
     *
     * @param idStrutCorrente  id struttura corrente
     * @param tipoFascicoloExp tipo fascicolo
     *
     * @return DecTipoFascicolo decodifica tipo fascicolo
     */
    public DecTipoFascicolo inserisciTipoFascicolo(BigDecimal idStrutCorrente,
            DecTipoFascicolo tipoFascicoloExp) {
    ...
    public AplValoreParamApplic inserisciValoreParamApplic(String nmParamApplic,
            String dsValoreParamApplicExp, DecTipoUnitaDoc tipoUnitaDoc) {
    ...
    /**
     * Creo un nuovo oggetto registro unit\u00E0 documentaria partendo dallo stesso ricavato
     * dall'XML
     *
     * @param idStrutCorrente     id struttura corrente
     * @param registroUnitaDocExp registro unita doc
     *
     * @return DecRegistroUnitaDoc registro unita doc
     */
    public DecRegistroUnitaDoc inserisciRegistro(BigDecimal idStrutCorrente,
            DecRegistroUnitaDoc registroUnitaDocExp) {
        ...
        registroUnitaDoc.setFlTipoSerieMult(registroUnitaDocExp.getFlTipoSerieMult() != null
                ? registroUnitaDocExp.getFlTipoSerieMult()
                : "0");
        ...
        // Aggiungo i nuovi campi sulla Consigurazione Serie
        registroUnitaDoc
                .setFlCreaTipoSerieStandard(registroUnitaDocExp.getFlCreaTipoSerieStandard());
    ...
    public void importaRegole(BigDecimal idStrutCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
            DecTipoUnitaDoc tipoUnitaDoc) throws Exception {
        List<OrgRegolaValSubStrut> regolaValSubStrutExpList = tipoUnitaDocExp
                .getOrgRegolaValSubStruts();
        ...
        for (OrgRegolaValSubStrut regolaValSubStrutExp : regolaValSubStrutExpList) {
            ...
            if (tipoDoc != null) {
                ...
                // Verifico se il tipo doc \u00E8 inserito anche in ORG_REGOLA_VAL_SUB_STRUT
                // (relazione tipoUd-tipoDoc)
                String nmTipoUnitaDocExp = regolaValSubStrutExp.getDecTipoUnitaDoc()
                        .getNmTipoUnitaDoc();

                boolean existsRegola = subStrutHelper.existOrgRegolaSubStrut(idStrutCorrente,
                        nmTipoUnitaDocExp, nmTipoDocExp);

                // Se la regola non \u00E8 presente, la creo e la salvo su DB, altrimenti non faccio
                // nulla
                if (!existsRegola) {
                    try {
                        inserisciRegola(idStrutCorrente, regolaValSubStrutExp, tipoUnitaDoc,
                                tipoDoc);
                    } catch (ParerUserError e) {
    ...
    public void importaCriteriTipoUd(BigDecimal idStrutCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
        ...
        // Ricavo i criteri di raggruppamento ATTIVI per il tipo unit\u00E0 documentaria da
        // importare
        // e recupero i criteri coinvolti dal tipo unita doc
        for (DecCriterioFiltroMultiplo criterioFiltroMultiploExp : tipoUnitaDocExp
        ...
        // Ricavo i criteri di raggruppamento ATTIVI per i registri importati e reupero i criteri
        // coinvolti da ogni
        // registro importato
        for (CoppiaRegistri registroImportato : registriImportati) {
            for (DecCriterioFiltroMultiplo criterioFiltroMultiploExp : registroImportato
                    .getRegistroUnitaDocExp().getDecCriterioFiltroMultiplos()) {
                if (criterioFiltroMultiploExp.getDecCriterioRaggr().getDtSoppres()
                        .after(new Date())) {
                    criteriCoinvoltiExp.add(criterioFiltroMultiploExp.getDecCriterioRaggr());
                }
            }
        ...
        // Ricavo i criteri di raggruppamento ATTIVI per i tipi doc importati e reupero i criteri
        // coinvolti da ogni tipo
        // doc importato
        for (CoppiaTipiDoc tipoDocImportato : tipiDocImportati) {
            for (DecCriterioFiltroMultiplo criterioFiltroMultiploExp : tipoDocImportato
                    .getTipoDocExp().getDecCriterioFiltroMultiplos()) {
                if (criterioFiltroMultiploExp.getDecCriterioRaggr().getDtSoppres()
                        .after(new Date())) {
                    criteriCoinvoltiExp.add(criterioFiltroMultiploExp.getDecCriterioRaggr());
                }
        ...
        // anche ad altri tipi ud, registri o tipi doc e nel caso verifico che siano presenti per la
        // struttura
        for (DecCriterioRaggr criterioCoinvoltoExp : criteriCoinvoltiExp) {
            ...
            DecCriterioRaggr criterioRaggr = crHelper
                    .getDecCriterioRaggrByStrutturaCorrenteAndCriterio(idStrutCorrente,
                            criterioCoinvoltoExp.getNmCriterioRaggr());

            // Se il criterio non esiste su DB, pu\u00E8 darsi che vada inserito... dipender\u00E8
            // dai tipiUd, tipiDoc,
            // registri coinvolti
            if (criterioRaggr == null) {
                ...
                List<DecCriterioFiltroMultiplo> criteriFiltriMultipliCoinvolti = criterioCoinvoltoExp
                        .getDecCriterioFiltroMultiplos();
                // Per ognuno di essi, verifico se il tipoUd, il registro o il tipoDoc, \u00E8
                // presente per quella
                // struttura
                for (DecCriterioFiltroMultiplo criterioFiltroMultiploCoinvolto : criteriFiltriMultipliCoinvolti) {

                    if (criterioFiltroMultiploCoinvolto.getTiFiltroMultiplo()
                            .equals("TIPO_UNI_DOC")) {
                        ...
                        DecTipoUnitaDoc tipoUd = tipoUnitaDocHelper
                                .getDecTipoUnitaDocByName(nmTipoUnitaDoc, idStrutCorrente);
                        if (tipoUd == null) {
                            elementoMancante = true;
                            break;
                        }
                    } else if (criterioFiltroMultiploCoinvolto.getTiFiltroMultiplo()
                            .equals("TIPO_DOC")) {
                        String nmTipoDoc = criterioFiltroMultiploCoinvolto.getDecTipoDoc()
                                .getNmTipoDoc();
                        // Verifico se il tipoDoc esiste per la struttura coinvolta
                        DecTipoDoc tipoDoc = tipoDocHelper.getDecTipoDocByName(nmTipoDoc,
                                idStrutCorrente);
                        if (tipoDoc == null) {
                            elementoMancante = true;
                            break;
                            ...
                            .equals("REGISTRO_UNI_DOC")) {
                        String cdRegistroUnitaDoc = criterioFiltroMultiploCoinvolto
                                .getDecRegistroUnitaDoc().getCdRegistroUnitaDoc();
                        // Verifico se il registroUd esiste per la struttura coinvolta
                        DecRegistroUnitaDoc registroUd = registroHelper
                                .getDecRegistroUnitaDocByName(cdRegistroUnitaDoc, idStrutCorrente);
                        if (registroUd == null) {
                            elementoMancante = true;
                            break;
                        }
            ...
            if (!elementoMancante) {
                ...
                for (DecCriterioFiltroMultiplo criterioFiltroMultiploExp : criterioCoinvoltoExp
                    ...
                    if (criterioFiltroMultiploExp.getTiFiltroMultiplo().equals("TIPO_UNI_DOC")) {
                        inserisciFiltroMultiploTipoUd(idStrutCorrente, criterioFiltroMultiploExp,
                                criterioRaggr);
                    } else if (criterioFiltroMultiploExp.getTiFiltroMultiplo()
                            .equals("REGISTRO_UNI_DOC")) {
                        inserisciFiltroMultiploRegistroUnitaDoc(idStrutCorrente,
                                criterioFiltroMultiploExp, criterioRaggr);
                    } else if (criterioFiltroMultiploExp.getTiFiltroMultiplo().equals("TIPO_DOC")) {
                        inserisciFiltroMultiploTipoDoc(idStrutCorrente, criterioFiltroMultiploExp,
                                criterioRaggr);
                    }
                }
            }
    ...
    public void importaCriteriRaggrFasc(BigDecimal idStrutCorrente,
            DecTipoFascicolo tipoFascicoloExp, DecTipoFascicolo tipoFascicolo,
            Set<BigDecimal> criteriRaggruppamentoFascDaLoggare) {
        ...
        for (DecSelCriterioRaggrFasc selOld : tipoFascicoloExp.getDecSelCriterioRaggrFascicolos()) {
            ...
            if (selOld.getTiSel().equals(
                ...
                DecCriterioRaggrFasc criterioFasc = crfHelper
                        .getDecCriterioRaggrFascByStrutturaCorrenteAndCriterio(idStrutCorrente,
                                selOld.getDecCriterioRaggrFasc().getNmCriterioRaggr());

                // Se il criterio è nullo, lo inserisco
                if (criterioFasc == null) {
                    criterioFasc = inserisciCriterioFasc(idStrutCorrente,
                            selOld.getDecCriterioRaggrFasc(), criteriRaggruppamentoFascDaLoggare);
                    // Da fotografare in XML se inseriti ex-novo
                    criteriRaggruppamentoFascDaLoggare
                            .add(new BigDecimal(criterioFasc.getIdCriterioRaggrFasc()));
                }

                // Inserisco la relazione tra tipoFascicolo e criterioRaggrFascicolo
                DecSelCriterioRaggrFasc selNew = tipoFascicoloHelper.getDecSelCriterioRaggrFasc(
                        idStrutCorrente, criterioFasc.getNmCriterioRaggr(),
                        tipoFascicoloExp.getNmTipoFascicolo());
                if (selNew == null) {
    ...
    public DecXsdDatiSpec inserisciXsdTipoUd(BigDecimal idStrutCorrente,
            DecXsdDatiSpec xsdDatiSpecExp, DecTipoUnitaDoc tipoUnitaDoc) throws ParerUserError {

        datiSpecEjb.parseStringaXsd(xsdDatiSpecExp.getBlXsd());

        // Inserisco il record in DEC_XSD_DATI_SPEC
        DecXsdDatiSpec xsdDatiSpec = inserisciXsdDatiSpec(idStrutCorrente, xsdDatiSpecExp,
                tipoUnitaDoc, null, null);
        for (DecXsdAttribDatiSpec xsdAttribDatiSpecExp : xsdDatiSpecExp
            ...
            DecAttribDatiSpec attribDatiSpec = datiSpecHelper.getDecAttribDatiSpecByName(
                    idStrutCorrente, attribDatiSpecExp.getTiUsoAttrib(),
                    attribDatiSpecExp.getTiEntitaSacer(),
            ...
            if (attribDatiSpec == null) {
                attribDatiSpec = inserisciAttribDatiSpec(idStrutCorrente, attribDatiSpecExp,
                        tipoUnitaDoc, null, null, Constants.TipoEntitaSacer.UNI_DOC);
            }

            // Inserisco la relazione xsdDatiSpec-attribDatiSpec
            inserisciXsdAttribDatiSpec(xsdDatiSpec, attribDatiSpec,
                    xsdAttribDatiSpecExp.getNiOrdAttrib(),
                    xsdAttribDatiSpecExp.getDsAttribDatiSpec());
        }
        return xsdDatiSpec;
    }

    public DecXsdDatiSpec inserisciXsdTipoDoc(BigDecimal idStrutCorrente,
            DecXsdDatiSpec xsdDatiSpecExp, DecTipoDoc tipoDoc) {
        // Inserisco il record in DEC_XSD_DATI_SPEC
        DecXsdDatiSpec xsdDatiSpec = inserisciXsdDatiSpec(idStrutCorrente, xsdDatiSpecExp, null,
                tipoDoc, null);
        for (DecXsdAttribDatiSpec xsdAttribDatiSpecExp : xsdDatiSpecExp
                .getDecXsdAttribDatiSpecs()) {
            DecAttribDatiSpec attribDatiSpecExp = xsdAttribDatiSpecExp.getDecAttribDatiSpec();
            ...
            DecAttribDatiSpec attribDatiSpec = datiSpecHelper.getDecAttribDatiSpecByName(
                    idStrutCorrente, attribDatiSpecExp.getTiUsoAttrib(),
                    attribDatiSpecExp.getTiEntitaSacer(), null,
                    attribDatiSpecExp.getDecTipoDoc().getNmTipoDoc(), null, null,
                    attribDatiSpecExp.getNmAttribDatiSpec());
            // Inserisco il record in DEC_ATTRIB_DATI_SPEC
            if (attribDatiSpec == null) {
                attribDatiSpec = inserisciAttribDatiSpec(idStrutCorrente, attribDatiSpecExp, null,
                        tipoDoc, null, Constants.TipoEntitaSacer.DOC);
            }
    ...
    public DecXsdDatiSpec inserisciXsdTipoCompDoc(BigDecimal idStrutCorrente,
            DecXsdDatiSpec xsdDatiSpecExp, DecTipoCompDoc tipoCompDoc) {
        // Inserisco il record in DEC_XSD_DATI_SPEC
        DecXsdDatiSpec xsdDatiSpec = inserisciXsdDatiSpec(idStrutCorrente, xsdDatiSpecExp, null,
                null, tipoCompDoc);
        for (DecXsdAttribDatiSpec xsdAttribDatiSpecExp : xsdDatiSpecExp
            ...
            DecAttribDatiSpec attribDatiSpec = datiSpecHelper.getDecAttribDatiSpecByName(
                    idStrutCorrente, attribDatiSpecExp.getTiUsoAttrib(),
                    attribDatiSpecExp.getTiEntitaSacer(), null, null,
                    attribDatiSpecExp.getDecTipoCompDoc().getNmTipoCompDoc(), null,
                    attribDatiSpecExp.getNmAttribDatiSpec());
            // Inserisco il record in DEC_ATTRIB_DATI_SPEC
            if (attribDatiSpec == null) {
                attribDatiSpec = inserisciAttribDatiSpec(idStrutCorrente, attribDatiSpecExp, null,
                        null, tipoCompDoc, Constants.TipoEntitaSacer.COMP);
            }
    ...
    public void importaXsdTipoUd(BigDecimal idStrutCorrente, DecTipoUnitaDoc tipoUnitaDocExp,
        ...
        for (DecXsdDatiSpec xsdDatiSpecExp : xsdDatiSpecExpList) {
            ...
            DecXsdDatiSpec xsdDatiSpec = datiSpecHelper.getDecXsdDatiSpec(idStrutCorrente,
                    xsdDatiSpecExp.getTiUsoXsd(), xsdDatiSpecExp.getTiEntitaSacer(),
                    xsdDatiSpecExp.getDecTipoUnitaDoc().getNmTipoUnitaDoc(), null, null, null,
                    xsdDatiSpecExp.getNmSistemaMigraz(), xsdDatiSpecExp.getCdVersioneXsd());
    ...
    public void importaXsdTipoDoc(BigDecimal idStrutCorrente, DecTipoDoc tipoDocExp,
            DecTipoDoc tipoDoc) {
        ...
        for (DecXsdDatiSpec xsdDatiSpecExp : xsdDatiSpecExpList) {
            ...
            DecXsdDatiSpec xsdDatiSpec = datiSpecHelper.getDecXsdDatiSpec(idStrutCorrente,
                    xsdDatiSpecExp.getTiUsoXsd(), xsdDatiSpecExp.getTiEntitaSacer(), null,
                    xsdDatiSpecExp.getDecTipoDoc().getNmTipoDoc(), null, null,
                    xsdDatiSpecExp.getNmSistemaMigraz(), xsdDatiSpecExp.getCdVersioneXsd());
            // Se la versione XSD NON \u00E8 presente su DB
            if (xsdDatiSpec == null) {
                ///////////////////
    ...
    public void importaXsdTipoCompDoc(BigDecimal idStrutCorrente, DecTipoCompDoc tipoCompDocExp,
        ...
        for (DecXsdDatiSpec xsdDatiSpecExp : xsdDatiSpecExpList) {
            ...
            // Controlla se esiste la versione xsd del tipo doc su DB
            DecXsdDatiSpec xsdDatiSpec = datiSpecHelper.getDecXsdDatiSpec(idStrutCorrente,
                    xsdDatiSpecExp.getTiUsoXsd(), xsdDatiSpecExp.getTiEntitaSacer(), null, null,
    ...
    public DecRegistroUnitaDoc inserisciRegistroUd(BigDecimal idStrutCorrente,
            DecRegistroUnitaDoc registroUnitaDocExp) {
        DecRegistroUnitaDoc registroUnitaDoc = inserisciRegistro(idStrutCorrente,
                registroUnitaDocExp);

        // Inserisco i record relativi agli "anni" ed a "parte numero registro"
        for (DecAaRegistroUnitaDoc aaRegistroUnitaDocExp : registroUnitaDocExp
                .getDecAaRegistroUnitaDocs()) {
            DecAaRegistroUnitaDoc decAaRegistroUnitaDoc = inserisciAaRegistroUnitaDoc(
                    aaRegistroUnitaDocExp, registroUnitaDoc);
            for (DecParteNumeroRegistro parteNumeroRegistro : aaRegistroUnitaDocExp
    ...
    public DecRegistroUnitaDoc recuperaRegistroUd(BigDecimal idStrutCorrente,
            DecRegistroUnitaDoc registroUnitaDocExp) {
        String cdRegistroUnitaDocExp = registroUnitaDocExp.getCdRegistroUnitaDoc();
        DecRegistroUnitaDoc registroUnitaDoc = registroHelper
                .getDecRegistroUnitaDocByName(cdRegistroUnitaDocExp, idStrutCorrente);

        // Se il registro esiste, verifico che gli anni dell'XML non si sovrappongano a quelli
        // gi\u00E0 presenti
        for (DecAaRegistroUnitaDoc decAaRegistroUnitaDocExp : registroUnitaDocExp
                .getDecAaRegistroUnitaDocs()) {
            boolean anniSovrapposti = false;
            for (DecAaRegistroUnitaDoc decAaRegistroUnitaDocDB : registroUnitaDoc
                    .getDecAaRegistroUnitaDocs()) {
                // anniSovrapposti(decExp, dec)
                if (anniSovrapposti = checkAnniSovrapposti(decAaRegistroUnitaDocExp,
                        decAaRegistroUnitaDocDB)) {
                    break;
                }
            }
            ...
            if (!anniSovrapposti) {
                DecAaRegistroUnitaDoc decAaRegistroUnitaDoc = inserisciAaRegistroUnitaDoc(
                        decAaRegistroUnitaDocExp, registroUnitaDoc);
    ...
    public DecTipoDoc gestisciTipoDoc(BigDecimal idStrutCorrente,
            DecTipoDocAmmesso tipoDocAmmessoExp, DecTipoStrutUnitaDoc tipoStrutUnitaDoc,
            List<CoppiaTipiDoc> tipiDocImportati) {
        ...
        DecTipoDocAmmesso tipoDocAmmesso = tipoUnitaDocHelper.getDecTipoDocAmmessoByName(
                idStrutCorrente, tipoDoc.getNmTipoDoc(),
                tipoStrutUnitaDoc.getNmTipoStrutUnitaDoc());
    ...
    public DecTipoStrutDoc gestisciTipoStrutDoc(BigDecimal idStrutCorrente,
            ...
            Set<BigDecimal> tipiStrutturaDocumentoDaLoggare,
            Set<BigDecimal> tipiRappresentazioneComponenteDaLoggare) {
        ...
        DecTipoStrutDoc tipoStrutDoc = tipoStrutDocHelper
                .getDecTipoStrutDocByName(nmTipoStrutDocExp, idStrutCorrente);
        ...
        DecTipoStrutDocAmmesso tipoStrutDocAmmesso = tipoDocHelper.getDecTipoStrutDocAmmessoByName(
                idStrutCorrente, nmTipoStrutDocExp, tipoDoc.getNmTipoDoc());
    ...
    public DecTipoCompDoc gestisciTipoCompDoc(BigDecimal idStrutCorrente,
            DecTipoCompDoc tipoCompDocExp, String nmTipoStrutDoc,
            Set<BigDecimal> tipiRappresentazioneComponenteDaLoggare) {
        String nmTipoCompDocExp = tipoCompDocExp.getNmTipoCompDoc();

        // Verifico se esiste gi\u00E0 su DB il DecTipoCompDoc
        DecTipoCompDoc tipoCompDoc = tipoStrutDocHelper.getDecTipoCompDocByName(idStrutCorrente,
                nmTipoStrutDoc, nmTipoCompDocExp);
        ...
        List<DecTipoRapprAmmesso> tipoRapprAmmessoExpList = tipoCompDocExp
        ...
        for (DecTipoRapprAmmesso tipoRapprAmmessoExp : tipoRapprAmmessoExpList) {
            ...
            DecTipoRapprComp tipoRapprComp = tipoRapprHelper
                    .getDecTipoRapprCompByName(nmTipoRapprComp, idStrutCorrente);

            if (tipoRapprComp == null) {
                tipoRapprComp = inserisciTipoRapprComp(idStrutCorrente, tipoRapprCompExp);
                // Da fotografare in XML se inseriti ex-novo
                tipiRappresentazioneComponenteDaLoggare
                        .add(new BigDecimal(tipoRapprComp.getIdTipoRapprComp()));
            }

            // Verifico se il tipoRapprAmmesso \u00E8 inserito anche in DEC_TIPO_RAPPR_AMMESSO
            // (relazione
            // tipoCompDoc-tipoRapprComp)
            DecTipoRapprAmmesso tipoRapprAmmesso = tipoStrutDocHelper
                    .getDecTipoRapprAmmessoByParentId(tipoCompDoc.getIdTipoCompDoc(),
                            tipoRapprComp.getIdTipoRapprComp());
    ...
    /**
     * Restituisce il rowBean della prima struttura template disponibile per l'ambiente passato come
     * parametro. Null in caso non siano disponibili strutture template
     *
     * @param idAmbiente id ambiente
     *
     * @return OrgStrutRowBean bean struttura
     */
    public OrgStrutRowBean getFirstStrutturaTemplateDisponibilePerAmbienteSelezionato(
            BigDecimal idAmbiente) {
        OrgStrutRowBean strutRowBean = null;
        OrgStrut strut = null;
        try {
            strut = struttureHelper
                    .getFirstOrgStrutTemplatePerAmbienteAndTipoDefTemplateEnte(idAmbiente, null);
            if (strut != null) {
                strutRowBean = (OrgStrutRowBean) Transform.entity2RowBean(strut);
            }
        } catch (Exception ex) {
            logger.error("Errore durante il recupero di una struttura tgemplate "
    ...
    /**
     * Restituisce un array di interi contenente come valori gli indici da utilizzare per creare le
     * strutture template
     *
     * @param numStruttureTemplateDaCreare il numero di strutture template
     *
     * @return indiciPerCreazioneTemplate: l'array, lungo quante strutture template devo creare, che
     *         mi dice con quali indici crearle
     */
    public int[] getProgressiviPerCreazioneStruttureTemplate(int numStruttureTemplateDaCreare) {
        ...
        List<Integer> listaProgressiviPresenti = struttureHelper
                .getProgressiviTemplatePresentiSuDB();
        ...
        if (!listaProgressiviPresenti.isEmpty()) {
            ...
            int ultimoProgressivoSuDB = listaProgressiviPresenti
                    .get(listaProgressiviPresenti.size() - 1);
            // Creo un array "di appoggio" di lunghezza il progressivo più alto di strutture
    ...
    private void loggaOggettiStruttura(LogParam param, OrgStrut strut) {
        ...
        if (strut.getDecRegistroUnitaDocs() != null) {
            for (DecRegistroUnitaDoc ogg : strut.getDecRegistroUnitaDocs()) {
                sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                        param.getNomeUtente(), param.getNomeAzione(),
                        SacerLogConstants.TIPO_OGGETTO_REGISTRO,
        ...
        if (strut.getDecTipoSeries() != null) {
            for (DecTipoSerie ogg : strut.getDecTipoSeries()) {
                sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                        param.getNomeUtente(), param.getNomeAzione(),
                        SacerLogConstants.TIPO_OGGETTO_TIPO_SERIE,
    ...
    /**
     * Ricerca le strutture che rispondono ai parametri di ricerca impostati non associate al
     * modello (non presenti nella DEC_USO_MODELLO_TIPO_SERIE per l'id del modello da associare)
     *
    ...
    public OrgStrutTableBean getOrgStrutTableBean(long idUtente, String nmStrut, BigDecimal idEnte,
            BigDecimal idAmbiente, BigDecimal idModelloTipoSerie, Boolean filterValid)
            throws ParerUserError {
        OrgStrutTableBean table = new OrgStrutTableBean();
        List<OrgStrut> list = struttureHelper.retrieveOrgStrutList(idUtente, nmStrut, idEnte,
                idAmbiente, idModelloTipoSerie, Boolean.FALSE, filterValid);
    ...
    public boolean registriDaImportareConTipoSerieMult(UUID uuid, List<String> nmTipoUnitaDocList) {
        ...
        for (String nmTipoUnitaDoc : nmTipoUnitaDocList) {
            ...
            DecTipoUnitaDoc tipoUnitaDoxExp = getDecTipoUnitaDocExpSelected(strutExp,
                    nmTipoUnitaDoc);

            // Controllo i registri associati al tipo ud selezionato
            if (tipoUnitaDoxExp.getDecTipoUnitaDocAmmessos() != null) {
                for (DecTipoUnitaDocAmmesso tipoUdAmmesso : tipoUnitaDoxExp
                        .getDecTipoUnitaDocAmmessos()) {
                    if (tipoUdAmmesso.getDecRegistroUnitaDoc().getFlTipoSerieMult() != null
                            && tipoUdAmmesso.getDecRegistroUnitaDoc().getFlTipoSerieMult()
                                    .equals("1")) {
                        return true;
                    }
                }
    ...
    /**
     * Recupera le strutture da mostrare nella combo della pagina di scelta strutture dopo aver
     * effettuato il login
     *
     * @param idUtente id utente
     *
    ...
    private void updateTipiServizioOnTipiUdInStruttura(BigDecimal idStrut) {
        ...
        for (DecTipoUnitaDoc tipoUd : tipiUd) {
            DecVCalcTiServOnTipoUd calcTiServOnTipoUd = tipoUnitaDocHelper
                    .getDecVCalcTiServOnTipoUd(idStrut,
                            new BigDecimal(
                                    tipoUd.getDecCategTipoUnitaDoc().getIdCategTipoUnitaDoc()),
                            "CLASSE_ENTE");
            if (calcTiServOnTipoUd != null) {
                OrgTipoServizio tipoServizioConserv = calcTiServOnTipoUd
                        .getIdTipoServizioConserv() != null
                                ? struttureHelper.findById(OrgTipoServizio.class,
                                        calcTiServOnTipoUd.getIdTipoServizioConserv())
                                : null;
                OrgTipoServizio tipoServizioAttiv = calcTiServOnTipoUd
                        .getIdTipoServizioAttiv() != null
                                ? struttureHelper.findById(OrgTipoServizio.class,
                                        calcTiServOnTipoUd.getIdTipoServizioAttiv())
                                : null;
                ...
                OrgTipoServizio tipoServizioConserv = calcTiServOnTipoUd2
                        .getIdTipoServizioConserv() != null
                                ? struttureHelper.findById(OrgTipoServizio.class,
                                        calcTiServOnTipoUd2.getIdTipoServizioConserv())
                                : null;
                OrgTipoServizio tipoServizioAttiv = calcTiServOnTipoUd2
                        .getIdTipoServizioAttiv() != null
                                ? struttureHelper.findById(OrgTipoServizio.class,
                                        calcTiServOnTipoUd2.getIdTipoServizioAttiv())
                                : null;
    ...
    public List<BigDecimal> getIdAmbitoTerritorialePerRicerca(List<BigDecimal> regioniList,
        ...
        if (!provinceList.isEmpty()) {
            ...
            for (BigDecimal idProvincia : provinceList) {
                if (!struttureHelper.haFigliPresentiInSottoLivelloOnlineList(idProvincia,
                        formeAssociateList)) {
                    idAmbitoTerritList.add(idProvincia);
                    idAmbitoTerritList
                            .addAll(struttureHelper.getIdAmbitoTerritChildList(idProvincia));
                }
            }
        }
        if (!regioniList.isEmpty()) {
            for (BigDecimal idRegione : regioniList) {
                if (!struttureHelper.haFigliPresentiInSottoLivelloOnlineList(idRegione,
                        provinceList)) {
                    idAmbitoTerritList.add(idRegione);
                    List<BigDecimal> figliRegione = struttureHelper
                            .getIdAmbitoTerritChildList(idRegione);
                    for (BigDecimal provincia : figliRegione) {
                        idAmbitoTerritList.add(provincia);
                        idAmbitoTerritList
                                .addAll(struttureHelper.getIdAmbitoTerritChildList(provincia));
                    }
                }
            }
        }
    ...
    public BigDecimal getIdAmbitoTerritorialePerRicerca(BigDecimal regione, BigDecimal provincia,
        ...
        List<BigDecimal> result = getIdAmbitoTerritorialePerRicerca(idAmbitoTerrit1Livello,
    ...
    public void insertValoriParametriStrutturaTemplate(BigDecimal idStrut) {
        ...
        AplParamApplic flGestFascicoli = configurationHelper
                .getParamApplic(CostantiDB.ParametroAppl.FL_GEST_FASCICOLI);
        ...
        String numMaxCompCriterioRaggrWarn = configurationHelper.getValoreParamApplicByApplic(
                CostantiDB.ParametroAppl.NUM_MAX_COMP_CRITERIO_RAGGR_WARN);

        amministrazioneEjb.insertAplValoreParamApplic(null, strut, null, null,
                BigDecimal.valueOf(tiTempoScadChius.getIdParamApplic()), TiAppart.STRUT.name(),
                "GIORNI");
        amministrazioneEjb.insertAplValoreParamApplic(null, strut, null, null,
                BigDecimal.valueOf(niTempoScadChius.getIdParamApplic()), TiAppart.STRUT.name(),
                "30");
        amministrazioneEjb.insertAplValoreParamApplic(null, strut, null, null,
                BigDecimal.valueOf(flGestFascicoli.getIdParamApplic()), TiAppart.STRUT.name(), "0");
        amministrazioneEjb.insertAplValoreParamApplic(null, strut, null, null,
                BigDecimal.valueOf(numMaxCompCriterioRaggr.getIdParamApplic()),
                TiAppart.STRUT.name(), numMaxCompCriterioRaggrWarn);
    }
    ...
    /*
     * Verifica se il codice normalizzato passato è univoco sul db; se non è univoco tenta di
     * aggiungere il carattere "_" alla fine della stringa finche non lo trova univoco sul DB e lo
     * torna indietro. Viene fatto fino al raggiungimento del centesimo carattere della stringa
     * dopodiché si ferma.
     */
    ...
    public OrgVCorrPingTableBean retrieveOrgVCorrPingList(BigDecimal idStrut) {
        ...
        lista = corrispondenzeHelper.retrieveOrgVCorrPingList(idStrut,
                new BigDecimal(orgEnte.getIdEnte()), new BigDecimal(orgAmbiente.getIdAmbiente()));

        if (!lista.isEmpty()) {
            try {
                for (OrgVCorrPing orgVCorrPing : lista) {
                    OrgVCorrPingRowBean row = (OrgVCorrPingRowBean) Transform
                            .entity2RowBean(orgVCorrPing);
                    switch (row.getTiDichVers()) {
                    case "STRUTTURA":
                        row.setObject("ti_corrispondenza",
                                "Corrispondenza su struttura " + row.getNmEntita());
                        break;
                    case "ENTE":
                        row.setObject("ti_corrispondenza",
                                "Corrispondenza su ente " + row.getNmEntita());
                        break;
                    case "AMBIENTE":
                        row.setObject("ti_corrispondenza",
                                "Corrispondenza su ambiente " + row.getNmEntita());
                        break;
                    default:
                        break;
                    }

...
public class StruttureHelper extends GenericHelper {
    ...
    /**
     * Metodo per eseguire la merge di un entity, da utilizzare <strong>solo e solamente</strong> in
     * caso di entity DETACHED già presente su db, e di cui si stanno aggiornando i dati
     *
     * @param <T>    Classe di tipo entity
     * @param entity entita jpa
     *
     ...
     */
    public List<OrgStrut> retrieveOrgStrutList(String nmStrut, BigDecimal idEnte,
        ...
        if (isTemplate != null) {
            if (Boolean.TRUE.equals(isTemplate)) {
                queryStr.append(
                        " AND ente.tipoDefTemplateEnte IN ('TEMPLATE_DEF_AMBIENTE','TEMPLATE_DEF_ENTE')");
            } else {
                queryStr.append(
                        " AND ente.tipoDefTemplateEnte IN ('NO_TEMPLATE','TEMPLATE_DEF_ENTE')");
            }
        }
     ...
     */
    public List<OrgStrut> retrieveOrgStrutList(long idUtente, BigDecimal idEnte,
            Boolean filterValid) {
        return retrieveOrgStrutList(idUtente, null, idEnte, null, null, null, filterValid);
    }

    public List<OrgVRicStrut> retrieveOrgVRicStrutList(String nmStrut, BigDecimal idEnte,
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT DISTINCT new it.eng.parer.viewEntity.OrgVRicStrut(ricStrut.idAmbiente, "
                        ...
                        + "FROM OrgVRicStrut ricStrut "
                        + "WHERE ricStrut.id.idUserIamCor = :idUserIamCor ");
     ...
     */
    public List<OrgStrut> retrieveOrgStrutList(long idUtente, String nmStrut, BigDecimal idEnte,
            BigDecimal idAmbiente, BigDecimal idModelloTipoSerie, Boolean isTemplate,
            Boolean filterValid) {
        StringBuilder queryStr = new StringBuilder(
    ...
    public List<OrgStrut> retrieveOrgStrutList(long idUtente, Collection<BigDecimal> idEntiSet,
        ...
        queryStr.append(
                "WHERE iao.idOrganizApplic = strut.idStrut AND iao.iamUser.idUserIam = :idUtente ");
    ...
    public void deleteOrgStrutRelations(OrgStrut oldStrut) {
        ...
        // cancello registri
        StringBuilder queryStr = new StringBuilder("DELETE FROM DecRegistroUnitaDoc registro "
                + "WHERE registro.orgStrut.idStrut = :idStrut");
        ...
        // cancello tipoDoc
        queryStr = new StringBuilder(
                "DELETE FROM DecTipoDoc tipoDoc " + "WHERE tipoDoc.orgStrut.idStrut = :idStrut");
        ...
        queryStr = new StringBuilder("DELETE FROM DecTipoStrutDoc tipoStrutDoc "
                + "WHERE tipoStrutDoc.orgStrut.idStrut = :idStrut");
        ...
        // cancello formatoDoc
        queryStr = new StringBuilder("DELETE FROM DecFormatoFileDoc formatoFileDoc "
        ...
        // cancello tipoRappr
        queryStr = new StringBuilder("DELETE FROM DecTipoRapprComp tipoRapprComp "
                + "WHERE tipoRapprComp.orgStrut.idStrut = :idStrut");
    ...
    /**
     * Restituisce la prima struttura template disponibile per l'ambiente passato come parametro.
     * Null in caso non siano disponibili strutture template
     *
     * @param idAmbiente          id ambiente
     * @param tipoDefTemplateEnte tipo template ente
     *
    ...
    /**
     * Restituisce la prima struttura template partizionata disponibile per l'ambiente passato come
     * parametro. Se non ne trova, restituisce la prima struttura template. Null in caso non siano
     * disponibili strutture template
     *
     * @param idAmbiente          id ambiente
     * @param tipoDefTemplateEnte tipo template
     *
     * @return OrgStrut entity OrgStrut
     */
    public OrgStrut getFirstOrgStrutTemplatePerAmbienteAndTipoDefTemplateEntePartizionata(
            BigDecimal idAmbiente, String tipoDefTemplateEnte) {
        OrgStrut strut = null;
        if (idAmbiente != null && tipoDefTemplateEnte != null) {
            String queryStr = "SELECT strut FROM OrgStrut strut, OrgVChkStrutPartition chk "
                    + "WHERE strut.flTemplate = '1' " + "AND strut.idStrut = chk.idStrut "
                    + "AND chk.flPartOk = '1' "
            ...
            if (list.isEmpty()) {
                String queryStr2 = "SELECT strut FROM OrgStrut strut "
                        + "WHERE strut.flTemplate = '1' "
                        + "AND strut.orgEnte.orgAmbiente.idAmbiente = :idAmbiente "
                        + "AND strut.orgEnte.tipoDefTemplateEnte = :tipoDefTemplateEnte ";
    ...
    /**
     * Restituisce la prima struttura template partizionata disponibile per l'ente passato come
     * parametro. Se non ne trova, restituisce la prima struttura template. Null in caso non siano
     * disponibili strutture template
     *
     * @param idEnte id ente
    ...
    /**
     * Conta il numero di strutture template definite su enti TEMPLATE_DEF_AMBIENTE per ogni
     * ambiente e su enti TEMPLATE_DEF_ENTE
     *
    ...
    public List<Object[]> countOrgStrutTemplateRaggruppati(long idUserIam) {
        String queryStr = "SELECT count(strut), ambiente.nmAmbiente " + "FROM OrgStrut strut "
                ...
                + "IamAbilOrganiz abilOrganiz " + "WHERE strut.flTemplate = '1' "
                + "AND strut.idStrut = abilOrganiz.idOrganizApplic "
                + "AND abilOrganiz.iamUser.idUserIam = :idUserIam "
                + "AND ente.tipoDefTemplateEnte = 'TEMPLATE_DEF_AMBIENTE' "
                + "GROUP BY ambiente.nmAmbiente " + "HAVING COUNT(strut) > 0";
        Query query = getEntityManager().createQuery(queryStr);
        ...
        String queryStr2 = "SELECT count(strut), 'Enti con strutture template definite specificatamente per ente'  "
                + "FROM OrgStrut strut " + "JOIN strut.orgEnte ente, "
                + "IamAbilOrganiz abilOrganiz " + "WHERE strut.flTemplate = '1' "
                + "AND strut.idStrut = abilOrganiz.idOrganizApplic "
                + "AND abilOrganiz.iamUser.idUserIam = :idUserIam "
                + "AND ente.tipoDefTemplateEnte = 'TEMPLATE_DEF_ENTE' " + " GROUP BY 2 "
    ...
    public Long countOrgStrutTemplatePerAmbienteEnte(Long idAmbiente, Long idEnte,
            String tipoDefTemplateEnte) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT count(strut) " + "FROM OrgStrut strut " + "JOIN strut.orgEnte ente "
                        + "JOIN ente.orgAmbiente ambiente " + "WHERE strut.flTemplate = '1' ");
    ...
    /**
     * Conta il numero di strutture template partizionate definite su enti TEMPLATE_DEF_AMBIENTE per
     * ogni ambiente e su enti TEMPLATE_DEF_ENTE
     *
     * @param idUserIam id user IAM
     *
     * @return l'object array con numeroStrutture e ambiente
     */
    public List<Object[]> countOrgStrutTemplateWithCompletedPartitioningRaggruppati(
            long idUserIam) {
        String queryStr = "SELECT count(strut), ambiente.nmAmbiente "
                ...
                + "WHERE strut.idStrut = part.idStrut " + "AND strut.flTemplate = '1' "
        ...
        String queryStr2 = "SELECT count(strut), 'Enti con strutture template definite specificatamente per ente'  "
                + "FROM OrgStrut strut, OrgVChkStrutPartition part " + "JOIN strut.orgEnte ente, "
                + "IamAbilOrganiz abilOrganiz " + "WHERE strut.idStrut = part.idStrut "
                + "AND strut.flTemplate = '1' " + "AND part.flPartOk = '1' "
                + "AND strut.idStrut = abilOrganiz.idOrganizApplic "
                + "AND abilOrganiz.iamUser.idUserIam = :idUserIam "
                + "AND ente.tipoDefTemplateEnte = 'TEMPLATE_DEF_ENTE' " + "GROUP BY 2 "
                + "HAVING COUNT(strut) > 0";
    ...
    public List<OrgCategStrut> getOrgCategStrutList(String cdCategStrut, String dsCategStrut) {
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT orgCategStrut " + "FROM OrgCategStrut orgCategStrut");

        String whereWord = " WHERE ";
        if (cdCategStrut != null) {
            queryStr.append(whereWord)
                    .append(" UPPER(orgCategStrut.cdCategStrut) LIKE  :cdCategStrut ");
            whereWord = " AND ";
        }
        if (dsCategStrut != null) {
            queryStr.append(whereWord)
                    .append(" UPPER(orgCategStrut.dsCategStrut) LIKE  :dsCategStrut ");
        }
    ...
    public OrgCategStrut getOrgCategStrutByCd(String cdCategStrut) {
        Query query = getEntityManager()
                .createQuery("SELECT orgCategStrut " + "FROM OrgCategStrut orgCategStrut "
    ...
    public boolean hasAroUnitaDoc(BigDecimal idStrut) {
        String queryStr = "SELECT strut " + "FROM OrgStrut strut " + "WHERE EXISTS( SELECT ud "
                + "FROM AroUnitaDoc ud " + "WHERE ud.orgStrut.idStrut = :idStrut )";
        Query query = getEntityManager().createQuery(queryStr);
    ...
    public List<DecCriterioFiltroMultiplo> getRelationsWithCriteriRaggruppamento(long idTipoDato,
        ...
        case REGISTRO:
            queryStr.append(
                    "WHERE criterioFiltroMultiplo.decRegistroUnitaDoc.idRegistroUnitaDoc = :idTipoDato "
                            + "OR criterioFiltroMultiplo.decRegistroRangeUnitaDoc.idRegistroUnitaDoc = :idTipoDato ");
            break;
        case TIPO_UNITA_DOC:
            queryStr.append(
                    "WHERE criterioFiltroMultiplo.decTipoUnitaDoc.idTipoUnitaDoc = :idTipoDato ");
            break;
        case TIPO_DOC:
    ...
    /**
     * @param idTipoFascicolo id del tipo fasciolo
     *
    ...
    public List<DecSelCriterioRaggrFasc> getRelationsByIdTipoFascicolo(long idTipoFascicolo) {
        ...
        queryStr.append(
                "WHERE selCriterioRaggrFasc.decTipoFascicolo.idTipoFascicolo = :idTipoFascicolo ");
        Query query = getEntityManager().createQuery(queryStr.toString());
    ...
    /**
     * @param idTipoDato id del tipo di dato
     * @param tipoDato   costante Constants.TipoDato
     *
     * @return Lista di {@link DecSelCriterioRaggrFasc}
     *
    ...
    public boolean checkManyRelationsAreEmptyForStruttura(long idStrut) {
        ...
        if (!list1.isEmpty()) {
            ...
            // Se ho record, vuol dire che non ho relazioni, quindi proseguo
            if (!list2.isEmpty()) {
                String queryStr3 = "SELECT strut FROM OrgStrut strut "
                        + "WHERE strut.idStrut = :idStrut "
    ...
    public List<Integer> getProgressiviTemplatePresentiSuDB() {
        ...
        Query query = getEntityManager().createQuery(
                "SELECT strut.nmStrut FROM OrgStrut strut " + "WHERE strut.flTemplate = '1' ");
        ...
        for (String nmStrutturaTemplate : struttureTemplateList) {
            ...
            // Controllo che ci siano 2 elementi, che il primo sia uguale a "Template" e che il
            // secondo sia un
            // progressivo
            if (nomeAndProgressivo[0] != null && nomeAndProgressivo[0].equals("Template")
                ...
                int progressivo = org.apache.commons.lang3.math.NumberUtils
                        .toInt(nomeAndProgressivo[1], 0);
                if (progressivo != 0) {
     ...
     *
     * @param idUserIam     id dell'utente
     * @param idAmbientiSet identificativi degli ambienti
     *
    ...
    public List<BigDecimal> getIdStrutAbilitatiFromAmbienteSet(long idUserIam,
        ...
        Query query = getEntityManager().createQuery(
                "SELECT DISTINCT strut.idStrut " + " FROM IamAbilOrganiz iao, OrgStrut strut "
                        + " WHERE strut.orgEnte.orgAmbiente.idAmbiente IN (:idAmbiente) "
                        + " AND iao.iamUser.idUserIam = :idUtente "
     ...
     *
     * @param idUserIam id dell'utente
     * @param idEntiSet gli id degli enti
     *
     ...
     */
    @Deprecated
    public List<BigDecimal> getIdStrutAbilitatiFromEnteSet(long idUserIam,
            Set<? extends BigDecimal> idEntiSet) {
        Query query = getEntityManager().createQuery(
                "SELECT DISTINCT strut.idStrut " + " FROM IamAbilOrganiz iao, OrgStrut strut "
                        + " WHERE strut.orgEnte.idEnte IN (:idEnte) "
                        + " AND iao.iamUser.idUserIam = :idUtente "
                        + " AND strut.idStrut = iao.idOrganizApplic "
        /* + " ORDER BY strut.nmStrut" */
        );
    ...
    public Long countOrgStrut(BigDecimal idEnte) {
        Query query = getEntityManager().createQuery(
                "SELECT COUNT(strut) FROM OrgStrut strut WHERE strut.orgEnte.idEnte = :idEnte");
        query.setParameter("idEnte", longFromBigDecimal(idEnte));
    ...
    public boolean haFigliPresentiInSottoLivelloOnlineList(BigDecimal idPadre,
        ...
        // Se la lista di figli presenti nell'online è vuota, è ovvio che non avrò figli presenti
        // nell'online
    ...
    /**
     * Controlla che nel periodo compreso tra data di inizio e di fine validità la struttura non sia
     * già associata ad un altro ente convenzionato ricercando sulla tabella dell'associazione
     * ovvero SIOrgEnteConvenzOrg
     *
     * @param nmApplic         nome aplplicazione
     * @param idStrut          id struttura
     * @param dtIniVal         data inizio validita
     * @param dtFineVal        data fine validita
     * @param idEnteConvenzOrg id ente convenzionato
     *
     * @return true se esiste già l'associazione esiste gia
     */
    public boolean checkEsistenzaAssociazioneEnteConvenzStrutVers(String nmApplic,
            BigDecimal idStrut, Date dtIniVal, Date dtFineVal, BigDecimal idEnteConvenzOrg) {
     ...
     * associato alla struttura abbia un accordo valido (compreso tra data di inizio e data di fine
     * validità)
     *
     ...
     */
    public boolean checkEsistenzaPeriodoValiditaAssociazioneEnteConvenzStrutVers(
    ...
    /**
     * Controlla che l'ente convenzionato associato alla struttura abbia un accordo valido alla data
     * corrente (compresa tra data di inizio e data di fine validità accordo)
     *
     * @param idEnteConvenz id ente convenzionato
     *
     * @return true se esiste accordo valido
    ...
    /**
     * Controlla che data inizio validità sia maggiore o uguale data decorrenza più vecchia di tutti
     * gli accordi definiti sull'ente e data fine minore data fine validità ente siam
     *
     * @param idEnteConvenz l'ente convenzionato di cui controllare gli accordi
     * @param dtIniVal      la data di inizio validità dell'associazione struttura/ente
     *                      convenzionato
     * @param dtFineVal     la data di fin validità dell'associazione struttura/ente convenzionato
     *
     * @return true se viene soddisfatto il controllo
     */
    public boolean existsPeriodoValiditaAssociazioneEnteConvenzStrutVersAccordi(
            BigDecimal idEnteConvenz, Date dtIniVal, Date dtFineVal) {
        String queryStr = "SELECT accordo FROM SIOrgAccordoEnte accordo "
    ...
    public boolean existsPeriodoValiditaAssociazioneEnteConvenzStrutVersAccordoValido(
            BigDecimal idEnteConvenz, Date dtIniVal, Date dtFineVal) {
    ...
    /**
     * Verifica se per un accordo valido (data odierna compresa nell'intervallo di validità) le date
     * dell'associazione rientrano nell'intervallo tra la data di decorrenza accordo e la data di
     * fine validità dell'ente siam
     *
     * @param idEnteConvenz l'ente convenzionato su cui controllare se esiste un accordo valido
     * @param dtIniVal      la data di inizio validità dell'associazione
     * @param dtFineVal     la data di fine validità dell'associazione
     *
     * @return vero o falso a seconda che l'intervallo sia valido
     */
    public boolean existsIntervalloValiditaPerAssociazione(BigDecimal idEnteConvenz, Date dtIniVal,
            Date dtFineVal) {
        Query query = getEntityManager().createQuery("SELECT accordo FROM SIOrgAccordoEnte accordo "
                + "JOIN  accordo.siOrgEnteConvenz enteSiam "
                + "WHERE enteSiam.idEnteSiam = :idEnteConvenz "
    ...
    public boolean containsStrutEnteNonCessata(BigDecimal idStrut) {
        String queryStr = "SELECT strut FROM OrgStrut strut " + "JOIN strut.orgEnte ente "
                + "WHERE strut.idStrut = :idStrut "
                + "AND EXISTS (SELECT strut_valida FROM OrgStrut strut_valida "
    ...
    public void writeEsitoIamEnteConvenzDaAllinea(Long idEnteConvenzDaAllinea,
        ...
        IamEnteConvenzDaAllinea enteConvenzDaAllinea = getEntityManager()
                .find(IamEnteConvenzDaAllinea.class, idEnteConvenzDaAllinea);
        if (esitoServizio != null) {
            switch (esitoServizio) {
            case OK:
                enteConvenzDaAllinea.setTiStatoAllinea(
    ...
    public List<OrgStrut> retrieveOrgStrutList() {
        Query q = getEntityManager()
                .createQuery("SELECT strut FROM OrgStrut strut ORDER BY strut.nmStrut");
        return q.getResultList();
    }

    public boolean existsCdStrutNormaliz(String cdStrutNormaliz, BigDecimal idEnte,
            BigDecimal idStrutExcluded) {
        String queryStr = "SELECT strut FROM OrgStrut strut "
                + "WHERE strut.cdStrutNormaliz = :cdStrutNormaliz "
     ...
     *
     * @param cdStrutNormaliz il codice struttura normalizzato
     *
    ...
    public OrgVRicEnteConvenzByEsterno findOrgVRicEnteConvenzByEsternoByEnte(
        ...
        TypedQuery<OrgVRicEnteConvenzByEsterno> query = getEntityManager().createQuery(
                "SELECT o FROM OrgVRicEnteConvenzByEsterno o WHERE o.id.idEnteConvenz=:"
                        + parmIdEnte,
    ...
    public List<Long> getIdTipoStrutDocList() {
        Query q = getEntityManager().createQuery(
                "SELECT tipoStrutDoc.idTipoStrutDoc FROM DecTipoStrutDoc tipoStrutDoc ");
        return q.getResultList();
    }

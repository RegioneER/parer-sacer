...
public class AmbienteEjb {
     ...
     *
     * @param enteRowBean row bean dati ente
	...
	List<OrgVRicEnte> listaEnti = ambienteHelper.getEntiAbilitatiRicerca(idUtente,
		enteRowBean.getIdAmbiente(), enteRowBean.getNmEnte(),
		enteRowBean.getTipoDefTemplateEnte());

	try {
	    if (!listaEnti.isEmpty()) {

		for (OrgVRicEnte orgEnte : listaEnti) {
		    OrgVRicEnteRowBean enteRow = (OrgVRicEnteRowBean) Transform
			    .entity2RowBean(orgEnte);
		    OrgCategEnte categEnte = ambienteHelper.findById(OrgCategEnte.class,
			    orgEnte.getIdCategEnte());
    ...
    /**
     * Metodo che ritorna un rowBean ricercando nella tabella Org Ambiente in base o all'id o al
     * nome Ritorna null se la ricerca non ha successo
     *
	...
	if (ambiente != null) {
		...
		if (ambiente.getIdEnteGestore() != null) {
		    ...
		    ambienteRowBean.setBigDecimal("id_ente_gestore",
			    BigDecimal.valueOf(enteGestore.getIdEnteSiam()));
    ...
    /**
     * Metodo che ritorna un rowBean ricercando nella tabella Org Ente in base o all'id o al nome
     * Ritorna null se la ricerca non ha buon fine
     *
    ...
    public void updateOrgAmbiente(BigDecimal idAmb, OrgAmbienteRowBean ambienteRowBean)
    ...
    public IamOrganizDaReplic saveAmbiente(BigDecimal idAmb, OrgAmbienteRowBean ambienteRowBean,
	    boolean update) throws ParerUserError {
	...
	/*
	 * N.B.: La replica va fatta solo se la modifica ha modificato il nome o la descrizione
	 * dell'ambiente e in questo caso non servono controlli perchè la modifica può SOLO
	 * modificare o nome o descrizione...
	 */
	...
	for (AplParamApplicRowBean paramApplicRowBean : paramApplicTableBean) {
	    ...
	    String[] tokens = paramApplicRowBean.getString("ds_valore_param_applic_multi")
		    .split("\\|");
	    // Recupero i token dei valori relativi allo specifico parametro presenti su DB
	    List<AplValParamApplicMulti> listaParametriMultipliDB = amministrazioneHelper
		    .getAplValParamApplicMultiList(paramApplicRowBean.getIdParamApplic(),
			    BigDecimal.valueOf(ambiente.getIdAmbiente()));
		...
		if (token != null && !token.equals("")) {
		    AplValParamApplicMulti valParamApplicMulti = amministrazioneHelper
			    .getAplValParamApplicMulti(paramApplicRowBean.getIdParamApplic(),
				    BigDecimal.valueOf(ambiente.getIdAmbiente()), token);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamOrganizDaReplic saveEnte(BigDecimal idEnte, OrgEnteRowBean enteRowBean,
	    boolean update) throws ParerUserError {
	OrgEnte orgEnte;
	...
	if (update) {
		...
		throw new ParerUserError(
			"Nome ente gi\u00E0 presente all'interno dell'intero sistema</br>");
	    }

	    if (enteRowBean.getTipoDefTemplateEnte().equals("TEMPLATE_DEF_ENTE")
		    || enteRowBean.getTipoDefTemplateEnte().equals("NO_TEMPLATE")) {
		if (enteRowBean.getCdEnteNormaliz() == null || ambienteHelper
			.existsCdEnteNormaliz(enteRowBean.getCdEnteNormaliz(), idEnte)) {
		    throw new ParerUserError(
	    ...
	    OrgCategEnte orgCategEnte = ambienteHelper.findById(OrgCategEnte.class,
		    enteRowBean.getIdCategEnte());
		...
		if (dbOrgEnte != null && (dbOrgEnte.getDtIniValAppartAmbiente()
		    ...
		    if (orgEnte.getDtFinValAppartAmbiente()
			    .compareTo(enteRowBean.getDtIniValAppartAmbiente()) <= 0) {
	    ...
	    if (enteRowBean.getTipoDefTemplateEnte().equals("TEMPLATE_DEF_ENTE")
		    || enteRowBean.getTipoDefTemplateEnte().equals("NO_TEMPLATE")) {
		if (enteRowBean.getCdEnteNormaliz() == null || ambienteHelper
			.existsCdEnteNormaliz(enteRowBean.getCdEnteNormaliz(), null)) {
		    throw new ParerUserError(
			    "Il nome normalizzato dell'ente non è stato indicato o non è univoco</br>");
		}
	    }
    ...
    public OrgAmbitoTerritTableBean getOrgAmbitoTerritChildTableBean(
	    BigDecimal idAmbitoTerritoriale) {
	OrgAmbitoTerritTableBean tableBean = new OrgAmbitoTerritTableBean();
	List<OrgAmbitoTerrit> list = ambienteHelper
		.getOrgAmbitoTerritChildList(idAmbitoTerritoriale);
	if (list != null) {
	    try {
		tableBean = (OrgAmbitoTerritTableBean) Transform.entities2TableBean(list);
	    } catch (Exception e) {
    ...
    public OrgAmbitoTerritTableBean getOrgAmbitoTerritChildTableBean(
	    List<BigDecimal> idPapiAmbitoTerritList) {
	OrgAmbitoTerritTableBean tableBean = new OrgAmbitoTerritTableBean();
	List<OrgAmbitoTerrit> list = ambienteHelper
		.getOrgAmbitoTerritChildList(idPapiAmbitoTerritList);
	if (list != null) {
	    try {
		tableBean = (OrgAmbitoTerritTableBean) Transform.entities2TableBean(list);
	    } catch (Exception e) {
	...
	OrgAmbitoTerrit orgAmbitoTerrit = ambienteHelper
	...
	OrgAmbitoTerritRowBean orgAmbitoTerritRowBean = new OrgAmbitoTerritRowBean();
	OrgAmbitoTerrit orgAmbitoTerrit = ambienteHelper.findById(OrgAmbitoTerrit.class,
	...
	OrgAmbitoTerrit orgAmbitoTerritDest = ambienteHelper.findById(OrgAmbitoTerrit.class,
		nodeDestId);
	...
	OrgCategEnte categEnteDB = ambienteHelper
		.getOrgCategEnteByCd(categEnteRowBean.getCdCategEnte());
	if (categEnteDB != null) {
	    throw new ParerUserError(
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateOrgCategEnte(BigDecimal idCategEnte, OrgCategEnteRowBean categEnteRowBean)
	    throws ParerUserError {
	OrgCategEnte categEnteDB = ambienteHelper
		.getOrgCategEnteByCd(categEnteRowBean.getCdCategEnte());
	...
	OrgCategEnte categEnteDB = ambienteHelper.findById(OrgCategEnte.class,
		orgCategEnteRowBean.getIdCategEnte());
	...
	return ambienteHelper.getUsrVChkCreaAmbSacer(idUser, nmApplic).getFlCreaAmbiente()
		.equals("1");
    }

    public BaseTableInterface getAmbientiAbilitatiPerEnte(long idUser, String nmApplic) {
	BaseTable table = new BaseTable();
	List<UsrVAbilAmbXente> ambientiAbilitati = ambienteHelper
		.getAmbientiAbilitatiPerEnte(idUser, nmApplic);
	...
	BaseTable table = new BaseTable();
	List<UsrVAbilAmbSacerXstrut> ambientiAbilitati = ambienteHelper
		.getAmbientiAbilitatiPerStrut(idUser, nmApplic);
	for (UsrVAbilAmbSacerXstrut ambiente : ambientiAbilitati) {
	    BaseRow row = new BaseRow();
	    row.setBigDecimal("id_ambiente",
		    ambiente.getUsrVAbilAmbSacerXstrutId().getIdOrganizApplic());
	    row.setString("nm_ambiente", ambiente.getNmOrganiz());
	    row.setString("ds_ambiente", ambiente.getDsOrganiz());
	    table.add(row);
	}
	...
	BaseTable table = new BaseTable();
	List<UsrVAbilEnteSacerXstrut> entiAbilitati = ambienteHelper.getEntiAbilitatiPerStrut(
		idUser, nmApplic, nmEnte, idAmbiente, tipoDefTemplateEnte);
    ...
    public BaseTableInterface getEntiValidiAbilitatiPerStrut(long idUser, String nmApplic,
	...
	for (UsrVAbilEnteSacerXstrut enteAbil : entiAbilitati) {
	    // MEV#20463: filtro gli enti validi alla data, (La validità di un ente e’ definita
	    // dall’intervallo “Data
	    ...
	    if (!ente.getDtIniVal().after(currentDate)
		    && !ente.getDtFineVal().before(currentDate)) {
		BaseRow row = new BaseRow();
     ...
     *
     * @param idUser     id utente abilitato
     * @param nmAmbiente nome ambiente da ricercare
     *
	...
	List<OrgVRicAmbiente> listaAmbienti = ambienteHelper.getAmbientiAbilitatiRicerca(idUser,
		nmAmbiente);
	try {
	    for (OrgVRicAmbiente ambiente : listaAmbienti) {
		// trasformo la lista in un tableBean
		OrgVRicAmbienteRowBean ambienteRowBean = (OrgVRicAmbienteRowBean) Transform
			.entity2RowBean(ambiente);
		ambienteRowBean.setString("nm_ente_conserv",
			ambienteHelper.findById(SIOrgEnteSiam.class, ambiente.getIdEnteConverv())
				.getNmEnteSiam());
		ambienteRowBean.setString("nm_ente_gestore",
			ambienteHelper.findById(SIOrgEnteSiam.class, ambiente.getIdEnteGestore())
				.getNmEnteSiam());
		ambientiTableBean.add(ambienteRowBean);
	    }
	} catch (Exception e) {
	    log.error("Errore nel recupero degli ambienti" + ExceptionUtils.getRootCauseMessage(e),
		    e);
	...
	try {
		...
		// trasformo la lista in un tableBean
		ambientiTableBean = (OrgAmbienteTableBean) Transform
			.entities2TableBean(listaAmbienti);
	    }
	} catch (Exception e) {
	    log.error("Errore nel recupero degli ambienti" + ExceptionUtils.getRootCauseMessage(e),
		    e);
	...
	OrgVRicEnteTableBean entiTableBean = new OrgVRicEnteTableBean();
	List<OrgVRicEnte> listaEnti = ambienteHelper.getEntiAbilitatiRicerca(idUser, idAmbiente,
		null, tipoDefTemplateEnte);
	try {
	    if (!listaEnti.isEmpty()) {
		// trasformo la lista in un tableBean
     ...
     */
    public OrgEnteTableBean getEntiAbilitatiNoTemplate(long idUser, long idAmbiente,
	    Boolean filterValid) {
	OrgEnteTableBean entiTableBean = new OrgEnteTableBean();
	List<OrgEnte> listaEnti = ambienteHelper.retrieveOrgEnteAbilNoTemplate(idUser, idAmbiente,
		filterValid);
	...
	OrgEnteTableBean entiTableBean = new OrgEnteTableBean();
	List<OrgEnte> listaEnti = ambienteHelper.retrieveOrgEnteAbil(idUser, idAmbiente, null, null,
		filterValid, (String[]) null);
	return transformOrgEnteList(listaEnti, entiTableBean);
	// trasformo la lista in un tableBean
    }
     ...
     * @return OrgEnteTableBean
     */
    public OrgEnteTableBean getEntiAbilitati(long idUser, List<BigDecimal> idAmbiente,
	    Boolean filterValid) {
	OrgEnteTableBean entiTableBean = new OrgEnteTableBean();
	List<OrgEnte> listaEnti = ambienteHelper.retrieveOrgEnteAbil(idUser, idAmbiente, null, null,
		filterValid, (String[]) null);
    ...
    /**
     * Recupera l'ente in base all'ambiente, agli ambiti territoriali e alle categorie enti (a sua
     * volta recuperato in base alle abilitazioni)
     *
     * @param idUser             id utente
     * @param idAmbiente         id ambiente
     * @param idAmbitoTerritList id ambito territoriale (lista)
     * @param idCategEnteList    id categoria ente (lista)
     * @param filterValid        true/false
     *
     * @return OrgEnteTableBean
     */
    public OrgEnteTableBean getEntiAbilitatiAmbitoCateg(long idUser, BigDecimal idAmbiente,
	    ...
	    Boolean filterValid) {
	OrgEnteTableBean entiTableBean = new OrgEnteTableBean();
	List<OrgEnte> listaEnti = ambienteHelper.retrieveOrgEnteAbil(idUser, idAmbiente.longValue(),
		idAmbitoTerritList, idCategEnteList, filterValid);
	return transformOrgEnteList(listaEnti, entiTableBean);
    }
    ...
    public OrgEnteTableBean getEntiAbilitatiAmbitoCateg(long idUser, List<BigDecimal> idAmbiente,
	    ...
	    Boolean filterValid) {
	OrgEnteTableBean entiTableBean = new OrgEnteTableBean();
	List<OrgEnte> listaEnti = ambienteHelper.retrieveOrgEnteAbil(idUser, idAmbiente,
		idAmbitoTerritList, idCategEnteList, filterValid);
	return transformOrgEnteList(listaEnti, entiTableBean);
    }

    private OrgEnteTableBean transformOrgEnteList(List<OrgEnte> listaEnti,
	    OrgEnteTableBean entiTableBean) throws IllegalStateException {
	try {
	    if (!listaEnti.isEmpty()) {
		// trasformo la lista in un tableBean
	...
	List<SIOrgEnteSiam> listaEnti = ambienteHelper
	...
	try {
	    ...
	    log.error("Errore nel recupero dell'ente siam" + ExceptionUtils.getRootCauseMessage(e),
		    e);
     ...
     *
     * @param idUserUamCor          id utente corrente
     * @param idAmbienteEnteConvenz id ambiente convenzionato
     *
     * @return BaseTable
     */
    public BaseTable getSIOrgEnteConvenzTableBean(long idUserUamCor,
	...
	BaseTable entiTableBean = new BaseTable();
	List<SIOrgEnteSiam> listaEnti = ambienteHelper
		.getEntiConvenzionatiValidiAbilitati(idUserUamCor, idAmbienteEnteConvenz);
	try {
	    for (SIOrgEnteSiam ente : listaEnti) {
		BaseRow riga = new BaseRow();
		riga.setBigDecimal("id_ente_siam", new BigDecimal(ente.getIdEnteSiam()));
    ...
    /**
     * Restituisce il rowbean contenente le date di inizio e fine validità dell'associazione ente
     * siam/struttura. Il sistema propone in automatico la data inizio validità dell’associazione
     * (dt_ini_val) = data decorrenza accordo valido e data fine validità associazione (dt_fine_val)
     * = data fine validità ente siam
     *
     * @param idEnteConvenz l'id dell'ente convenzionato da recuperare su DB
     *
     * @return il rowbean contenente il le date
     *
     * @throws ParerUserError errore generico
     */
    public BaseRow getDateAssociazioneEnteSiamStrutRowBean(BigDecimal idEnteConvenz)
	    throws ParerUserError {
	BaseRow dateEnteConvenzStrutRowBean = null;
	SIOrgEnteSiam enteSiam = ambienteHelper.findById(SIOrgEnteSiam.class, idEnteConvenz);
	SIOrgAccordoEnte accordoEnte = ambienteHelper
		.retrieveOrgAccordoValidoEnteConvenz(idEnteConvenz);
	if (accordoEnte != null && enteSiam != null) {
		...
		String msg = "Errore durante il recupero delle date"
	...
	if (!siEnteConvenzOrgList.isEmpty()) {
		...
		for (SIOrgEnteConvenzOrg orgEnteConvenzOrg : siEnteConvenzOrgList) {
		    ...
		    row.setString("nm_ambiente_ente_convenz",
			    orgEnteConvenzOrg.getSiOrgEnteConvenz().getSiOrgAmbienteEnteConvenz()
				    .getNmAmbienteEnteConvenz());
		    row.setBigDecimal("id_ente_convenz", new BigDecimal(
			...
			OrgVTreeAmbitoTerrit treeAmbitoTerrit = ambienteHelper
				.findViewById(OrgVTreeAmbitoTerrit.class, idAmbitoTerrit);
			row.setString("ds_tree_cd_ambito_territ",
				treeAmbitoTerrit.getDsTreeCdAmbitoTerrit());
		    }
		    // end MEV#20767
	...
	SIOrgEnteConvenzOrg siEnteConvenzOrg = ambienteHelper.findById(SIOrgEnteConvenzOrg.class,
		idEnteConvenzOrg);
	if (siEnteConvenzOrg != null) {
	    try {
		siEnteConvenzRowBean = (SIOrgEnteConvenzOrgRowBean) Transform
			.entity2RowBean(siEnteConvenzOrg);
	...
	try {
	    ...
	    log.error("Errore nel recupero dell'ente gestore corrente: "
		    + ExceptionUtils.getRootCauseMessage(e), e);
	    throw new IllegalStateException(
		    "Errore inatteso nel recupero del'ente gestore corrente");
	}
	return ricEnteConvenzTableBean;
    }
	...
	List<SIOrgEnteSiam> entiConvenzConserv = ambienteHelper
	...
	SIOrgEnteConvenzOrg siEnteConvenzOrg = getOrgEnteConvenzOrgMostRecent(
		BigDecimal.valueOf(strut.getIdStrut()));
	strut.setIdEnteConvenz(
		BigDecimal.valueOf(siEnteConvenzOrg.getSiOrgEnteConvenz().getIdEnteSiam()));
	strut.setDtIniVal(siEnteConvenzOrg.getDtIniVal());
	strut.setDtFineVal(siEnteConvenzOrg.getDtFineVal());
    }

    /**
     * Inserisce una nuova associazione struttura - ente convenzionato. In caso di problemi
     ...
     * @throws ParerUserError errore generico
     */
    public BigDecimal insertEnteConvenzOrg(LogParam param, BigDecimal idStrut,
	...
	SIOrgEnteConvenzOrg enteSalvato = ambienteHelper.getSIOrgEnteConvenzOrg(idStrut,
     ...
     *
     * @param param            parametro
     * @param idEnteConvenzOrg id ente convezionato su organizzazione
     * @param idEnteConvenz    id ente convenzionato
     * @param idStrut          id struttura
     * @param dtIniVal         data inizio validita
     * @param dtFineVal        data fine validita
    ...
    public BigDecimal updateEnteConvenzOrg(LogParam param, BigDecimal idEnteConvenzOrg,
	    BigDecimal idEnteConvenz, BigDecimal idStrut, Date dtIniVal, Date dtFineVal)
	    throws ParerUserError {

	List<IamEnteConvenzDaAllinea> enteConvenzDaAllineaList = context
		.getBusinessObject(AmbienteEjb.class).saveEnteConvenzOrg(param, idEnteConvenzOrg,
			idStrut, idEnteConvenz, dtIniVal, dtFineVal);
     ...
     *
     * @param param            parametro
     * @param idEnteConvenzOrg Sul metodo procede ad eseguire il rollback. Se invece il metodo non
     *                         genera eccezioni viene chiamata l'allineamento su Iam. In questo
     *                         caso, dovesse verificarsi un errore, la rollback verrebbe gestita
     *                         tramite ParerUserError (del metodo alignsEnteConvenzToIam) ed essendo
     *                         stato un creato un nuovo contesto transazionale (REQUIRES_NEW) la
     *                         rollback avrebbe effetto solo sulla replica (non voglio rollbackare
     *                         tutto...)
     *
     * @throws ParerUserError errore generico
     */
    ...
    /**
     * Inserimento di una nuova associazione struttura - ente convenzionato dati i parametri in
     * input
     *
     * @param param         parametro
     * @param idStrut       id della struttura su cui eseguire l'associazione
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamEnteConvenzDaAllinea saveEnteConvenzOrg(LogParam param, BigDecimal idStrut,
	    BigDecimal idEnteConvenz, Date dtIniVal, Date dtFineVal) throws ParerUserError {
	...
	try {
	    ...
	    if (struttureHelper.checkEsistenzaAssociazioneEnteConvenzStrutVers(nmApplic, idStrut,
	    ...
	    if (!struttureHelper.existsIntervalloValiditaPerAssociazione(idEnteConvenz, dtIniVal,
		    dtFineVal)) {
		throw new ParerUserError(
	    ...
	    sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		    ...
		    SacerLogConstants.TIPO_OGGETTO_STRUTTURA, idStrut, param.getNomePagina());

	    // Salvo il record dell'associazione da salvare in IAM
	    allinea = struttureEjb.insertIamEnteConvenzDaAllinea(
		    BigDecimal.valueOf(enteConvenz.getIdEnteSiam()), enteConvenz.getNmEnteSiam());
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public List<IamEnteConvenzDaAllinea> saveEnteConvenzOrg(LogParam param,
	    BigDecimal idEnteConvenzOrg, BigDecimal idStrut, BigDecimal idEnteConvenz,
	...
	try {
	    ...
	    // MAC 26960
	    if (!struttureHelper.existsPeriodoValiditaAssociazioneEnteConvenzStrutVersAccordi(
		...
		enteConvenzOrg.setSiOrgEnteConvenz(
			ambienteHelper.findById(SIOrgEnteSiam.class, idEnteConvenz));
		...
		enteConvenzDaAllineaList.add(struttureEjb.insertIamEnteConvenzDaAllinea(
			BigDecimal.valueOf(idEnteConvenzOld), nmEnteConvenzOld));
		enteConvenzDaAllineaList.add(struttureEjb.insertIamEnteConvenzDaAllinea(
			idEnteConvenz, enteConvenzOrg.getSiOrgEnteConvenz().getNmEnteSiam()));
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamEnteConvenzDaAllinea deleteEnteConvenzOrgTx(LogParam param,
	    BigDecimal idEnteConvenzOrg) throws ParerUserError {
	log.debug("Eseguo la delete dell'ente convenzionato");
	SIOrgEnteConvenzOrg siOrgEnteConvenzOrg = ambienteHelper.findById(SIOrgEnteConvenzOrg.class,
		idEnteConvenzOrg);
    ...
    public boolean isAmbienteModificato(BigDecimal idAmbiente, BigDecimal idEnte, Date dtIniVal,
	...
	// Se ho modificato l'ambiente di appartenenza dell'ente, verifico id ente gestore e id ente
	// conservatore
	if (idAmbienteDB != idAmbiente.longValue()) {
		...
		if (strutDB.getIdEnteConvenz() != null) {
		    ...
		    if ((ricEnteConvenz.getIdEnteGestore()
			    .compareTo(ambienteNuovo.getIdEnteGestore()) != 0)
			    || (ricEnteConvenz.getIdEnteConserv()
				    .compareTo(ambienteNuovo.getIdEnteConserv()) != 0)) {
			throw new ParerUserError(
    ...
    public void saveParametriAmbiente(AplParamApplicTableBean parametriAmministrazioneAmbiente,
	    AplParamApplicTableBean parametriConservazioneAmbiente,
	    AplParamApplicTableBean parametriGestioneAmbiente,
    ...
    /**
     * Data una struttura in input, viene recuperato l'ente convenzionato associato con data più
     * recente
     *
     * @param idStrut struttura per la quale ricercare l'associazione con ente convenzionato più
     *                recente
     *
	...
	List<SIOrgEnteConvenzOrg> associazioniStrutturaEnteConvenzionatoList = ambienteHelper
		.retrieveSIOrgEnteConvenzOrg(idStrut);
	// Trasformo la lista in mappa: chiave --> la più recente ha la data di inizio validità più
	// vicina alla data
    ...
    /**
     * Dato in input un range di date, verifica se questo rispetta la contiguità con i range delle
     * associazioni ente convenzionato - struttura già presenti
     *
     * @param idStrut   la struttura di cui estrarre tutte le associazioni
     * @param dtIniVal  data inizio validità associazione trattata (nuova o esistente in fase di
     *                  modifica)
     * @param dtFineVal data Fine validità associazione trattata (nuova o esistente in fase di
     *                  modifica)
     *
	...
	for (int i = 0; i < associazioniStrutturaEnteConvenzionatoListOrdered.size(); i++) {
	    ...
	    Date dataInizioCorrente = associazioniStrutturaEnteConvenzionatoListOrdered.get(i)
		    .getDtIniVal();
		...
		Date dataFinePrecedente = associazioniStrutturaEnteConvenzionatoListOrdered
			.get(i + 1).getDtFineVal();

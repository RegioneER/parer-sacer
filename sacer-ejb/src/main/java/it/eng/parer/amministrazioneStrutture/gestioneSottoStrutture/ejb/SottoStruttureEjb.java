...
public class SottoStruttureEjb {
    ...
    public OrgSubStrutTableBean getOrgSubStrutTableBean(BigDecimal idStrut) {
        ...
        if (listaSubStrut != null && !listaSubStrut.isEmpty()) {
            try {
                ...
                logger.error(
                        "Errore durante il recupero delle sottostrutture " + ExceptionUtils.getRootCauseMessage(ex),
                        ex);
            }
        }
        return tableBean;
    ...
    public OrgSubStrutTableBean getOrgSubStrutTableBeanAbilitate(long idUtente, BigDecimal idStrut) {
        OrgSubStrutTableBean tableBean = new OrgSubStrutTableBean();
        List<OrgSubStrut> listaSubStrut = helper.retrieveOrgSubStrutListAbilitate(idUtente, idStrut);
        if (listaSubStrut != null && !listaSubStrut.isEmpty()) {
            try {
                tableBean = (OrgSubStrutTableBean) Transform.entities2TableBean(listaSubStrut);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException | IllegalAccessException
    ...
    public OrgRegolaValSubStrutTableBean getOrgRegolaValSubStrutTableBean(BigDecimal idStrut, BigDecimal idSubStrut) {
        ...
        try {
            if (regoleList != null && !regoleList.isEmpty()) {
                for (OrgRegolaValSubStrut rule : regoleList) {
                    ...
                    OrgRegolaValSubStrutRowBean row = (OrgRegolaValSubStrutRowBean) Transform.entity2RowBean(rule);
                    ...
                    for (OrgCampoValSubStrut tmpCampo : rule.getOrgCampoValSubStruts()) {
                        ...
                        regolaComposita.append(tmpCampo.getTiCampo() + "." + tmpCampo.getNmCampo() + ";");
                    }
                    ...
                    row.setString("nm_tipo_unita_doc", rule.getDecTipoUnitaDoc().getNmTipoUnitaDoc());
                ...
                | IllegalArgumentException | InvocationTargetException ex) {
            logger.error("Errore durante il recupero delle regole " + ExceptionUtils.getRootCauseMessage(ex), ex);
        }
        return tableBean;
    }

    public OrgRegolaValSubStrutTableBean getOrgRegolaValSubStrutTableBean(BigDecimal id, Constants.TipoDato tipoDato,
            boolean isFilterValid) {
        List<OrgRegolaValSubStrut> regoleList = helper.getOrgRegolaSubStrut(id, tipoDato, isFilterValid);
        OrgRegolaValSubStrutTableBean tableBean = new OrgRegolaValSubStrutTableBean();
        try {
            if (regoleList != null && !regoleList.isEmpty()) {
                for (OrgRegolaValSubStrut rule : regoleList) {
    ...
    public OrgCampoValSubStrutTableBean getOrgCampoValSubStrutTableBean(BigDecimal idOrgRegolaValSubStrut) {
        ...
        try {
            if (campiList != null && !campiList.isEmpty()) {
                ...
                | IllegalArgumentException | InvocationTargetException ex) {
            logger.error("Errore durante il recupero dei campi " + ExceptionUtils.getRootCauseMessage(ex), ex);
        }
        return tableBean;
    }
    ...
    public boolean existOrgRegolaSubStrut(BigDecimal idRegolaValSubStrut, BigDecimal idTipoUnitaDoc,
        ...
        return helper.existOrgRegolaSubStrut(idRegolaValSubStrut, idTipoUnitaDoc, idTipoDoc, dtIstituz, dtSoppres);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public Long saveRegolaSubStrut(LogParam param, BigDecimal idTipoUnitaDoc, BigDecimal idTipoDoc, Date dtIstituz,
            Date dtSoppres) throws ParerUserError {
        ...
        /* Questo metodo può essere invocato dall'importazione parametri che non deve loggare il TIPO UD */
        if (param != null) {
            sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                    ...
                    param.getNomeTipoOggetto().equals(SacerLogConstants.TIPO_OGGETTO_TIPO_UNITA_DOCUMENTARIA)
                            ? idTipoUnitaDoc : idTipoDoc,
                    param.getNomePagina());
        }
    ...
    public void saveRegolaSubStrut(LogParam param, BigDecimal idRegolaValSubStrut, Date dtIstituz, Date dtSoppres,
            BigDecimal idTipoDoc) throws ParerUserError {
        OrgRegolaValSubStrut regola = helper.findById(OrgRegolaValSubStrut.class, idRegolaValSubStrut);
        if (regola == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare la regola indicata per la modifica");
        }

        if (regola.getDecTipoDoc().getIdTipoDoc() != idTipoDoc.longValue()) {
            DecTipoDoc tipoDoc = helper.findById(DecTipoDoc.class, idTipoDoc);
            if (tipoDoc == null) {
                throw new ParerUserError("Errore inaspettato - impossibile inserire la regola con tipo documento NULL");
            }
            regola.setDecTipoDoc(tipoDoc);
        }
        regola.setDtIstituz(dtIstituz);
        ...
        sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                param.getNomeAzione(), param.getNomeTipoOggetto(), param.getIdOggetto(), param.getNomePagina());
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveRegolaSubStrutFromTipoDoc(LogParam param, BigDecimal idRegolaValSubStrut, Date dtIstituz,
            Date dtSoppres, BigDecimal idTipoUnitaDoc) throws ParerUserError {
        OrgRegolaValSubStrut regola = helper.findById(OrgRegolaValSubStrut.class, idRegolaValSubStrut);
        if (regola == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare la regola indicata per la modifica");
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteRegolaSubStrut(LogParam param, BigDecimal idRegolaValSubStrut) throws ParerUserError {
        OrgRegolaValSubStrut regola = helper.findById(OrgRegolaValSubStrut.class, idRegolaValSubStrut);
        if (regola == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare la regola indicata per la rimozione");
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteCampoSubStrut(LogParam param, BigDecimal idCampoValSubStrut) throws ParerUserError {
        OrgCampoValSubStrut campo = helper.findById(OrgCampoValSubStrut.class, idCampoValSubStrut);
        if (campo == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare il campo indicato per la rimozione");
        }
    ...
    public boolean existOrgCampoSubStrut(BigDecimal idCampoValSubStrut, BigDecimal idRegolaValSubStrut, String tiCampo,
        ...
        return helper.existOrgCampoSubStrut(idCampoValSubStrut, idRegolaValSubStrut, tiCampo, nmCampo, idRecord);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long saveCampoSubStrut(LogParam param, BigDecimal idRegolaValSubStrut, String tiCampo, String nmCampo,
            BigDecimal idRecord) throws ParerUserError {
        OrgRegolaValSubStrut regola = helper.findById(OrgRegolaValSubStrut.class, idRegolaValSubStrut);
        return saveCampoSubStrut(param, regola, tiCampo, nmCampo, idRecord);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public Long saveCampoSubStrut(LogParam param, OrgRegolaValSubStrut regola, String tiCampo, String nmCampo,
            BigDecimal idRecord) throws ParerUserError {
        if (regola == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare la regola indicata per la modifica");
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveCampoSubStrut(LogParam param, String tiCampo, String nmCampo, BigDecimal idRecord,
            BigDecimal idCampoValSubStrut) throws ParerUserError {
        OrgCampoValSubStrut campo = helper.findById(OrgCampoValSubStrut.class, idCampoValSubStrut);
        if (campo == null) {
            throw new ParerUserError("Errore inaspettato - impossibile recuperare il campo indicato per la modifica");
        }
        campo.setTiCampo(tiCampo);
        campo.setNmCampo(nmCampo);
        CostantiDB.TipoCampo campoEnum = CostantiDB.TipoCampo.valueOf(tiCampo);
    ...
    public void updateOrgSubStrut(LogParam param, String name, String desc, BigDecimal idSubStrut)
        ...
        Object[] datiRitorno = me.saveOrgSubStrut(param, name, desc, idSubStrut, StruttureEjb.TipoOper.MOD);
        if (datiRitorno[0] != null) {
            struttureEjb.replicateToIam((IamOrganizDaReplic) datiRitorno[0]);
        }
    }

    public Long insertOrgSubStrut(LogParam param, String name, String desc, BigDecimal idStrut) throws ParerUserError {
        SottoStruttureEjb me = context.getBusinessObject(SottoStruttureEjb.class);
        Object[] datiRitorno = me.saveOrgSubStrut(param, name, desc, idStrut, StruttureEjb.TipoOper.INS);
        if (datiRitorno[0] != null) {
            struttureEjb.replicateToIam((IamOrganizDaReplic) datiRitorno[0]);
        }
    ...
    public Object[] saveOrgSubStrut(LogParam param, String name, String desc, BigDecimal id,
        ...
        /*
         * La replica va fatta solo se la sottostruttura appartiene ad una struttura non appartenente ad un ente di tipo
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public IamOrganizDaReplic deleteSubStruttura(LogParam param, long idSubStrut, boolean isFromDeleteStruttura)
            throws ParerUserError {
        ...
        if (!isFromDeleteStruttura) {
            sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                    param.getNomeAzione(), SacerLogConstants.TIPO_OGGETTO_STRUTTURA, idStrutDaLoggare,
                    param.getNomePagina());
        }
        ...
        logger.info("Cancellazione sotto struttura " + nmSubStrut + " della struttura " + struttura.getIdStrut()

...
public class TipoUnitaDocEjb {
	...
	for (DecTipoUnitaDocAmmesso tipoUnitaDocAmmesso : tipoUd.getDecTipoUnitaDocAmmessos()) {
	    ...
	    if (registro.getFlCreaTipoSerieStandard() != null
		    && registro.getFlCreaTipoSerieStandard().equals("1")) {
		result = true;
		break;
	    }
	}
     ...
     * @param tipoUnitaDocRowBean bean contenente i dati del tipo ud
     * @param criterioAutomTipoUd flag relativo alla creazione automatica del criterio di
     *                            raggruppamento
     *
	...
	IamOrganizDaReplic replic = me.saveDecTipoUnitaDoc(param, null, tipoUnitaDocRowBean,
		StruttureEjb.TipoOper.INS, criterioAutomTipoUd);
	if (replic != null) {
     ...
     *
     * @param param               parametri per il logging
     * @param idTipoUnita         id del tipo di ud da modificare
     * @param tipoUnitaDocRowBean bean contenente i dati del tipo ud
     * @param criterioAutomTipoUd flag relativo alla creazione automatica del criterio di
     *                            raggruppamento
     *
    ...
    public void updateDecTipoUnitaDoc(LogParam param, BigDecimal idTipoUnita,
	    DecTipoUnitaDocRowBean tipoUnitaDocRowBean, String criterioAutomTipoUd)
    ...
    /**
     * Metodo che esegue il salvataggio della tipologia di unità documentaria su database dati i
     * parametri
     *
     * @param param               bean con i dati per il logging
     * @param idTipoUnitaDoc      id del tipo ud da modificare (se != null)
     * @param tipoUnitaDocRowBean bean contenente i dati del tipo ud
    ...
    public IamOrganizDaReplic saveDecTipoUnitaDoc(LogParam param, BigDecimal idTipoUnitaDoc,
	    DecTipoUnitaDocRowBean tipoUnitaDocRowBean, StruttureEjb.TipoOper tipoOper,
	    String criterioAutomTipoUd) throws ParerUserError {
	...
	if (tipoOper.name().equals((StruttureEjb.TipoOper.INS.name()))) {
	    ...
	    DecTipoUnitaDoc dbTipoUnitaDoc = helper.getDecTipoUnitaDocByName(
		    tipoUnitaDocRowBean.getNmTipoUnitaDoc(), tipoUnitaDocRowBean.getIdStrut());
	    if (dbTipoUnitaDoc != null
		    && dbTipoUnitaDoc.getIdTipoUnitaDoc() != idTipoUnitaDoc.longValue()) {
		throw new ParerUserError(
	    ...
	    if (!tipoUnitaDoc.getNmTipoUnitaDoc().equals(tipoUnitaDocRowBean.getNmTipoUnitaDoc())
		    || !tipoUnitaDoc.getDsTipoUnitaDoc()
			    .equals(tipoUnitaDocRowBean.getDsTipoUnitaDoc())) {
		modificatiNomeDescrizione = true;
	    }
		...
		if (tipoUnitaDoc.getDecModelloTipoSerie() != null) {
		    if (tipoUnitaDoc.getDecModelloTipoSerie()
			    .getIdModelloTipoSerie() != idModelloTipoSerie.longValue()) {

			DecModelloTipoSerie modelloTipoSerieOld = tipoUnitaDoc
				.getDecModelloTipoSerie();
			List<DecTipoSerie> tipiSerieVecchioModello = tipoSerieHelper
				.retrieveDecTipoSerieList(tipoUnitaDoc.getOrgStrut().getIdStrut(),
		...
		idTipoServAttivTipoUdDB = tipoUnitaDoc.getOrgTipoServAttivTipoUd()
			.getIdTipoServizio();
	    }
	    if (tipoUnitaDoc.getOrgTipoServConservTipoUd() == null) {
		idTipoServConservTipoUdDB = 0L;
	    } else {
		idTipoServConservTipoUdDB = tipoUnitaDoc.getOrgTipoServConservTipoUd()
			.getIdTipoServizio();
	    }
	    ...
	    boolean ricalcolaServiziErogati = idTipoServAttivTipoUdDB != idTipoServAttivTipoUdRowBean
		    .intValue()
	    ...
	    tipoUnitaDoc
		    .setFlCreaTipoSerieStandard(tipoUnitaDocRowBean.getFlCreaTipoSerieStandard());
	...
	/*
	 * La replica va fatta solo se la tipologia unit\u00E0 documentaria appartiene ad una
	 * struttura non appartenente ad un ente di tipo template e se ho modificato nome e/o
	 * descrizione
	 */
    ...
    public void saveParametriTipoUd(LogParam param,
	    AplParamApplicTableBean parametriAmministrazioneTipoUd,
	    AplParamApplicTableBean parametriConservazioneTipoUd,
	    AplParamApplicTableBean parametriGestioneTipoUd, BigDecimal idTipoUnitaDoc) {
	DecTipoUnitaDoc tipoUnitaDoc = helper.findById(DecTipoUnitaDoc.class, idTipoUnitaDoc);
	gestioneParametriTipoUd(parametriAmministrazioneTipoUd, parametriConservazioneTipoUd,
		parametriGestioneTipoUd, tipoUnitaDoc);
	sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		param.getNomeUtente(), param.getNomeAzione(),
		SacerLogConstants.TIPO_OGGETTO_TIPO_UNITA_DOCUMENTARIA,
		new BigDecimal(tipoUnitaDoc.getIdTipoUnitaDoc()), param.getNomePagina());
    ...
    private void gestioneParametriTipoUd(AplParamApplicTableBean parametriAmministrazioneTipoUd,
	    AplParamApplicTableBean parametriConservazioneTipoUd,
	    AplParamApplicTableBean parametriGestioneTipoUd, DecTipoUnitaDoc tipoUnitaDoc) {
	// Gestione parametri amministrazione
	manageParametriPerTipoUd(parametriAmministrazioneTipoUd,
		"ds_valore_param_applic_tipo_ud_amm", tipoUnitaDoc);
	// Gestione parametri conservazione
	manageParametriPerTipoUd(parametriConservazioneTipoUd,
		"ds_valore_param_applic_tipo_ud_cons", tipoUnitaDoc);
	// Gestione parametri gestione
	manageParametriPerTipoUd(parametriGestioneTipoUd, "ds_valore_param_applic_tipo_ud_gest",
		tipoUnitaDoc);
    }
    ...
    /**
     * Metodo di salvataggio di una nuova associazione tra registri e tipi di unit\u00E0
     * documentarie
     *
     * @param param              parametri per logging
     * @param azioneInsCriterio  inserimento criterio
     * @param azioneModCriterio  modifica criterio
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void insertDecTipoUnitaDocAmmesso(LogParam param, String azioneInsCriterio,
	    String azioneModCriterio, BigDecimal idRegistroUnitaDoc, BigDecimal idTipoUnitaDoc)
	...
	if (registroUnitaDoc.getFlRegistroFisc().equals("1")) {
	    boolean areAllRegistriFiscali = critEjb
		    .checkAllRegistriAssociatiFiscali(idTipoUnitaDoc);
	    // ...verifico se esiste un criterio valido che usa il tipo ud dell'associazione
	    // Se NON esiste
	    if (!critEjb.existsCriterioPerTipoDato(idTipoUnitaDoc.longValue(),
		...
		if (areAllRegistriFiscali) {
			...
			// DEC_V_CREA_CRIT_RAGGR_TIPO_UD agisce sull'associazione, già creata
			// pocanzi
		...
		List<DecCriterioRaggr> criteriEsistenti = crHelper.getCriteriPerTipoDato(
			idTipoUnitaDoc.longValue(), TipoDato.TIPO_UNITA_DOC, "1", "0");
		if (!criteriEsistenti.isEmpty()) {
		    ...
		    sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
			    param.getNomeUtente(), param.getNomeAzione(),
			    SacerLogConstants.TIPO_OGGETTO_CRITERIO_RAGGRUPPAMENTO,
			    new BigDecimal(criteriEsistenti.get(0).getIdCriterioRaggr()),
			    param.getNomePagina());
	...
	if (!criteriNonCoerenti.isEmpty()) {
	    ...
	    errorMessage.append(
		    registroUnitaDoc.getFlRegistroFisc().equals("1") ? "fiscale " : "non fiscale ");
	    errorMessage.append(
	...
	try {
	    ...
	    throw new IllegalStateException(
		    "Errore durante il recupero dei registri associati al tipo unita doc");
	}
	return registroUnitaDocTableBean;
    }

    public DecXsdDatiSpecTableBean getDecXsdDatiSpecTableBean(BigDecimal idTipoUnitaDoc) {
	List<DecXsdDatiSpec> xsdDatiSpecList = helper
		.getDecXsdDatiSpecByTipoUnitaDoc(idTipoUnitaDoc);
	...
	try {
		...
		String dsVersioneXsd = xsdDatiSpecRowBean.getDsVersioneXsd() != null
			...
			: "";
		xsdDatiSpecRowBean.setString("descrizione",
			xsdDatiSpecRowBean.getCdVersioneXsd() + dsVersioneXsd);
	    ...
	    logger.error("Errore durante il recupero degli xsd associati al tipo unita doc "
		    + ExceptionUtils.getRootCauseMessage(e), e);
	    throw new IllegalStateException(
		    "Errore durante il recupero degli xsd associati al tipo unita doc");
	}
    ...
    public DecTipoUnitaDocAmmessoRowBean getDecTipoUnitaDocAmmessoRowBean(
	    BigDecimal idRegistroUnitaDoc, BigDecimal idTipoUnitaDoc) {
	DecTipoUnitaDocAmmessoRowBean tipoUnitaDocAmmessoRowBean = null;
	DecTipoUnitaDocAmmesso tipoUnitaDocAmmesso = helper
		.getDecTipoUnitaDocAmmessoByParentId(idRegistroUnitaDoc, idTipoUnitaDoc);
    ...
    /**
     * Metodo di salvataggio modifica di una nuova associazione tra registri e tipi di unit\u00E0
     * documentarie
     *
     * @param param                 parametri per logging
     * @param idTipoUnitaDocAmmesso id tipo unita doc ammessa
     * @param idRegistroUnitaDoc    id registro unita doc
     * @param idTipoUnitaDoc        id tipo unita doc
     *
     * @throws ParerUserError errore generico
	...
	if (helper.getDecTipoUnitaDocAmmessoByParentId(idRegistroUnitaDoc,
		idTipoUnitaDoc) != null) {
	    throw new ParerUserError(
    ...
    /**
     * Metodo di eliminazione di una nuova associazione tra registri e tipi di unit\u00E0
     * documentarie
     *
     * @param param                                  parametri per logging
     * @param azioneModCriterioPerDeleteAssociazione modifica criterio per cancellazione
     *                                               associazione
     * @param nmPaginaPerCriteri                     numero pagina per criteri
     * @param idTipoUnitaDocAmmesso                  id tipo unita doc ammesso
     *
     * @throws ParerUserError errore generico
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteDecTipoUnitaDocAmmesso(LogParam param,
	    String azioneModCriterioPerDeleteAssociazione, String nmPaginaPerCriteri,
	    BigDecimal idTipoUnitaDocAmmesso) throws ParerUserError {
	DecTipoUnitaDocAmmesso tipoUnitaDocAmmesso = helper.findById(DecTipoUnitaDocAmmesso.class,
		idTipoUnitaDocAmmesso);
	BigDecimal idTipoUnitaDoc = new BigDecimal(
		tipoUnitaDocAmmesso.getDecTipoUnitaDoc().getIdTipoUnitaDoc());
	...
	List<DecCriterioRaggr> criteriAssociazione = crHelper.getCriteriPerTipoDato(
		tipoUnitaDocAmmesso.getDecTipoUnitaDoc().getIdTipoUnitaDoc(),
		TipoDato.TIPO_UNITA_DOC, null, null);
	for (DecCriterioRaggr criterioAssociazione : criteriAssociazione) {
	    // Gestione del flag fiscale
	    String flCriterioRaggrFiscMessage = crHelper.getFlCriterioRaggrFiscMessage(
		    new BigDecimal(criterioAssociazione.getIdCriterioRaggr()));
	    if (flCriterioRaggrFiscMessage
		    .equals(ApplEnum.FlagFiscaleMessage.FISCALE.getDescrizione())) {
		if (criterioAssociazione.getFlCriterioRaggrFisc().equals("0")) {
		    ...
		    sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
			    param.getNomeUtente(), param.getNomeAzione(),
			    SacerLogConstants.TIPO_OGGETTO_CRITERIO_RAGGRUPPAMENTO,
			    BigDecimal.valueOf(criterioAssociazione.getIdCriterioRaggr()),
			    nmPaginaPerCriteri);
		}
	    } else if (flCriterioRaggrFiscMessage
		    .equals(ApplEnum.FlagFiscaleMessage.NON_FISCALE.getDescrizione())) {
     ...
     */
    public DecTipoUnitaDocTableBean getTipiUnitaDocAbilitati(long idUtente,
    ...
    public DecTipoUnitaDocRowBean getDecTipoUnitaDocRowBean(String nmTipoUnitaDoc,
	    BigDecimal idStrut) {
	...
	if (tipoUnitaDoc != null) {
	    try {
		tipoUnitaDocRowBean = (DecTipoUnitaDocRowBean) Transform
		...
		logger.error("Errore durante il recupero della tipologia unità documentarie "
			+ ExceptionUtils.getRootCauseMessage(e), e);
		throw new IllegalStateException(
			"Errore durante il recupero della tipologia unità documentarie ");
	    }
	}
    ...
    public DecTipoUnitaDocRowBean getDecTipoUnitaDocRowBean(BigDecimal idTipoUnita,
	    BigDecimal idStrut) {
	DecTipoUnitaDocRowBean tipoUnitaDocRowBean = getDecTipoUnitaDoc(idTipoUnita, null, idStrut);
	return tipoUnitaDocRowBean;
    }

    public DecTipoUnitaDocRowBean getDecTipoUnitaDoc(BigDecimal idTipoUnitaDoc,
	    String nmTipoUnitaDoc, BigDecimal idStrut) {
	...
	try {
		...
		for (DecTipoUnitaDoc tipoUnitaDoc : list) {
		    ...
		    DecXsdDatiSpec lastXsd = datiSpecHelper.getLastDecXsdDatiSpecForTipoUnitaDoc(
			    idStrut.longValue(), tipoUnitaDoc.getIdTipoUnitaDoc());
		    if (lastXsd != null) {
		    ...
		    List<DecTipoUnitaDocAmmesso> listatipiunitadocAmmessi = tipoUnitaDoc
		    ...
		    List<DecUsoModelloXsdUniDoc> usoModelloXsdUniDoc = tipoUnitaDoc
			...
			tmpBuilder.append(
				dsXsdUn.stream().distinct().collect(Collectors.joining("; ")));
		    }
		    ...
		    tmpBuilder = new StringBuilder();
		    List<DecTipoStrutUnitaDoc> listaTipoStrutUnitaDoc = tipoUnitaDoc
		    ...
		    String flagStandard = critEjb.getCriterioStandardPerTipoDatoAnno(
			    tipoUnitaDoc.getIdTipoUnitaDoc(), TipoDato.TIPO_UNITA_DOC);
		    tipoUnitaRow.setString("flag_criterio_standard", flagStandard);
		    String a = null;

		    // Recupero i Sistemi versanti
		    tipoUnitaRow.setString("nm_sistema_versante",
			    helper.getNmSistemiVersantiRaggruppatiPerTipoUd(
				    new BigDecimal(tipoUnitaDoc.getIdTipoUnitaDoc())));

		    Object[] sisVersPerTipoUd = helper.getDtErogSistemiVersantiPerTipoUd(
			    new BigDecimal(tipoUnitaDoc.getIdTipoUnitaDoc()));
		    ...
		    tipoUnitaRow.setString("fl_parametri_tipo_ud",
			    existsValoreParamApplicTipoUd ? "1" : "0");
    ...
    public DecTipoStrutUnitaDocRowBean getDecTipoStrutUnitaDocRowBean(
	    BigDecimal idTipoStrutUnitaDoc) {
	DecTipoStrutUnitaDocRowBean tipoStrutUnitaDocRowBean = getDecTipoStrutUnitaDoc(
		idTipoStrutUnitaDoc, null, null);
	...
	DecTipoStrutUnitaDocRowBean tipoStrutUnitaDocRowBean = getDecTipoStrutUnitaDoc(
		BigDecimal.ZERO, nmTipoStrutUnitaDoc, idTipoUnitaDoc);
	return tipoStrutUnitaDocRowBean;
	...
	if (idTipoStrutUnitaDoc == BigDecimal.ZERO && nmTipoStrutUnitaDoc != null) {
	    tipoStrutUnitaDoc = helper.getDecTipoStrutUnitaDocByName(nmTipoStrutUnitaDoc,
		    idTipoUnitaDoc);
	}
	...
	if (tipoStrutUnitaDoc != null) {
	    try {
		tipoStrutUnitaDocRowBean = (DecTipoStrutUnitaDocRowBean) Transform
			.entity2RowBean(tipoStrutUnitaDoc);
	    } catch (Exception e) {
		logger.error(e.getMessage(), e);
	    }
	}
	...
	try {
	    if (!list.isEmpty()) {
		tipoUnitaDocAmmessoTableBean = (DecVLisTiUniDocAmsTableBean) Transform
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void insertDecTipoStrutUnitaDoc(LogParam param,
	    DecTipoStrutUnitaDocRowBean tipoStrutUnitaDocRowBean,
	...
	if (helper.getDecTipoStrutUnitaDocByName(tipoStrutUnitaDocRowBean.getNmTipoStrutUnitaDoc(),
	    ...
	    throw new ParerUserError(
		    "Tipo struttura gi\u00E0 esistente all'interno della struttura</br>");
	}
	...
	tipoStrutUnitaDoc
		.setDsAnnoTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getDsAnnoTipoStrutUnitaDoc());
	tipoStrutUnitaDoc
		.setDsDataTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getDsDataTipoStrutUnitaDoc());
	tipoStrutUnitaDoc.setDsNumeroTipoStrutUnitaDoc(
		tipoStrutUnitaDocRowBean.getDsNumeroTipoStrutUnitaDoc());
	tipoStrutUnitaDoc
		.setDsOggTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getDsOggTipoStrutUnitaDoc());
	tipoStrutUnitaDoc
		.setDsRifTempTipoStrutUd(tipoStrutUnitaDocRowBean.getDsRifTempTipoStrutUd());
	...
	tipoStrutUnitaDoc
		.setAaMaxTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getAaMaxTipoStrutUnitaDoc());
	tipoStrutUnitaDoc
		.setAaMinTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getAaMinTipoStrutUnitaDoc());
	...
	if (idSistemaVersanteList != null && !idSistemaVersanteList.isEmpty()) {
		...
		tipoStrutUdSisVers.setAplSistemaVersante(
			helper.findById(AplSistemaVersante.class, idSistemaVersante));
	...
	if (idRegistroUnitaDocList != null && !idRegistroUnitaDocList.isEmpty()) {
		...
		tipoStrutUdReg.setDecRegistroUnitaDoc(
			helper.findById(DecRegistroUnitaDoc.class, idRegistroUnitaDoc));
	...
	if (idXsdDatiSpecList != null && !idXsdDatiSpecList.isEmpty()) {
		...
		tipoStrutUdXsd
			.setDecXsdDatiSpec(helper.findById(DecXsdDatiSpec.class, idXsdDatiSpec));
	...
	DecTipoStrutUnitaDoc dbTipoStrutUnitaDoc = helper.getDecTipoStrutUnitaDocByName(
		tipoStrutUnitaDocRowBean.getNmTipoStrutUnitaDoc(),
		tipoStrutUnitaDocRowBean.getIdTipoUnitaDoc());
	if (dbTipoStrutUnitaDoc != null && dbTipoStrutUnitaDoc
		.getIdTipoStrutUnitaDoc() != idTipoStrutUnitaDoc.longValue()) {
	    throw new ParerUserError(
		    "Nome tipo struttura unit\u00E0 documentaria gi\u00E0 associato a questa struttura all'interno del database</br>");
	}

	DecTipoStrutUnitaDoc tipoStrutUnitaDoc = struttureHelper
		.findById(DecTipoStrutUnitaDoc.class, idTipoStrutUnitaDoc);
	tipoStrutUnitaDoc.setNmTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getNmTipoStrutUnitaDoc());
	tipoStrutUnitaDoc.setDsTipoStrutUnitaDoc(tipoStrutUnitaDocRowBean.getDsTipoStrutUnitaDoc());
	tipoStrutUnitaDoc.setDtIstituz(tipoStrutUnitaDocRowBean.getDtIstituz());
	tipoStrutUnitaDoc.setDtSoppres(tipoStrutUnitaDocRowBean.getDtSoppres());
	...
	if (idSistemaVersanteList != null && !idSistemaVersanteList.isEmpty()) {
	    ...
	    for (DecTipoStrutUdSisVer tipoStrutUdSisVer : tipoStrutUnitaDoc
		    .getDecTipoStrutUdSisVers()) {
		boolean trovato = false;
		for (BigDecimal idSistemaVersante : idSistemaVersanteList) {

		    if (idSistemaVersante.compareTo(BigDecimal.valueOf(tipoStrutUdSisVer
			    .getAplSistemaVersante().getIdSistemaVersante())) == 0) {
			trovato = true;
		    }
		}
		if (!trovato) {
		    helper.deleteDecTipoStrutUdSisVers(tipoStrutUnitaDoc.getIdTipoStrutUnitaDoc(),
			    BigDecimal.valueOf(tipoStrutUdSisVer.getAplSistemaVersante()
	...
	if (idRegistroUnitaDocList != null && !idRegistroUnitaDocList.isEmpty()) {
		...
		for (BigDecimal idRegistroUnitaDoc : idRegistroUnitaDocList) {
		    ...
		    if (idRegistroUnitaDoc.compareTo(BigDecimal.valueOf(tipoStrutUdReg
			    .getDecRegistroUnitaDoc().getIdRegistroUnitaDoc())) == 0) {
			trovato = true;
		    }
		}
		if (!trovato) {
	...
	if (idXsdDatiSpecList != null && !idXsdDatiSpecList.isEmpty()) {
		...
		for (BigDecimal idXsdDatiSpec : idXsdDatiSpecList) {
		    ...
		    if (idXsdDatiSpec.compareTo(BigDecimal
			    .valueOf(tipoStrutUdXsd.getDecXsdDatiSpec().getIdXsdDatiSpec())) == 0) {
			trovato = true;
		    }
		}
		if (!trovato) {
    ...
    public void insertDecTipoStrutUdSisVers(DecTipoStrutUnitaDoc tipoStrutUnitaDoc,
    ...
    public void insertDecTipoStrutUdReg(DecTipoStrutUnitaDoc tipoStrutUnitaDoc,
    ...
    public void insertDecTipoStrutUdXsd(DecTipoStrutUnitaDoc tipoStrutUnitaDoc,
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public IamOrganizDaReplic deleteTipoUnitaDocPuntuale(LogParam param, long idTipoUnitaDoc)
	    throws ParerUserError {
	return eseguiDeleteTipoUnitaDoc(param, idTipoUnitaDoc, false);
    }

    /**
     * Business method per la cancellazione di un tipo unità documentaria di una determinata
     * struttura. Il metodo esegue le seguenti istruzioni: 1) Controlla i legami con Tipologia Serie
     * e Unità documentarie; 2) Controllo i legami con criteri di raggruppamento e, se può, li
     * elimina 3) Esegue la cancellazione del Tipo Ud con cascade 4) Scrive il record
	...
	boolean existsRelationsWithTipiSerie = tipoSerieHelper.existsRelationsWithTipiSerie(
	...
	boolean existsRelationsWithUnitaDoc = unitaDocHelper.existsRelationsWithUnitaDoc(
		tipoUnitaDoc.getIdTipoUnitaDoc(), Constants.TipoDato.TIPO_UNITA_DOC);
	...
	boolean existsRelationsWithModello = modelliSerieHelper.existsRelationsWithModello(
		tipoUnitaDoc.getIdTipoUnitaDoc(), Constants.TipoDato.TIPO_UNITA_DOC);
	...
	List<DecCriterioFiltroMultiplo> criteriAssociati = struttureHelper
		.getRelationsWithCriteriRaggruppamento(tipoUnitaDoc.getIdTipoUnitaDoc(),
	...
	IamOrganizDaReplic replic = null;
	/*
	 * La replica va fatta solo se la tipologia unità documentaria apparteneva ad una struttura
	 * non appartenente ad un ente di tipo template
	 */
	...
	logger.info("Cancellazione tipo unità documentaria " + nmTipoUnitaDoc + " della struttura "
		+ idStrut + " avvenuta con successo!");
	return replic;
    }

    public DecTipoDocAmmessoTableBean getDecTipoDocAmmessoTableBean(
	    BigDecimal idTipoStrutUnitaDoc) {
	...
	try {
		...
		for (DecTipoDocAmmesso row : list) {
		    DecTipoDocAmmessoRowBean rowBean = (DecTipoDocAmmessoRowBean) Transform
			    .entity2RowBean(row);
    ...
    public DecTipoDocAmmessoRowBean getDecTipoDocAmmessoRowBean(BigDecimal idTipoDocAmmesso,
	    BigDecimal idTipoDoc, BigDecimal idTipoStrutUnitaDoc) {
	...
	if (tipoDocAmmesso != null) {
	    try {
		tipoDocAmmessoRowBean = (DecTipoDocAmmessoRowBean) Transform
			.entity2RowBean(tipoDocAmmesso);
	    } catch (Exception e) {
		logger.error(e.getMessage(), e);
	    }
	}
    ...
    public DecTipoDocAmmessoRowBean getDecTipoDocAmmessoRowBean(BigDecimal idTipoDocAmmesso,
	    BigDecimal idTipoDoc, BigDecimal idTipoStrutUnitaDoc, String tiDoc) {
	DecTipoDocAmmessoRowBean tipoDocAmmessoRowBean = null;
	List<DecTipoDocAmmesso> tipoDocAmmessoList = helper
		.getDecTipoDocAmmessoList(idTipoDocAmmesso, idTipoDoc, idTipoStrutUnitaDoc, tiDoc);
	if (!tipoDocAmmessoList.isEmpty()) {
	    try {
		tipoDocAmmessoRowBean = (DecTipoDocAmmessoRowBean) Transform
			.entity2RowBean(tipoDocAmmessoList.get(0));
	    } catch (Exception e) {
		logger.error(e.getMessage(), e);
	    }
	}
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void insertDecTipoDocAmmesso(LogParam param,
	    DecTipoDocAmmessoRowBean tipoDocAmmessoRowBean) {
	...
	DecTipoDoc tipoDoc = helper.findById(DecTipoDoc.class,
	...
	sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
		param.getNomeUtente(), param.getNomeAzione(),
		SacerLogConstants.TIPO_OGGETTO_TIPO_UNITA_DOCUMENTARIA,
		new BigDecimal(tipoDocAmmesso.getDecTipoStrutUnitaDoc().getDecTipoUnitaDoc()
			.getIdTipoUnitaDoc()),
		param.getNomePagina());
    }
	...
	return helper.existsDecTipoDocAmmesso(idTipoDocAmmesso, idTipoDoc, idTipoStrutUnitaDoc,
		tipoElemento);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateDecTipoDocAmmesso(LogParam param, BigDecimal idTipoDocAmmesso,
	    DecTipoDocAmmessoRowBean tipoDocAmmessoRowBean, String tiDocIniziale)
	    throws ParerUserError {
	DecTipoDoc tipoDoc = helper.findById(DecTipoDoc.class,
		tipoDocAmmessoRowBean.getIdTipoDoc());
	if (!tipoDocAmmessoRowBean.getTiDoc().equals(tiDocIniziale) && helper
		.existsDecTipoDocAmmesso(idTipoDocAmmesso, tipoDocAmmessoRowBean.getIdTipoDoc(),
			tipoDocAmmessoRowBean.getIdTipoStrutUnitaDoc(),
			tipoDocAmmessoRowBean.getTiDoc())) {
	    throw new ParerUserError(
		    "Associazione gi\u00E0 esistente all'interno della struttura per il tipo documento "
			    + tipoDoc.getNmTipoDoc() + " elemento "
			    + tipoDocAmmessoRowBean.getTiDoc() + "</br>");
	}
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteDecTipoStrutUnitaDoc(LogParam param,
	    DecTipoStrutUnitaDocRowBean tipoStrutUnitaDocRowBean) throws ParerUserError {
	DecTipoStrutUnitaDoc tipoStrutUnitaDoc = helper.findById(DecTipoStrutUnitaDoc.class,
		tipoStrutUnitaDocRowBean.getIdTipoStrutUnitaDoc());
	boolean isEmptyRelation = helper.checkRelationsAreEmptyForDecTipoStrutUnitaDoc(
		tipoStrutUnitaDoc.getIdTipoStrutUnitaDoc());
	if (isEmptyRelation) {
	    throw new ParerUserError(
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteDecTipoDocAmmesso(LogParam param,
	    DecTipoDocAmmessoRowBean tipoDocAmmessoRowBean) {
	DecTipoDocAmmesso tipoDocAmmesso = helper.findById(DecTipoDocAmmesso.class,
    ...
    public DecCategTipoUnitaDocTableBean getDecCategTipoUnitaDocChildTableBean(
	    BigDecimal idCategStrut) {
	DecCategTipoUnitaDocTableBean tableBean = new DecCategTipoUnitaDocTableBean();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void insertDecCategTipoUnitaDoc(DecCategTipoUnitaDocRowBean categTipoUnitaDocRowBean)
	    throws ParerUserError {
	if (categTipoUnitaDocRowBean != null) {
	    if (helper.getDecCategTipoUnitaDocByCodeLike(
		    categTipoUnitaDocRowBean.getCdCategTipoUnitaDoc()) != null) {
		throw new ParerUserError(
			"Codice categoria gi\u00E0 utilizzato all'interno del database.");
	    }
	}
	...
	try {
	    categTipoUnitaDocRowBean = (DecCategTipoUnitaDocRowBean) Transform
		    .entity2RowBean(categTipoUnitaDoc);
	} catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		| IllegalAccessException | IllegalArgumentException
		| InvocationTargetException ex) {
	...
	DecCategTipoUnitaDoc categToRemove = helper
		.getDecCategTipoUnitaDocByCodeLike(cdCategTipoUnitaDoc);
	...
	if (helper
		.getListDecCategTipoUnitaDocInUse(categToRemove.getIdCategTipoUnitaDoc()) != null) {
	    throw new ParerUserError(
		    "Nodo utilizzato da Tipi Unita Documentarie. Impossibile cancellare");
	}

	helper.removeEntity(categToRemove, true);
    }

    private boolean nodeDecCategTipoUnitaDocHasChild(String cdCategTipoUnitaDoc,
	    BigDecimal idToRemove) {
	if (idToRemove == null) {
	    idToRemove = new BigDecimal(
		    helper.getDecCategTipoUnitaDocByCodeLike(cdCategTipoUnitaDoc)
			    .getIdCategTipoUnitaDoc());
	}
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void updateDecCategTipoUnitaDoc(BigDecimal idToUpdate,
	    DecCategTipoUnitaDocRowBean categTipoUnitaDocRowBean) throws ParerUserError {
	DecCategTipoUnitaDoc categDB = helper.getDecCategTipoUnitaDocByCodeLike(
		categTipoUnitaDocRowBean.getCdCategTipoUnitaDoc());
	if ((categDB != null) && (categDB.getIdCategTipoUnitaDoc() != idToUpdate.longValue())) {
	    throw new ParerUserError(
		    "Codice categoria gi\u00E0 utilizzato all'interno del database.");
	}

	DecCategTipoUnitaDoc categTipoUnitaDoc = struttureHelper
		.findById(DecCategTipoUnitaDoc.class, idToUpdate);
	DecCategTipoUnitaDoc categTipoUnitaDocPadre = (categTipoUnitaDocRowBean
		.getIdCategTipoUnitaDocPadre() != null
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void moveDecCategTipoUDNode(BigDecimal nodeId, BigDecimal nodeDestId)
	    throws ParerUserError {
	...
	try {
	    ...
	    List<OrgTipoServizio> tipiServizioList = helper
		    .retrieveOrgTipoServizioList(tiClasseTipoServizio);

	    if (!tipiServizioList.isEmpty()) {
		tipoServizioTableBean = (OrgTipoServizioTableBean) Transform
			.entities2TableBean(tipiServizioList);
	    }
	} catch (Exception e) {
	    logger.error(e.getMessage(), e);
	}
    ...
    public DecVCalcTiServOnTipoUdRowBean getDecVCalcTiServOnTipoUd(BigDecimal idStrut,
	...
	try {
	    ...
	    DecVCalcTiServOnTipoUd tipoServizioOnTipoUd = helper.getDecVCalcTiServOnTipoUd(idStrut,
		    idCategTipoUnitaDoc, cdAlgoTariffario);
	...
	try {
	    List<Object[]> sistemiVersantiList = helper
		    .retrieveAplSistemaVersanteListPerTipoUd(idTipoUnitaDoc);
		...
		/*
		 * LS: su jboss le entity restitiscono sempre delle java.util.Date, non c'è più
		 * alcuna mediazione da parte di annotazioni come la "ORACLE_DATE" fatta su
		 * glassfish. I rowbean necessitano dei java.sql.Timestamp.
		 */
    ...
    public List<String> getIdSistemaVersanteListByTipoStrutUnitaDoc(
	    BigDecimal idTipoStrutUnitaDoc) {
	DecTipoStrutUnitaDoc tipiStrutUnitaDoc = helper.findById(DecTipoStrutUnitaDoc.class,
		idTipoStrutUnitaDoc);
	List<String> idSistemaVersanteList = new ArrayList<>();
	for (DecTipoStrutUdSisVer tipoStrutUdSisVer : tipiStrutUnitaDoc
		.getDecTipoStrutUdSisVers()) {
	    idSistemaVersanteList
		    .add("" + tipoStrutUdSisVer.getAplSistemaVersante().getIdSistemaVersante());
	}
	return idSistemaVersanteList;
    }

    public List<String> getIdRegistroUnitaDocListByTipoStrutUnitaDoc(
	    BigDecimal idTipoStrutUnitaDoc) {
	DecTipoStrutUnitaDoc tipiStrutUnitaDoc = helper.findById(DecTipoStrutUnitaDoc.class,
		idTipoStrutUnitaDoc);
	...
	for (DecTipoStrutUdReg tipoStrutUdReg : tipiStrutUnitaDoc.getDecTipoStrutUdRegs()) {
	    idRegistroUnitaDocList
		    .add("" + tipoStrutUdReg.getDecRegistroUnitaDoc().getIdRegistroUnitaDoc());
	}
    ...
    /**
     * Ottiene la configurazione del backend utilizzato per la tipologia di ud rispetto al
     * versamento sincrono.
     *
     * @param idTipoUnitaDoc id tipo unita doc
     *
    ...
    /**
     * Ottiene la configurazione del backend utilizzato per la tipologia di ud rispetto all'aggiunta
     * documenti.
     *
     * @param idTipoUnitaDoc id tipo unita doc
     *
	...
	try {
		...
		versioneWsTableBean = (DecVersioneWsTableBean) Transform
			.entities2TableBean(versioneWsList);
	    }
	} catch (Exception e) {
	    logger.error(e.getMessage(), e);
	}

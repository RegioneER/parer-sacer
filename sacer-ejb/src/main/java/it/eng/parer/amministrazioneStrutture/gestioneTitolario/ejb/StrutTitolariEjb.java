...
public class StrutTitolariEjb {
     ...
     *
     * @param titolario             Intestazione del titolario
     * @param vociMap               Albero delle voci ordinate
     * @param levelsList            lista oggetti di tipo {@link LivelloType}
     * @param idStrut               Id Struttura
     * @param cdRegistroKeyUnitaDoc Registro
     * @param aaKeyUnitaDoc         Anno
     * @param cdKeyUnitaDoc         Numero
     * @param dtDocInvio            Data invio
     * @param dtIstituz             Data istituzione del titolario
     * @param dtSoppres             Data soppressione del titolario
     *
     * @return pk DecTitolario inserito
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public long saveMap(TitolarioType titolario, Map<String, Voce> vociMap,
	    List<LivelloType> levelsList, BigDecimal idStrut, String cdRegistroKeyUnitaDoc,
	    BigDecimal aaKeyUnitaDoc, String cdKeyUnitaDoc, Date dtDocInvio, Date dtIstituz,
	    Date dtSoppres) {
	...
	OrgOperTitol orgOperTitol = context.getBusinessObject(StrutTitolariEjb.class)
		.saveOrgOperTitol(Voce.Operation.CREA, strut, aaKeyUnitaDoc, cdKeyUnitaDoc,
			cdRegistroKeyUnitaDoc, dtDocInvio, dtIstituz, titol);
	...
	context.getBusinessObject(StrutTitolariEjb.class).alterVociRicorsivo(vociMap, livelliMap,
		null, titol, orgOperTitol, counterVoci, new ArrayList<String>(),
		new HashSet<Voce>(), false);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void saveMap(BigDecimal idTitolario, TitolarioType titolarioType,
	    Map<String, Voce> vociMap, String cdRegistroKeyUnitaDoc, BigDecimal aaKeyUnitaDoc,
	    String cdKeyUnitaDoc, Date dtDocInvio, Date dtIstituz) {
	...
	OrgOperTitol orgOperTitol = context.getBusinessObject(StrutTitolariEjb.class)
		.saveOrgOperTitol(Voce.Operation.MODIFICA, strut, aaKeyUnitaDoc, cdKeyUnitaDoc,
			cdRegistroKeyUnitaDoc, dtDocInvio, dtIstituz, titolario);
	...
	context.getBusinessObject(StrutTitolariEjb.class).alterVociRicorsivo(vociMap, livelliMap,
		null, titolario, orgOperTitol, counterVoci, codiceVociChiuse, vociDaChiudere,
		false);
	logger.info("Eseguo la chiusura delle voci da chiudere");
	for (Voce voce : vociDaChiudere) {
	    voce.setOperation(Voce.Operation.CHIUDI);
	    context.getBusinessObject(StrutTitolariEjb.class).chiudiVoce(voce, titolario,
		    orgOperTitol, counterVoci);
    ...
    public Map<String, DecLivelloTitol> createLivelliMap(DecTitol titol,
	    List<LivelloType> levelsList) {
	Map<String, DecLivelloTitol> livelliMap = new HashMap<>();
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public OrgOperTitol saveOrgOperTitol(Voce.Operation operation, OrgStrut strut,
	    BigDecimal aaKeyUnitaDoc, String cdKeyUnitaDoc, String cdRegistroKeyUnitaDoc,
	    Date dtDocInvio, Date dtIstituz, DecTitol titol) {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void alterVociRicorsivo(Map<String, Voce> vociMap,
	    Map<String, DecLivelloTitol> livelliMap, DecVoceTitol vocePadre, DecTitol titol,
	    OrgOperTitol orgOperTitol, MutableInt counterVoci, List<String> codiceVociChiuse,
	    Set<Voce> vociDaChiudere, boolean toClose) {
	...
	for (Entry<String, Voce> entry : vociMap.entrySet()) {
		...
		if (voceXml.getOperation() == Voce.Operation.CREA
		    ...
		    logger.info("VOCE " + voceXml.getCodiceVoceComposito()
			    + " preesistente - La metto in modifica");
		    voceXml.setOperation(Voce.Operation.MODIFICA);
		}

		if (voceXml.getOperation() == Voce.Operation.CREA) {
		    voce = context.getBusinessObject(StrutTitolariEjb.class).creaVoce(voceXml,
			    entry.getKey(), titol, vocePadre, livelliMap, orgOperTitol, counterVoci,
			    calSoppres.getTime());
		    counterVoci.increment();
		    if (toClose) {
		    ...
		    voce = context.getBusinessObject(StrutTitolariEjb.class).modificaVoce(voceXml,
			    titol, orgOperTitol, counterVoci, calSoppres.getTime());
		    counterVoci.increment();

		    if (voceXml.getDataFineValidita() != null
			    && !voceXml.getDataFineValidita().equals(calSoppres.getTime())
			    && voceXml.getDataFineValidita().getTime() != voce.getDtSoppres()
				    .getTime()) {
			// Se la data fine validità è diversa da quella della voce, la voce è da
			// chiudere.
		...
		logger.info("VOCE " + voceXml.getCodiceVoceComposito()
			+ " - NESSUNA OPERAZIONE da xml");
		if (toClose) {
		    logger.info("----- DA CHIUDERE");
		    /*
		     * E' un nodo figlio di un nodo chiuso. Lo mantengo alla fine in una lista di
		     * nodi da chiudere, nel caso lo stesso nodo figlio abbia operazioni in sospeso.
		     * L'operazione viene eseguita, e poi viene fatta la chiusura
		     */
		    vociDaChiudere.add(voceXml);
		} else {
		...
		context.getBusinessObject(StrutTitolariEjb.class).alterVociRicorsivo(
			voceXml.getFigli(), livelliMap, voce, titol, orgOperTitol, counterVoci,
			codiceVociChiuse, vociDaChiudere, toClose);
	    }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public DecVoceTitol creaVoce(final Voce voceXml, String cdVoce, DecTitol titol,
	    DecVoceTitol vocePadre, Map<String, DecLivelloTitol> livelliMap,
	    OrgOperTitol orgOperTitol, MutableInt counterVoci, Date dtDefaultSoppres) {
	DecVoceTitol voce = new DecVoceTitol();
	voce.setCdCompositoVoceTitol(
		StringEscapeUtils.escapeEcmaScript(voceXml.getCodiceVoceComposito()));
	...
	voce.setDtSoppres(voceXml.getDataFineValidita() != null ? voceXml.getDataFineValidita()
		: dtDefaultSoppres);
	...
	context.getBusinessObject(StrutTitolariEjb.class).saveDecValVoceTitol(voce, voceXml,
		voce.getDtIstituz(), voce.getDtSoppres());

	context.getBusinessObject(StrutTitolariEjb.class).saveOrgOperVoceTitol(orgOperTitol,
		voceXml, counterVoci, voce, voceXml.getDataInizioValidita());

	return voce;
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public DecVoceTitol modificaVoce(Voce voceXml, DecTitol titol, OrgOperTitol orgOperTitol,
	    MutableInt counterVoci, Date dtDefaultSoppres) {
	...
	if (voceXml.getDataFineValidita() != null
		...
		/*
		 * E' stata modificata la data fine validità del nodo, di conseguenza lascio
		 * invariate le date in fase di modifica, ma eseguo anche la chiusura del nodo
		 */
		toClose = true;
	    } else {
	...
	if (valVoce.getDtIniVal().compareTo(orgOperTitol.getDtValOperTitol()) == 0) {
		...
		valVoce.setDtFinVal(
			voceXml.getDataFineValidita() != null ? voceXml.getDataFineValidita()
				: dtDefaultSoppres);
	    }
		...
		dataDefinitiva = voceXml.getDataFineValidita() != null
	...
	context.getBusinessObject(StrutTitolariEjb.class).saveOrgOperVoceTitol(orgOperTitol,
		voceXml, counterVoci, voce, orgOperTitol.getDtValOperTitol());
	return voce;
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public DecVoceTitol chiudiVoce(Voce voceXml, DecTitol titol, OrgOperTitol orgOperTitol,
	    MutableInt counterVoci) {
	Calendar calSoppres = Calendar.getInstance();
	calSoppres.set(2444, Calendar.DECEMBER, 31, 0, 0, 0);
	calSoppres.set(Calendar.MILLISECOND, 0);
	...
	context.getBusinessObject(StrutTitolariEjb.class).saveOrgOperVoceTitol(orgOperTitol,
		voceXml, counterVoci, voce,
		(voceXml.getDataFineValidita().equals(calSoppres.getTime())
			? orgOperTitol.getDtValOperTitol()
			: voceXml.getDataFineValidita()));

	voce.setDtSoppres(voceXml.getDataFineValidita().equals(calSoppres.getTime())
		? orgOperTitol.getDtValOperTitol()
		: voceXml.getDataFineValidita());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void saveOrgOperVoceTitol(OrgOperTitol orgOperTitol, final Voce voceXml,
	    MutableInt counterVoci, DecVoceTitol voce, Date dtValidita) {
	...
	return helper.existChiaveUd(idStrut, cdRegistroKeyUnitaDoc, aaKeyUnitaDoc, cdKeyUnitaDoc,
		dtDocInvio);
    }
    ...
    public DecVTreeTitolTableBean getDecVociTreeTableBean(BigDecimal idTitol, Date dtValidita,
	    boolean addTitolRoot) {
	...
	if (treeList != null && !treeList.isEmpty()) {
		...
		for (DecVTreeTitol voce : treeList) {
		    DecVTreeTitolRowBean row = (DecVTreeTitolRowBean) Transform
			    .entity2RowBean(voce);
		    row.setCdCompositoVoceTitol(
			    StringEscapeUtils.unescapeJava(voce.getCdCompositoVoceTitol()));
		    ...
		    row.setString("nome_composito",
			    row.getCdCompositoVoceTitol()
				    + (StringUtils.isNotBlank(voce.getDsVoceTitol())
					    ? " - " + row.getDsVoceTitol()
					    : ""));
		    String vocePadre = row.getString("cd_composito_voce_padre");
		    row.setString("cd_composito_voce_padre",
			    StringEscapeUtils.unescapeJava(vocePadre));
		    if (voce.getNiFascicVociFiglie().intValue() > 0
			    || voce.getNiFascic().intValue() > 0) {
		...
		logger.error("Errore durante il recupero dei titolari "
    ...
    public DecVTreeTitolTableBean getDecVociTreeTableBean(String nomeTitolario,
	...
	List<Map.Entry<String, Voce>> list = new LinkedList<Map.Entry<String, Voce>>(
		vociMap.entrySet());
    ...
    public void getVoceRicorsiva(Voce root, DecVTreeTitolTableBean treeTableBean,
	    BigDecimal idRoot) {
	...
	row.setString("nome_composito",
		root.getCodiceVoce() + (StringUtils.isNotBlank(root.getDescrizioneVoce())
			? " - " + root.getDescrizioneVoce()
			: ""));
	row.setString("enum_icon", Tree.IconColours.BLUE.name());
    ...
    private OperazioneCreaType getOperazioneCreaType(Voce root)
	    throws DatatypeConfigurationException {
	...
	if (oper != null) {
	    ...
	    row.setTimestamp("dt_doc_invio",
		    (oper.getDtDocInvio() != null ? new Timestamp(oper.getDtDocInvio().getTime())
			    : null));
	}
	return row;
    }
	...
	if (list != null && !list.isEmpty()) {
		...
		table = (DecVLisValVoceTitolTableBean) Transform.entities2TableBean(list);
		    ...
		    | InvocationTargetException ex) {
		logger.error(
			"Errore durante il recupero della traccia delle voci di classificazione "
    ...
    public Map<String, Voce> getVociMap(BigDecimal idTitol, List<LivelloType> livelli,
	    AttributesTitolario attribs) throws ParerUserError {
	...
	if (treeList != null && !treeList.isEmpty()) {
		...
		for (Pattern regEx : regExs) {
			...
			Voce newVoce = new Voce(codiceVoce, descVoce,
				voce.getNiOrdVoceTitol().intValue(), voce.getDtIniVal(),
				voce.getDtFinVal(),
				Voce.AttivoClass.fromValue(voce.getFlUsoClassif()),
				voce.getNiAnniConserv().intValue(), voce.getDlNote());
			...
			if (attribs != null) {
			    ...
			    attribs.getLivelliVociMap().get(new BigDecimal(liv.getNumeroLivello()))
				    .add(row);
			}
			break;
		    }
	...
	if (treeList != null && !treeList.isEmpty()) {
		...
		for (DecVTreeTitol t : treeList) {
		    ...
		    rowbean.setString("ds_tot_voce_titol",
			    t.getCdCompositoVoceTitol() + " - " + t.getDsVoceTitol());
		    result.add(rowbean);
		}
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
	...
	try {
	    ...
	    logger.error("Errore inaspettato nella validazione del titolario : "
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void chiudiTitolario(BigDecimal idTitolario, Map<String, Voce> vociMap,
	    Date dataFineValidita, String note) throws ParerUserError {
	logger.info("Recupero titolario");
	DecTitol titolario = helper.findById(DecTitol.class, idTitolario.longValue());
	OrgStrut strut = titolario.getOrgStrut();
	...
	OrgOperTitol orgOperTitol = context.getBusinessObject(StrutTitolariEjb.class)
		.saveOrgOperTitol(Voce.Operation.CHIUDI, strut, null, null, null, null,
			dataFineValidita, titolario);
	Map<String, DecLivelloTitol> livelliMap = new HashMap<>();
	for (DecLivelloTitol livello : titolario.getDecLivelloTitols()) {
	    livelliMap.put(livello.getNmLivelloTitol(), livello);
	}
    ...
    public AroVRicUnitaDocTableBean getUnitaDocTableBean(String cdRegistroKeyUnitaDoc,
	...
	AroVRicUnitaDoc unitaDoc = helper.getUnitaDoc(cdRegistroKeyUnitaDoc, aaKeyUnitaDoc,
		cdKeyUnitaDoc, idStrut);
	if (unitaDoc != null) {
	    try {
		AroVRicUnitaDocRowBean row = (AroVRicUnitaDocRowBean) Transform
			.entity2RowBean(unitaDoc);
		table.add(row);
	    } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
		    | IllegalAccessException | IllegalArgumentException
		    | InvocationTargetException ex) {
    ...
    public AttributesTitolario initAttributesTitolario(BigDecimal idTitol, Date dataValidita)
	...
	logger.info(
		"Elimino, se esistono, i livelli presenti su db ma non nella lista creata dall'utente");
	...
	if (!livelliToDelete.isEmpty()) {
		...
		if (!livelloToDelete.getDecVoceTitols().isEmpty()) {
		    throw new ParerUserError(
			    "Errore inaspettato nel salvataggio dei livelli: il livello "
				    + livelloToDelete.getNmLivelloTitol()
				    + " presenta ancora delle voci associate ad esso");
		}
    ...
    public CreaTitolario generateXmlObject(DecTitolRowBean titol, Date dtValidita)
	    throws ParerUserError {
	...
	try {
	    ...
	    logger.error("Errore nell'esportazione delle voci: "
		    + ExceptionUtils.getRootCauseMessage(ex));
    ...
    public BaseTable getDecVoceTitolsTableBean(BigDecimal idCriterioRaggrFasc, String tiSel,
	    boolean isFilterValid) {
	BaseTable tableBean = new BaseTable();
	BaseRow tableRow = null;

	List<DecVoceTitol> listaDecVoceTitol = helper.getDecVoceTitols(idCriterioRaggrFasc, tiSel,
		isFilterValid);
	if (listaDecVoceTitol != null && !listaDecVoceTitol.isEmpty()) {
	    for (Iterator<DecVoceTitol> iterator = listaDecVoceTitol.iterator(); iterator
		    .hasNext();) {
	...
	if (decVoceTitol != null) {
	    rowBean = new BaseRow();
	    rowBean.setBigDecimal("id_titol",
		    new BigDecimal(decVoceTitol.getDecTitol().getIdTitol()));
	    ...
	    rowBean.setTimestamp("dt_istituz",
		    new Timestamp(decVoceTitol.getDtIstituz().getTime()));
	    rowBean.setTimestamp("dt_soppres",
		    new Timestamp(decVoceTitol.getDtSoppres().getTime()));
	...
	if (decValVoceTitols != null && !decValVoceTitols.isEmpty())
	    val = (decValVoceTitols.size() > 1) ? "Diversi"
		    : decValVoceTitols.get(0).getDsVoceTitol();

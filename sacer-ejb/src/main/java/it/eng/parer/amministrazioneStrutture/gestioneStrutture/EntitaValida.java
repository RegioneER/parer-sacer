 ...
 * @author Iacolucci_M
 *
 *         Classe che determina se un'entità deve essere considerata (flag valida==true) e quale
 *         deve essere la risultante data istituzione (dataInizio) e data soppressione (dataFine).
 *         per ogni entità che si intende processare si passano al costruttore i due flag che dicono
 *         come si deve comportare l'oggetto, la data istituzione e soppressione originali del
 *         vecchio oggetto e la data attuale di riferimento. Questa classe determina poi se
 *         l'oggetto è valido e quali devono essere le date giuste da mettere nel nuovo oggetto che
 *         si andrà a creare come importazione/duplicazione del vecchio.
 *
 */
public class EntitaValida {
    ...
    public EntitaValida(boolean includiEntitaDisattiva, boolean mantieniDateFineValiditaOriginale,
            Date dataIstituzione, Date dataSoppressione, Date dataAttuale) {
        ...
        /*
         * Se la data inizio è uguale o antecedente ad adesso e la data fine è uguale o successiva
         * ad adesso l'oggetto è ancora attivo
         */
        if ((dataIstituzione.before(dataAttuale) || dataIstituzione.equals(dataAttuale))
            ...
            if (dataIstituzione.after(dataAttuale) && dataSoppressione.after(dataAttuale)) {
                 ...
                 */
            } else if (dataIstituzione.before(dataAttuale)
                    && dataSoppressione.before(dataAttuale)) {

...
public class SottoStruttureHelper extends GenericHelper {
    ...
    public List<OrgRegolaValSubStrut> getOrgRegolaValSubStrut(BigDecimal idStrut,
            BigDecimal idSubStrut) {
        String queryStr = "SELECT regola FROM OrgCampoValSubStrut campo "
                + "JOIN campo.orgRegolaValSubStrut regola "
                ...
                + "WHERE tipoUnitaDoc.orgStrut.idStrut = :idStrut "
                + "AND campo.orgSubStrut.idSubStrut = :idSubStrut "
    ...
    public List<OrgRegolaValSubStrut> getOrgRegolaSubStrut(BigDecimal id,
            Constants.TipoDato tipoDato, boolean filterValid) {
        ...
        String queryStr = String.format(
                "SELECT regola FROM OrgRegolaValSubStrut regola " + "WHERE %s = :id ", tmpTipoDato);
        if (filterValid) {
    ...
    public boolean existOrgRegolaSubStrut(BigDecimal idRegolaValSubStrut, BigDecimal idTipoUnitaDoc,
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT count(regola.idRegolaValSubStrut) " + "FROM OrgRegolaValSubStrut regola "
                        + "WHERE regola.decTipoUnitaDoc.idTipoUnitaDoc = :idTipoUnitaDoc "
    ...
    public boolean existOrgRegolaSubStrut(BigDecimal idStrut, String nmTipoUnitaDoc,
            String nmTipoDoc) {
        Query query = getEntityManager()
                .createQuery("SELECT COUNT(regola) FROM OrgRegolaValSubStrut regola "
    ...
    public OrgRegolaValSubStrut getOrgRegolaSubStrut(Long idStrut, String nmTipoUnitaDoc,
            String nmTipoDoc, Date dtIstituz, Date dtSoppres) {
        Query query = getEntityManager()
                .createQuery("SELECT regola FROM OrgRegolaValSubStrut regola "
                        + "JOIN regola.decTipoUnitaDoc tipoUd JOIN regola.decTipoDoc tipoDoc "
                        + "WHERE tipoUd.nmTipoUnitaDoc = :nmTipoUnitaDoc "
                        + "AND tipoUd.orgStrut.idStrut = :idStrut "
                        + "AND tipoDoc.nmTipoDoc = :nmTipoDoc "
                        + "AND tipoDoc.orgStrut.idStrut = :idStrut "
    ...
    public boolean existOrgCampoSubStrut(BigDecimal idCampoValSubStrut,
        ...
        StringBuilder queryStr = new StringBuilder("SELECT count(campi.idCampoValSubStrut) "
                + "FROM OrgCampoValSubStrut campi " + "WHERE campi.tiCampo = :tiCampo "
                + "AND campi.nmCampo = :nmCampo "
        ...
        case DATO_SPEC_UNI_DOC:
            queryStr.append(" AND ")
                    .append("campi.decAttribDatiSpec.idAttribDatiSpec = :idAttribDatiSpec");
            break;
        case SUB_STRUT:
    ...
    public List<OrgSubStrut> retrieveOrgSubStrutListAbilitate(long idUtente,
        ...
        String queryStr = "SELECT DISTINCT u FROM OrgSubStrut u, IamAbilTipoDato iatd "
                ...
                + "and iatd.iamAbilOrganiz.idOrganizApplic = :idstrut "
                + "AND u.idSubStrut = iatd.idTipoDatoApplic " + "ORDER BY u.nmSubStrut ";

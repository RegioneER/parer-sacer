...
public class ElenchiVersamentoHelper extends GenericHelper {
    ...
    private String getQueryRicElenchiVersByUd(String unitaDocFilter) {
        return "SELECT /*+ parallel */  " + "       e.id_elenco_vers, " + "       e.nm_elenco, "
                + "       e.ds_elenco, " + "       e.ti_stato_elenco, " + "       e.dt_creazione_elenco, "
                + "       e.dt_chius, " + "       e.dt_firma_indice, " + "       e.ni_comp_vers_elenco, "
                + "       e.ni_comp_agg_elenco, " + "       e.ni_size_vers_elenco, " + "       e.ni_size_agg_elenco, "
                + "       e.nt_indice_elenco, " + "       e.nt_elenco_chiuso, " + "       e.id_criterio_raggr, "
                + "       e.dt_creazione_elenco_ix_aip, " + "       e.dt_firma_elenco_ix_aip, "
                + "       e.fl_elenco_fisc, " + "       e.fl_elenco_standard, " + "       e.ni_indici_aip, "
                + "       e.ti_valid_elenco, " + "       crit.nm_criterio_raggr, "
                + "       daelab.TS_STATO_ELENCO ts_Stato_Elenco_In_Coda_Jms, " + "       e.id_strut    id_strut_ud, "
                + "       case " + " when e.ti_gest_elenco is not null " + "then e.ti_gest_elenco " + " else "
                ...
                + "           WHEN    (    e.ti_stato_elenco IN " + "                            ('FIRMA_IN_CORSO', "
                + "                             'VALIDATO', "
                ...
                + "                OR crit.ti_valid_elenco = 'FIRMA' " + "           THEN " + "               '1' "
                + "           ELSE " + "               '0' " + "       END            fl_elenco_firmato, "
                + "       amb.id_ambiente, " + "       amb.nm_ambiente, " + "       ente.id_ente, "
                + "       ente.nm_ente, " + "       strut.id_strut, " + "       strut.nm_strut "
                ...
                + "           ON daelab.ID_ELENCO_VERS = tab2.id_elenco_Vers " + "       JOIN DEC_CRITERIO_RAGGR crit "
    ...
    private String getTiGestElencoCase() {
        return " CASE " + "            WHEN e.ti_gest_elenco IS NOT NULL THEN " + "                e.ti_gest_elenco "
                + "            ELSE  " + "                CASE  "
                + "                    WHEN crit.ti_gest_elenco_criterio IS NOT NULL THEN  "
                + "                            crit.ti_gest_elenco_criterio  " + "                    ELSE  "
                + "                        CASE  "
    ...
    private String createFiltriUdCondition(Filtri filtri) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdRegistroKeyUnitaDoc())) {
            queryStr.append(whereWord).append("a.cd_Registro_Key_Unita_Doc = :cdRegistroKeyUnitaDoc ");
        }
        ...
        if (filtri.getAaKeyUnitaDocDa() != null && filtri.getAaKeyUnitaDocA() != null) {
            queryStr.append(whereWord).append("a.aa_Key_Unita_Doc BETWEEN :aaKeyUnitaDocDa AND :aaKeyUnitaDocA ");
        }

        if (filtri.getCdKeyUnitaDocDa() != null && filtri.getCdKeyUnitaDocA() != null) {
            queryStr.append(whereWord)
                    .append("LPAD( a.cd_Key_Unita_Doc, 12, '0') BETWEEN :cdKeyUnitaDocDa AND :cdKeyUnitaDocA ");
        }
    ...
    private Query createElvVRicElencoVersByUdNativeQuery(String selectQuery, Filtri filtri) {
        ...
        if ((dtCreazioneElencoDa != null) && (dtCreazioneElencoA != null)) {
            queryStr.append(whereWord)
                    .append("(e.dt_Creazione_Elenco between :dtCreazioneElencoDa AND :dtCreazioneElencoA) ");
            whereWord = " AND ";
        }
        ...
        if ((dtValidazioneElencoDa != null) && (dtValidazioneElencoA != null)) {
            queryStr.append(whereWord)
                    .append("(e.dt_Firma_Indice between :dtValidazioneElencoDa AND :dtValidazioneElencoA) ");
            whereWord = " AND ";
        }
        ...
        if (StringUtils.isNotBlank(filtri.getFlElencoFirmato())) {
            queryStr.append(whereWord).append(getFlElencoFirmatoCase()).append(" = :flElencoFirmato ");
        ...
        if ((dtFirmaElencoIxAipDa != null) && (dtFirmaElencoIxAipA != null)) {
            queryStr.append(whereWord)
                    .append("(e.dt_Firma_Elenco_Ix_Aip between :dtFirmaElencoIxAipDa AND :dtFirmaElencoIxAipA) ");
            whereWord = " AND ";
        }

        /* Inserimento nella query del filtro HH_STATO_ELENCO_IN_CODA_JMS */
        if (filtri.getHhStatoElencoInCodaJms() != null) {
            queryStr.append(whereWord).append("daelab.hh_Stato_Elenco_In_Coda_Jms >= :hhStatoElencoInCodaJms ");
            whereWord = " AND ";
        }

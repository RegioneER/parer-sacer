...
public class MonitoraggioAggMetaEjb {
    ...
    public enum fieldSetToPopulate {
        ...
        AGG_METADATI, AGG_METADATI_ATTESA_SCHED, AGG_METADATI_NON_SELEZ_SCHED, FALLITI_NON_RISOLTI, NON_RISOLUBILI,
        VERIFICATI, NON_VERIFICATI, AGG_METADATI_B30, AGG_METADATI_ATTESA_SCHED_B30, AGG_METADATI_NON_SELEZ_SCHED_B30,
        FALLITI_NON_RISOLTI_B30, NON_RISOLUBILI_B30, VERIFICATI_B30, NON_VERIFICATI_B30
    }

    public enum queryDaUtilizzare {

        MON_CHK_UPD_UD_COR_BY_AMB, MON_CHK_UPD_UD_COR_BY_ENTE, MON_CHK_UPD_UD_COR_BY_STRUT, MON_CNT_UPD_UD_NOCOR_BY_AMB,
        MON_CNT_UPD_UD_COR_BY_AMB, MON_CHK_UPD_UD_KO_COR_BY_AMB, MON_CNT_UPD_UD_KO_NOCOR_BY_AMB,
        MON_CNT_UPD_UD_KO_COR_BY_AMB
    }

    public enum StatoGenerazioneIndiceAip {
        IN_ATTESA_SCHED, NON_SELEZ_SCHED, IN_ELENCO_APERTO, IN_ELENCO_CHIUSO, IN_ELENCO_DA_CHIUDERE, IN_ELENCO_VALIDATO,
        IN_ELENCO_CON_FIRME_VERIFICATE_DT_VERS, IN_ELENCO_IN_CODA_INDICE_AIP, IN_ELENCO_CON_INDICI_AIP_GENERATI,
    ...
    public BaseRow calcolaRiepilogoAggMeta(BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte,
        ...
        if (idStrut != null) {
            ...
            throw new RuntimeException("Errore parametri nella chiamata al metodo calcolaRiepilogoAggMeta() dell'ejb!");
        }
    ...
    public void assignValueFields(BaseRow row, Map<String, Object> risultati) {
        ...
        if (row.getString("fl_upd_ko_norisolub_corr").equals("1") || row.getString("fl_upd_ko_verif_corr").equals("1")
        ...
        if (row.getString("fl_ud_upd_ko_norisolub").equals("1") || row.getString("fl_ud_upd_ko_verif").equals("1")
        ...
        for (Object[] objTotNoCorKoResult : totNoCorKoResult) {
            ...
            if (tiStatoUpdUdKo.equals("NON_RISOLUBILE")) {
                ...
                if (tiDtCreazione.equals("30gg")) {
                    row.setBigDecimal("ni_upd_ko_noverif_30gg", row.getBigDecimal("ni_upd_ko_noverif_30gg").add(niUpd));
                } else if (tiDtCreazione.equals("before30gg")) {
                ...
                if (tiDtCreazione.equals("30gg")) {
                    row.setBigDecimal("ni_upd_ko_verif_30gg", row.getBigDecimal("ni_upd_ko_verif_30gg").add(niUpd));
                } else if (tiDtCreazione.equals("before30gg")) {
                    row.setBigDecimal("ni_upd_ko_verif_b30gg", row.getBigDecimal("ni_upd_ko_verif_b30gg").add(niUpd));
                }
            }
        }

        row.setBigDecimal("ni_upd_ko_totali_30gg", row.getBigDecimal("ni_upd_ko_norisolub_30gg")
                .add(row.getBigDecimal("ni_upd_ko_noverif_30gg")).add(row.getBigDecimal("ni_upd_ko_verif_30gg")));
        row.setBigDecimal("ni_upd_ko_totali_b30gg", row.getBigDecimal("ni_upd_ko_norisolub_b30gg")
    ...
    public void assignValueFieldsDataCorrente(BaseRow row, Map<String, Object> risultati) {
        ...
        for (Object[] objTotCorResult : totCorResult) {
            ...
            if (tiStatoUpdUd.equals("TOTALE")) {
                ...
                row.setBigDecimal("ni_upd_attesa_sched_corr", row.getBigDecimal("ni_upd_attesa_sched_corr").add(niUpd));
            } else if (tiStatoUpdUd.equals("NON_SELEZ_SCHED")) {
                row.setBigDecimal("ni_upd_nosel_sched_corr", row.getBigDecimal("ni_upd_nosel_sched_corr").add(niUpd));
            }
        }
    }
    ...
    public void assignValueFieldsFalliti(BaseRow row, Map<String, Object> risultati) {
        ...
        for (Object[] objTotCorKoResult : totCorKoResult) {
            ...
            if (tiStatoUpdUdKo.equals("NON_RISOLUBILE")) {
                row.setBigDecimal("ni_upd_ko_norisolub_corr", row.getBigDecimal("ni_upd_ko_norisolub_corr").add(niUpd));
            } else if (tiStatoUpdUdKo.equals("NON_VERIFICATO")) {
                row.setBigDecimal("ni_upd_ko_noverif_corr", row.getBigDecimal("ni_upd_ko_noverif_corr").add(niUpd));
            } else if (tiStatoUpdUdKo.equals("VERIFICATO")) {
                row.setBigDecimal("ni_upd_ko_verif_corr", row.getBigDecimal("ni_upd_ko_verif_corr").add(niUpd));
            }
        }
        row.setBigDecimal("ni_upd_ko_totali_corr", row.getBigDecimal("ni_upd_ko_norisolub_corr")
                .add(row.getBigDecimal("ni_upd_ko_noverif_corr")).add(row.getBigDecimal("ni_upd_ko_verif_corr")));
    }
    ...
    public void assignValueFieldsUdUpdFalliti(BaseRow row, Map<String, Object> risultati) {
        ...
        for (Object[] objTotUdUpdKoResult : totUdUpdKoResult) {
            ...
            if (tiStatoUpdUdKo.equals("NON_RISOLUBILE")) {
                row.setBigDecimal("ni_ud_upd_ko_norisolub", row.getBigDecimal("ni_ud_upd_ko_norisolub").add(niUpd));
            } else if (tiStatoUpdUdKo.equals("NON_VERIFICATO")) {
                row.setBigDecimal("ni_ud_upd_ko_noverif", row.getBigDecimal("ni_ud_upd_ko_noverif").add(niUpd));
            } else if (tiStatoUpdUdKo.equals("VERIFICATO")) {
                row.setBigDecimal("ni_ud_upd_ko_verif", row.getBigDecimal("ni_ud_upd_ko_verif").add(niUpd));
            }
        }
        row.setBigDecimal("ni_ud_upd_ko_totali", row.getBigDecimal("ni_ud_upd_ko_norisolub")
                .add(row.getBigDecimal("ni_ud_upd_ko_noverif")).add(row.getBigDecimal("ni_ud_upd_ko_verif")));
    }
    // end MEV#22438

    public BaseTable ricercaAggMetaPerMonitoraggio(BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte,
            BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc, BigDecimal idTipoDoc,
            Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA, String rangeNumeroDa,
            String rangeNumeroA, Set<String> statoIndiceAip, String flSesUpdKoRisolti) {
        BaseTable t = new BaseTable();
        List<MonVLisUpdUdInterface> l = monitoraggioAggMetaHelper.retrieveVLisAggMeta(idUser, idAmbiente, idEnte,
                idStrut, idTipoUnitaDoc, idRegistroUnitaDoc, idTipoDoc, dateValidate, rangeAnnoDa, rangeAnnoA,
                rangeNumeroDa, rangeNumeroA, statoIndiceAip, flSesUpdKoRisolti);
        ...
        for (MonVLisUpdUdInterface objInterface : l) {
            ...
            r.setString("nm_ente_struttura", objInterface.getNmEnte() + " / " + objInterface.getNmStrut());
            t.add(r);
        }
        return t;
    }

    public BaseTable ricercaAggMetaFallitiPerMonitoraggio(BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte,
            BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc, BigDecimal idTipoDoc,
            Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA, String rangeNumeroDa,
            String rangeNumeroA, Set<String> statoSessione, BigDecimal idClasseErr, BigDecimal idErr) {
        return ricercaAggMetaFallitiPerMonitoraggioInternal(idUser, idAmbiente, idEnte, idStrut, idTipoUnitaDoc,
    ...
    public BaseTable ricercaAggMetaFallitiPerMonitoraggioErr(BigDecimal idUser, BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
            BigDecimal idTipoDoc, Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA,
            String rangeNumeroDa, String rangeNumeroA, Set<String> statoSessione, BigDecimal idClasseErr,
            BigDecimal idErr) {
        return ricercaAggMetaFallitiPerMonitoraggioInternal(idUser, idAmbiente, idEnte, idStrut, idTipoUnitaDoc,
                idRegistroUnitaDoc, idTipoDoc, dateValidate, rangeAnnoDa, rangeAnnoA, rangeNumeroDa, rangeNumeroA,
                statoSessione, idClasseErr, idErr);
    ...
    private BaseTable ricercaAggMetaFallitiPerMonitoraggioInternal(BigDecimal idUser, BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
            BigDecimal idTipoDoc, Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA,
            String rangeNumeroDa, String rangeNumeroA, Set<String> statoSessione, BigDecimal idClasseErr,
            BigDecimal idErr) {
        BaseTable t = new BaseTable();
        List<MonVLisUpdUdKoInterface> l = monitoraggioAggMetaHelper.retrieveVLisAggMetaFalliti(idUser, idAmbiente,
                idEnte, idStrut, idTipoUnitaDoc, idRegistroUnitaDoc, idTipoDoc, dateValidate, rangeAnnoDa, rangeAnnoA,
                rangeNumeroDa, rangeNumeroA, statoSessione, idClasseErr, idErr);

        for (MonVLisUpdUdKoInterface objInterface : l) {
            BaseRow r = new BaseRow();
            r.setBigDecimal("id_ses_upd_unita_doc_ko", objInterface.getIdSesUpdUnitaDocKo());
            r.setString("ds_ente_strut", objInterface.getNmEnte() + " / " + objInterface.getNmStrut());
            r.setString("ds_unita_doc", objInterface.getDsUnitaDoc());
            r.setString("nm_struttura", objInterface.getNmStrut());
            r.setString("ds_unita_doc", objInterface.getDsUnitaDoc());
            r.setString("nm_tipo_unita_doc", objInterface.getNmTipoUnitaDoc());
            r.setString("nm_tipo_doc_princ", objInterface.getNmTipoDocPrinc());
            r.setTimestamp("ts_ini_ses", new Timestamp(objInterface.getTsIniSes().getTime()));
            r.setString("cd_ds_err_princ", objInterface.getCdErrPrinc() + " / " + objInterface.getDsErrPrinc());
    ...
    // MEV#22438
    public BaseTable ricercaUnitaDocAggMetaFallitiPerMonitoraggio(BigDecimal idUser, BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
            BigDecimal idTipoDoc, Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA,
            String rangeNumeroDa, String rangeNumeroA, Set<String> statoSessione, BigDecimal idClasseErr,
            BigDecimal idErr) {
        return ricercaUnitaDocAggMetaFallitiPerMonitoraggioInternal(idUser, idAmbiente, idEnte, idStrut, idTipoUnitaDoc,
                idRegistroUnitaDoc, idTipoDoc, dateValidate, rangeAnnoDa, rangeAnnoA, rangeNumeroDa, rangeNumeroA,
                statoSessione, idClasseErr, idErr);
    }

    private BaseTable ricercaUnitaDocAggMetaFallitiPerMonitoraggioInternal(BigDecimal idUser, BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
            BigDecimal idTipoDoc, Date[] dateValidate, BigDecimal rangeAnnoDa, BigDecimal rangeAnnoA,
            String rangeNumeroDa, String rangeNumeroA, Set<String> statoSessione, BigDecimal idClasseErr,
            BigDecimal idErr) {
        BaseTable t = new BaseTable();
        List<MonVLisUdUpdKoInterface> l = monitoraggioAggMetaHelper.retrieveVLisUnitaDocAggMetaFalliti(idUser,
                idAmbiente, idEnte, idStrut, idTipoUnitaDoc, idRegistroUnitaDoc, idTipoDoc, dateValidate, rangeAnnoDa,
    ...
    public MonVLisUpdUdErrTableBean ricercaAggMetaErrati(Date[] dateValidate, Set<String> tiStatoIndice,
            BigDecimal idClasseErr, BigDecimal idErr) {
        MonVLisUpdUdErrTableBean t = new MonVLisUpdUdErrTableBean();
        List<MonVLisUpdUdErr> l = monitoraggioAggMetaHelper.retrieveVLisAggMetaErrati(dateValidate, tiStatoIndice,
                idClasseErr, idErr);
    ...
    public DecodeMap getIdClasseErrSacerByTipiUsoDecodeMap(List<String> tipiUso) {
        ...
        BaseRow riga = null;
        List<DecClasseErrSacer> l = monitoraggioAggMetaHelper.retrieveClasseErrSacerByTipiUso(tipiUso);
        for (DecClasseErrSacer ogg : l) {
            riga = new BaseRow();
            riga.setBigDecimal("id_classe_err_sacer", BigDecimal.valueOf(ogg.getIdClasseErrSacer()));
    ...
    public MonVVisUpdUdKoRowBean getMonVVisUpdUdKoRowBean(BigDecimal idSesUpdUnitaDocKo) {
        ...
        MonVVisUpdUdKo mon = monitoraggioAggMetaHelper.findViewById(MonVVisUpdUdKo.class, idSesUpdUnitaDocKo);
        try {
    ...
    public MonVVisUdUpdKoRowBean getMonVVisUdUpdKoRowBean(BigDecimal idUpdUnitaDocKo) {
        ...
        MonVVisUdUpdKo mon = monitoraggioAggMetaHelper.findViewById(MonVVisUdUpdKo.class, idUpdUnitaDocKo);
        try {
    ...
    public MonVVisUpdUdErrRowBean getMonVVisUpdUdErrRowBean(BigDecimal idSesUpdUnitaDocErr) {
        ...
        MonVVisUpdUdErr mon = monitoraggioAggMetaHelper.findViewById(MonVVisUpdUdErr.class, idSesUpdUnitaDocErr);
        try {
    ...
    public AroVVisUpdUnitaDocRowBean getAroVVisUpdUnitaDocRowBean(BigDecimal idUpdUnitaDoc) {
        ...
        AroVVisUpdUnitaDoc aro = monitoraggioAggMetaHelper.findViewById(AroVVisUpdUnitaDoc.class, idUpdUnitaDoc);
        try {
    ...
    public AroVLisUpdDocUnitaDocTableBean getAroVLisUpdDocUnitaDocTableBean(BigDecimal idUpdUnitaDoc) {
        AroVLisUpdDocUnitaDocTableBean tb = new AroVLisUpdDocUnitaDocTableBean();
        List<AroVLisUpdDocUnitaDoc> list = monitoraggioAggMetaHelper.getAroVLisUpdDocUnitaDocList(idUpdUnitaDoc);
        try {
    ...
    public AroVLisUpdCompUnitaDocTableBean getAroVLisUpdCompUnitaDocTableBean(BigDecimal idUpdUnitaDoc) {
        AroVLisUpdCompUnitaDocTableBean tb = new AroVLisUpdCompUnitaDocTableBean();
        List<AroVLisUpdCompUnitaDoc> list = monitoraggioAggMetaHelper.getAroVLisUpdCompUnitaDocList(idUpdUnitaDoc);
        try {
            if (list != null && !list.isEmpty()) {
    ...
    public AroVLisUpdKoRisoltiTableBean getAroVLisUpdKoRisoltiTableBean(BigDecimal idUpdUnitaDoc) {
        ...
        List<AroVLisUpdKoRisolti> list = monitoraggioAggMetaHelper.getAroVLisUpdKoRisoltiList(idUpdUnitaDoc);
        try {
            if (list != null && !list.isEmpty()) {
    ...
    public AroWarnUpdUnitaDocTableBean getAroWarnUpdUnitaDocTableBean(BigDecimal idUpdUnitaDoc) {
        ...
        List<AroWarnUpdUnitaDoc> list = monitoraggioAggMetaHelper.getAroWarnUpdUnitaDocList(idUpdUnitaDoc);
        try {
            if (list != null && !list.isEmpty()) {
    ...
    public VrsErrSesUpdUnitaDocKoTableBean ricercaVersamentiErrSesUpdUnitaDocKo(BigDecimal idSesUpdUnitaDocKo) {
    ...
    public VrsErrSesUpdUnitaDocErrTableBean ricercaVersamentiErrSesUpdUnitaDocErr(BigDecimal idSesUpdUnitaDocErr) {
        ...
        try {
            for (VrsErrSesUpdUnitaDocErr rec : list) {
                ...
                | IllegalArgumentException | InvocationTargetException ex) {
            logger.error("Errore durante il recupero dei dati della tabella VrsErrSesUpdUnitaDocErr "
                    + ExceptionUtils.getRootCauseMessage(ex), ex);
        }
        return tb;
    }

    public void salvaStatoAggiornamentoFallito(BigDecimal idSesUpdUnitaDocKo, String tiStatoPreModifica,
            String statoArrivo) throws ParerInternalError {
        // Recupero la sessione di errore aggiornamento metadati
        VrsSesUpdUnitaDocKo sesUpdUnitaDocKo = monitoraggioAggMetaHelper.findByIdWithLock(VrsSesUpdUnitaDocKo.class,
                idSesUpdUnitaDocKo);
        // Il sistema assume lock esclusivo sull’aggiornamento fallito
        VrsUpdUnitaDocKo updUnitaDocKo = monitoraggioAggMetaHelper.findByIdWithLock(VrsUpdUnitaDocKo.class,
        ...
        if (!tiStatoPreModifica.equals(sesUpdUnitaDocKo.getTiStatoSesUpdKo().name())) {
            throw new ParerInternalError(
                    "L’aggiornamento fallito " + idSesUpdUnitaDocKo + " non ha stato = " + tiStatoPreModifica);
        }

        // Il sistema verifica se per la chiave di totalizzazione degli aggiornamenti falliti
        // (data di inizio aggiornamento fallito + l’identificatore della struttura + lo stato di partenza della
        // modifica
        // + l’anno dell’unità doc + gli identificatori del registro, del tipo unità doc e del tipo doc principale,
        ...
        MonContaSesUpdUdKo contaSes = monitoraggioAggMetaHelper.retrieveMonContaSesUpdUdKo(
                ...
                sesUpdUnitaDocKo.getAaKeyUnitaDoc(), sesUpdUnitaDocKo.getDecRegistroUnitaDoc().getIdRegistroUnitaDoc(),
                sesUpdUnitaDocKo.getDecTipoUnitaDoc().getIdTipoUnitaDoc(),
                sesUpdUnitaDocKo.getDecTipoDocPrinc().getIdTipoDoc(), sesUpdUnitaDocKo.getTiStatoSesUpdKo().name());
        if (contaSes != null) {
            // Assume lock esclusivo
            MonContaSesUpdUdKo contaSesLockato = monitoraggioAggMetaHelper.findByIdWithLock(MonContaSesUpdUdKo.class,
                    contaSes.getIdContaSesUpdUdKo());
            // Aggiorna il numero di aggiornamenti falliti sottraendo 1
            contaSesLockato.setNiSesUpdUdKo(contaSesLockato.getNiSesUpdUdKo().subtract(BigDecimal.ONE));
        }

        // Aggiorna lo stato
        sesUpdUnitaDocKo.setTiStatoSesUpdKo(
                it.eng.parer.entity.constraint.VrsSesUpdUnitaDocKo.TiStatoSesUpdKo.valueOf(statoArrivo));

        // Il sistema aggiorna unità doc derivante da aggiornamenti falliti,
        // a cui e’ relativo l’aggiornamento fallito, assegnando lo stato dell’unità doc mediante i valori restituito
        // dalla vista
        // VRS_V_MODIF_UPD_UD_KO a cui si accede mediante l’identificatore della struttura e la chiave dell’unità doc
        VrsVModifUpdUdKo modifUpd = monitoraggioAggMetaHelper.findViewById(VrsVModifUpdUdKo.class,
                updUnitaDocKo.getIdUpdUnitaDocKo());
        updUnitaDocKo.setVrsSesUpdUnitaDocKoFirst(
                monitoraggioAggMetaHelper.findById(VrsSesUpdUnitaDocKo.class, modifUpd.getIdSesUpdUdKoFirst()));
        updUnitaDocKo.setVrsSesUpdUnitaDocKoLast(
                monitoraggioAggMetaHelper.findById(VrsSesUpdUnitaDocKo.class, modifUpd.getIdSesUpdUdKoLast()));
        updUnitaDocKo.setDecTipoUnitaDocLast(
                monitoraggioAggMetaHelper.findById(DecTipoUnitaDoc.class, modifUpd.getIdTipoUnitaDocLast()));
        if (modifUpd.getIdErrSacerPrinc() != null) {
            updUnitaDocKo.setDecErrSacerPrinc(
                    monitoraggioAggMetaHelper.findById(DecErrSacer.class, modifUpd.getIdErrSacerPrinc()));
        }
        if (modifUpd.getIdControlloWsPrinc() != null) {
            updUnitaDocKo.setDecControlloWsPrinc(
                    monitoraggioAggMetaHelper.findById(DecControlloWs.class, modifUpd.getIdControlloWsPrinc()));
        }
        updUnitaDocKo.setDecTipoDocPrincLast(
                monitoraggioAggMetaHelper.findById(DecTipoDoc.class, modifUpd.getIdTipoDocPrincLast()));
        updUnitaDocKo.setDecRegistroUnitaDocLast(
                monitoraggioAggMetaHelper.findById(DecRegistroUnitaDoc.class, modifUpd.getIdRegistroUnitaDocLast()));
        ...
        updUnitaDocKo.setTiStatoUdpUdKo(
                it.eng.parer.entity.constraint.VrsUpdUnitaDocKo.TiStatoUdpUdKo.valueOf(modifUpd.getTiStatoUpdUdKo()));
        ...
        if (sesUpdUnitaDocKo.getTsIniSes().before(visLastSched.getDtRegLogJobIni())) {
            // Il sistema verifica se per la chiave di totalizzazione degli aggiornamenti falliti
            // (data di inizio aggiornamento fallito + l’identificatore della struttura + lo stato di arrivo della
            // modifica
            // + l’anno dell’unità doc + gli identificatori del registro, del tipo unità doc e del tipo doc principale,
            // dell’aggiornamento fallito),
            // esiste un totale aggiornamenti in MON_CONTA_SES_UPD_UD_KO, assumendo lock esclusivo
            MonContaSesUpdUdKo contaSesArrivo = monitoraggioAggMetaHelper.retrieveMonContaSesUpdUdKo(
                    sesUpdUnitaDocKo.getTsIniSes(), sesUpdUnitaDocKo.getOrgStrut().getIdStrut(),
            ...
            if (contaSesArrivo != null) {
                ...
                MonContaSesUpdUdKo contaSesLockato = monitoraggioAggMetaHelper
                        .findByIdWithLock(MonContaSesUpdUdKo.class, contaSesArrivo.getIdContaSesUpdUdKo());
                // Aggiorna il numero di aggiornamenti falliti sottraendo 1
                contaSesLockato.setNiSesUpdUdKo(contaSesLockato.getNiSesUpdUdKo().add(BigDecimal.ONE));
            } else {

...
public class VolumiHelper {
    ...
    private VolVRicVolumeTableBean getVolVRicVolumeTableBeanFrom(List<VolVRicVolume> listaVolumi) {
        ...
        try {
            if (listaVolumi != null && !listaVolumi.isEmpty()) {
                volumiTableBean = (VolVRicVolumeTableBean) Transform.entities2TableBean(listaVolumi);
            }
        } catch (Exception e) {
    ...
    public void saveNomeDesNote(Long idUser, BigDecimal idVolume, String nomeVol, String descrVol, String noteVol,
        ...
        try {
            ...
            entityManager.flush();
            writeLogOper(record, record.getOrgStrut(), idUser, VolumeEnums.OpTypeEnum.MODIFICA_VOLUME.name());

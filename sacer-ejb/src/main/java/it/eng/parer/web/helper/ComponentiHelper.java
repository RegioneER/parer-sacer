...
/**
 * Session Bean implementation class ComponentiHelper Contiene i metodi (implementati di
 * ComponentiHelperLocal), per la gestione della persistenza su DB per le operazioni CRUD su oggetti
 * di ComponentiTableBean e ComponentiRowBean
 */
@SuppressWarnings({
	"unchecked", "rawtypes" })
@Stateless
...
public class ComponentiHelper extends GenericHelper {
    ...
    public AroVRicCompTableBean getAroVRicCompViewBean(BigDecimal idStrut,
	    RicComponentiFiltri filtri, Date[] dateAcquisizioneValidate,
	...
	return getAroVRicCompViewBeanSimpleTypeParameters(idStrut, dateAcquisizioneValidate,
		maxResults, filtri.getCd_registro_key_unita_doc().parse(),
		...
		filtri.getDs_id_comp_vers().parse(), filtri.getNm_tipo_strut_doc().parse(),
		...
		filtri.getTi_esito_verif_firme().parse(), filtri.getDt_scad_firma_comp_da().parse(),
		filtri.getDt_scad_firma_comp_a().parse(), filtri.getFl_rif_temp_vers().parse(),
		filtri.getDs_rif_temp_vers().parse(), filtri.getDs_nome_comp_vers().parse(),
		filtri.getFl_hash_vers().parse(), filtri.getDs_hash_file_vers().parse(),
		filtri.getNm_mimetype_file().parse(), filtri.getDl_urn_comp_vers().parse(),
		filtri.getDs_formato_rappr_calc().parse(),
		filtri.getDs_formato_rappr_esteso_calc().parse(),
		filtri.getFl_forza_accettazione().parse(),
		filtri.getFl_forza_conservazione().parse(),
		filtri.getTi_esito_contr_formato_file().parse(),
		filtri.getDs_hash_file_calc().parse(), filtri.getDs_algo_hash_file_calc().parse(),
		filtri.getCd_encoding_hash_file_calc().parse(),
		filtri.getDs_urn_comp_calc().parse(), filtri.getTi_supporto_comp().parse(),
		filtri.getNm_tipo_rappr_comp().parse(), filtri.getNm_sub_strut().parse(),
		filtri.getCd_registro_key_unita_doc().getDecodeMap().keySet(), idTipoUnitaDocSet,
		idTipoDocSet);
    }
    ...
    public AroVRicCompTableBean getAroVRicCompViewBeanSimpleTypeParameters(BigDecimal idStrut,
	    Date[] dateAcquisizioneValidate, int maxResults, final String registro,
	    final BigDecimal anno, final String codice, final BigDecimal annoRangeDa,
	    final BigDecimal annoRangeA, String codiceRangeDa, String codiceRangeA,
	    final String idComp, final BigDecimal tipoStrutDoc, final BigDecimal tipoCompDoc,
	    final String formato, BigDecimal fileSizeDa, final BigDecimal fileSizeA,
	    final String presenza, final String conformita, final String esitoFirme,
	    final Timestamp dtScadFirmaCompDa, final Timestamp dtScadFirmaCompA,
	    final String flRifTempVers, final String dsRifTempVers, final String dsNomeCompVers,
	    final String flHashVers, final String dsHashFileVers, final String nmMimetypeFile,
	    final String dlUrnCompVers, final String dsFormatoRapprCalc,
	    final String dsFormatoRapprEstesoCalc, final String forzaAcc,
	    final String forzaConserva, final String tiEsitoContrFormatoFile,
	    final String dsHashFileCalc, final String dsAlgoHashFileCalc,
	    final String cdEncodingHashFileCalc, final String dsUrnCompCalc,
	    final String tiSupportoComp, final String nmTipoRapprComp,
	    final List<BigDecimal> subStruts, final Set<Object> keySet,
	...
	if (fileSizeDa != null && fileSizeA != null) {
	    queryStr.append(whereWord)
		    .append("u.niSizeFileCalc between :filesizedain AND :filesizeain ");
	    whereWord = "and ";
	}
	...
	if ((dataValDa != null) && (dataValA != null)) {
	    queryStr.append(whereWord)
		    .append("(u.dtScadCertifFirmatario between :datavalda AND :datavala) ");
	    whereWord = "and ";
	}
	...
	if (StringUtils.isNotBlank(dsFormatoRapprEstesoCalc)) {
	    queryStr.append(whereWord)
		    .append("u.dsFormatoRapprEstesoCalc = :formatoRapprEstesoCalcIn ");
	    whereWord = " AND ";
	}
	...
	if (StringUtils.isNotBlank(tiEsitoContrFormatoFile)) {
	    queryStr.append(whereWord)
		    .append("u.tiEsitoContrFormatoFile = :esitoContrFormatoFileIn ");
	    whereWord = " AND ";
	}
	...
	if (StringUtils.isNotBlank(cdEncodingHashFileCalc)) {
	    queryStr.append(whereWord)
		    .append("u.cdEncodingHashFileCalc = :encodingHashFileCalcIn ");
	    whereWord = " AND ";
	}
	...
	return lazyListHelper.getTableBean(query,
		list -> getAroVRicCompTableBeanFromResultList(list), "u.id.idCompDoc");

    }

    private AroVRicCompTableBean getAroVRicCompTableBeanFromResultList(
	    List<AroVRicComp> listaComponenti) {
	AroVRicCompTableBean componentiTableBean = new AroVRicCompTableBean();
	try {
	    if (listaComponenti != null && !listaComponenti.isEmpty()) {
		componentiTableBean = (AroVRicCompTableBean) Transform
			.entities2TableBean(listaComponenti);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public it.eng.parer.slite.gen.viewbean.VolVListaCompVolTableBean getVolVListaCompVolViewBean(
	    BigDecimal idVolume, ComponentiFiltri filtri, Date[] dateValidate,
	    DecTipoUnitaDocTableBean tmpTableBeanTipoUD, DecTipoDocTableBean tmpTableBeanTipoDoc,
	    OrgSubStrutTableBean tmpSubStrutsTableBean, int maxResults)
	...
	return getVolVListaCompVolViewBeanSimpleTypeParameters(idVolume, dateValidate, maxResults,
		...
		filtri.getNi_size_file_da().parse(), filtri.getNi_size_file_a().parse(),
		filtri.getFl_comp_firmato().parse(), filtri.getTi_esito_contr_conforme().parse(),
		filtri.getTi_esito_verif_firme_vers().parse(),
		filtri.getTi_esito_verif_firme_chius().parse(),
		...
		filtri.getDs_hash_file_calc().parse(), filtri.getDs_algo_hash_file_calc().parse(),
		filtri.getCd_encoding_hash_file_calc().parse(),
		filtri.getDs_urn_comp_calc().parse(), idTipoUnitaDocSet, idTipoDocSet,
		idSubStrutSet);
    }

    // Metodo che restituisce un viewbean con i record dei componenti trovati in base
    // ai filtri di ricerca passati in ingresso e all'id del volume
    VolVListaCompVolTableBean getVolVListaCompVolViewBeanSimpleTypeParameters(BigDecimal idVolume,
	    Date[] dateValidate, int maxResults, final String registro, final BigDecimal anno,
	    final String codice, final BigDecimal annoRangeDa, final BigDecimal annoRangeA,
	    String codiceRangeDa, String codiceRangeA, final BigDecimal tipoStrutDoc,
	    final BigDecimal tipoCompDoc, final String formato, BigDecimal fileSizeDa,
	    final BigDecimal fileSizeA, final String presenza, final String conformita,
	    final String esitoFirme, final String esitoFirmeChius,
	    final Timestamp dtScadFirmaCompDa, final Timestamp dtScadFirmaCompA,
	    final String dsNomeCompVers, final String dsHashFileVers, final String nmMimetypeFile,
	    final String dlUrnCompVers, final String dsFormatoRapprCalc,
	    final String dsFormatoRapprEstesoCalc, final String flForzaAccettazione,
	    final String flForzaConservazione, final String tiEsitoContrFormatoFile,
	    final String dsHashFileCalc, final String dsAlgoHashFileCalc,
	    final String cdEncodingHashFileCalc, final String dsUrnCompCalc,
	    Set<BigDecimal> idTipoUnitaDocSet, Set<BigDecimal> idTipoDocSet,
	...
	query.setMaxResults(maxResults);
	return lazyListHelper.getTableBean(query,
		list -> getVolVListaCompVolTableBeanFromResultList(list), "id.idAppartCompVolume");
    }
	...
	try {
	    if (listaComponenti != null && !listaComponenti.isEmpty()) {
		componentiTableBean = (VolVListaCompVolTableBean) Transform
			.entities2TableBean(listaComponenti);
	    }
	...
	if (certifCaNameList != null) {
		...
		FirCertifCa fircertif = getEntityManager().find(FirCertifCa.class,
			idCertif.longValue());
	...
	String SELECT_LISTA_CRL = "select distinct " + "crl.id_crl nomeFileCRL "
		...
		+ "join ARO_FIRMA_COMP firma " + "on (firma.id_comp_doc = app_comp.id_comp_doc) "
	...
	String SELECT_LISTA_CERTIFICATI_CA = "select distinct "
		+ "cert.id_certif_ca nomeFileCertificatoCA " + "from VOL_VOLUME_CONSERV vol "
    ...
    public AroVLisFirmaCompTableBean getAroVLisFirmaCompTableBean(BigDecimal idcompdoc,
	    int maxResults) {
	...
	try {
		...
		for (AroVLisFirmaComp row : listaFirmaComp) {
		    AroVLisFirmaCompRowBean rowBean = (AroVLisFirmaCompRowBean) Transform
			    .entity2RowBean(row);
    ...
    public AroVLisMarcaCompTableBean getAroVLisMarcaCompTableBean(BigDecimal idcompdoc,
	    int maxResults) {
	...
	try {
		...
		listaMarcaCompTableBean = (AroVLisMarcaCompTableBean) Transform
			.entities2TableBean(listaMarcaComp);
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
	...
	try {
		...
		compMarcaRowBean = (AroVVisMarcaCompRowBean) Transform
			.entity2RowBean(compMarca.get(0));
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public AroVLisCertifCaMarcaCompTableBean getAroVLisCertifCaMarcaCompTableBean(
	    BigDecimal idmarca, String tiContr) {
	...
	try {
		...
		compFirmaRowBean = (AroVVisFirmaCompRowBean) Transform
			.entity2RowBean(compFirma.get(0));
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
    ...
    public AroVLisCertifCaFirmaCompTableBean getAroVLisCertifCaFirmaCompTableBean(
	    BigDecimal idfirma, String tiContr) {
    ...
    public AroVLisControfirmaFirmaTableBean getAroVLisControfirmaFirmaTableBean(
	    BigDecimal idfirma) {
    ...
    public it.eng.parer.slite.gen.viewbean.ElvVListaCompElvTableBean getElvVListaCompElvViewBean(
	    BigDecimal idElencoVers, BigDecimal idStrut) throws EMFError {

	// Ricavo l'informazione se la struttura ha più di una sottostruttura
	String queryStrSubStrut = "SELECT u FROM OrgSubStrut u where u.orgStrut.idStrut = :idStrut ";

	// CREO LA QUERY ATTRAVERSO L'ENTITY MANAGER
	Query querySubStrut = getEntityManager().createQuery(queryStrSubStrut);
	querySubStrut.setParameter("idStrut", idStrut.longValue());

	Long idSubStrut = null;

	List<OrgSubStrut> subStrutList = querySubStrut.getResultList();
	// Se una struttura ha una sola sottostruttura
	if (subStrutList.size() == 1) {
	    idSubStrut = subStrutList.get(0).getIdSubStrut();
	}
	// Se ne ha più di una, prendo quella associata alle ud
	else {
	    // Ricavo l'informazione se la struttura ha più di una sottostruttura
		...
		idSubStrut = risultati.get(0); // Restituisce il primo (e unico) elemento
	    }
	}
	return getElvVListaCompElvViewBean(idElencoVers, idSubStrut);
    }

    public ElvVListaCompElvTableBean getElvVListaCompElvViewBean(BigDecimal idElencoVers,
	    Long idSubStrut) {
	String queryStr = "SELECT u.* FROM Elv_Vp_Lista_Comp_Elv(:idElencoVers, :idSubStrut) u ";
	...
	for (Object[] componenteObj : listaComponentiObj) {
	    ...
	    componentiRowBean.setIdElencoVers(
		    componenteObj[0] != null ? (BigDecimal) componenteObj[0] : null);
	    componentiRowBean
		    .setIdCompDoc(componenteObj[1] != null ? (BigDecimal) componenteObj[1] : null);
	    componentiRowBean.setDsUrnCompCalcShort(
		    componenteObj[2] != null ? (String) componenteObj[2] : null);
	    componentiRowBean
		    .setDsNomeCompVers(componenteObj[3] != null ? (String) componenteObj[3] : null);
	    componentiRowBean
		    .setDsOrdComp(componenteObj[4] != null ? (String) componenteObj[4] : null);
	    componentiRowBean
		    .setTiSupportoComp(componenteObj[5] != null ? (String) componenteObj[5] : null);
	    componentiRowBean
		    .setNmTipoCompDoc(componenteObj[6] != null ? (String) componenteObj[6] : null);
	    componentiRowBean.setNmFormatoFileDocVers(
		    componenteObj[7] != null ? (String) componenteObj[7] : null);
	    componentiRowBean.setDtCreazioneDoc(
		    componenteObj[8] != null ? (Timestamp) componenteObj[8] : null);
	    componentiRowBean.setNiSizeFileCalc(
		    componenteObj[9] != null ? (BigDecimal) componenteObj[9] : null);
	    // Caso speciale per Character -> String
	    componentiRowBean.setFlCompFirmato(
		    componenteObj[10] != null ? ((Character) componenteObj[10]).toString() : null);
	    componentiRowBean.setTiEsitoVerifFirmeVers(
		    componenteObj[11] != null ? (String) componenteObj[11] : null);
	    componentiRowBean.setIdUnitaDoc(
	    ...
	    componentiRowBean.setIdTipoDoc(
		    componenteObj[15] != null ? (BigDecimal) componenteObj[15] : null);
	    componentiRowBean.setIdSubStrut(
		    componenteObj[16] != null ? (BigDecimal) componenteObj[16] : null);
	    componentiRowBean.setTiStatoElencoVers(
	...
	String queryStr = "SELECT COUNT(u) FROM ElvVListaCompElv u "
		+ "WHERE u.id.idCompDoc = :idCompDoc";
	Query query = getEntityManager().createQuery(queryStr);
	...
	String queryStr = "SELECT COUNT(u) FROM VolVListaCompVol u "
		+ "WHERE u.idCompDoc = :idCompDoc";
	Query query = getEntityManager().createQuery(queryStr);
	query.setParameter("idCompDoc", idCompDoc);
	Long conta = (Long) query.getSingleResult();
	return conta > 0;
	...
	try {
		...
		compVolRowBean = (AroVVisCompVolRowBean) Transform
			.entity2RowBean(compVolList.get(0));
	    }
	} catch (Exception e) {
	    log.error(e.getMessage(), e);
	}
	...
	ids[0] = BigDecimal
		.valueOf(compDoc.getAroStrutDoc().getAroDoc().getAroUnitaDoc().getIdUnitaDoc());

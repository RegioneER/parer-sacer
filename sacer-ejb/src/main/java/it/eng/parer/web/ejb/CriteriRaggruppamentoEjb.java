...
public class CriteriRaggruppamentoEjb {
    ...
    public boolean isCriterioRaggrStandard(CriterioRaggrStandardBean bean) throws EMFError {
        ...
        if ((bean.getReg().size() == 1 && bean.getTipiUd().isEmpty() && bean.getTipiDoc().isEmpty())
                || (bean.getReg().isEmpty() && bean.getTipiUd().size() == 1
                        && bean.getTipiDoc().isEmpty())
                || (bean.getReg().isEmpty() && bean.getTipiUd().isEmpty()
                        && bean.getTipiDoc().size() == 1)) {
            numeroCondizioniSoddisfatte++;

            // Finchè sono qui controllo la sesta condizione:
            // 6) Controllo che il nome criterio sia pari a: nome tipo ud (o registro o tipo doc a
            // seconda del caso)
            String nomeNecessario = (bean.getReg().size() == 1
                    ? bean.getReg().toArray(new String[1])[0]
                    : bean.getTipiUd().size() == 1 ? bean.getTipiUd().toArray(new String[1])[0]
                            : bean.getTipiDoc().size() == 1
                                    ? bean.getTipiDoc().toArray(new String[1])[0]
                                    : "");
        ...
        if (bean.getAaKeyUnitaDoc() == null && bean.getAaKeyUnitaDocDa() == null
                && bean.getAaKeyUnitaDocA() == null) {
            numeroCondizioniSoddisfatte++;
        }

        // // 3) Controllo che se il flag automatico è true, l'anno sia impostato con l'anno
        // corrente - Se è false,
        // l'anno deve essere diverso dall'anno corrente
        // if (bean.getAaKeyUnitaDoc() != null) {
        // if ((bean.getFlCriterioRaggrAutom().equals("1")
        // && bean.getAaKeyUnitaDoc().compareTo(new
        // BigDecimal(GregorianCalendar.getInstance().get(Calendar.YEAR))) ==
        // 0)
        ...
        if (bean.getNiTempoScadChius() != null
                && bean.getNiTempoScadChius().compareTo(new BigDecimal("30")) == 0
                && bean.getTiTempoScadChius() != null
                && bean.getTiTempoScadChius().equals("GIORNI")) {
            numeroCondizioniSoddisfatte++;
        }
        ...
        if (bean.getNiMaxComp() != null && bean.getNiMaxComp()
                .compareTo(new BigDecimal(numMaxCompCriterioRaggrWarn)) == 0) {
            numeroCondizioniSoddisfatte++;
        }
        /*
         * 6) Controllo che tutti i campi da oggetto unità documentaria in giù nella form siano
         * blank DL_OGGETTO_UNITA_DOC, DT_REG_UNITA_DOC_DA, DT_REG_UNITA_DOC_A, DL_DOC,
         * DS_AUTORE_DOC, DT_CREAZIONE_UNITA_DOC_DA, ORE_DT_CREAZIONE_UNITA_DOC_DA,
         * MINUTI_DT_CREAZIONE_UNITA_DOC_DA, DT_CREAZIONE_UNITA_DOC_A, ORE_DT_CREAZIONE_UNITA_DOC_A,
         * MINUTI_DT_CREAZIONE_UNITA_DOC_A, TI_CONSERVAZIONE, FL_UNITA_DOC_FIRMATO,
         * FL_FORZA_ACCETTAZIONE, FL_FORZA_CONSERVAZIONE, TI_ESITO_VERIF_FIRME,
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrByIdStrut(BigDecimal idStrut,
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrDaRegistro(BigDecimal idStrut,
        ...
        try {
            ...
            logger.error(
                    "Errore durante il recupero dei criteri di raggruppamento del registro avente id "
                            + idRegistroUnitaDoc + " - " + ExceptionUtils.getRootCauseMessage(ex),
                    ex);
        }
        return tableBean;
    }

    public DecVRicCriterioRaggrTableBean getCriteriRaggrDaTipoUnitaDoc(BigDecimal idStrut,
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrDaTipoDoc(BigDecimal idStrut,
        ...
        try {
            ...
            logger.error(
                    "Errore durante il recupero dei criteri di raggruppamento del tipo documento avente id "
                            + idTipoDoc + " - " + ExceptionUtils.getRootCauseMessage(ex),
                    ex);
        }
        return tableBean;
    }
    ...
    // public Map<String, List<String>> creaCriteriRaggruppamentoDaPeriodoDiValidita(LogParam param,
    // BigDecimal idStrut,
    // BigDecimal idRegistroUnitaDoc, int aaMinRegistroUnitaDoc, int aaMaxRegistroUnitaDoc) throws
    // ParerUserError {
    ...
    // String exists = getCriterioStandardPerTipoDatoAnno(idRegistroUnitaDoc.longValue(),
    // "REGISTRO_UD", i);
    ...
    // String nmCriterioMod = creaCritRegistro.getNmCriterioRaggr().replace("<anno da definire>", ""
    // + i);
    ...
    public DecCriterioRaggr salvataggioAutomaticoCriterioRaggrStdNoAutomRegistro(LogParam param,
        ...
        criterioDaSalvare
                .setFlFiltroRangeRegistroKey(creaCritRaggrRegistro.getFlFiltroRangeRegistroKey());
        criterioDaSalvare.setFlFiltroRegistroKey(creaCritRaggrRegistro.getFlFiltroRegistroKey());
        criterioDaSalvare
                .setFlFiltroSistemaMigraz(creaCritRaggrRegistro.getFlFiltroSistemaMigraz());
        criterioDaSalvare
                .setFlFiltroTiEsitoVerifFirme(creaCritRaggrRegistro.getFlFiltroTiEsitoVerifFirme());
        ...
        OrgStrut orgStrut = crHelper.findById(OrgStrut.class,
                creaCritRaggrRegistro.getIdStrut().longValue());
        ...
        criterioDaSalvare.setTiValidElenco(creaCritRaggrRegistro.getTiValidElenco() != null
                ? TiValidElencoCriterio.valueOf(creaCritRaggrRegistro.getTiValidElenco())
                : null);
        criterioDaSalvare.setTiModValidElenco(creaCritRaggrRegistro.getTiModValidElenco() != null
                ? TiModValidElencoCriterio.valueOf(creaCritRaggrRegistro.getTiModValidElenco())
                : null);
    ...
    public DecCriterioRaggr salvataggioAutomaticoCriterioRaggrStdNoAutomTipoUd(
        ...
        criterioDaSalvare.setDtSoppres(get2444());
        criterioDaSalvare
                .setFlFiltroRangeRegistroKey(creaCritRaggrTipoUd.getFlFiltroRangeRegistroKey());
        ...
        criterioDaSalvare
                .setFlFiltroTiEsitoVerifFirme(creaCritRaggrTipoUd.getFlFiltroTiEsitoVerifFirme());
        ...
        criterioDaSalvare.setDecCriterioFiltroMultiplos(new ArrayList<DecCriterioFiltroMultiplo>());
        OrgStrut orgStrut = crHelper.findById(OrgStrut.class,
                creaCritRaggrTipoUd.getIdStrut().longValue());
        criterioDaSalvare.setOrgStrut(orgStrut);
        ...
        criterioDaSalvare.setTiValidElenco(creaCritRaggrTipoUd.getTiValidElenco() != null
                ? TiValidElencoCriterio.valueOf(creaCritRaggrTipoUd.getTiValidElenco())
                : null);
        criterioDaSalvare.setTiModValidElenco(creaCritRaggrTipoUd.getTiModValidElenco() != null
                ? TiModValidElencoCriterio.valueOf(creaCritRaggrTipoUd.getTiModValidElenco())
                : null);
        crHelper.insertEntity(criterioDaSalvare, true);
        return criterioDaSalvare;
    }
    ...
    public DecCriterioRaggr salvataggioAutomaticoCriterioRaggrStdNoAutomTipoDoc(
        ...
        criterioDaSalvare.setDtSoppres(get2444());
        criterioDaSalvare
                .setFlFiltroRangeRegistroKey(creaCritRaggrTipoDoc.getFlFiltroRangeRegistroKey());
        ...
        criterioDaSalvare
                .setFlFiltroTiEsitoVerifFirme(creaCritRaggrTipoDoc.getFlFiltroTiEsitoVerifFirme());
        ...
        criterioDaSalvare.setDecCriterioFiltroMultiplos(new ArrayList<DecCriterioFiltroMultiplo>());
        OrgStrut orgStrut = crHelper.findById(OrgStrut.class,
                creaCritRaggrTipoDoc.getIdStrut().longValue());
        criterioDaSalvare.setOrgStrut(orgStrut);
        DecTipoDoc decTipoDoc = crHelper.findById(DecTipoDoc.class,
                creaCritRaggrTipoDoc.getIdTipoDoc().longValue());
        DecCriterioFiltroMultiplo filtro = new DecCriterioFiltroMultiplo();
        filtro.setDecCriterioRaggr(criterioDaSalvare);
        filtro.setDecRegistroRangeUnitaDoc(null);
        filtro.setDecRegistroUnitaDoc(null);
        ...
        criterioDaSalvare.setTiValidElenco(creaCritRaggrTipoDoc.getTiValidElenco() != null
                ? TiValidElencoCriterio.valueOf(creaCritRaggrTipoDoc.getTiValidElenco())
                : null);
        criterioDaSalvare.setTiModValidElenco(creaCritRaggrTipoDoc.getTiModValidElenco() != null
                ? TiModValidElencoCriterio.valueOf(creaCritRaggrTipoDoc.getTiModValidElenco())
                : null);
        crHelper.insertEntity(criterioDaSalvare, true);
        return criterioDaSalvare;
    }
    ...
    public DecCriterioRaggr salvataggioAutomaticoCriterioRaggrStdFiscNoAutomTipoUd(
            DecVCreaCritRaggrTipoUd creaCritRaggrTipoUd) {
        DecCriterioRaggr criterioDaSalvare = salvataggioAutomaticoCriterioRaggrStdNoAutomTipoUd(
                creaCritRaggrTipoUd);
    ...
    public DecCriterioRaggrRowBean getDecCriterioRaggrRowBean(BigDecimal idStrut,
            String nmCriterioRaggr) {
        DecCriterioRaggr crit = crHelper.getDecCriterioRaggrByStrutturaCorrenteAndCriterio(idStrut,
                nmCriterioRaggr);
        ...
        if (crit != null) {
            try {
                ...
                String tiGestElencoNoStd = configurationHelper.getValoreParamApplicByStrut(
                        ...
                        BigDecimal.valueOf(
                                crit.getOrgStrut().getOrgEnte().getOrgAmbiente().getIdAmbiente()),
                        idStrut);
                ...
                throw new IllegalStateException(
                        "Errore durante il recupero del criterio di raggruppamento");
            }
        }
        return row;
    }

    public DecCriterioRaggrTableBean getDecCriterioRaggrTableBean(BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, String nmCriterioRaggr) {
        DecCriterioRaggrTableBean table = new DecCriterioRaggrTableBean();
        List<DecCriterioRaggr> list = crHelper.retrieveDecCriterioRaggrList(idAmbiente, idEnte,
                idStrut, nmCriterioRaggr);
        if (list != null && !list.isEmpty()) {
            try {
                ...
                logger.error("Errore durante il recupero dei criteri di raggruppamento "
                        + ExceptionUtils.getRootCauseMessage(ex), ex);
                throw new IllegalStateException(
                        "Errore durante il recupero dei criteri di raggruppamento");
            }
        }
        return table;
    }
        ...
        try {
            if (idRegistroUnitaDoc != null) {
                ...
                context.getBusinessObject(CriteriRaggruppamentoEjb.class)
                        .salvataggioAutomaticoCriterioRaggrStdNoAutomRegistro(param,
                                creaCritRegistro);
                ...
                DecCriterioRaggr criterioSalvato = context
                        .getBusinessObject(CriteriRaggruppamentoEjb.class)
                        .salvataggioAutomaticoCriterioRaggrStdNoAutomTipoUd(creaCritTipoUd);
                sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                        param.getNomeUtente(), param.getNomeAzione(),
                        SacerLogConstants.TIPO_OGGETTO_CRITERIO_RAGGRUPPAMENTO,
                        new BigDecimal(criterioSalvato.getIdCriterioRaggr()),
                        param.getNomePagina());
            } else if (idTipoDoc != null) {
                DecVCreaCritRaggrTipoDoc creaCritTipoDoc = crHelper
                        .getDecVCreaCritRaggrTipoDoc(idTipoDoc.longValue());
                DecCriterioRaggr criterioSalvato = context
                        .getBusinessObject(CriteriRaggruppamentoEjb.class)
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long creaCriteriRaggruppamentoRegistroDaBottone(LogParam param,
            BigDecimal idRegistroUnitaDoc) throws ParerUserError {
        ...
        try {
            ...
            logger.error(
                    "Errore durante la creazione del criterio di raggruppamento per il registro: "
    ...
    public Long creaCriteriRaggruppamentoTipoUdDaBottone(LogParam param, BigDecimal idTipoUnitaDoc)
        ...
        try {
            ...
            logger.error(
                    "Errore durante la creazione del criterio di raggruppamento per il tipo unità documentaria: "
                            + ExceptionUtils.getRootCauseMessage(ex),
                    ex);
            throw new ParerUserError("Errore durante la creazione del criterio di raggruppamento");
        }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long creaCriteriRaggruppamentoTipoDocDaBottone(LogParam param, BigDecimal idTipoDoc)
            throws ParerUserError {
        Long idCriterioRaggr = null;
        try {
            if (idTipoDoc != null) {
            ...
            logger.error(
                    "Errore durante la creazione del criterio di raggruppamento per il tipo documento: "
                            + ExceptionUtils.getRootCauseMessage(ex),
                    ex);
            throw new ParerUserError("Errore durante la creazione del criterio di raggruppamento");
        }
    ...
    public boolean flCriterioRaggrFiscByRegistriTipiUdFiscali(
            List<BigDecimal> idRegistroKeyUnitaDocList, List<BigDecimal> idTipoUnitaDocList,
            boolean flCriterioRaggrStandard) throws ParerUserError {
            ...
            if (allRegistriFiscali && allRegistriAssociatiFiscali) {
                if (flCriterioRaggrStandard) {
                    ...
                    throw new ParerUserError(
                            "Almeno uno dei registri associati al tipo di unità documentaria presente "
    ...
    public boolean checkAllRegistriFiscali(List<BigDecimal> idRegistroKeyUnitaDocList)
            throws ParerUserError {
        return crHelper.areAllRegistriFiscali(idRegistroKeyUnitaDocList);
    }

    public boolean checkAllRegistriAssociatiFiscali(List<BigDecimal> idTipoUnitaDocList)
            throws ParerUserError {
        return crHelper.areAllRegistriAssociatiFiscali(idTipoUnitaDocList);
    }

    public boolean checkAllRegistriAssociatiFiscali(BigDecimal idTipoUnitaDoc)
            throws ParerUserError {

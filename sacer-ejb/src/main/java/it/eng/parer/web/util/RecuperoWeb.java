...
public class RecuperoWeb {
     ...
     * @param user                bean User
     * @param idUnitaDoc          id unita doc
     * @param idCompDoc           id documento componente
     * @param tipoSalvataggioFile tipo salvataggio file CostantiDB.TipoSalvataggioFile
     * @param tipoEntitaSacer     tipo entita sacer CostantiDB.TipiEntitaRecupero
     */
    ...
    public RispostaWSRecupero recuperaOggetto() throws EMFError {
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            ...
            // prepara risposta
            recuperoSync.recuperaOggetto(rispostaWs, myRecuperoExt,
    ...
    public RispostaWSRecupero recuperaOggettoSip() throws EMFError {
        ...
        // Recupera l'ejb, se possibile - altrimenti segnala errore
        try {
            downloadSip = (DownloadSip) new InitialContext()
                    .lookup("java:app/Parer-ejb/DownloadSip");
        } catch (NamingException ex) {
            log.error("Errore nel recupero dell'EJB ", ex);
            throw new EMFError(EMFError.ERROR, "Impossibile recuperare l'ejb: " + ex);
        }
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            ...
            myRecuperoExt.setTipoSalvataggioFile(tipoSalvataggioFile);
            // prepara risposta
            downloadSip.recuperaSip(rispostaWs, myRecuperoExt,
                    System.getProperty("java.io.tmpdir"));
        }
        return rispostaWs;
    }
    ...
    public RispostaWSRecupero recuperaReportFirma() throws EMFError {
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            ...
            myRecuperoExt.setTipoSalvataggioFile(tipoSalvataggioFile);
            // prepara risposta
            downloadReportFirma.recuperaReportFirma(rispostaWs, myRecuperoExt,
                    System.getProperty("java.io.tmpdir"));
        }
        return rispostaWs;
    }

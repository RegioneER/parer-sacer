...
public class CriteriRaggrHelper extends GenericHelper {
    ...
    public Long saveCritRaggr(LogParam param, CreaCriterioRaggr filtri,
            Date[] dateCreazioneValidate, BigDecimal idStruttura, String nome,
            String criterioStandard) throws EMFError, ParerUserError {
        ...
        if (filtri.getNm_tipo_unita_doc().parse() != null
        ...
        if (filtri.getNm_tipo_doc().parse() != null && !filtri.getNm_tipo_doc().parse().isEmpty()) {
            ...
            if (!lista.isEmpty()) {
                ...
                for (DecTipoDoc tipo : lista) {
                    // Se sto modificando il criterio, verifico se ho già inserito precedentemente
                    // il filtro,
        ...
        if (filtri.getNm_sistema_migraz().parse() != null
                && !filtri.getNm_sistema_migraz().parse().isEmpty()) {
            List<String> nmSistemaMigrazList = filtri.getNm_sistema_migraz().parse();
            List<String> orgUsoSistemaMigrazList = getOrgUsoSistemaMigrazByNmSistemaMigrazList(
                    idStruttura, nmSistemaMigrazList);
            if (!orgUsoSistemaMigrazList.isEmpty()) {
                ...
                if (nome != null) {
                    deleteDecCriterioRaggrByDecCriterioRaggrNotInNmSistemaMigraz(rec,
                            nmSistemaMigrazList);
                }
            }
        } else {
        ...
        if (filtri.getCd_registro_key_unita_doc().parse() != null
            ...
            List<Long> cdRegistroKeyUnitaDocList = longListFrom(
                    filtri.getCd_registro_key_unita_doc().parse());
            List<DecRegistroUnitaDoc> lista = getDecRegistroUnitaDocByIdRegistroUnitaDoc(
                    ...
                    if (nome != null) {
                        ...
                        if (decCriterioFiltroMultiploList.isEmpty()) {
                            saveCritRaggrFiltroMultiplo(rec, reg, null, null, null, null,
                                    ApplEnum.TipoFiltroMultiploCriteriRaggr.REGISTRO_UNI_DOC
                                            .name());
                        }
                    } else {
                        saveCritRaggrFiltroMultiplo(rec, reg, null, null, null, null,
        ...
        if (filtri.getTi_esito_verif_firme().parse() != null
            ...
            if (!asList.isEmpty()) {
                    ...
                    if (nome != null) {
                        ...
                        if (decCriterioFiltroMultiploList.isEmpty()) {
                            saveCritRaggrFiltroMultiplo(rec, null, null, null, null, tipo,
                                    ApplEnum.TipoFiltroMultiploCriteriRaggr.TIPO_ESITO_VERIF_FIRME
                                            .name());
                        }
                    } else {
                        saveCritRaggrFiltroMultiplo(rec, null, null, null, null, tipo,
                ...
                // In caso di modifica, l'utente potrebbe aver eliminato qualche filtro dalle
                // multiselect, che non
                // risulterebbero più presenti nella lista
                // Eseguo perciò una bulk delete sui record non presenti nella lista
        ...
        // Salvo di default come non fiscale, poi i successivi controlli stabiliranno l'esatto
        // valore
        ...
        rec.setTiModValidElenco(
                TiModValidElencoCriterio.valueOf(filtri.getTi_mod_valid_elenco().parse()));
        ...
        String flCriterioRaggrFiscMessage = getFlCriterioRaggrFiscMessage(
                new BigDecimal(rec.getIdCriterioRaggr()));
    ...
    public void saveCritRaggrFiltroMultiplo(DecCriterioRaggr crit, DecRegistroUnitaDoc reg,
            DecRegistroUnitaDoc regRange, DecTipoDoc tipoDoc, DecTipoUnitaDoc tipoUD,
            String tiEsitoVerifFirme, String tiFiltroMultiplo) {
        DecCriterioFiltroMultiplo filtro = new DecCriterioFiltroMultiplo();
        filtro.setDecCriterioRaggr(crit);
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggr(
            CriteriRaggruppamentoForm.FiltriCriteriRaggr filtriCriteri, long idUtente)
            throws EMFError {
        return getCriteriRaggr(filtriCriteri.getId_ambiente().parse(),
                filtriCriteri.getId_ente().parse(), filtriCriteri.getId_strut().parse(),
                filtriCriteri.getNm_criterio_raggr().parse(),
                filtriCriteri.getFl_criterio_raggr_standard().parse(),
                filtriCriteri.getFl_criterio_raggr_fisc().parse(),
                filtriCriteri.getTi_valid_elenco().parse(),
                filtriCriteri.getTi_mod_valid_elenco().parse(),
                filtriCriteri.getTi_gest_elenco_criterio().parse(),
                filtriCriteri.getId_registro_unita_doc().parse(),
                filtriCriteri.getId_tipo_unita_doc().parse(),
    ...
    public List<DecCriterioRaggr> retrieveDecCriterioRaggrList(BigDecimal idAmbiente,
    ...
    public boolean deleteDecCriterioRaggr(LogParam param, BigDecimal idStrut,
        ...
        if (row != null && row.getVolVolumeConservs().isEmpty()
                && row.getElvElencoVers().isEmpty()) {
            sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(),
                    param.getNomeUtente(), param.getNomeAzione(),
                    SacerLogConstants.TIPO_OGGETTO_CRITERIO_RAGGRUPPAMENTO,
            ...
            getEntityManager().flush();
            log.info(
                    "Cancellazione criterio di raggruppamento {} della struttura {} avvenuta con successo!",
                    nmCriterioRaggr, idStrut);
            result = true;
        } else {
            if (row == null) {
                throw new ParerUserError("Errore nell'eliminazione del criterio " + nmCriterioRaggr
                        + ", non ci sono record per idStrut " + idStrut + " e nmCriterioRaggr "
                        + nmCriterioRaggr);
            }
    ...
    public DecCriterioFiltroMultiploTableBean getCriteriRaggrFiltri(BigDecimal idCriterioRaggr,
        ...
        StringBuilder queryStr = new StringBuilder("SELECT f FROM DecCriterioFiltroMultiplo f "
                + "WHERE f.decCriterioRaggr.idCriterioRaggr = :idcrit ");
        ...
        try {
            if (listaFiltriCritRaggr != null && !listaFiltriCritRaggr.isEmpty()) {
                ...
                if (ApplEnum.TipoFiltroMultiploCriteriRaggr.RANGE_REGISTRO_UNI_DOC.name()
                        .equals(tiFiltroMultiplo)) {
                    for (DecCriterioFiltroMultiplo d : listaFiltriCritRaggr) {
                        for (DecCriterioFiltroMultiploRowBean row : listaFiltriTableBean) {
                            if (d.getIdCriterioFiltroMult() == row.getIdCriterioFiltroMult()
                                    .longValue()) {
                                row.setIdRegistroRangeUnitaDoc(new BigDecimal(
                                        d.getDecRegistroRangeUnitaDoc().getIdRegistroUnitaDoc()));
                                break;
                            }
                        }
                    }
    ...
    public DecCriterioRaggrTableBean getCriteriRaggrbyId(BigDecimal id, BigDecimal idStruttura) {
        ...
        try {
            if (listaCritRaggr != null && !listaCritRaggr.isEmpty()) {
                listaCritRaggrTableBean = (DecCriterioRaggrTableBean) Transform
                        .entities2TableBean(listaCritRaggr);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
        ...
        try {
            ...
            if (criterioRaggr != null) {
                ...
                String tiGestElencoNoStd = configurationHelper.getValoreParamApplicByStrut(
                        ...
                        BigDecimal.valueOf(criterioRaggr.getOrgStrut().getOrgEnte().getOrgAmbiente()
                                .getIdAmbiente()),
     ...
     *
     * @param idTipiDocumentoList lista id tipi documento
     *
     * @return true o false
     */
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrFromAmministrazioneStruttura(
            BigDecimal idStrut, List<BigDecimal> idRegistroUnitaDocList,
            List<BigDecimal> idTipoUnitaDocList, List<BigDecimal> idTipoDocList) {
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrFromStruttura(BigDecimal idStrut,
            boolean filterValid) {
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggrFromRegistroNoRange(BigDecimal idStrut,
        ...
        return listaCriteriToTableBean(
                getDecVRicCriterioRaggrsByRegistroNoRange(idStrut, idRegistroUnitaDoc));
    }

    public DecVRicCriterioRaggrTableBean getCriteriRaggrFromTipoUnitaDoc(BigDecimal idStrut,
            BigDecimal idTipoUnitaDoc) {
        return listaCriteriToTableBean(
                getDecVRicCriterioRaggrsByTipoUnitaDoc(idStrut, idTipoUnitaDoc));
    }

    public DecVRicCriterioRaggrTableBean getCriteriRaggrFromTipoDoc(BigDecimal idStrut,
            BigDecimal idTipoDoc) {
        return listaCriteriToTableBean(getDecVRicCriterioRaggrsByTipoDoc(idStrut, idTipoDoc));
    }

    private DecVRicCriterioRaggrTableBean listaCriteriToTableBean(
            List<DecVRicCriterioRaggr> listaCritRaggr) {
        ...
        try {
            if (listaCritRaggr != null && !listaCritRaggr.isEmpty()) {
                for (DecVRicCriterioRaggr scriteriato : listaCritRaggr) {
                    critRaggrRowBean = (DecVRicCriterioRaggrRowBean) Transform
                            .entity2RowBean(scriteriato);
                    ...
                    if (scriteriato.getAaKeyUnitaDoc() != null) {
                        ...
                        aaUnitaDoc = scriteriato.getAaKeyUnitaDocDa() + " - "
                                + scriteriato.getAaKeyUnitaDocA();
                    }
    ...
    /**
     * Recupera i criteri di raggruppamento che hanno come campo: - l'anno passato come parametro e
     * - il registro passato come parametro oppure un tipo ud legato al registro passato come
     * parametro
     *
     ...
     */
    public List<DecCriterioRaggr> getDecCriterioRaggrRegistroOTipiUdAssociatiList(
            BigDecimal idRegistroUnitaDoc, int aaKeyUnitaDoc) {
        Query query = getEntityManager().createQuery(
                "SELECT criterioRaggr FROM DecCriterioFiltroMultiplo criterioFiltroMultiplo "
                        + "JOIN criterioFiltroMultiplo.decCriterioRaggr criterioRaggr "
                        + "JOIN criterioFiltroMultiplo.decRegistroUnitaDoc registro "
                        // Prendo o il registro passato come parametro o un tipo ud legato al
                        // registro passato come
                        // parametro
    ...
    /**
     * Restituisce una Stringa con i seguenti possibili valori - 1 = ESISTE UN SOLO Criterio
     * Stardard ATTIVO - 0 = ESISTONO ASSOCIAZIONI ad almeno un criterio non standard - 2 = non è
     * associata a criteri di raggruppamento (né standard né non standard) o se tutti i criteri cui
     * è associata sono disattivi alla data
     *
     * NB. Il valore 2 è utilizzato al posto di NULL perché il framework lo gestisce (per altri
     * casi, correttamente) come se fosse il valore FALSE
     *
     * @param idTipoDato id del tipo dato
     * @param tipoDato   registro, tipo ud oppure tipo doc
     *
    ...
    public String getCriterioStandardPerTipoDatoAnno(long idTipoDato, TipoDato tipoDato) {
        String queryStr = "SELECT criterio FROM DecCriterioFiltroMultiplo multiplo "
                ...
                + "WHERE criterio.flCriterioRaggrStandard = :flagStandard "
    ...
    public boolean existsCriterioPerTipoDato(Long idTipoDato, TipoDato tipoDato,
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT COUNT(criterio) FROM DecCriterioFiltroMultiplo multiplo "
                        + "JOIN multiplo.decCriterioRaggr criterio "
                        + "WHERE criterio.dtSoppres >= :data AND criterio.dtIstituz <= :data ");
        String whereWord = " AND ";
        if (flCriterioRaggrStandard != null) {
            queryStr.append(whereWord)
                    .append("criterio.flCriterioRaggrStandard = :flCriterioRaggrStandard ");
            whereWord = " AND ";
        }
        if (flCriterioRaggrFisc != null) {
            queryStr.append(whereWord)
                    .append("criterio.flCriterioRaggrFisc = :flCriterioRaggrFisc ");
            whereWord = " AND ";
        }
        ...
        case TIPO_UNITA_DOC:
            queryStr.append(whereWord)
                    .append(" multiplo.decTipoUnitaDoc.idTipoUnitaDoc = :idTipoDato ");
            break;
        case TIPO_DOC:
    ...
    public List<DecCriterioRaggr> getCriteriPerTipoDato(Long idTipoDato, TipoDato tipoDato,
            String flCriterioRaggrStandard, String flCriterioRaggrFisc) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT criterio FROM DecCriterioFiltroMultiplo multiplo "
                        + "JOIN multiplo.decCriterioRaggr criterio "
                        + "WHERE criterio.dtSoppres >= :data AND criterio.dtIstituz <= :data ");
        String whereWord = " AND ";
        if (flCriterioRaggrStandard != null) {
    ...
    public DecVCreaCritRaggrTipoUd getDecVCreaCritRaggrTipoUd(Long idTipoUnitaDoc) {
        String queryStr = "SELECT u FROM DecVCreaCritRaggrTipoUd u "
                + "WHERE u.idTipoUnitaDoc = :idTipoUnitaDoc ";
        Query query = getEntityManager().createQuery(queryStr);
    ...
    public DecVCreaCritRaggrTipoDoc getDecVCreaCritRaggrTipoDoc(Long idTipoDoc) {
        String queryStr = "SELECT u FROM DecVCreaCritRaggrTipoDoc u "
                + "WHERE u.idTipoDoc = :idTipoDoc ";
        Query query = getEntityManager().createQuery(queryStr);
     ...
     *
     * @param idRegistroKeyUnitaDocList id registro chiave unita doc
     *
     * @return true/false
     *
     * @throws ParerUserError errore generico
     */
    public boolean areAllRegistriFiscali(List<BigDecimal> idRegistroKeyUnitaDocList)
        ...
        if (!idRegistroKeyUnitaDocList.isEmpty()) {
            ...
            Query query = getEntityManager().createQuery(queryStr);
            query.setParameter("idRegistroKeyUnitaDocList",
                    longListFrom(idRegistroKeyUnitaDocList));
     ...
     *
     * @param idTipoUnitaDocList lista id tipo unita doc
     *
     * @return true/false
     *
     * @throws ParerUserError errore generico
     */
    public boolean areAllRegistriAssociatiFiscali(List<BigDecimal> idTipoUnitaDocList)
            throws ParerUserError {
        if (!idTipoUnitaDocList.isEmpty()) {
            ...
            // (metto in DISTINCT per evitare di avere registri ripetuti associati a tipi ud
            // diverse)
            // li scorro per verificare se sono tutti fiscali. Se la lista da controllare è vuota, è
            // come se fosse false
            Query query = getEntityManager().createQuery(queryStr);
    ...
    public DecCriterioRaggr getDecCriterioRaggrByStrutturaCorrenteAndCriterio(
            BigDecimal idStrutCorrente, String nmCriterioRaggr) {
        StringBuilder queryStr = new StringBuilder("SELECT u FROM DecCriterioRaggr u ");
        String whereWord = "WHERE ";
    ...
    public DecCriterioRaggr getDecCriterioRaggrByStrutturaAndCriterio(BigDecimal idStruttura,
            String nome) {
        DecCriterioRaggr decCriterioRaggr;// Se c'è il parametro nome, carico il criterio di
        // raggruppamento
        // corrispondente
        String queryStr = "SELECT u FROM DecCriterioRaggr u WHERE u.orgStrut.idStrut = :idstrut and u.nmCriterioRaggr = :nomecrit";
    ...
    public DecVRicCriterioRaggrTableBean getCriteriRaggr(BigDecimal idAmbiente, BigDecimal idEnte,
            BigDecimal idStrut, String nmCriterioRaggr, String flCriterioRaggrStandard,
            String flCriterioRaggrFisc, String tiValidElenco, String tiModValidElenco,
            String tiGestElencoCriterio, BigDecimal idRegistroUnitaDoc, BigDecimal idTipoUnitaDoc,
            BigDecimal idTipoDoc, BigDecimal aaKeyUnitaDoc, String criterioAttivo, long idUtente) {
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT DISTINCT new it.eng.parer.viewEntity.DecVRicCriterioRaggr "
        ...
        // Mappa per tenere traccia dei parametri dei chunk (nel caso in cui vengano passati più di
        // 1000 valori), gli
        ...
        if (idAmbiente != null) {
            ...
            if (listaAmbienti != null && !listaAmbienti.isEmpty()) {
                List<BigDecimal> ambientiAbilitati = Utils.bigDecimalFromLong(listaAmbienti.stream()
                        .map(OrgAmbiente::getIdAmbiente).collect(Collectors.toList()));
                queryStr.append(whereWord);
                appendInClauseWithChunking(queryStr, "u.idAmbiente", "ambientiAbilitati",
                        ambientiAbilitati.size(), chunkParameters, ambientiAbilitati);
                whereWord = "AND ";
            }
        }
        ...
        if (idEnte != null) {
            ...
            if (listaEnti != null && !listaEnti.isEmpty()) {
                List<BigDecimal> entiAbilitati = Utils.bigDecimalFromLong(
                        listaEnti.stream().map(OrgEnte::getIdEnte).collect(Collectors.toList()));
                queryStr.append(whereWord);
                appendInClauseWithChunking(queryStr, "u.idEnte", "entiAbilitati",
                        entiAbilitati.size(), chunkParameters, entiAbilitati);
                whereWord = "AND ";
            }
        }
        ...
        if (idStrut != null) {
            ...
            if (strutList != null && !strutList.isEmpty()) {
                List<BigDecimal> struttureAbilitate = Utils.bigDecimalFromLong(
                        strutList.stream().map(OrgStrut::getIdStrut).collect(Collectors.toList()));
                queryStr.append(whereWord);
                appendInClauseWithChunking(queryStr, "u.idStrut", "struttureAbilitate",
                        struttureAbilitate.size(), chunkParameters, struttureAbilitate);
                whereWord = "AND ";
            }
        }
        if (nmCriterioRaggr != null) {
        ...
        if (flCriterioRaggrStandard != null) {
            queryStr.append(whereWord)
                    .append("u.flCriterioRaggrStandard = :flCriterioRaggrStandard ");
            whereWord = "AND ";
        }
        if (flCriterioRaggrFisc != null) {
     ...
     *
     * @param queryStr      StringBuilder della query a cui aggiungere.
     * @param columnName    Nome della colonna dell'entità (es. "u.idEnte").
     * @param baseParamName Nome base per i parametri generati (es. "entiAbilitati").
     * @param totalSize     Dimensione totale della lista di valori.
     * @param parameters    Mappa dove aggiungere i parametri per la query JPA (solo per i chunk).
     * @param values        Lista completa dei valori da usare nella clausola IN.
     */
    private void appendInClauseWithChunking(StringBuilder queryStr, String columnName,
            String baseParamName, int totalSize, Map<String, Object> parameters, List<?> values) {
        if (values == null || values.isEmpty()) {
            // Se la lista di abilitazioni è vuota, significa che l'utente non ha accesso
            // a nulla per quella dimensione, quindi aggiungiamo una condizione che non sarà mai
            // vera.
    ...
    public List<DecVRicCriterioRaggr> getDecVRicCriterioRaggrsByAmministrazioneStruttura(
            BigDecimal idStrut, List<BigDecimal> idRegistroUnitaDocList,
            List<BigDecimal> idTipoUnitaDocList, List<BigDecimal> idTipoDocList) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT DISTINCT new it.eng.parer.viewEntity.DecVRicCriterioRaggr "
    ...
    public List<DecVRicCriterioRaggr> getDecVRicCriterioRaggrsByStruttura(BigDecimal idStrut,
            boolean filterValid) {
        String queryStr = "SELECT DISTINCT new it.eng.parer.viewEntity.DecVRicCriterioRaggr "
                ...
                + "u.dtIstituz, u.dtSoppres) FROM DecVRicCriterioRaggr u "
                + "WHERE u.idStrut = :idStrut ";
    ...
    public List<DecVRicCriterioRaggr> getDecVRicCriterioRaggrsByTipoDoc(BigDecimal idStrut,
            BigDecimal idTipoDoc) {
        String queryStr = "SELECT DISTINCT new it.eng.parer.viewEntity.DecVRicCriterioRaggr "
                + "(u.idAmbiente, u.nmAmbiente, u.idEnte, u.nmEnte, u.idStrut, u.nmStrut, u.id.idCriterioRaggr, u.nmCriterioRaggr, u.nmTipoUnitaDoc, "
                + "u.cdRegistroUnitaDoc, u.cdRegistroRangeUnitaDoc, u.nmTipoDoc, u.flCriterioRaggrStandard, u.flCriterioRaggrFisc, u.tiValidElenco, u.tiModValidElenco, "
                + "u.tiGestElencoCriterio, u.aaKeyUnitaDoc, u.aaKeyUnitaDocDa, u.aaKeyUnitaDocA, u.niMaxComp, u.dsScadChius, u.dtIstituz, u.dtSoppres) FROM DecVRicCriterioRaggr u "
                + "WHERE u.idStrut = :idStrut AND u.id.idTipoDoc = :idTipoDoc "
                + "ORDER BY u.nmCriterioRaggr ";
    ...
    public void deleteDecCriterioFiltroMultiploByDecCriterioRaggrNotInEsitoVerifFirme(
            DecCriterioRaggr decCriterioRaggr, List<String> esitiVerifFirme) {
        Query q = getEntityManager().createQuery("DELETE FROM DecCriterioFiltroMultiplo u "
    ...
    public void deleteDecCriterioFiltroMultiploTipoRegistroUniDocByDecCriterioRaggr(
            DecCriterioRaggr decCriterioRaggr) {
        Query q = getEntityManager().createQuery("DELETE FROM DecCriterioFiltroMultiplo u "
                + "WHERE u.tiFiltroMultiplo = :filtro and u.decCriterioRaggr.idCriterioRaggr = :crit");
    ...
    public void deleteDecCriterioFiltroMultiploByDecCriterioRaggrNotInIdRegistroUnitaDoc(
        ...
        Query q = getEntityManager().createQuery(
                "DELETE FROM DecCriterioFiltroMultiplo u " + "WHERE u.tiFiltroMultiplo = :filtro "
                        + "and u.decCriterioRaggr.idCriterioRaggr = :crit "
    ...
    public List<DecRegistroUnitaDoc> getDecRegistroUnitaDocByIdRegistroUnitaDoc(
            List<Long> cdRegistroKeyUnitaDocList) {
    ...
    public void deleteDecCriterioFiltroMultiploTipoSistemaMigrazByDecCriterioRaggr(
            DecCriterioRaggr decCriterioRaggr) {
        Query q = getEntityManager().createQuery("DELETE FROM DecCriterioFiltroMultiplo u "
                + "WHERE u.tiFiltroMultiplo = :filtro and u.decCriterioRaggr.idCriterioRaggr = :crit");
    ...
    public void deleteDecCriterioRaggrByDecCriterioRaggrNotInNmSistemaMigraz(
            DecCriterioRaggr decCriterioRaggr, List<String> nmSistemaMigrazList) {
    ...
    public List<String> getOrgUsoSistemaMigrazByNmSistemaMigrazList(BigDecimal idStruttura,
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT DISTINCT v.nmSistemaMigraz FROM OrgUsoSistemaMigraz u JOIN u.aplSistemaMigraz v "
                        + "WHERE u.orgStrut.idStrut = :idStrutturain "
                        + "AND v.nmSistemaMigraz is not null ");
    ...
    public void deleteDecCriterioFiltroMultiploByDecCriterioNotInTipoDocs(
            DecCriterioRaggr decCriterioRaggr, List<Long> nmTipoDocList) {
        Query q = getEntityManager().createQuery(
                "DELETE FROM DecCriterioFiltroMultiplo u " + "WHERE u.tiFiltroMultiplo = :filtro "
                        + "and u.decCriterioRaggr.idCriterioRaggr = :crit "
    ...
    public void deleteDecCriterioFiltroMultiploTipoUniDocByDecCriterio(
            DecCriterioRaggr decCriterioRaggr) {
        Query q = getEntityManager().createQuery("DELETE FROM DecCriterioFiltroMultiplo u "
                + "WHERE u.tiFiltroMultiplo = :filtro and u.decCriterioRaggr.idCriterioRaggr = :crit");
    ...
    public void deleteDecCriterioFiltroMultiploTipoDocByDecCriterio(
            DecCriterioRaggr decCriterioRaggr) {
        Query q = getEntityManager().createQuery("DELETE FROM DecCriterioFiltroMultiplo u "
                + "WHERE u.tiFiltroMultiplo = :filtro and u.decCriterioRaggr.idCriterioRaggr = :crit");
        q.setParameter("filtro", ApplEnum.TipoFiltroMultiploCriteriRaggr.TIPO_DOC.name());
    ...
    public void deleteDecCriterioFiltroMultiploByDecCriterioNotInTipoUnitaDoc(
            DecCriterioRaggr decCriterioRaggr, List<Long> asList) {
        Query q = getEntityManager().createQuery(
                "DELETE FROM DecCriterioFiltroMultiplo u " + "WHERE u.tiFiltroMultiplo = :filtro "
                        + "and u.decCriterioRaggr.idCriterioRaggr = :crit "
    ...
    public List<DecCriterioFiltroMultiplo> getDecCriterioFiltroMultiploList(
            DecCriterioRaggr decCriterioRaggr, DecTipoUnitaDoc tipo) {
        Query query = getEntityManager().createQuery("SELECT u FROM DecCriterioFiltroMultiplo u "

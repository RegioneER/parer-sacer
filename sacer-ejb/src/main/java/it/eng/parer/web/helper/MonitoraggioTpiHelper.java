...
public class MonitoraggioTpiHelper {
    ...
    public VrsDtVersTableBean getDateVersamentoArk(Date dateVersDa, Date dateVersA, String flMigraz,
        ...
        StringBuilder queryStr = new StringBuilder(
                "SELECT vrsDtVers from VrsDtVers vrsDtVers WHERE ");
        ...
        if (dateVersDa != null) {
            // MAC#27666
            query.setParameter("dataDa",
                    dateVersDa.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
            // end MAC#27666
        }
        if (dateVersA != null) {
            // MAC#27666
            query.setParameter("dataA",
                    dateVersA.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
            // end MAC#27666
        }
        if (flMigraz != null) {
        ...
        try {
            if (pathsList != null && !pathsList.isEmpty()) {
                for (VrsPathDtVers path : pathsList) {
                    VrsPathDtVersRowBean pathRowBean = (VrsPathDtVersRowBean) Transform
                            .entity2RowBean(path);
                    /*
                     * Modifica custom per la gestione delle archiviazioni
                     */
                    if (path.getVrsArkPathDtVers() != null
                            && !path.getVrsArkPathDtVers().isEmpty()) {
                        ...
                        for (VrsArkPathDtVers arkPath : arkPathList) {
                            if (arkPath.getTiArkPath()
                                    .equals(JobConstants.ArkPath.PRIMARIO.name())) {
                                if (primarioAccapo) {
                                    primarioBuilder.append("\n");
                                }
                                primarioBuilder.append(arkPath.getDsArk());
                                primarioAccapo = true;
                            } else if (arkPath.getTiArkPath()
                                    .equals(JobConstants.ArkPath.SECONDARIO.name())) {
                                if (secondarioAccapo) {
                        ...
                        pathRowBean.setDlArk(
                                primarioBuilder.length() > 0 ? primarioBuilder.toString() : null);
                        pathRowBean.setDlArkSecondario(
                                secondarioBuilder.length() > 0 ? secondarioBuilder.toString()
                                        : null);
                    }
    ...
    public VrsFileNoarkPathDtVersTableBean getFilesNoArkPath(BigDecimal idPathDtVers,
            String tiArkFileNoArk) {
        Query query = entityManager.createQuery(
        ...
        try {
            if (fileNoArkList != null && !fileNoArkList.isEmpty()) {
                fileNoArkTableBean = (VrsFileNoarkPathDtVersTableBean) Transform
                        .entities2TableBean(fileNoArkList);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
     ...
     */
    public TpiVLisDtSchedTableBean getDateSchedulazioniTpi(Date dateSchedDa, Date dateSchedA,
            String flAnomalie, String statoDtSched) {
        StringBuilder queryStr = new StringBuilder(
                "SELECT tpiDtSched from TpiVLisDtSched tpiDtSched WHERE ");
        ...
        try {
            if (dateSched != null && !dateSched.isEmpty()) {
                dateSchedViewBean = (TpiVLisDtSchedTableBean) Transform
                        .entities2TableBean(dateSched);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
        }
    ...
    public TpiSchedJobTableBean getJobList(BigDecimal idTpiDtSched, String flMigraz,
            String tiTpiSchedJob) {
        ...
        try {
            if (jobSchedList != null && !jobSchedList.isEmpty()) {
                for (TpiSchedJob schedJob : jobSchedList) {
                    TpiSchedJobRowBean row = (TpiSchedJobRowBean) Transform
                            .entity2RowBean(schedJob);
                    if (schedJob.getFlJobOk() != null
                            && schedJob.getFlJobOk().equals(JobConstants.DB_TRUE)
                    ...
                    if (schedJob.getNmJob().startsWith("ARCHIVIAZIONE_")
                            || schedJob.getNmJob().startsWith("COPIA_")) {

...
public class CalcoloEstrazioneAsync {
        ...
        try {
            ...
            for (OrgStrut struttura : strutture) {
                ...
                if (struttura.getIdStrut() == 3323) {
                    ...
                    newCalcoloEstrazioneAsyncRef1.manageRichiestaByStrut(struttura.getIdStrut(),
                            rootFolderEcRaPath);
                }
            }
    ...
    public void manageRichiestaByStrut(long idStrut, String rootFolderEcRaPath) throws Exception {
        ...
        SIOrgEnteSiam enteConvenz = calcoloHelper
                .retrieveOrgEnteConvenzById(struttura.getIdEnteConvenz());
        ...
        /*
         * Controllo se esistono richieste di estrazioni appartenenti all'ente convenzionati della
         * struttura corrente
         */
        if (calcoloHelper.checkRichiestaInCoda(struttura.getIdEnteConvenz())) {
            log.debug("Estrazione già in coda per l'ente convenzionato della struttura '"
                    + struttura.getNmStrut());
            /*
             * Ritorno un messaggio di errore che dice che l’estrazione è in coda (non dovrebbe mai
             * succedere in quanto la richiesta dovrebbe essere inibita da UI, doppio controllo)
             */
            throw new ParerInternalError(
                    "Estrazione già in coda per l'ente convenzionato della struttura '"
                            + struttura.getNmStrut());
        }
        ...
        if (!newRichiestaRa.getTiStato().equals(AroRichiestaTiStato.ANNULLATO)) {
            ...
            CalcoloEstrazioneAsync newCalcoloEstrazioneAsyncRef1 = context
                    .getBusinessObject(CalcoloEstrazioneAsync.class);
            newCalcoloEstrazioneAsyncRef1
                    .creaRichiestaEstrazioneAsync(newRichiestaRa.getIdRichiestaRa(), idStrut);
        }
    }

    public AroRichiestaRa generaRichiestaRa(long idStrut, String rootFolderEcRaPath)
            throws Exception {
        String cdErrore = "";
        // ricavo la struttura
        OrgStrut struttura = calcoloHelper.findById(OrgStrut.class, idStrut);
        // Il sistema logga l’inizio di una nuova richiesta di calcolo estrazione AIP sulla tabella
        // ARO_RICHIESTA_RA
        AroRichiestaRa richiestaRa = calcoloHelper.writeAtomicAroRichiestaRa(idStrut);
        // Vista per verificare che tutte le ud, le serie e i fascicoli appartenenti all'ente
        // convenzionato in capo alla
        // struttura abbiano i requisiti
        AroVChkRaUd chkRaUdView = calcoloHelper.findViewById(AroVChkRaUd.class,
        ...
        if (chkRaUdView.getFlUdNonInElenco().equals("1")) {
            ...
            log.debug(
                    "Ud con stato di conservazione diverso da AIP_FIRMATO o IN_ARCHIVIO - annullo la richiesta");
    ...
    public void creaRichiestaEstrazioneAsync(long idRichiestaRa, long idStrut) throws Exception {
        ...
        List<UdSerFascObj> udSerFascObjectList = calcoloHelper
                .retrieveUdSerFascToProcess(struttura);
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                // serve per capire se
                // è l'ultimo elemento
                newCalcoloEstrazioneAsyncRef1.manageUdSerFascObjFase1(
                        richiestaRa.getIdRichiestaRa(), o, !i.hasNext(), isTheFirst);
                /* Verifico se la richiesta corrente è stata annullata */
                isAnnullata = newCalcoloEstrazioneAsyncRef1
                        .checkRichiestaAnnullata(richiestaRa.getIdRichiestaRa());
                if (isAnnullata) {
                    break;
                }
                isTheFirst = false;
        ...
        if (!isAnnullata) {
            // Assegno alla richiesta stato = IN_ATTESA_ESTRAZIONE nella tabella ARO_RICHIESTA_RA
            // definendo anche la
            // data di fine
            log.debug("Richiesta id = " + richiestaRa.getIdRichiestaRa() + " settata con stato "
                    + AroRichiestaTiStato.IN_ATTESA_ESTRAZIONE.name());
            newCalcoloEstrazioneAsyncRef1.setStatoRichiestaRaAtomicByStato(
                    richiestaRa.getIdRichiestaRa(), AroRichiestaTiStato.IN_ATTESA_ESTRAZIONE);
        }
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manageUdSerFascObjFase1(long idRichiestaRa, UdSerFascObj udSerFascObj,
    ...
    public void manageUdFase1(BigDecimal idUd, long idRichiestaRa, boolean isTheLast,
            boolean isTheFirst) throws Exception {
        AroRichiestaRa richiestaRa = calcoloHelper.retrieveRichiestaById(idRichiestaRa);
        AroUnitaDoc ud = calcoloHelper.retrieveUnitaDocById(idUd.longValue());
        AroIndiceAipUd aroIndiceAipUd = calcoloHelper.retrieveIndiceAIPByIdUd(ud.getIdUnitaDoc()); // TODO:
        // si
        // potrebbe
        // mettere
        // nella
        ...
        /*
         * Aggiungo l'indice aip unità documentaria corrente, assegnando stato = DA_ELABORARE e
         * valorizzando la FK alla richiesta corrente
         */
        calcoloHelper.writeAroAipRestituzioneArchivio(aroIndiceAipUd, richiestaRa,
                TiStatoAroAipRa.DA_ELABORARE, AroAipRaTipologiaOggetto.UD);
    ...
    public void elaboraRichiesteScadute(long idStrut) throws Exception {
        ...
        // determino le richieste con stato diverso da ANNULLATO appartenenti all'ente convenzionato
        // della struttura
        // corrente,
        // la cui occorrenza sulla ARO_RICHIESTA_RA sia con dt_fine == null e dt_inizio + 24h sia
        // antecedente
        // all'istante corrente
        List<Long> richiesteScaduteDaProcessare = calcoloHelper
                .retrieveRichiesteScaduteDaProcessare(idStrut);
        CalcoloEstrazioneAsync newCalcoloEstrazioneAsyncRef1 = context
                .getBusinessObject(CalcoloEstrazioneAsync.class);
        ...
        for (Long richiestaId : richiesteScaduteDaProcessare) {
            log.debug("trovata richiesta " + richiestaId
                    + " scaduta da settare in stato = ANNULLATO");
            newCalcoloEstrazioneAsyncRef1.setDaAnnullareAtomic("", richiestaId, idStrut,
                    "RICHIESTA SCADUTA");
        }
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void setDaAnnullareAtomic(String cdErrore, Long idRichiestaRa, long idStrut,
            String annullReason) throws Exception {
    ...
    private void setAnnullata(String cdErrore, AroRichiestaRa richiesta, OrgStrut struttura,
            String annullReason) throws Exception {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void setStatoRichiestaRaAtomicByStato(long idRichiestaRa, AroRichiestaTiStato tiStato)
            throws Exception {
    ...
    private void setStatoRichiestaRaByStato(AroRichiestaRa richiesta, AroRichiestaTiStato tiStato)
            throws Exception {
    ...
    public boolean checkRichiestaAnnullata(long idRichiestaRa) {
        ...
        log.debug("Verifico se la richiesta '" + richiesta.getIdRichiestaRa() + "' con data inizio "
                + dateToString(richiesta.getTsInizio())
                + " risulta annullata all'istante corrente (" + dateToString(actualDate) + ")");

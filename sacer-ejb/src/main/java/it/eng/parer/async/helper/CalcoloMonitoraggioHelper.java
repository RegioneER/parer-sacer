...
public class CalcoloMonitoraggioHelper {
	...
	/*
	 * Ricavo le entità di tipo VrsSessioneVers di tipo chiusa in errore dove la struttura è
	 * nulla
	 */
	StringBuilder queryStr = new StringBuilder(
		"SELECT ses.idSessioneVersKo FROM VrsSessioneVersKo ses "
			+ "WHERE ses.orgStrut is null ");
    ...
    public List<VrsVVersFallitiDaVerif> getListaVersFallitiDaVerif(Long idStrut,
    ...
    public List<VrsVVersFallitiDaNorisol> getListaVersFallitiDaNorisol(Long idStrut,
	    Date ultimaRegistrazione) {
	String queryStr = "SELECT u FROM VrsVVersFallitiDaNorisol u "
		+ "WHERE u.idStrut = :idStrut " + "AND u.dtApertura > :ultimaRegistrazione ";
	Query query = entityManager.createQuery(queryStr);
	query.setParameter("idStrut", bigDecimalFromLong(idStrut));
	query.setParameter("ultimaRegistrazione", ultimaRegistrazione);
	return query.getResultList();
	...
	/*
	 * Imposto la sezione relativa alla data all'1 gennaio 2011 relativa all'ora alle
	 * 0:00:00.000
	 */
	...
	TypedQuery<VrsXmlDatiSessioneVersKo> query = entityManager.createQuery("SELECT xmlRich "
		+ "FROM VrsSessioneVersKo ses " + "JOIN ses.vrsDatiSessioneVersKos datiSes "
    ...
    /*
     * MEV 30369 - TODO da rimuovere quando viene completata la migrazione dei BLOB
     * (https://parermine.regione.emilia-romagna.it/issues/29978) se non lo trovo su
     * VRS_XML_DATI_SESSIONE_VERS_KO lo cerco su VRS_XML_DATI_SESSIONE_VERS perché evidentemente non
     * è ancora stato spostato il CLOB
     */
    private String scegliXmlDatiSessioneVers(String blXml, Long idXmlDatiSessioneVersKo) {
	if (StringUtils.isBlank(blXml)) {
	    VrsXmlDatiSessioneVers vrsXmlDatiSessioneVers = entityManager
		    .find(VrsXmlDatiSessioneVers.class, idXmlDatiSessioneVersKo);
	    return !Objects.isNull(vrsXmlDatiSessioneVers) ? vrsXmlDatiSessioneVers.getBlXml()
		    : StringUtils.EMPTY;
	}
	return blXml;
    }
	...
	if (xmlRich != null) {
	    /*
	     * ... e se uno dei campi della chiave unità documentaria (registro/anno/numero) sono
	     * vuoti, provo a procedere al calcolo della chiave unità documentaria
	     */
		...
		if (chiaveStart != -1 && chiaveStop != -1) {
		    String chiave = xmlRich.substring(chiaveStart + ("<Chiave>".length()),
			    chiaveStop);
			...
			if (numero != null && !numero.equals("")) {
			    // Controllo che cdKeyUnitaDoc abbia una lunghezza inferiore o uguale ai
			    // 100 byte previsti
			    // su DB, altrimenti tronco
			    ses.setCdKeyUnitaDoc(
				    numero.length() > 100 ? numero.substring(0, 100) : numero);
			}
			if (tipoReg != null && !tipoReg.equals("")) {
			    // Controllo che cdRegistroKeyUnitaDoc abbia una lunghezza inferiore o
			    // uguale ai 100 byte
			    // previsti su DB, altrimenti tronco
			    ses.setCdRegistroKeyUnitaDoc(
				    tipoReg.length() > 100 ? tipoReg.substring(0, 100) : tipoReg);
			}
		    ...
		    // Ricerca unita doc con idStrut e questi dati, se esiste setto l'id nella
		    // sessione
		    if (ses.getCdKeyUnitaDoc() != null && ses.getAaKeyUnitaDoc() != null
			    && ses.getCdRegistroKeyUnitaDoc() != null) {
			AroUnitaDoc ud = me.getUnitaDocIfExists(ses.getOrgStrut().getIdStrut(),
				ses.getCdKeyUnitaDoc(), ses.getAaKeyUnitaDoc(),
				ses.getCdRegistroKeyUnitaDoc());
			if (ud != null) {
	    ...
	    if (ses.getTiSessioneVers() != null && ses.getTiSessioneVers()
		...
		if (idDoc != null && !idDoc.equals("")) {
		    // Controllo che cdKeyDocVers abbia una lunghezza inferiore o uguale ai 100 byte
		    // previsti su DB,
		    // altrimenti tronco
	...
	if (StringUtils.isBlank(xml)) {
	    Map<String, String> xmls = objectStorageService
		    .getObjectSipInStaging(ses.getIdSessioneVersKo());
	...
	Query query = entityManager.createQuery(
		"SELECT strut FROM OrgStrut strut JOIN strut.orgEnte ente JOIN ente.orgAmbiente ambiente "
    ...
    public AroUnitaDoc getUnitaDocIfExists(Long idStrut, String cdKeyUnitaDoc,
	    BigDecimal aaKeyUnitaDoc, String cdRegistroKeyUnitaDoc) {
    ...
    /**
     * Ricavo la lista dei "Versamenti unità documentarie falliti" Di questi ricavo quelli con
     * chiave nulla
     *
     * @param filtriSes filtro monitoraggio
     *
     * @return listaVersErr in errore
     */
    public List<Object[]> getSessioniKoSenzaChiave(
	    MonitoraggioFiltriListaVersFallitiBean filtriSes) {
	...
	if (tipoSes != null) {
		...
		queryStr.append(whereWord).append("u.cdRegistroKeyUnitaDoc is null "
			+ "AND u.cdKeyUnitaDoc is null " + "AND u.aaKeyUnitaDoc is null ");
	    } else if (tipoSes.equals(Constants.TipoSessione.AGGIUNGI_DOCUMENTO.name())) {
		queryStr.append(whereWord)
			.append("((u.cdRegistroKeyUnitaDoc is null "
				+ "AND u.cdKeyUnitaDoc is null " + "AND u.aaKeyUnitaDoc is null) "
				+ "OR u.cdKeyDocVers is null) ");
	    }
	}
	...
	/*
	 * MEV 30369 - TODO da rimuovere quando viene completata la migrazione dei BLOB obj[0] =
	 * idSessioneVers obj[1] = blob xml obj[2] = idXmlDatiSessioneVersKo
	 */
	return resultList.stream().map(obj -> new Object[] {
		obj[0], scegliXmlDatiSessioneVers((String) obj[1], (Long) obj[2]) })
		.collect(Collectors.toList());
    }

    /*
     * Eseguo l'update dell'idStruttura trovato nei record della sessione relativi a
     * VrsContenutoFileKo
     */
	...
	if (ses.getTiSessioneVers().equals(Constants.TipoSessione.VERSAMENTO.name())) {
	    if (ses.getOrgStrut() != null && ses.getCdRegistroKeyUnitaDoc() != null
		    && ses.getAaKeyUnitaDoc() != null && ses.getCdKeyUnitaDoc() != null) {
		if (!isSessioneRisolta(ses.getIdSessioneVersKo(), ses.getTiSessioneVers())) {
		    // MEV 29971
		    log.debug(
			    "Sessione non risolta - verifico se l'UD non è ancora stata versata in ARO_UNITA_DOC");
		    VrsUnitaDocNonVer udNonVer;
		    if (!udHelper.existAroUnitaDoc(
			    BigDecimal.valueOf(ses.getOrgStrut().getIdStrut()),
			    ses.getCdRegistroKeyUnitaDoc(), ses.getAaKeyUnitaDoc(),
			    ses.getCdKeyUnitaDoc())) {
			log.debug(
		...
		if (!isSessioneRisolta(ses.getIdSessioneVersKo(), ses.getTiSessioneVers())) {
		    log.debug(
			    "Sessione non risolta - creo un record in VrsDocNonVers, se non esiste già");
		    VrsDocNonVer docNonVer;
		    if ((docNonVer = existVrsDocNonVers(ses.getOrgStrut().getIdStrut(),
			    ses.getCdRegistroKeyUnitaDoc(), ses.getAaKeyUnitaDoc(),
			    ses.getCdKeyUnitaDoc(), ses.getCdKeyDocVers())) == null) {
			...
			if (ses.getDsErrPrinc() != null
				&& !ses.getDsErrPrinc().equals(docNonVer.getDsErrPrinc())) {
			    docNonVer.setDsErrPrinc(DIVERSI);
			}
			if (ses.getCdErrPrinc() != null
				&& !ses.getCdErrPrinc().equals(docNonVer.getCdErrPrinc())) {
	...
	String queryStr = "SELECT v FROM " + table
		+ " v WHERE v.idSessioneVers = :idSessioneVersKo";
	// CREO LA QUERY ATTRAVERSO L'ENTITY MANAGER
	Query query = entityManager.createQuery(queryStr);
	...
	Query query = entityManager.createQuery("SELECT v FROM VrsUnitaDocNonVer v "
		+ "WHERE v.orgStrut.idStrut = :idStrut "
		+ "AND v.cdRegistroKeyUnitaDoc = :cdRegistroKeyUnitaDoc "
		+ "AND v.aaKeyUnitaDoc = :aaKeyUnitaDoc " + "AND v.cdKeyUnitaDoc = :cdKeyUnitaDoc");
	query.setParameter("idStrut", idStrut);
	query.setParameter("cdRegistroKeyUnitaDoc", cdRegistroKeyUnitaDoc);
	query.setParameter("aaKeyUnitaDoc", aaKeyUnitaDoc);
	query.setParameter("cdKeyUnitaDoc", cdKeyUnitaDoc);
	return (VrsUnitaDocNonVer) (!query.getResultList().isEmpty() ? query.getResultList().get(0)
		: null);
    }

    private VrsDocNonVer existVrsDocNonVers(long idStrut, String cdRegistroKeyUnitaDoc,
	    BigDecimal aaKeyUnitaDoc, String cdKeyUnitaDoc, String cdKeyDocVers) {

	Query query = entityManager.createQuery("SELECT v FROM VrsDocNonVer v "
		+ "WHERE v.orgStrut.idStrut = :idStrut "
		+ "AND v.cdRegistroKeyUnitaDoc = :cdRegistroKeyUnitaDoc "
		+ "AND v.aaKeyUnitaDoc = :aaKeyUnitaDoc " + "AND v.cdKeyUnitaDoc = :cdKeyUnitaDoc "
		+ "AND v.cdKeyDocVers = :cdKeyDocVers");
	query.setParameter("idStrut", idStrut);
	query.setParameter("cdRegistroKeyUnitaDoc", cdRegistroKeyUnitaDoc);
	query.setParameter("aaKeyUnitaDoc", aaKeyUnitaDoc);
	query.setParameter("cdKeyUnitaDoc", cdKeyUnitaDoc);
	query.setParameter("cdKeyDocVers", cdKeyDocVers);
	return (VrsDocNonVer) (!query.getResultList().isEmpty() ? query.getResultList().get(0)
		: null);

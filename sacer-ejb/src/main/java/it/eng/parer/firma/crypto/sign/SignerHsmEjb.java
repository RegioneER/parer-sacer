...
public class SignerHsmEjb {
    ...
    public Future<SigningResponse> signP7MRequest(SigningRequest request) {
	...
	if (request != null) {
		...
		if (!ejb.hasUserActiveSessions(request.getIdUtente())) {
		    ...
		    result = context.getBusinessObject(SignerHsmEjb.class).signP7m(sessione,
			    request.getUserHSM());
		} else {
	...
	try {
		...
		try {
		    // byte[] signedFile = clientHsm.signP7MRequest(hsmSession, file2sign);
		    ...
		    /*
		     * Nel caso in cui si tratti di gestione elenchi indici AIP la cui gestione può
		     * essere MARCA o SIGILLO con anche MARCA allora il parametro generico di
		     * entrata "marcaTemporale" viene ignorato e si richiede per XADES la sola firma
		     * oppure anche la marca in base al tipo gestione dell'elenco di versamento.
		     */
		    if (sessione.getTiSessioneFirma()
			    .equals(sessione.getTiSessioneFirma().ELENCO_INDICI_AIP)) {
			String tipoGestione = elencoIndiciAipSignEjb.getTipoGestioneElenco(idFile,
				sessione.getIamUser().getIdUserIam());
			if (tipoGestione != null && (tipoGestione
				.equals(ElencoEnums.GestioneElencoEnum.MARCA_FIRMA.name())
				|| tipoGestione.equals(
					ElencoEnums.GestioneElencoEnum.MARCA_SIGILLO.name()))) {
			    ...
			    .equals(sessione.getTiSessioneFirma().SERIE)) {
			/*
			 * Per le serie si è deciso che la firma è sempre Xades senza marca in
			 * quanto il tipo di conservazione fiscale probabilmente diventerà
			 * deprecato, quindi quella parte di codice che gestisce lo stato della
			 * serie rimane che lo stato lo mette a FIRMATA_NO_MARCA
			 */
			signedFile = clientHsm.signP7M(hsmSession, file2sign, false);
		    } else if (sessione.getTiSessioneFirma()
			    .equals(sessione.getTiSessioneFirma().ELENCHI_INDICI_AIP_FASC)) {
			signedFile = clientHsm.signP7M(hsmSession, file2sign, false); // Richiede
										      // sempre
										      // firma senza
			// marca
		    } else {
			...
			if (verificaOk) {
			    ...
			    // Questo controllo serve a garantire che il documento firmato fosse
			    // realmente quello
			    // inviato
		    ...
		    /*
		     * LM 23/08/2017 Se il metodo di firma lancia questa eccezione invece che una di
		     * quelle soprastanti, bisogna controllare se "wrappa" correttamente le
		     * eccezioni provenienti dal HSM (vd documentazione su Wiki)
		     */
		    throw ex;
		}
	    }
	...
	Future<SigningResponse> result = null;
	...
	try {
		...
		try {
			...
			if (tipoGestione != null && (tipoGestione
			    ...
			    .equals(sessione.getTiSessioneFirma().SERIE)) {
			/*
			 * Per le serie si è deciso che la firma è sempre senza marca in quanto il
			 * tipo di conservazione fiscale probabilmente diventerà deprecato, quindi
			 * quella parte di codice che gestisce lo stato della serie rimane che lo
			 * stato lo mette a FIRMATA_NO_MARCA
			 */
			signedFile = clientHsm.signXAdES(hsmSession, file2sign, false);
		    } else if (sessione.getTiSessioneFirma()
			    .equals(sessione.getTiSessioneFirma().ELENCHI_INDICI_AIP_FASC)) {
			signedFile = clientHsm.signXAdES(hsmSession, file2sign, false); // Richiede
											// sempre
											// firma
											// senza
											// marca

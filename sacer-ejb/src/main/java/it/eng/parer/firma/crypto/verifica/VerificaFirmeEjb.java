...
public class VerificaFirmeEjb {
    ...
    public void verificaFirme() throws Exception {
        ...
        int numMaxUdInCodaFase1 = Integer.parseInt(configurationHelper.getValoreParamApplicByApplic(
                CostantiDB.ParametroAppl.NUM_MAX_UD_IN_CODA_VERIFICA_FIRME_DT_VERS));
        ...
        for (OrgVLisStrutPerEle struttura : strutture) {
            if (totMessiInCodaFase1 < numMaxUdInCodaFase1) {
                ...
                totMessiInCodaFase1 = elaboraStrutturaFase1(struttura.getIdStrut().longValue(),
                        totMessiInCodaFase1, numMaxUdInCodaFase1, numGgResetStatoInElenco);
            } else {
                LOG.info(JOB_NAME + " - processate " + totMessiInCodaFase1
                        + " unità documentarie su " + numMaxUdInCodaFase1
                        + ", salta messa in coda per verifica firme");
            }
        }
        jobHelper.writeLogJob(OpTypeEnum.VERIFICA_FIRME_A_DATA_VERS.name(),
    ...
    private Integer elaboraStrutturaFase1(long idStruttura, Integer totMessiInCoda, int numMax,
        ...
        // determino gli elenchi in stato VALIDATO o IN_CODA_JMS_VERIFICA_FIRME_DT_VERS in modo che
        // si elaborano prima i
        // fiscali, per anno del contenuto descending e per data di firma e poi i non fiscali e
        // appartenenti alla
        // struttura corrente
        ...
        LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": trovati "
                + elenchiDaVerificare.size()
        ...
        for (ElvElencoVersDaElab elencoDaElab : elenchiDaVerificare) {
            ...
            // MEV#26288
            if (evHelper.checkStatoAllUdInElencoPerVerificaFirmeDtVers(
                ...
                for (BigDecimal idUd : evHelper.retrieveUdVersOrAggOrUpdInElencoValidate(
                    ...
                    if (totMessiInCoda < numMax) {
                        ...
                        LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": processate "
                                + totMessiInCoda + " unità documentarie dei "
                                + elenchiDaVerificare.size()
                ...
                LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": processate "
                        + totMessiInCoda + " unità documentarie dei " + elenchiDaVerificare.size()
                        + " elenchi trovati da processare con stato VALIDATO o IN_CODA_JMS_VERIFICA_FIRME_DT_VERS");
                return totMessiInCoda;
            }
        }
     ...
     * <li>Il sistema aggiorna elenco da elaborare corrente (ELV_ELENCO_VERS_DA_ELAB) assegnando
     * stato = FIRME_VERIFICATE_DT_VERS</li>
     * </ol>
     *
     * @param idElencoVers       - elenco di cui deve essere aggiornato lo stato
     * @param idElencoVersDaElab - elencoDaElab di cui aggiornare lo stato
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)

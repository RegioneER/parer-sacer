...
/**
 * This helper read and store the informations about signature session of <code>Elenco Indici AIP fascicoli</code>
 *
 * @author DiLorenzo_F
 */
...
public class ElenchiIndiciAipFascSignatureHelper extends SigningHelper {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ElenchiIndiciAipFascSignatureHelper.class);
    ...
    public void addFile2SessioneFirma(long sessionId, long elencoId) {
        ...
        logger.debug("Added elenco (id {}) to HsmSessioneFirma (id {})", elencoId, session.getIdSessioneFirma());
    }
    ...
    public List<HsmSessioneFirma> getBlockedSessionsByUser(long userId) {
        ...
        Query query = getEntityManager().createQuery("SELECT se.idSessioneFirma, MAX(e.tsEsito) AS TsLastOperation "
                + "FROM HsmSessioneFirma se INNER JOIN se.hsmElencoFascSesFirmas e " + "WHERE se.tsFine IS NULL "
    ...
    /**
     * Returns the {@link HsmElencoFascSesFirma} of {@code session} whose <code>Elenco</code> id is {@code idElenco}
     *
     * @param session
     *            sessione HSM
     * @param idElenco
    ...
    public HsmElencoFascSesFirma findElencoFascSes(long sessionId, long idElenco) {
        ...
        Query query = getEntityManager().createQuery(
                "SELECT e " + "FROM HsmElencoFascSesFirma e " + "WHERE e.hsmSessioneFirma.idSessioneFirma = :sessionId "

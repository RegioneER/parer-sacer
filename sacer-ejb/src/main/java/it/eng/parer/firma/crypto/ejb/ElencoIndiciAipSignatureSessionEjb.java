...
public class ElencoIndiciAipSignatureSessionEjb implements SignatureSessionEjb {
    ...
    private static Logger logger = LoggerFactory.getLogger(ElencoIndiciAipSignatureSessionEjb.class.getName());
    ...
    public HsmSessioneFirma startSessioneFirma(SigningRequest request) {
        ...
        for (BigDecimal elencoId : request.getFiles()) {
            ...
            if (elenco.getTiStatoElenco().equals(ElencoStatusEnum.ELENCO_INDICI_AIP_CREATO.name())) {
                ...
                elencoDaElab.setTiStatoElenco(ElencoStatusEnum.ELENCO_INDICI_AIP_FIRMA_IN_CORSO.name());
                ...
                logger.warn("Elenco (id: " + elenco.getIdElencoVers() + ") non è nello stato "
                        + ElencoStatusEnum.ELENCO_INDICI_AIP_CREATO.name() + " ma in " + elenco.getTiStatoElenco());
            }
        }
    ...
    public boolean isFileEquals(long idFile, byte[] fileSbustato) {
        ...
        if (fileSbustato != null) {
            ...
            if (elencoIndiceAip.size() != 1) {
                throw new IllegalStateException("Impossibile trovare l'elenco indici AIP da firmare");
            } else {
                ...
                if (fileElenco.getCdEncodingHashFile() != null
                        && fileElenco.getCdEncodingHashFile().equals(TipiEncBinari.BASE64.descrivi())

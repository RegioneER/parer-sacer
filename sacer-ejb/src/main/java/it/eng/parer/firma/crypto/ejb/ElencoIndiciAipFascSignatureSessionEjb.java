...
public class ElencoIndiciAipFascSignatureSessionEjb implements SignatureSessionEjb {
    ...
    private static Logger logger = LoggerFactory
            .getLogger(ElencoIndiciAipFascSignatureSessionEjb.class.getName());
    ...
    public HsmSessioneFirma startSessioneFirma(SigningRequest request) {
        ...
        for (BigDecimal elencoId : request.getFiles()) {
            ...
            if (statoElenco.getTiStato().equals(TiStatoElencoFasc.ELENCO_INDICI_AIP_CREATO)) {
                ...
                // aggiorno l’elenco da elaborare assegnando stato =
                ...
                elencoVersFascDaElab
                        .setTiStato(TiStatoElencoFascDaElab.ELENCO_INDICI_AIP_FIRMA_IN_CORSO);
    ...
    public boolean isFileEquals(long idFile, byte[] fileSbustato) {
        ...
        if (fileSbustato != null) {
            ...
            List<ElvFileElencoVersFasc> elencoIndiceAipFasc = elencoHlp
                    .retrieveFileIndiceElenco(idFile, new String[] {
                            ElencoEnums.FileTypeEnum.ELENCO_INDICI_AIP.name() });
            if (elencoIndiceAipFasc.size() != 1) {
                throw new IllegalStateException(
                        "Impossibile trovare l'elenco indici AIP fascicolo da firmare");
            } else {
    ...
    public void storeSignature(HsmSessioneFirma session, long idFile, byte[] signedFile,
        ...
        // tipoFirma);
        // MEV#30399
        BackendStorage backendIndiciAip = objectStorageService.lookupBackendElenchiIndiciAipFasc(
                elencoSession.getElvElencoVersFasc().getOrgStrut().getIdStrut());
        ElvFileElencoVersFasc fileElencoVers = elencoEjb.storeFirmaElencoIndiceAipFasc(idFile,
                signedFile, signingDate, session.getIamUser().getIdUserIam(), backendIndiciAip,
                tipoFirma);
        /*
        ...
        if (backendIndiciAip.isObjectStorage()) {
            ...
            ObjectStorageResource res = objectStorageService.createResourcesInElenchiIndiciAipFasc(
                    urn, backendIndiciAip.getBackendName(), signedFile,
                    fileElencoVers.getIdFileElencoVersFasc(), fileElencoVers.getIdStrut());
            logger.debug(
                    "Salvato il file dell'elenco indici aip fascicoli firmato nel bucket {} con chiave {} ",
    ...
    private void signatureSessionInError(HsmSessioneFirma session, SignatureSession.CdErr codError,
        ...
        for (HsmElencoFascSesFirma elencoSess : session.getHsmElencoFascSesFirmas()) {
            if (elencoSess.is2sign()) {
                // Doesn't open a new transaction
                this.errorFile(session, elencoSess.getElvElencoVersFasc().getIdElencoVersFasc(),

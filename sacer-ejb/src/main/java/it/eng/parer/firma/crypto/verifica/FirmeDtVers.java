...
public class FirmeDtVers {
    ...
    public void verificaFirme(AroCompDoc compDoc, Date dataRif)
            throws SignatureException, CRLNotFoundException {
        LOG.info("Verifica firme: Id componente {} data di riferimento {}", compDoc.getIdCompDoc(),
                dataRif);
        ...
        try {
            if (firmeComp != null) {
                ...
                for (AroFirmaComp firmaComp : firmeComp) {
                    if (firmaComp.getTiEsitoContrConforme()
                            .equals(EsitoControllo.FORMATO_NON_CONFORME.name())) {
                        esitoPositivo = false;
                        continue;
                    }
                    ...
                    this.certificateExpiration(certificateExpiration, firmaComp, dataRif,
                            isCertificatoErrato);
                    // MAC#28280
                    // CryproLibrary
                    ...
                    AroVerifFirmaDtVer firma = this.buildFirma(crl, caCertificateExpiration,
                            certificateExpiration, crlValidation,
                            firmaComp.getFirCertifFirmatario().getFirCertifCa(), controlliVers,
                            true);
    ...
    protected AroVerifFirmaDtVer buildFirma(ParerCRL crl, ValidationInfos caCertificateExpiration,
            ValidationInfos certificateExpiration, ValidationInfos crlValidation,
            FirCertifCa certificatoCa, HashMap<String, AroContrFirmaComp> controlliVers,
            boolean flagExceptionNoCrl) throws CryptoParerException, CRLNotFoundException {
        ...
        if (controlliVers.get(TipoControlli.CRITTOGRAFICO_ABILITATO.name()) != null) { // TEST
            ...
            esito = controlliVers.get(TipoControlli.CRITTOGRAFICO_ABILITATO.name())
                    .getTiEsitoContrFirma();
            msg = controlliVers.get(TipoControlli.CRITTOGRAFICO_ABILITATO.name())
        ...
        if (controlliVers.get(TipoControlli.CATENA_TRUSTED_ABILITATO.name()) != null) { // TEST
            // NECESSARIO PER IL PREGRESSO
            esito = controlliVers.get(TipoControlli.CATENA_TRUSTED_ABILITATO.name())
                    .getTiEsitoContrFirma();
            msg = controlliVers.get(TipoControlli.CATENA_TRUSTED_ABILITATO.name())
                    .getDsMsgEsitoContrFirma();
        } else {
            ...
            if (caCertificateExpiration != null) {
                if (caCertificateExpiration.isValid()) {
                    ...
                    controlliCertificato
                            .setTiEsitoContrVerif(caCertificateExpiration.getEsito().name());
                    controlliCertificato
                            .setDsMsgContrVerif(caCertificateExpiration.getEsito().message() + " : "
        ...
        if (certificateExpiration != null) {
            if (certificateExpiration.isValid()) {
                ...
                controlliCertificato.setDsMsgContrVerif(certificateExpiration.getEsito().message()
        ...
        // MAC#28280
        /*
         * CONTROLLI FIRMA - CRL nb: la nuova chiamata alla libreria di chiusura dovrà lanciare
         * un'eccezione se l'esito di verifica CRL è pari a CRL_SCADUTA o CRL_NON_SCARICABILE e la
         * CA risulta attiva (ie. la CA ha un certificato in corso di validità)
         *
         */
        ...
        String tiEsitoMsg;
        if (esitoVerifiche) {
            tiEsito = (isCRLWarning ? EsitoControllo.WARNING.name()
                    : EsitoControllo.POSITIVO.name());
            tiEsitoMsg = (isCRLWarning ? EsitoControllo.WARNING.message()
                    : EsitoControllo.POSITIVO.message());
        } else {
    ...
    private void certificateExpiration(ValidationInfos validationInfos, AroFirmaComp firmaComp,
            Date dataRiferimento, boolean isCertificatoErrato) {
    ...
    private void caCertificateExpiration(ValidationInfos validationInfos, AroFirmaComp firmaComp,
        ...
        if (cerCa.getDtIniValCertifCa().before(dataRiferimento)
                && cerCa.getDtFinValCertifCa().after(dataRiferimento)) {
            validationInfos.setEsito(EsitoControllo.POSITIVO);
        } else if (!cerCa.getDtIniValCertifCa().before(dataRiferimento)) {
            validationInfos
                    .addError("Il certificato di certificazione è entrato in vigore in data: "
                            + dateFormatter.format(cerCa.getDtIniValCertifCa())
                            + " successivo al riferimento temporale usato: "
                            + dateFormatter.format(dataRiferimento));
            ...
            validationInfos.addError("Il certificato di certificazione è scaduto in data: "
                    + dateFormatter.format(cerCa.getDtFinValCertifCa())
    ...
    private boolean isNull(ParerCRL crl) {
        return crl == null || (crl.getKeyId() == null && crl.getPrincipalName() == null
                && crl.getSubjectDN() == null);
    }
    ...
    private ParerCRL certificateRevocation(ValidationInfos validationInfos, AroFirmaComp firmaComp,
        ...
        try {
            ...
            if (keyId == null) {
                ...
                if (dnCa == null || StringUtils.isBlank(dnCa)) {
                    LOG.debug(
                            "Per la CA con id {} non e' stato trovato il SubjectDN. Utilizzo l'issuer DN",
        ...
        if (isNull(crl) || (crl.getNextUpdate().before(dataRiferimento) && isCaAttiva)
                || (crl.getNextUpdate().before(dataRiferimento) && !isCaAttiva && crl
                        .getNextUpdate().before(cerFirma.getFirCertifCa().getDtFinValCertifCa()))) {
            try {
        ...
        if (crl.getNextUpdate().after(dataRiferimento)) {
            ...
            if (crlEntry != null) {
                if (dataRiferimento != null
                        && crlEntry.getRevocationDate().before(dataRiferimento)) {
    ...
    private void dumpEjbException(EJBException e) {
        ...
        LOG.warn("Eccezione di tipo {} invocando le CRL. La transazione è marcata per rollback? {}",
                tipo, context.getRollbackOnly(), e);
    }

...
public class SerieSignatureSessionEjb implements SignatureSessionEjb {
    ...
    private static Logger logger = LoggerFactory.getLogger(SerieSignatureSessionEjb.class.getName());
    ...
    public HsmSessioneFirma startSessioneFirma(SigningRequest request) {
        ...
        for (BigDecimal verSerieId : request.getFiles()) {
            ...
            SerStatoVerSerie currentStato = signHlp.findById(SerStatoVerSerie.class, verSerie.getIdStatoVerSerieCor());
            ...
            if (currentStato.getTiStatoVerSerie().equals(StatoVersioneSerie.DA_FIRMARE.name())) {
                ...
                SerStatoVerSerie statoVerSerie = serieEjb.createSerStatoVerSerie(
                        currentStato.getPgStatoVerSerie().add(BigDecimal.ONE), StatoVersioneSerie.FIRMA_IN_CORSO.name(),
                ...
                verSerieDaElab.setTiStatoVerSerie(CostantiDB.StatoVersioneSerie.FIRMA_IN_CORSO.name());

                // Insert the document into the signature session
                signHlp.addFile2SessioneFirma(sessionId, verSerie.getIdVerSerie());
            } else {
                logger.warn("Versione serie {} non è nello stato {} ma in {}", verSerie.getIdVerSerie(),
                        StatoVersioneSerie.DA_FIRMARE, currentStato.getTiStatoVerSerie());
            }
        }
    ...
    public byte[] getFile2Sign(long idFile) {
        byte[] result = serieEjb.getSerFileVerSerieBlob(idFile, CostantiDB.TipoFileVerSerie.IX_AIP_UNISINCRO);
        return result;
    }
    ...
    public void errorFile(HsmSessioneFirma session, long idFile, String codError, String descrError) {
        ...
        if (currentStato.getTiStatoVerSerie().equals(CostantiDB.StatoVersioneSerie.FIRMA_IN_CORSO.name())) {
            // Creates the new status DA_FIRMARE that associates to the SerVerSerie
            SerStatoVerSerie statoVerSerie = serieEjb.createSerStatoVerSerie(
                    currentStato.getPgStatoVerSerie().add(BigDecimal.ONE),
                    CostantiDB.StatoVersioneSerie.DA_FIRMARE.name(), "Richiesta firma indice AIP serie fallita", null,
                    session.getIamUser().getIdUserIam(), new Date(), verSerie.getIdVerSerie());
            signHlp.insertEntity(statoVerSerie, false);
            verSerie.setIdStatoVerSerieCor(new BigDecimal(statoVerSerie.getIdStatoVerSerie()));

            SerVerSerieDaElab verSerieDaElab = serieHelper.getSerVerSerieDaElabByIdVerSerie(verSerie.getIdVerSerie());
    ...
    private void signatureSessionInError(HsmSessioneFirma session, SignatureSession.CdErr codError, String descrError) {
        ...
        for (HsmVerSerieSessioneFirma verSerieSess : session.getHsmVerSerieSessioneFirmas()) {
            if (verSerieSess.is2sign()) {
                // Doesn't open a new transaction
                this.errorFile(session, verSerieSess.getSerVerSerie().getIdVerSerie(), codError.name(), descrError);
            }
        }
        session.setTsFine(new Date());
        logger.info("Sessione con id {} andata in errore ({})", session.getIdSessioneFirma(), codError);
    }

...
public class ElencoFascSignatureSessionEjb implements SignatureSessionEjb {
    ...
    private static Logger logger = LoggerFactory.getLogger(ElencoFascSignatureSessionEjb.class.getName());
    ...
    public HsmSessioneFirma startSessioneFirma(SigningRequest request) {
        ...
        for (BigDecimal elencoId : request.getFiles()) {
            ...
            if (statoElenco.getTiStato().equals(TiStatoElencoFasc.CHIUSO)) {
                ...
                logger.warn("Elenco (id: {}) non è nello stato {} ma in {}", elenco.getIdElencoVersFasc(),
                        TiStatoElencoFasc.CHIUSO, statoElenco.getTiStato());
            }
        }
    ...
    public byte[] getFile2Sign(long idFile) {
        byte[] result = elencoEjb.retrieveFileIndiceElenco(idFile, ElencoEnums.FileTypeEnum.INDICE_ELENCO.name());
        return result;
    }
    ...
    private void signatureSessionInError(HsmSessioneFirma session, SignatureSession.CdErr codError, String descrError) {
        ...
        for (HsmElencoFascSesFirma elencoFascSes : session.getHsmElencoFascSesFirmas()) {
            if (elencoFascSes.is2sign()) {
                // Doesn't open a new transaction
                this.errorFile(session, elencoFascSes.getElvElencoVersFasc().getIdElencoVersFasc(), codError.name(),
                        descrError);
            }
        }

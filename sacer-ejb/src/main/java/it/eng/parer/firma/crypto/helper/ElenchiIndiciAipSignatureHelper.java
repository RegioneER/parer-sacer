...
/**
 * This helper read and store the informations about signature session of
 * <code>Elenco Indici AIP</code>
 *
 * @author Bonora_L
 */
@SuppressWarnings({
...
public class ElenchiIndiciAipSignatureHelper extends SigningHelper {
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(ElenchiIndiciAipSignatureHelper.class);
    ...
    public long createSessioneFirma(long userId) {
        ...
        session.setTiSessioneFirma(TiSessioneFirma.ELENCO_INDICI_AIP);
    ...
    public List<HsmSessioneFirma> getBlockedSessionsByUser(long userId) {
        ...
        Query query = getEntityManager()
                .createQuery("SELECT se.idSessioneFirma, MAX(e.tsEsito) AS TsLastOperation "
                        + "FROM HsmSessioneFirma se INNER JOIN se.hsmElencoSessioneFirmas e "
    ...
    /**
     * Returns the {@link HsmElencoSessioneFirma} of {@code session} whose <code>Elenco</code> id is
     * {@code idElenco}
     *

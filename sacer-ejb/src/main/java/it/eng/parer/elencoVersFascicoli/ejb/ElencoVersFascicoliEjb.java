...
public class ElencoVersFascicoliEjb {
	...
	log.info(
		"Elenco Versamento Fascicoli - Creazione automatica elenchi versamento fascicoli...");
	// ricavo le strutture
	...
	for (OrgStrut struttura : strutture) {
	    log.debug("Elenco Versamento Fascicoli - processo struttura: {}",
	...
	log.info("Elenco Versamento Fascicoli - Struttura: id ='{}' nome = '{}'", idStruttura,
		struttura.getNmStrut());
	elaboraElenchiScaduti(idStruttura, logJob.getIdLogJob());
	/*
	 * determino tutti i criteri di raggruppamento fascicoli appartenenti alla struttura
	 * versante corrente, il cui intervallo (data istituzione - data soppressione) includa la
	 * data corrente (con estremi compresi); i criteri sono selezionati in ordine di data
	 * istituzione
	 */
	List<DecCriterioRaggrFasc> criteriRaggrFasc = elencoHelper
		.retrieveCriterioByStrut(struttura, logJob.getDtRegLogJob());
	for (DecCriterioRaggrFasc criterio : criteriRaggrFasc) {
	    ...
	    List<FasFascicoloObj> fasFascicoliObjectList = elencoHelper
		    .retrieveFascicoliToProcess(criterio);
	    log.debug(
		    "Elenco Versamento Fascicoli - Trovati {} fascicoli versati relativi al criterio '{}'",
		    fasFascicoliObjectList.size(), criterio.getNmCriterioRaggr());

	    Collections.sort(fasFascicoliObjectList, comp);

	    ElencoVersFascicoliEjb newElencoEjbRef1 = context
		    .getBusinessObject(ElencoVersFascicoliEjb.class);
		...
		while (i.hasNext()) {
		    ...
		    newElencoEjbRef1.manageFasFascicoloObj(criterio.getIdCriterioRaggrFasc(),
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manageFasFascicoloObj(long idCriterio, long idStruttura, long idLogJob,
	    FasFascicoloObj fasFascicoloObj, boolean isTheLast, boolean isTheFirst)
	    throws Exception {
	if (fasFascicoloObj.getTiEntitaSacer() == Constants.TipoEntitaSacer.FASC) { // fascicolo
	    manageFasFascicolo(fasFascicoloObj.getId(), fasFascicoloObj.getAaFascicolo(),
		    idCriterio, idStruttura, isTheFirst);
	}
    }

    public void manageFasFascicolo(BigDecimal fascicoloId, BigDecimal aaFascicolo, long idCriterio,
	    long idStruttura, boolean isTheFirst) throws Exception {
	...
	if (!annullato) { // fascicolo non annullato
		...
		elenco = createElencoByCriterio(criterio, aaFascicolo, struttura); // questo volume
										   // è managed
		// Aggiugo fascicolo ad elenco solo dopo aver controllato se ci sta. Se non ci sta è
		// un problema di
		...
		if (secondCheckFascOk) { // il fascicolo sta nell'elenco: aggiungo.
		    ...
		    throw new ParerInternalError(
			    "ATTENZIONE non è possibile aggiungere il fascicolo '"
				    + ff.getIdFascicolo()
    ...
    public ElvElencoVersFasc retrieveElenco(DecCriterioRaggrFasc criterio, BigDecimal aaFascicolo,
	    OrgStrut struttura, boolean isTheFirst) {
    ...
    public ElvElencoVersFasc findOpenedElenco(DecCriterioRaggrFasc criterio, BigDecimal aaFascicolo,
	    OrgStrut struttura, boolean isTheFirst) {
	ElvElencoVersFasc elenco = null;
	// Recupero l'elenco aperto per il criterio corrente
	try {
	    elenco = elencoHelper.retrieveElencoByCriterio(criterio, aaFascicolo, struttura); // questo
											      // elenco
											      // è
											      // managed
	    log.debug(
	...
	elenco = createElencoByCriterio(criterio, aaFascicolo, struttura); // questo volume è
									   // managed
	return elenco;
    }

    private ElvElencoVersFasc createElencoByCriterio(DecCriterioRaggrFasc criterio,
	    BigDecimal aaFascicolo, OrgStrut struttura) {
	...
	/*
	 * Registro l'elenco creato nella coda degli elenchi da elaborare
	...
	elencoVersFascDaElab
		.setIdCriterioRaggrFasc(new BigDecimal(criterio.getIdCriterioRaggrFasc()));
	...
	Long idStatoElencoVersFasc = elencoHelper.retrieveStatoElencoByIdElencoVersFascStato(
		elenco.getIdElencoVersFasc(), TiStatoElencoFasc.APERTO).getIdStatoElencoVersFasc();
	...
	// aggiorno l'elenco corrente, incrementando di 1 il numero dei fascicoli versati inclusi
	// nell'elenco
	elenco.setNiFascVersElenco(elenco.getNiFascVersElenco().add(BigDecimal.ONE));
	// registro un nuovo stato pari a IN_ELENCO_APERTO per il fascicolo corrente in
	...
	// aggiorno il fascicolo corrente, assegnando stato = IN_ELENCO_APERTO e valorizzando la FK
	// all'elenco corrente
	...
	log.debug(
		"Elenco Versamento Fascicoli - controllo se ci sono elenchi di versamento fascicoli scaduti");
	...
	for (Long elencoId : elenchiDaProcessare) {
	    log.debug(
		    "Elenco Versamento Fascicoli - trovato elenco {} scaduto da settare stato = DA_CHIUDERE",
		    elencoId);
	...
	(elenco.getElvElencoVersFascDaElabs().get(0))
		.setTiStato(TiStatoElencoFascDaElab.DA_CHIUDERE);
	...
	for (FasFascicolo ff : fasFascicoloList) {
	    // il sistema registra un nuovo stato pari a IN_ELENCO_DA_CHIUDERE per il fascicolo
	    // corrente in
	    // FAS_STATO_FASCICOLO_ELENCO
	    FasStatoFascicoloElenco statoFascicoloElenco = new FasStatoFascicoloElenco();
	    statoFascicoloElenco.setFasFascicolo(ff);
	    ...
	    // il sistema assegna ad ogni fascicolo appartenente all'elenco stato =
	    // IN_ELENCO_DA_CHIUDERE
	    ff.setTiStatoFascElencoVers(TiStatoFascElencoVers.IN_ELENCO_DA_CHIUDERE);
	    log.debug("Elenco Versamento Fascicoli - Assegnato a fascicolo '{}' lo stato {}",
		    ff.getIdFascicolo(), TiStatoFascElencoVers.IN_ELENCO_DA_CHIUDERE.name());
	}
    }
	...
	log.debug("Elenco Versamento Fascicoli - Data di scadenza {}",
	...
	if (ElencoEnums.ExpirationTypeEnum.GIORNALIERA.name().equals(tiScadChiusVolume)) {
	    ...
	    log.debug(LOG_EXPIRATION_DATE, ElencoEnums.ExpirationTypeEnum.GIORNALIERA.name(),
		    newdate);
	}
	if (ElencoEnums.ExpirationTypeEnum.SETTIMANALE.name().equals(tiScadChiusVolume)) {
	    ...
	    cal.set(Calendar.SECOND, 00);
	    String newdate = dateformat.format(cal.getTime());
	    log.debug(LOG_EXPIRATION_DATE, ElencoEnums.ExpirationTypeEnum.SETTIMANALE.name(),
		    newdate);
	}
	if (ElencoEnums.ExpirationTypeEnum.QUINDICINALE.name().equals(tiScadChiusVolume)) {
	    ...
	    String newdate = dateformat.format(cal.getTime());
	    log.debug(LOG_EXPIRATION_DATE, ElencoEnums.ExpirationTypeEnum.QUINDICINALE.name(),
		    newdate);
	}
	...
	if (tiTempo != null && niTempo != null) {
		...
		String newdate = dateformat.format(cal.getTime());
		log.debug(LOG_AUMENTO_TEMPO, tempo, ElencoEnums.TimeTypeEnum.MINUTI.name(),
			newdate);
	    }
		...
		String newdate = dateformat.format(cal.getTime());
		log.debug(LOG_AUMENTO_TEMPO, tempo, ElencoEnums.TimeTypeEnum.GIORNI.name(),
			newdate);
	    }
	}
	return cal.getTime();

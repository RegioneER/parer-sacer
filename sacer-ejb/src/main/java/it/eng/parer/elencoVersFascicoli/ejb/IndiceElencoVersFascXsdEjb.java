...
public class IndiceElencoVersFascXsdEjb {
    ...
    public byte[] createIndex(ElvElencoVersFasc elenco, boolean manualClosing) throws DatatypeConfigurationException {
        // Accedo all’elenco mediante la vista ELV_V_CREA_IX_ELENCO_FASC
        // che fornisce le informazioni per valorizzare i tag EnteProduttore, DescrizioneElencoVersamento e
        ...
        contenutoSinteticoElenco.setNumeroFascicoliVersati(indiceElenco.getNiFascVersElenco().toBigInteger());
        ...
        if (indiceElenco.getLisNmTipoFascicolo() != null) {
            ...
            Collections.addAll(setNmTipoFascicolo, indiceElenco.getLisNmTipoFascicolo().split(delim));
            contenutoSinteticoElenco
                    .setTipologieFascicoloVersate(StringUtils.collectionToDelimitedString(setNmTipoFascicolo, delim));
        }
        ...
        // costruisco tutti i FascicoloType e li aggiungo a fascicoliVersatiAnalitico (FascicoliType)
        ...
        byte[] byteIndice;
        try {
            Marshaller jaxbMarshaller = xmlContextCache.getElencoversamentoFascicoliCtx().createMarshaller();
    ...
    private FascicoloType buildFascicoloVersato(FasFascicolo ff) throws DatatypeConfigurationException {
        // Accedo al contenuto dell’elenco mediante la vista ELV_CREA_LIS_FASC_ELENCO
        // che fornisce l’insieme dei fascicoli contenuti nell’elenco (da ordinare per timestamp di versamento)

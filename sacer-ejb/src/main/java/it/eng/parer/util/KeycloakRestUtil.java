...
public class KeycloakRestUtil {
    ...
    public List<Client> getClients() throws Exception {
        ...
        try {
            ...
            ResponseEntity<List<Client>> res = restTemplate.exchange(builder.toUriString(), HttpMethod.GET, request,
                    new ParameterizedTypeReference<List<Client>>() {
                    });
            List<Client> lista = res.getBody();
            for (Client client : lista) {
                if (client.getName() != null
                        && client.getName().toLowerCase().contains(SUFFISSO_PER_VISUALIZZAZIONE_DA_SACER)) {
                    Client c = new Client();
    ...
    public Client getClientData(Client c) throws Exception {
        ...
        try {
            ...
            UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(urlLocal);
            HttpEntity<String> request = new HttpEntity<>(headers);
            ResponseEntity<Client> res = restTemplate.exchange(builder.toUriString(), HttpMethod.GET, request,

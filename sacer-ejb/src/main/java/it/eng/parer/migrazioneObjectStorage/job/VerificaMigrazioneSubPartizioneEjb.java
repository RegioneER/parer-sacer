...
public class VerificaMigrazioneSubPartizioneEjb {
    ...
    public void verificaMigrazioneSubPartizione() throws ParerInternalError {
        ...
        /* Scrivo nel LogJob */
        if (errori.length() > 0) {
            jobHelper.writeAtomicLogJob(JobConstants.JobEnum.VERIFICA_MIGRAZIONE_SUBPARTIZIONE.name(),
    ...
    public void verificaSubPartizioniInCorsoDiMigrazione() {
        ...
        for (OstMigrazSubPart migrazSubPart : migrazSubPartList) {
            ...
            for (OstMigrazFile migrazFile : migrazFileList) {
                // mofificato dal prof.
                me.gestisciFileInCorsoMigrazione(migrazSubPart.getIdMigrazSubPart(), migrazFile.getIdMigrazFile());
            }
        }
    }
    ...
    public void gestisciFileInCorsoMigrazione(long idMigrazSubPart, long idMigrazFile) {
        ...
        OstMigrazSubPart migrazSubPart = vmspHelper.findByIdWithLock(OstMigrazSubPart.class, idMigrazSubPart);
        ...
        if (migrazFile.getTiStatoCor()
            ...
            migrazSubPart.setNiFileMigrazInCorso(migrazSubPart.getNiFileMigrazInCorso().subtract(BigDecimal.ONE));

            // Recupero lo stato corrente della sub-partizione
            OstStatoMigrazSubPart statoMigrazSubPartCor = vmspHelper.findById(OstStatoMigrazSubPart.class,
                    migrazSubPart.getIdStatoMigrazSubPartCor());
            if (statoMigrazSubPartCor.getTiStato()
                    .equals(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRAZ_IN_CORSO.name())
    ...
    public void registraOstStatoMigrazSubPart(long idMigrazSubPart, Date dataCorrente, String tiStato) {
        OstMigrazSubPart migrazSubPart = vmspHelper.findById(OstMigrazSubPart.class, idMigrazSubPart);
        ...
        migrazSubPart.setIdStatoMigrazSubPartCor(BigDecimal.valueOf(statoMigrazSubPart.getIdStatoMigrazSubPart()));
    }
    ...
    public boolean verificaSubPartizioniInErroreDiMigrazione() {
        ...
        for (OstMigrazSubPart migrazSubPart : migrazSubPartList) {
            ...
            for (OstMigrazFile migrazFile : migrazFileList) {
                // modificato dal prof
                contaFileResettati += me.gestisciFileErroreMigrazione(migrazSubPart.getIdMigrazSubPart(),
    ...
    public int gestisciFileErroreMigrazione(long idMigrazSubPart, long idMigrazFile) {
        ...
        if (migrazFile.getTiStatoCor()
                .equals(it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.MIGRAZ_IN_ERRORE.name())) {
            migrazFile.setTiStatoCor(it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.DA_MIGRARE.name());
            migrazFile.setTsRegStatoCor(new Date());
            migrazFile.setTsMigrato(null);
            migrazSubPart.setNiFileDaMigrare(migrazSubPart.getNiFileDaMigrare().add(BigDecimal.ONE));
            migrazSubPart.setNiFileMigrazInErrore(migrazSubPart.getNiFileMigrazInErrore().subtract(BigDecimal.ONE));
            ...
            if (migrazSubPart.getNiFileMigrazInErrore().compareTo(BigDecimal.ZERO) == 0) {
                ...
                if (migrazSubPart.getNiFileMigrazInCorso().compareTo(BigDecimal.ZERO) > 0
                    ...
                    stato = it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.MIGRAZ_IN_CORSO.name();
                } else {
                    stato = it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.DA_MIGRARE.name();
                }
                // Registro lo stato di migrazione
                registraOstStatoMigrazSubPart(migrazSubPart.getIdMigrazSubPart(), new Date(), stato);
            }
        }
        return count;
    }
    ...
    public void verificaSubPartizioniMigrate() {
        ...
        List<String> tabellaSpazioList = vmspHelper
                .getTableSpaceList(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRATA.name());
        ...
        for (String tabellaSpazio : tabellaSpazioList) {
            ...
            boolean tuttePartizioniTabellaSpazio = vmspHelper.checkAllSubPartitionsWithTableSpaceAndState(tabellaSpazio,
                    it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRATA.name());

            if (tuttePartizioniTabellaSpazio) {
                String tiStato = null;
                // Verifica se tablespace usato solo dalla tabella su cui sono definite le subpartizioni
                if (vmspHelper.usoTbsOK(tabellaSpazio)) {
                    tiStato = it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.DA_ELIMINARE.name();
                } else {
                    tiStato = it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.TBS_NON_ELIMINABILE.name();
                }
                ...
                for (OstMigrazSubPart migrazSubPart : migrazSubPartTableSpace) {
                    registraOstStatoMigrazSubPart(migrazSubPart.getIdMigrazSubPart(), dataCorrente, tiStato);
                }
            }
        }
    }
    ...
    /*
     * Metodo finale che aggiorna i totalizzatori delle subpartizioni. Questa fase era all'interno dei consumer ma
     * succedeva che si accavallavano tutti i lock sulle subpartizioni e si bloccava tutto.
     */
    public void aggiornaTotaliDelleSubpartizioni() {
        ...
        for (OstMigrazSubPart ostMigrazSubPart : lista) {
            me.aggiornaTotaliDellaSubpartizioneInNewTransaction(ostMigrazSubPart.getIdMigrazSubPart());
        }
    }
    ...
    public void aggiornaTotaliDellaSubpartizioneInNewTransaction(long idSubPart) {
        ...
        OstMigrazSubPart ostMigrazSubPart = vmspHelper.findByIdWithLock(OstMigrazSubPart.class, idSubPart);
        ...
        long totMigrazInErrore = vmspHelper.getCountComponentsSubPartAndState(ostMigrazSubPart,
                it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.MIGRAZ_IN_ERRORE.name());
        BigDecimal totDaMigrare = ostMigrazSubPart.getNiFileDaMigrare(); // Si deve prendere dal totalizzatore della
                                                                         // subpartizione non dal conteggio effettivo
                                                                         // sui file!

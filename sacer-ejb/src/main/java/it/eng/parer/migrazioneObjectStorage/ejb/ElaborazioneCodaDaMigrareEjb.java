...
public class ElaborazioneCodaDaMigrareEjb {
    ...
    public void completaSubpartizioniBlob(int numeroJob) {
        List<OstMigrazSubPart> lista = verificaMigrazioneSubPartizioneHelper.getOstMigrazSubPartPerPartizStatoCorrList(
                OrgPartition.TiPartition.BLOB.name(),
                it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.FILE_DA_SCARICARE.name(), numeroJob);
    ...
    public void aggiungiSubpartizioniBlob(int numeroJob) {
        ...
        for (OstVLisSubpartBlobByIstz ostVLisSubpartBlobByIstz : lista) {
            ...
            alStati.add(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.DA_MIGRARE.name());
            alStati.add(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRAZ_IN_CORSO.name());
            alStati.add(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRAZ_IN_ERRORE.name());
            List<Object[]> listaContatori = verificaMigrazioneSubPartizioneHelper.getSommeFileMigrazConStati(alStati);
            long numeroTotale = 0;
            if (listaContatori != null && !listaContatori.isEmpty()) {
                Object[] arr = listaContatori.get(0);
                if (arr != null && arr[0] != null && arr[1] != null && arr[2] != null) {
                    numeroTotale = ((BigDecimal) arr[0]).longValueExact() + ((BigDecimal) arr[1]).longValueExact()
                            + ((BigDecimal) arr[2]).longValueExact();
                }
            }
            if (numeroTotale < numMaxFileDaElab) {
                long idMigrazSubPart = me.registraStatoMigrazSubPartizione(ostVLisSubpartBlobByIstz);
    ...
    public ContatoriPerMigrazioni aggiungiInCodaDaMigrare(int numeroJob) {
        ...
        if (totFileConStatoMigrazioneInCorso < numMaxMessaggiInCodaDaMigrare) {
            ...
            al.add(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRAZ_IN_CORSO.name());
            al.add(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.MIGRAZ_IN_ERRORE.name());
                    ...
                    if (totalizzatore.getFileDaMigrareDefinitivo() > 0) {
                        List<OstMigrazFile> listaFile = verificaMigrazioneSubPartizioneHelper
                                .getOstMigrazFileByStateAndSubPartitionWithLimit(
                                        ...
                                        ostMigrazSubPart, totalizzatore.getFileDaMigrareDefinitivo());
                        ...
                        for (OstMigrazFile ostMigrazFile : listaFile) {
                            ...
                            me.aggiungiInCodaDaMigrareInNewTransaction(ostMigrazFile, totalizzatore, compHashCalc);
                        }
                        if (totalizzatore.getNumFileSelezionatiPerLaPartizione() > 0) {
                            totalizzatore.setIsAddedInQueue(true);
                        }
                        totalizzatore.setFileDaMigrareDefinitivo(totalizzatore.getFileDaMigrareDefinitivo()
    ...
    public void aggiungiInCodaDaMigrareInNewTransaction(OstMigrazFile ostMigrazFilePar,
        ...
        OstMigrazSubPart ostMigrazSubPart = verificaMigrazioneSubPartizioneHelper
                .findByIdWithLock(OstMigrazSubPart.class, ostMigrazFilePar.getOstMigrazSubPart().getIdMigrazSubPart());
        OstMigrazFile ostMigrazFile = verificaMigrazioneSubPartizioneHelper.findByIdWithLock(OstMigrazFile.class,
                ostMigrazFilePar.getIdMigrazFile());
        ...
        if (totalizzatore.getNumFileSelezionatiPerLaPartizione() == 1) {
            OstStatoMigrazSubPart ostStatoMigrazSubPart = verificaMigrazioneSubPartizioneHelper
                    .findById(OstStatoMigrazSubPart.class, ostMigrazSubPart.getIdStatoMigrazSubPartCor());
            if (ostStatoMigrazSubPart != null && ostStatoMigrazSubPart.getTiStato()
                    .equals(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.DA_MIGRARE.name())) {
                ...
                verificaMigrazioneSubPartizioneHelper.getEntityManager().persist(newOstStatoMigrazSubPart);
                verificaMigrazioneSubPartizioneHelper.getEntityManager().flush();
                ostMigrazSubPart
                        .setIdStatoMigrazSubPartCor(new BigDecimal(newOstStatoMigrazSubPart.getIdStatoMigrazSubPart()));
            }
        }
        // Fine nuove modifiche MEV#16016
        ostMigrazFile.setTiStatoCor(it.eng.parer.entity.constraint.OstMigrazFile.TiStatoCor.MIGRAZ_IN_CORSO.name());
        ostMigrazFile.setTsRegStatoCor(new Date());
        ostMigrazSubPart
                .setNiFileMigrazInCorso(new BigDecimal(ostMigrazSubPart.getNiFileMigrazInCorso().longValueExact() + 1));
        ostMigrazSubPart.setNiFileDaMigrare(new BigDecimal(ostMigrazSubPart.getNiFileDaMigrare().longValueExact() - 1));
        ...
        blobInfo.setTipoHashDaCalcolare(compHashCalc.getDsAlgoHashFile().equals(TipiHash.SHA_1.descrivi())
                ? TipiHash.SHA_256.descrivi() : null);
        ...
        blobInfo.setUrn(urn);
        jmsProducerUtilEjb.inviaMessaggioInFormatoJson(connectionFactory, queue, pl, "CodaOggettiDaMigrare");
    }
    ...
    public void aggiungiStrutturaMeseASubPartizione(OstMigrazStrutMese ostMigrazStrutMese) {
        OstMigrazStrutMese orgMese = verificaMigrazioneSubPartizioneHelper.findByIdWithLock(OstMigrazStrutMese.class,
                ostMigrazStrutMese.getIdMigrazStrutMese());
    ...
    private void aggiungiFileASubPartizioneBlob(OstMigrazStrutMese ostMigrazStrutMese) {
        ...
        List<BigDecimal> lista = verificaMigrazioneSubPartizioneHelper.getOstVLisFileBlobIdByStrutMeseBetweenDate(
                ostMigrazStrutMese.getIdStrut(), ostMigrazStrutMese.getDtVersIni(), ostMigrazStrutMese.getDtVersFine());
        log.debug("COMPONENTI ESTRATTI [" + lista.size() + "}");
        for (BigDecimal idComp : lista) {
            me.gestisciComponentePerAggiungiFileASubPartizione(idComp, ostMigrazStrutMese.getOstMigrazSubPart(),
                    nomeStruttura, nomeStrutturaNorm, nomeEnte, nomeEnteNorm, nomeAmbiente);
        }
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void gestisciComponentePerAggiungiFileASubPartizione(BigDecimal idComp, OstMigrazSubPart ostMigrazSubPartPar,
            String nomeStruttura, String nomeStrutturaNorm, String nomeEnte, String nomeEnteNorm, String nomeAmbiente) {
        ...
        if (codErr == null) {
            ...
            if (comp.getTiSupportoComp().equals(it.eng.parer.entity.constraint.AroCompDoc.TiSupportoComp.FILE.name())
                    && ud.getDecTipoUnitaDoc().getTiSaveFile().equals(CostantiDB.TipoSalvataggioFile.BLOB.name())) {
                registraFileDaMigrare(ostMigrazSubPart.getIdMigrazSubPart(), comp.getIdCompDoc(), nomeEnteNorm,
                        nomeStrutturaNorm, ostVLisFileBlobBystrumese.getCdRegistroNormaliz(),
                        ostVLisFileBlobBystrumese.getAaKeyUnitaDoc(), ostVLisFileBlobBystrumese.getIdStrut(),
                        ostVLisFileBlobBystrumese.getDtCreazione());
            } // MEV#17324: Altrimenti qui dovrebbe esserci il punto in cui registriamo il file da non migrare
            else {
                // Determina la causale della non migrazione (punto 3.1.2.1.1.6 registra file da non migrare)
                ...
                } else if (ud.getDecTipoUnitaDoc().getTiSaveFile().equals(CostantiDB.TipoSalvataggioFile.FILE.name())) {
                    tiCausaleNoMigraz = TipoCausaleNoMigraz.FILE_NON_BLOB.name();
                }
                registraFileDaNonMigrare(ostVLisFileBlobBystrumese, ostMigrazSubPart, tiCausaleNoMigraz);
            }
        } else {
            registraFileInErrore(ostMigrazSubPart.getIdMigrazSubPart(), comp.getIdCompDoc(), nomeEnteNorm,
                    nomeStrutturaNorm, ostVLisFileBlobBystrumese.getCdRegistroNormaliz(),
                    ostVLisFileBlobBystrumese.getAaKeyUnitaDoc(), ostVLisFileBlobBystrumese.getIdStrut(),
                    ostVLisFileBlobBystrumese.getDtCreazione());
        }
    ...
    public void aggiornaStatoSubPartizione(OstMigrazSubPart ostMigrazSubPartPar) {
        ...
        verificaMigrazioneSubPartizioneHelper.getEntityManager().flush();
        ostMigrazSubPart.setIdStatoMigrazSubPartCor(BigDecimal.valueOf(stato.getIdStatoMigrazSubPart()));
    }
    ...
    public long registraStatoMigrazSubPartizione(OstVLisSubpartBlobByIstz ostVLisSubpartBlob) {
        OstMigrazSubPart mig = verificaMigrazioneSubPartizioneHelper
                .findOstMigrazSubPartByOrgSubPartitionWithLock(ostVLisSubpartBlob.getIdSubPartition());
        // Registra lo stato della migrazione della subpartizione
        OstStatoMigrazSubPart stato = new OstStatoMigrazSubPart();
        stato.setOstMigrazSubPart(mig);
        stato.setTsRegStato(new Date());
        stato.setTiStato(it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.FILE_DA_SCARICARE.name());
        verificaMigrazioneSubPartizioneHelper.getEntityManager().persist(stato);
        verificaMigrazioneSubPartizioneHelper.getEntityManager().flush();
    ...
    private void registraFileDaMigrare(long idMigrazSubPart, long idCompDoc, String nmEnteNorm, String nmStrutNorm,
            String cdRegistroNorm, BigDecimal anno, BigDecimal idStrut, Date dtCreazione) {
        OstMigrazSubPart migrazSubPart = verificaMigrazioneSubPartizioneHelper.findById(OstMigrazSubPart.class,
                idMigrazSubPart);
        AroCompDoc compDoc = verificaMigrazioneSubPartizioneHelper.findById(AroCompDoc.class, idCompDoc);
    ...
    private void registraFileDaNonMigrare(OstVLisFileBlobBystrumese ostVLisFileBlobBystrumese,
        ...
        noMigraz.setMmFile(Utils.getNumericAnnoMeseFromDate(ostVLisFileBlobBystrumese.getDtCreazione()));
    ...
    private void registraFileInErrore(long idMigrazSubPart, long idCompDoc, String nmEnteNorm, String nmStrutNorm,
            String cdRegistroNorm, BigDecimal anno, BigDecimal idStrut, Date dtCreazione) {
        OstMigrazSubPart migrazSubPart = verificaMigrazioneSubPartizioneHelper.findById(OstMigrazSubPart.class,
                idMigrazSubPart);
        AroCompDoc compDoc = verificaMigrazioneSubPartizioneHelper.findById(AroCompDoc.class, idCompDoc);
        migrazSubPart.setNiFileErroreNormaliz(migrazSubPart.getNiFileErroreNormaliz().add(BigDecimal.ONE));
        migrazSubPart.setNiByteSize(migrazSubPart.getNiByteSize()
                .add(compDoc.getNiSizeFileCalc() != null ? compDoc.getNiSizeFileCalc() : BigDecimal.ZERO));

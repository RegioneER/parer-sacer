...
public class VerificaMigrazioneSubPartizioniHelper extends GenericHelper {
    ...
    /*
     * Determina la lista delle migrazioni delle sub partizioni con stato corrente DA_ELIMINARE ordinate su anno-mese
     * massimo presente nella sub partizione (asc) + numero di file migrati (desc).
     */
    public List<Object[]> getOstMigrazSubPartDaEliminareListOrdered() {
        ...
        Query query = entityManager.createQuery(queryStr);
        query.setParameter("tiStato", it.eng.parer.entity.constraint.OstStatoMigrazSubPart.TiStato.DA_ELIMINARE.name());
        return query.getResultList();
    }

    /**
     * Determina i file della sub partizione con stato tiStatoCor da un numero di giorni maggiore di quanto definito dal
     * parametro passato in input
     *
     * @param idMigrazSubPart
     *            id migrazione sub partizione
     * @param tiStatoCor
     *            tipo stato
    ...
    public List<OstMigrazFile> getOstMigrazFileBeforeNumGiorni(long idMigrazSubPart, String tiStatoCor,
        ...
        String queryStr = "SELECT migrazFile FROM OstMigrazFile migrazFile "
                ...
                + "WHERE migrazSubPart.idMigrazSubPart = :idMigrazSubPart " + "AND migrazFile.tiStatoCor = :tiStatoCor "
                + "AND migrazFile.tsRegStatoCor < :c ";
        Query query = entityManager.createQuery(queryStr);
    ...
    public List<OstMigrazFile> getOstMigrazFileByStateAndSubPartitionWithLimit(String stato,
        ...
        String queryStr = "SELECT migrazFile FROM OstMigrazFile migrazFile " + "WHERE migrazFile.tiStatoCor = :stato "
    ...
    public List<OstMigrazSubPart> getOstMigrazSubPartPerPartizStatoCorrList(String tipoPartizione,
        ...
        String queryStr = "SELECT migraz FROM OstMigrazSubPart migraz, OstStatoMigrazSubPart stato "
                ...
                + "WHERE migraz.idStatoMigrazSubPartCor=stato.idStatoMigrazSubPart " + "AND stato.tiStato=:tiStato "
                + "AND part.tiPartition=:tiPartition " + "AND migraz.niIstanzaJobPrepara=:numeroJob "
                + "ORDER BY migraz.mmMax ASC";
    ...
    /**
     * Verifica che TUTTE le partizioni che usano il tableSpace passato in input abbiano lo stato passato in input
     *
     * @param nmTablespace
     *            nome table space
     * @param tiStato
    ...
    public boolean checkAllSubPartitionsWithTableSpaceAndState(String nmTablespace, String tiStato) {
        // Recupero il numero di sub-partizioni che usano la tabella spazio e hanno stato diverso da MIGRATA
        String queryStr2 = "SELECT migrazSubPart FROM OstStatoMigrazSubPart statoMigrazSubPart "
                + "JOIN statoMigrazSubPart.ostMigrazSubPart migrazSubPart "
                + "WHERE statoMigrazSubPart.tiStato != :tiStato " + "AND migrazSubPart.nmTablespace = :nmTablespace "
                + "AND statoMigrazSubPart.idStatoMigrazSubPart = migrazSubPart.idStatoMigrazSubPartCor ";
    ...
    public List<OstMigrazStrutMese> getOstMigrazStrutMeseByOrgSubPartitionFlag(OstMigrazSubPart ostMigrazSubPart,
            boolean flagFileAggiunti) {
        String queryString = "SELECT migraz "
                ...
                + "AND migraz.ostMigrazSubPart=:ostMigrazSubPart " + "AND migraz.flFileAggiunti=:flagFileAggiunti "
    ...
    public List<OstMigrazSubPart> getOstMigrazSubPartBySubPartition(OrgSubPartition orgSubPartition) {
    ...
    public List<OstMigrazSubPart> getSubPartitionsMigrazInCorsoErroreMoreThanZero() {
        String queryString = "SELECT  sub " + "FROM    OstMigrazSubPart sub, OstStatoMigrazSubPart stato "
                + "WHERE   sub.idStatoMigrazSubPartCor = stato.idStatoMigrazSubPart "
                + "AND     stato.tiStato IN ( :stato1, :stato2 ) " + "AND     sub.niFileMigrazInCorso > 0 ";
        Query query = entityManager.createQuery(queryString);
    ...
    /*
     * Torna il numero di componenti con lo stato passato come parametro della subpartizione passata.
     */
    ...
    public long getCountComponentsSubPartAndState(OstMigrazSubPart ostMigrazSubPart, List<String> tiStato) {
        long ret = 0;
     ...
     */
    public List<BigDecimal> findIdAroContenutoCompByCodiceIdentificativo(String codiceIdentificativo) {
        // Il codice identificativo non deve essere passato come stringa (delimitata dagli apici)ma come costante ORACLE
        String qString = "select id_conten_comp " + "from aro_contenuto_comp subpartition (" + codiceIdentificativo
     ...
     * Query nativa per estrarre gli id dei componenti
     */
    public BigDecimal countAroContenutoCompByCodiceIdentificativo(String codiceIdentificativoPartizione) {
        BigDecimal ret = null;
        // Il codice identificativo non deve essere passato come stringa (delimitata dagli apici)ma come costante ORACLE
    ...
    public OstMigrazSubPart findOstMigrazSubPartByOrgSubPartitionWithLock(BigDecimal idSubPartition) {

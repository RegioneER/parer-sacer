...
public class ElenchiVersFascicoliEjb {
	...
	ElvElencoVersFasc elenco = evfWebHelper.findById(ElvElencoVersFasc.class,
	...
	List<ElvFileElencoVersFasc> elencoIndiceAip = evfHelper
		.retrieveFileIndiceElenco(idElencoVersFasc.longValue(), new String[] {
	...
	try {
	    ...
	    List<ElvFileElencoVersFasc> elencoIndiceAipFirmato = evfHelper
		    .retrieveFileIndiceElenco(idElencoVersFasc.longValue(), new String[] {
	    ...
	    elencoRowBean.setString("nm_criterio_raggr",
		    elenco.getDecCriterioRaggrFasc().getNmCriterioRaggr());
	    elencoRowBean.setString("ds_criterio_raggr",
		    elenco.getDecCriterioRaggrFasc().getDsCriterioRaggr());
	    ...
	    elencoRowBean.setBigDecimal("num_fasc",
		    elenco.getNiFascVersElenco().add(elenco.getNiFascVersElenco()));
		...
		elencoRowBean.setString("cd_ver_xsd_file",
    ...
    public ElvStatoElencoVersFascTableBean getElvStatiElencoVersFascTableBean(
	    BigDecimal idElencoVersFasc) throws EMFError {
	...
	try {
	    ...
	    throw new EMFError(
		    "Errore nel recupero della lista degli stati assunti dall'elenco di versamento fascicoli",
		    e);
	}
	return statiElencoVersFascTableBean;
    }

    public ElvStatoElencoVersFascRowBean getElvStatoElencoVersFascRowBean(
	    BigDecimal idStatoElencoVersFasc) {
	ElvStatoElencoVersFasc statoElenco = evfWebHelper.findById(ElvStatoElencoVersFasc.class,
		idStatoElencoVersFasc.longValue());
	ElvStatoElencoVersFascRowBean statoElencoRowBean = new ElvStatoElencoVersFascRowBean();
	try {
	    statoElencoRowBean = (ElvStatoElencoVersFascRowBean) Transform
		    .entity2RowBean(statoElenco);
    ...
    public DecCriterioRaggrFascRowBean getDecCriterioRaggrFascRowBean(
	    BigDecimal idCriterioRaggrFasc) throws EMFError {
	...
	try {
	    ...
	    log.error("Errore durante il recupero del criterio di raggruppamento fascicolo "
		    + ExceptionUtils.getRootCauseMessage(ex), ex);
	    throw new EMFError(
		    "Errore durante il recupero del criterio di raggruppamento fascicolo", ex);
	}
	return criterioRowBean;
    }

    public ElvVRicElencoFascTableBean getElvVRicElencoFascTableBean(long idUserIam,
	    FiltriElenchiVersFascicoli filtri) throws EMFError {
	List<ElvVRicElencoFasc> listaElenchiVersFascicoli = evfWebHelper
		.retrieveElvVRicElencoFascList(idUserIam, filtri);
	...
	try {
		...
		for (ElvVRicElencoFasc elencoVersFasc : listaElenchiVersFascicoli) {
		    ...
		    elenchiVersFascRowBean.setString("nm_ente_nm_strut",
			    elenchiVersFascRowBean.getNmEnte() + " - "
				    + elenchiVersFascRowBean.getNmStrut());
	    ...
	    log.error(e.getMessage(), e);
	    throw new EMFError(
		    "Errore nel recupero della lista degli elenchi di versamento fascicoli", e);
	}
    ...
    public ElvVRicElencoFascByStatoTableBean getElenchiVersFascicoliDaFirmareTableBean(
	    ...
	    Date[] dateCreazioneElencoValidate, long idUserIam) throws EMFError {
	List<ElvVRicElencoFascByStato> listaElenchiVersFascicoli = evfWebHelper
		.getListaElenchiVersFascicoliDaFirmare(idAmbiente, idEnte, idStrut,
			idElencoVersFasc, note, tiStato, dateCreazioneElencoValidate, idUserIam);
	...
	try {
		...
		for (ElvVRicElencoFascByStato elenco : listaElenchiVersFascicoli) {
		    ...
		    elenchiVersFascicoliRowBean = (ElvVRicElencoFascByStatoRowBean) Transform
	    ...
	    log.error(e.getMessage(), e);
	    throw new EMFError(
		    "Errore nel recupero della lista degli elenchi di versamento fascicoli con stato "
			    + tiStato.name(),
		    e);
	}
	...
	/*
	 * Elimino lo storico delle sessioni di firma in errore per l'elenco di versamento fascicoli
	 * (tabella HSM_ELENCO_FASC_SES_FIRMA)
	 */
	List<HsmElencoFascSesFirma> listElencoFascSesFirmaHsm = evfHelper
		.retrieveListaElencoInError(elenco, TiEsitoFirmaElencoFasc.IN_ERRORE);
    ...
    /**
     * Esegue la chiusura manuale dell'elenco di versamento fascicoli passato come parametro in
     * input.
     *
     * @param idUserIam        id user Iam
     * @param idElencoVersFasc id elenco versamento fascicolo
     * @param modifica         lista modifiche di tipo OpTypeEnum
     * @param note             valore
     *
     ...
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manualClosingElenco(long idUserIam, BigDecimal idElencoVersFasc,
	...
	if (!modifica.isEmpty()) {
	    evfWebHelper.saveNote(idUserIam, idElencoVersFasc, note, elenco.getNtElencoChiuso(),
		    modifica);
	}
	...
	/* Creo l'indice */
	byte[] indexFile = null;
	log.info("Creazione indice per elenco di versamento fascicoli avente id '"
		+ elenco.getIdElencoVersFasc());
	...
	String urnXmlIndice = buildUrnIndiceElencoVersFascicoli(elenco, nomeStruttura, null,
		nomeEnte, null);
	// Costruisco l'urn normalizzato dell'indice
	String urnXmlIndiceNormaliz = buildUrnIndiceElencoVersFascicoli(elenco, null,
		nomeStrutturaNorm, null, nomeEnteNorm);
	// Registro nella tabella ElvFileElencoVersFasc:
	// Setto l'hash dell'indice, l'algoritmo usato per il calcolo hash (=SHA-256),
	// l'encoding del hash (=hexBinary), la versione del XSD (=1.0) con cui è creato l'indice
	// dell'elenco e
	// l'urn dell’indice “urn:<sistemaconservazione>:<ente>:<struttura>:ElencoVers-FA-<data
	// creazione>-<id
	// elenco>:Indice”
	evfHelper.storeFileIntoElenco(elenco, indexFile,
		ElencoEnums.FileTypeEnum.INDICE_ELENCO.name(), new Date(), hashXmlIndice,
		TipiHash.SHA_256.descrivi(), TipiEncBinari.HEX_BINARY.descrivi(), urnXmlIndice,
	...
	Long idStatoElencoVersFasc = evfHelper.getStatoElencoByIdElencoVersFascStato(
	...
	try {
	    Class entityClass = (rowBeanClass.getSimpleName().substring(0,
		    rowBeanClass.getSimpleName().length() - 4)).getClass();
    ...
    /**
     * Data una lista di elenchi di versamento fascicoli ed una dataChiusura impostati restituisce
     * true o false a seconda che tutti gli elenchi non presenti nella lista passata, con data
     * chiusura inferiore a quella passata, siano firmati.
     *
     * @param elencoTableBean entity bean ElvVRicElencoFascByStato
     * @param dataChiusura    data chiusura
     * @param idStrut         id struttura
     *
     * @return true/false
     */
    public boolean areAllElenchiNonPresentiFirmati(
	    ElvVRicElencoFascByStatoTableBean elencoTableBean, Date dataChiusura,
	    BigDecimal idStrut) {
	...
	return evfWebHelper.areAllElenchiNonPresentiFirmati(idElencoVersFascSelezionatiList,
		dataChiusura, idStrut);
    }

    /**
     * Controlla se su DB esiste già, per quella struttura, un elenco che abbia l'id passato come
     * parametro
     *
	...
	evfWebHelper.saveNote(idUserIam, idElencoVersFasc, ntIndiceElenco, ntElencoChiuso,
		operList);
	// ElvElencoVersFasc elenco = evfWebHelper.findById(ElvElencoVersFasc.class,
	// idElencoVersFasc.longValue());
    }
     ...
     *
     * @param idElencoVersFasc id elenco versamento fascicolo
     * @param idFascToRemove   id fascicolo da rimuovere
     * @param idUserIam        id user Iam
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void deleteFascFromElencoVersFascicoli(Long idElencoVersFasc,
	    Set<BigDecimal> idFascToRemove, long idUserIam) {
	...
	for (BigDecimal idFf : idFascToRemove) {
		...
		if (fasFascicolo.getDtAnnull().getTime() == fineDelMondo.getTime().getTime()) {
		    ...
		    statoFascicoloElenco.setFasFascicolo(fasFascicolo);
		    statoFascicoloElenco.setTsStato(new Date());
		    statoFascicoloElenco
			    .setTiStatoFascElencoVers(TiStatoFascElenco.IN_ATTESA_SCHED);
		...
		/* Ricalcolo i valori */
		elenco.setNiFascVersElenco(
			new BigDecimal(evfHelper.contaFascVersati(idElencoVersFasc)));
	...
	if (elenco.getFasFascicoli().isEmpty()) {
	    // TODO: verificare, evHelper.writeLogElencoVers(elenco, elenco.getOrgStrut(),
	    // idUserIam,
	    // ElencoEnums.OpTypeEnum.ELIMINA_ELENCO.name());
    ...
    public ElvFileElencoVersFasc retrieveElvFileElencoVersFasc(long idElencoVersFasc,
    ...
    // fascicoli
    public void streamOutFileIndiceElencoNoFirma(ZipOutputStream out, String fileNamePrefix,
	    String fileNameSuffix, long idElencoVersFasc)
	...
	addEntryToZip(out, dati,
		fileNamePrefix + fileNameSuffix + FileTypeEnum.INDICE_ELENCO.getFileExtension());
	out.flush();
    }
    ...
    public void streamOutFileIndiceElenco(ZipOutputStream out, String fileNamePrefix,
	    ...
	    throws IOException {
	List<ElvFileElencoVersFasc> retrieveFileIndiceElenco = evfHelper.retrieveFileIndiceElenco(
		idElencoVersFasc, FileTypeEnum.getStringEnumsList(fileTypes));
	for (ElvFileElencoVersFasc elvFileElencoVersFasc : retrieveFileIndiceElenco) {
	    FileTypeEnum fileType = ElencoEnums.FileTypeEnum
		    .valueOf(elvFileElencoVersFasc.getTiFileElencoVers());
	    fileNamePrefix = StringUtils.defaultString(fileNamePrefix);
	    fileNameSuffix = StringUtils.defaultString(fileNameSuffix);
		...
		blFileElencoVers = objectStorageService.getObjectElencoIndiciAipFasc(
			elvFileElencoVersFasc.getIdFileElencoVersFasc());
	    }
	    // end MEV#30399
    ...
    public void storeFirma(Long idElencoVersFasc, byte[] fileFirmato, Date signatureDate,
	    long idUtente, it.eng.parer.elencoVersamento.utils.ElencoEnums.TipoFirma tipoFirma)
	    throws NoSuchAlgorithmException, IOException {
	ElvElencoVersFasc elenco = evfHelper.retrieveElencoById(idElencoVersFasc);
	// calcolo l'hash SHA-256 del file .p7m
	String hashXmlIndice = new HashCalculator().calculateHashSHAX(fileFirmato, TipiHash.SHA_256)
		.toHexBinary();
	// determina nome ente e struttura normalizzati e non
	OrgStrut struttura = elenco.getOrgStrut();
	OrgEnte ente = struttura.getOrgEnte();
	...
	String nomeEnteNorm = ente.getCdEnteNormaliz();
	// Costruisco l'urn originale dell'indice
	String urnXmlIndice = buildUrnIndiceElencoVersFascicoliFirmati(elenco, nomeStruttura, null,
		nomeEnte, null);
	// Costruisco l'urn normalizzato dell'indice
	String urnXmlIndiceNormaliz = buildUrnIndiceElencoVersFascicoliFirmati(elenco, null,
		nomeStrutturaNorm, null, nomeEnteNorm);
	...
	evfHelper.storeFileIntoElenco(elenco, fileFirmato,
		ElencoEnums.FileTypeEnum.FIRMA_INDICE_ELENCO.name(), signatureDate, hashXmlIndice,
		TipiHash.SHA_256.descrivi(), TipiEncBinari.HEX_BINARY.descrivi(), urnXmlIndice,
		urnXmlIndiceNormaliz, ElencoEnums.ElencoInfo.VERSIONE_ELENCO.message(), tipoFirma);
	...
	/* Aggiorno l’elenco specificando l’identificatore dello stato corrente */
	Long idStatoElencoVersFasc = evfHelper.getStatoElencoByIdElencoVersFascStato(
		elenco.getIdElencoVersFasc(), TiStatoElencoFasc.FIRMATO).getIdStatoElencoVersFasc();
	elenco.setIdStatoElencoVersFascCor(new BigDecimal(idStatoElencoVersFasc));
	...
	ElvElencoVersFascDaElab elencoDaElab = evfHelper
		.getElvElencoVersFascDaElabByIdElencoVersFasc(idElencoVersFasc);
	...
	Long idStatoElencoVersFasc = evfHelper
		.getStatoElencoByIdElencoVersFascStato(elenco.getIdElencoVersFasc(),
			TiStatoElencoFasc.VALIDATO)
     ...
     *
     * @param idElencoVersFasc id elenco versamento fascicolo
     * @param fileFirmato      array di byte file firmato
     ...
     */
    public ElvFileElencoVersFasc storeFirmaElencoIndiceAipFasc(Long idElencoVersFasc,
	    byte[] fileFirmato, Date signatureDate, long idUtente, BackendStorage backendMetadata,
	...
	// salvo ORIGINALE
	final String urnElencoIndiceAIPFirmato = MessaggiWSFormat
		.formattaUrnElencoIndiciAIPFascicoliFirmati(
			MessaggiWSFormat.formattaUrnPartVersatore(versatore),
			Long.toString(idElencoVersFasc), sdf.format(elenco.getTsCreazioneElenco()));
	// salvo NORMALIZZATO
	final String urnNormalizElencoIndiceAIPFirmato = MessaggiWSFormat
		.formattaUrnElencoIndiciAIPFascicoliFirmati(
	...
	fileElencoVersFasc
		.setTiFileElencoVers(ElencoEnums.FileTypeEnum.FIRMA_ELENCO_INDICI_AIP.name());
	// MEV#30399
	...
	// Determino i fascicoli appartenenti all'elenco con stato =
	...
	for (FasFascicolo fasFascicolo : fasFascicoliFiltered) {
		...
		// Registra un nuovo stato di conservazione del fascicolo specificando stato =
		// IN_ARCHIVIO e user quello
		// corrente
    ...
    public void manageElencoFascAnnulDaFirmaElencoFasc(BigDecimal idElencoVersFasc,
	    long idUserIam) {
	ElvElencoVersFasc elencoVersFasc = evfHelper.findById(ElvElencoVersFasc.class,
		idElencoVersFasc);

	List<FasFascicolo> fascicoloElencoList = evfHelper
		.getFasFascicoloVersatiElenco(idElencoVersFasc);
	...
	for (FasFascicolo fascicoloElenco : fascicoloElencoList) {
		...
		// Registro il fascicolo nella coda dei fascicoli da elaborare (tabella
		// ELV_FASC_DA_ELAB_ELENCO)
		evfHelper.insertFascCodaFascDaElab(fascicoloElenco.getIdFascicolo(),
			TiStatoFascDaElab.IN_ATTESA_SCHED);
	    }
	}

	// Elimino l'elenco di versamento fascicoli dalla coda degli elenchi in elaborazione
	ElvElencoVersFascDaElab elencoVersFascDaElab = evfHelper
		.retrieveElencoInQueue(elencoVersFasc);
	 ...
	 */
	List<HsmElencoFascSesFirma> listElencoFascSesFirmaHsm = evfHelper
		.retrieveListaElencoInError(elencoVersFasc, TiEsitoFirmaElencoFasc.IN_ERRORE);
	for (HsmElencoFascSesFirma elencoFascSesFirmaHsm : listElencoFascSesFirmaHsm) {
	    evfHelper.removeEntity(elencoFascSesFirmaHsm, true);
	}
	...
	ElvVChkSoloFascAnnul soloFascAnnul = evfHelper.findViewById(ElvVChkSoloFascAnnul.class,
		idElencoVersFasc);
	return soloFascAnnul.getFlSoloFascAnnul() != null
		&& soloFascAnnul.getFlSoloFascAnnul().equals("1");
    }
    ...
    public void manageElencoFascAnnulDaFirmaElencoIndiciAipFasc(BigDecimal idElencoVersFasc,
	    long idUserIam) {
	...
	ElvFileElencoVersFasc fileElencoVersFasc = evfHelper.getFileIndiceElenco(
		idElencoVersFasc.longValue(), FileTypeEnum.ELENCO_INDICI_AIP.name());
    ...
    public String buildUrnIndiceElencoVersFascicoliFirmati(ElvElencoVersFasc elenco,
	    String nomeStruttura, String nomeStrutturaNorm, String nomeEnte, String nomeEnteNorm) {
	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
	String sistema = configurationHelper
		.getValoreParamApplicByApplic(CostantiDB.ParametroAppl.NM_SISTEMACONSERVAZIONE);
	String ente = (nomeEnte == null) ? nomeEnteNorm : nomeEnte;
	String struttura = (nomeStruttura == null) ? nomeStrutturaNorm : nomeStruttura;
	return MessaggiWSFormat.formattaUrnIndiceElencoVersFascicoliFirmati(sistema, ente,
		struttura, sdf.format(elenco.getTsCreazioneElenco()),
		String.format("%d", elenco.getIdElencoVersFasc()));
    }

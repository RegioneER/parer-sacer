...
public class FascicoliEjb {
    ...
    public FasVRicFascicoliTableBean ricercaFascicoli(RicercaFascicoliBean bean,
            BigDecimal idStruct, long userId) {
        ...
        for (Object[] obj : list) {
            ...
            row.setFlUpdAnnulUnitaDoc(((Character) obj[16]).toString());
            row.setFlUpdModifUnitaDoc(((Character) obj[17]).toString());
    ...
    public FasVRicFascicoliTableBean ricercaFascicoliAnnullati(RicercaFascicoliBean bean,
            BigDecimal idStruct, long userId) {
        FasVRicFascicoliTableBean result = new FasVRicFascicoliTableBean();
        List<FasVRicFascicoli> list = fascicoliHelper.retrieveFascicoliAnnullati(bean, idStruct,
                userId);

        if (list != null && !list.isEmpty()) {
            try {
                for (FasVRicFascicoli fasc : list) {
                    FasVRicFascicoliRowBean rowBean = (FasVRicFascicoliRowBean) Transform
                            .entity2RowBean(fasc);

                    FasFascicolo ff = fascicoliHelper.findById(FasFascicolo.class,
                            fasc.getIdFascicolo());
                ...
                logger.error("Errore durante il recupero dei fascicoli annullati"
    ...
    public FasVVisFascicoloRowBean retrieveFasVVisFascicolo(BigDecimal idFascicolo) {
        ...
        FasVVisFascicolo dettaglio = fascicoliHelper
                .retrieveFasVVisFascicolo(idFascicolo.longValue());
        ...
        if (dettaglio != null) {
            try {
                ...
                if (StringUtils.isNotBlank(result.getCdIndiceClassif())
                    ...
                    // Regola: tutti i caratteri / di posizione pari diventano - tutti i dispari
                    ...
                    result.setString("indice_classif", result.getCdIndiceClassif() + "    "
                            + myString.substring(1, myString.length()));
                }

                if (StringUtils.isNotBlank(result.getCdRegKeyUnitaDocFirst())
                        && result.getAaKeyUnitaDocFirst() != null
                        && StringUtils.isNotBlank(result.getCdKeyUnitaDocFirst())) {
                    result.setString("unita_doc_first",
                            result.getCdRegKeyUnitaDocFirst() + " - "
                                    + result.getAaKeyUnitaDocFirst() + " - "
                                    + result.getCdKeyUnitaDocFirst());
                }

                if (StringUtils.isNotBlank(result.getCdRegKeyUnitaDocLast())
                        && result.getAaKeyUnitaDocLast() != null
                        && StringUtils.isNotBlank(result.getCdKeyUnitaDocLast())) {
                    result.setString("unita_doc_last",
                            result.getCdRegKeyUnitaDocLast() + " - " + result.getAaKeyUnitaDocLast()
                                    + " - " + result.getCdKeyUnitaDocLast());
                }

                if (StringUtils.isNotBlank(result.getCdIpaAmminTitol())
                        && StringUtils.isNotBlank(result.getDsAmminTitol())) {
                    result.setString("ammin_titol",
                            result.getCdIpaAmminTitol() + " - " + result.getDsAmminTitol());
                }

                if (StringUtils.isNotBlank(result.getCdProcAmmin())
                        && StringUtils.isNotBlank(result.getDsProcAmmin())) {
                    result.setString("proc_ammin",
                            result.getCdProcAmmin() + " - " + result.getDsProcAmmin());
                }
                ...
                // end MEV#29090
                    ...
                    | IllegalAccessException | InvocationTargetException ex) {
                logger.error("Errore durante il recupero dei fascicoli "
                        + ExceptionUtils.getRootCauseMessage(ex), ex);
            }
        }
        return result;
    ...
    // MEV#29090
    /**
     * Nel caso in cui il backend di salvataggio degli XML di versamento del fascicolo sia l'object
     ...
     *
     * @param riga FasVVisFascicoloRowBean
     */
    ...
    /**
     * Nel caso in cui il backend di salvataggio degli XML di profilo del fascicolo sia l'object
     * storage (gestito dal parametro <strong>applicativo</strong>) si possono verificare 2 casi:
     * <ul>
     * <li>gli xml sono <em>ancora</em> sul DB perché non ancora migrati</li>
     * <li>gli xml sono effettivamente sull'object storage</li>
    ...
    private void addXmlFascFromOStoFasVVisFascicoloBean(FasVVisFascicoloRowBean riga) {
        ...
        if (xmlVuoti) {
            Map<String, String> xmls = objectStorageService
                    .getObjectXmlFascicolo(riga.getIdFascicolo().longValue());
            // recupero oggetti da O.S. (se presenti)
            if (!xmls.isEmpty()) {
                riga.setBlXmlVersProfilo(
                        xmls.get(CostantiDB.TiModelloXsdProfilo.PROFILO_GENERALE_FASCICOLO.name()));
                riga.setBlXmlNormativo(xmls
                        .get(CostantiDB.TiModelloXsdProfilo.PROFILO_NORMATIVO_FASCICOLO.name()));
                riga.setBlXmlSegnatura(xmls
                        .get(CostantiDB.TiModelloXsdProfilo.PROFILO_ARCHIVISTICO_FASCICOLO.name()));
                riga.setBlXmlSpecifico(xmls
                        .get(CostantiDB.TiModelloXsdProfilo.PROFILO_SPECIFICO_FASCICOLO.name()));
            }
        }
    }
    // end MEV#29090
    ...
    public FasVRicFascicoliRowBean retrieveFasVRicFascicoli(BigDecimal idFascicolo) {
        ...
        FasVRicFascicoli dettaglio = fascicoliHelper
                .retrieveFasVRicFascicoli(idFascicolo.longValue());
    ...
    public BaseRow retrieveFasFileMetaVerAipFasc(BigDecimal idFascicolo, String tiMeta) {
        ...
        FasFileMetaVerAipFasc result = fascicoliHelper
                .getFasFileMetaVerAipFasc(idFascicolo.longValue(), tiMeta);
        if (result != null) {
            row = new BaseRow();
            if (it.eng.parer.entity.constraint.FasMetaVerAipFascicolo.TiMeta.valueOf(tiMeta)
                ...
                row.setString("ds_urn_aip_fascicolo",
                        result.getFasMetaVerAipFascicolo().getDsUrnMetaFascicolo());
                ...
                row.setString("ds_urn_file_fascicolo",
                        result.getFasMetaVerAipFascicolo().getDsUrnMetaFascicolo());
                row.setString("ds_urn_normaliz_file_fascicolo",
                        result.getFasMetaVerAipFascicolo().getDsUrnNormalizMetaFascicolo());
            }
            row.setString("ds_hash_file", result.getFasMetaVerAipFascicolo().getDsHashFile());
            row.setString("ds_algo_hash_file",
                    result.getFasMetaVerAipFascicolo().getDsAlgoHashFile());
            row.setString("cd_encoding_hash_file",
                    result.getFasMetaVerAipFascicolo().getCdEncodingHashFile());
            ...
            if (result.getFasMetaVerAipFascicolo().getFasVerAipFascicolo()
                    .getIdEnteConserv() != null) {
                row.setString("nm_ente_conserv",
                        fascicoliHelper
                                .findById(SIOrgEnteSiam.class, result.getFasMetaVerAipFascicolo()
                                        .getFasVerAipFascicolo().getIdEnteConserv())
                                .getNmEnteSiam());
            }
        }
    ...
    public void setIndiceAipFascByOS(BigDecimal idFascicolo, BaseRow fileMetaIndiceAipDetail) {
        ...
        fileMetaIndiceAipDetail.setBigDecimal("id_ver_aip_fascicolo",
                BigDecimal.valueOf(idVerAipFascicolo));
        addXmlIndiceAipFascFromOStoFasFileMetaVerAipFascBean(fileMetaIndiceAipDetail);
    }

    /**
     * Nel caso in cui il backend di salvataggio degli XML indice AIP fascicolo sia l'object storage
     ...
     * Se si avvera il secondo caso li devo recuperare
     *
     * @param fileMetaVerAipFascDetail BaseRow rappresentante FileMetaVerAipFasc
     */
    public void addXmlIndiceAipFascFromOStoFasFileMetaVerAipFascBean(
            BaseRow fileMetaVerAipFascDetail) {
    ...
    public long getIdMetaVerAipFascicolo(BigDecimal idFascicolo) {
        FasMetaVerAipFascicolo meta = fascicoliHelper.getFasMetaVerAipFascicolo(
                idFascicolo.longValue(),
    ...
    public FasVLisUdInFascTableBean retrieveFasVLisUdInFasc(BigDecimal idFascicolo, long userId) {
        ...
        List<FasVLisUdInFasc> list = fascicoliHelper
                .retrieveFasVLisUdInFasc(idFascicolo.longValue(), userId);

        if (list != null && !list.isEmpty()) {
            try {
                for (FasVLisUdInFasc l : list) {
                    FasVLisUdInFascRowBean rowBean = (FasVLisUdInFascRowBean) Transform
                            .entity2RowBean(l);
                    String stato = rowBean.getTiStatoConservazione();

                    if (StringUtils.isNotBlank(stato)) {
                        if (it.eng.parer.viewEntity.constants.FasVLisUdInFasc.TiStatoConservazione.ANNULLATA
                                .name().equals(stato)) {
    ...
    public FasSogFascicoloTableBean retrieveFasSogFascicolo(BigDecimal idFascicolo) {
        ...
        List<FasSogFascicolo> list = fascicoliHelper
                .retrieveFasSogFascicolo(idFascicolo.longValue());
        ...
        for (FasSogFascicolo sogFascicolo : list) {
            try {
                ...
                if (sogFascicolo.getDsDenomSog() != null) {
                    ...
                    row.setString("denominazione",
                            sogFascicolo.getNmNomeSog() + " " + sogFascicolo.getNmCognSog());
                }
                row.setString("cd_sog", fascicoliHelper
                        .getFasCodIdeSog(sogFascicolo.getIdSogFascicolo(), "NON_PREDEFINITO"));
                row.setString("ti_cd_sog", fascicoliHelper
                        .getFasCodIdeSog(sogFascicolo.getIdSogFascicolo(), "PREDEFINITO"));
                row.setString("evento_soggetto",
                        fascicoliHelper.getEventiSoggetto(sogFascicolo.getIdSogFascicolo()));
                result.add(row);
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
                    | IllegalAccessException | InvocationTargetException ex) {
                logger.error("Errore durante il recupero dei fascicoli "
    ...
    public FasEventoFascicoloTableBean retrieveFasEventoFascicolo(BigDecimal idFascicolo) {
        ...
        List<FasEventoFascicolo> list = fascicoliHelper
                .retrieveFasEventoFascicolo(idFascicolo.longValue());
        ...
        for (FasEventoFascicolo eventoFascicolo : list) {
            try {
                ...
                if (eventoFascicolo.getTsApertura() != null) {
                    Date dataAperturaFormattata = formatter
                            .parse(formatter.format(eventoFascicolo.getTsApertura()));
                    row.setTsApertura(new Timestamp(dataAperturaFormattata.getTime()));
                }
                if (eventoFascicolo.getTsChiusura() != null) {
                    Date dataChiusuraFormattata = formatter
                            .parse(formatter.format(eventoFascicolo.getTsChiusura()));
    ...
    public FasValoreAttribFascicoloTableBean retrieveFasValoreAttribFascicolo(
            BigDecimal idFascicolo, BigDecimal idModelloXsdFasc) {
        ...
        if (idModelloXsdFasc != null) {
            List<FasValoreAttribFascicolo> list = fascicoliHelper.retrieveFasValoreAttribFascicolo(
                    idFascicolo.longValue(), idModelloXsdFasc.longValue());

            for (FasValoreAttribFascicolo valore : list) {
                try {
                    riga = (FasValoreAttribFascicoloRowBean) Transform.entity2RowBean(valore);
                    riga.setString("nm_attrib_dati_spec",
                            valore.getDecAttribFascicolo().getNmAttribFascicolo());
                    result.add(riga);
                } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
                        | IllegalAccessException | InvocationTargetException ex) {
    ...
    public FasRespFascicoloTableBean retrieveFasRespFascicolo(BigDecimal idFascicolo) {
        ...
        List<FasRespFascicolo> list = fascicoliHelper
                .retrieveFasRespFascicolo(idFascicolo.longValue());
        if (list != null && !list.isEmpty()) {
    ...
    public FasUniOrgRespFascicoloTableBean retrieveFasUniOrgRespFascicolo(BigDecimal idFascicolo) {
        ...
        List<FasUniOrgRespFascicolo> list = fascicoliHelper
                .retrieveFasUniOrgRespFascicolo(idFascicolo.longValue());
        if (list != null && !list.isEmpty()) {
            try {
    ...
    public FasLinkFascicoloTableBean retrieveFasLinkFascicolo(BigDecimal idFascicolo) {
        ...
        List<FasLinkFascicolo> list = fascicoliHelper
                .retrieveFasLinkFascicolo(idFascicolo.longValue());
        List<FasLinkFascicolo> listparent = fascicoliHelper
                .retrieveFasLinkFascicoloParent(idFascicolo.longValue());
        try {
    ...
    public ElvVRicElencoFascByFasTableBean retrieveFasElvFascicolo(BigDecimal idFascicolo) {
        ...
        List<ElvVRicElencoFascByFas> list = fascicoliHelper
                .retrieveFasElvFascicolo(idFascicolo.longValue());

        if (list != null && !list.isEmpty()) {
            try {
                for (ElvVRicElencoFascByFas l : list) {
                    ElvVRicElencoFascByFasRowBean rowBean = (ElvVRicElencoFascByFasRowBean) Transform
                            .entity2RowBean(l);
                    rowBean.setString("nm_ente_nm_strut",
                            rowBean.getNmEnte() + " - " + rowBean.getNmStrut());
                    result.add(rowBean);
                }
            } catch (ClassNotFoundException | NoSuchMethodException | InstantiationException
                    | IllegalAccessException | InvocationTargetException ex) {
        ...
        if (list != null && !list.isEmpty()) {
            try {
                result = (FasFascicoloTableBean) Transform.entities2TableBean(list);
                    ...
                    | IllegalAccessException | InvocationTargetException ex) {
                logger.error("Errore durante il recupero dei sottofascicoli "
                        + ExceptionUtils.getRootCauseMessage(ex), ex);
            }
        }
        return result;
    ...
    private void buildParentLinks(FasLinkFascicoloTableBean result,
            List<FasLinkFascicolo> listparent) throws ClassNotFoundException, NoSuchMethodException,
        ...
        if (listparent != null && !listparent.isEmpty()) {
            ...
            for (FasLinkFascicolo ent : listparent) {
                ...
                FasFascicolo fasFascicolo = fascicoliHelper.getEntityManager()
                        .find(FasFascicolo.class, rowBean.getIdFascicolo().longValue());
    ...
    public MonVChkCntFascRowBean calcolaTotFascicoliVersati(MonVChkCntFascRowBean rowBean,
            BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
    ...
    public MonVChkCntFascRowBean calcolaTotFascicoliFalliti(MonVChkCntFascRowBean rowBean,
            BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo) {
    ...
    public MonVChkCntFascRowBean calcolaRiepilogo(BigDecimal idUser, BigDecimal idAmbiente,
            BigDecimal idEnte, BigDecimal idStrut, BigDecimal idTipoFascicolo) {
        ...
        if (idTipoFascicolo != null) {
            ...
            throw new RuntimeException(
                    "Errore parametri nella chiamata al metodo calcolaRiepilogo() dell'ejb!");
        }
    ...
    private String getTiStatoFascFromCntFasc(Object obj) {
        ...
        if (obj instanceof MonVCntFascKoByTiFasc) {
            ...
            throw new RuntimeException(
                    "Tipo di istanza non prevista per il metodo getTiStatoFascFromCntFasc()");
        }
    }
    ...
    private String getTiDtCreazioneFromCntFasc(Object obj) {
        ...
        if (obj instanceof MonVCntFascKoByTiFasc) {
            ...
            throw new RuntimeException(
                    "Tipo di istanza non prevista per il metodo getTiDtCreazioneFromCntFasc()");
        }
    }
    ...
    public MonVLisFascTableBean ricercaFascicoliPerMonitoraggio(BigDecimal idUser,
            BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoIndiceAip,
            Set<String> statiConservazione, String flSessioneFascicoloKo) {
        CriteriaQuery criteriaQuery = fascicoliHelper.retrieveVLisFascCriteriaQuery(idUser,
                idAmbiente, idEnte, idStrut, idTipoFascicolo, dateValidate, rangeAnnoDa, rangeAnnoA,
                rangeNumeroDa, rangeNumeroA, statoIndiceAip, statiConservazione,
                flSessioneFascicoloKo);
        return lazyListHelper.getTableBean(criteriaQuery,
                this::getMonVLisFascTableBeanFromResultList);
    }
    ...
    private MonVLisFascTableBean getMonVLisFascTableBeanFromResultList(List l) {
        ...
        if (l != null && !l.isEmpty()) {
            ...
            for (Iterator iterator = l.iterator(); iterator.hasNext();) {
                ...
                rb.setString("nm_ente_struttura",
                        rb.getString("nm_ente") + " / " + rb.getString("nm_strut"));
                t.add(rb);
            }
        }
        return t;
    ...
    public MonVLisFascKoTableBean ricercaFascicoliKoPerMonitoraggio(BigDecimal idUser,
            BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoSessione,
            String cdClasseErr, String cdErr) {
        return ricercaFascicoliKoPerMonitoraggioInternal(idUser, idAmbiente, idEnte, idStrut,
                idTipoFascicolo, dateValidate, rangeAnnoDa, rangeAnnoA, rangeNumeroDa, rangeNumeroA,
                statoSessione, cdClasseErr, cdErr);
    }

    public MonVLisFascKoTableBean ricercaFascicoliKoPerMonitoraggioErr(BigDecimal idUser,
            BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoSessione,
            String cdClasseErr, String cdErr) {
    ...
    private MonVLisFascKoTableBean ricercaFascicoliKoPerMonitoraggioInternal(BigDecimal idUser,
            BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoSessione,
            String cdClasseErr, String cdErr) {
        CriteriaQuery query = fascicoliHelper.retrieveVLisFascKoCriteriaQuery(idUser, idAmbiente,
    ...
    private VrsSesFascicoloKoTableBean getVrsSesFascicoloKoTableBeanFrom(
            List<VrsSesFascicoloKo> list) {
    ...
    public DettaglioVersamentoFascicoloKo getDettaglioVersamentoFascicoloKo(
            BigDecimal idSesFascicoloKo) {
        ...
        if (ses != null) {
            ...
            if (l != null && !l.isEmpty()) {
                ...
                for (Iterator<VrsXmlSesFascicoloKo> iterator = l.iterator(); iterator.hasNext();) {
                    ...
                    if (xml.getTiXml().equals(tipoXmlSessioneFascicoloErrata.RICHIESTA.name())) {
                        ...
                        dett.setString("bl_xml_sip",
                                valXml == null ? null : formatter.prettyPrintWithDOM3LS(valXml));
                    } else if (xml.getTiXml()
                            .equals(tipoXmlSessioneFascicoloErrata.RISPOSTA.name())) {
                        String valXml = xml.getBlXml();
            ...
            if (lerr != null && !lerr.isEmpty()) {
                ...
                for (Iterator<VrsErrSesFascicoloKo> iterator = lerr.iterator(); iterator
                        .hasNext();) {
    ...
    // MEV#29090
    /**
     * Nel caso in cui il backend di salvataggio degli XML di versamento fascicolo fallito sia
     * l'object storage (gestito dal parametro <strong>applicativo</strong>) si possono verificare 2
     * casi:
     * <ul>
     * <li>gli xml sono <em>ancora</em> sul DB perché non ancora migrati</li>
     * <li>gli xml sono effettivamente sull'object storage</li>
     ...
     * Se si avvera il secondo caso li devo recuperare
     *
     * @param riga DettaglioVersamentoFascicoloKo
     */
    private void aggiungiXmlSesFascFallitaDaObjectStorage(DettaglioVersamentoFascicoloKo riga) {
         ...
         * Se gli xml non sono ancora stati migrati, però, sono ancora presenti sulle tabelle
         */
        if (riga.getDettaglioVersamentoRB().getBigDecimal("id_ses_fascicolo_ko") != null
                && xmlSesFascKoVuoti) {
            Map<String, String> xmls = objectStorageService.getObjectSipFascFallito(riga
                    .getDettaglioVersamentoRB().getBigDecimal("id_ses_fascicolo_ko").longValue());
            // recupero oggetti se presenti su O.S
            if (!xmls.isEmpty()) {
                XmlPrettyPrintFormatter formatter = new XmlPrettyPrintFormatter();
                riga.getDettaglioVersamentoRB().setString("bl_xml_sip", formatter
    ...
    /**
     * Nel caso in cui il backend di salvataggio degli XML di versamento fascicolo errato sia
     * l'object storage (gestito dal parametro <strong>applicativo</strong>) si possono verificare 2
     * casi:
     * <ul>
     * <li>gli xml sono <em>ancora</em> sul DB perché non ancora migrati</li>
    ...
    private void aggiungiXmlSesFascErrataDaObjectStorage(BaseRow riga) {
        boolean xmlSesFascErrVuoti = riga.getString("bl_xml_sip") == null
                && riga.getString("bl_xml_rapp_vers") == null;
        /*
         * Se gli xml non sono ancora stati migrati, però, sono ancora presenti sulle tabelle
         */
        if (riga.getBigDecimal("id_ses_fascicolo_err") != null && xmlSesFascErrVuoti) {
            ...
            if (!xmls.isEmpty()) {
                XmlPrettyPrintFormatter formatter = new XmlPrettyPrintFormatter();
                riga.setString("bl_xml_sip", formatter
                        .prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RICHIESTA)));
    ...
    public BaseTable ricercaSessFascErrate(Date dataDa, Date dataA, String statoSessione,
            String cdClasseErr, String cdErr) {
        CriteriaQuery query = fascicoliHelper.createSessFascErrateCriteriaQuery(dataDa, dataA,
                statoSessione, cdClasseErr, cdErr);
    ...
    private BaseTable getBaseTableFromVrsSesFascicoloErr(List<VrsSesFascicoloErr> l) {
        ...
        if (l != null && !l.isEmpty()) {
            for (Iterator<VrsSesFascicoloErr> iterator = l.iterator(); iterator.hasNext();) {
                ...
                r.setString("calc_fascicolo",
                        (ogg.getAaFascicolo() != null ? ogg.getAaFascicolo() : "")
                                + (ogg.getCdKeyFascicolo() != null
                                        ? (" / " + ogg.getCdKeyFascicolo())
                                        : ""));
    ...
    /*
     * Salva i dati di dettaglio della sessione fascicolo errato eventualmente modificati E verifica
     * se si può trasformare una Sessione errata in un versamento fallito
     */
    public SalvaDettaSessErrDto salvaDettaglioSessioneFascicoloErr(BigDecimal id,
            BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStrut,
            BigDecimal idTipoFascicolo, BigDecimal annoFascicolo, String numFascicolo) {
        ...
        // SE TUTTI I CONTROLLI HANNO SUCCESSO SI TRASFORMA UNA SESSIONE ERRATA IN VERSAMENTO
        // FALLITO
        ...
        if (amb != null && ente != null && str != null && tip != null && annoFascicolo != null
            ...
            List<FasFascicolo> fascicoli = fascicoliHelper.retrieveFasFascicoloByStrutAnnoNumValid(
                    str, annoFascicolo.longValueExact(), numFascicolo);
            ...
            if (fascicoli != null && !fascicoli.isEmpty()) {
                ...
                List<VrsFascicoloKo> fasKo = fascicoliHelper.retrieveFasNonVersatoByStrutAnnoNum(
                        str, annoFascicolo.longValueExact(), numFascicolo, true); // RICHIEDE LOCK
                // PESSIMISTICO
                ...
                if (fasKo != null && !fasKo.isEmpty()) {
                    ...
                    // Aggiorna contatore
                    // Punto b) III
                    List<MonContaFascicoliKo> cntKo = fascicoliHelper
                            .retrieveMonContaFascicoliKoByChiaveTotaliz(fko.getTsIniLastSes(), str,
                                    fko.getTiStatoFascicoloKo(),
                                    fko.getAaFascicolo().longValueExact(),
                                    fko.getDecTipoFascicolo(), true); // ASSSUME IL
                    // LOCK
                    // PESSIMISTICO
                    ...
                    if (listaVrs != null && !listaVrs.isEmpty()) {
                        ...
                        fko.setDecTipoFascicolo(fascicoliHelper.findById(DecTipoFascicolo.class,
                                vrsUpd.getIdTipoFascicoloLast()));
                    ...
                    // Punto b) VI
                    List<LogVVisLastSched> logVs = monitoraggioHelper.getLogVVisLastSched(
                    ...
                    if (logVs != null && !logVs.isEmpty()
                        ...
                        List<MonContaFascicoliKo> cntKo2 = fascicoliHelper
                                .retrieveMonContaFascicoliKoByChiaveTotaliz(fko.getTsIniLastSes(),
                                        str, fko.getTiStatoFascicoloKo(),
                                        fko.getAaFascicolo().longValueExact(),
                                        fko.getDecTipoFascicolo(), true); // ASSSUME IL
                        // LOCK
                        // PESSIMISTICO
                    ...
                    // Punto c) III
                    List<MonContaFascicoliKo> cntKo3 = fascicoliHelper
                            .retrieveMonContaFascicoliKoByChiaveTotaliz(vrsFasKo.getTsIniLastSes(),
                                    vrsFasKo.getOrgStrut(), vrsFasKo.getTiStatoFascicoloKo(),
                                    vrsFasKo.getAaFascicolo().longValueExact(),
                                    vrsFasKo.getDecTipoFascicolo(), true); // ASSSUME
                    // IL
                    // LOCK
                    // PESSIMISTICO
            ...
            if (xmls != null) {
                for (Iterator<VrsXmlSesFascicoloErr> iterator = xmls.iterator(); iterator
                        .hasNext();) {
    ...
    public BaseTable ricercaCriteriRaggrFascicoli(
        ...
        List<DecCriterioRaggrFasc> l = fascicoliHelper
                .retrieveCriteriRaggrFascicoli(filtriCriteriRaggrFasc, filterValid);
        ...
        if (l != null && !l.isEmpty()) {
            for (Iterator<DecCriterioRaggrFasc> iterator = l.iterator(); iterator.hasNext();) {
                ...
                r.setBigDecimal("id_criterio_raggr_fasc",
                        new BigDecimal(ogg.getIdCriterioRaggrFasc()));
                r.setString("nm_ente_nm_strut", ogg.getOrgStrut().getOrgEnte().getNmEnte() + " - "
                        + ogg.getOrgStrut().getNmStrut());
                ...
                r.setString("nm_tipo_fascicolo", populateValueField(
                        new BigDecimal(ogg.getIdCriterioRaggrFasc()), "TIPO_FASCICOLO"));
                ...
                r.setString("cd_composito_voce_titol", populateValueField(
                        new BigDecimal(ogg.getIdCriterioRaggrFasc()), "VOCE_TITOL"));
    ...
    public BaseTable ricercaSelCriterioRaggrFascicoli(BigDecimal idCriterioRaggrFasc,
            String tiSel) {
        BaseTable t = new BaseTable();
        BaseRow r = null;

        List<DecSelCriterioRaggrFasc> l = fascicoliHelper
                .retrieveSelCriterioRaggrFascicoli(idCriterioRaggrFasc, tiSel);

        if (l != null && !l.isEmpty()) {
            for (Iterator<DecSelCriterioRaggrFasc> iterator = l.iterator(); iterator.hasNext();) {
                DecSelCriterioRaggrFasc ogg = iterator.next();
                r = new BaseRow();
                r.setBigDecimal("id_sel_criterio_raggr_fasc",
                        new BigDecimal(ogg.getIdSelCriterioRaggrFasc()));
                r.setBigDecimal("id_criterio_raggr_fasc",
                ...
                if (ogg.getDecVoceTitol() != null) {
                    r.setBigDecimal("id_voce_titol",
                            new BigDecimal(ogg.getDecVoceTitol().getIdVoceTitol()));
                }
                t.add(r);
            }
        }
    ...
    public BaseRow getDettaglioCriterioRaggrFascicolo(BigDecimal idCriterioRaggrFasc) {
        ...
        DecCriterioRaggrFasc decCriterioRaggrFasc = fascicoliHelper
                .getDecCriterioRaggrFascById(idCriterioRaggrFasc);

        if (decCriterioRaggrFasc != null) {
            r = new BaseRow();
            r.setBigDecimal("id_criterio_raggr_fasc",
                    new BigDecimal(decCriterioRaggrFasc.getIdCriterioRaggrFasc()));
            r.setBigDecimal("id_strut",
                    new BigDecimal(decCriterioRaggrFasc.getOrgStrut().getIdStrut()));
            ...
            r.setString("fl_criterio_raggr_standard",
                    decCriterioRaggrFasc.getFlCriterioRaggrStandard());
            ...
            r.setTimestamp("dt_istituz",
                    new Timestamp(decCriterioRaggrFasc.getDtIstituz().getTime()));
            r.setTimestamp("dt_soppres",
                    new Timestamp(decCriterioRaggrFasc.getDtSoppres().getTime()));
            r.setString("fl_filtro_tipo_fascicolo",
                    decCriterioRaggrFasc.getFlFiltroTipoFascicolo());
            r.setString("fl_filtro_voce_titol", decCriterioRaggrFasc.getFlFiltroVoceTitol());
            r.setString("fl_filtro_sistema_migraz",
                    decCriterioRaggrFasc.getFlFiltroSistemaMigraz());
    ...
    public BaseRow getDecCriterioRaggrFascRowBean(BigDecimal idStrut, String nmCriterioRaggr) {
        ...
        DecCriterioRaggrFasc crit = fascicoliHelper.getDecCriterioRaggrFasc(idStrut,
                nmCriterioRaggr);

        if (crit != null) {
            row = new BaseRow();
            row.setBigDecimal("id_criterio_raggr_fasc",
                    new BigDecimal(crit.getIdCriterioRaggrFasc()));
    ...
    public long saveCriterioRaggrFascicoli(LogParam param, CreaCriterioRaggrFascicoli filtri,
            Object[] anniFascicoliValidati, String nome, String criterioStandard,
            List<BigDecimal> voceTitolList) throws EMFError {
        return fascicoliHelper.saveCritRaggrFasc(param, filtri, anniFascicoliValidati,
                filtri.getId_strut().parse(), nome, criterioStandard, voceTitolList);
    }

    public boolean deleteDecCriterioRaggrFascicoli(LogParam param, BigDecimal idStrut,
            String nmCriterioRaggr) throws ParerUserError {
    ...
    public boolean isCriterioRaggrFascStandard(BigDecimal idStrut, BigDecimal aaFascicolo,
            BigDecimal aaFascicoloDa, BigDecimal aaFascicoloA, BigDecimal niTempoScadChius,
            String tiTempoScadChius, Set<String> tipiFascicolo, List<BigDecimal> voceTitolList,
            BigDecimal niMaxFasc) {
        int numeroCondizioniSoddisfatte = 0;
        /*
         * ATTENZIONE: IL NUMERO DI CONDIZIONI NECESSARIE DIPENDE DA QUANTE CONDIZIONI DEVONO ESSERE
         * SODDISFATTE ALL'INTERNO DEL METODO
         */
        int numeroCondizioniNecessarie = 4;

        // 1) Controllo che sia presente un solo filtro per tipiFascicolo O un solo indice di
        // classificazione O sia un
        ...
        // 3) Controlla che la scadenza chiusura sia impostata a un numero di giorni pari a quelli
        // indicati nel
        // parametro NI_GG_SCAD_CRITERIO in ORG_STRUT_CONFIG_FASCICOLO
        if (niTempoScadChius != null
                && niTempoScadChius
                        .compareTo(new BigDecimal(getNumGgScadCriterioFascStd(idStrut))) == 0
                && tiTempoScadChius != null && tiTempoScadChius.equals("GIORNI")) {
            numeroCondizioniSoddisfatte++;
        }

        // 4) Controlla che il numero massimo di fascicoli sia pari al valore del parametro
        // NUM_FASC_CRITERIO_STD
        if (niMaxFasc != null
                && niMaxFasc.compareTo(new BigDecimal(getNumFascCriterioStd(idStrut))) == 0) {
            numeroCondizioniSoddisfatte++;
        }
    ...
    public DecTipoFascicoloTableBean getTipiFascicoloAbilitati(long idUtente,
            BigDecimal idStruttura) {
        DecTipoFascicoloTableBean table = new DecTipoFascicoloTableBean();
        List<DecTipoFascicolo> list = fascicoliHelper.getTipiFascicoloAbilitati(idUtente,
                idStruttura);
        if (list != null && !list.isEmpty()) {
            try {
    ...
    public DecCriterioRaggrFascRowBean getDecCriterioRaggrFascRowBean(
            BigDecimal idCriterioRaggrFasc, BigDecimal idStrut) {
        return getDecCriterioRaggrFasc(idCriterioRaggrFasc, null, idStrut);
    }

    private DecCriterioRaggrFascRowBean getDecCriterioRaggrFasc(BigDecimal idCriterioRaggrFasc,
            String nmCriterioRaggr, BigDecimal idStrut) {
        ...
        if (nmCriterioRaggr == null && idCriterioRaggrFasc != BigDecimal.ZERO) {
            criterioRaggrFasc = fascicoliHelper.findById(DecCriterioRaggrFasc.class,
                    idCriterioRaggrFasc);
        }

        if (criterioRaggrFasc != null) {
            try {
                criterioRaggrDocRowBean = (DecCriterioRaggrFascRowBean) Transform
     ...
     * @return DecModelloXsdFascicoloTableBean
     */
    public DecModelloXsdFascicoloTableBean getDecModelloXsdFascicoloTableBeanByTipoFascicolo(
     ...
     *
     * @param idTipoFascicolo       id tipo fascicolo
     * @param idModelloXsdFascicolo id modello fascicolo
     *
     * @return DecModelloXsdFascicoloTableBean
     */

...
public class FascicoliHelper extends GenericHelper {
     ...
     */
    public List<Object[]> retrieveFascicoli(RicercaFascicoliBean filtri, BigDecimal idStrut, long userId) {
     ...
     *
     * @param filtri
     *            filtri per la ricerca dei fascicoli
     * @param idStrut
     *            id della struttura
     * @param userId
     *            id dell'utente
    ...
    public List<FasEventoFascicolo> retrieveFasEventoFascicolo(long idFascicolo) {
        ...
        try {
            Query query = getEntityManager().createNamedQuery("FasEventoFascicolo.findEventiFascicolo",
                    FasEventoFascicolo.class);
            query.setParameter("idFascicolo", idFascicolo);
            result = query.getResultList();
        } catch (RuntimeException ex) {
            logger.error("Errore nell'estrazione delle FasEventoFascicolo per il fascicolo [{}]", idFascicolo, ex);
            throw ex;
        }
        return result;
    }
    ...
    public List<FasValoreAttribFascicolo> retrieveFasValoreAttribFascicolo(long idFascicolo,
        ...
        try {
            Query query = getEntityManager().createQuery("SELECT valore FROM FasValoreAttribFascicolo valore "
                    + "JOIN valore.decUsoModelloXsdFasc uso " + "JOIN uso.decModelloXsdFascicolo modello "
            ...
            logger.error("Errore nell'estrazione delle FasValoreAttribFascicolo per il fascicolo [{}]", idFascicolo,
                    ex);
            throw ex;
        }
        return result;
    ...
    public List<FasEventoSog> retrieveFasEventoSog(long idSogFascicolo) {
        ...
        try {
            Query query = getEntityManager().createNamedQuery("FasEventoSog.findEventiSoggetto", FasEventoSog.class);
            query.setParameter("idSogFascicolo", idSogFascicolo);
            result = query.getResultList();
        } catch (RuntimeException ex) {
            logger.error("Errore nell'estrazione delle FasEventoSog per il soggetto fascicolo [{}]", idSogFascicolo,
                    ex);
            throw ex;
        }
        return result;
    ...
    public String getEventiSoggetto(long idSogFascicolo) {
        ...
        try {
            ...
            logger.error("Errore nell'estrazione delle stringhe FasEventoSog per il soggetto fascicolo [{}]",
                    idSogFascicolo, ex);
            throw ex;
        }
    }
    ...
    public List<ElvVRicElencoFascByFas> retrieveFasElvFascicolo(long idFascicolo) {
        ...
        try {
            String queryStr = "SELECT DISTINCT new it.eng.parer.viewEntity.ElvVRicElencoFascByFas(u.id.idElencoVersFasc, u.tiStato, u.aaFascicoloElenco, u.niFascVersElenco, "
                    ...
                    + "WHERE u.id.idFascicolo = :idFascicolo "
    ...
    public List<DecCriterioRaggrFasc> retrieveCriteriRaggrFascicoli(
            ...
            throws EMFError {
        return retrieveCriteriRaggrFascicoli(filterValid, new FiltriCriteriRaggrFascicoliPlain(filtriCriteriRaggrFasc));
    }

    /**
     *
     * @param idCriterioRaggrFasc
     *            id criterio di raggruppamento fascicoli
     * @param tiSel
     *            tipo sel
     *
    ...
    public List<FasFascicolo> getSottofascicoli(long idFascicolo) {
        ...
        try {
            ...
            logger.error("Errore nell'estrazione dei sottofascicoli per il fascicolo [{}]", idFascicolo, ex);
            throw ex;
        }
        return result;
    }
    ...
    public Long saveCritRaggrFasc(LogParam param, CreaCriterioRaggrFascicoli filtri, Object[] anniFascicoliValidati,
        ...
        if (filtri.getNm_tipo_fascicolo().parse() != null && !filtri.getNm_tipo_fascicolo().parse().isEmpty()) {
        ...
        if (voceTitolList != null && !voceTitolList.isEmpty()) {
            ...
            if (!lista.isEmpty()) {
                ...
                if (nome != null) {
                    ...
                    // risulterebbero più presenti nella lista
                    // Eseguo perciò una bulk delete sui record non presenti nella lista
                    deleteDecSelCriterioRaggrFascNotInVoceTitolo(voceTitolList, decCriterioRaggrFasc);
                }
            }
        } else {
            // Se sono in modifica, potrei avere eliminato tutti i filtri che avevo creato precedentemente
        ...
        try {
            ...
            sacerLogEjb.log(param.getTransactionLogContext(), param.getNomeApplicazione(), param.getNomeUtente(),
                    ...
                    new BigDecimal(decCriterioRaggrFasc.getIdCriterioRaggrFasc()), param.getNomePagina());
    ...
    public boolean deleteCritRaggrFasc(LogParam param, BigDecimal idStrut, String nmCriterioRaggr)
        ...
        DecCriterioRaggrFasc row = getDecCriterioRaggrFascByStrutAndCriterio(idStrut, nmCriterioRaggr);
    ...
    public List<BigDecimal> retrieveFasVLisFascByRichann(long idRichAnnulVers) {
        Query query = getEntityManager().createQuery("SELECT fas.id.idFascicolo FROM FasVLisFascByRichannUd fas "
    ...
    public CriteriaQuery<VrsSesFascicoloErr> createSessFascErrateCriteriaQuery(Date dataDa, Date dataA,
    ...
    public List<FasVRicFascicoli> retrieveFascicoli_old(BigDecimal idStrut, long userId,
        ...
        if (StringUtils.isNotBlank(filtri.getCdKeyFascicoloDa())
            ...
            cdKeyFascicoloA = StringPadding.padString(filtri.getCdKeyFascicoloA(), "0", 12, StringPadding.PADDING_LEFT);
            whereClauseStr.append(andClause)
                    .append("LPAD(f.cdKeyFascicolo, 12, '0') BETWEEN :cdKeyFascicolo_da AND :cdKeyFascicolo_a ");
        }
        ...
        if (StringUtils.isNotBlank(filtri.getCdKeyFascicoloPadreDa())
            ...
            cdKeyFascicoloPadreDa = StringPadding.padString(filtri.getCdKeyFascicoloPadreDa(), "0", 12,
                    StringPadding.PADDING_LEFT);
            cdKeyFascicoloPadreA = StringPadding.padString(filtri.getCdKeyFascicoloPadreA(), "0", 12,
                    StringPadding.PADDING_LEFT);
        ...
        if (StringUtils.isNotBlank(filtri.getCdKeyUnitaDocDa()) && StringUtils.isNotBlank(filtri.getCdKeyUnitaDocA())) {
            ...
            cdKeyUnitaDocA = StringPadding.padString(filtri.getCdKeyFascicoloPadreA(), "0", 12,
                    StringPadding.PADDING_LEFT);
            whereClauseStr.append(andClause)
                    .append("LPAD( aud.cdKeyUnitaDoc, 12, '0') BETWEEN :cdKeyUnitaDoc_da AND :cdKeyUnitaDoc_a ");
            udFilter = true;
        }
        if (udFilter) {
        ...
        if (StringUtils.isNotBlank(filtri.getDsOggettoFascicolo())) {
            query.setParameter("dsOggettoFascicolo", "%" + filtri.getDsOggettoFascicolo().toUpperCase() + "%");
        }
        if (filtri.getDtApeFascioloDa() != null && filtri.getDtApeFascioloA() != null) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdLivelloRiserv())) {
            query.setParameter("cdLivelloRiserv", "%" + filtri.getCdLivelloRiserv().toUpperCase() + "%");
        }
        if (StringUtils.isNotBlank(filtri.getNmSistemaVersante())) {
            query.setParameter("nmSistemaVersante", "%" + filtri.getNmSistemaVersante().toUpperCase() + "%");
        }
        if (StringUtils.isNotBlank(filtri.getNmUserid())) {
            query.setParameter("nmUserid", "%" + filtri.getNmUserid().toUpperCase() + "%");
        }
        if (StringUtils.isNotBlank(filtri.getCdCompositoVoceTitol())) {
            query.setParameter("cdCompositoVoceTitol", "%" + filtri.getCdCompositoVoceTitol().toUpperCase() + "%");
        }
        if (filtri.getAaFascicoloPadre() != null) {
        ...
        if (StringUtils.isNotBlank(cdKeyFascicoloPadreDa) && StringUtils.isNotBlank(cdKeyFascicoloPadreA)) {
    ...
    /**
     * Ricerca dei fascicoli in stato diverso da ANNULLATO filtrandoli per Struttura di appartenenza e abilitazioni
     ...
     * @return Lista di {@link FasVRicFascicoli}
     */
    public List<Object[]> retrieveFascicoli(BigDecimal idStrut, long userId, FiltriRicercaFascicoli filtri) {
        List<Object[]> result = null;
        String whereWord = "WHERE ";
        ...
        StringBuilder queryStr = new StringBuilder("SELECT DISTINCT f.id_Fascicolo, "
                ...
                + "f.fl_Forza_Contr_Numero, f.fl_Forza_Contr_Colleg, f.ti_Stato_Fasc_Elenco_Vers, f.ti_Stato_Conservazione FROM Fm_Ric_Fasc(:idStrut, :userId) f ");
        ...
        if (filtri.getAaFascicoloDa() != null && filtri.getAaFascicoloA() != null) {
            whereString.append(whereWord).append("f.aa_Fascicolo BETWEEN :aaFascicolo_da AND :aaFascicolo_a ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getCdKeyFascicolo())) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdKeyFascicoloDa())
            ...
            whereString.append(whereWord)
                    .append("LPAD(f.cd_Key_Fascicolo, 12, '0') BETWEEN :cdKeyFascicolo_da AND :cdKeyFascicolo_a ");
            whereWord = " AND ";
        }
        if (filtri.getNmTipoFascicolo() != null) {
        ...
        if (StringUtils.isNotBlank(filtri.getDsOggettoFascicolo())) {
            whereString.append(whereWord).append("UPPER(f.ds_Oggetto_Fascicolo) LIKE :dsOggettoFascicolo ");
            whereWord = " AND ";
        }
        if (filtri.getDtApeFascioloDa() != null && filtri.getDtApeFascioloA() != null) {
            whereString.append(whereWord)
        ...
        if (filtri.getDtChiuFascioloDa() != null && filtri.getDtChiuFascioloA() != null) {
            whereString.append(whereWord)
                    .append("f.dt_Chiu_Fascicolo BETWEEN :dtChiuFascicolo_da AND :dtChiuFascicolo_a ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getCdProcAmmin())) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdLivelloRiserv())) {
            whereString.append(whereWord).append("UPPER(f.cd_Livello_Riserv) LIKE :cdLivelloRiserv ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getNmSistemaVersante())) {
            whereString.append(whereWord).append("UPPER(f.nm_Sistema_Versante) LIKE :nmSistemaVersante ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getNmUserid())) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdCompositoVoceTitol())) {
            whereString.append(whereWord).append("UPPER(f.cd_Composito_Voce_Titol) LIKE :cdCompositoVoceTitol ");
            whereWord = " AND ";
        }
        if (filtri.getAaFascicoloPadre() != null) {
        ...
        if (filtri.getAaFascicoloPadreDa() != null && filtri.getAaFascicoloPadreA() != null) {
            whereString.append(whereWord)
                    .append("f.aa_Fascicolo_Padre BETWEEN :aaFascicoloPadre_da AND :aaFascicoloPadre_a ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getCdKeyFascicoloPadre())) {
            whereString.append(whereWord).append("f.cd_Key_Fascicolo_Padre = :cdKeyFascicoloPadre ");
            whereWord = " AND ";
        }
        String cdKeyFascicoloPadreDa = null;
        String cdKeyFascicoloPadreA = null;
        ...
        if (StringUtils.isNotBlank(filtri.getDsOggettoFascicoloPadre())) {
            whereString.append(whereWord).append("UPPER(f.ds_Oggetto_Fascicolo_Padre) LIKE :dsOggettoFascicoloPadre ");
            whereWord = " AND ";
        }
        boolean udFilter = false;
        if (StringUtils.isNotBlank(filtri.getCdRegistroKeyUnitaDoc())) {
            whereString.append(whereWord).append("aud.cd_Registro_Key_Unita_Doc = :cdRegistroKeyUnitaDoc ");
            udFilter = true;
            whereWord = " AND ";
        }
        if (filtri.getAaKeyUnitaDoc() != null) {
        ...
        if (StringUtils.isNotBlank(filtri.getCdKeyUnitaDocDa()) && StringUtils.isNotBlank(filtri.getCdKeyUnitaDocA())) {
            ...
            cdKeyUnitaDocA = StringPadding.padString(filtri.getCdKeyFascicoloPadreA(), "0", 12,
                    StringPadding.PADDING_LEFT);
            whereString.append(whereWord)
                    .append("LPAD( aud.cd_Key_Unita_Doc, 12, '0') BETWEEN :cdKeyUnitaDoc_da AND :cdKeyUnitaDoc_a ");
            udFilter = true;
            whereWord = " AND ";
        }
        String addJoin = " ";
        ...
        if (StringUtils.isNotBlank(filtri.getFlForzaContrClassif())) {
            whereString.append(whereWord).append("f.fl_Forza_Contr_Classif = :flForzaContrClassif ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getFlForzaContrNumero())) {
        ...
        if (filtri.getTsVersFascicoloDa() != null && filtri.getTsVersFascicoloA() != null) {
            whereString.append(whereWord)
                    .append("f.ts_Vers_Fascicolo BETWEEN :tsVersFascicolo_da AND :tsVersFascicolo_a ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getTiEsito())) {
        ...
        if (StringUtils.isNotBlank(filtri.getTiStatoConservazione())) {
            whereString.append(whereWord).append("f.ti_Stato_Conservazione = :tiStatoConservazione ");
            whereWord = " AND ";
        }
        if (StringUtils.isNotBlank(filtri.getTiStatoFascElencoVers())) {
            whereString.append(whereWord).append("f.ti_Stato_Fasc_Elenco_Vers = :tiStatoFascElencoVers ");
            whereWord = " AND ";
        }
        ...
        // whereWord = " AND ";
        queryStr.append(addJoin).append(whereString).append(" ORDER BY f.aa_Fascicolo, f.cd_Key_Fascicolo ");
    ...
    public List<FasVRicFascicoli> retrieveFascicoliAnnullati(FiltriRicercaFascicoli filtri, BigDecimal idStrut,
            long userId) {
        List<FasVRicFascicoli> result = null;
        String andClause = "AND ";
        StringBuilder queryStr = new StringBuilder(
    ...
    public CriteriaQuery retrieveVLisFascKoCriteriaQuery(BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte,
            BigDecimal idStrut, BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoSessione, String cdClasseErr,
            String cdErr) {
        try {
            CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
            ...
            if (dateValidate != null) {
                LocalDateTime dataDa = dateValidate[0].toInstant().atZone(ZoneId.systemDefault()).toLocalDate()
                        .atTime(0, 0, 0, 0);
                LocalDateTime dataA = dateValidate[1].toInstant().atZone(ZoneId.systemDefault()).toLocalDate().atTime(0,
                        0, 0, 0);
                condizioni.add(cb.between(cb.function("TRUNC", Date.class, entity.get("tsIniLastSes")),
                        cb.literal(Timestamp.valueOf(dataDa)), cb.literal(Timestamp.valueOf(dataA))));
            }
            if (statoSessione != null && (!statoSessione.equals(""))) {
    ...
    public CriteriaQuery retrieveVLisFascCriteriaQuery(BigDecimal idUser, BigDecimal idAmbiente, BigDecimal idEnte,
            BigDecimal idStrut, BigDecimal idTipoFascicolo, Date[] dateValidate, BigDecimal rangeAnnoDa,
            BigDecimal rangeAnnoA, String rangeNumeroDa, String rangeNumeroA, String statoIndiceAip,
            Set<String> statiConservazione, String flSesFascicoloKo) {
        try {
            CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
            Root entity = null;
            CriteriaQuery cq = null;
    ...
    public void deleteDecSelCriterioRaggrFascNotInVoceTitolo(List<BigDecimal> voceTitolList,
        ...
        Query q = getEntityManager().createQuery("DELETE FROM DecSelCriterioRaggrFasc u " + "WHERE u.tiSel = :filtro "
    ...
    public List<DecSelCriterioRaggrFasc> getDecSelCriterioRaggrFascList(DecCriterioRaggrFasc decCriterioRaggrFasc,
            DecVoceTitol voce) {
        Query query = getEntityManager().createQuery(
                "SELECT u FROM DecSelCriterioRaggrFasc u " + "WHERE u.decVoceTitol = :voce and u.tiSel = :filtro "
                        + "and u.decCriterioRaggrFasc.idCriterioRaggrFasc = :crit");
    ...
    public void deleteDecSelCriterioRaggrFascFiltroVoceTitolo(DecCriterioRaggrFasc decCriterioRaggrFasc) {
    ...
    public void deleteDecSelCriterioRaggrFascFiltroTipoFascicolo(DecCriterioRaggrFasc decCriterioRaggrFasc) {
        Query q = getEntityManager().createQuery("DELETE FROM DecSelCriterioRaggrFasc u "
                + "WHERE u.tiSel = :filtro and u.decCriterioRaggrFasc.idCriterioRaggrFasc = :crit");
    ...
    public void deleteDecSelCriterioRaggrFascNotInTipoFascicolo(DecCriterioRaggrFasc decCriterioRaggrFasc,
            List<BigDecimal> tipoFascicoloList) {
        Query q = getEntityManager().createQuery("DELETE FROM DecSelCriterioRaggrFasc u " + "WHERE u.tiSel = :filtro "
                + "and u.decCriterioRaggrFasc.idCriterioRaggrFasc = :crit "
    ...
    public List<DecSelCriterioRaggrFasc> getDecSelCriterioRaggrFascList(DecCriterioRaggrFasc decCriterioRaggrFasc,
            DecTipoFascicolo tipo) {
        Query query = getEntityManager().createQuery(
                "SELECT u FROM DecSelCriterioRaggrFasc u " + "WHERE u.decTipoFascicolo = :tipo and u.tiSel = :filtro "
                        + "and u.decCriterioRaggrFasc.idCriterioRaggrFasc = :crit");
    ...
    public List<DecTipoFascicolo> getDecTipoFascicoloList(StringBuilder queryStr, List<BigDecimal> asList) {
        Query query = getEntityManager().createQuery(queryStr.toString());
    ...
    public FasFascicoloTableBean getListaFasFascicoloElvViewBean(BigDecimal idElencoVersFasc,
        ...
        if (codiceRangeDa != null && codiceRangeA != null) {
            codiceRangeDa = StringPadding.padString(filtri.getCdKeyFascicoloDa(), "0", 12, StringPadding.PADDING_LEFT);
            codiceRangeA = StringPadding.padString(filtri.getCdKeyFascicoloA(), "0", 12, StringPadding.PADDING_LEFT);
            queryStr.append(whereWord).append("LPAD( f.cdKeyFascicolo, 12, '0') BETWEEN :codicein_da AND :codicein_a ");
            whereWord = "AND ";
        }
        ...
        if (StringUtils.isNotBlank(filtri.getCdCompositoVoceTitolo())) {
            query.setParameter("cdcompositovocetitolin", filtri.getCdCompositoVoceTitolo().toUpperCase());
        }
    ...
    public List<DecCriterioRaggrFasc> retrieveCriteriRaggrFascicoli(boolean filterValid,
        ...
        try {
            ...
            String flCriterioRaggrStandard = filtriCriteriRaggrFascicoliPlain.getFlCriterioRaggrStandard();
            ...
            String cdCompositoVoceTitol = filtriCriteriRaggrFascicoliPlain.getCdCompositoVoceTitol();
    ...
    public static class FiltriElenchiVersFascicoli {
        ...
        FiltriElenchiVersFascicoli(ElenchiVersFascicoliForm.FascicoliFiltri filtri) throws EMFError {
    ...
    protected static class FiltriCriteriRaggrFascicoliPlain {
        ...
        protected FiltriCriteriRaggrFascicoliPlain(
                CriteriRaggrFascicoliForm.FiltriCriteriRaggrFascicoli filtriCriteriRaggrFasc) throws EMFError {
            ...
            this.flCriterioRaggrStandard = filtriCriteriRaggrFasc.getFl_criterio_raggr_standard().parse();
    ...
    public List<DecModelloXsdFascicolo> getDecModelloXsdFascicoloDisp() {
        ...
        Query query = getEntityManager().createQuery(queryStr);
        query.setParameter("aaFinTipoFascicolo", BigDecimal.valueOf(Calendar.getInstance().get(Calendar.YEAR)));

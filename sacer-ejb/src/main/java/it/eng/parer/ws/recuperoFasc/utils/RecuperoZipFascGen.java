...
public class RecuperoZipFascGen {
    ...
    // The ZipArchiveEntry.setXxxTime() methods write the time taking into account the local time
    // zone,
    ...
    /*
     * Determino la versione Unisincro di riferimento per la quale effettuare il recupero del
     * pacchetto aip (default: v2.0)
     */
    private static final String UNISINCRO_V2_REF = "2.0";
    /*
     * Determino le versioni del servizio di versamento fascicolo per le quali forzare la
     * generazione del contenuto della cartella xmlschema nel pacchetto AIP conforme alla versione
     * Unisincro di riferimento (default: v1.0 e v1.1)
     */
	...
	recuperoDocumento = (RecuperoDocumento) new InitialContext()
		.lookup("java:app/Parer-ejb/RecuperoDocumento");
    }

    public void generaZipOggettoFasc(String outputPath, RecuperoFascExt recuperoFasc)
	    throws Exception {
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
	    ...
	    case FASC_UNISYNCRO:
		rispostaControlli = controlliRecuperoFasc.leggiContVerAipUdInFASCAIPV2(
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		if (includiSessFileVersamento && rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		    this.aggiungiXMLVersamentoFasc(tmpZipOutputStream, recuperoFasc,
			    TipiLetturaXml.FASC_COMPLETO, TipiXmlDaIncludere.TUTTI);
		}
		...
		if (includiFirmaMarcaElencoIndiceAIP
			&& rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		    this.aggiungiElencoIndiciAipFasc(tmpZipOutputStream,
			    ...
			    it.eng.parer.elencoVersFascicoli.utils.ElencoEnums.OpTypeEnum.FIRMA_ELENCO_INDICI_AIP
				    .name(),
			    it.eng.parer.elencoVersFascicoli.utils.ElencoEnums.OpTypeEnum.ELENCO_INDICI_AIP
				    .name());
    ...
    private void aggiungiFileContenutoFasc(ZipArchiveOutputStream zipOutputStream,
	    RecuperoFascExt recuperoFasc, List<ContenutoRec> lstConten)
    ...
    private void gestisciRecuperoAipUdArchiveEntry(ZipArchiveOutputStream zipOutputStream,
	    ContenutoRec tmpConten, RecuperoFascExt recuperoFasc) throws IOException, Exception {

	AroVerIndiceAipUd verIndiceAipUd = fasHelper.findById(AroVerIndiceAipUd.class,
		tmpConten.getIdVerIndiceAipUd());
	...
	String fileName = it.eng.parer.async.utils.IOUtils.getFilename(
		tmpConten.getNomeFileCompleto(),
	...
	if (rispostaWs.getSeverity() == SeverityEnum.OK) {
	    ...
	    zipOutputStream.write(
		    FileUtils.readFileToByteArray(rispostaWs.getRifFileBinario().getFileSuDisco()));
	}
    }

    /**
     * Metodo per fare il download del pacchetto aip di una determinata unita documentaria in un
     * file zip
     *
     * @param verIndiceAipUd entity AroVerIndiceAipUd
     * @param recuperoFasc   bean RecuperoFascExt
     *
     * @return RispostaWSRecupero oggetto con risposta recupero
     *
     * @throws Exception errore generico
     */
    public RispostaWSRecupero recuperaAipUdUnisincro(AroVerIndiceAipUd verIndiceAipUd,
	    RecuperoFascExt recuperoFasc) throws Exception {
    ...
    private void aggiungiElencoIndiciAipFasc(ZipArchiveOutputStream zipOutputStream,
	    long idFascicolo, String tiFileElencoVersFasc1, String tiFileElencoVersFasc2)
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
	    rispostaControlli.reset();
	    rispostaControlli = controlliRecuperoFasc.leggiElvFileElencoVersFasc(idFascicolo,
		    tiFileElencoVersFasc1, tiFileElencoVersFasc2);
		...
		fileElencoVersFasc = rispostaControlli.getrObject() != null
			? (ElvFileElencoVersFasc) rispostaControlli.getrObject()
			: null;
		prefisso = tiFileElencoVersFasc1.equals(
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		String nmAmbiente = fileElencoVersFasc.getElvElencoVersFasc().getOrgStrut()
			.getOrgEnte().getOrgAmbiente().getNmAmbiente();
		String nmEnte = fileElencoVersFasc.getElvElencoVersFasc().getOrgStrut().getOrgEnte()
			.getNmEnte();
		String nmStrut = fileElencoVersFasc.getElvElencoVersFasc().getOrgStrut()
			.getNmStrut();
    ...
    private void aggiungiXMLVersamentoFasc(ZipArchiveOutputStream zipOutputStream,
	    RecuperoFascExt recuperoFasc, TipiLetturaXml tlx, TipiXmlDaIncludere xmlDaIncludere)
	    throws IOException {
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		rispostaControlli = controlliRecuperoFasc.leggiXMLSessioneversFasc(
			recuperoFasc.getParametriRecuperoFasc().getIdFascicolo());
	    }
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		case FASC_UNISYNCRO:
			...
			/* Definisco la folder relativa al sistema di conservazione */
			String tmpPath = urnXmlVers.replaceAll(
				it.eng.parer.async.utils.IOUtils.extractPartUrnName(urnXmlVers),
				"SIP-FA");
			String path = it.eng.parer.async.utils.IOUtils.extractPartUrnName(tmpPath,
				true);
			...
			if (pathSip != null) {
				...
				Map<String, String> xmls = objectStorageService
					.getObjectXmlVersFascicolo(
						tmpXml.getFasFascicolo().getIdFascicolo());
		...
		default:
			...
			if (fileName != null) {
			    ...
			    zipOutputStream.putArchiveEntry(zipArchiveEntry);
			    zipOutputStream
				    .write(tmpXml.getBlXmlVers().getBytes(StandardCharsets.UTF_8));
			    zipOutputStream.closeArchiveEntry();
			}
		    }
		    break;
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		FasXmlVersFascicolo xmlVersFascicolo = lstXmlVersFascicolo.stream()
			.filter(xml -> xml.getTiXmlVers().equals("RICHIESTA")).findAny()
			.orElse(null);

		// MEV#29726
		// Se la modalità strict non è attiva la logica forza la generazione del contenuto
		// della cartella
		...
		if (STRICT_MODE.equals(Boolean.FALSE)
			&& FORZA_VERSIONI_XML_NOT_STRICT
				.contains(xmlVersFascicolo.getCdVersioneXml())
		...
		FileXSD xsdPIndexFileSource = (!"2.0".equals(codiceVersioneXsd))
			? FileXSD.AIP_FASC_UNISINCRO_2_XSD
			: FileXSD.AIP_FASC_UNISINCRO_2_XSD_V2;
		FileXSD xsdMoreInfoSelfDescSource = (!"2.0".equals(codiceVersioneXsd))
			? FileXSD.AIP_FASC_SELF_DESC_XSD
			: FileXSD.AIP_FASC_SELF_DESC_XSD_V2;
		// end MEV#29726

		FileXSD xsdMoreInfoPVolumeSource = (!"2.0"
			.equals(xmlVersFascicolo.getCdVersioneXml())) ? FileXSD.AIP_FASC_PROF_XSD
				: FileXSD.AIP_FASC_PROF_XSD_V2;
		...
		if (xsdMoreInfoSelfDesc != null) {
		    ...
		    zipOutputStream.putArchiveEntry(zae);
		    zipOutputStream.write(
			    FileUtils.readFileToString(xsdMoreInfoSelfDesc, StandardCharsets.UTF_8)
				    .getBytes());
		    closeEntry = true;
		}
		...
		if (xsdMoreInfoPVolume != null) {
		    ...
		    zipOutputStream.putArchiveEntry(zae);
		    zipOutputStream.write(
			    FileUtils.readFileToString(xsdMoreInfoPVolume, StandardCharsets.UTF_8)
				    .getBytes());
		    closeEntry = true;
		}
		...
		if ("2.0".equals(xmlVersFascicolo.getCdVersioneXml())) {
			...
			rispostaControlli = controlliRecuperoFasc
				.leggiXMLProfiloFascList(idFascicolo);
			...
			if (!rispostaControlli.isrBoolean()) {
			    ...
			    lstXmlFascicolo = (List<FasXmlFascicolo>) rispostaControlli
				    .getrObject();
			}
		    }
			...
			for (FasXmlFascicolo xmlFascicolo : lstXmlFascicolo) {
			    ...
			    /* Definisco il nome e l'estensione del file */
			    String fileName = it.eng.parer.async.utils.IOUtils.getFilename(
				    CaseUtils.toCamelCase(profilo, true, '_') + "-"
					    + xmlFascicolo.getDecModelloXsdFascicolo().getCdXsd(),
				    "xsd");

			    ZipArchiveEntry zae = new ZipArchiveEntry(
				    it.eng.parer.async.utils.IOUtils.getAbsolutePath(
					    DIRECTORY_XSD_AIPV2, fileName,
					    it.eng.parer.async.utils.IOUtils.UNIX_FILE_SEPARATOR));
			    this.filterZipEntry(zae);
			    zipOutputStream.putArchiveEntry(zae);
			    zipOutputStream.write(xmlFascicolo.getDecModelloXsdFascicolo()
				    .getBlXsd().getBytes(StandardCharsets.UTF_8));
			    closeEntry = true;
			}
		    }
		}
    ...
    private void aggiungiIndiciAipFasc(ZipArchiveOutputStream zipOutputStream, long idFascicolo)
	    throws IOException {
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		if (fileMetaIndiceLastVers.getFasMetaVerAipFascicolo() != null) {
		    // Recupero lo urn ORIGINALE
		    // fileName =
		    // fileMetaIndiceLastVers.getFasMetaVerAipFascicolo().getDsUrnMetaFascicolo();
		...
		for (FasFileMetaVerAipFasc fileMetaIndicePrecVers : lstFileMetaIndice) {
			...
			// Recupero lo urn ORIGINALE
			String urnIxAip = fileMetaIndicePrecVers.getFasMetaVerAipFascicolo()
			...
			this.filterZipEntry(zae);
			zipOutputStream.putArchiveEntry(zae);
			zipOutputStream.write(fileMetaIndicePrecVers.getBlFileVerIndiceAip()
    ...
    private void aggiungiMetadatiFascV2(ZipArchiveOutputStream zipOutputStream, long idFascicolo)
	    throws IOException {
	...
	if (rispostaWsFasc.getSeverity() == SeverityEnum.OK) {
		...
		if (fileMetaIndiceLastVers.getFasMetaVerAipFascicolo() != null) {
		     ...
		     * di conservazione
		     */
		    String path = it.eng.parer.async.utils.IOUtils.getAbsolutePath(
			    DIRECTORY_REC_FASC_METADATI, fileName,
			    it.eng.parer.async.utils.IOUtils.UNIX_FILE_SEPARATOR);
		...
		for (FasFileMetaVerAipFasc fileMetaIndicePrecVers : lstFasFileMetaFasc) {
			...
			/* Definisco il nome e l'estensione del file */
			fileName = it.eng.parer.async.utils.IOUtils.getFilename(
				it.eng.parer.async.utils.IOUtils.extractPartUrnName(urnMetaFasc,
					true),
				it.eng.parer.async.utils.IOUtils.CONTENT_TYPE.XML.getFileExt());
			/*
			 * Definisco il percorso relativo del file rispetto alla posizione

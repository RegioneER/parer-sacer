...
public class AltriStatusMonitor {
	...
	STATO_CONNESSIONE_ORCL, PRESENZA_ERRORI_CODA_JMS, PRESENZA_RECORD_INDICE_AIPUD_TOELAB,
	TPI_NI_FILE_PATH_ARK, TPI_ARCHIVIATA_ERR, TPI_DATA_NON_ARCHIVIATA
    }
    ...
    private static final String JMS_SELECTOR = Costanti.JMSMsgProperties.MSG_K_APP + " = '"
	    + Costanti.JMSMsgProperties.MSG_V_APP_SACER + "' OR "
	    + Costanti.JMSMsgProperties.MSG_K_APP + " = '"
	...
	try {
	    List<DLQMsgInfo> infoCodas = monitorCoda.retrieveMsgInQueue(NomeCoda.dmqQueue.name(),
		    messageSelector);
    ...
    /**
     * Se esiste un attributo payloadType elaboro il messaggio secondo un template prestabilito
     * Elabora quindi il numero di KO
     *
     * @param tmpAltro
     * @param infoCodas
     */
    ...
    public void calcolaStatoIndiceAipUdInCoda(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> tmpLstAltro) {
	MonitorAltro tmpAltro = new MonitorAltro();
	// nome
	tmpAltro.setNome(MonitorAltri.PRESENZA_RECORD_INDICE_AIPUD_TOELAB.name());
	// delta
	int delta = Integer.parseInt(configurationHelper.getValoreParamApplicByApplic(
		CostantiDB.TipoParametroAppl.PARERMT_INDICE_UD_DAELAB_NI_GG_MAX));
	...
	// query
	RispostaControlli rc = controlliMonitor
		.leggiStatoIndiceAipUdDaElab(new Date(dateTime.getMillis()), "1");
	// elab for message
	if (rc.isrBoolean()) {
		...
		// preparo messaggio da inviare di ERROR
		tmpAltro.setStato(
			MonitorSondeGenEsiti.ERROR.name() + "| rilevato/i " + rc.getrLong()
				+ " record di ARO_INDICE_AIP_UD_DA_ELAB da elaborare più vecchi di "
				+ delta + " giorno/i");
	    }
    ...
    public void calcolaTpiNiPathFileArk(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> tmpLstAltro) {
	MonitorAltro tmpAltro = new MonitorAltro();
	// MAC#27666
	...
	if (rc.isrBoolean()) {
		...
		sb.append(MonitorSondeGenEsiti.ERROR.name()
			+ "| rilevato/i errore numero file archiviati: ");
		for (VrsPathDtVers pathDtVers : result) {
		    ...
		    sb.append("Nr. File Path Ark Secondario = "
			    + (pathDtVers.getNiFilePathArkSecondario() != null
				    ? pathDtVers.getNiFilePathArkSecondario()
				    : "solo primario"));
		    sb.append(";");
		}
		// remove last char
    ...
    public void calcolaTpiArkErr(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> tmpLstAltro) {
	MonitorAltro tmpAltro = new MonitorAltro();
	// MAC#27666
	// DateFormat dateFormat = new SimpleDateFormat(Constants.DATE_FORMAT_DATE_TYPE);
	...
	if (rc.isrBoolean()) {
		...
		sb.append(result.stream().map(v -> dateFormat.format(v.getDtVers()))
    ...
    public void calcolaTpiDataNotArk(RispostaWSStatusMonitor rispostaWs,
	    List<MonitorAltro> tmpLstAltro) {
	MonitorAltro tmpAltro = new MonitorAltro();
	// MAC#27666
	// DateFormat dateFormat = new SimpleDateFormat(Constants.DATE_FORMAT_DATE_TYPE);

...
public class ControlliMonitor {
    ...
    public RispostaControlli leggiUltimaRegistrazione(String jobName) {
        ...
        try {
            ...
            if (lstDate != null && lstDate.size() > 0 && lstDate.get(0) != null) {
                ...
                query.setParameter("nmJob", jobName);
                query.setParameter("tiRegLogJob1", JobConstants.OpTypeEnum.FINE_SCHEDULAZIONE.name());
                ...
                if (lstJob != null && lstJob.size() > 0) {
                    ...
                    rispostaControlli.setrObject(JobConstants.OpTypeEnum.valueOf(lstJob.get(0).getTiRegLogJob()));
                } else {
    ...
    public RispostaControlli leggiStatoIndiceAipUdDaElab(Date dateMax, String flagInCoda) {
        ...
        try {
            String queryStr = "select count(a) from AroIndiceAipUdDaElab a " + "where a.flInCoda = :flInCoda "
                    + "and a.tsInCoda <= :tsInCodaMax ";
            ...
            rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
            rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
                    "Eccezione ControlliMonitor.leggiStatoMessaggiIndiceAipUdDaElab " + e.getMessage()));
    ...
    public RispostaControlli verificaDataNotArk() {
        ...
        try {
            String queryStr = "select v1 from VrsDtVers v1 " + "where v1.tiStatoDtVers in :tiStatoDtVersRegDaArk "
            ...
            List<String> tiStatoDtVersRegDaArk = Arrays.asList(JobConstants.ArkStatusEnum.REGISTRATA.name(),
                    JobConstants.ArkStatusEnum.DA_ARCHIVIARE.name());
            List<String> tiStatoDtVersArkArkErr = Arrays.asList(JobConstants.ArkStatusEnum.ARCHIVIATA.name(),

...
public class GestSessRecDip {
    ...
    public GestSessRecDip(RispostaWSRecupero risp) {
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            try {
                controlliRecDip = (ControlliRecDip) new InitialContext()
                        .lookup("java:module/ControlliRecDip");
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            try {
                ...
                recBlbOracle = (RecBlbOracle) new InitialContext()
                        .lookup("java:module/RecBlbOracle");
            } catch (NamingException ex) {
                rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "GestSessRecDip Errore nel recupero dell'EJB RecBlbOracle  "
                                + ex.getMessage());
        ...
        if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.OK) {
            try {
                recuperoDocumento = (RecuperoDocumento) new InitialContext()
                        .lookup("java:module/RecuperoDocumento");
            } catch (NamingException ex) {
                rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "GestSessRecDip Errore nel recupero dell'EJB del recupero documento  "
                                + ex.getMessage());
                log.error("Errore nel recupero dell'EJB dei controlli  recupero DIP ", ex);
            }
        }
            ...
            for (CompRecDip comp : compRecDips.values()) {
                if (comp.isErroreFormatoContenuto()) {
                    ...
                    comp.setErrorMessage(
                            "Il formato del file versato non è corretto per il convertitore");
                } else if (comp
                        .getTipoAlgoritmoRappresentazione() == CostantiDB.TipoAlgoritmoRappr.ALTRO) {
                    ...
                    try {
                        ...
                        RecuperoDocBean csRecuperoDoc = new RecuperoDocBean(
                                TiEntitaSacerObjectStorage.COMP_DOC, comp.getIdCompConvertitore(),
                        ...
                        String hash = hashCalculator
                                .calculateHashSHAX(bos.toByteArray(),
                                        TipiHash.evaluateByDesc(comp.getDsAlgoHashFileCalc()))
                                .toHexBinary();
                        rispostaControlli = controlliRecDip.cercaInsConvertitore(comp, hash,
                                bos.toByteArray());
                        if (!rispostaControlli.isrBoolean()) {
                        ...
                        rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
                        rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                                "GestSessRecDip: Errore caricaListaComponenti DIP  "
                                        + e.getMessage());
    ...
    public void caricaParametri(RecuperoExt recupero) {
        ...
        if (rispostaControlli.isrBoolean() == false) {
            ...
            recupero.setDipRootImg(
                    imgDefaults.get(CostantiDB.ParametroAppl.IMAGE_ROOT_IMAGE_TRASFORM));
        }
    }
        ...
        try {
            if (comp.isErroreFormatoContenuto()) {
                ...
                comp.setErrorMessage(
                        "Il formato di rappresentazione non prevede una trasformazione.");
                /*
                 * Non sono stati individuati componenti il cui formato di rappresentazione sia
                 * compatibile con il recupero DIP
                 */
                ...
                fileDaScaricare.setFileSuDisco(File.createTempFile("output_",
                        "." + comp.getEstensioneFile(), new File(outputPath)));
                tmpOutputStream = new FileOutputStream(fileDaScaricare.getFileSuDisco());
                ///
                rispostaControlli = this.converti(comp, compRecDips, tmpOutputStream,
                        recupero.getDipRootImg());
                if (!rispostaControlli.isrBoolean()) {
                    if (rispostaControlli.getCodErr() == null) {
                        /**
                         * Il convertitore {0} del formato di rappresentazione {1} relativo al
                         * componente {2} ha reso un errore di conversione ed è stato marcato come
                         * ERRATO: {3}
                         */
            ...
            File file = fileDaScaricare.getFileSuDisco();
    ...
    public void recuperaZip(String outputPath, RecuperoExt recupero,
            LinkedHashMap<Long, CompRecDip> compRecDips) {
        RispostaControlli rispostaControlli;
        FileBinario fileDaScaricare = new FileBinario();
        FileOutputStream tmpOutputStream = null;
        ...
        try {
            rispostaControlli = controlliRecDip.calcolaNomeFileZip(recupero.getParametriRecupero(),
                    compRecDips);
                ...
                if (comp.isErroreFormatoContenuto()) {
                    ...
                    if (!rispostaControlli.isrBoolean()) {
                        if (rispostaControlli.getCodErr() == null) {
                            /**
                             * Il convertitore {0} del formato di rappresentazione {1} relativo al
                             * componente {2} ha reso un errore di conversione ed è stato marcato
                             * come ERRATO: {3}
                             */
                            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.RECDIP_002_001,
                                    comp.getNomeConvertitore() + "."
                                            + comp.getVersioneConvertitore(),
                                    comp.getNomeFormatoRappresentazione(), comp.getNomeFileBreve(),
                                    rispostaControlli.getDsErr());
                        } else {
                            rispostaWs.setEsitoWsError(rispostaControlli.getCodErr(),
                        ...
                        break; // esce dal ciclo for, se incontra un errore: ormai lo zip è
                               // condannato
                    }
                }
            }
            ...
            for (CompRecDip comp : compRecDips.values()) {
                ...
                if (comp.isErroreFormatoContenuto()) {
                    ...
                    tmpSuffisso = SUFF_ERRORE;
                        ...
                        .getTipoAlgoritmoRappresentazione() == CostantiDB.TipoAlgoritmoRappr.ALTRO) {
                    bos.write("Il formato di rappresentazione non prevede una trasformazione."
                            .getBytes("UTF-8"));
                    tmpSuffisso = SUFF_WARNING;
                } else {
                    rispostaControlli = this.converti(comp, compRecDips, bos,
                            recupero.getDipRootImg());
                    if (!rispostaControlli.isrBoolean()) {
                        tmpSuffisso = SUFF_ERRORE;
                        if (rispostaControlli.getCodErr() == null) {
                            String tmpString = MessaggiWSBundle.getString(
                                    MessaggiWSBundle.RECDIP_002_001,
                                    comp.getNomeConvertitore() + "."
                                            + comp.getVersioneConvertitore(),
                                    comp.getNomeFormatoRappresentazione(), comp.getNomeFileBreve(),
                                    rispostaControlli.getDsErr());
    ...
    /*
     * se coderr è valorizzato, è un errore applicativo grave (di norma è 666) se coderr NON è
     * valorizzato è un errore di conversione del componente e va riportato sulla tabella
     * DecTrasformTipoRappr come ERRATO
     */
    private RispostaControlli converti(CompRecDip comp, LinkedHashMap<Long, CompRecDip> compRecDips,
            OutputStream out, String rootImgDip) {
        RispostaControlli rispostaControlli;
        //
        ...
        rispostaControlli = controlliRecDip.estraiImmaginiConv(comp.getIdFileTrasform(),
                rootImgDip);
        if (rispostaControlli.isrBoolean()) {
            ...
            if (rispostaControlli.getCodErr() == null) {
                 ...
                 * NON è valorizzato significa che la conversione è riuscita oppure è un errore di
                 * conversione del componente e va riportato su tutte le righe CompRecDip con lo
                 * stesso convertitore
                 */
    ...
    public void collaudaConvertitore(String outputPath, RecuperoExt recupero, CompRecDip comp) {
        ...
        try {
            if (comp.getTipoAlgoritmoRappresentazione() == CostantiDB.TipoAlgoritmoRappr.ALTRO) {
                ...
                tmpOutputStream = new FileOutputStream(fileDaScaricare.getFileSuDisco());
                ///
                rispostaControlli = this.convertiPerCollaudo(comp, tmpOutputStream,
                        recupero.getDipRootImg());
                if (!rispostaControlli.isrBoolean()) {
                    if (rispostaControlli.getCodErr() == null) {
                        String tmpString = "Il convertitore ha reso un errore di conversione "
                                + "ed è stato marcato come ERRATO, errore: "
                                + rispostaControlli.getDsErr();
     ...
     * DecTrasformTipoRappr come ERRATO
     */
    private RispostaControlli convertiPerCollaudo(CompRecDip comp, OutputStream out,
            String rootImgDip) {
        RispostaControlli rispostaControlli;
        //
        RispostaControlli risposta;

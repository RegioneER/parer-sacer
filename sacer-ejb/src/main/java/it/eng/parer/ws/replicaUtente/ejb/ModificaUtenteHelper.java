...
public class ModificaUtenteHelper {
     ...
     *
     * @param muExt      bean ModificaUtenteExt
     * @param rispostaWs bean RispostaWSModificaUtente
     *
	...
	if (listaIndIp != null && listaIndIp.getIndIp() != null) {
	    log.debug("Necessario persistere " + listaIndIp.getIndIp().size()
		    + " record di IamIndIpUser per l'utente " + utente.getNmUserid());
	    ...
	    log.debug(
		    "Eseguita persist di nuove IamIndIpUser per l'utente " + utente.getNmUserid());
	}
	...
	log.debug("Recuperate " + listaAbilOrganizDB.size() + " organizzazioni per l'utente "
		+ utente.getNmUserid());
	...
	for (IamAbilOrganiz abilOrganizDB : listaAbilOrganizDB) {
		...
		if (ltda.getTipoDatoAbilList() != null) {
			...
			abilTipoDatoSet
				.add(new PairAbil<String, BigDecimal>(tda.getNmClasseTipoDato(),

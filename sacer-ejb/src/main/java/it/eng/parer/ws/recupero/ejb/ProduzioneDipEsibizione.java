...
public class ProduzioneDipEsibizione {
    ...
    // nome query, nome colonna resa della query diventano il nome della variabile da sostiuire nel
    // template
    ...
    public RispostaControlli caricaModello(long idUnitaDoc, TipiUsoModello usoModello,
        ...
        try {
            ...
            if (tmpEnteConvenz == null) {
                rc.setrBoolean(false);
                rc.setDsErr(
                        "(Impossibile determinare l'Ente convenzionato in IAM - OrgEnteConvenz)");
                return rc;
            }

            // lettura modello
            SIDecModelloComunic tmpModelloComunic = this.leggiModelloComunic(tmpEnteConvenz,
                    usoModello, oggQryModello);
    ...
    public RispostaControlli caricaDatiDaQuery(long idModello, TipiUsoQuery tuq,
            long idEntitaSacer) {
        RispostaControlli rc = new RispostaControlli();
    ...
    private SIDecModelloComunic leggiModelloComunic(SIOrgEnteSiam enteConvenz,
            TipiUsoModello usoModello, TipiOggQryModello oggQryModello) {
        List<SIDecModelloComunic> modelli;

        String queryStr = "select t from SIDecModelloComunic t "
                + "join t.decUsoModelloComunics tu " + "where t.tiComunic = :tiComunic "
                + "and t.tiOggettoQuery = :tiOggettoQuery " + "and t.dtIstituz <= :dataOggi "
                + "and t.dtSoppres > :dataOggi "
    ...
    private List<SIDecQueryModelloComunic> leggiQueryModello(long idModello, TipiUsoQuery tuq) {
        ...
        javax.persistence.Query query = entityManager.createQuery(queryStr,
                SIDecQueryModelloComunic.class);
    ...
    private void leggiDatiQueryInMappa(Map<String, String> tmpMainMap,
            SIDecQueryModelloComunic sidqmc, long idOggetto) {
        ...
        for (SIDecColQueryModelloComunic tmpPos : sidqmc.getDecColQueryModelloComunics()) {
            String tmpNomePlaceHolder = MessageFormat.format(FMT_KEY_TEMPLATE, prefisso,
                    tmpPos.getNmCol());

...
public class RecuperoSync {
    ...
    public void verificaVersione(String versione, RispostaWSRecupero rispostaWs, RecuperoExt rec) {
        ...
        tmpRispostaControlli = myControlliWs.checkVersione(versione, rec.getDescrizione().getNomeWs(),
                rec.getWsVersions(), TipiWSPerControlli.VERSAMENTO_RECUPERO);
    ...
    // viene impiegato sia dal recupero sincrono che in quello multimedia, poichè al termine dell'operazione
    // provvede a lasciare un file zip nell'area temporanea, pronto per lo streaming o per il trasferimento
    public void recuperaOggetto(RispostaWSRecupero rispostaWs, RecuperoExt recupero, String path) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK
            ...
            /*
             * se rende OK -> recupera da filesystem se rende WARNING -> prenotazione effettuata o recupero in corso,
             * non recupera nulla se rende ERROR -> recupero e prenotazione impossibili
             */
        }
        ...
        if (salvaSessioneRecupero) {
            if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                recupero.getDatiSessioneRecupero().setStatoSess(JobConstants.StatoSessioniRecupEnum.ELIMINATO);
                recupero.getDatiSessioneRecupero().setStatoDtVers(JobConstants.StatoDtVersRecupEnum.RECUPERATA);
            } else {
                recupero.getDatiSessioneRecupero().setStatoSess(JobConstants.StatoSessioniRecupEnum.CHIUSO_ERR);
                recupero.getDatiSessioneRecupero().setStatoDtVers(JobConstants.StatoDtVersRecupEnum.ERRORE);

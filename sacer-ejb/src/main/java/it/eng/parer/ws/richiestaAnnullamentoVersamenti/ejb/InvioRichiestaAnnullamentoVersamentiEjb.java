...
public class InvioRichiestaAnnullamentoVersamentiEjb {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(InvioRichiestaAnnullamentoVersamentiEjb.class);
    ...
    public void esaminaRichiesteAnnullamentoVersamento(
	...
	try {
	    ...
	    Long idRichiesta = context
		    .getBusinessObject(InvioRichiestaAnnullamentoVersamentiEjb.class)
		...
		AroRichAnnulVers richAnnulVers = ravHelper.findById(AroRichAnnulVers.class,
			idRichiesta);
		AroStatoRichAnnulVers statoRichAnnulVers = ravHelper.findById(
			AroStatoRichAnnulVers.class, richAnnulVers.getIdStatoRichAnnulVersCor());
		if (statoRichAnnulVers.getTiStatoRichAnnulVers()
		    ...
		    context.getBusinessObject(InvioRichiestaAnnullamentoVersamentiEjb.class)
			    .evasioneRichiestaAnnullamento(idRichiesta,
				    ravExt.getUser().getIdUtente(), flRichiestaPing);
		    log.info("... evasa");
		    // Chiusa transazione
		    // Conto il numero di item presenti nella richiesta e quelli presenti nella
		    // richiesta con stato
		    // NON_ANNULLABILE
			...
			codiceErrore = (richAnnulVers.getTiRichAnnulVers()
				.equals(CostantiDB.TiRichAnnulVers.UNITA_DOC.name()))
					? MessaggiWSBundle.RICH_ANN_VERS_012
					: MessaggiWSBundle.RICH_ANN_VERS_023;
			// end MEV#26446
		...
		Long numeroItems = avEjb.countItemsInRichAnnulVers(
			new BigDecimal(richAnnulVers.getIdRichAnnulVers()));
		Long numeroItemsNonAnnullabili = avEjb.countItemsInRichAnnulVers(
			new BigDecimal(richAnnulVers.getIdRichAnnulVers()),
			CostantiDB.StatoItemRichAnnulVers.NON_ANNULLABILE.name());
		String xmlRisposta = generaRisposta(rispostaWs, ravExt, numeroItems,
			numeroItemsNonAnnullabili, richAnnulVers);
		context.getBusinessObject(InvioRichiestaAnnullamentoVersamentiEjb.class)
			.createAroXmlRichAnnulVers(richAnnulVers,
				CostantiDB.TiXmlRichAnnulVers.RISPOSTA.name(), xmlRisposta,
	...
	if (rispostaWs.getSeverity() != IRispostaWS.SeverityEnum.ERROR) {
	    ...
	    String descrizione = ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
		    .getDescrizione();
	    String note = ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
		    .getMotivazione();
	    ...
	    // flImmediata è un parametro facoltativo che dunque, in caso non venga inviato, di
	    // default risulta essere
	    // false
	    if (ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().isImmediata() != null) {
		flImmediata = ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
			.isImmediata();
	    }

	    if (ravExt.getModificatoriWS().contains(Costanti.ModificatoriWS.TAG_ANNUL_FORZA_PING)) {

		// flForzaAnnul è un parametro facoltativo che dunque, in caso non venga inviato, di
		// default risulta
		// essere false
		if (ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
			.isForzaAnnullamento() != null) {
		    flForzaAnnul = ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
			    .isForzaAnnullamento();
		}

		// flRichiestaPing è un parametro facoltativo che dunque, in caso non venga inviato,
		// di default risulta
		// essere false
		if (ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
			.isRichiestaDaPreIngest() != null) {
		...
		tipoVersamento = ravExt.getRichiestaAnnullamentoVersamenti()
			.getVersamentiDaAnnullare().getVersamentoDaAnnullare().get(0)
			.getTipoVersamento();
		// flImmediata è un parametro facoltativo ma, in caso non venga inviato, di default
		// risulta essere
	    ...
	    // MEV#26446
	    String partMsg = (!TipoVersamentoType.UNITA_DOCUMENTARIA.value()
		    .equals(tipoVersamento.value())) ? "fascicoli" : "unità documentarie";
	    ...
	    // Determino se esiste un'altra richiesta con stato corrente pari a INVIO_FALLITO, e in
	    // caso la cancello
	    ravHelper.deleteRichiestaSePresente(ravExt.getIdStrut(),
		    ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().getCodice());

	    // MEV#26446
	    String tiRichAnnulVers = (!TipoVersamentoType.UNITA_DOCUMENTARIA.value()
		    .equals(tipoVersamento.value())) ? CostantiDB.TiRichAnnulVers.FASCICOLI.name()
			    : CostantiDB.TiRichAnnulVers.UNITA_DOC.name();
	    // end MEV#26446
		...
		if (!ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta()
			.getTipoAnnullamento().value().equals(tipoAnnullamento.value())) {
		    tiAnnulRichAnnulVers = "CANCELLAZIONE";
		}
	    }

	    // Registro la richiesta di annullamento con i relativi item, eseguendo infine i
	    // controlli sulle unità
	    // documentarie o sui fascicoli
	    AroRichAnnulVers richAnnulVers = avEjb.insertRichAnnulVers(user.getIdUtente(), idStrut,
		    codice, descrizione, note, tiRichAnnulVers, ravExt.getDataElaborazione(),
		    flImmediata, flForzaAnnul, flRichiestaPing, tiAnnulRichAnnulVers, ravExt);

	    boolean presentiDaAnnulInPing = false;
	    for (AroItemRichAnnulVers aroItemRichAnnulVers : richAnnulVers
	    ...
	    // Se la richiesta proviene da PreIngest il sistema elimina tutti gli errori di tipo
	    // DA_ANNULLARE_IN_PING
	    // per tutte le unità doc
	    if (flRichiestaPing) {
		// PERFORMANCE ma se sono appena stati creati da insertRichAnnulVers non poteva
		// evitare di farlo
	    ...
	    // Conto il numero di item presenti nella richiesta e quelli presenti nella richiesta
	    // con stato
	    // NON_ANNULLABILE
	    // PERFORMANCE perché torniamo su DB
	    Long numeroItems = avEjb
		    .countItemsInRichAnnulVers(new BigDecimal(richAnnulVers.getIdRichAnnulVers()));
	    ...
	    String statoRichiesta = checkPostElaborazione(rispostaWs, numeroItems,
		    numeroItemsNonAnnullabili, flImmediata, presentiDaAnnulInPing, tiRichAnnulVers);

	    // Registro lo STATO della richiesta
	    avEjb.insertAroStatoRichAnnulVers(richAnnulVers, statoRichiesta,
		    ravExt.getDataElaborazione(), null, user.getIdUtente());
	    log.info(InvioRichiestaAnnullamentoVersamentiEjb.class.getSimpleName()
		    + " --- Inserimento richiesta annullamento versamenti " + partMsg
		    + " completata con successo: inseriti " + numeroItems + " item di cui "
		    + numeroItemsNonAnnullabili + " con stato NON_ANNULLABILE ");
	    log.info(InvioRichiestaAnnullamentoVersamentiEjb.class.getSimpleName()
		    + " --- Registro su DB gli XML di richiesta ed esito annullamento versamenti "
		    + partMsg);
    ...
    /**
     * Controlli da eseguire successivamente all'elaborazione necessari a rilevare eventuali altri
     * WARNING
     *
     * @param numeroItems               numero item da elaborare
     * @param numeroItemsNonAnnullabili numero item non annullabili
     * @param flImmediata               flag 1/0 (true/false)
     *
     * @return stato richiesta
     */
    private String checkPostElaborazione(RispostaWSInvioRichiestaAnnullamentoVersamenti rispostaWs,
	    Long numeroItems, Long numeroItemsNonAnnullabili, boolean flImmediata,
	    boolean presentiDaAnnulInPing, String tiRichAnnulVers) {
	...
	if (numeroItems.intValue() == numeroItemsNonAnnullabili.intValue()) {
	    ...
	    codiceErrore = (tiRichAnnulVers.equals(CostantiDB.TiRichAnnulVers.UNITA_DOC.name()))
		    ? MessaggiWSBundle.RICH_ANN_VERS_011
		    : MessaggiWSBundle.RICH_ANN_VERS_022;
	    // end MEV#26446
	    messaggioErrore = MessaggiWSBundle.getString(codiceErrore);
	} else {
	    if (flImmediata) {
		if (presentiDaAnnulInPing) {
		    ...
		    messaggioErrore = MessaggiWSBundle
			    .getString(MessaggiWSBundle.RICH_ANN_VERS_013);
		} else {
    ...
    private String generaRisposta(RispostaWSInvioRichiestaAnnullamentoVersamenti rispostaWs,
	    InvioRichiestaAnnullamentoVersamentiExt ravExt, Long numeroItems,
	    Long numeroItemsNonAnnullabili, AroRichAnnulVers richAnnulVers)
	    throws IndexOutOfBoundsException, IOException, MarshalException, ValidationException,
	    JAXBException {
	// Preparo la risposta
	rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
		.setNumeroVersamentiDaAnnullare(
			numeroItems != null ? BigInteger.valueOf(numeroItems) : null);
	rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
		.setNumeroVersamentiNonAnnullabili(numeroItemsNonAnnullabili != null
			? BigInteger.valueOf(numeroItemsNonAnnullabili)
			: null);
	...
	if (!itemList.isEmpty()) {
	    rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().setVersamentiDaAnnullare(
		    new EsitoRichiestaAnnullamentoVersamenti.VersamentiDaAnnullare());
	    if (rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getVersioneXmlEsito()
		    .equals("1.4")) {
		it.eng.parer.ws.xml.esitoRichAnnullVers.TipoAnnullamentoType tiAnnullamento = richAnnulVers
			.getTiAnnullamento()
			.equals(it.eng.parer.ws.xml.esitoRichAnnullVers.TipoAnnullamentoType.CANCELLAZIONE
				.value())
		...
		rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
			.setTipoAnnullamento(tiAnnullamento);
	    }
		...
		// MEV#26446
		if (item.getTiItemRichAnnulVers()
		...
		} else if (item.getTiItemRichAnnulVers()
			.equals(CostantiDB.TipiEntitaSacer.FASC.name())) {
		    versamentoDaAnnullare.setTipoVersamento(
			    it.eng.parer.ws.xml.esitoRichAnnullVers.TipoVersamentoType.FASCICOLO);
		    //
		...
		if (rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getVersioneXmlEsito()
			.equals("1.2")
			|| rispostaWs.getEsitoRichiestaAnnullamentoVersamenti()
				.getVersioneXmlEsito().equals("1.3")
			|| rispostaWs.getEsitoRichiestaAnnullamentoVersamenti()
				.getVersioneXmlEsito().equals("1.4")) {
	...
	String xmlRisposta = avEjb.marshallaEsitoRichiestaAnnullamentoVersamenti(
		rispostaWs.getEsitoRichiestaAnnullamentoVersamenti());
	return xmlRisposta;
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void evasioneRichiestaAnnullamento(Long idRichAnnulVers, long idUserIam,
	    boolean isFromPreingest) throws ParerUserError {
	log.debug("locking AroRichAnnulVers {} ...", idRichAnnulVers);
	AroRichAnnulVers richiestaAnnullamento = ravHelper.findByIdWithLock(AroRichAnnulVers.class,
		idRichAnnulVers);
	log.debug("... locked");
	// Controlli item
	log.debug("controlloItemWsRichiestaAnnul - INIZIO");
	avEjb.controlloItemWsRichiestaAnnul(BigDecimal.valueOf(idRichAnnulVers), idUserIam,
		isFromPreingest);
	...
	return avEjb.createAroXmlRichAnnulVers(richAnnulVers, tiXmlRichAnnulVers,
		blXmlRichAnnulVers, cdVersioneXml);
    }

    /**
     * Inizializza la risposta WS ed effettua contestualmente un controllo sulla versione impostata
     * nella richiesta
     *
     * @param rispostaWs Risposta di cui inizializzare i campi
     * @param ravExt     Richiesta esterna originale
     * @param now        data da impostare come data dell'esito della risposta
     */
	...
	rispostaWs.setEsitoRichiestaAnnullamentoVersamenti(
		new EsitoRichiestaAnnullamentoVersamenti());
	RispostaControlli rs = this.loadWsVersions(ravExt);
	// Se il controllo sulla versione è ha dato esito positivo metto a POSITIVO anche la
	// risposta generale
	if (rs.isrBoolean()) {
	    rispostaWs.getEsitoRichiestaAnnullamentoVersamenti()
		    .setEsitoRichiesta(new EsitoRichiestaType());
	    rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getEsitoRichiesta()
	...
	rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().setVersioneXmlRichiesta(
		ravExt.getDescrizione().getVersione(ravExt.getWsVersions()));
	rispostaWs.getEsitoRichiestaAnnullamentoVersamenti()
    ...
    /**
     * Controlla che l'utente che attiva il servizio sia presente nella tabella IAM_USER, che sia
     * attivo e che la sua password sia corretta e non scaduta
     *
	...
	tmpRispostaControlli = controlliWS.checkVersione(versione,
		ravExt.getDescrizione().getNomeWs(), ravExt.getWsVersions(),

...
public class InvioRichiestaAnnullamentoVersamentiParser {
    ...
    private static final Logger log = LoggerFactory.getLogger(InvioRichiestaAnnullamentoVersamentiParser.class);
    ...
    public InvioRichiestaAnnullamentoVersamentiParser(RispostaWSInvioRichiestaAnnullamentoVersamenti risp) {
        rispostaWs = risp;
    ...
    public void parseXML(String datiXml, InvioRichiestaAnnullamentoVersamentiExt ravExt) {
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            try {
                Unmarshaller unmarshaller = xmlContextCache.getRichAnnVersCtx_RichiestaAnnullamentoVersamenti()
                        .createUnmarshaller();
                ...
                rispostaWs.setErrorMessage(
                        MessaggiWSBundle.getString(MessaggiWSBundle.RICH_ANN_VERS_002, event.getMessage()));
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.RICH_ANN_VERS_002, event.getMessage());
            } catch (Exception e) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setErrorCode(MessaggiWSBundle.RICH_ANN_VERS_002);
                rispostaWs.setErrorMessage(MessaggiWSBundle.getString(MessaggiWSBundle.RICH_ANN_VERS_002));
            ...
            try {
                ...
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore nel recupero dell'EJB dei Controlli Richiesta Annullamento Versamenti  "
                                + ex.getMessage());
                log.error("Errore nel recupero dell'EJB dei Controlli Richiesta Annullamento Versamenti ", ex);
            }
        }
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR
                && !ravExt.getModificatoriWS().contains(Costanti.ModificatoriWS.TAG_ANNUL_FORZA_PING)
                // MEV#26446
                && !ravExt.getModificatoriWS().contains(Costanti.ModificatoriWS.TAG_ANNUL_FASC)
                // end MEV#26446
                && (ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().isForzaAnnullamento() != null || ravExt
                        .getRichiestaAnnullamentoVersamenti().getRichiesta().isRichiestaDaPreIngest() != null)) {
            rispostaWs.setSeverity(SeverityEnum.ERROR);
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            if ((richiesta.getVersamentiDaAnnullare().getVersamentoDaAnnullare().stream()
                    .collect(Collectors.groupingBy(VersamentoDaAnnullareType::getTipoVersamento)).values().stream()
                    .distinct().count() > 1)) {
                // I versamenti da annullare definiti nella richiesta devono avere stesso Tipo Versamento
                rispostaWs.setSeverity(SeverityEnum.ERROR);
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            rispostaControlli = controlliSemantici.checkIdStrut(tmpCSVersatore, TipiWSPerControlli.ANNULLAMENTO);
        ...
        /*
         * CONTROLLO ABILITAZIONE SERVIZIO -- (RICH_ANN_VERS_008) N.B. LO METTO QUI PERCHE' DEVO AVER CALCOLATO LA
         * STRUTTURA PRIMA!
         */
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
        ...
        if (rispostaWs.getSeverity() != SeverityEnum.ERROR) {
            ...
            rispostaControlli.reset();
            rispostaControlli = controlliWSrav.checkRichiestaEsistenteInvioFallito(ravExt.getIdStrut(),
    ...
    // private void popolaVersioniEDataEsito(EsitoRichiestaAnnullamentoVersamenti esito, String versioneXsd, Date
    // adesso) {
    ...
    private void popolaVersatoreRichiestaEsito(InvioRichiestaAnnullamentoVersamentiExt ravExt) {
        ...
        rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getVersatore()
                .setAmbiente(ravExt.getRichiestaAnnullamentoVersamenti().getVersatore().getAmbiente());
        rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getVersatore()
        ...
        rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
                .setDescrizione(ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().getDescrizione());
        rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
                .setMotivazione(ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().getMotivazione());
        rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta()
                .setImmediata(ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().isImmediata());
        if (ravExt.getModificatoriWS().contains(Costanti.ModificatoriWS.TAG_ANNUL_FORZA_PING)) {
            rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta().setForzaAnnullamento(
                    ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().isForzaAnnullamento());
            rispostaWs.getEsitoRichiestaAnnullamentoVersamenti().getRichiesta().setRichiestaDaPreIngest(
                    ravExt.getRichiestaAnnullamentoVersamenti().getRichiesta().isRichiestaDaPreIngest());
        }
    }
}

...
public class ControlliRecTpi {
    ...
    public RispostaControlli caricaDateDocumenti(ParametriRecupero parametri) {
        ...
        // determina l'insieme delle date da recuperare e per le quali creare i record in
        // rec_dt_vers_recup
        List<AroDoc> aroDocs;
        try {
            ...
            case UNI_DOC_DIP_ESIBIZIONE: // di fatto sono lo stesso oggetto
                querySb.append("select t from AroDoc t "
                        + "where t.aroUnitaDoc.idUnitaDoc = :idParametroIn ");
            ...
            case SUB_COMP:
                queryStr = "select t from AroDoc t " + "join t.aroStrutDocs sd "
                        + "join sd.aroCompDocs cd " + "where cd.idCompDoc = :idParametroIn ";
                query = entityManager.createQuery(queryStr);
            ...
            /*
             * tronco le date mantenenendo solo la parte data, e le inserisco in un LinkedHashSet in
             * modo che i documenti versati nello stesso giorno producano una sola entry. Questo per
             * compensare l'impossibilità di effettuare una operazione DISTINCT via JPQL solo sulla
             * parte "data" di una serie di timestamp.
             */
    ...
    public RispostaControlli verificaPrenotaRecupero(ParametriRecupero parametri,
        ...
        try {
            ...
            queryStr = "select t from RecUnitaDocRecup t where "
                    + "t.aroUnitaDoc.idUnitaDoc = :idUnitaDocIn";
            query = entityManager.createQuery(queryStr);
            ...
            if (recUnitaDocRecups.size() > 0) {
                ...
                queryStr = "select t from RecSessioneRecup t where  "
                        + "t.recUnitaDocRecup = :recUnitaDocRecupIn "
                ...
                if (recSessioneRecups.size() > 0) {
                    ...
                    datiSessDaCreare
                            .setIdRecSessioneRecupero(tmpRecSessioneRecup.getIdSessioneRecup());
                    ...
                    case CHIUSO_ERR:
                        /*
                         * nota rendo un codice errore ed un messaggio, ma pongo a true il valore
                         * boolean di risposta. In questo modo il chiamante può segnalare una
                         * notifica di Warning invece che di Errore.
                         */
                        creaSessioneRec = true;
                        //
                        /**
                         * La precedente richiesta di recupero dei file dell'unità documentaria {0}
                         * è in errore: [{1}]. SACER proverà nuovamente a rendere disponibili i file
                         * dell'unità documentaria non appena possibile
                         */
                        rispostaControlli.setrBoolean(true);
                        rispostaControlli.setCodErr(MessaggiWSBundle.SESREC_001_002);
                        rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                                MessaggiWSBundle.SESREC_001_002, parametri.getDescUnitaDoc(),
                                String.format("%s - %s", tmpRecSessioneRecup.getCdErr(),
                                        tmpRecSessioneRecup.getDlErr())));
                        break;
                    case ELIMINATO:
                        /*
                         * creo la sessione di recupero inoltre nello step successivo produco un
                         * messaggio di warning per indicare che i dati saranno pronti il prima
                         * possibile
                         */
                        creaSessioneRec = true;
                        break;
                    case IN_CORSO:
                        ...
                        rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                                MessaggiWSBundle.SESREC_002_001, parametri.getDescUnitaDoc()));
                        //
                        this.calcolaErrore(rispostaControlli, tmpRecSessioneRecup, parametri);
                        break;
                    }
                ...
                /*
                 * creo l'UD recupero e la sessione di recupero; inoltre nello step successivo
                 * produco un messaggio di warning per indicare che i dati saranno pronti il prima
                 * possibile
                 */
                tmpRecUnitaDocRecup = new RecUnitaDocRecup();
                tmpRecUnitaDocRecup.setAroUnitaDoc(
                        entityManager.find(AroUnitaDoc.class, parametri.getIdUnitaDoc()));
            ...
            if (creaSessioneRec) {
                if (rispostaControlli.getCodErr() == null
                        || rispostaControlli.getCodErr().isEmpty()) {
                    /*
                     * nota: rendo un codice errore ed un messaggio, ma pongo a true il valore
                     * boolean di risposta. In questo modo il chiamante può segnalare una notifica
                     * di Warning invece che di Errore.
                     */
                    /**
                     * Il recupero dei file dell'Unità Documentaria {0} sarà disponibile il prima
                     * possibile
                     */
                    rispostaControlli.setrBoolean(true);
                    rispostaControlli.setCodErr(MessaggiWSBundle.SESREC_001_001);
                    rispostaControlli.setDsErr(MessaggiWSBundle.getString(
                            MessaggiWSBundle.SESREC_001_001, parametri.getDescUnitaDoc()));
                }
                // crea riga sessione
                // crea righe date
    ...
    public RispostaControlli chiudiSessRecupero(RecuperoExt recupero) {
        ...
        try {
            ...
            query.setParameter("tiStatoSessioneRecupIn",
                    datiSessioneRecupero.getStatoSess().name());
            query.setParameter("idSessioneRecupIn",
                    datiSessioneRecupero.getIdRecSessioneRecupero());
    ...
    private void calcolaErrore(RispostaControlli rc, RecSessioneRecup rsr, ParametriRecupero par) {
        if (JobConstants.TipoSessioniRecupEnum.valueOf(rsr.getTiSessioneRecup()) != par
                .getTipoRichiedente()) {
            if (par.getTipoRichiedente() == JobConstants.TipoSessioniRecupEnum.DOWNLOAD) {
                /**
                 * Il recupero dei file dell'unità documentaria {0} è in corso da parte del servizio
                 * di recupero; per il momento non è possibile richiedere il recupero dell?unità
                 * documentaria mediante download on-line
                 */
                rc.setrBoolean(false);
                rc.setCodErr(MessaggiWSBundle.SESREC_002_005);
                rc.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SESREC_002_005,
                        par.getDescUnitaDoc()));
            } else {
                /**
                 * Il recupero dei file dell'unità documentaria {0} è in corso da parte di un
                 * operatore on-line; per il momento non è possibile richiedere il recupero
                 * dell?unità documentaria mediante servizio web
                 */
                rc.setrBoolean(false);
                rc.setCodErr(MessaggiWSBundle.SESREC_002_004);
                rc.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SESREC_002_004,
                        par.getDescUnitaDoc()));
            }
        } else if (rsr.getIamUser().getIdUserIam() != par.getUtente().getIdUtente()) {
            /**
             * E' già in corso il recupero di file dell'unità documentaria {0} da parte di un altro
             * utente
             */
            rc.setrBoolean(false);
            rc.setCodErr(MessaggiWSBundle.SESREC_002_003);
            rc.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SESREC_002_003,
                    par.getDescUnitaDoc()));
        } else if (CostantiDB.TipiEntitaRecupero.valueOf(rsr.getTiOutputRecup()) != par
                .getTipoEntitaSacer()) {
            /**
             * L'utente ha già in corso il recupero di file dell'unità documentaria {0}: si deve
             * aspettare che tale recupero sia terminato
             */
            rc.setrBoolean(false);
            rc.setCodErr(MessaggiWSBundle.SESREC_002_002);
            rc.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.SESREC_002_002,
                    par.getDescUnitaDoc()));
        }
    }
    ...
    private RispostaControlli creaSessioneRecupero(ParametriRecupero parametri,
        ...
        /*
         * nota bene: non c'è un blocco try... catch perché in caso di errore l'eccezione deve
        ...
        tmpRecSessioneRecup
                .setIamUser(entityManager.find(IamUser.class, parametri.getUtente().getIdUtente()));

        // nel caso sia richiesta una unità doc, normale, DIP, DIP per esibizione o per unisyncro,
        ...
        if (parametri.getTipoEntitaSacer() == CostantiDB.TipiEntitaRecupero.DOC
                ...
                || parametri
                        .getTipoEntitaSacer() == CostantiDB.TipiEntitaRecupero.DOC_DIP_ESIBIZIONE) {
            tmpRecSessioneRecup
                    .setAroDoc(entityManager.find(AroDoc.class, parametri.getIdDocumento()));
                ...
                || parametri
                        .getTipoEntitaSacer() == CostantiDB.TipiEntitaRecupero.COMP_DIP_ESIBIZIONE) {
            tmpRecSessioneRecup.setAroCompDoc(
                    entityManager.find(AroCompDoc.class, parametri.getIdComponente()));
        }
        ...
        if (datiSessioneRecupero.getDateDocumenti() != null) {
            for (Date data : datiSessioneRecupero.getDateDocumenti()) {
                ...
                // MAC#27666
                tmpDtVersRecup
                        .setDtVers(data.toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
                // end MAC#27666
                tmpDtVersRecup.setTiStatoDtVersRecup(datiSessioneRecupero.getStatoDtVers().name());
                /*
                 * per ogni record è definito l'indicatore che segnala se la data di versamento è
                 * definita per migrazione in funzione del fatto che sia antecedente o pari al
                 * parametro dataFineUsoBlob, oppure che sia successiva
                 */

...
public class RecuperoDip {
    ...
    public void contaComponenti(RispostaWSRecupero rispostaWs, RecuperoExt rec) {
        ...
        if (!controlli.isrBoolean()) {
            ...
            /*
             * da notare che nel caso il valore fosse 0. questo sarà da trattare come un errore nel ws e come un caso
             * normale - non viene mostrato il pulsante estrai/converti - nel caso dell'online. per questo qui non viene
             * impostato il flag ed il messaggio di errore
             */
        }
    ...
    public void recuperaCompConvertito(RispostaWSRecupero rispostaWs, RecuperoExt rec, String path) {
        StatoConservazione myEsito = rispostaWs.getIstanzaEsito();
        ...
        if (rec.getParametriRecupero().getTipoEntitaSacer() != CostantiDB.TipiEntitaRecupero.COMP_DIP) {
            rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            ...
            try {
                gestSessRecDip.recuperaCompConvertito(path, rec, rispostaWs.getDatiRecuperoDip().getElementiTrovati());
            } catch (Exception e) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
        ...
        if (salvaSessioneRecupero) {
            if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                rec.getDatiSessioneRecupero().setStatoSess(JobConstants.StatoSessioniRecupEnum.ELIMINATO);
                rec.getDatiSessioneRecupero().setStatoDtVers(JobConstants.StatoDtVersRecupEnum.RECUPERATA);
            } else {
                rec.getDatiSessioneRecupero().setStatoSess(JobConstants.StatoSessioniRecupEnum.CHIUSO_ERR);
                rec.getDatiSessioneRecupero().setStatoDtVers(JobConstants.StatoDtVersRecupEnum.ERRORE);
    ...
    public void recuperaUnitaDocumentaria(RispostaWSRecupero rispostaWs, RecuperoExt rec, String path) {
        StatoConservazione myEsito = rispostaWs.getIstanzaEsito();
        boolean salvaSessioneRecupero = false;
        ...
        if (rispostaWs.getSeverity() == SeverityEnum.OK) {
            salvaSessioneRecupero = true;
            try {
                gestSessRecDip.recuperaZip(path, rec, rispostaWs.getDatiRecuperoDip().getElementiTrovati());
            } catch (Exception e) {
                rispostaWs.setSeverity(SeverityEnum.ERROR);
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
    ...
    public void collaudaConvertitore(RispostaWSRecupero rispostaWs, long idConvertitore, String path) {

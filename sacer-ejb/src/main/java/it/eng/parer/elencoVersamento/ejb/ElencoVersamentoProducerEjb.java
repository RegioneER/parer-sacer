...
public class ElencoVersamentoProducerEjb {
    ...
    public void sendBatchJmsMessageGrouping(final Connection connection, final Session session,
            final MessageProducer messageProducer, OrgStrut struttura, final int numMax, final LogJob logJob,
            final Constants.TipoSessioneJMS jmsSession, final int jmsMsgChunk) throws JMSException {
            ...
            if (totMessiInCoda.get() < numMax) {
                ...
                if (maxResultUd > 0) {
                    iteratorChain.addIterator(
                            elencoHelper.retrieveUdToValidate(struttura.getIdStrut(), maxResultUd).iterator());
                }
                ...
                StreamSupport
                        ...
                        .filter(obj -> validator.validate(new CriterioRaggrValidation(criterio, obj.getUdDocUpdObj(),
                                obj.getAaKeyUnitaDoc(), obj.getDtCreazione())).isEmpty())
                        .forEach(obj -> {
                            ...
                            // TIP: invece di persistere il record di log "SET_ELENCO_APERTO" nella ELV_LOG_ELENCO_VERS
                            ...
                            try {
                                ...
                                throw new RuntimeException(
                                        String.format("Errore nell'invio del messaggio con JMSXGroupID %s in coda",
    ...
    public void sendMessage(final Session session, final MessageProducer messageProducer, final String jmsGroupId,
            final PayLoad pl) throws JMSException, JsonProcessingException {
    ...
    public void commitChunk(final Session session, final Integer totMessiInCoda, final int jmsMsgChunk)
            throws JMSException {
    ...
    public void commitLastChunk(final Session session, final Integer totMessiInCoda, final int jmsMsgChunk)
            throws JMSException {
        if (totMessiInCoda != 0 && totMessiInCoda % jmsMsgChunk != 0) {
            session.commit();
        }

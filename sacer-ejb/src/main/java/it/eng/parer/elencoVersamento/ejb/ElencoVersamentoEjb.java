...
public class ElencoVersamentoEjb {
    ...
    public void buildElencoVersamento(LogJob logJob) throws Exception {
        ...
        for (OrgAmbiente ambiente : ambienti) {
            ...
            // Ricavo i parametri
            final String DATA_SCAD_CHIUSURA_ELV_FISC = configHelper
                    .getValoreParamApplicByAmb(CostantiDB.ParametroAppl.DATA_SCAD_CHIUSURA_ELV_FISC, idAmbiente);
            final String NI_GG_CHIUSURA_ELV_FISC = configHelper
                    .getValoreParamApplicByAmb(CostantiDB.ParametroAppl.NI_GG_CHIUSURA_ELV_FISC, idAmbiente);
            final String ORARIO_CHIUSURA_ELV_FISC = configHelper
                    .getValoreParamApplicByAmb(CostantiDB.ParametroAppl.ORARIO_CHIUSURA_ELV_FISC, idAmbiente);
            final String ANNO_CHIUSURA_ELV_FISC = configHelper
                    .getValoreParamApplicByAmb(CostantiDB.ParametroAppl.ANNO_CHIUSURA_ELV_FISC, idAmbiente);
                ...
                if (oraCorrente >= oraChiusura && minutoCorrente >= minutoChiusura) {
                    ...
                    for (OrgStrut strut : struttureByAmb) {
                        ...
                        for (ElvElencoVer elenco : elenchi) {
                            ...
                            log.debug("CAV - trovato elenco {} fiscale da settare stato = DA_CHIUDERE",
        ...
        // MEV#27891
        final boolean flAbilitaElenchiBatch = Boolean.parseBoolean(configurationHelper
                .getValoreParamApplicByApplic(CostantiDB.ParametroAppl.FL_ABILITA_JOB_ELENCHI_BATCH));

        final int numMaxInCoda = Integer.parseInt(configurationHelper
                .getValoreParamApplicByApplic(CostantiDB.ParametroAppl.NUM_MAX_IN_CODA_ELENCHI_BATCH));
        ...
        final int jmsMsgChunk = Integer.parseInt(
                configurationHelper.getValoreParamApplicByApplic(CostantiDB.ParametroAppl.JMS_MSG_CHUNK_ELENCHI_BATCH));

        final String JMS_MSG_DELIVERY_ELENCHI_BATCH = configurationHelper
                .getValoreParamApplicByApplic(CostantiDB.ParametroAppl.JMS_MSG_DELIVERY_ELENCHI_BATCH);

        final DeliveryModeJMS jmsMsgDelivery = (JMS_MSG_DELIVERY_ELENCHI_BATCH.equals("PERSISTENT"))
                ? DeliveryModeJMS.PERSISTENT : DeliveryModeJMS.NON_PERSISTENT;
        // end MEV#27891
        ...
        for (OrgStrut struttura : strutture) {
            ...
            if (flAbilitaElenchiBatch) {
                ...
                if (!jmsSession.equals(TipoSessioneJMS.DISABLED)) {
                    manageStrutBatchJms(struttura.getIdStrut(), numMaxInCoda, logJob, jmsSession, jmsMsgChunk,
                            jmsMsgDelivery);
                } else {
                    manageStrutBatch(struttura.getIdStrut(), numMaxInCoda, logJob, numMaxInCoda < 0);
                }
                // end MAC#28020
            } else {
    ...
    public void manageStrutBatch(final long idStruttura, final int numMax, final LogJob logJob,
        ...
        log.info("{} --- CAE - Struttura: id = '{}' nome = '{}'", JobConstants.JobEnum.CREAZIONE_ELENCHI_VERS.name(),
                idStruttura, struttura.getNmStrut());
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manageBatchFilteredByView(final OrgStrut struttura, final int numMax, final LogJob logJob)
            throws Exception {
        ...
        for (DecCriterioRaggr criterio : criteriRaggrList) {
            log.debug("{} --- CAE - Criterio della struttura '{}' trovato: nome criterio = '{}' (id = '{}')",
            ...
            /*
             * Determino se per il criterio il numero massimo di elenchi che si puÃ² creare in un giorno e' non nullo
             */
            ...
            log.info("{} --- CAE - Elaborazione batch...", JobConstants.JobEnum.CREAZIONE_ELENCHI_VERS.name());

            /*
             * Determino le Unità  Documentarie, i Documenti Aggiunti e gli Aggiornamenti Metadati valide per il
             * criterio e itero l'insieme
             */
            Iterator<ElvVSelUdDocUpdByCrit> i = elencoHelper.streamUpdDocUdToProcess(criterio, numMax).iterator();
            try {
                // Itero l'insieme
                while (i.hasNext()) {
                    // Recupera l'elemento e sposta il cursore all'elemento successivo
                    ElvVSelUdDocUpdByCrit o = i.next();
                    // Nota: il controllo sull'iteratore (!i.hasNext(), "se non ho altri elementi"), mi serve per
                    // capire se è l'ultimo elemento
                    numElenchi = context.getBusinessObject(ElencoVersamentoEjb.class).manageUpdDocUdBatchFilteredByView(
                            criterio.getIdCriterioRaggr(), struttura.getIdStrut(), logJob.getIdLogJob(), o,
                            !i.hasNext(), isTheFirst, numElenchi);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void manageBatchToValidate(final OrgStrut struttura, final int numMax, final LogJob logJob)
            throws Exception {
        ...
        for (DecCriterioRaggr criterio : criteriRaggrList) {
            ...
            if (maxResultUd > 0) {
                iteratorChain
                        .addIterator(elencoHelper.retrieveUdToValidate(struttura.getIdStrut(), maxResultUd).iterator());
            }

            if (maxResultDoc > 0) {
                iteratorChain.addIterator(
                        elencoHelper.retrieveDocToValidate(struttura.getIdStrut(), maxResultDoc).iterator());
            }

            if (maxResultUpd > 0) {
                iteratorChain.addIterator(
                        elencoHelper.retrieveUpdToValidate(struttura.getIdStrut(), maxResultUpd).iterator());
            }
            ...
            try {
                ...
                while (iteratorChain.hasNext()) {
                    ...
                    // Nota: il controllo sull'iteratore (!i.hasNext(), "se non ho altri elementi"), mi serve per
                    // capire se è l'ultimo elemento
                    numElenchi = context.getBusinessObject(ElencoVersamentoEjb.class).manageUpdDocUdBatchToValidate(
                            criterio, struttura.getIdStrut(), logJob.getIdLogJob(), obj, !iteratorChain.hasNext(),
    ...
    public void manageStrutBatchJms(final long idStruttura, final int numMax, final LogJob logJob,
            final TipoSessioneJMS jmsSession, final int jmsMsgChunk, final DeliveryModeJMS jmsMsgDelivery)
            throws Exception {
        ...
        try {
            ...
            messageProducer.setDeliveryMode((Constants.DeliveryModeJMS.PERSISTENT.equals(jmsMsgDelivery))
                    ? DeliveryMode.PERSISTENT : DeliveryMode.NON_PERSISTENT);
            messageProducer.setPriority(0);

            ElencoVersamentoEjb newEjbRef1 = context.getBusinessObject(ElencoVersamentoEjb.class);
            newEjbRef1.sendBatchJms(connection, session, messageProducer, idStruttura, numMax, logJob, jmsSession,
                    jmsMsgChunk);

        } catch (JMSException ex) {
            throw new RuntimeException(
                    String.format("Errore nell'invio dei messaggi con JMSXGroupID %s in coda", idStruttura));
        } finally {
            try {
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void sendBatchJms(final Connection connection, final Session session, final MessageProducer messageProducer,
            final long idStruttura, final int numMax, final LogJob logJob, final TipoSessioneJMS jmsSession,
            final int jmsMsgChunk) throws Exception {
        ...
        OrgStrut struttura = elencoHelper.retrieveOrgStrutByid(new BigDecimal(idStruttura));
        // gestisco gli elenchi scaduti
        log.info("{} --- CAE - Struttura: id = '{}' nome = '{}'", JobConstants.JobEnum.CREAZIONE_ELENCHI_VERS,
                idStruttura, struttura.getNmStrut());
        elaboraElenchiScaduti(idStruttura, logJob.getIdLogJob());

        elencoVersamentoProducerEjb.sendBatchJmsMessageGrouping(connection, session, messageProducer, struttura, numMax,
    ...
    public void manageStrut(long idStruttura, LogJob logJob) throws Exception {
        ...
        for (DecCriterioRaggr criterio : criteriRaggr) {
            log.debug("CAE - Criterio della struttura '{}' trovato: nome criterio = '{}' (id = '{}')",
                    struttura.getNmStrut(), criterio.getNmCriterioRaggr(), criterio.getIdCriterioRaggr());
            ...
            /*
             * Determino le UnitÃ  Documentarie, i Documenti Aggiunti e gli Aggiornamenti Metadati che soddisfano il
             * criterio corrente
             */
            List<UpdDocUdObj> updDocUdObjectList = elencoHelper.retrieveUpdDocUdToProcess(criterio);
            log.debug("CAE - Trovati {} oggetti versati relativi al criterio '{}'", updDocUdObjectList.size(),
    ...
    public Long manageUpdDocUdBatchFilteredByView(long idCriterio, long idStruttura, Long idLogJob,
            ElvVSelUdDocUpdByCrit updDocUdObj, boolean isTheLast, boolean isTheFirst, Long numElenchi)
            throws Exception {
        Long numElenchiTmp = null;
        ...
        case "01_UNI_DOC":
            numElenchiTmp = manageUd(updDocUdObj.getElvVSelUdDocUpdByCritId().getIdUnitaDoc(),
                    updDocUdObj.getAaKeyUnitaDoc(), idCriterio, idStruttura, idLogJob, isTheLast, isTheFirst,
                    numElenchi);
            break;
        case "02_DOC_AGG":
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public Long manageUpdDocUdBatchToValidate(final DecCriterioRaggr criterio, long idStruttura, Long idLogJob,
            ElvUdDocUpdDaElabElenco<?> updDocUdObj, boolean isTheLast, boolean isTheFirst, Long numElenchi)
            throws Exception {
        Long numElenchiTmp = null;
        ...
        if (obj instanceof AroUnitaDoc) {
            ...
            CriterioRaggrValidation<AroUnitaDoc> critRaggrValidation = new CriterioRaggrValidation<>(criterio, ud,
                    updDocUdObj.getAaKeyUnitaDoc(), updDocUdObj.getDtCreazione());
            Set<ConstraintViolation<CriterioRaggrValidation>> violations = validator.validate(critRaggrValidation);
            if (violations.isEmpty()) {
                numElenchiTmp = manageUd(new BigDecimal(ud.getIdUnitaDoc()), updDocUdObj.getAaKeyUnitaDoc(),
                        criterio.getIdCriterioRaggr(), idStruttura, idLogJob, isTheLast, isTheFirst, numElenchi);
            }
            ...
            Set<ConstraintViolation<CriterioRaggrValidation>> violations = validator.validate(critRaggrValidation);
            if (violations.isEmpty()) {
                numElenchiTmp = manageDoc(new BigDecimal(doc.getIdDoc()), updDocUdObj.getAaKeyUnitaDoc(),
                        criterio.getIdCriterioRaggr(), idStruttura, idLogJob, isTheLast, isTheFirst, numElenchi);
            }
            ...
            Set<ConstraintViolation<CriterioRaggrValidation>> violations = validator.validate(critRaggrValidation);
            if (violations.isEmpty()) {
                numElenchiTmp = manageUpd(new BigDecimal(upd.getIdUpdUnitaDoc()), updDocUdObj.getAaKeyUnitaDoc(),
                        criterio.getIdCriterioRaggr(), idStruttura, idLogJob, isTheLast, isTheFirst, numElenchi);
            }
        }
    ...
    private Long manageUd(BigDecimal udId, BigDecimal aaKeyUnitaDoc, long idCriterio, long idStruttura, Long idLogJob,
        ...
        if (firstCheckUdOk) { // l'ud sta nell'elenco: aggiungo.
            ...
            if (numElenchi != null) {
                // per il criterio il numero elenchi creati in un giorno e' non nullo
                // se aggiungendo un elenco non si supera il numero massimo di elenchi che si puÃ² creare in un giorno
                // ne
                // creo uno nuovo
                numElenchi++;
                if (numElenchi <= criterio.getNiMaxElenchiByGg().longValue()) {
                ...
                elenco = createElencoByCriterio(criterio, aaKeyUnitaDoc, struttura); // questo volume Ã¨ managed
            }
            // Aggiugo unitÃ  doc ad elenco sono dopo aver controllato se ci sta. Se non ci sta Ã¨ un problema di
            ...
            if (secondCheckUdOk) { // l'ud sta nell'elenco: aggiugo.
                ...
                throw new ParerInternalError("ATTENZIONE non Ã¨ possibile aggiungere l'ud '" + ud.getIdUnitaDoc()
    ...
    private Long manageDoc(BigDecimal idDoc, BigDecimal aaKeyUnitaDoc, long idCriterio, long idStruttura, Long idLogJob,
        ...
        long numComps = (long) numSizeArray[0];
        BigDecimal sizeComps = (BigDecimal) numSizeArray[1];
        // ATTENZIONE: verifico se il numero di componenti del documento corrente, Ã¨ inferiore o uguale al numero di
        // componenti che l'elenco puÃ² ancora includere
        // (tale numero Ã¨ definito dal numero massimo di componenti previsto dal volume a cui si sottrae
        // il numero di componenti derivanti da unitÃ  doc versate giÃ  incluse nell'elenco, il numero di componenti
        ...
        if (firstCheckDocOk) { // il doc sta nell'elenco: aggiungo.
            ...
            evEjb.registraStatoElencoVersamento(BigDecimal.valueOf(elenco.getIdElencoVers()),
                    TIPO_ELENCO_VERS_DA_CHIUDERE, DS_SUPERO_MAX_COMPONENTI, ElvStatoElencoVer.TiStatoElenco.DA_CHIUDERE,
                    null);
            /* Creazione elenco per criterio */
            if (numElenchi != null) {
                // per il criterio il numero elenchi creati in un giorno e' non nullo
            ...
            // Aggiungo il doc ad elenco solo dopo aver controllato se ci sta. Se non ci sta Ã¨ un problema di
            // configurazione del criterio
            ...
            if (secondCheckDocOk) { // il doc sta nell'elenco: aggiungo.
                ...
                throw new ParerInternalError("ATTENZIONE non Ã¨ possibile aggiungere il doc '" + doc.getIdDoc()
                        + "' all'elenco. Possibile errore nella definizione del criterio");
            }
        } else {
            log.warn(LOG_ERR_CONF_CRIT, doc.getIdDoc());
    ...
    private Long manageUpd(BigDecimal idUpdUnitaDoc, BigDecimal aaKeyUnitaDoc, long idCriterio, long idStruttura,
        ...
        // il numero di componenti derivanti da unitÃ  doc versate giÃ  incluse nell'elenco, il numero di componenti
        // derivanti da documenti giÃ  inclusi nell'elenco
        // ed il numero di aggiornamento metadati per unitÃ  doc giÃ  inclusi nellâ€™elenco) e' maggiore o uguale ad 1
        ...
        if (firstCheckUpdOk) { // la upd sta nell'elenco: aggiungo.
            ...
            // Aggiungo la upd ad elenco solo dopo aver controllato se ci sta. Se non ci sta Ã¨ un problema di
            // configurazione del criterio
            ...
            if (secondCheckUpdOk) { // la upd sta nell'elenco: aggiungo.
                ...
                throw new ParerInternalError("ATTENZIONE non Ã¨ possibile aggiungere la upd '" + upd.getIdUpdUnitaDoc()
                        + "' all'elenco. Possibile errore nella definizione del criterio");
            }
        } else {
            log.warn(LOG_ERR_CONF_CRIT, upd.getIdUpdUnitaDoc());
    ...
    public ElvElencoVer findOpenedElenco(DecCriterioRaggr criterio, BigDecimal aaKeyUnitaDoc, OrgStrut struttura,
        ...
        try {
            elenco = elencoHelper.retrieveElencoByCriterio(criterio, aaKeyUnitaDoc, struttura); // questo elenco Ã¨
            // managed
            ...
            log.debug("CAE - Elenco aperto trovato: nome = {}; data di scadenza = {}; numero componenti versati = {};"
                    + " numero componenti aggiunti= {} ; numero aggiornamenti metadati= {}; per un totale di {}",
                    elenco.getNmElenco(), dateToString(elenco.getDtScadChius()), elenco.getNiCompVersElenco(),
                    elenco.getNiCompAggElenco(), elenco.getNiUpdUnitaDoc(), sommaCompAggCompVersUpd);
    ...
    private void addUnitaDocIntoElenco(AroUnitaDoc ud, ElvElencoVer elenco, long numDocs, long numComp,
        ...
        // end MAC#28020
        log.debug("Num doc da agg: {}; num comp da agg: {}; sizeComps: {}", numDocs, numComp, sizeComps);
        // aggiorno l'elenco corrente, incrementando di 1 il numero delle unitÃ  doc versate incluse nell'elenco
        ...
        // aggiorno l'unitÃ  documentaria corrente, assegnando stato = IN_ELENCO_APERTO e valorizzando la FK all'elenco
        // corrente
        ud.setTiStatoUdElencoVers(ElencoEnums.UdDocStatusEnum.IN_ELENCO_APERTO.name());
        ud.setElvElencoVer(elenco);
        log.debug("aggiorno AroUnitaDoc idUnitaDoc={} imposto stato IN_ELENCO_APERTO e idElencoVers={}",
                ud.getIdUnitaDoc(), elenco.getIdElencoVers());
        elenco.getAroUnitaDocs().add(ud);
        // elimino l'unitÃ  documentaria corrente dalla coda delle unitÃ  documentarie da elaborare per gli elenchi
    ...
    private void addDocIntoElenco(AroDoc doc, ElvElencoVer elenco, long numComps, BigDecimal sizeComps,
        ...
        // MAC#28020
        boolean checkAddDocAgg = (!elencoHelper.containsElenco(elenco)) ? checkAddDocAggNoEleCor(doc)
        ...
        if (checkAddDocAgg) {
            ...
            // la dimensione puÃ² essere nulla quando inserisco un componente con tipo supporto = metadati
            if (sizeComps == null) {
                sizeComps = BigDecimal.ZERO;
            }
    ...
    private void addUpdIntoElenco(AroUpdUnitaDoc upd, ElvElencoVer elenco, OrgStrut struttura, LogJob logJob) {
        // MAC#28020
        ...
        if (checkAddUpdUd) {
            ...
            gestisciSalvataggioElenco(elenco, struttura, logJob);
            // end MAC#28020
            // aggiorno l'elenco, incrementando di 1 il numero aggiornamenti metadati inclusi nellâ€™elenco
    ...
    public void setDaChiudereAtomic(String closeReason, Long idElenco, long idStruttura, long logJobId) {
        ...
        ElvStatoElencoVer.TiStatoElenco tiStatoelenco = it.eng.parer.entity.constraint.ElvElencoVer.TiValidElenco.NO_INDICE
                .equals(elenco.getTiValidElenco()) ? ElvStatoElencoVer.TiStatoElenco.CHIUSO
                        : ElvStatoElencoVer.TiStatoElenco.DA_CHIUDERE;
        // EVO 19304
        evEjb.registraStatoElencoVersamento(BigDecimal.valueOf(elenco.getIdElencoVers()), TIPO_ELENCO_VERS_DA_CHIUDERE,
                DS_ELENCO_SCADUTO, tiStatoelenco, null);
    }

    // MAC#28020
    /**
     * Nuovo metodo introdotto con la mac 28020: se l'elenco corrente Ã¨ stato creato da zero non Ã¨ "managed" perchÃ©
     * non Ã¨ ancora stata chiamata la persist dell'elenco, quest'ultima verrÃ  effettuata solo se l'inserimento della
     * prima ud/doc/upd ha successo.
     *
     * @param elenco
     *            elenco oggetto dell'inserimento
     * @param struttura
    ...
    private void gestisciSalvataggioElenco(ElvElencoVer elenco, OrgStrut struttura, LogJob logJob) {
        if (!elencoHelper.containsElenco(elenco)) {
            ...
            // EVO 19304
            evEjb.registraStatoElencoVersamento(BigDecimal.valueOf(elenco.getIdElencoVers()), "CREAZIONE_ELENCO_VERS",
                    null, ElvStatoElencoVer.TiStatoElenco.APERTO, null);

            elencoHelper.writeLogElencoVers(elenco, struttura, ElencoEnums.OpTypeEnum.CREA_ELENCO.name(), logJob);

            log.debug("CVC - Creato nuovo elenco: nome = {}; data scadenza = {}", elenco.getNmElenco(),
    ...
    /**
     * Nuovo metodo introdotto con la mev 24534: se l'elenco ha come tipo validazione <em>NO_INDICE</em> non ne verrÃ 
    ...
    private void setChiuso(String closeReason, ElvElencoVer elenco, OrgStrut struttura, LogJob logJob) {
        ...
        /*
         * Non aggiorno piÃ¹ l'elenco con il numero delle unitÃ  doc modificate incluse nell'elenco, lo faccio nella
         * addDocIntoElenco
         */
        ...
        // il sistema aggiorna l'elenco con il numero delle unitÃ  documentarie modificate incluse nell'elenco a causa
        // di
        // documenti aggiunti
        ...
        // il sistema assegna ad ogni unitÃ  documentaria appartenente all'elenco stato = IN_ELENCO_CHIUSO
        ...
        // il sistema assegna ad ogni aggiornamento unitÃ  doc appartenente all'elenco stato = IN_ELENCO_CHIUSO
    ...
    public void setDaChiudereFiscAtomic(String closeReason, Long idElenco, long idStruttura, long logJobId) {
        ...
        ElvStatoElencoVer.TiStatoElenco tiStatoelenco = it.eng.parer.entity.constraint.ElvElencoVer.TiValidElenco.NO_INDICE
                .equals(elenco.getTiValidElenco()) ? ElvStatoElencoVer.TiStatoElenco.CHIUSO
                        : ElvStatoElencoVer.TiStatoElenco.DA_CHIUDERE;
        // EVO 19304
        evEjb.registraStatoElencoVersamento(BigDecimal.valueOf(idElenco), TIPO_ELENCO_VERS_DA_CHIUDERE,
    ...
    private void setDaChiudere(String closeReason, ElvElencoVer elenco, OrgStrut struttura, LogJob logJob) {
        ...
        elenco.setNiUnitaDocModElenco(
                new BigDecimal(elencoHelper.contaUdModificatePerByDocAggiuntiByUpd(elenco.getIdElencoVers())));
        // il sistema assegna ad ogni unitÃ  documentaria appartenente all'elenco stato = IN_ELENCO_DA_CHIUDERE
        List<AroUnitaDoc> udDocList = elencoHelper.retrieveUdDocsInElenco(elenco);
        for (AroUnitaDoc ud : udDocList) {
        ...
        // il sistema assegna ad ogni aggiornamento unitÃ  doc appartenente allâ€™elenco stato = IN_ELENCO_DA_CHIUDERE
        List<AroUpdUnitaDoc> updList = elencoHelper.retrieveUpdsInElenco(elenco);
        for (AroUpdUnitaDoc upd : updList) {
    ...
    private Date adjustElencoDate(Date plainDate, String tiTempo, BigDecimal niTempo, String opType) {
        ...
        if (tiTempo != null && niTempo != null) {
            ...
            if (ElencoEnums.TimeTypeEnum.MINUTI.name().equals(tiTempo)) {
                ...
                String newdate = dateformat.format(cal.getTime());
                log.debug(LOG_CAE_AUMENTO_DI_SCADENZA, tempo, ElencoEnums.TimeTypeEnum.MINUTI, newdate);
            }
            if (ElencoEnums.TimeTypeEnum.ORE.name().equals(tiTempo)) {
                cal.add(Calendar.HOUR_OF_DAY, tempo);
                String newdate = dateformat.format(cal.getTime());
                log.debug(LOG_CAE_AUMENTO_DI_SCADENZA, tempo, ElencoEnums.TimeTypeEnum.ORE, newdate);
            }
            if (ElencoEnums.TimeTypeEnum.GIORNI.name().equals(tiTempo)) {
                cal.add(Calendar.DAY_OF_WEEK, tempo);
                String newdate = dateformat.format(cal.getTime());
                log.debug(LOG_CAE_AUMENTO_DI_SCADENZA, tempo, ElencoEnums.TimeTypeEnum.GIORNI, newdate);
            }
        }
        return cal.getTime();
    }
    ...
    private boolean checkElencoExpired(ElvElencoVer elenco) {
        ...
        log.debug("CAV - Verifico se l'elenco '{}' con data scadenza {} Ã¨ scaduto all'istante corrente ({})",
                elenco.getNmElenco(), dateToString(elenco.getDtScadChius()), dateToString(actualDate));
    ...
    private void manageLast(ElvElencoVer elenco, OrgStrut struttura, LogJob logJob) {
        if (elenco.getNiUnitaDocVersElenco().intValue() > 0 || elenco.getNiDocAggElenco().intValue() > 0
            ...
            /*
             * Aggiorno l'elenco con il numero delle unitÃ  documentarie modificate incluse nell'elenco a causa di
             * documenti aggiunti e/o di aggiornamenti metadati unitÃ  doc mediante la vista ELV_V_COUNT_UD_MODIF
             */
            elenco.setNiUnitaDocModElenco(
                    new BigDecimal(elencoHelper.contaUdModificatePerByDocAggiuntiByUpd(elenco.getIdElencoVers())));
            /*
             ...
             */
            if (logJob != null) {
                elencoHelper.writeLogElencoVers(elenco, struttura, ElencoEnums.OpTypeEnum.SET_ELENCO_APERTO.name(),
                        logJob);
            }
        }
    }
    ...
    public void elaboraElenchiVuoti(long idStruttura) {
        ...
        log.info("CAV - trovati {} elenchi di versamento vuoti da rimuovere", elenchiVuotiDaProcessare.size());
    ...
    /*
     * Metodo che controlla che: 1) la unitÃ  doc a cui appartiene il documento 2) i documenti aggiunti dellâ€™unitÃ 
     * doc a cui appartiene il documento versati prima del documento aggiunto siano presenti nellâ€™elenco corrente
     * oppure per essi sia definita la FK ad un altro elenco e che tale elenco abbia stato INDICI_AIP_GENERATI o
     * ELENCO_INDICI_AIP_CREATO o ELENCO_INDICI_AIP_FIRMATO o ELENCO_INDICI_AIP_ERR_MARCA o COMPLETATO; se controllo
     * fallisce, lâ€™elemento viene scartato e si passa al successivo
     */
    ...
    // MAC#28020
    /*
     * Metodo che controlla che per: 1) la unitÃ  doc a cui appartiene il documento 2) i documenti aggiunti
     * dellâ€™unitÃ  doc a cui appartiene il documento versati prima del documento aggiunto sia definita la FK ad un
     * altro elenco e che tale elenco abbia stato INDICI_AIP_GENERATI o ELENCO_INDICI_AIP_CREATO o
     * ELENCO_INDICI_AIP_FIRMATO o ELENCO_INDICI_AIP_ERR_MARCA o COMPLETATO; se controllo fallisce, lâ€™elemento viene
    ...
    /*
     * Metodo che controlla che: 1) la unitÃ  doc a cui appartiene l'aggiornamento 2) tutti i documenti aggiunti
     * presenti nell'aggiornamento 3) tutti gli aggiornamenti precedenti (con progressivo minore di quello corrente)
     * siano presenti nell'elenco corrente oppure per essi sia definita la FK ad un altro elenco e che tale elenco abbia
     * stato INDICI_AIP_GENERATI o ELENCO_INDICI_AIP_CREATO o ELENCO_INDICI_AIP_FIRMATO o ELENCO_INDICI_AIP_ERR_MARCA o
    ...
    // MAC#28020
    /*
     * Metodo che controlla che per: 1) la unitÃ  doc a cui appartiene l'aggiornamento 2) tutti i documenti aggiunti
     * presenti nell'aggiornamento 3) tutti gli aggiornamenti precedenti (con progressivo minore di quello corrente) sia
     * definita la FK ad un altro elenco e che tale elenco abbia stato INDICI_AIP_GENERATI o ELENCO_INDICI_AIP_CREATO o
     * ELENCO_INDICI_AIP_FIRMATO o ELENCO_INDICI_AIP_ERR_MARCA o COMPLETATO; se controllo fallisce, l'elemento viene
     * scartato e si passa al successivo
     */
    private boolean checkAddUpdUdNoEleCor(AroUpdUnitaDoc upd) {
        ...
        if ("1".equals(chkAddUpdUdNoEleCor.getFlAddUpdUdOk()) && "1".equals(chkAddUpdUdNoEleCor.getFlAllAddUpdDocOk())

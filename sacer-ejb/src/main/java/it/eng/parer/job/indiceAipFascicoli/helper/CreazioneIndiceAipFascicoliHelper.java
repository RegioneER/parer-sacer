...
public class CreazioneIndiceAipFascicoliHelper extends GenericHelper {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(CreazioneIndiceAipFascicoliHelper.class);
	...
	try {
	    ...
	    log.warn(
		    "Creazione Indice AIP fascicolo - Eccezione durante la conversione del parametro di configurazione "
			    + CostantiDB.ParametroAppl.MAX_FETCH_INDICE_AIP_FASC
			    + " uso il default.");
	}
	...
	String queryStr = "SELECT u FROM FasVerAipFascicolo u "
		+ "WHERE u.fasFascicolo.idFascicolo = :idFascicolo "
     ...
     *
     * @param idFascicolo id fascicolo
     * @param tiCreazione tipo creazione
     *
	...
	try {
		...
		// Ricavo l'ultima versione dell'AIP di tipo "ANTICIPATO" (cdVerAip che comincia per
		// 0.)"
		queryStr = "SELECT u FROM FasVerAipFascicolo u "
			+ "WHERE u.fasFascicolo.idFascicolo = :idFascicolo "
		...
		// Ricavo l'ultima versione dell'AIP di tipo "ARCHIVIO" (cdVerAip che comincia per
		// 1.)"
		queryStr = "SELECT u FROM FasVerAipFascicolo u "
			+ "WHERE u.fasFascicolo.idFascicolo = :idFascicolo "
     ...
     *
     * @param idFascicolo id unita doc
     * @param tiCreazione tipo creazione
     *
     * @return la nuova versione
     *
     ...
     */
    @Deprecated
    public String getVersioneIndiceAIPV2(Long idFascicolo, String tiCreazione)
     ...
     *
     * @param idFascicolo id fascicolo
     * @param tiXmlVers   tipo xml versamento
     *
     ...
     * @throws ParerInternalError errore generico
     */
    public String getVersioneMetadatiAIPV2(Long idFascicolo, String tiCreazione)
	    throws ParerInternalError {
	String versione = null;
	try {
	    String queryStr;
		...
		// Se non ho risultati significa che sto inserendo la prima versione dei metadati
		// del fascicolo
		versione = "2.0";
	    }
	    // }
    ...
    /**
     * Metodo transazionale per la registrazione dell'indice AIP attraverso la memorizzazione dei
     * record nelle apposite entity.
     *
     * @param fascDaElab           entity FasAipFascicoloDaElab
     * @param progressivoVersione  progressivo
	...
	verAipFascicolo.setIdEnteConserv(
		fasFascicolo.getOrgStrut().getOrgEnte().getOrgAmbiente().getIdEnteConserv());
	// calcolo parte urn ORIGINALE
	String tmpUrn = MessaggiWSFormat.formattaBaseUrnFascicolo(
	...
	String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnFascicolo(
		...
		MessaggiWSFormat.formattaUrnPartFasc(chiaveFasc, true,
	...
	// salvo NORMALIZZATO
	verAipFascicolo
		.setDsUrnNormalizAipFascicolo(MessaggiWSFormat.formattaUrnAipFascicolo(tmpUrnNorm));
	fasFascicolo.getFasVerAipFascicolos().add(verAipFascicolo);

	/* Inserisco FAS_CONTEN_VER_AIP_FASCICOLO */
	for (FasUdAipFascicoloDaElab udAipFascicoloDaElab : fascDaElab
		.getFasUdAipFascicoloDaElabs()) {
	    ...
	    chiave.setTipoRegistro(udAipFascicoloDaElab.getAroVerIndiceAipUd().getAroIndiceAipUd()
		    .getAroUnitaDoc().getCdRegistroKeyUnitaDoc());
	    chiave.setAnno(udAipFascicoloDaElab.getAroVerIndiceAipUd().getAroIndiceAipUd()
		    .getAroUnitaDoc().getAaKeyUnitaDoc().longValue());
	    chiave.setNumero(udAipFascicoloDaElab.getAroVerIndiceAipUd().getAroIndiceAipUd()
		    .getAroUnitaDoc().getCdKeyUnitaDoc());
	    // <sistemaconservazione>_<ente>_<struttura>_<registro>-<anno>-<numero>_IndiceAIP-UD-<versione>.xml
	    contenVerAipFascicolo.setNmConten(
		    MessaggiWSFormat.formattaChiaveUdFull(versatore, chiave) + "_IndiceAIP-UD-"
			    + udAipFascicoloDaElab.getAroVerIndiceAipUd().getCdVerIndiceAip()
	...
	FasXmlVersFascicolo fasXmlVersFascicoloRich = IterableUtils.find(
	...
	FasXmlVersFascicolo fasXmlVersFascicoloRisp = IterableUtils.find(
		fasFascicolo.getFasXmlVersFascicolos(),
	...
	Query query = entityManager.createQuery(
		"SELECT lisIxAipFascByEle FROM ElvVLisIxAipFascByEle lisIxAipFascByEle "
    ...
    /**
     * Ricava il modello xsd attivo per l'ambiente di appartenenza della struttura a cui il
     * fascicolo appartiene e per il tipo in input
     *
     ...
     */
    public List<DecModelloXsdFascicolo> retrieveIdModelliDaElaborare(long idAmbiente,
	    String tiModelloXsd) {
	String queryStr = "SELECT modello " + "FROM DecModelloXsdFascicolo modello "
		+ "WHERE modello.orgAmbiente.idAmbiente = :idAmbiente "
		+ "AND modello.tiModelloXsd = :tiModelloXsd "
    ...
    /**
     * Ricava il modello xsd attivo per l'ambiente di appartenenza della struttura a cui il
     * fascicolo appartiene e per la versione del modello xsd corrispondente a quella del servizio
     * di versamento fascicolo, con il tipo "AIP_UNISYNCRO"
     *
     * @param idAmbiente    id ambiente
     * @param tiModelloXsd  tipo modello
     * @param cdVersioneXml versione del servizio di versamento fascicolo
     *
     * @return lista oggetti di tipo {@link DecModelloXsdFascicolo}
     */
    public List<DecModelloXsdFascicolo> retrieveIdModelliDaElaborareV2(long idAmbiente,
	    String tiModelloXsd, String cdVersioneXml) {
	String queryStr = "SELECT modello " + "FROM DecModelloXsdFascicolo modello "
		+ "WHERE modello.orgAmbiente.idAmbiente = :idAmbiente "
		+ "AND modello.tiModelloXsd = :tiModelloXsd "
		+ "AND modello.dtIstituz <= :filterDate AND modello.dtSoppres >= :filterDate "
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public FasMetaVerAipFascicolo registraFasMetaVerAipFascicolo(FasVerAipFascicolo verAipFascicolo,
	    String hash, String algoHash, String encodingHash, String codiceVersione,
	    CSVersatore versatore, CSChiaveFasc chiaveFasc) {
	...
	String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnFascicolo(
		...
		MessaggiWSFormat.formattaUrnPartFasc(chiaveFasc, true,
			Costanti.UrnFormatter.FASC_FMT_STRING));
	// salvo ORIGINALE
	fasMetaVerAipFascicolo.setDsUrnMetaFascicolo(MessaggiWSFormat.formattaUrnIndiceAipFascicoli(
		tmpUrn, codiceVersione, Costanti.UrnFormatter.URN_INDICE_AIP_FASC_FMT_STRING_V2));
	// salvo NORMALIZZATO
	fasMetaVerAipFascicolo.setDsUrnNormalizMetaFascicolo(
		MessaggiWSFormat.formattaUrnIndiceAipFascicoli(tmpUrnNorm, codiceVersione,
    ...
    public FasFileMetaVerAipFasc registraFasFileMetaVerAipFasc(long idMetaVerAipFascicolo,
	    String file, OrgStrut strut, Date dtCreazione, BackendStorage backendMetadata,
	    Map<String, String> indiceAipFascicoloBlob) {
	FasFileMetaVerAipFasc fileMetaVerAipFasc = new FasFileMetaVerAipFasc();
	FasMetaVerAipFascicolo fasMetaVerAipFascicolo = getEntityManager()
		.find(FasMetaVerAipFascicolo.class, idMetaVerAipFascicolo);
    ...
    public FasXsdMetaVerAipFasc registraFasXsdMetaVerAipFasc(long idMetaVerAipFascicolo,
	    long idModelloXsdFascicolo, String nmXsd) {
	FasXsdMetaVerAipFasc xsdMetaVerAipFasc = new FasXsdMetaVerAipFasc();
	FasMetaVerAipFascicolo fasMetaVerAipFascicolo = getEntityManager()
		.find(FasMetaVerAipFascicolo.class, idMetaVerAipFascicolo);
	DecModelloXsdFascicolo decModelloXsdFascicolo = getEntityManager()
		.find(DecModelloXsdFascicolo.class, idModelloXsdFascicolo);

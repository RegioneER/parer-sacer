...
public class AllineamentoOrganizzazioniHelper {
	...
	String queryStr = "SELECT u FROM DecTipoUnitaDoc u "
		+ "WHERE u.orgStrut.idStrut IN (:idStruts) ";
	...
	String queryStr = "SELECT u FROM DecTipoDoc u "
		+ "WHERE u.orgStrut.idStrut IN (:idStruts) ";
	Query q = entityManager.createQuery(queryStr);
	q.setParameter("idStruts", idStruts);
	return q.getResultList();
	...
	String queryStr = "SELECT u FROM DecRegistroUnitaDoc u "
		+ "WHERE u.orgStrut.idStrut IN (:idStruts) ";
	Query q = entityManager.createQuery(queryStr);
	q.setParameter("idStruts", idStruts);
	return q.getResultList();
	...
	String queryStr = "SELECT u FROM OrgSubStrut u "
		+ "WHERE u.orgStrut.idStrut IN (:idStruts) ";
	Query q = entityManager.createQuery(queryStr);
	q.setParameter("idStruts", idStruts);
	return q.getResultList();
	...
	String queryStr = "SELECT u FROM DecTipoFascicolo u "
		+ "WHERE u.orgStrut.idStrut IN (:idStruts) ";
	Query q = entityManager.createQuery(queryStr);
	q.setParameter("idStruts", idStruts);
	return q.getResultList();
    ...
    public void writeEsitoIamOrganizDaReplic(Long idOrganizDaReplic,
	    CostantiReplicaOrg.EsitoServizio esitoServizio, String cdErr, String dsErr) {
	...
	if (esitoServizio == CostantiReplicaOrg.EsitoServizio.OK) {
		...
		organizDaReplic.setTiStatoReplic(
			CostantiReplicaOrg.TiStatoReplic.REPLICA_IN_ERRORE.name());
		...
		organizDaReplic
			.setTiStatoReplic(CostantiReplicaOrg.TiStatoReplic.REPLICA_OK.name());
		organizDaReplic.setCdErr(null);
		organizDaReplic.setDsMsgErr(null);
		organizDaReplic.setDtErr(null);
		break;
		...
		organizDaReplic.setTiStatoReplic(
			CostantiReplicaOrg.TiStatoReplic.REPLICA_NON_POSSIBILE.name());
		organizDaReplic.setCdErr(cdErr);
		organizDaReplic.setDsMsgErr(dsErr);
		organizDaReplic.setDtErr(new Date());
		break;
	    ...
	    organizDaReplic
		    .setTiStatoReplic(CostantiReplicaOrg.TiStatoReplic.REPLICA_IN_TIMEOUT.name());
	    organizDaReplic.setCdErr(cdErr);
	    organizDaReplic.setDsMsgErr(dsErr);
	    organizDaReplic.setDtErr(new Date());
	}

...
public class ElaborazioneElencoIndiceAipFascicoli {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ElaborazioneElencoIndiceAipFascicoli.class);
    ...
    public void creaElencoIndiciAIPFascicoli(long idElencoVersFasc, long idLogJob) throws ParerInternalError,
            IOException, DatatypeConfigurationException, JAXBException, ParseException, NoSuchAlgorithmException {
        ElvElencoVersFasc elenco = ciafHelper.findByIdWithLock(ElvElencoVersFasc.class, idElencoVersFasc);
        ...
        if (fascSoloAnnul.getFlSoloFascAnnul().equals("1")) {
            ...
            ElencoIndiciAIP elencoIndiciAip = creaElencoIndiceAipFascicoliMarshallObject(elenco, sistema);
            ...
            fileElencoVersFasc.setTiFileElencoVers(ElencoEnums.FileTypeEnum.ELENCO_INDICI_AIP.name());
            ...
            // salvo ORIGINALE
            final String urnElencoIndiciAIP = MessaggiWSFormat.formattaUrnElencoIndiciAIPFascicoliNonFirmati(
                    MessaggiWSFormat.formattaUrnPartVersatore(versatore), Long.toString(elenco.getIdElencoVersFasc()),
                    sdf.format(elenco.getTsCreazioneElenco()));
            fileElencoVersFasc.setDsUrnFile(urnElencoIndiciAIP);
            // salvo NORMALIZZATO
            final String urnNormalizElencoIndiciAIP = MessaggiWSFormat.formattaUrnElencoIndiciAIPFascicoliNonFirmati(
                    MessaggiWSFormat.formattaUrnPartVersatore(versatore, true, Costanti.UrnFormatter.VERS_FMT_STRING),
                    Long.toString(elenco.getIdElencoVersFasc()), sdf.format(elenco.getTsCreazioneElenco()));
            ...
            elenco.setNiIndiciAip(new BigDecimal(elencoIndiciAip.getContenutoSintetico().getNumeroIndiciAIP()));

            Set<Long> idFascicoloSet = elencoHelper.retrieveFascVersInElencoAipCreato(elenco.getIdElencoVersFasc());
            for (Long idFascicolo : idFascicoloSet) {
                ...
                if (!annullato) { // fascicolo non annullato
                    ...
                    // Aggiorno il fascicolo assegnando stato nell’elenco pari a IN_ELENCO_CON_ELENCO_INDICI_AIP_CREATO
                    if (ff.getElvElencoVersFasc().getIdElencoVersFasc() == elencoDaElab.getElvElencoVersFasc()
                            .getIdElencoVersFasc()) {
                        logger.debug(
                                "Creazione Indice AIP Fascicolo - Aggiorno lo stato fascicolo a IN_ELENCO_CON_ELENCO_INDICI_AIP_CREATO");
                        ff.setTiStatoFascElencoVers(TiStatoFascElencoVers.IN_ELENCO_CON_ELENCO_INDICI_AIP_CREATO);
                    }
                    ...
                    statoFascicoloElenco
                            .setTiStatoFascElencoVers(TiStatoFascElenco.IN_ELENCO_CON_ELENCO_INDICI_AIP_CREATO);
    ...
    private ElencoIndiciAIP creaElencoIndiceAipFascicoliMarshallObject(ElvElencoVersFasc elenco, String sistema)
        ...
        objElenco.getContenutoSintetico().setNumeroIndiciAIP(BigInteger.valueOf(fasVerAipFascNoAnnul.size()));
    ...
    private void popolaDescrizioniUrnElenco(ElencoIndiciAIP objElenco, ElvElencoVersFasc elenco, String sistema)
            throws DatatypeConfigurationException {
    ...
    private void popolaContenutoAnalitico(ElencoIndiciAIP objElenco, List<ElvVLisIxAipFascByEle> indiciAip,
            String sistema) {
        ...
        for (ElvVLisIxAipFascByEle indiceAip : indiciAip) {
            ...
            String urnIndiceAip = MessaggiWSFormat.formattaUrnIndiceAipElenchiFascicoli(sistema, indiceAip.getNmEnte(),
                    indiceAip.getNmStrut(), indiceAip.getAaFascicolo().toString(), indiceAip.getCdKeyFascicolo(),
    ...
    private String marshallElenco(ElencoIndiciAIP elencoIndiciAIP) throws IOException, JAXBException {
        // byte[] byteIndice;
        StringWriter tmpWriter = new StringWriter();
        Marshaller jaxbMarshaller = xmlContextCache.getElencoIndiciAipFascicoliCtx().createMarshaller();
    ...
    private void setElencoCompletato(long idElencoVersFasc) {
        ...
        elencoDaElab.getElvElencoVersFasc().getElvStatoElencoVersFascicoli().add(statoElencoVersFasc);

        /* Aggiorno l’elenco specificando l’identificatore dello stato corrente */
        Long idStatoElencoVersFasc = elencoHelper
                .getStatoElencoByIdElencoVersFascStato(elencoDaElab.getElvElencoVersFasc().getIdElencoVersFasc(),
                        TiStatoElencoFasc.COMPLETATO)
                .getIdStatoElencoVersFasc();
        elencoDaElab.getElvElencoVersFasc().setIdStatoElencoVersFascCor(new BigDecimal(idStatoElencoVersFasc));

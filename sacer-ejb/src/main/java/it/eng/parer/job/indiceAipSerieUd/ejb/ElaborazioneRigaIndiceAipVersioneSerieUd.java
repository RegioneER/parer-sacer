...
public class ElaborazioneRigaIndiceAipVersioneSerieUd {
    ...
    public void creaIndiceAipSerieUd(long idVerSerieDaElab, String creatingApplicationProducer)
            throws NamingException, ValidationException, JAXBException, MarshalException,
            NoSuchAlgorithmException, IOException {

        SerVerSerieDaElab verSerieDaElab = serieHelper.findById(SerVerSerieDaElab.class,
                idVerSerieDaElab);
        ...
        OrgStrut orgStrut = serieHelper.findById(OrgStrut.class,
                verSerieDaElab.getIdStrut().longValue());
        ...
        Map<String, String> mappaAgenti = confHelper.getParamApplicMapValue(agentParamList,
                ...
                BigDecimal.valueOf(strut.getIdStrut()), null, null,
        ...
        if (!"2.0".equals(sincroVersion)) {
            ...
            IdCType idc = indiceAipSerieUdUtil.generaIndiceAIPSerieUd(verSerieDaElab,
                    dataRegistrazione, mappaAgenti, creatingApplicationProducer);

            tmpWriter = marshallIdC(idc);
        } else {
            SIOrgEnteSiam orgEnteConvenz = null;
            if (strut.getIdEnteConvenz() != null) {
                orgEnteConvenz = ciasudHelper.findById(SIOrgEnteSiam.class,
                        strut.getIdEnteConvenz());
            }
            if (orgEnteConvenz != null && orgEnteConvenz.getSiOrgAmbienteEnteConvenz() != null) {
                ...
                log.warn("orgEnteConvenz o il suo ambiente è nullo per la struttura con id: {}",
                        strut.getIdStrut());
                // Gestisci il caso di errore secondo la logica applicativa
            }
            PIndex pindex = creazioneIndiceAipSerieUdUtilV2.generaIndiceAIPSerieUd(verSerieDaElab,
                    dataRegistrazione, mappaAgenti, creatingApplicationProducer);
        ...
        BackendStorage backendIndiciAip = objectStorageService
                .lookupBackendIndiciAipSerieUD(orgStrut.getIdStrut());
        ...
        if (objectStorageService.isSerFileVerSerieUDOnOs(
            ...
            String eTagFromObjectMetadata = objectStorageService.getObjectMetadataIndiceAipSerieUD(
                    verSerieDaElab.getSerVerSerie().getIdVerSerie(),
                    CostantiDB.TipoFileVerSerie.IX_AIP_UNISINCRO.name()).eTag();
        ...
        // DATO CHE L'URN DEL FILE è SU UNA TABELLA SEPARATA SONO OBBLIGATO A CREARE SEMPRE IL
        // RECORD SULLA
        // SerFileVerSerie
        SerFileVerSerie serFileVerSerie = serieHelper.storeFileIntoSerFileVerSerie(
                verSerieDaElab.getSerVerSerie().getIdVerSerie(),
                CostantiDB.TipoFileVerSerie.IX_AIP_UNISINCRO.name(),
                tmpWriter.toString().getBytes("UTF-8"), sincroVersion, verSerieDaElab.getIdStrut(),
                dataRegistrazione, putOnOs, null);
        ...
        BigDecimal pgStatoVerSerie = ciasudHelper.getUltimoProgressivoSerStatoVerSerie(
                verSerieDaElab.getSerVerSerie().getIdVerSerie());
        Long idUserIam = userHelper.findIamUser(confHelper.getValoreParamApplicByStrut(
                ...
                BigDecimal.valueOf(strut.getIdStrut()))).getIdUserIam();

        SerStatoVerSerie statoVerSerie = serieEjb.createSerStatoVerSerie(
                pgStatoVerSerie.add(BigDecimal.ONE),
                CostantiDB.StatoVersioneSerie.DA_FIRMARE.name(), "Creazione indice AIP serie", null,
                idUserIam, new Date(), verSerieDaElab.getSerVerSerie().getIdVerSerie());
        ...
        SerStatoSerie statoSerie = serieEjb.createSerStatoSerie(pgStatoSerie.add(BigDecimal.ONE),
                CostantiDB.StatoConservazioneSerie.AIP_GENERATO.name(),
                "Creazione indice AIP serie", null, idUserIam, new Date(), idSerie);
    ...
    private StringWriter marshallIdC(IdCType idc)
            throws ValidationException, JAXBException, MarshalException {
        ...
        Marshaller tmpMarshaller = xmlContextCache.getVersRespUniSincroCtx_IdC_Serie()
                .createMarshaller();
    ...
    private StringWriter marshallPIndex(PIndex pindex) throws JAXBException {
        ...
        StringWriter tmpWriter = new StringWriter();
        Marshaller tmpMarshaller = xmlContextCache.getVersRespUniSincroCtx_PIndex_Serie()
                .createMarshaller();

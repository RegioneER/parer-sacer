...
import it.eng.parer.job.serie.ejb.GenerazioneContenutoEffettivoSerieJob;
import it.eng.parer.job.utils.JobConstants;

@Singleton(mappedName = "GenerazioneContenutoEffettivoSerieTimer")
@LocalBean
@Lock(LockType.READ)
public class GenerazioneContenutoEffettivoSerieTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(GenerazioneContenutoEffettivoSerieTimer.class);
    @EJB
    private GenerazioneContenutoEffettivoSerieTimer thisTimer;
    @EJB
    private GenerazioneContenutoEffettivoSerieJob generaEffettivoEjb;

    public GenerazioneContenutoEffettivoSerieTimer() {
        super(JobConstants.JobEnum.GENERAZIONE_AUTOMATICA_CONTENUTO_EFFETTIVO.name());
        logger.debug(GenerazioneContenutoEffettivoSerieTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String appplicationName) {
        ...
        if (!existTimer) {
            ...
            logger.info("Lancio il timer GenerazioneContenutoEffettivoSerieTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            ...
            generaEffettivoEjb.generaContenutoEffettivo();
            ...
            logger.error("Errore nell'esecuzione del job di generazione automatica contenuto effettivo serie", e);
            timer.cancel();
        } catch (Exception e) {
            // questo log viene scritto solo in caso di errore.
            String message = ExceptionUtils.getRootCauseMessage(e);
            jobHelper.writeAtomicLogJob(jobName, JobConstants.OpTypeEnum.ERRORE.name(),
                    "Errore nell'esecuzione del job di generazione automatica contenuto effettivo serie:" + message);
            logger.error("Errore nell'esecuzione del job di generazione automatica contenuto effettivo serie", e);
            timer.cancel();
        }
    }

...
import it.eng.parer.job.serie.ejb.CreazioneSerieJob;
import it.eng.parer.job.utils.JobConstants;

@Singleton(mappedName = "CreazioneSerieTimer")
@LocalBean
@Lock(LockType.READ)
public class CreazioneSerieTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(CreazioneSerieTimer.class);
    @EJB
    private CreazioneSerieTimer thisTimer;
    @EJB
    private CreazioneSerieJob creaSerieEjb;

    public CreazioneSerieTimer() {
	super(JobConstants.JobEnum.CREAZIONE_AUTOMATICA_SERIE.name());
	logger.debug(CreazioneSerieTimer.class.getName() + " creato");
    }
	...
	if (!existTimer) {
	    ...
	    logger.info("Lancio il timer CreazioneSerieTimer...");
	...
	try {
	    ...
	    logger.error("Errore nell'esecuzione del job di creazione automatica serie", e);
	    timer.cancel();
	} catch (Exception e) {
	    // questo log viene scritto solo in caso di errore.
	    String message = ExceptionUtils.getRootCauseMessage(e);
	    jobHelper.writeAtomicLogJob(jobName, JobConstants.OpTypeEnum.ERRORE.name(),
		    "Errore nell'esecuzione del job di creazione automatica serie:" + message);
	    logger.error("Errore nell'esecuzione del job di creazione automatica serie", e);
	    timer.cancel();
	}
    }

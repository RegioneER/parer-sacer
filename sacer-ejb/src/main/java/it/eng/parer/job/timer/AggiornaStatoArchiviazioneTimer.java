...
import it.eng.parer.job.tpi.ejb.AggiornaStatoArchiviazioneJob;
import it.eng.parer.job.utils.JobConstants;
import it.eng.parer.ws.dto.RispostaControlli;
import it.eng.parer.ws.ejb.ControlliTpi;

@Singleton(mappedName = "AggiornaStatoArchiviazioneTimer")
@LocalBean
@Lock(LockType.READ)
public class AggiornaStatoArchiviazioneTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(AggiornaStatoArchiviazioneTimer.class);
    @EJB
    private AggiornaStatoArchiviazioneTimer thisTimer;
    @EJB
    private ControlliTpi controlliTpi;
    @EJB
    private AggiornaStatoArchiviazioneJob statoArkEjb;
    // private final String NAME_JOB_AGGORNA_STATO_ARK = "IntervalloAggiornaStatoArchiviazione";

    public AggiornaStatoArchiviazioneTimer() {
	super(JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name());
	logger.debug(AggiornaStatoArchiviazioneTimer.class.getName() + " creato");
    }
	...
	if (!existTimer) {
	    ...
	    logger.info("Lancio il timer AggiornaStatoArchiviazioneTimer...");
	...
	try {
	    ...
	    logger.error("Errore nell'esecuzione del job di aggiornamento stato archiviazione", e);
	    timer.cancel();
	} catch (Exception e) {
	    // questo log viene scritto solo in caso di errore.
	    String message = null;

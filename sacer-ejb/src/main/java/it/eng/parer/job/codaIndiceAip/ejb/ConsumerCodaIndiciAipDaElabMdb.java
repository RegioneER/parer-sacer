...
public class ConsumerCodaIndiciAipDaElabMdb implements MessageListener {
	...
	try {
		...
		if (statoElencoUd.equals(
		    ...
		    // IN_CODA_JMS_INDICE_AIP_DA_ELAB e che l’elenco abbia stato
		    // IN_CODA_JMS_INDICE_AIP_DA_ELAB (vedi
			...
			// Recupero l'unità doc appartenente all'elenco il cui stato relativo
			// all'elenco sia pari a
			// IN_CODA_JMS_INDICE_AIP_DA_ELAB (serve per definire il motivo per cui
			// l'unità documentaria è
			...
			try {
			    elaboraCodaIndiciAipDaElabEjb.elaboraUDFase2(unitaInElenco,
				    elencoDaElab);

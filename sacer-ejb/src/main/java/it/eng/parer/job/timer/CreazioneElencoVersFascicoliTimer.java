...
public class CreazioneElencoVersFascicoliTimer extends JobTimer {
    ...
    public void doJob(Timer timer) {
        if (timer.getInfo().equals(jobName)) {
            try {
                ...
                logger.error(
                        "Errore nell'esecuzione del job di creazione automatica degli elenchi versamento fascicoli", e);
            }
        }
    }
    ...
    public void startProcess(Timer timer) throws Exception {
        logger.info("Job automatico per la creazione automatica elenchi versamento fascicoli avviato");
         ...
         */
        LogJob logJob = jobHelper.writeAtomicLogJob(JobConstants.JobEnum.CREAZIONE_ELENCHI_VERS_FASCICOLI.name(),
        ...
        try {
            ...
            jobHelper.writeAtomicLogJob(JobConstants.JobEnum.CREAZIONE_ELENCHI_VERS_FASCICOLI.name(),
                    ElencoEnums.OpTypeEnum.ERRORE.name(), ue.getDescription());
            logger.error("Errore nell'esecuzione del job di creazione automatica degli elenchi versamento fascicoli",
                    ue);

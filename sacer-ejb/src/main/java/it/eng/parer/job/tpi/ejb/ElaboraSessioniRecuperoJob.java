...
public class ElaboraSessioniRecuperoJob {
    ...
    public void elaboraSessioniRecupero() throws ParerInternalError {
        ...
        log.info("{} --- Trovate {} sessioni", JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO, sessioni.size());
        ...
        for (RecSessioneRecup sessione : sessioni) {
            String dirStruttura = sessione.getRecUnitaDocRecup().getAroUnitaDoc().getOrgStrut().getOrgEnte()
                    .getOrgAmbiente().getNmAmbiente() + "-"
                    + sessione.getRecUnitaDocRecup().getAroUnitaDoc().getOrgStrut().getOrgEnte().getNmEnte() + "-"
                    + sessione.getRecUnitaDocRecup().getAroUnitaDoc().getOrgStrut().getNmStrut();
            String dirUnitaDoc = sessione.getRecUnitaDocRecup().getAroUnitaDoc().getCdRegistroKeyUnitaDoc() + "-"
            ...
            for (RecDtVersRecup dataRecup : sessione.getRecDtVersRecups()) {
                if (dataRecup.getTiStatoDtVersRecup().equals(JobConstants.StatoDtVersRecupEnum.RECUPERATA.name())) {
                    countRecuperate++;
                } else if (dataRecup.getTiStatoDtVersRecup()
                        .equals(JobConstants.StatoDtVersRecupEnum.DA_RECUPERARE.name())) {
                    List<VrsDtVers> listaDateVers = elabRecHelper.getVrsDtVersByDate(dataRecup.getDtVers());
                    countRecuperate++;
                    ...
                    Integer timeout = Integer.parseInt(params.get(CostantiDB.ParametroAppl.TPI_TIMEOUT_RETRIEVE));
                    ...
                    RichiestaTpiInput inputParams = new RichiestaTpiInput(
                            ...
                            new BasicNameValuePair(RichiestaTpi.ROOT_DT_VERS, listaDateVers.get(0).getDlPathDtVers()),
                    ...
                    if (esitoConn.isErroreConnessione()) {
                        ...
                        jobHelper.writeAtomicLogJob(JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO.name(),
                                JobConstants.OpTypeEnum.ERRORE.name(), "Timeout dal servizio RetrieveFileUnitaDoc");
                        jobChiusoOk = false;
                        break;
                    } else if (codiceEsito.equals(EsitoConnessione.Esito.KO.name())) {
                        log.error("{} --- RetrieveFileUnitaDoc - {} - {}",
                                JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO, codiceErrore, messaggioErrore);
                        ...
                        if (!codiceErrore.equals(JobConstants.RetrieveTpiErrors.RETRIEVE_004.name())
                                && !codiceErrore.equals(JobConstants.RetrieveTpiErrors.RETRIEVE_006.name())) {
                            log.debug(
                        ...
                        RetrieveFileUnitaDocRisposta resp = (RetrieveFileUnitaDocRisposta) esitoConn.getResponse();
                        if (resp != null) {
                            log.debug("{} --- Servizio RetrieveFileUnitaDoc OK : gestisco la risposta",
                                    JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO);
                            elabRecHelper.retrieveFileUnitaDoc(dataRecup.getIdDtVersRecup(), countRecuperate,
                                    sessione.getRecDtVersRecups().size());
                        } else {
                            log.error("{} --- Risposta inaspettata dal servizio RetrieveFileUnitaDoc",
                                    JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO);
                            throw new ParerInternalError("Risposta inaspettata dal servizio RetrieveFileUnitaDoc");
                        }
                    }
                }
            }
            if (jobChiusoOk) {
                // Chiudo tutte le possibili casistiche (i.e. Sessione in corso e tutte le date recuperate)
        ...
        if (jobChiusoOk) {
            log.info("{} --- Fine schedulazione job", JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO);
            jobHelper.writeAtomicLogJob(JobConstants.JobEnum.ELABORA_SESSIONI_RECUPERO.name(),

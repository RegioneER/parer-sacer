...
public class VerificaPeriodoRegistroEjb {
    ...
    public void verificaPeriodo(long idAaRegistroUnitaDoc) {
        ...
        VerificaPeriodoRegistroEjb me = sessionContext.getBusinessObject(VerificaPeriodoRegistroEjb.class);
        //
        ConfigRegAnno configRegAnno = controlliSemantici.caricaPartiAARegistro(idAaRegistroUnitaDoc);
        ...
        KeyOrdUtility tmpKeyOrdUtility = new KeyOrdUtility(configRegAnno, KeySizeUtility.MAX_LEN_CHIAVEORD);
        DecAaRegistroUnitaDoc tmpAaRegistroUnitaDoc = entityManager.find(DecAaRegistroUnitaDoc.class,
        ...
        Long idRegistroUnitaDoc = tmpAaRegistroUnitaDoc.getDecRegistroUnitaDoc().getIdRegistroUnitaDoc();
        List<OrgSubStrut> orgSubStruts = tmpAaRegistroUnitaDoc.getDecRegistroUnitaDoc().getOrgStrut().getOrgSubStruts();
        //
        ...
        //
        while (anno <= annoMax) {
            RispostaControlli rispostaControlli = me.verificaAnnoNewTrans(idRegistroUnitaDoc, anno, idSubStruts,
                    tmpKeyOrdUtility);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public RispostaControlli verificaAnnoNewTrans(Long idRegistroUnitaDoc, long anno, List<Long> idSubStruts,
            KeyOrdUtility tmpKeyOrdUtility) {
        RispostaControlli risposta = new RispostaControlli();
        risposta.setrBoolean(true);

        // elabora anno
        List<AroUnitaDoc> aroUnitaDocs = verificaCompRegHelper.getListaUdDaVerificare(idRegistroUnitaDoc, idSubStruts,
                anno);
        for (AroUnitaDoc tmpUd : aroUnitaDocs) {
            ...
            if (rispostaControlli.isrBoolean()) {
                ...
                // aggiorno il progressivo calcolato, estratto dal numero della chiave.. potrebbe non esistere

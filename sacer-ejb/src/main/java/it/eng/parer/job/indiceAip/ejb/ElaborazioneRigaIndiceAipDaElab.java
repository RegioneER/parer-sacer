...
public class ElaborazioneRigaIndiceAipDaElab {
    ...
    private static final List<String> agentParam = Arrays.asList(AGENT_PRESERVER_FORMALNAME,
            AGENT_PRESERVER_TAXCODE, AGENT_PRESERVATION_MNGR_TAXCODE,
            AGENT_PRESERVATION_MNGR_LASTNAME, AGENT_PRESERVATION_MNGR_FIRSTNAME);
    ...
    public void gestisciIndiceAipDaElaborareNelJob(long idUdDaElab)
        ...
        AroIndiceAipUdDaElab entity = ciaHelper.findByIdWithLock(AroIndiceAipUdDaElab.class,
                idUdDaElab);
        if (entity != null) {
            AroUnitaDoc ud = ciaHelper.findByIdWithLock(AroUnitaDoc.class,
                    entity.getAroUnitaDoc().getIdUnitaDoc());
    ...
    private void inviaMessaggio(AroIndiceAipUdDaElab entity) throws EJBException {
        ...
        try (Connection connection = connectionFactory.createConnection();) {
            ...
            log.debug("Creo la connessione alla coda per l'indiceAIP da elaborare {}",
                    idIndiceAipDaElab);
            ...
            throw new EJBException(
                    String.format("ERRORE nell'invio del messaggio per l'indiceAIP da elaborare %s",
                            idIndiceAipDaElab),
                    ex);
        } finally {
            try {
    ...
    public void verificaUrnUd(long idUnitaDoc) throws ParerInternalError, ParseException {
        ...
        AroVerIndiceAipUd aroVerIndiceAipUd = ciaHelper
        ...
        List<AroLinkUnitaDoc> lisLinkUnitaDoc = ciaHelper
                .getAroLinkUnitaDoc(aroUnitaDoc.getIdUnitaDoc());
        for (AroLinkUnitaDoc linkUnitaDoc : lisLinkUnitaDoc) {
            AroUnitaDoc ud = ciaHelper.findByIdWithLock(AroUnitaDoc.class,
                    linkUnitaDoc.getAroUnitaDocLink().getIdUnitaDoc());// id_unita_doc_link
            if (!ud.getTiStatoConservazione()
                ...
                // Sistema URN INDICI AIP PREGRESSI
                AroVerIndiceAipUd aroVerIndiceAipUdColleg = ciaHelper
                        .getUltimaVersioneIndiceAip(ud.getIdUnitaDoc());
                if (aroVerIndiceAipUdColleg != null
                        && !aroVerIndiceAipUdColleg.getDtCreazione().after(dataInizio)) {
                    // eseguo registra urn aip pregressi
    ...
    public void sistemaUrnUnitaDoc(AroUnitaDoc aroUnitaDoc, Date dataInizio, CSVersatore versatore,
        ...
        AroVDtVersMaxByUnitaDoc aroVDtVersMaxByUd = ciaHelper
    ...
    public void gestisciIndiceAipDaElab(long idUdDaElab) throws ParerInternalError, NamingException,
            NoSuchAlgorithmException, IOException, JAXBException {
        AroIndiceAipUdDaElab udDaElab = ciaHelper.findByIdWithLock(AroIndiceAipUdDaElab.class,
                idUdDaElab);
        // Se non trova il record esce e non fa nulla!
        if (udDaElab != null) {
            BigDecimal idAmbiente = BigDecimal.valueOf(udDaElab.getAroUnitaDoc().getOrgStrut()
                    .getOrgEnte().getOrgAmbiente().getIdAmbiente());
            String sincroVersion = configurationHelper.getValoreParamApplicByAmb(
                    CostantiDB.ParametroAppl.UNISINCRO_VERSION, idAmbiente);
    ...
    public void gestisciIndiceAipDaElab(AroIndiceAipUdDaElab udDaElab, String sincroVersion)
        ...
        AroUnitaDoc unitaDoc = ciaHelper.findByIdWithLock(AroUnitaDoc.class,
        ...
        Map<String, String> mappaAgent = configurationHelper.getParamApplicMapValue(agentParamList,
                BigDecimal.valueOf(
                        unitaDoc.getOrgStrut().getOrgEnte().getOrgAmbiente().getIdAmbiente()),
        ...
        if ("2.0".equals(sincroVersion)) {
            ...
            if (unitaDoc.getOrgStrut().getIdEnteConvenz() != null) {
                orgEnteConvenz = ciaHelper.findById(SIOrgEnteSiam.class,
                        unitaDoc.getOrgStrut().getIdEnteConvenz());
            }
            mappaAgent.putAll(paramIamHelper.getParamApplicMapValue(agentParamIamV2,
                    BigDecimal.valueOf(orgEnteConvenz.getSiOrgAmbienteEnteConvenz()
        ...
        log.debug("{} - Elaboro l'unità documentaria {}", desJobMessage,
                udDaElab.getAroUnitaDoc().getIdUnitaDoc());
        ...
        String creatingApplicationProducer = configurationHelper.getValoreParamApplicByApplic(
                CostantiDB.ParametroAppl.CREATING_APPLICATION_PRODUCER);
        /* Determino la chiave */
        ...
        if (!"2.0".equals(sincroVersion)) {
            ...
            IdCType idc = creazione.generaIndiceAIP(udDaElab, codiceVersione,
                    Costanti.VERSIONE_XSD_INDICE_AIP, sistemaConservazione, mappaAgent,
            ...
            PIndex pindex = creazioneIndiceAipUtilV2.generaIndiceAIPV2(udDaElab, codiceVersione,
                    Costanti.VERSIONE_XSD_INDICE_AIP_V2, sistemaConservazione, mappaAgent,
                    creatingApplicationProducer);
        ...
        AroVerIndiceAipUd lastVer = ciaHelper.creaAIP(udDaElab, annoMese, progressivoVersione,
                codiceVersione, hash, tmpWriter.toString(), versatore, chiave);

        // MEV#30395
        BackendStorage backendIndiciAip = objectStorageService.lookupBackend(
                unitaDoc.getDecTipoUnitaDoc().getIdTipoUnitaDoc(),
                CostantiDB.ParametroAppl.BACKEND_INDICI_AIP);
            ...
            if (putOnOs) {
                ObjectStorageResource indiceAipSuOS = objectStorageService
                        .createResourcesInIndiciAipUnitaDoc(backendIndiciAip.getBackendName(),
                                tmpWriter.toString(), lastVer.getIdVerIndiceAip(),
                                new BigDecimal(unitaDoc.getOrgSubStrut().getIdSubStrut()),
                                unitaDoc.getAaKeyUnitaDoc());
        ...
        /**
         * Se la UD cui l’AIP si riferisce ha stato di conservazione ANNULLATO, lo stato rimane
         * ANNULLATO, altrimenti se ha stato PRESA_IN_CARICO, AIP_IN_AGGIORNAMENTO o
         * AIP_DA_GENERARE, controllo sui componenti...
         */
        if (aroUnitaDoc.getTiStatoConservazione()
            ...
            log.debug("{} - Controllo i componenti e gli aggiornamenti metadati presenti",
                    desJobMessage);
            ...
            // end MAC#27786
                ...
                aroUnitaDoc.setTiStatoConservazione(
                        CostantiDB.StatoConservazioneUnitaDoc.AIP_IN_AGGIORNAMENTO.name());
                // MEV #31162
                udEjb.insertLogStatoConservUd(aroUnitaDoc.getIdUnitaDoc(),
                        Constants.JOB_CREAZIONE_INDICE_AIP, Constants.CREAZIONE_INDICE_AIP_UD,
                        CostantiDB.StatoConservazioneUnitaDoc.AIP_IN_AGGIORNAMENTO.name(),
        ...
        if (aroUnitaDoc.getElvElencoVer().getIdElencoVers()
                .equals(udDaElab.getElvElencoVer().getIdElencoVers())) {
            log.debug("{} - Aggiorno lo stato ud a IN_ELENCO_CON_INDICI_AIP_GENERATI",
                    desJobMessage);
            aroUnitaDoc.setTiStatoUdElencoVers(
                    ElencoEnums.UdDocStatusEnum.IN_ELENCO_CON_INDICI_AIP_GENERATI.name());
        }

        // BULK_UPDATE con idUnitaDoc di aroUnitaDoc e idElencoVers di udDaElab dei documenti
        // aggiunti
        int updateDoc = ciaHelper.updateDocumentiAggiuntiElencoIndiceAIP(
                aroUnitaDoc.getIdUnitaDoc(), udDaElab.getElvElencoVer().getIdElencoVers());
        log.debug(
                "Creazione Indice AIP - Aggiorno lo stato doc aggiunti a IN_ELENCO_CON_INDICI_AIP_GENERATI - aggiornati {} records",
                updateDoc);

        // BULK_UPDATE con idUnitaDoc di aroUnitaDoc e idElencoVers di udDaElab degli aggiornamenti
        // metadati
        int updateUpd = ciaHelper.updateAggiornamentiMetadatiElencoIndiceAIP(
                aroUnitaDoc.getIdUnitaDoc(), udDaElab.getElvElencoVer().getIdElencoVers(),
        ...
        // Aggiorno le versioni serie settando l’indicatore che segnala se la serie deve essere
        // ricalcolata a causa
        // di aggiornamento di almeno una unità documentaria
        setFlagVerSerieDaAggiornare(aroUnitaDoc.getIdUnitaDoc());

        // Aggiorno i fascicoli settando l’indicatore che segnala se il fascicolo deve essere
        // ricalcolato a causa di
        // aggiornamento di almeno una unità documentaria
        setFlagFasFascicoliDaAggiornare(aroUnitaDoc.getIdUnitaDoc());

        // MEV#24597
        // Aggiorno le note dell'unità documentaria settando l’identificativo che segnala la prima
        // versione
    ...
    private StringWriter marshallPIndex(PIndex pindex) throws JAXBException {
        ...
        Marshaller tmpMarshaller = xmlContextCache.getVersRespUniSincroCtx_PIndex_Ud()
                .createMarshaller();
    ...
    private StringWriter marshallIdC(IdCType idc) throws JAXBException {
        ...
        StringWriter tmpWriter = new StringWriter();
        Marshaller tmpMarshaller = xmlContextCache.getVersRespUniSincroCtx_IdC_Ud()
                .createMarshaller();
    ...
    /*
     * Crea la prima versione dell'indice AIP dell'unita doc appartenente alla serie o al fascicolo
     * con stato di conservazione = IN_VOLUME_DI_CONSERVAZIONE. Metodo richiamato dalla Validazione
     * Serie o Validazione Fascicoli
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void gestisciIndiceAipOs(long idUnitaDoc, String nomeAgente) throws ParerInternalError,
        ...
        /* Crea AIP */
        CreazioneIndiceAipUtil creazione = new CreazioneIndiceAipUtil();
        IdCType idc = creazione.generaIndiceAIP(unitaDoc, codiceVersione,
                Costanti.VERSIONE_XSD_INDICE_AIP, sistemaConservazione, mappaAgent,
                creatingApplicationProducer);
        ...
        /* Persisto nelle varie tabelle di creazione dell'indice AIP */
        AroVerIndiceAipUd lastVer = ciaHelper.creaAIP(unitaDoc, annoMese, progressivoVersione,
                codiceVersione, hash, tmpWriter.toString(), versatore, chiave);
        ...
        // MEV #31162
        udEjb.insertLogStatoConservUd(unitaDoc.getIdUnitaDoc(), nomeAgente,
    ...
    // EVO#20972
    /*
     * Crea la prima versione dell'indice AIP UNISINCRO V2 dell'unita doc appartenente alla serie o
     * al fascicolo con stato di conservazione = IN_VOLUME_DI_CONSERVAZIONE. Metodo richiamato dalla
     * Validazione Serie o Validazione Fascicoli
     */
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void gestisciIndiceAipV2Os(long idUnitaDoc, String tipoValidazione)
        ...
        // Aggiungiamo i parametri dell'ente convenzionato alla mappa SOLO SE l'ente esiste.
        ...
        // MEV #31162
        udEjb.insertLogStatoConservUd(unitaDoc.getIdUnitaDoc(), tipoValidazione,
                Constants.CREAZIONE_INDICE_AIP_UD,
                CostantiDB.StatoConservazioneUnitaDoc.AIP_GENERATO.name(),
                Constants.NM_AGENTE_JOB_SACER);
        // end MEV #31162
    ...
    public void setFlagVerSerieDaAggiornare(long idUnitaDoc) {
        List<SerVerSerie> verSeries = ciaHelper.getVersioniSerieCorrentiContenEffettivoByUdAndStato(
                idUnitaDoc, CostantiDB.StatoVersioneSerie.CONTROLLATA.name(),

...
public class ControlliRecIndiceAipFascicoli {
	...
	FasXmlVersFascicolo fasXmlVersFasc = entityManager.find(FasXmlVersFascicolo.class,
		idXmlVersFascicolo);
	...
	if (fasXmlVersFasc != null) {
	    ...
	    rispostaControlli.setDsErr(
		    "Errore interno: non sono stati eseguiti versamenti per il fascicolo id="
			    + idFascicolo);
	}
	...
	String queryStr = "select t from FasXmlVersFascicolo t "
		+ "where t.fasFascicolo.idFascicolo = :idFascicolo "
	...
	try {
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione durante il recupero della versione corrente dei metadati "
			    + e.getMessage()));
	...
	try {
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione ControlliRecIndiceAipFascicoli.getFasContenVerAipFascicolo "
			    + e.getMessage()));
	    log.error(
	...
	try {
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione ControlliRecIndiceAipFascicoli.getFasSipVerAipFascicolo "
			    + e.getMessage()));
	    log.error(
    ...
    // log.error("Eccezione nella lettura dei dati riguardanti il modello XSD di tipo
    // FILE_GROUP_FILE_MORE_INFO durante
	...
	try {
	    String queryStr = "SELECT m " + "FROM DecModelloXsdFascicolo m "
		    + "WHERE m.orgAmbiente.idAmbiente = :idAmbiente "
		    + "AND m.tiModelloXsd = :tiModelloXsd "
		    + "AND m.dtIstituz <= :filterDate AND m.dtSoppres >= :filterDate ";
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione ControlliRecIndiceAipFascicoli.getDecModelloSelfDescMoreInfo "
			    + e.getMessage()));
	    log.error(
     ...
     * di versamento fascicolo, con il tipo "AIP_SELF_DESCRIPTION_MORE_INFO"
     *
     ...
     */
    public RispostaControlli getDecModelloSelfDescMoreInfoV2(long idAmbiente,
	...
	try {
	    ...
	    rispostaControlli.setCodErr(MessaggiWSBundle.ERR_666);
	    rispostaControlli.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666,
		    "Eccezione ControlliRecIndiceAipFascicoli.getDecModelloSelfDescMoreInfoV2 "
			    + e.getMessage()));
	    log.error(

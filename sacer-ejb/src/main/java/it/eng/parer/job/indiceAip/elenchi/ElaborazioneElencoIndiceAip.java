...
public class ElaborazioneElencoIndiceAip {
    ...
    public void creaElencoIndiciAIP(long idElencoVers, long idLogJob)
        ...
        if (tiGestione.equals(ElencoEnums.GestioneElencoEnum.FIRMA.name())
            ...
            if (udSoloAnnul.getFlSoloDocAnnul().equals("1") && udSoloAnnul.getFlSoloUdAnnul().equals("1")
        ...
        ElencoEnums.GestioneElencoEnum tiGestioneEnum = ElencoEnums.GestioneElencoEnum.valueOf(tiGestione);
        ...
        case MARCA_SIGILLO:
            ...
            String hash = new HashCalculator().calculateHashSHAX(elencoIndiciAipString, TipiHash.SHA_256).toHexBinary();
            ...
            elencoDaElab.setTiStatoElenco(ElencoEnums.ElencoStatusEnum.ELENCO_INDICI_AIP_CREATO.name());
            ...
            elenco.setNiIndiciAip(
                    BigDecimal.valueOf(elencoIndiciAip.getContenutoSintetico().getNumeroIndiciAIP().longValue()));
    ...
    private ElencoIndiciAIP creaElencoIndiceAipMarshallObject(ElvElencoVer elenco) {
        ...
        objElenco.getContenutoSintetico().setNumeroIndiciAIP(BigInteger.valueOf(aroVerIndiceAipUdNoAnnul.size()));
    ...
    private void popolaDescrizioniUrnElenco(ElencoIndiciAIP objElenco, ElvElencoVer elenco) {
        ...
        objElenco.getDescrizioneElencoIndiciAIP()
                .setDataCreazione(XmlDateUtility.dateToXMLGregorianCalendar(Calendar.getInstance().getTime()));
        objElenco.setElencoVersamentoDiOrigine(new ElencoVersamentoDiOrigineType());
        objElenco.getElencoVersamentoDiOrigine().setIdElenco(BigInteger.valueOf(elenco.getIdElencoVers()));
        ...
        final String urnElencoVersamento = MessaggiWSFormat.formattaUrnElencoVersamento(sistema, versatore.getEnte(),
                versatore.getStruttura(), sdf.format(elenco.getDtCreazioneElenco()),
                Long.toString(elenco.getIdElencoVers()));
        // end EVO#16486
        objElenco.getElencoVersamentoDiOrigine().setUrnElenco(urnElencoVersamento);
    }

    private void popolaContenutoAnalitico(ElencoIndiciAIP objElenco, List<AroVerIndiceAipUd> indiciAip) {
        ...
        for (AroVerIndiceAipUd aroVerIndiceAipUd : indiciAip) {
            ...
            indice.setHashIndiceAIP(StringUtils.isNotBlank(aroVerIndiceAipUd.getDsHashIndiceAip())
                    ? aroVerIndiceAipUd.getDsHashIndiceAip() : aroVerIndiceAipUd.getDsHashAip());

            objElenco.getContenutoAnalitico().getIndiceAIP().add(indice);
        }
    }

    private String marshallElenco(ElencoIndiciAIP elencoIndiciAIP) throws MarshalException, JAXBException {
        StringWriter tmpWriter = new StringWriter();
        Marshaller tmpMarshaller = xmlContextCache.getElencoIndiciAipCtx_ElencoIndiciAIP().createMarshaller();
    ...
    public void setCompletato(ElvElencoVer elenco, List<String> statiUdDocDaCompletare, long idUtente,
        ...
        ciaHelper.updateAggiornamentiElencoIndiceAIP(elenco.getIdElencoVers(),
                transformCollection(statiUdDocDaCompletare), AroUpdUDTiStatoUpdElencoVers.IN_ELENCO_COMPLETATO);
    ...
    private void setElencoCompletato(long idElencoVers) {
        ...
        // EVO 19304
        evEjb.registraStatoElencoVersamento(BigDecimal.valueOf(elenco.getIdElencoVers()), "CREAZIONE_ELENCO_INDICE_AIP",
        ...
        ciaHelper.updateDocumentiElencoIndiceAIPStatoConservDiverso(elenco.getIdElencoVers(),
                CostantiDB.StatoConservazioneUnitaDoc.ANNULLATA.name(),
                UdDocStatusEnum.IN_ELENCO_CON_INDICI_AIP_GENERATI.name(), UdDocStatusEnum.IN_ELENCO_COMPLETATO.name());
        ciaHelper.updateUnitaDocElencoIndiceAIPStatoConvervDiverso(elenco.getIdElencoVers(),
                CostantiDB.StatoConservazioneUnitaDoc.ANNULLATA.name(),
                UdDocStatusEnum.IN_ELENCO_CON_INDICI_AIP_GENERATI.name(), UdDocStatusEnum.IN_ELENCO_COMPLETATO.name());
        ciaHelper.updateAggiornamentiElencoIndiceAIPStatoConservDiverso(elenco.getIdElencoVers(),
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void updateDocumentiElencoIndiceAIP(long idElencoVers, List<String> tiStatoDocElencoVersOld,
            String tiStatoDocElencoVersNew) {
        ciaHelper.updateDocumentiElencoIndiceAIP(idElencoVers, tiStatoDocElencoVersOld, tiStatoDocElencoVersNew);
    }
    ...
    public void updateDocumentiElencoIndiceAIPStatoConservDiverso(long idElencoVers, String tiStatoConservazione,
        ...
        ciaHelper.updateDocumentiElencoIndiceAIPStatoConservDiverso(idElencoVers, tiStatoConservazione,
                tiStatoDocElencoVersCor, tiStatoDocElencoVersNew);
    }
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void updateUnitaDocElencoIndiceAIPStatoConservDiverso(long idElencoVers, String tiStatoConservazione,
            String tiStatoUdElencoVersCor, String tiStatoElencoNew) {
        ciaHelper.updateUnitaDocElencoIndiceAIPStatoConvervDiverso(idElencoVers, tiStatoConservazione,
                tiStatoUdElencoVersCor, tiStatoElencoNew);
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void updateAggiornamentiElencoIndiceAIPStatoConservDiverso(long idElencoVers, String tiStatoConservazione,
            AroUpdUDTiStatoUpdElencoVers tiStatoUpdElencoVersCor, AroUpdUDTiStatoUpdElencoVers tiStatoElencoNew) {
        ciaHelper.updateAggiornamentiElencoIndiceAIPStatoConservDiverso(idElencoVers, tiStatoConservazione,
                tiStatoUpdElencoVersCor, tiStatoElencoNew);
    }

    private List<AroUpdUDTiStatoUpdElencoVers> transformCollection(List<String> stati) {
        return (List<AroUpdUDTiStatoUpdElencoVers>) CollectionUtils.collect(stati, new Transformer() {
            @Override
    ...
    public void verificaUrnAipElenco(long idElencoVers) throws ParerInternalError, ParseException {
        ...
        for (ElvVLisaipudUrndacalcByele elvVLisUrndacalc : elvVLisUrndacalcList) {
            ...
            if (!elvVLisUrndacalc.getDtCreazione().after(dataInizio)) {
                ...
                this.sistemaUrnIndiceAipElenco(aroUnitaDoc, elvVLisUrndacalc.getDtVersMax(), dataInizio, versatore,
                        chiave);
            }
        }
    }
    ...
    public void sistemaUrnIndiceAipElenco(AroUnitaDoc aroUnitaDoc, Date dtVersMax, Date dataInizio,
        ...
        if (!dtVersMax.after(dataInizio) && StringUtils.isBlank(aroUnitaDoc.getCdKeyUnitaDocNormaliz())) {

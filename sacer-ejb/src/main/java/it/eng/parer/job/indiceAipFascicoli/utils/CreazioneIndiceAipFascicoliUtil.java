...
public class CreazioneIndiceAipFascicoliUtil {
    ...
    private static final Logger log = LoggerFactory
            .getLogger(CreazioneIndiceAipFascicoliUtil.class);
     ...
     * @param enteSiam                    entity SIOrgEnteSiam
    ...
    public IdCType generaIndiceAIP(FasVerAipFascicolo verAipFascicolo, String codiceVersione,
        ...
        popolaIdC(istanzaUnisincro, verAipFascicolo.getIdVerAipFascicolo(),
                verAipFascicolo.getFasFascicolo(), codiceVersione, mappaAgenti,
    ...
    public IdCType generaIndiceAIP(long idVerAipFascicolo, FasFascicolo fasc, String codiceVersione,
            ...
            throws ParerInternalError, DatatypeConfigurationException, JAXBException {
        IdCType istanzaUnisincro = new IdCType();
        popolaIdC(istanzaUnisincro, idVerAipFascicolo, fasc, codiceVersione, mappaAgenti,
                sistemaConservazione, enteSiam, creatingApplicationProducer);
        return istanzaUnisincro;
    }

    private void popolaIdC(IdCType idc, long idVerAipFascicolo, FasFascicolo fasFascicolo,
            String codiceVersione, Map<String, String> mappaAgenti, String sistemaConservazione,
            SIOrgEnteSiam enteSiam, String creatingApplicationProducer)
            throws ParerInternalError, DatatypeConfigurationException, JAXBException {
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            List<FasVerAipFascicolo> versioniPrecedenti = (List<FasVerAipFascicolo>) rispostaControlli
                    .getrObject();
            ...
            selfie.getSourceIdC().addAll(Arrays.asList(sorgenteArray)); // TODO: verificare addAll e
            // ordinamento per
        ...
        extraInfoDescGenerale
                .setAny(jaxbObjFactorySelfDes.createMetadatiIntegratiSelfDescription(miSelfD));
        ...
        String urn = MessaggiWSFormat.formattaUrnAipFascicolo(
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            for (FasContenVerAipFascicolo fasContenVerAipFasc : fasContenVerAipFascList) {
                ...
                csChiaveUd.setTipoRegistro(fasContenVerAipFasc.getAroVerIndiceAipUd()
                        .getAroIndiceAipUd().getAroUnitaDoc().getCdRegistroKeyUnitaDoc());
                csChiaveUd.setAnno(fasContenVerAipFasc.getAroVerIndiceAipUd().getAroIndiceAipUd()
                        .getAroUnitaDoc().getAaKeyUnitaDoc().longValue());
                csChiaveUd.setNumero(fasContenVerAipFasc.getAroVerIndiceAipUd().getAroIndiceAipUd()
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            for (FasSipVerAipFascicolo fasSipVerAipFasc : fasSipVerAipFascList) {
                ...
                this.popolaIndiceSipFile(
                        fasSipVerAipFasc.getFasXmlVersFascicoloRich().getIdXmlVersFascicolo(),
                        tmpFasFascicolo, fileGroupVers);
                /* Creo un sottotag <File> riferito al rapporto di versamento */
                this.popolaIndiceSipFile(
                        fasSipVerAipFasc.getFasXmlVersFascicoloRisp().getIdXmlVersFascicolo(),
                        tmpFasFascicolo, fileGroupVers);
            }
        }
    ...
    private void popolaMetadatiIntegratiSelfDesc(FasFascicolo tmpFasFascicolo,
        ...
        /*
         * Determino il modello xsd per l'ambiente di appartenenza della struttura a cui il
         * fascicolo appartiene, con il tipo "AIP_SELF_DESCRIPTION_MORE_INFO"
         */
        rispostaControlli.reset();
        rispostaControlli = controlliRecIndiceAipFascicoli
                .getDecModelloSelfDescMoreInfo(idAmbiente);
        if (!rispostaControlli.isrBoolean()) {
            setRispostaError();
        } else {
            List<DecModelloXsdFascicolo> decModelloXsdFascList = (List<DecModelloXsdFascicolo>) rispostaControlli
                    .getrObject();
            log.info("Creazione Indice AIP Fascicoli - ambiente id {}: trovati {}"
    ...
    private void popolaIndiceSipFile(Long idXmlVersFascicolo, FasFascicolo tmpFasFascicolo,
        ...
        rispostaControlli.reset();
        rispostaControlli = controlliRecIndiceAipFascicoli
                .leggiXmlVersFascicoliAip(idXmlVersFascicolo, tmpFasFascicolo.getIdFascicolo());
        if (!rispostaControlli.isrBoolean()) {
            setRispostaError();
        } else {
            FasXmlVersFascicolo fasXmlVersFasc = (FasXmlVersFascicolo) rispostaControlli
                    .getrObject();
            FileType file = new FileType();
            file.setEncoding(null);
            file.setFormat("application/xml");

...
public class CalcoloConsistenzaHelper {
    ...
    private static final String UD_AIP_GENERATO = "SELECT new it.eng.parer.entity.MonContaByStatoConservNew("
            ...
            + " ud.idDecTipoUnitaDoc, docPrinc.idDecTipoDoc, " + " nvl(count(comp.idStrut), 0), 'UD_AIP_GENERATO' ) "
    ...
    private static final String UD_AIP_NON_GENERATO_PRESA_IN_CARICO = "SELECT new it.eng.parer.entity.MonContaByStatoConservNew("
            + " trunc(doc.dtCreazione), ud.idOrgStrut, ud.idOrgSubStrut, ud.aaKeyUnitaDoc, ud.idDecRegistroUnitaDoc, "
            + " ud.idDecTipoUnitaDoc, docPrinc.idDecTipoDoc, "
            + " nvl(count(comp.idStrut), 0), 'UD_AIP_NON_GENERATO_PRESA_IN_CARICO') " + " FROM AroCompDoc comp "
    ...
    private void executeNativeQueryCalcolo4(Date dataCalcoloDa, Date dataCalcoloA, String queryString) {
    ...
    public void insertTotaliPerGiornoOptimized(boolean firstTime, Date dtRifContaDa, Date dtRifContaA)
        ...
        if (!dtRifContaA.equals(a.getTime())) {
            ...
            // Recupero se nel periodo in questione esistono dei record con dt_annul dove dt_creazione < dt_annul:
            // mi interessano solo questi perché se l'annullamento avviene all'interno del periodo considerato per il
            ...
            if (recordDaRicalcolareList != null && !recordDaRicalcolareList.isEmpty()) {
                for (int i = 0; i < recordDaRicalcolareList.size(); i++) {
                    dataDaCuiRicalcolareSet.add((Date) (((Object[]) recordDaRicalcolareList.get(i))[1]));
                    idStrutDaRicalcolare
                            .add(((BigDecimal) (((Object[]) recordDaRicalcolareList.get(i))[0])).longValue());
                }
            }

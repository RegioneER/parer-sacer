...
public class CreazioneIndiceVersFascicoliTimer extends JobTimer {
    ...
    public void doJob(Timer timer) {
        if (timer.getInfo().equals(jobName)) {
            try {
                ...
                logger.error("Errore nell'esecuzione del job di creazione degli indici versamento fascicoli", e);
            }
        }
    }
    ...
    public void startProcess(Timer timer) throws Exception {
        logger.info("Job automatico per la creazione automatica indici versamento fascicoli avviato");
        // Operazione ATOMICA di inizio schedulazione Job
        LogJob logJob = jobHelper.writeAtomicLogJob(OpTypeEnum.CREAZIONE_INDICI_ELENCHI_VERS_FASC.name(),
                OpTypeEnum.INIZIO_SCHEDULAZIONE.name());
        try {
            indexEjb.buildIndex(logJob);
        } catch (ParerUserError ue) {
            jobHelper.writeAtomicLogJob(OpTypeEnum.CREAZIONE_INDICI_ELENCHI_VERS_FASC.name(), OpTypeEnum.ERRORE.name(),
                    ue.getDescription());
            logger.error("Errore nell'esecuzione del job di creazione automatica degli indici versamento fascicoli",
                    ue);
            ...
            jobHelper.writeAtomicLogJob(OpTypeEnum.CREAZIONE_INDICI_ELENCHI_VERS_FASC.name(), OpTypeEnum.ERRORE.name(),
                    message);
            logger.error("Errore nell'esecuzione del job di creazione automatica degli indici versamento fascicoli", e);
            logger.info("Timer cancellato");
            timer.cancel();
        }
    }

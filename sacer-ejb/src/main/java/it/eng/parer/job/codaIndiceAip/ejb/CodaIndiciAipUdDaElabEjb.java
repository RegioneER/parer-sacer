...
public class CodaIndiciAipUdDaElabEjb {
    ...
    public void codaIndiciAipUdDaElab() throws Exception {
        ...
        for (OrgVLisStrutPerEle struttura : strutture) {
            ...
            if (totMessiInCodaFase2 < numMaxUdInCodaFase2) {
                ...
                totMessiInCodaFase2 = elaboraStrutturaFase2(struttura.getIdStrut().longValue(), totMessiInCodaFase2,
                        numMaxUdInCodaFase2, numGgResetStatoInElenco);
            } else {
                LOG.info(JOB_NAME + " - processate " + totMessiInCodaFase2 + " unità documentarie su "
                        + numMaxUdInCodaFase2 + ", salta messa in coda per produzione indici aip ed esce");
            }

        }
        // TODO: spostare la seguente riga nel timer.E' sbagliato avere una dipendenza da jobHelper in questa classe.
        jobHelper.writeLogJob(OpTypeEnum.PRODUCER_CODA_INDICI_AIP_DA_ELAB.name(), OpTypeEnum.FINE_SCHEDULAZIONE.name());
    }

    private Integer elaboraStrutturaFase2(long idStruttura, Integer totMessiInCoda, int numMax,
            int numGgResetStatoInElenco) {
        // determino gli elenchi appartenenti alla struttura corrente, con stato FIRME_VERIFICATE_DT_VERS o
        // IN_CODA_JMS_INDICE_AIP_DA_ELAB in modo che si elaborano prima i fiscali, per anno del contenuto descending e
                ...
                for (BigDecimal idUd : evHelper.retrieveUdVersOrAggOrUpdInElencoValidate(
                    ...
                    if (totMessiInCoda < numMax) {
                        ...
                        LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": processate " + totMessiInCoda
                                + " unità documentarie dei " + elenchiPerAip.size()
                ...
                if (almenoUnaUd) {
                    ...
                    if (!esisteStato) {
                        ...
                        evEjb.registraStatoElencoVersamento(
                                ...
                                ElvStatoElencoVer.TiStatoElenco.IN_CODA_JMS_INDICE_AIP_DA_ELAB, null);
                    }
                }
            } else {
                LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": processate " + totMessiInCoda
        ...
        LOG.info(JOB_NAME + " - struttura id " + idStruttura + ": processati " + elenchiPerAip.size()
    ...
    public void processaElencoPerLeFasi(ElvElencoVersDaElab elencoDaElab, String stato) {
        ...
        if (evEjb.soloUdAnnul(BigDecimal.valueOf(elencoDaElabInNuovaTransazione.getElvElencoVer().getIdElencoVers()))) {
            ...
            // MAC#18167: Job PRODUCER_CODA_INDICI_AIP_DA_ELAB: set timestamp su elenchi solo se serve
            if (elencoDaElab.getTiStatoElenco().equals(ElencoEnums.ElencoStatusEnum.FIRME_VERIFICATE_DT_VERS.name())) {
    ...
    public void aggiornaElencoFase2(long idElencoVers, long idElencoVersDaElab) {
        evHelper.aggiornaElencoCorrente(idElencoVers, ElencoEnums.ElencoStatusEnum.IN_CODA_INDICE_AIP);
        evHelper.aggiornaElencoDaElabCorrente(idElencoVersDaElab, ElencoEnums.ElencoStatusEnum.IN_CODA_INDICE_AIP);
        // EVO 19304

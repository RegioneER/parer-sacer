...
public class VerificaFirmeTimer extends JobTimer {
    ...
    public void doJob(Timer timer) {
        if (timer.getInfo().equals(jobName)) {
            try {
                ...
                logger.error("Errore nell'esecuzione del job di verifica firme alla data di versamento", e);
            }
        }
    }
    ...
    public void startProcess(Timer timer) throws Exception {
        ...
        LogJob logJob = jobHelper.writeAtomicLogJob(JobConstants.JobEnum.VERIFICA_FIRME_A_DATA_VERS.name(),
        ...
        try {
            ...
            logger.error("Errore nell'esecuzione del job di verifica firme alla data di versamento", ue);
            ...
            jobHelper.writeAtomicLogJob(OpTypeEnum.VERIFICA_FIRME_A_DATA_VERS.name(), OpTypeEnum.ERRORE.name(),
                    message);
            logger.error("Errore nell'esecuzione del job di verifica firme alla data di versamento", e);
            logger.info("Timer cancellato");
            timer.cancel();

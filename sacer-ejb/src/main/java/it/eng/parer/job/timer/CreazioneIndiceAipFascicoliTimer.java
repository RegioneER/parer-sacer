 ...
 * @author DiLorenzo_F
 */
@Singleton(mappedName = "CreazioneIndiceAipFascicoliTimer")
@Lock(LockType.READ)
@LocalBean
@Interceptors({ it.eng.parer.aop.TransactionInterceptor.class })
public class CreazioneIndiceAipFascicoliTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(CreazioneIndiceAipFascicoliTimer.class);
    @EJB
    private CreazioneIndiceAipFascicoliTimer thisTimer;
    @EJB
    private CreazioneIndiceAipFascicoliEjb creazioneIndiceAipFascicoliEjb;

    public CreazioneIndiceAipFascicoliTimer() {
        super(JobConstants.JobEnum.CREAZIONE_INDICE_AIP_FASC.name());
        logger.debug("{} creato", CreazioneIndiceAipFascicoliTimer.class.getName());
    }
    ...
    public void startCronScheduled(CronSchedule sched, String appplicationName) {
        ...
        if (!existTimer) {
            ...
            logger.info("Lancio il timer CreazioneIndiceAipFascicoliTimer...");
    ...
    public void doJob(Timer timer) {
        if (timer.getInfo().equals(jobName)) {
            try {
                ...
                logger.error("Errore nell'esecuzione del job di creazione degli indici AIP fascicoli", e);
            }
        }
    }
    ...
    public void startProcess(Timer timer) {
        try {
            ...
            creazioneIndiceAipFascicoliEjb.creazioneIndiceAipFascicoli();
            ...
            logger.error("Errore nell'esecuzione del job di creazione indice AIP fascicoli", e);
            timer.cancel();
        } catch (Exception e) {
            // questo log viene scritto solo in caso di errore.
            String message = "Errore nell'esecuzione del job di creazione indice AIP fascicoli "

...
public class VerificaMassivaVersamentiFallitiEjb {
    ...
    public void verificaMassivaVersamentiFalliti() {
        ...
        log.info("Recuperate " + struttureVersanti.size() + " strutture per verifica massiva versamenti falliti");
            ...
            if (idLock != null) {
                try {
                    ...
                    jobHelper.writeAtomicLogJob(JobConstants.JobEnum.VERIFICA_VERS_FALLITI.name(),
                            JobConstants.OpTypeEnum.INIZIO_SCHEDULAZIONE.name(), null, strutturaVersante.getIdStrut());
                    calcoloAsync.verificaVersamentiFalliti(strutturaVersante.getIdStrut(), idLock, ultimaRegistrazione);
                    log.info(VerificaMassivaVersamentiFallitiEjb.class.getSimpleName()
                            + " --- FINE chiamata per verifica versamenti falliti per la struttura: "
                            + strutturaVersante.getIdStrut());
                } catch (ParerInternalError ex) {
                    // Inutile in quanto già gestita nell'interceptor all'interno della classe asincrona
                    log.error(
                            "Errore nel job di VERIFICA_VERSAMENTI_FALLITI per la struttura: "
                                    + strutturaVersante.getIdStrut() + " " + ExceptionUtils.getRootCauseMessage(ex),
                            ex);
                    struttureWithProblems.add(strutturaVersante);
                }
            } else {
                // Se è fallita l'acquisizione del lock
                log.error("Errore nell'acquisizione del lock nel job di VERIFICA_VERSAMENTI_FALLITI per la struttura: "
                        + strutturaVersante.getIdStrut());
                struttureWithProblems.add(strutturaVersante);
            }
        }
        if (struttureWithProblems.isEmpty()) {
            ...
            jobHelper.writeAtomicLogJob(JobConstants.JobEnum.VERIFICA_MASSIVA_VERS_FALLITI.name(),
                    ...
                    "Problemi sulle seguenti strutture: " + this.esplodiStrutture(struttureWithProblems).toString());
            log.info(VerificaMassivaVersamentiFallitiEjb.class.getSimpleName()

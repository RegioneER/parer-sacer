...
public class CalcoloContenutoFascicoliEjb {
    ...
    private final org.slf4j.Logger logger = LoggerFactory.getLogger(CalcoloContenutoFascicoliEjb.class);
    ...
    public void calcolaContenutoFascicoli() throws ParerInternalError {
        ...
        Date ultimaDataEsecuzione = jobHelper
                .findUltimaAttivazioneByJob(JobConstants.JobEnum.CALCOLO_CONTENUTO_FASCICOLI.name());
        ...
        logger.info("{} - Inizio esecuzione job - {}", JobConstants.JobEnum.CALCOLO_CONTENUTO_FASCICOLI.name(),
                dataJob);
        CalcoloContenutoFascicoliEjb me = context.getBusinessObject(CalcoloContenutoFascicoliEjb.class);
        ...
        dataJob = new Date();
        logger.info("{} - Fine esecuzione job - {}", JobConstants.JobEnum.CALCOLO_CONTENUTO_FASCICOLI.name(), dataJob);
    }
    ...
    private void calcolaFascicoliVersatiDelGiorno(Date data) {
        ...
        if (l != null && !l.isEmpty()) {
            for (Iterator<Object[]> iterator = l.iterator(); iterator.hasNext();) {
                ...
                BigDecimal idStrut = new BigDecimal((long) ogg[0]), idTipoFascicolo = new BigDecimal((long) ogg[1]);
                BigDecimal aaFascicolo = (BigDecimal) ogg[2], idUser = new BigDecimal((long) ogg[3]),
                        conteggio = new BigDecimal((long) ogg[4]);
                List<MonContaFascicoli> lm = fascicoliHelper.retrieveMonContaFascicoliByChiaveTotaliz(idStrut, data,
                        idTipoFascicolo, aaFascicolo, idUser);
                ...
                m.setDecTipoFascicolo(fascicoliHelper.findById(DecTipoFascicolo.class, idTipoFascicolo));
    ...
    private void calcolaFascicoliNonVersatiDelGiorno(Date data) {
        ...
        if (l != null && !l.isEmpty()) {
            for (Iterator<Object[]> iterator = l.iterator(); iterator.hasNext();) {
                Object[] ogg = iterator.next();
                BigDecimal idStrut = new BigDecimal((long) ogg[0]), idTipoFascicolo = new BigDecimal((long) ogg[1]);
                BigDecimal aaFascicolo = (BigDecimal) ogg[2], conteggio = new BigDecimal((long) ogg[3]);
                String statoFascicoloNonVersato = (String) ogg[4];
                List<MonContaFascicoliKo> lm = fascicoliHelper.retrieveMonContaFascicoliNonVersByChiaveTotaliz(idStrut,
                        data, idTipoFascicolo, aaFascicolo, statoFascicoloNonVersato);

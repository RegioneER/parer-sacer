...
public class ValidazioneFascicoliEjb {
    ...
    public void validazioneFascicoli() throws ParerInternalError, ParerUserError, IOException,
	    NoSuchAlgorithmException, NamingException, JAXBException, ParseException {
	logger.info(ValidazioneFascicoliEjb.class.getSimpleName()
		+ " --- Validazione fascicoli --- Inizio job");
	/* Determino le strutture versanti in ordine alfabetico */
	List<OrgStrut> strutList = struttureHelper.retrieveOrgStrutList();
	logger.info(ValidazioneFascicoliEjb.class.getSimpleName()
		+ " --- Validazione fascicoli - Sono state ricavate: " + strutList.size()
		+ " strutture versanti da elaborare");

	for (OrgStrut strut : strutList) {
	    // Determina gli elenchi con stato VALIDATO appartenenti alla struttura corrente, in
	    // ordine di data di firma
	    // crescente
	    List<ElvElencoVersFascDaElab> elencoVersFascDaElabList = vfHelper
		    .getElvElencoVersFascDaElab(strut.getIdStrut(),
			    TiStatoElencoFascDaElab.VALIDATO);
	    logger.debug(ValidazioneFascicoliEjb.class.getSimpleName()
		    + " --- Validazione fascicoli - Per la struttura " + strut.getNmStrut()
		    + " sono stati ricavati " + elencoVersFascDaElabList.size()
		    + " elenchi con stato VALIDATO da elaborare");

	    /* Per ogni elenco VALIDATO */
	    for (ElvElencoVersFascDaElab elencoVersFascDaElab : elencoVersFascDaElabList) {
		// Apro nuova transazione
		context.getBusinessObject(ValidazioneFascicoliEjb.class).elaboraElencoFirmato(
			strut.getIdStrut(), elencoVersFascDaElab.getIdElencoVersDaElab());
	    }
	}
    ...
    public void elaboraElencoFirmato(long idStrut, long idElencoVersDaElab)
	    throws ParerUserError, ParerInternalError, IOException, NoSuchAlgorithmException,
	    NamingException, JAXBException, ParseException {

	ElvElencoVersFascDaElab elencoVersFascDaElab = helper
		.findById(ElvElencoVersFascDaElab.class, idElencoVersDaElab);

	// Determina i fascicoli contenuti nell'elenco NON ANNULLATI in ordine di anno e numero
	// fascicolo
	List<FasFascicolo> fascicoli = vfHelper.getFascicoliInElencoNonAnnullati(
		elencoVersFascDaElab.getElvElencoVersFasc().getIdElencoVersFasc());

	logger.debug(ValidazioneFascicoliEjb.class.getSimpleName()
		+ " --- Validazione fascicoli - Per l'elenco "
		+ elencoVersFascDaElab.getElvElencoVersFasc().getIdElencoVersFasc()
		+ " sono stati ricavati " + fascicoli.size()
		+ " fascicoli non annullati da elaborare");

	OrgStrut strut = vfHelper.findById(OrgStrut.class, elencoVersFascDaElab.getIdStrut());
	BigDecimal idAmbiente = BigDecimal
		.valueOf(strut.getOrgEnte().getOrgAmbiente().getIdAmbiente());

	Long idUserIam = userHelper.findIamUser(confHelper.getValoreParamApplicByStrut(
		CostantiDB.ParametroAppl.USERID_CREAZIONE_IX_AIP_SERIE, idAmbiente,
		elencoVersFascDaElab.getIdStrut())).getIdUserIam();

	// Per ogni fascicolo
	for (FasFascicolo fascicolo : fascicoli) {
	    // Apro nuova transazione
	    context.getBusinessObject(ValidazioneFascicoliEjb.class).verificaFascicolo(idUserIam,
		    idStrut, fascicolo.getIdFascicolo(),
		    elencoVersFascDaElab.getElvElencoVersFasc().getIdElencoVersFasc());
	}
	...
	if (vfHelper.allFascicoliAnnullati(elencoVersFasc.getIdElencoVersFasc())) {
	    ...
	    long idStatoElencoVersFasc = insertElvStatoElencoVersFasc(
		    elencoVersFasc.getIdElencoVersFasc(), idUserIam, TiStatoElencoFasc.COMPLETATO);
	    ...
	    String noteTemp = elencoVersFascDaElab.getElvElencoVersFasc()
		    .getNtElencoChiuso() != null
		...
		long idStatoElencoVersFasc = insertElvStatoElencoVersFasc(
			elencoVersFascDaElab.getElvElencoVersFasc().getIdElencoVersFasc(),
			idUserIam, TiStatoElencoFasc.IN_CODA_CREAZIONE_AIP);
    ...
    @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    public void verificaFascicolo(long idUserIam, long idStrut, long idFascicolo,
	    long idElencoVersFasc) throws ParerUserError, ParerInternalError, IOException,
	    NoSuchAlgorithmException, NamingException, JAXBException, ParseException {
	...
	// Verifico se il fascicolo è annullato. L'ho già fatto prima, ma siccome non avevo
	// effettuato LOCK, eseguo un
	// ulteriore controllo
	...
	// Assumo lock esclusivo su tutte le unità doc del fascicolo (dovrebbe verificarsi una
	// SELECT FOR UPDATE...)
    ...
    private void annullaFascicoloPerUdAnnullata(long idStrutCorrente, long idFascicolo,
	    BigDecimal aaFascicolo, String cdKeyFascicolo) throws ParerInternalError {
	// Determino l’insieme delle unità doc appartenenti al fascicolo con stato di conservazione
	// = ANNULLATA
	// e preparo una stringa con la concatenazione degli estremi delle ud annullate contenute
	// nel fascicolo
	...
	if (!udAnnullateFascicolo.isEmpty()) {
		...
		estremiConcatenati = estremiConcatenati
			+ udAnnullataFascicolo.getCdRegistroKeyUnitaDoc() + " - "
			+ udAnnullataFascicolo.getAaKeyUnitaDoc() + " - "
			+ udAnnullataFascicolo.getCdKeyUnitaDoc() + ";\n ";
	    }

	    // Ricavo l'utente che ha definito la più recente richiesta di annullamento con cui sono
	    // state annullate le
	    // ud
	    // (lo ricavo tramite ARO_STATO_RICH_ANNUL_VERS con stato APERTA)
	    long idUserIam = helper.getStatoRichiestaRecenteAnnulUd(idUdAnn).getIamUser()
		    .getIdUserIam();
	    ...
	    String note = "Nel fascicolo sono presenti le seguenti unità documentarie annullate: "
		    + estremiConcatenati;
	    ...
	    // Evasione richiesta
	    avEjb.evasioneRichiestaAnnullamento(rich, idUserIam, Constants.WS_ANNULLAMENTO);
	}
    }

    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    private void validaFascicolo(long idUserIam, long idFascicolo, long idElencoVersFasc)
	    throws ParerInternalError, IOException, NoSuchAlgorithmException, NamingException,
	    JAXBException, ParerUserError, ParseException {
	/*
	 * Determino se tutte le unità doc hanno stato di conservazione = AIP_GENERATO o AIP_FIRMATO
	 * (per assicurare che non siano in corso modifiche) oppure IN_ARCHIVIO o
	 * VERSAMENTO_IN_ARCHIVIO (caso di ud appartenente ad altro fascicolo / serie che sta
	 * entrando in archivio o e’ in archivio) oppure IN_VOLUME_DI_CONSERVAZIONE (stato delle ud
	 * in volumi firmati e, quindi, senza indice AIP per cui si deve creare l’AIP)
	 */
	List<String> statiConservazione = Arrays.asList(
	...
	if (vfHelper.allUdFascicoloStatiConservazione(idFascicolo, statiConservazione)) {
	    Date dataCorrente = new Date();
	    /*
	     * Aggiorna le unità doc appartenenti al fascicolo con stato di conservazione =
	     * AIP_GENERATO o AIP_FIRMATO o IN_ARCHIVIO, assegnando stato di conservazione =
	    ...
	    List<String> statiUdDocDaCambiare = new ArrayList<>(
		    Arrays.asList(CostantiDB.StatoConservazioneUnitaDoc.AIP_GENERATO.name(),
			    CostantiDB.StatoConservazioneUnitaDoc.AIP_FIRMATO.name(),
			    CostantiDB.StatoConservazioneUnitaDoc.IN_ARCHIVIO.name()));
	    String statoUdNuovo = CostantiDB.StatoConservazioneUnitaDoc.VERSAMENTO_IN_ARCHIVIO
		    .name();

	    // MEV #31162
	    List<Long> idUnitaDocDaFascicoloList = vfHelper.getIdUnitaDocDaFascicolo(idFascicolo,
		    statiUdDocDaCambiare);
	    // end MEV #31162

	    vfHelper.updateStatoConservazioneUdFascicolo(idFascicolo, statiUdDocDaCambiare,
		    statoUdNuovo);

	    // MEV #31162
	    for (Long idUnitaDocDaFascicolo : idUnitaDocDaFascicoloList) {
		udEjb.insertLogStatoConservUd(idUnitaDocDaFascicolo,
			Constants.JOB_VALIDAZIONE_FASCICOLO, Constants.VALIDA_FASCICOLO,
			CostantiDB.StatoConservazioneUnitaDoc.VERSAMENTO_IN_ARCHIVIO.name(),
	    ...
	    /*
	     * Aggiorna le unità doc appartenenti al fascicolo con stato di conservazione =
	     * IN_VOLUME_DI_CONSERVAZIONE, assegnando stato di conservazione = AIP_DA_GENERARE
	     */
	    statiUdDocDaCambiare = new ArrayList<>(Arrays.asList(
		    CostantiDB.StatoConservazioneUnitaDoc.IN_VOLUME_DI_CONSERVAZIONE.name()));
	    statoUdNuovo = CostantiDB.StatoConservazioneUnitaDoc.AIP_DA_GENERARE.name();

	    // MEV #31162
	    idUnitaDocDaFascicoloList = vfHelper.getIdUnitaDocDaFascicolo(idFascicolo,
		    statiUdDocDaCambiare);
	    // end MEV #31162
	    ...
	    // Registro in FAS_AIP_FASCICOLO_DA_ELAB il fascicolo per cui si deve generare l'indice
	    // AIP
	    FasAipFascicoloDaElab aipFascicoloDaElab = new FasAipFascicoloDaElab();
	    FasFascicolo fascicolo = vfHelper.findById(FasFascicolo.class, idFascicolo);
	    aipFascicoloDaElab.setFasFascicolo(fascicolo);
	    BigDecimal pgCreazioneDaElab = BigDecimal
		    .valueOf(vfHelper.getLastPgFascicoloCoda(idFascicolo)).add(BigDecimal.ONE);
	    ...
	    ElvElencoVersFasc elencoVersFasc = vfHelper.findById(ElvElencoVersFasc.class,
		    idElencoVersFasc);
	    ...
	    fascicolo.setTiStatoFascElencoVers(
	    ...
	    statoFascicoloElenco
		    .setTiStatoFascElencoVers(TiStatoFascElenco.IN_ELENCO_IN_CODA_CREAZIONE_AIP);
	    ...
	    // Determina le unità doc del fascicolo (avranno tutte stato di conservazione pari a
	    // VERSAMENTO_IN_ARCHIVIO)
		...
		for (AroUnitaDoc ud : udList) {
		    // Determino la versione più recente dell'indice AIP dell'unità doc (quella con
		    // progressivo
		    // maggiore)
		    AroVerIndiceAipUd ultimaVersione = vfHelper
			    .getUltimaVersioneIndiceAip(ud.getIdUnitaDoc());
			...
			if (ultimaVersione.getDsHashAip() == null) {
			    ...
			    String hashAipFile = getHashAIPPerValidazioneFascicoli(
				    user.getIdUserIam(), ud.getIdUnitaDoc(),
				    ud.getCdRegistroKeyUnitaDoc(), ud.getAaKeyUnitaDoc(),
				    ud.getCdKeyUnitaDoc(), ultimaVersione.getCdVerIndiceAip());
			    ultimaVersione.setDsHashAip(hashAipFile);
			    ultimaVersione
				    .setCdEncodingHashAip(TipiEncBinari.HEX_BINARY.descrivi()); // hexBinary
			    ultimaVersione.setDsAlgoHashAip(TipiHash.SHA_256.descrivi()); // SHA-256
			}

			// Registro in FAS_UD_AIP_FASCICOLO_DA_ELAB la versione indice aip
			// dell'unità doc usata dal
    ...
    private void generaIndiceAipUdVolume(long idUnitaDoc) throws ParerInternalError,
	    NamingException, NoSuchAlgorithmException, IOException, JAXBException {
	...
	String causale = "Aggiornamento alle nuove modalità di conservazione di unità documentaria già presente nel Volume di conservazione "
		+ "n. " + volumeConserv.getIdVolumeConserv() + " del "
		+ volumeConserv.getTmMarcaIndice();
	...
	unitaDoc.setTiStatoConservazione(
		CostantiDB.StatoConservazioneUnitaDoc.VERSAMENTO_IN_ARCHIVIO.name());
	// MEV #31162
    ...
    public void creaPrimoIndiceAipUniSincro(Long idUnitaDoc, String causale)
	    throws ParerInternalError, NamingException, NoSuchAlgorithmException, IOException,
	    JAXBException {
	logger.debug(ValidazioneFascicoliEjb.class.getSimpleName()
	...
	String sincroVersion = confHelper
    ...
    // @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
    @TransactionAttribute(TransactionAttributeType.REQUIRED)
    public void verificaUrnUdFascicolo(long idUnitaDoc, long idFascicolo)
	    throws ParerUserError, ParerInternalError, IOException, NoSuchAlgorithmException,
	    NamingException, JAXBException, ParseException {
	...
	AroVerIndiceAipUd aroVerIndiceAipUd = vfHelper
    ...
    public long insertElvStatoElencoVersFasc(long idElencoVersFasc, long idUserIam,
	    TiStatoElencoFasc tiStato) {
     ...
     * @param cdVerIndiceAip        versione indice aip
     *
     ...
     */
    public String getHashAIPPerValidazioneFascicoli(long idUserIam, long idUnitaDoc,
	    String cdRegistroKeyUnitaDoc, BigDecimal aaKeyUnitaDoc, String cdKeyUnitaDoc,
	    String cdVerIndiceAip)
	...
	CostantiDB.TipiEntitaRecupero tipoEntitaRecupero = (majorNumber < 1)
		? CostantiDB.TipiEntitaRecupero.UNI_DOC_UNISYNCRO
	...
	recXml.getVersatore()
		.setAmbiente(ud.getOrgStrut().getOrgEnte().getOrgAmbiente().getNmAmbiente());

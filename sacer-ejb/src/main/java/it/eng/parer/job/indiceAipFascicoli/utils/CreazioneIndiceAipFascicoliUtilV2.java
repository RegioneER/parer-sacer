...
public class CreazioneIndiceAipFascicoliUtilV2 {
    ...
    private static final Logger log = LoggerFactory.getLogger(CreazioneIndiceAipFascicoliUtilV2.class);
     ...
     *            codice versione
     * @param codiceVersioneMetadati
     *            codice versione metadati
     * @param mappaAgenti
     *            mappa chiave/valore
    ...
    public PIndex generaIndiceAIPV2Strict(FasVerAipFascicolo verAipFascicolo, String codiceVersione,
            String codiceVersioneMetadati, Map<String, String> mappaAgenti, String sistemaConservazione,
        ...
        popolaPIndex(istanzaUnisincro, verAipFascicolo.getIdVerAipFascicolo(), verAipFascicolo.getFasFascicolo(),
                codiceVersione, codiceVersioneMetadati, mappaAgenti, sistemaConservazione, creatingApplicationProducer,
                Boolean.TRUE);
     ...
     *             errore generico
     */
    public PIndex generaIndiceAIPV2NotStrict(FasVerAipFascicolo verAipFascicolo, String codiceVersione,
            String codiceVersioneMetadati, Map<String, String> mappaAgenti, String sistemaConservazione,
            String creatingApplicationProducer)
            throws ParerInternalError, DatatypeConfigurationException, JAXBException {
        PIndex istanzaUnisincro = new PIndex();
     ...
     *             errore generico
     */
    public PIndex generaIndiceAIPV2Strict(long idVerAipFascicolo, FasFascicolo fasc, String codiceVersione,
            String codiceVersioneMetadati, Map<String, String> mappaAgenti, String sistemaConservazione,
            String creatingApplicationProducer)
            throws ParerInternalError, DatatypeConfigurationException, JAXBException {
        PIndex istanzaUnisincro = new PIndex();
        popolaPIndex(istanzaUnisincro, idVerAipFascicolo, fasc, codiceVersione, codiceVersioneMetadati, mappaAgenti,
                sistemaConservazione, creatingApplicationProducer, Boolean.TRUE);
        return istanzaUnisincro;
    }

    private void popolaPIndex(PIndex pIndex, long idVerAipFascicolo, FasFascicolo fasFascicolo, String codiceVersione,
            String codiceVersioneMetadati, Map<String, String> mappaAgenti, String sistemaConservazione,
            String creatingApplicationProducer, Boolean strictMode)
            throws ParerInternalError, DatatypeConfigurationException, JAXBException {
        ...
        String tmpUrnNorm = MessaggiWSFormat.formattaBaseUrnFascicolo(
                ...
                MessaggiWSFormat.formattaUrnPartFasc(csChiaveFasc, true, Costanti.UrnFormatter.FASC_FMT_STRING));
        // salvo NORMALIZZATO
        // MAC#28051
        ...
        String urnAIPFasc = MessaggiWSFormat.formattaUrnAipFascicolo(
                MessaggiWSFormat.formattaBaseUrnFascicolo(urnPartVersatoreNorm, urnPartChiaveFascNorm));
        ...
        String tmpUrnName = MessaggiWSFormat.formattaUrnAipMetaFascicolo(
                MessaggiWSFormat.formattaChiaveFascicolo(csVersatore, csChiaveFasc), codiceVersioneMetadati);
        // end MEV#29589
            ...
            FasMetaVerAipFascicolo fasMetaVerAipFasc = (FasMetaVerAipFascicolo) CollectionUtils.find(versioneCorrente,
                    new Predicate() {
                        ...
                        public boolean evaluate(final Object object) {
                            return ((FasMetaVerAipFascicolo) object).getNmMeta().toUpperCase().equals("FASCICOLO");
                        }
                    });

            hash.setValue(fasMetaVerAipFasc.getDsHashFile());
            String function = fasMetaVerAipFasc.getDsAlgoHashFile() != null ? fasMetaVerAipFasc.getDsAlgoHashFile()
                    : NON_DEFINITO;
        ...
        if (!rispostaControlli.isrBoolean()) {
            ...
            for (int i = 0; i < authSignerList.size(); i++) {
                ...
                rispostaControlli = controlliRecuperoFasc
                        .leggiRuoloAuthorizedSigner(authSignerList.get(i).getIdUserIam(), idAmbiente);
                if (!rispostaControlli.isrBoolean()) {
                    setRispostaError();
                } else {
    ...
    private void popolaFileGroupAipUdList(PIndex pIndex, List<FasContenVerAipFascicolo> fasContenVerAipFascList) {
        ...
        for (FasContenVerAipFascicolo fasContenVerAipFasc : fasContenVerAipFascList) {
            ...
            AroUrnVerIndiceAipUd aroUrnAipIndiceAipUdNorm = (AroUrnVerIndiceAipUd) CollectionUtils.find(
                    fasContenVerAipFasc.getAroVerIndiceAipUd().getAroUrnVerIndiceAipUds(),
                    object -> ((AroUrnVerIndiceAipUd) object).getTiUrn()
                            .equals(it.eng.parer.entity.constraint.AroUrnVerIndiceAipUd.TiUrnVerIxAipUd.NORMALIZZATO));
            if (aroUrnAipIndiceAipUdNorm != null) {
                File tmpFileItem = new File();
                // ID
                ID tmpIdFileItem = new ID();
                tmpIdFileItem.setValue(aroUrnAipIndiceAipUdNorm.getDsUrn()
                        .replaceAll(IOUtils.extractPartUrnName(aroUrnAipIndiceAipUdNorm.getDsUrn()), "AIP-UD"));
                ...
                String tmpPath = aroUrnAipIndiceAipUdNorm.getDsUrn().replaceAll(
                        IOUtils.extractPartUrnName(aroUrnAipIndiceAipUdNorm.getDsUrn()), "unitadocumentarie");
                ...
                String pathZip = IOUtils.getAbsolutePath(folder, fileName, IOUtils.UNIX_FILE_SEPARATOR);
                ...
                String function = fasContenVerAipFasc.getAroVerIndiceAipUd() != null
                        ? fasContenVerAipFasc.getAroVerIndiceAipUd().getDsAlgoHashAip() : NON_DEFINITO;
                tmpHashFileItem.setHashFunction(function);
                tmpFileItem.setHash(tmpHashFileItem);
                //
                /* FILEGROUP DEI DOCUMENTI DEL PACCHETTO DI ARCHIVIAZIONE (AIP) DELLE UNITA' DOCUMENTARIE */
                FileGroup fileGroupAipUp = mappaGruppoFileAipUd
                        .computeIfAbsent(
                                fasContenVerAipFasc.getFasVerAipFascicolo().getDsUrnAipFascicolo().replaceAll(
    ...
    private void popolaFileGroupSipFascList(PIndex pIndex, List<FasSipVerAipFascicolo> fasSipVerAipFascList) {
        ...
        for (FasSipVerAipFascicolo fasSipVerAipFasc : fasSipVerAipFascList) {
            ...
            for (FasXmlVersFascicolo fasXmlVersFascicolo : xmlVersFascicoloList) {
                if (fasXmlVersFascicolo.getDsUrnXmlVers() != null) {
                    ...
                    // Path
                    /* Definisco la folder relativa al sistema di conservazione */
                    String tmpPath = fasXmlVersFascicolo.getDsUrnXmlVers()
                            .replaceAll(IOUtils.extractPartUrnName(fasXmlVersFascicolo.getDsUrnXmlVers()), "SIP-FA");
                    String path = IOUtils.extractPartUrnName(tmpPath, true);
                    ...
                    String function = fasXmlVersFascicolo.getDsAlgoHashXmlVers() != null
                            ? fasXmlVersFascicolo.getDsAlgoHashXmlVers() : NON_DEFINITO;
                    tmpHashFileItem.setHashFunction(function);
                    // end MAC#25654
                    tmpFileItem.setHash(tmpHashFileItem);
                    //
                    ...
                    FileGroup fileGroupSipFasc = mappaGruppoFileSipFasc.computeIfAbsent(
                            fasXmlVersFascicolo.getDsUrnXmlVers()
                                    .replaceAll(IOUtils.extractPartUrnName(fasXmlVersFascicolo.getDsUrnXmlVers()), ""),
    ...
    private FileGroup getFileGroupAipUd(String urn) {
        ...
        fileGroupAipUd.setDescription("Raggruppa tutti gli Aip delle unità documentarie contenute nel fascicolo");
    ...
    private void popolaMetadatiIntegratiSelfDesc(FasFascicolo tmpFasFascicolo,
            MetadatiIntegratiSelfDescriptionType miSelfD, String codiceVersione, XMLGregorianCalendar timeRef)
         ...
         * e per la versione del modello xsd corrispondente a quella del servizio di versamento fascicolo, con il tipo
         * "AIP_SELF_DESCRIPTION_MORE_INFO"
         */
        rispostaControlli.reset();
        rispostaControlli = controlliRecIndiceAipFascicoli.getDecModelloSelfDescMoreInfoV2(idAmbiente, cdVersioneXml);
        if (!rispostaControlli.isrBoolean()) {
            setRispostaError();
        } else {
            List<DecModelloXsdFascicolo> decModelloXsdFascList = (List<DecModelloXsdFascicolo>) rispostaControlli
                    .getrObject();
            log.info(
                    "Creazione Indice AIP Fascicoli v2 - ambiente id {}: trovati {}"
            ...
            if (decModelloXsdFascList.isEmpty()) {
                throw new ParerInternalError(
                        "Il modello di tipo AIP_SELF_DESCRIPTION_MORE_INFO per la data corrente e l'ambiente "
                                ...
                                + " non è definito per la versione " + cdVersioneXml + " del servizio di versamento");
            }

...
public class ElaborazioneRigaVersioneFascicoliDaElab {
    ...
    public void gestisciIndiceAipFascicoliDaElab(long idFascDaElab) throws Exception {
        ...
        if (fascDaElab != null) {
            BigDecimal idFascicolo = BigDecimal
                    .valueOf(fascDaElab.getFasFascicolo().getIdFascicolo());
            FasXmlVersFascicolo xmlVersFascicolo = ciafHelper
                    .getFasXmlVersFascicolo(idFascicolo.longValue(), "RICHIESTA");
            gestisciIndiceAipFascicoliDaElab(fascDaElab, xmlVersFascicolo.getCdVersioneXml());
        }
    }
    // end MEV#26576

    public void gestisciIndiceAipFascicoliDaElab(FasAipFascicoloDaElab fascDaElab,
            String cdVersioneXml) throws Exception {
        ...
        BackendStorage backendIndiciAipFascicoli = objectStorageService.lookupBackendFasc(
                aaTipoFascicoloCorrente.getIdAaTipoFascicolo(),
                CostantiDB.ParametroAppl.BACKEND_INDICI_AIP_FASCICOLI);
        ...
        log.debug("{} - Elaboro il fascicolo {}", desJobMessage,
                fascDaElab.getFasFascicolo().getIdFascicolo());
        FasFascicolo fascicolo = ciafHelper.findById(FasFascicolo.class,
                fascDaElab.getFasFascicolo().getIdFascicolo());
        ...
        String codiceVersioneMetadati = ciafHelper
                .getVersioneMetadatiAIPV2(fascicolo.getIdFascicolo(), tiCreazione);
        ...
        FasVerAipFascicolo lastVer = ciafHelper.registraAIP(fascDaElab, progressivoVersione,
                codiceVersione, sistemaConservazione);

        /* Crea File fascicolo --> file FASCICOLO */
        log.debug("{} - Creo il file XML", desJobMessage);
        elaborazioneMeta.creaMetaVerFascicolo(lastVer.getIdVerAipFascicolo(),
        ...
        // MEV #30398
        /*
         * Se backendIndiciAipFascicoli di tipo O.S. si effettua il salvataggio (con link su
         * apposita entity)
         */
        if (backendIndiciAipFascicoli.isObjectStorage()) {
            ...
            versatore.setAmbiente(fascDaElab.getFasFascicolo().getOrgStrut().getOrgEnte()
            ...
            ObjectStorageResource indiceAipFascSuOS = objectStorageService
                    .createResourcesInIndiciAipFasc(tmpUrnNorm,
                            backendIndiciAipFascicoli.getBackendName(), indiciAipFascicoliBlob,
                            lastVer.getIdVerAipFascicolo(), BigDecimal.valueOf(
                                    fascDaElab.getFasFascicolo().getOrgStrut().getIdStrut()));
            log.debug("Salvati i file indice AIP fascicolo nel bucket {} con chiave {} ",
                    indiceAipFascSuOS.getBucket(), indiceAipFascSuOS.getKey());
        }
        // end MEV #30398
        ...
        log.debug(
                "{} - Registro un nuovo stato nell’elenco del fascicolo a IN_ELENCO_CON_AIP_CREATO",
                desJobMessage);
        ...
        log.debug(
                "{} - Aggiorno il fascicolo assegnando stato di conservazione pari a AIP_GENERATO",
                desJobMessage);
        fascicolo.setTiStatoConservazione(
                it.eng.parer.entity.constraint.FasFascicolo.TiStatoConservazione.AIP_GENERATO);

        // Registro un nuovo stato di conservazione del fascicolo specificando stato = AIP_GENERATO
        log.debug("{} - Registro un nuovo stato di conservazione del fascicolo a AIP_GENERATO",
                desJobMessage);
        ...
        // Vista per verificare che tutti i fascicoli aggiunti appartenenti all'indice aip abbiano
        // stato
        // IN_ELENCO_CON_AIP_CREATO
        ...
        if (view.getFlAllAipCreati().equals("1")) {
            ...
            fascDaElab.getElvElencoVersFasc().getElvStatoElencoVersFascicoli()
            ...
            Long idStatoElencoVersFasc = elencoHelper.getStatoElencoByIdElencoVersFascStato(
                    fascDaElab.getElvElencoVersFasc().getIdElencoVersFasc(),
                    TiStatoElencoFasc.AIP_CREATI).getIdStatoElencoVersFasc();
            fascDaElab.getElvElencoVersFasc()

...
import it.eng.parer.job.calcoloEstrazione.RestituzioneArchivioJob;
 ...
 * @author DiLorenzo_F
 */
@Singleton(mappedName = "RestituzioneArchivioTimer")
@LocalBean
@Lock(LockType.READ)
public class RestituzioneArchivioTimer extends JobTimer {

    private static final Logger logger = LoggerFactory.getLogger(RestituzioneArchivioTimer.class);

    @EJB
    private RestituzioneArchivioTimer thisTimer;
    @EJB
    private RestituzioneArchivioJob restituzioneArchivioJob;

    public RestituzioneArchivioTimer() {
        super(JobConstants.JobEnum.EVASIONE_RICH_REST_ARCH.name());
        logger.debug(RestituzioneArchivioTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String applicationName) {
        ...
        if (!existTimer) {
            ...
            logger.info("Lancio il timer RestituzioneArchivioTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            logger.info(JobConstants.JobEnum.EVASIONE_RICH_REST_ARCH.name() + " --- Inizio schedulazione job");
            LogJob logJob = jobHelper.writeAtomicLogJob(jobName, JobConstants.OpTypeEnum.INIZIO_SCHEDULAZIONE.name());
            restituzioneArchivioJob.restituzioneArchivio(logJob);
            ...
            logger.error("Errore nell'esecuzione del job di restituzione archivio", e);
            timer.cancel();
        } catch (Exception e) {
            // questo log viene scritto solo in caso di errore.
            String message = ExceptionUtils.getRootCauseMessage(e);
            jobHelper.writeAtomicLogJob(jobName, JobConstants.OpTypeEnum.ERRORE.name(),
                    "Errore nell'esecuzione del job di restituzione archivio:" + message);
            logger.error("Errore nell'esecuzione del job di restituzione archivio", e);
            timer.cancel();
        }
    }

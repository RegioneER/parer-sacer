 ...
 * @author Gilioli_P
 */
@Singleton(mappedName = "CreazioneIndiceAipTimer")
@Lock(LockType.READ)
@LocalBean
@Interceptors({
	it.eng.parer.aop.TransactionInterceptor.class })
public class CreazioneIndiceAipTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(CreazioneIndiceAipTimer.class);
    @EJB
    private CreazioneIndiceAipTimer thisTimer;
    @EJB
    private CreazioneIndiceAipEjb creazioneIndiceAipEjb;

    public CreazioneIndiceAipTimer() {
	super(JobConstants.JobEnum.CREAZIONE_INDICE_AIP.name());
	logger.debug("{} creato", CreazioneIndiceAipTimer.class.getName());
    }
	...
	if (!existTimer) {
	    ...
	    logger.info("Lancio il timer CreazioneIndiceAipTimer...");
	...
	if (timer.getInfo().equals(jobName)) {
		...
		logger.error("Errore nell'esecuzione del job di creazione degli indici AIP", e);
	    }
	}
    }
	...
	try {
	    ...
	    creazioneIndiceAipEjb.creazioneIndiceAip();
	    ...
	    logger.error("Errore nell'esecuzione del job di creazione indice AIP", e);
	    timer.cancel();
	} catch (Exception e) {
	    // questo log viene scritto solo in caso di errore.
	    String message = "Errore nell'esecuzione del job di creazione indice AIP "

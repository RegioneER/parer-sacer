...
import it.eng.parer.job.serie.ejb.ControlloContenutoEffettivoSerieJob;
import it.eng.parer.job.utils.JobConstants;

@Singleton(mappedName = "GenerazioneContenutoEffettivoSerieTimer")
@LocalBean
@Lock(LockType.READ)
public class ControlloContenutoEffettivoSerieTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(ControlloContenutoEffettivoSerieTimer.class);
    @EJB
    private ControlloContenutoEffettivoSerieTimer thisTimer;
    @EJB
    private ControlloContenutoEffettivoSerieJob controlloEffettivoEjb;

    public ControlloContenutoEffettivoSerieTimer() {
        super(JobConstants.JobEnum.CONTROLLO_AUTOMATICO_CONTENUTO_EFFETTIVO.name());
        logger.debug(ControlloContenutoEffettivoSerieTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String appplicationName) {
        ...
        if (!existTimer) {
            ...
            logger.info("Lancio il timer ControlloContenutoEffettivoSerieTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            ...
            controlloEffettivoEjb.controllaContenutoEffettivo();
            ...
            logger.error("Errore nell'esecuzione del job di controllo automatico contenuto effettivo serie", e);
            timer.cancel();
        } catch (Exception e) {
            // questo log viene scritto solo in caso di errore.
            String message = ExceptionUtils.getRootCauseMessage(e);
            jobHelper.writeAtomicLogJob(jobName, JobConstants.OpTypeEnum.ERRORE.name(),
                    "Errore nell'esecuzione del job di controllo automatico contenuto effettivo serie:" + message);
            logger.error("Errore nell'esecuzione del job di controllo automatico contenuto effettivo serie", e);
            timer.cancel();
        }
    }

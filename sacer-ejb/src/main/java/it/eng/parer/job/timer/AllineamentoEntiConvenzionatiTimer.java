 ...
 * @author Gilioli_P
 */
@Singleton(mappedName = "AllineamentoEntiConvenzionatiTimer")
@Lock(LockType.READ)
@LocalBean
public class AllineamentoEntiConvenzionatiTimer extends JobTimer {

    private Logger logger = LoggerFactory.getLogger(AllineamentoEntiConvenzionatiTimer.class);
    @EJB
    private AllineamentoEntiConvenzionatiTimer thisTimer;
    @EJB
    private AllineamentoEntiConvenzionatiEjb allineamentoEntiConvenzionatiEjb;

    public AllineamentoEntiConvenzionatiTimer() {
        super(JobConstants.JobEnum.ALLINEA_ENTI_CONVENZIONATI.name());
        logger.debug(AllineamentoEntiConvenzionatiTimer.class.getName() + " creato");
    }
    ...
    public void startCronScheduled(CronSchedule sched, String appplicationName) {
        ...
        if (!existTimer) {
            ...
            logger.info("Lancio il timer AllineamentoEntiConvenzionatiTimer...");
    ...
    public void startProcess(Timer timer) {
        try {
            ...
            allineamentoEntiConvenzionatiEjb.allineaEntiConvenzionati();
        } catch (Exception e) {
            ...
            logger.error("Errore nell'esecuzione del job di allineamento enti convenzionati", e);
        }
    }
}

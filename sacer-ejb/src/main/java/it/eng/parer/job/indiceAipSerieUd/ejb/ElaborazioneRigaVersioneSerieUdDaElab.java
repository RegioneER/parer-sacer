...
public class ElaborazioneRigaVersioneSerieUdDaElab {
    ...
    public void gestisciIndiceAipSerieUdDaElab(SerVerSerieDaElab verSerieDaElab,
            String creatingApplicationProducer) throws Exception {
        Long idVerSerie = verSerieDaElab.getSerVerSerie().getIdVerSerie();
        // Numero di unità documentarie con cui creare i volumi definito dal tipo di serie della
        // versione della serie
        BigDecimal num = verSerieDaElab.getSerVerSerie().getSerSerie().getDecTipoSerie()
                .getNiUnitaDocVolume();
        ...
        // Numero di unità documentarie appartenenti al contenuto di tipo EFFETTIVO della versione
        // serie
        // per le quali la foreign key al volume sia nulla (cioè non fanno ancora parte di un
        // volume)
        BigDecimal numUdEffettive = new BigDecimal(
                ciasudHelper.getNumUdEffettiveSenzaVolume(idVerSerie));
        // Numero volumi da creare, arrotondando all'intero superiore
        BigDecimal numVolumiDaCreare = (numUdEffettive.divide(numUdPerVolume,
                RoundingMode.CEILING));

        log.info(ElaborazioneRigaVersioneSerieUdDaElab.class.getSimpleName()
                + " --- Creazione Indice Aip Versione Serie Ud --- " + "Per la versione serie "
                + idVerSerie + " verranno creati " + numVolumiDaCreare + " volumi, con "
                + numUdPerVolume + " unità documentarie in ogni volume");

        /* CREA I VOLUMI ED I RELATIVI INDICI */
        // (una transazione per ogni volume)
        log.info(ElaborazioneRigaVersioneSerieUdDaElab.class.getSimpleName()
                + " --- Creazione Indice Aip Versione Serie Ud --- "
                + "Inizio creazione volumi e relativi indici ");
        ...
        log.info(ElaborazioneRigaVersioneSerieUdDaElab.class.getSimpleName()
                + " --- Creazione Indice Aip Versione Serie Ud --- "
                + "Termine creazione volumi e relativi indici ");

        /* CREA INDICE AIP VERSIONE SERIE */
        log.info(ElaborazioneRigaVersioneSerieUdDaElab.class.getSimpleName()
                + " --- Creazione Indice Aip Versione Serie Ud --- "
                + "Inizio creazione indice AIP versione serie ");
        elaborazioneAip.creaIndiceAipSerieUd(verSerieDaElab.getIdVerSerieDaElab(),
                creatingApplicationProducer);
        log.info(ElaborazioneRigaVersioneSerieUdDaElab.class.getSimpleName()
                + " --- Creazione Indice Aip Versione Serie Ud --- "
                + "Termine creazione indice AIP versione serie ");
    }
}

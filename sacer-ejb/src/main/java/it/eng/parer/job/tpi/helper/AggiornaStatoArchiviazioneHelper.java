...
public class AggiornaStatoArchiviazioneHelper {
	...
	Query query = entityManager
		.createQuery("SELECT CONCAT(amb.nmAmbiente, '-', ente.nmEnte,'-', strut.nmStrut) "
			+ "FROM OrgStrut strut JOIN strut.orgEnte ente "
			+ "JOIN ente.orgAmbiente amb " + "WHERE strut.idStrut = :idStrut");
	...
	Query query = entityManager.createQuery(
		"SELECT COUNT(comp) FROM AroCompDoc comp JOIN comp.aroStrutDoc strutDoc "
			+ "JOIN strutDoc.aroDoc doc JOIN doc.aroUnitaDoc ud JOIN ud.decTipoUnitaDoc tipoUd "
			+ "WHERE doc.idStrut = :idStrut "
			+ "AND doc.dtCreazione between :dtVersFrom and :dtVersTo "
    ...
    public boolean handleResponse(StatoArchiviazioneCartellaRisposta resp, Long idVrsDate,
	    Map<String, String> params) throws ParerInternalError {
	...
	vrsDate.setFlArkSecondario(Boolean.FALSE.equals(resp.getFlPresenzaSitoSecondario()) ? null
		: JobConstants.DB_TRUE);
	vrsDate.setFlFileNoArkSecondario(
		Boolean.FALSE.equals(resp.getFlPresenzaSitoSecondario()) ? null
			: JobConstants.DB_FALSE);
	Date dtLastArk = null;
	Date dtLastArkSecondario = null;
	for (VrsPathDtVers path : vrsDate.getVrsPathDtVers()) {
	    /*
	     * Elimina i record di VRS_ARK_PATH_DT_VERS e VRS_FILE_NOARK_PATH_DT_VERS relativi al
	     * path nella data di versamento
	     */
	    log.debug("{} --- gestisco il path {}",
		...
		if (aggregato.getListaArk() == null || aggregato.getListaArk().isEmpty()) {
		    ...
		    if (pathStrutture.getListaFileNoArk() != null
			    && !pathStrutture.getListaFileNoArk().isEmpty()) {
		...
		if (Boolean.TRUE.equals(resp.getFlPresenzaSitoSecondario())) {
		    if (aggregato.getListaArkSecondario() == null
			    || aggregato.getListaArkSecondario().isEmpty()) {
			...
			if (pathStrutture.getListaFileNoArkSecondario() != null
			    ...
			    for (FilePathNoArk filePathNoArk : pathStrutture
				    .getListaFileNoArkSecondario()) {
				VrsFileNoarkPathDtVers vrsFileNoArk = new VrsFileNoarkPathDtVers();
				vrsFileNoArk.setDsFileNoark(filePathNoArk.getDlFile());
				vrsFileNoArk
					.setTiArkFileNoark(JobConstants.ArkPath.SECONDARIO.name());
				vrsFileNoArk.setVrsPathDtVers(path);
				path.getVrsFileNoarkPathDtVers().add(vrsFileNoArk);
				entityManager.persist(vrsFileNoArk);
			    }
		...
		if (dtLastArk == null
			|| (dtLastArkPath != null && dtLastArk.compareTo(dtLastArkPath) < 0)) {
		...
		// SimpleDateFormat(Costanti.UrnFormatter.SPATH_DATA_FMT_STRING);
		DateTimeFormatter df = DateTimeFormatter
			.ofPattern(Costanti.UrnFormatter.SPATH_DATA_FMT_STRING);
		// end MAC#27666
		log.error(
			"{} --- Il path {} non è presente in nessun aggregato esistente per la data di versamento {}",
			JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name(), path.getDlPath(),
	...
	log.debug("{} --- Data aggiornata con stato {}",
		JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name(),
	...
	if (vrsDate.getTiStatoDtVers().equals(JobConstants.ArkStatusEnum.ARCHIVIATA.name())) {
	    ...
	    // MAC#27666
	    from.setTime(Date
		    .from(vrsDate.getDtVers().atStartOfDay(ZoneId.systemDefault()).toInstant()));
	    // end MAC#27666
		...
		log.info("{} --- VERSAMENTO - Eseguo l'update tutte le UD con data creazione "
			+ "inclusa nella data di versamento corrente, assegnando stato = IN_ATTESA_SCHED",
			JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name());
		tmpUdDaElabElencoList = updateUdDaElabElenco(from.getTime(), to.getTime(),
			idStrutList);
		log.info("{} --- VERSAMENTO - Eseguo l'update tutti i documenti con data creazione "
			+ "inclusa nella data di versamento corrente, assegnando stato = IN_ATTESA_SCHED",
			JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name());
		tmpDocDaElabElencoList = updateDocDaElabElenco(from.getTime(), to.getTime(),
			idStrutList);
	    }
	    ...
	    RichiestaTpiInput inputParams = new RichiestaTpiInput(
		    RichiestaTpi.TipoRichiesta.ELIMINA_CARTELLA_ARK, urlRequest, timeout,
		...
		jobHelper.writeAtomicLogJob(
			...
			"Timeout dal servizio EliminaCartellaArk");
		jobChiusoOk = false;
	    } else if (codiceEsito.equals(EsitoConnessione.Esito.KO.name())) {
		log.error("{} --- EliminaCartellaArk - {} - {}",
			JobConstants.JobEnum.AGGIORNA_STATO_ARCHIVIAZIONE.name(), codiceErrore,
			messaggioErrore);
		...
		throw new ParerInternalError(
			"EliminaCartellaArk - " + codiceErrore + " - " + messaggioErrore);
	    } else {
	...
	/*
	 * Se non ci sono stati problemi nella eventuale chiamata di EliminaCartellaArk e se la data
	 * di versamento corrente ha stato = DA_ARCHIVIARE o ARCHIVIATA o ARCHIVIATA_ERR e se per la
	 * data di versamento esiste almeno un record in VRS_PATH_DT_VERS con ni_file_path nullo)
	 * aggiorna tutti i record VRS_PATH_DT_VERS della data di versamento corrente, calcolando
	 * ni_file_path
	 */
	if (jobChiusoOk && (vrsDate.getTiStatoDtVers()
		.equals(JobConstants.ArkStatusEnum.DA_ARCHIVIARE.name())
	...
	if (tmpUdDaElabElencoList != null && !tmpUdDaElabElencoList.isEmpty()) {
		...
		jmsProducerUtilEjb.manageMessageGroupingInFormatoJson(connectionFactory, queue, pl,
			"CodaElenchiDaElab", String.valueOf(pl.getIdStrut()));
	    }
	}
	...
	String queryStr = "DELETE FROM " + table
		+ " tb WHERE tb.vrsPathDtVers.idPathDtVers = :idPath";
    ...
    public List<ElvUdVersDaElabElenco> updateUdDaElabElenco(Date from, Date to,
	    List<Long> idStrutList) {
	List<ElvUdVersDaElabElenco> tmpUdElabElencoList = new ArrayList<>();
	for (Long idStrut : idStrutList) {
	    String queryStr = "SELECT deel from ElvUdVersDaElabElenco deel "
		    + "where deel.idStrut = :idStrut " + "AND deel.tiStatoUdDaElab =  :tiStato "
		    + "AND deel.dtCreazione between :dtVersFrom and :dtVersTo ";
	    Query q = entityManager.createQuery(queryStr);
	    q.setParameter("idStrut", bigDecimalFromLong(idStrut));
	    q.setParameter("tiStato",
		    CostantiDB.TipiStatoElementoVersato.IN_ATTESA_MEMORIZZAZIONE.name());
		...
		elenco.setTiStatoUdDaElab(
			CostantiDB.TipiStatoElementoVersato.IN_ATTESA_SCHED.name());
		elenco.getAroUnitaDoc().setTiStatoUdElencoVers(
			CostantiDB.TipiStatoElementoVersato.IN_ATTESA_SCHED.name());
	    }
    ...
    public List<ElvDocAggDaElabElenco> updateDocDaElabElenco(Date from, Date to,
	    List<Long> idStrutList) {
	List<ElvDocAggDaElabElenco> tmpDocElabElencoList = new ArrayList<>();
	for (Long idStrut : idStrutList) {
	    String queryStr = "SELECT deel from ElvDocAggDaElabElenco deel "
		    + "where deel.idStrut = :idStrut " + "AND deel.tiStatoDocDaElab =  :tiStato "
		    + "AND deel.dtCreazione between :dtVersFrom and :dtVersTo ";
	    Query q = entityManager.createQuery(queryStr);
	    q.setParameter("idStrut", bigDecimalFromLong(idStrut));
	    q.setParameter("tiStato",
		...
		elenco.setTiStatoDocDaElab(
			CostantiDB.TipiStatoElementoVersato.IN_ATTESA_SCHED.name());
		elenco.getAroDoc().setTiStatoDocElencoVers(
			CostantiDB.TipiStatoElementoVersato.IN_ATTESA_SCHED.name());
	    }

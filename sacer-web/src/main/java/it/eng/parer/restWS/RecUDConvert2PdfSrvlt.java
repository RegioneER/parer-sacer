...
@WebServlet(urlPatterns = {
        "/RecDIPComponenteTrasformatoSync" }, asyncSupported = true)
public class RecUDConvert2PdfSrvlt extends KeycloakAuthorizationServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        myRecuperoExt.getParametriParser()
                .setPresenzaDocumento(ParametriParser.TipiPresenzaTag.Richiesto);
        myRecuperoExt.getParametriParser()
                .setPresenzaComponente(ParametriParser.TipiPresenzaTag.Richiesto);
        /**
            ...
            if (isMultipart) {
                ...
                try {
                    ...
                    if (rispostaWs.getSeverity() == SeverityEnum.OK
                        ...
                        /*
                         * Non sono stati individuati componenti il cui formato di rappresentazione
                         * sia compatibile con il recupero DIP
                         */
                    ...
                    if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                        for (CompRecDip compRecDip : rispostaWs.getDatiRecuperoDip()
                                .getElementiTrovati().values()) {
                            if (compRecDip
                                    .getStatoFileTrasform() == CostantiDB.StatoFileTrasform.ERRATO) {
                                rispostaWs.setSeverity(SeverityEnum.ERROR);
                                /*
                                 * Il convertitore {0} del formato di rappresentazione {1} relativo
                                 * al componente {2} risulta marcato come ERRATO, impossibile
                                 * procedere con la trasformazione
                                 */
                                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.RECDIP_001_002,
                                        compRecDip.getNomeConvertitore() + "."
                                                + compRecDip.getVersioneConvertitore(),
                                        compRecDip.getNomeFormatoRappresentazione(),
                                        compRecDip.getNomeFileBreve());
                                myEsito = rispostaWs.getIstanzaEsito();
                                myEsito.setXMLRichiesta(myRecuperoExt.getDatiXml());
                                break;
                            }

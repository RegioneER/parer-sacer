 ...
 * @author Fioravanti_F
 */
public class RecUDConvert2PdfSrvlt extends HttpServlet {

    private static final long serialVersionUID = 1L;
    private static final Logger log = LoggerFactory.getLogger(RecUDConvert2PdfSrvlt.class);
    private String uploadDir;
    private String instanceName;
    private int BUFFERSIZE = 10 * 1024 * 1024;

    public RecUDConvert2PdfSrvlt() throws IOException {
        super();
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        Response405.fancy405(resp, Response405.NomeWebServiceRest.REC_UD_CONVERT_PDF_SYNC);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        myRecuperoExt.setDescrizione(new WSDescRecUDPdf());
        ...
        try {
            ...
            log.error("Errore nel recupero dell'EJB RecuperoDip ", ex);
            throw new ServletException("Impossibile recuperare l'ejb RecuperoDip ", ex);
        }
        ...
        myRecuperoExt.getParametriParser().setPresenzaDocumento(ParametriParser.TipiPresenzaTag.Richiesto);
        myRecuperoExt.getParametriParser().setPresenzaComponente(ParametriParser.TipiPresenzaTag.Richiesto);
        /**
            ...
            if (isMultipart) {
                ...
                try {
                    ...
                    if (rispostaWs.getSeverity() == SeverityEnum.OK
                        ...
                        myEsito = rispostaWs.getIstanzaEsito();
                        myEsito.setXMLRichiesta(myRecuperoExt.getDatiXml());
                    }
                    if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                        recuperoDip.listaComponenti(rispostaWs, myRecuperoExt);
                    }
                    if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                        for (CompRecDip compRecDip : rispostaWs.getDatiRecuperoDip().getElementiTrovati().values()) {
                            if (compRecDip.getStatoFileTrasform() == CostantiDB.StatoFileTrasform.ERRATO) {
                                rispostaWs.setSeverity(SeverityEnum.ERROR);
                                /*
                                 * Il convertitore {0} del formato di rappresentazione {1} relativo al componente {2}
                                 * risulta marcato come ERRATO, impossibile procedere con la trasformazione
                                 */
                                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.RECDIP_001_002,
                                        compRecDip.getNomeConvertitore() + "." + compRecDip.getVersioneConvertitore(),
                                        compRecDip.getNomeFormatoRappresentazione(), compRecDip.getNomeFileBreve());
                                myEsito = rispostaWs.getIstanzaEsito();
                                myEsito.setXMLRichiesta(myRecuperoExt.getDatiXml());
                                break;
                            }
                    ...
                    if (rispostaWs.getSeverity() == SeverityEnum.OK) {
                        ...
                        recuperoDip.recuperaUnitaDocumentaria(rispostaWs, myRecuperoExt, uploadDir);
                    }
                    myEsito = rispostaWs.getIstanzaEsito();

...
public class DownloadDip {
    ...
    public DownloadDip(User user, BigDecimal idUnitaDoc) throws NamingException {
        ...
        AvanzamentoWs tmpAvanzamento = AvanzamentoWs.nuovoAvanzamentoWS("prova", AvanzamentoWs.Funzioni.RecuperoWeb);
        ...
        recuperoExt.getParametriRecupero().setTipoEntitaSacer(CostantiDB.TipiEntitaRecupero.UNI_DOC_DIP);
        recuperoExt.getParametriRecupero().setUtente(this.user);
        recuperoExt.getParametriRecupero().setIdUnitaDoc(idUnitaDoc.longValue());
    }

    public DownloadDip(RecuperoExt myRecuperoExt, RispostaWSRecupero rispostaWs) throws NamingException {
        ...
        this.idCompDoc = myRecuperoExt.getParametriRecupero().getIdComponente() != null
                ? new BigDecimal(myRecuperoExt.getParametriRecupero().getIdComponente()) : null;
        this.idDoc = myRecuperoExt.getParametriRecupero().getIdDocumento() != null
                ? new BigDecimal(myRecuperoExt.getParametriRecupero().getIdDocumento()) : null;
    ...
    public BaseTable populateComponentiDipTable() throws EMFError {
        ...
        if (getRispostaWs().getSeverity() == IRispostaWS.SeverityEnum.OK) {
            if (!getRispostaWs().getDatiRecuperoDip().getElementiTrovati().isEmpty()) {
                baseTable = DownloadDip
                        .generaTableBean(getRispostaWs().getDatiRecuperoDip().getElementiTrovati().values());
            }
        }
        return baseTable;
    }
    ...
    public void scaricaDipZip(TIPO_DOWNLOAD tipoDownload) throws EMFError {
        this.recuperoExt.getParametriRecupero().setTipoRichiedente(JobConstants.TipoSessioniRecupEnum.DOWNLOAD);
        switch (tipoDownload) {
        case SCARICA_COMP_CONV:
            recuperoDip.recuperaCompConvertito(getRispostaWs(), this.recuperoExt, System.getProperty("java.io.tmpdir"));
            break;
        case SCARICA_ZIP:
    ...
    public static BaseTable generaTableBean(Collection<CompRecDip> collection) {
        ...
        for (CompRecDip compRecDip : collection) {
            ...
            row.setObject("ti_stato_conv",
                    (compRecDip.getStatoFileTrasform() != null ? compRecDip.getStatoFileTrasform().name() : "---"));

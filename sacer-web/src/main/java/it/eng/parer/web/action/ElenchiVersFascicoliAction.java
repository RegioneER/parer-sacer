...
public class ElenchiVersFascicoliAction extends ElenchiVersFascicoliAbstractAction {
    ...
    private void initComboFiltriFascicoli(BigDecimal idStrut) {
        ...
        DecTipoFascicoloTableBean tmpTableBeanTipoFasc = fascicoliEjb.getTipiFascicoloAbilitati(getUser().getIdUtente(),
                idStrut);
    ...
    private void initBottoniPaginaDettaglioElencoVersFascicoli(BigDecimal idElencoVersFasc, String tiStato) {
        ...
        if (evfEjb.areFascDeletables(idElencoVersFasc)) {
            getForm().getListaFascicoliButtonList().getEliminaAppartenenzaFascDaElenco().setHidden(false);
            getForm().getListaFascicoliButtonList().getEliminaAppartenenzaFascDaElenco().setEditMode();
        }
        ...
        if (evfEjb.isElencoClosable2(idElencoVersFasc)) {
            getForm().getDettaglioElenchiVersFascicoliButtonList().getChiudiElencoButton().setHidden(false);
            getForm().getDettaglioElenchiVersFascicoliButtonList().getChiudiElencoButton().setEditMode();
        }
    ...
    public void loadListaElenchiVersFascicoliDaFirmare() throws EMFError {
                ...
                if (!getMessageBox().hasError()) {
                    ...
                    ElvVRicElencoFascByStatoTableBean elenchiVersFascicoliTableBean = evfEjb
                            .getElenchiVersFascicoliDaFirmareTableBean(strut.getBigDecimal("id_ambiente"),
                                    ...
                                    getForm().getFiltriElenchiVersFascDaFirmare().getElenchi_con_note().parse(),
                                    ElencoEnums.ElencoStatusEnum.CHIUSO, dateCreazioneElencoFascValidate,
                                    getUser().getIdUtente());
        ...
        Future<Boolean> futureFirma = (Future<Boolean>) getSession().getAttribute(Signature.FUTURE_ATTR_ELENCHI_FASC);
        ...
        if (elencoFascSignSessionEjb.hasUserActiveSessions(getUser().getIdUtente()) || futureFirma != null) {
    ...
    private void initComboRicercaElenchi() throws EMFError {
        ...
        getForm().getFiltriElenchiVersFascicoli().getTi_stato().setDecodeMap(ComboGetter
                .getMappaSortedGenericEnum("ti_stato", ElencoEnums.ElencoStatusEnum.getComboMappaStatoElencoRicerca()));
        ...
        checkUniqueStrutInCombo(idStrut, ActionEnums.SezioneElenchiVersFascicoli.RICERCA_ELENCHI_VERS_FASC);
    }

    /**
     * Metodo utilizzato per controllare il valore nella combo struttura quando questo è l'unico presente e settare di
     * conseguenza le combo Tipo Fascicolo e Criterio di Raggruppamento Fascicoli
     *
     * @param idStrut
     *            id struttura
     * @param sezione
    ...
    public void checkUniqueStrutInCombo(BigDecimal idStrut, Enum<ActionEnums.SezioneElenchiVersFascicoli> sezione)
        ...
        if (idStrut != null) {
            ...
            DecodeMap mappaCriteri = DecodeMap.Factory.newInstance(tmpTableBeanCriteri, "id_criterio_raggr_fasc",
                    "nm_criterio_raggr");
            ...
            if (sezione.equals(ActionEnums.SezioneElenchiVersFascicoli.RICERCA_ELENCHI_VERS_FASC)) {
                getForm().getFiltriElenchiVersFascicoli().getId_criterio_raggr_fasc().setDecodeMap(mappaCriteri);
                getForm().getFiltriElenchiVersFascicoli().getId_tipo_fascicolo().setDecodeMap(mappaTipoFascicolo);
    ...
    public Fields triggerAmbienteGenerico(Fields campi, ActionEnums.SezioneElenchiVersFascicoli sezione)
            throws EMFError {
    ...
    public Fields triggerEnteGenerico(Fields campi, ActionEnums.SezioneElenchiVersFascicoli sezione) throws EMFError {
        campi.post(getRequest());
        ...
        if (idEnte != null) {
            ...
            } else if (sezione.equals(ActionEnums.SezioneElenchiVersFascicoli.RICERCA_ELENCHI_VERS_FASC)) {
                if (tipoFascicoloCombo != null) {
                    tipoFascicoloCombo.setDecodeMap(new DecodeMap());
                }
                if (criterioRaggrCombo != null) {
    ...
    public void loadDettaglio() throws EMFError {
        ...
        if (lista != null && azione != null && !azione.equals(NE_DETTAGLIO_INSERT)) {
            ...
            if (lista.equals(getForm().getElenchiVersFascicoliList().getName())) {
                BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow()
                ...
                BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliDaFirmareList().getTable()
                        .getCurrentRow().getBigDecimal(ElvVRicElencoFascByStatoTableDescriptor.COL_ID_ELENCO_VERS_FASC);
                dettaglioElencoVersFascicoli(idElencoVersFasc);
            } else if (lista.equals(getForm().getElenchiIndiciAipFascDaFirmareList().getName())) {
                BigDecimal idElencoVersFasc = getForm().getElenchiIndiciAipFascDaFirmareList().getTable()
                        .getCurrentRow().getBigDecimal(ElvVRicElencoFascByStatoTableDescriptor.COL_ID_ELENCO_VERS_FASC);
                dettaglioElencoVersFascicoli(idElencoVersFasc);
            } else if (lista.equals(getForm().getElenchiVersFascicoliSelezionatiList().getName())) {
                BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliSelezionatiList().getTable()
                        .getCurrentRow().getBigDecimal(ElvVRicElencoFascByStatoTableDescriptor.COL_ID_ELENCO_VERS_FASC);
                dettaglioElencoVersFascicoli(idElencoVersFasc);
            } else if (lista.equals(getForm().getElenchiIndiciAipFascSelezionatiList().getName())) {
                BigDecimal idElencoVersFasc = getForm().getElenchiIndiciAipFascSelezionatiList().getTable()
                        .getCurrentRow().getBigDecimal(ElvVRicElencoFascByStatoTableDescriptor.COL_ID_ELENCO_VERS_FASC);
                dettaglioElencoVersFascicoli(idElencoVersFasc);
            }
        }
    ...
    public void dettaglioElencoVersFascicoli(BigDecimal idElencoVersFasc) throws EMFError {
        ...
        ElvElencoVersFascRowBean elencoVersFascRowBean = evfEjb.getElvElencoVersFascRowBean(idElencoVersFasc);
        ElvStatoElencoVersFascRowBean statoElencoVersFascRowBean = evfEjb
                .getElvStatoElencoVersFascRowBean(elencoVersFascRowBean.getIdStatoElencoVersFascCor());
        ...
        getForm().getStatiElencoVersFascicoliList().getTable()
                .addSortingRule(getForm().getStatiElencoVersFascicoliList().getTs_stato().getName(), SortingRule.ASC);
        ...
        if ((getTableName() != null && (getTableName().equals(getForm().getElenchiVersFascicoliList().getName())
                || getTableName().equals(getForm().getElenchiVersFascicoliDaFirmareList().getName())
                || getTableName().equals(getForm().getElenchiIndiciAipFascDaFirmareList().getName())))
                || getTableName() == null) {
            /* Gestione dei tipi dato soggetti alle abilitazioni */
            DecTipoFascicoloTableBean tmpTableBeanTipoFasc = fascicoliEjb
                    .getTipiFascicoloAbilitati(getUser().getIdUtente(), elencoVersFascRowBean.getIdStrut());
            FasFascicoloTableBean listaFasc = fascicoliHelper.getListaFasFascicoloElvViewBean(idElencoVersFasc,
                    new ElenchiVersFascicoliForm.FascicoliFiltri(), tmpTableBeanTipoFasc);
    ...
    public void dettaglioOnClick() throws EMFError {
        ...
        if (!azione.equals(ListAction.NE_DETTAGLIO_DELETE)) {
            ...
            } /* Dettaglio FASCICOLO */ else if (lista.equals(getForm().getFascicoliList().getName())) {
                BigDecimal idFascicolo = null;
                /* Verifico se l'utente può accedere al dettaglio fascicolo */
                FasFascicoloRowBean rigaFasc = (FasFascicoloRowBean) getForm().getFascicoliList().getTable()
                        .getCurrentRow();
                if (tipiFascicoloAbilitatiSet.contains(rigaFasc.getIdTipoFascicolo())) {
                    idFascicolo = rigaFasc.getIdFascicolo();
                } else {
                    getMessageBox().addError("Utente non abilitato ad accedere al dettaglio del fascicolo selezionato");
                    ...
                    if (!fasVRicFascicoliRowBean.getTiStatoConservazione()
                        ...
                        redirectToAction(Application.Actions.FASCICOLI,
                                "?operation=listNavigationOnClick&navigationEvent=" + ListAction.NE_DETTAGLIO_VIEW
                                        ...
                                        + getForm().getFascicoliList().getTable().getCurrentRowIndex(),
                                form);
                    } else {
    ...
    public void updateElenchiVersFascicoliList() {
        String statoElenco = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow().getString("ti_stato");
        boolean editable = false;
        if (statoElenco.equals(ElencoStatusEnum.CHIUSO.name()) || statoElenco.equals(ElencoStatusEnum.FIRMATO.name())
    ...
    public void deleteElenchiVersFascicoliList() throws EMFError {
        BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow()
                .getBigDecimal("id_elenco_vers_fasc");
        /* Se non posso eliminare l'elenco, avverto l'utente... */
        if (!evfEjb.isElencoDeletable(idElencoVersFasc)) {
            getMessageBox().addError("L'elenco di versamento fascicoli è stato firmato: eliminazione non consentita");
            ...
            try {
                ...
                getMessageBox().addError("Errore durante l'eliminazione dell'elenco di versamento fascicoli");
            } finally {
    ...
    public void deleteElenchiVersFascicoliDaFirmareList() throws EMFError {
        BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getCurrentRow()
                .getBigDecimal("id_elenco_vers_fasc");
        try {
            evfEjb.deleteElenco(getIdUtenteCorrente(), idElencoVersFasc);
            ...
            getMessageBox().addError("Errore durante l'eliminazione dell'elenco di versamento fascicoli da firmare");
        } finally {
            String lastPublisher = getLastPublisher();
            if (Application.Publisher.ELENCHI_VERS_FASCICOLI_DETAIL.equals(lastPublisher)) {
                goBack();
    ...
    public void saveDettaglio() throws EMFError {
        ...
        BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow()
                .getBigDecimal("id_elenco_vers_fasc");
        String rowNtElencoChiuso = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow()
                .getString("nt_elenco_chiuso");
        String rowNtIndiceElenco = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow()
                .getString("nt_indice_elenco");
        String rowTiStato = getForm().getElenchiVersFascicoliList().getTable().getCurrentRow().getString("ti_stato");

        /* Valori post modifiche */
        getForm().getElenchiVersFascicoliDetail().post(getRequest());
        String ntIndiceElenco = getForm().getElenchiVersFascicoliDetail().getNt_indice_elenco().parse();
        String ntElencoChiuso = getForm().getElenchiVersFascicoliDetail().getNt_elenco_chiuso().parse();
    ...
    public void undoDettaglio() throws EMFError {
        dettaglioElencoVersFascicoli(getForm().getElenchiVersFascicoliDetail().getId_elenco_vers_fasc().parse());
        setElencoVersFascicoliListAndDetailViewMode();
        forwardToPublisher(Application.Publisher.ELENCHI_VERS_FASCICOLI_DETAIL);
    }
    ...
    public void reloadAfterGoBack(String publisherName) {
        if (publisherName != null) {
            String lista = getTableName();
            /*
             * Se tornando indietro devo andare nella pagina di Ricerca Elenchi di Versamento Fascicoli rilancio la
             * ricerca in quanto posso aver modificato l'elenco esaminato
             */
            if (publisherName.equals(Application.Publisher.ELENCHI_VERS_FASCICOLI_RICERCA)
                    && (lista != null && lista.equals(getForm().getElenchiVersFascicoliList().getName()))
    ...
    public void ricercaElenchiButton() throws EMFError {
        ...
        if (filtriElenchiVersFascicoli.validate(getMessageBox())) {
            ...
            validator.validaChiaviFascicoliElenchi(result, filtriElenchiVersFascicoli.getAa_fascicolo(),
                    filtriElenchiVersFascicoli.getAa_fascicolo_da(), filtriElenchiVersFascicoli.getAa_fascicolo_a(),
            ...
            if (!getMessageBox().hasError()) {
                ...
                if (range) {
                    filtriElenchiVersFascicoli.getAa_fascicolo_da().setValue(
                            result.getAa_fascicolo_da() != null ? result.getAa_fascicolo_da().toString() : null);
                    filtriElenchiVersFascicoli.getAa_fascicolo_a().setValue(
                            result.getAa_fascicolo_a() != null ? result.getAa_fascicolo_a().toString() : null);
                    filtriElenchiVersFascicoli.getCd_key_fascicolo_da()
                            .setValue(result.getCd_key_fascicolo_da() != null ? result.getCd_key_fascicolo_da() : null);
                    filtriElenchiVersFascicoli.getCd_key_fascicolo_a()
                            .setValue(result.getCd_key_fascicolo_a() != null ? result.getCd_key_fascicolo_a() : null);
                }
                ...
                /* Effettuo la ricerca */
                if (chiave || range || idTipoFascicolo != null || StringUtils.isNotBlank(cdCompositoVoceTitol)
                        || StringUtils.isBlank(tiStato)) {
                    // Se sono presenti i filtri del fascicolo
                    getForm().getElenchiVersFascicoliList().setTable(evfEjb
                            .getElvVRicElencoFascByFasTableBean(getUser().getIdUtente(), filtriElenchiVersFascicoli));
                } else {
                    ...
                    if (!tiStato.equals(ElencoEnums.ElencoStatusEnum.COMPLETATO.name())) {
                        ...
                        getForm().getElenchiVersFascicoliList().setTable(evfEjb.getElvVRicElencoFascByStatoTableBean(
                                getUser().getIdUtente(), filtriElenchiVersFascicoli));
                    } else {
                        getForm().getElenchiVersFascicoliList().setTable(evfEjb
                                .getElvVRicElencoFascTableBean(getUser().getIdUtente(), filtriElenchiVersFascicoli));
                    }
                }
                ...
                /*
                 * Imposto la sezione del filtro chiave fascicolo aperto dopo la ricerca se ho inserito un valore nel
                 * campo tipo fascicolo
                 */
                if (!getForm().getFiltriElenchiVersFascicoli().getId_tipo_fascicolo().getValue().equals("")) {
                    getForm().getFascicoloSection().setLoadOpened(true);
                }
            }
        }
    ...
    public void ricercaFasc() throws EMFError {
        ...
        if (fascfiltri.validate(getMessageBox())) {
            ...
            RicercaFascicoliBean result = new RicercaFascicoliBean();
            FascicoliValidator validator = new FascicoliValidator(getMessageBox());
            validator.validaOrdineDateOrari(fascfiltri.getTs_ini_ses_da(), fascfiltri.getTs_ini_ses_a());
            validator.validaOrdineDateOrari(fascfiltri.getDt_ape_fascicolo_da(), fascfiltri.getDt_ape_fascicolo_a());
            validator.validaOrdineDateOrari(fascfiltri.getDt_chiu_fascicolo_da(), fascfiltri.getDt_chiu_fascicolo_a());
            ...
            if (!getMessageBox().hasError()) {
                ...
                if (range) {
                    fascfiltri.getAa_fascicolo_da().setValue(
                            result.getAa_fascicolo_da() != null ? result.getAa_fascicolo_da().toString() : null);
                    fascfiltri.getAa_fascicolo_a().setValue(
                            result.getAa_fascicolo_a() != null ? result.getAa_fascicolo_a().toString() : null);
                ...
                getForm().getFascicoliList()
                        .setTable(fascicoliHelper.getListaFasFascicoloElvViewBean(
                                getForm().getElenchiVersFascicoliDetail().getId_elenco_vers_fasc().parse(), fascfiltri,
                                tmpTableBeanTipoFascicolo));
                getForm().getFascicoliList().getTable().setPageSize(10);
        ...
        // Workaround per evitare che il trigger scarichi la pagina HTML anziche' visualizzarla sul browser
        forwardToPublisher(Application.Publisher.ELENCHI_VERS_FASCICOLI_DETAIL);
    }
    ...
    /**
     * Bottone di chiusura elenco di versamento fascicolo. Se presente, significa che sono già soddisfatte le condizioni
     * per la chiusura dell'elenco (ovvero stato APERTO e con almeno un fascicolo appartenente)
     *
     * @throws Throwable
     *             errore generico
     */
    ...
    public void selectElenchiVersFascicoliDaFirmareList() throws EMFError {
        ...
        int index = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getCurrentRowIndex();
        ...
        int paginaCorrente = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getFirstRowPageIndex();
        ...
        getForm().getElenchiVersFascicoliSelezionatiList().getTable().addSortingRule(
                getForm().getElenchiVersFascicoliSelezionatiList().getTs_creazione_elenco().getName(), SortingRule.ASC);
        getForm().getElenchiVersFascicoliSelezionatiList().getTable().sort();
        forwardToPublisher(Application.Publisher.LISTA_ELENCHI_VERS_FASCICOLI_SELECT);
    }

    /**
     * Bottone "-" della "Lista elenchi di versamento fascicoli selezionati" per spostare un elenco da questa lista a
     * quella degli elenchi di versamento fascicoli da firmare
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectElenchiVersFascicoliSelezionatiList() throws EMFError {
        ...
        int index = getForm().getElenchiVersFascicoliSelezionatiList().getTable().getCurrentRowIndex();
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiVersFascicoliSelezionatiList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiVersFascicoliSelezionatiList().getTable().getFirstRowPageIndex();
        ...
        int paginaCorrenteVF = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getCurrentPageIndex();
        int inizioVF = getForm().getElenchiVersFascicoliDaFirmareList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
        ...
        if (fascicoliSelezionati != null) {
            for (String fasc : fascicoliSelezionati) {
                if (StringUtils.isNumeric(fasc)) {
                    ...
                    idFascToRemove.add(((FasFascicoloRowBean) getForm().getFascicoliList().getTable()
                            .getRow(Integer.parseInt(fasc))).getIdFascicolo());
                }
            }
    ...
    public void scaricaIndiceElencoButton() throws Throwable {
        ...
        String filesPrefix = sistema + "_" + nmEnte.replaceAll(" ", "_") + "_" + nmStrut.replaceAll(" ", "_")
                + "_ElencoVers-FA-" + idElencoVersFasc;
    ...
    private void reloadElenchiVersFascicoliList(
        ...
        validator.validaChiaviFascicoliElenchi(result, filtriElenchiVersFascicoli.getAa_fascicolo(),
                filtriElenchiVersFascicoli.getAa_fascicolo_da(), filtriElenchiVersFascicoli.getAa_fascicolo_a(),
                filtriElenchiVersFascicoli.getCd_key_fascicolo(), filtriElenchiVersFascicoli.getCd_key_fascicolo_da(),
                filtriElenchiVersFascicoli.getCd_key_fascicolo_a());

        /*
         * Controllo dove sono stati inseriti i filtri tra la chiave fascicolo singola e la chiave fascicolo per range
         */
        if (result.getAa_fascicolo() != null || result.getCd_key_fascicolo() != null) {
            chiave = true;
        }
        ...
        if (chiave || range || idTipoFascicolo != null || StringUtils.isNotBlank(cdCompositoVoceTitol)
            ...
            getForm().getElenchiVersFascicoliList().setTable(
                    evfEjb.getElvVRicElencoFascByFasTableBean(getUser().getIdUtente(), filtriElenchiVersFascicoli));
        } else {
            // Se non sono presenti i filtri del fascicolo
            if (!tiStato.equals(ElencoEnums.ElencoStatusEnum.COMPLETATO.name())) {
                // Se il filtro di stato elenco è != COMPLETATO
                getForm().getElenchiVersFascicoliList().setTable(evfEjb
                        .getElvVRicElencoFascByStatoTableBean(getUser().getIdUtente(), filtriElenchiVersFascicoli));
            } else {
                getForm().getElenchiVersFascicoliList().setTable(
                        evfEjb.getElvVRicElencoFascTableBean(getUser().getIdUtente(), filtriElenchiVersFascicoli));
            }
        }
    ...
    /**
     * Inizializza i FILTRI DI LISTA ELENCHI VERSAMENTO FASCICOLI DA FIRMARE in base alla struttura con la quale
     * l'utente è loggato
     *
     * @param idStruttura
     *            id struttura
    ...
    private void initFiltriElenchiVersFascicoliDaFirmare(BigDecimal idStruttura) {
        ...
        getForm().getFiltriElenchiVersFascDaFirmare().getId_ambiente().setValue(idAmbiente.toString());
        ...
        mappaStrut.populatedMap(tmpTableBeanStruttura, "id_strut", "nm_strut");
        getForm().getFiltriElenchiVersFascDaFirmare().getId_strut().setDecodeMap(mappaStrut);
        getForm().getFiltriElenchiVersFascDaFirmare().getId_strut().setValue(idStruttura.toString());
        ...
        // (per IE)
        getForm().getFiltriElenchiVersFascDaFirmare().getRicercaElenchiVersFascDaFirmareButton().setEditMode();
    ...
    public void ricercaElenchiVersFascDaFirmareButton() throws EMFError {
        ...
        BigDecimal idAmbiente = getForm().getFiltriElenchiVersFascDaFirmare().getId_ambiente().parse();
        ...
        BigDecimal idElencoVersFasc = getForm().getFiltriElenchiVersFascDaFirmare().getId_elenco_vers_fasc().parse();
        String presenzaNote = getForm().getFiltriElenchiVersFascDaFirmare().getElenchi_con_note().parse();
    ...
    public void firmaElenchiHsmJs() throws EMFError {
        ...
        char[] passwd = getForm().getFiltriElenchiVersFascDaFirmare().getPasswd().parse() != null
                ? getForm().getFiltriElenchiVersFascDaFirmare().getPasswd().parse().toCharArray() : null;
        char[] otp = getForm().getFiltriElenchiVersFascDaFirmare().getOtp().parse() != null
                ? getForm().getFiltriElenchiVersFascDaFirmare().getOtp().parse().toCharArray() : null;
    ...
    private ElvElencoVersFascTableBean checkElenchiVersFascicoliToSign() {
        ...
        if (getForm().getElenchiVersFascicoliSelezionatiList().getTable().isEmpty()) {
            ...
            for (ElvVRicElencoFascByStatoRowBean elencoVista : (ElvVRicElencoFascByStatoTableBean) getForm()
                ...
                ElvElencoVersFascRowBean elencoVersFasc = evfEjb.getElvElencoVersFascRowBean(idElencoVersFasc);
    ...
    public void scaricaElencoIdxAipFascBtn() throws Throwable {
        ...
        String nmStrut = getForm().getElenchiVersFascicoliDetail().getNm_strut().parse();
        String filesPrefix = sistema + "_" + nmEnte.replaceAll(" ", "_") + "_" + nmStrut.replaceAll(" ", "_")
                + "_ElencoIndiciAIP-FA-" + idElencoVersFasc;
    ...
    public void loadListaElenchiIndiciAipFascDaFirmare() throws EMFError {
        ...
        if (idStrut != null && cleanFilter) {
            ...
            if (cleanList) {
                if (getForm().getFiltriElenchiIndiciAipFascDaFirmare().validate(getMessageBox())) {
                    if (!getMessageBox().hasError()) {
                        FascicoliValidator fascicoliValidator = new FascicoliValidator(getMessageBox());
                        // Valido i filtri data creazione elenco indici aip fascicoli da - a restituendo le date
                        // comprensive di orario
                        Date[] dateCreazioneElencoIdxAipFascValidate = fascicoliValidator.validaDate(
                                getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_da()
                                        .parse(),
                                ...
                                getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_a()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipFascDaFirmare()
                        ...
                        if (!getMessageBox().hasError()) {
                            ...
                            getForm().getElenchiIndiciAipFascDaFirmareList().getTable().addSortingRule(getForm()
                                    .getElenchiIndiciAipFascDaFirmareList().getDt_creazione_elenco_ix_aip().getName(),
                                    SortingRule.ASC);
        ...
        // Verifico su db la presenza della sessione di firma o di un oggetto future (di una possibile sessione di firma
        // preesistente) in sessione
        if (elencoIndiciAipFascSignSessionEjb.hasUserActiveSessions(getUser().getIdUtente()) || futureFirma != null) {
            // Se esistono delle sessioni bloccate per quell'utente le sblocco
            if (elencoIndiciAipFascSignSessionEjb.hasUserBlockedSessions(getUser().getIdUtente())) {
                ...
                getForm().getElenchiIdxAipFascDaFirmareBtnList().getFirmaElenchiIndiciAipFascHsm().setReadonly(false);
                getMessageBox().addInfo("\u00C8 stata sbloccata una sessione di firma bloccata");
                getMessageBox().setViewMode(ViewMode.plain);
            } else {
                getForm().getElenchiIdxAipFascDaFirmareBtnList().getFirmaElenchiIndiciAipFascHsm().setReadonly(true);
                // Sessione di firma attiva
                getMessageBox().addInfo("Sessione di firma attiva");
                getMessageBox().setViewMode(ViewMode.plain);
            }
        ...
        getForm().getElenchiIdxAipFascDaFirmareBtnList().getFirmaElenchiIndiciAipFascHsm().setHidden(false);
    ...
    @Override
    public JSONObject triggerFiltriElenchiIndiciAipFascDaFirmareId_ambienteOnTrigger() throws EMFError {
    ...
    public void ricercaElenchiIdxAipFascDaFirmare() throws Throwable {
        ...
        BigDecimal idAmbiente = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_ambiente().parse();
        BigDecimal idEnte = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_ente().parse();
        BigDecimal idStrut = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_strut().parse();
        BigDecimal idElencoVersFasc = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_elenco_vers_fasc()
                .parse();
        ...
        if (getForm().getFiltriElenchiIndiciAipFascDaFirmare().validate(getMessageBox())) {
            if (!getMessageBox().hasError()) {
                FascicoliValidator fascicoliValidator = new FascicoliValidator(getMessageBox());
                // Valido i filtri data creazione elenco indici aip fascicoli da - a restituendo le date comprensive di
                // orario
                Date[] dateCreazioneElencoIdxAipFascValidate = fascicoliValidator.validaDate(
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_da().parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getOre_ts_creazione_elenco_idx_aip_da()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getMinuti_ts_creazione_elenco_idx_aip_da()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_a().parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getOre_ts_creazione_elenco_idx_aip_a()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getMinuti_ts_creazione_elenco_idx_aip_a()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_da()
                                .getHtmlDescription(),
                        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getTs_creazione_elenco_idx_aip_a()
                ...
                if (!getMessageBox().hasError()) {
                    ...
                    getForm().getElenchiIndiciAipFascDaFirmareList().getTable().setPageSize(10);
                    getForm().getElenchiIndiciAipFascDaFirmareList().getTable().first();
                    getForm().getElenchiIndiciAipFascDaFirmareList().getTable().addSortingRule(
                            getForm().getElenchiIndiciAipFascDaFirmareList().getDt_creazione_elenco_ix_aip().getName(),
                            SortingRule.ASC);
    ...
    // TODO: Questo metodo può essere unificato a initFiltriElenchiDaFirmare, bisogna aspettare però il rilascio del
    ...
    private void initFiltriElenchiIndiciAipFascDaFirmare(BigDecimal idStruttura) {
        ...
        DecodeMap mappaAmbiente = new DecodeMap();
        mappaAmbiente.populatedMap(tmpTableBeanAmbiente, "id_ambiente", "nm_ambiente");
        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_ambiente().setDecodeMap(mappaAmbiente);
        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_ambiente().setValue(idAmbiente.toString());
        ...
        getForm().getFiltriElenchiIndiciAipFascDaFirmare().getId_strut().setValue(idStruttura.toString());
    ...
    /**
     * Bottone "+" della "Lista elenchi indici AIP fascicoli da firmare" per spostare un elenco da questa lista a quella
     * degli elenchi selezionati pronti per essere firmati
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectElenchiIndiciAipFascDaFirmareList() throws EMFError {
        ...
        int index = getForm().getElenchiIndiciAipFascDaFirmareList().getTable().getCurrentRowIndex();
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiIndiciAipFascDaFirmareList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiIndiciAipFascDaFirmareList().getTable().getFirstRowPageIndex();
        ...
        getForm().getElenchiIndiciAipFascSelezionatiList().getTable().addSortingRule(
                getForm().getElenchiIndiciAipFascSelezionatiList().getDt_creazione_elenco_ix_aip().getName(),
                SortingRule.ASC);
    ...
    /**
     * Bottone "-" della "Lista elenchi indici AIP fascicoli selezionati" per spostare un elenco da questa lista a
     * quella degli elenchi indici AIP fascicoli da firmare
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectElenchiIndiciAipFascSelezionatiList() throws EMFError {
        ...
        int index = getForm().getElenchiIndiciAipFascSelezionatiList().getTable().getCurrentRowIndex();
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiIndiciAipFascSelezionatiList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiIndiciAipFascSelezionatiList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
        ...
        int paginaCorrenteVF = getForm().getElenchiIndiciAipFascDaFirmareList().getTable().getCurrentPageIndex();
        int inizioVF = getForm().getElenchiIndiciAipFascDaFirmareList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
    ...
    public void firmaElenchiIndiciAipFascHsmJs() throws EMFError {
        ...
        char[] passwd = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getPasswd().parse() != null
                ? getForm().getFiltriElenchiIndiciAipFascDaFirmare().getPasswd().parse().toCharArray() : null;
        char[] otp = getForm().getFiltriElenchiIndiciAipFascDaFirmare().getOtp().parse() != null
                ? getForm().getFiltriElenchiIndiciAipFascDaFirmare().getOtp().parse().toCharArray() : null;
    ...
    public JSONObject triggerFiltriElenchiVersFascicoliId_strutOnTrigger() throws EMFError {
        ...
        if (getForm().getFiltriElenchiVersFascicoli().getId_strut().parse() != null) {
            ...
            getForm().getFiltriElenchiVersFascicoli().getId_tipo_fascicolo().setDecodeMap(new DecodeMap());
        }
        return getForm().getFiltriElenchiVersFascicoli().asJSON();
    }
    ...
    public void confermaRimozioneFascButton() throws Throwable {
        ...
        BigDecimal idElencoVersFasc = getForm().getElenchiVersFascicoliDetail().getId_elenco_vers_fasc().parse();
        Set<BigDecimal> idFascToRemove = (Set<BigDecimal>) getSession().getAttribute("idFascToRemove");
        try {
            evfEjb.deleteFascFromElencoVersFascicoli(idElencoVersFasc.longValue(), idFascToRemove,
                    getIdUtenteCorrente());

            if (evfEjb.existIdElenco(getForm().getElenchiVersFascicoliDetail().getId_elenco_vers_fasc().parse(),
                    getIdStrutCorrente())) {
                getMessageBox().addInfo("Fascicoli eliminati con successo!");
                ricercaFasc();
            } else {
                getMessageBox().addInfo("Fascicoli eliminati con successo! "
                        + "In quanto rimasto privo di fascicoli, l'elenco di versamento fascicoli " + idElencoVersFasc
                        + " è stato eliminato!");
    ...
    /**
     * Metodo richiamato dal link per accedere alla pagina di dettaglio Criterio di Raggruppamento Fascicoli
     *
     * @throws EMFError
     *             errore generico
     */
    public void loadDettaglioCritRaggrFasc() throws EMFError {
        ...
        BigDecimal idCriterioRaggrFasc = ((BaseTableInterface) getForm().getElenchiVersFascicoliList().getTable())
                .getRow(numberRiga.intValue()).getBigDecimal("id_criterio_raggr_fasc");
        DecCriterioRaggrFascRowBean critRaggrFascRow = fascicoliEjb.getDecCriterioRaggrFascRowBean(idCriterioRaggrFasc,
                idStrut);
        // Setto la tabella dei criteri di raggruppamento fascicoli aggiungendo solo quella recuperata
        ...
        redirectToPage(Application.Actions.CRITERI_RAGGR_FASCICOLI, form,
                form.getCriterioRaggrFascicoliList().getName(), criterioRaggrFascTable, getNavigationEvent());
    }
    ...
    public void loadCritRaggrFasc() throws EMFError {
        ...
        // Recupero il criterio di raggruppamento fascicoli
        BigDecimal idCriterioRaggrFasc = getForm().getElenchiVersFascicoliDetail().getId_criterio_raggr_fasc().parse();
        DecCriterioRaggrFascRowBean critRaggrFascRow = fascicoliEjb.getDecCriterioRaggrFascRowBean(idCriterioRaggrFasc,
                idStrut);
        // Setto la tabella dei criteri di raggruppamento fascicoli aggiungendo solo quella recuperata
        DecCriterioRaggrFascTableBean criterioRaggrFascTable = new DecCriterioRaggrFascTableBean();

...
public class MonitoraggioTpiAction extends MonitoraggioTpiAbstractAction {
    ...
    public void loadDettaglio() throws EMFError {
        if (getRequest().getParameter("table").equals(getForm().getDateVersamentoList().getName())) {

            getForm().getDateVersamentoDetail().getFl_ark().setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
            getForm().getDateVersamentoDetail().getFl_ark_secondario()
                    .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
            getForm().getDateVersamentoDetail().getFl_file_no_ark().setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
            getForm().getDateVersamentoDetail().getFl_file_no_ark_secondario()
                    .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());

            BigDecimal idDtVers = ((VrsDtVersRowBean) getForm().getDateVersamentoList().getTable().getCurrentRow())
                    .getIdDtVers();
        ...
        } else if (getRequest().getParameter("table").equals(getForm().getDateVersamentoDetailPathList().getName())) {
            VrsPathDtVersRowBean currentPath = ((VrsPathDtVersRowBean) getForm().getDateVersamentoDetailPathList()
                    .getTable().getCurrentRow());

            getForm().getPathVersamentoDetail().getFl_path_ark().setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
            ...
            getForm().getFileNoArkPrimarioList().setTable(monitoraggioTpiHelper
                    .getFilesNoArkPath(currentPath.getIdPathDtVers(), JobConstants.ArkPath.PRIMARIO.name()));
            getForm().getFileNoArkPrimarioList().getTable().first();
            getForm().getFileNoArkPrimarioList().getTable().setPageSize(10);

            getForm().getFileNoArkSecondarioList().setTable(monitoraggioTpiHelper
                    .getFilesNoArkPath(currentPath.getIdPathDtVers(), JobConstants.ArkPath.SECONDARIO.name()));
            getForm().getFileNoArkSecondarioList().getTable().first();
            getForm().getFileNoArkSecondarioList().getTable().setPageSize(10);
        } else if (getRequest().getParameter("table").equals(getForm().getDateSchedJobTpiList().getName())) {
            BigDecimal currentSched = ((TpiVLisDtSchedRowBean) getForm().getDateSchedJobTpiList().getTable()
                    .getCurrentRow()).getIdDtSched();
            getForm().getDataSchedDetail().copyFromBean(monitoraggioTpiHelper.getDataSchedulazioneTpi(currentSched));

            getForm().getJobFileVersatiPrimarioList().setTable(monitoraggioTpiHelper.getJobList(currentSched,
                    JobConstants.DB_FALSE, JobConstants.ArkPath.PRIMARIO.name()));
            getForm().getJobFileVersatiPrimarioList().getTable().first();
            getForm().getJobFileVersatiPrimarioList().getTable().setPageSize(10);

            getForm().getJobFileMigratiPrimarioList().setTable(monitoraggioTpiHelper.getJobList(currentSched,
                    JobConstants.DB_TRUE, JobConstants.ArkPath.PRIMARIO.name()));
            getForm().getJobFileMigratiPrimarioList().getTable().first();
            getForm().getJobFileMigratiPrimarioList().getTable().setPageSize(10);

            getForm().getJobFileVersatiSecondarioList().setTable(monitoraggioTpiHelper.getJobList(currentSched,
                    JobConstants.DB_FALSE, JobConstants.ArkPath.SECONDARIO.name()));
            getForm().getJobFileVersatiSecondarioList().getTable().first();
            getForm().getJobFileVersatiSecondarioList().getTable().setPageSize(10);

            getForm().getJobFileMigratiSecondarioList().setTable(monitoraggioTpiHelper.getJobList(currentSched,
                    JobConstants.DB_TRUE, JobConstants.ArkPath.SECONDARIO.name()));
        ...
        try {
            ...
            if (lastMenuEntry.contains("RicercaDateVersamento")) {
                ...
                case Application.Publisher.MONITORAGGIO_DATE_VERS_RICERCA: {
                    int paginaCorrenteDtVers = getForm().getDateVersamentoList().getTable().getCurrentPageIndex();
                    int inizioDtVers = getForm().getDateVersamentoList().getTable().getFirstRowPageIndex();
                    ...
                    getForm().getDateVersamentoList()
                            .setTable(monitoraggioTpiHelper.getDateVersamentoArk(dataDa, dataA,
                                    ...
                                    getForm().getFiltriRicercaDateVers().getTi_stato_dt_vers().parse()));
                    // Workaround in modo che la lista punti al primo record, non all'ultimo
                ...
                case Application.Publisher.MONITORAGGIO_DATE_VERS_DETAIL: {
                    int paginaCorrentePath = getForm().getDateVersamentoDetailPathList().getTable()
                            .getCurrentPageIndex();
                    int inizioPath = getForm().getDateVersamentoDetailPathList().getTable().getFirstRowPageIndex();
                    int pageSize = getForm().getDateVersamentoDetailPathList().getTable().getPageSize();
                    getForm().getDateVersamentoDetailPathList().setTable(monitoraggioTpiHelper
                            .getPathsDateVersamentoArk(getForm().getDateVersamentoDetail().getId_dt_vers().parse()));
                    // Workaround in modo che la lista punti al primo record, non all'ultimo
                    getForm().getDateVersamentoDetailPathList().getTable().first();
                    getForm().getDateVersamentoDetailPathList().getTable().setPageSize(pageSize);
                    this.lazyLoadGoPage(getForm().getDateVersamentoDetailPathList(), paginaCorrentePath);
                    // Ritorno alla pagina
                    getForm().getDateVersamentoDetailPathList().getTable().setCurrentRowIndex(inizioPath);
                    break;
                }
                }
            }
    ...
    public void loadRicercaDateVers() throws EMFError {
        ...
        getForm().getFiltriRicercaDateVers().getTi_stato_dt_vers().setDecodeMap(ComboGetter
                .getMappaSortedGenericEnum("statoDtVers", JobConstants.ArkStatusEnum.getComboRicercaDtVers()));
        getForm().getFiltriRicercaDateVers().getFl_migraz().setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
    ...
    public void ricercaDateVers() throws EMFError {
        ...
        if (filtri.postAndValidate(getRequest(), getMessageBox())) {
            ...
            if (dataDa == null && dataA == null) {
                logger.debug("Date nulle - almeno una deve essere valorizzata");
                getMessageBox().addError("Selezionare almeno una data di versamento");
            } else if (dataDa != null && dataA != null && dataA.before(dataDa)) {
                logger.debug(filtri.getDt_vers_da().getHtmlDescription() + " superiore a "
                        + filtri.getDt_vers_a().getHtmlDescription());
                getMessageBox().addError(filtri.getDt_vers_da().getHtmlDescription() + " superiore a "
                        + filtri.getDt_vers_a().getHtmlDescription());
            }

            if (!getMessageBox().hasError()) {
                // La validazione non ha riportato errori. Carico la tabella con i filtri impostati
                getForm().getDateVersamentoList().setTable(monitoraggioTpiHelper.getDateVersamentoArk(dataDa, dataA,
                        filtri.getFl_migraz().parse(), filtri.getTi_stato_dt_vers().parse()));
                // Workaround in modo che la lista punti al primo record, non all'ultimo
                getForm().getDateVersamentoList().getTable().first();
    ...
    public void callRiArk() throws EMFError {
        ...
        if (getForm().getDateVersamentoDetail().getTi_stato_dt_vers().parse()
            ...
            String dateString = requestDateFormat.format(getForm().getDateVersamentoDetail().getDt_vers().parse());
            logger.info("Chiamo il servizio di ri archiviazione per la data " + dateString);
            String tipoRiArk;
            if (getForm().getDateVersamentoDetail().getFl_file_no_ark().parse().equals(JobConstants.DB_TRUE)
            ...
            } else if (getForm().getDateVersamentoDetail().getFl_file_no_ark().parse().equals(JobConstants.DB_TRUE)) {
            ...
            try {
                ...
                RichiestaTpiInput inputParams = new RichiestaTpiInput(
                        ...
                        new BasicNameValuePair(RichiestaTpi.FL_CARTELLA_MIGRAZ,
                                String.valueOf(getForm().getDateVersamentoDetail().getFl_migraz().parse()
                                        .equals(JobConstants.DB_TRUE))),
                ...
                if (esitoConn.isErroreConnessione()) {
                    logger.error("Servizio RiArkCartella --- " + esitoConn.getDescrErrConnessione());
                ...
                if (!getMessageBox().hasError()) {
                    if (codiceEsito.equals(EsitoConnessione.Esito.KO.name())) {
                        logger.error("Servizio RiArkCartella --- " + codiceErrore + " - " + messaggioErrore);
                        // se il risultato è stato inaspettatamente NEGATIVO registro la sessione con stato di errore e
                        // chiudo il job
                        getMessageBox().addError("Il servizio RegistraCartellaRiArk ha restituito l'errore: "
                                + codiceErrore + " - " + messaggioErrore);
                    } else {
                        // risultato OK! Non verifico nemmeno la risposta in quanto non contiene nessun dato utile
                        logger.debug("Servizio RiArkCartella OK");
                        BigDecimal idDtVers = getForm().getDateVersamentoDetail().getId_dt_vers().parse();
                        ...
                        getMessageBox().addInfo("Richiesta di riarchiviazione eseguita con successo");
    ...
    public void loadRicercaDateSchedJobTpi() throws EMFError {
        ...
        getForm().getFiltriRicercaDateSchedJobTpi().getTi_stato_dt_sched().setDecodeMap(
                ComboGetter.getMappaSortedGenericEnum("statoDtSched", JobConstants.StatoSchedJob.values()));
    ...
    public void ricercaDateSchedJobTpi() throws EMFError {
        MonitoraggioTpiForm.FiltriRicercaDateSchedJobTpi filtri = getForm().getFiltriRicercaDateSchedJobTpi();
        if (filtri.postAndValidate(getRequest(), getMessageBox())) {
            ...
            if (dataDa == null && dataA == null) {
                ...
                logger.debug(filtri.getDt_sched_da().getHtmlDescription() + " superiore a "
                        + filtri.getDt_sched_a().getHtmlDescription());
                getMessageBox().addError(filtri.getDt_sched_da().getHtmlDescription() + " superiore a "
                        + filtri.getDt_sched_a().getHtmlDescription());
            }

            if (!getMessageBox().hasError()) {
                // La validazione non ha riportato errori. Carico la tabella con i filtri impostati
                getForm().getDateSchedJobTpiList().setTable(monitoraggioTpiHelper.getDateSchedulazioniTpi(dataDa, dataA,
                        filtri.getFl_anomalia().parse(), filtri.getTi_stato_dt_sched().parse()));
                // Workaround in modo che la lista punti al primo record, non all'ultimo
    ...
    public void showPathElabList(String table) throws EMFError {
        ...
        getForm().getJobSchedDetail().copyFromBean(row);
        getForm().getPathElabJobTpiDetailList().setTable(monitoraggioTpiHelper.getPathElabJobList(row.getIdSchedJob()));

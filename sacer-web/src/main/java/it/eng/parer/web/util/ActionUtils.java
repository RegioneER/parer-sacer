...
public class ActionUtils {
    ...
    public ActionUtils() {
        try {
            ...
            ambienteEjb = (AmbienteEjb) new InitialContext()
                    .lookup("java:app/Parer-ejb/AmbienteEjb");
            tipoUnitaDocEjb = (TipoUnitaDocEjb) new InitialContext()
                    .lookup("java:app/Parer-ejb/TipoUnitaDocEjb");
    ...
    public Fields triggerAmbienteGenerico(Fields campi, long idUtente, Boolean filterValid)
            throws EMFError {
        ...
        if (idAmbiente != null) {
            ...
            OrgEnteTableBean tmpTableBeanEnte = ambienteEjb.getEntiAbilitatiNoTemplate(idUtente,
                    idAmbiente.longValue(), filterValid);
            ...
            if (tmpTableBeanEnte.size() == 1) {
                ...
                checkUniqueEnteInCombo(tmpTableBeanEnte.getRow(0).getIdEnte(), strutCombo, idUtente,
                        filterValid);
            } else {
    ...
    public Fields triggerEnteGenerico(Fields campi, long idUtente, Boolean filterValid)
            throws EMFError {
    ...
    public Fields triggerStrutGenerico(Fields campi, long idUtente) throws EMFError {
        ...
        if (idStrut != null) {
            ...
            DecTipoUnitaDocTableBean tmpTableBeanTUD = tipoUnitaDocEjb
                    .getTipiUnitaDocAbilitati(idUtente, idStrut);
    ...
    // */
    // private void checkUniqueAmbienteInCombo(BigDecimal idAmbiente, ComboBox ente, ComboBox strut,
    // long idUtente)
    // throws EMFError {
    // if (idAmbiente != null) {
    // // Ricavo il TableBean relativo agli enti dipendenti dall'ambiente scelto
    // OrgEnteTableBean tmpTableBeanEnte = comboHelper.getEnteNoTemplateFromAmbiente(idUtente,
     ...
     * @param strut    id struttura
     * @param idUtente id utente
     *
     * @throws EMFError errore generico
     */
    private void checkUniqueEnteInCombo(BigDecimal idEnte, ComboBox strut, long idUtente,
    ...
    /**
     * Inizializza le combo Ambiente/Ente/Struttura in base alle abilitazioni dell'utente,
     * impostando già selezionata la struttura eventualmente passata come parametro
     *
     * @param field       campo {@link Fields}
     * @param idUtente    id utente
     ...
     */
    public void initGenericComboAmbienteEnteStruttura(Fields field, long idUtente,
            BigDecimal idStrut, Boolean filterValid) throws ParerUserError {
        // Azzero i filtri
        ...
        if (idStrut != null) {
            OrgStrutRowBean strut = elenchiVersamentoEjb
            ...
            if (strut != null) {
                ...
                tmpTableBeanEnte = ambienteEjb.getEntiAbilitatiNoTemplate(idUtente, idAmbiente,
                        filterValid);
                mappaEnte.populatedMap(tmpTableBeanEnte, "id_ente", "nm_ente");
                ((ComboBox) field.getComponent("id_ente")).setDecodeMap(mappaEnte);
                ((ComboBox) field.getComponent("id_ente")).setValue("" + idEnte);

                tmpTableBeanStruttura = struttureEjb.getOrgStrutTableBean(idUtente,
                        new BigDecimal(idEnte), filterValid);
    ...
    /**
     * Metodo statico che ritorna la data in formato stringa da impostare come valore agli oggetti
     * di tipo input date Se <code>inputDate</code> è null, ritorna la data di soppressione di
     * default (31/12/2444)
     *
     * @param inputDate data da valutare
     *
    ...
    public static JSONObject getConservUnlimitedTrigger(Fields<Field> fields) throws EMFError {
        ComboBox<String> conservUnlimited = ((ComboBox<String>) fields
                .getComponent("Conserv_unlimited"));
        Input<BigDecimal> niAnniConserv = ((Input<BigDecimal>) fields
                .getComponent("Ni_anni_conserv"));
    ...
    /*
     * Metodo per consentire di effettuare un download completo di tutti i dati di una lista senza
     * limitazioni. Il parametro table viene passato anche se sarebbe stato ricavabile da fields
     * perché questo metodo potrebbe essere invocato anche se non si è popolata una lista a video.
     */
    public static void buildCsvString(it.eng.spagoLite.form.list.List<SingleValueField<?>> fields,
            BaseTableInterface<? extends BaseRowInterface> table, TableDescriptor tableDescriptor,
        ...
        try (FileWriter fw = new FileWriter(tmpFile, true);
            ...
            for (SingleValueField<?> field : fields.getComponentList()) {
                ...
                if (isFirst) {
                    ...
                    out.append(csvSeparator).append(quote).append(field.getDescription())
                            .append(quote);
                }
            }
            out.append(end);
             ...
             * table.getLazyListBean().getMaxResult() : table.fullSize(); int count =
             * table.fullSize(); double quantiPassi = Math.ceil(count / max);
             *
             ...
             * writeTable(table, columnMap, columns, out); } } else {
             */
    ...
    private static void writeTable(BaseTableInterface<? extends BaseRowInterface> table,
        ...
        for (BaseRowInterface row : table) {
            ...
            for (String column : columns) {
                ...
                case Types.DECIMAL:
                    out.append(row.getBigDecimal(column) != null
                            ? row.getBigDecimal(column).toPlainString()
                            : "");
                    break;
                case Types.VARCHAR:

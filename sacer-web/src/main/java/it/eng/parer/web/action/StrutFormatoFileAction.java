...
public class StrutFormatoFileAction extends StrutFormatoFileAbstractAction {
        ...
        if (lista != null) {
            if (lista.equals(getForm().getFormatoFileDocList().getName())) {
                ...
                if (getNavigationEvent().equals(NE_DETTAGLIO_INSERT)) {
                    ...
                    for (DecFormatoFileDocRowBean row : (DecFormatoFileDocTableBean) getForm().getFormatoFileDocList()
                            .getTable()) {
                        if (row != null && row.getNmFormatoFileDoc() != null
                                && !row.getNmFormatoFileDoc().contains(".")) {
                            // Se il nome non contiene un '.' allora è un formato standard e non concatenato
                    ...
                    getForm().getFormatoFileStandardToDocList().getTable().setPageSize(WebConstants.FORMATI_PAGE_SIZE);
                    getForm().getFormatoFileStandardToDocList().getTable().addSortingRule("cd_estensione_file",
                            SortingRule.ASC);
                    ...
                    /*
                     * MODIFICA: questa lista non deve più comparire al momento della load dettaglio, ma deve solo
                     * servire per la singola operazione di inserimento come promemoria dei formati da inserire
                     */
                    ...
                    getForm().getSelectFormatoFileStandardList().getCd_estensione_file_busta().setHidden(true);
                    getForm().getSelectFormatoFileStandardList().getCd_estensione_file().setHidden(false);
                    getForm().getSelectFormatoFileStandardList().getTable().setPageSize(WebConstants.FORMATI_PAGE_SIZE);
                    getForm().getSelectFormatoFileStandardList().getTable().addSortingRule("cd_estensione_file",
                            SortingRule.ASC);
                    ...
                    // Se sono in modifica devo mostrare la lista formati concatenabili, per permettere la
                    // concatenazione di formati
                    // FIXME : Bisogna correggere il db per poter richiedere solo i formati "concatenabili"
                    DecFormatoFileDocRowBean formato = (DecFormatoFileDocRowBean) getForm().getFormatoFileDocList()
                    ...
                    // E' già un formato concatenato, devo popolare la lista dei formati già concatenati
                    ...
                    // -----------
                } else if (getNavigationEvent().equals(NE_DETTAGLIO_VIEW) || getNavigationEvent().equals(NE_NEXT)
                        || getNavigationEvent().equals(NE_PREV) || getNavigationEvent().equals(NE_DETTAGLIO_CANCEL)) {
    ...
    private void salvaFormatoFileDoc() throws EMFError {
        ...
        if (getForm().getFormatoFileDocList().getStatus().equals(Status.insert)) {
            ...
            if (getForm().getSelectFormatoFileStandardList().getTable().isEmpty()) {
                getMessageBox()
                        .addError("Non è stato selezionato nessun formato file standard da aggiungere alla lista</br>");
                ...
                for (DecFormatoFileStandardRowBean row : (DecFormatoFileStandardTableBean) getForm()
                    ...
                    formatiListPresentiOnline.add(row.getString("cd_estensione_file").toUpperCase());
                    ...
                    formato.setNmFormatoFileDoc(row.getString("nm_formato_file_standard").toUpperCase());
                    ...
                    try {
                        ...
                        // Caso di formato già esistente eventualmente da attivare su richiesta dell'utente
                        getRequest().setAttribute("warningAttivazioneFormatoEsistente", ex.getDescription());
            ...
            if (getForm().getFormatoFileDoc().validate(getMessageBox())) {
                ...
                if (duplica != null && duplica) {
                    ...
                    formatiStandard.add(formatoFileStandardEjb.getDecFormatoFileStandardInList(standard, Status.insert,
                            getForm().getIdList().getId_strut().parse()).getRow(0));
                } else {
                    formato.setNmFormatoFileDoc(
                            getForm().getFormatoFileDoc().getNm_formato_file_doc().parse().toUpperCase());
                    formato.setDsFormatoFileDoc(getForm().getFormatoFileDoc().getDs_formato_file_doc().parse());
        ...
        if (getMessageBox().hasError()
                || getRequest().getAttribute("customAttivazioneFormatoEsistenteMessageBox") != null) {
    ...
    private void goBackToDettaglioStruttura() {
        ...
        for (ExecutionHistory exec : esecuzioni) {
            System.out.println("Publisher : " + exec.getPublisherName() + " \nBackParameter: " + exec.getBackParameter()
                    + " isAction :" + exec.isAction());
        }
    ...
    /**
     * Metodo che cancella l'entità DecFormatoFileDoc corrispondente al record della lista selezionato
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void deleteFormatoFileDocList() throws EMFError {
        ...
        if (getMessageBox().isEmpty()) {
            ...
            if (param.getNomePagina().equalsIgnoreCase(Application.Publisher.FORMATO_FILE_DOC_DETAIL)) {
                ...
                param.setNomeAzione(SpagoliteLogUtil.getDetailActionNameDelete(form, form.getFormatoFileDocList()));
            }
            try {
            ...
            for (String comp : indiciAssolutiFormatiAmmissibili) {
                if (StringUtils.isNotBlank(comp) && StringUtils.isNumeric(comp)) {
                    DecFormatoFileStandardRowBean currentRow = (DecFormatoFileStandardRowBean) getForm()
                            .getFormatoFileStandardToDocList().getTable().getRow((Integer.parseInt(comp) - indice));

                    if (getForm().getFormatoFileDocList().getStatus().equals(Status.insert)) {
                        getForm().getFormatoFileStandardToDocList().getTable().remove(Integer.parseInt(comp) - indice);
                    ...
                    if (!getForm().getFormatoFileDocList().getStatus().equals(Status.insert)) {
                        getForm().getSelectFormatoFileStandardList().getTable().getCurrentRow()
                                .setString("cd_estensione_file_busta", currentRow.getString("cd_estensione_file"));
                        getForm().getSelectFormatoFileStandardList().getCd_estensione_file().setHidden(true);
                    } else {
            ...
            for (String comp : indiciAssolutiFormatiAmmessi) {
                if (StringUtils.isNotBlank(comp) && StringUtils.isNumeric(comp)) {
                    DecFormatoFileStandardRowBean row = (DecFormatoFileStandardRowBean) getForm()
                            .getSelectFormatoFileStandardList().getTable().getRow((Integer.parseInt(comp) - indice));
                    getForm().getSelectFormatoFileStandardList().getTable().remove(Integer.parseInt(comp) - indice);
                    indice++;
    ...
    public void select_all() throws Throwable {
        if (getLastPublisher().equals(Application.Publisher.FORMATO_FILE_DOC_DETAIL)) {
            ...
            getForm().getSelectFormatoFileAmmessoList()
                    .setTable(formatoFileDocEjb.getDecFormatoFileAmmessoNotInList(new HashSet<>(), idStrut));
    ...
    /**
     * Gestisce il bottone di select della gestione di inserimento formati file doc. Inserisce il formato file standard
     * selezionato tra i formati file doc della struttura
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectFormatoFileStandardToDocList() throws EMFError {
        ...
        if (getForm().getFormatoFileDocList().getStatus().equals(Status.insert)) {
            ...
            StringBuilder formato = new StringBuilder(getForm().getFormatoFileDoc().getNm_formato_file_doc().parse());
    ...
    /**
     * Gestisce il bottone di select della gestione di inserimento formati file doc. Rimuove il formato file standard
     * selezionato dai formati file doc della struttura
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectSelectFormatoFileStandardList() throws EMFError {
        ...
        DecFormatoFileStandardRowBean row = (DecFormatoFileStandardRowBean) getForm().getSelectFormatoFileStandardList()
                .getTable().getCurrentRow();
        getForm().getFormatoFileDoc().post(getRequest());
    ...
    public void reloadFormatoFileStandardToDocList() throws EMFError {
        ...
        int paginaCorrente = getForm().getFormatoFileStandardToDocList().getTable().getCurrentPageIndex();
        getForm().getFormatoFileStandardToDocList().getTable().sort();
    ...
    public void reloadSelectFormatoFileStandardList() throws EMFError {
        ...
        int paginaCorrente = getForm().getSelectFormatoFileStandardList().getTable().getCurrentPageIndex();
        getForm().getSelectFormatoFileStandardList().getTable().sort();
    ...
    public void updateFormatoFileDocList() throws EMFError {
        ...
        DecFormatoFileDocRowBean formatoFileDocRowBean = (DecFormatoFileDocRowBean) getForm().getFormatoFileDocList()
                .getTable().getCurrentRow();
        getSession().setAttribute("inUse", formatoFileDocEjb.isDecFormatoFileDocInUse(formatoFileDocRowBean));
    }
    ...
    public void duplicaFormato() throws EMFError {
        ...
        DecFormatoFileDocRowBean formato = (DecFormatoFileDocRowBean) getForm().getFormatoFileDocList().getTable()
                .getCurrentRow();
        getForm().getFormatoFileDoc().copyFromBean(formato);
        ...
        getForm().getSelectFormatoFileStandardList().setTable(new DecFormatoFileStandardTableBean());

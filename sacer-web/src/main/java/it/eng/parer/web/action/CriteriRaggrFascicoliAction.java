...
public class CriteriRaggrFascicoliAction extends CriteriRaggrFascicoliAbstractAction {
    ...
    private static Logger log = LoggerFactory
	    .getLogger(CriteriRaggrFascicoliAction.class.getName());
    ...
    /**
     * Inizializza i FILTRI DI RICERCA CRITERI di raggruppamento fascicoli in base alla struttura
	...
	checkUniqueStrutInCombo(idStruttura,
	...
	// (per IE)
	getForm().getFiltriCriteriRaggrFascicoli().getRicercaCriteriRaggrFascicoliButton()
		.setEditMode();
	getForm().getFiltriCriteriRaggrFascicoli().getRicercaCriteriRaggrFascicoliButton()
    ...
    private boolean isCriterioRaggrFascicoliStandard(
	    CriteriRaggrFascicoliForm.CreaCriterioRaggrFascicoli filtri,
	...
	return fascicoliEjb.isCriterioRaggrFascStandard(idStrut, aaFascicolo, aaFascicoloDa,
		aaFascicoloA, niTempoScadChius, tiTempoScadChius, tipiFascicolo, voceTitolList,
		niMaxFasc);
    }

    /**
     * Inizializza le combo ambiente/ente/struttura del DETTAGLIO DI UN CRITERIO di raggruppamento
     * fascicoli, ricavando i valori da una struttura impostata
     *
    ...
    /**
     * Metodo che popola le liste della form di dettaglio criterio con i dati forniti e i dati
     * specifici se presenti
     *
     * @param critRB BaseRow del criterio di raggruppamento fascicoli
     *
     * @throws EMFError errore generico
     */
	...
	if ("1".equals(critRB.getString("fl_filtro_voce_titol"))) {
		...
		if (!isFirst) {
			...
			indMap.put("titolariPS",
				getForm().getTitolariList().getTable().getPageSize());
		    }
		}
		...
		getForm().getTitolariList().getTable().addSortingRule("dt_istituz",
			SortingRule.DESC);
		...
		if (!indMap.isEmpty()) {
			...
			getForm().getTitolariList().getTable()
				.setPageSize(indMap.get("titolariPS"));
		    }
		}
	    }
	}
	...
	if (idStrut != null) {
	    ...
	    DecTipoFascicoloTableBean tmpTableBeanTipoFascicolo = tipoFascicoloEjb
		    .getDecTipoFascicoloTableBean(idStrut, false);
	    mappaTipoFascicolo = new DecodeMap();
	    mappaTipoFascicolo.populatedMap(tmpTableBeanTipoFascicolo, "id_tipo_fascicolo",
		    "nm_tipo_fascicolo");

	    // Setto i valori della combo TITOLARIO ricavati dalla tabella DEC_TITOL
	    DecTitolTableBean tmpTableBeanTitol = titolariEjb.getDecTitolTableBean(idStrut, false); // TODO:
												    // verificare
     ...
     *
     * @param idCriterioRaggrFasc id criterio raggruppamento fascicoli
     *
     * @throws EMFError errore generico
     */
    private void initTableBeanTitolListByIdCritRaggrFasc(BigDecimal idCriterioRaggrFasc) {
	if (idCriterioRaggrFasc != null) {
	    // Setto i valori della LISTA TITOLARI ricavati dalla tabella
	    // DEC_SEL_CRITERIO_RAGGR_FASC
    ...
    /**
     * Metodo di inizializzazione combo di dettaglio, utilizzate sia nel dettaglio criteri sia nella
     * scelta indice classificazione
     *
     * @param idStrut id struttura
     *
     * @throws EMFError
	...
	getForm().getCreaCriterioRaggrFascicoli().getTi_scad_chius()
		.setDecodeMap(ComboGetter.getMappaOrdinalGenericEnum("ti_scad_chius",
	...
	getForm().getCreaCriterioRaggrFascicoli().getTi_conservazione()
		.setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_conservazione",
			VolumeEnums.TipoConservazioneFasc.values()));
    ...
    /**
     * Metodo di inizializzazione liste di dettaglio, utilizzate nel dettaglio criteri di
	...
	if (getForm().getCriterioRaggrFascicoliList().getStatus().equals(Status.insert)) {
	    ...
	    if (getLastPublisher().equals(Application.Publisher.CRITERIO_RAGGR_FASC_DETAIL)
		    && getForm().getCriterioRaggrFascicoliList().getStatus()
			    .equals(Status.update)) {
		getForm().getCreaCriterioRaggrFascicoli().getDuplicaCritButton().setHidden(false);
		getForm().getCreaCriterioRaggrFascicoli().getLogEventiCriteriRaggruppamento()
			.setHidden(false);
		getForm().getCreaCriterioRaggrFascicoli().getInserisciVoceClassificazione()
			.setHidden(true);
		loadDettaglio();
	...
	if (getRequest().getParameter("table")
		.equals(getForm().getCriterioRaggrFascicoliList().getName())
	...
	if (publisherName.equals(Application.Publisher.LISTA_CRITERI_RAGGR_FASC)) {
	    getSession().removeAttribute("insertActive");
	    int paginaCorrenteDocNonVers = getForm().getCriterioRaggrFascicoliList().getTable()
		    .getCurrentPageIndex();
	    int inizioDocNonVers = getForm().getCriterioRaggrFascicoliList().getTable()
		...
		this.lazyLoadGoPage(getForm().getCriterioRaggrFascicoliList(),
			paginaCorrenteDocNonVers);
		// Ritorno alla pagina
		getForm().getCriterioRaggrFascicoliList().getTable()
	...
	settaNumeroMassimoFascicoli(
		getForm().getCreaCriterioRaggrFascicoli().getId_strut().parse());
	// Setto il numero di giorni di scadenza chiusura con il valore definito sulla struttura
	// corrente
    ...
    /**
     * Inizializza le combo ambiente/ente/struttura del DETTAGLIO DI UN CRITERIO di raggruppamento
     * fascicoli, ricavando i valori dai filtri di ricerca criterio. Se impostato, restituisce il
     * valore dell'id del filtro relativo alla struttura
     *
	...
	return StringUtils
		.isNotBlank(getForm().getFiltriCriteriRaggrFascicoli().getId_strut().getValue())
			? new BigDecimal(
				getForm().getFiltriCriteriRaggrFascicoli().getId_strut().getValue())
			: null;
    }
	...
	getForm().getCreaCriterioRaggrFascicoli().getDt_istituz()
		.setValue(df.format(Calendar.getInstance().getTime()));
	getForm().getCreaCriterioRaggrFascicoli().getDt_soppres()
    ...
    /**
     * Metodo invocato sul bottone di eliminazione nella lista criteri di raggruppamento fascicoli o
     * nel dettaglio criterio sulla listNavBar
     *
     * @throws EMFError errore generico
     */
    @Override
	...
	try {
	    ...
	    if (param.getNomePagina()
		    .equalsIgnoreCase(Application.Publisher.LISTA_CRITERI_RAGGR_FASC)) {
		// Gestione della cancellazione da lista criteri raggr fasc
		CriteriRaggrFascicoliForm form = (CriteriRaggrFascicoliForm) SpagoliteLogUtil
			.getForm(this);
		...
		getMessageBox().addMessage(new Message(MessageLevel.INF,
			"Criterio di raggruppamento fascicoli eliminato con successo"));
	...
	try {
	    // Carica il rowBean del criterio tramite BaseRow
	    BigDecimal id = getForm().getCreaCriterioRaggrFascicoli().getId_criterio_raggr_fasc()
		    .parse();
	    BaseRow critRB = fascicoliEjb.getDettaglioCriterioRaggrFascicolo(id);
	    // TODO
	    // DA FINIRE !!!!!!!!!!!!
	...
	BigDecimal idCriterioRaggrFasc = (getForm().getCriterioRaggrFascicoliList().getTable())
		.getCurrentRow().getBigDecimal("id_criterio_raggr_fasc");

	/*
	 * Se il criterio è già legato a qualche elenco, posso modificare solo data inizio e fine
	 * validità, n° max fascicoli, tipo scadenza chiusura, scadenza chiusura, note
	 */
	...
	// TODO: verificare, Azzero tutto sugli IdFields che mi servono se provengo da
    ...
    public void ricercaCriteriFasc(
	...
	if (status == CRITERIO_EDIT) {
		...
		nome = ((BaseRow) getForm().getCriterioRaggrFascicoliList().getTable()
			.getCurrentRow()).getString("nm_criterio_raggr");
	    }
	    standardOld = getForm().getCreaCriterioRaggrFascicoli().getFl_criterio_raggr_standard()
		    .parse();
	}

	List<BigDecimal> voceTitolList = new ArrayList<>();
	for (BaseRowInterface row : (BaseTableInterface<?>) getForm().getTitolariList()
		.getTable()) {
	...
	if (filtri.validate(getMessageBox())) {
	    ...
	    validator.validaTipoScadenza(filtri.getTi_scad_chius().parse(),
		...
		getMessageBox().addMessage(new Message(MessageLevel.ERR,
			"Nome criterio di raggruppamento fascicoli obbligatorio <br>"));
		...
		// nome, escluso esso
		// stesso naturalmente
		if (!filtri.getNm_criterio_raggr().parse().equals(nome)
			&& fascicoliEjb.existNomeCriterio(filtri.getNm_criterio_raggr().parse(),
				filtri.getId_strut().parse())) {
		    getMessageBox().addMessage(new Message(MessageLevel.ERR,
			    "Nome criterio di raggruppamento fascicoli già esistente per la struttura utilizzata <br>"));
		}
	    ...
	    Object[] anniFascicoliValidati = validator.validaAnniFascicoli(
		    filtri.getAa_fascicolo().parse(), filtri.getAa_fascicolo_da().parse(),
		    filtri.getAa_fascicolo_a().parse());

	    // Verifica se presente almeno un tipo fascicolo o la voce di classificazione
	    if (filtri.getNm_tipo_fascicolo().parse().isEmpty()
		    && getForm().getTitolariList().getTable().isEmpty())
		getMessageBox().addMessage(new Message(MessageLevel.ERR,
			"E’ obbligatorio indicare il filtro sul tipo fascicolo o sull’indice di classificazione <br>"));

	    if (!getMessageBox().hasError()) {
		eseguiPrimoStepSalvataggioCriterioRaggrFascicoli(filtri, anniFascicoliValidati,
			nome, annoDa, annoA, status, standardOld, voceTitolList);
	    }
	}
	forwardToPublisher(Application.Publisher.CRITERIO_RAGGR_FASC_DETAIL);
    }
    ...
    public Fields triggerAmbienteGenerico(Fields campi, ActionEnums.SezioneCriteriRaggrFasc sezione)
	    throws EMFError {
    ...
    public Fields triggerEnteGenerico(Fields campi, ActionEnums.SezioneCriteriRaggrFasc sezione)
	    throws EMFError {
	campi.post(getRequest());
	...
	if (idEnte != null) {
		...
		checkUniqueStrutInCombo(tmpTableBeanStrut.getRow(0).getIdStrut(), sezione);
		    ...
		    .equals(ActionEnums.SezioneCriteriRaggrFasc.FILTRI_CRITERI_RAGGR_FASC)) {
		tipoFascicoloCombo.setDecodeMap(new DecodeMap());
		// TODO altre combo
	    }
	} else {
	...
	if (idEnte != null) {
		...
		getForm().getFiltriCriteriRaggrFascicoli().getId_strut().setDecodeMap(mappaStrut);
		    ...
		    .equals(ActionEnums.SezioneCriteriRaggrFasc.CRITERIO_RAGGR_FASC_DETAIL)) {
		getForm().getCreaCriterioRaggrFascicoli().getId_strut().setDecodeMap(mappaStrut);
	    }
     ...
     * presente e settare di conseguenza le combo Tipo Fascicolo e numero massimo componenti
     *
     * @param idStrut id struttura
     * @param sezione enumerativo
     *
	...
	if (idStrut != null) {
	    ...
	    DecTipoFascicoloTableBean tmpTableBeanTipoFascicolo = tipoFascicoloEjb
		    .getDecTipoFascicoloTableBean(idStrut, false);
	    DecodeMap mapTipoFascicolo = DecodeMap.Factory.newInstance(tmpTableBeanTipoFascicolo,
		    "id_tipo_fascicolo", "nm_tipo_fascicolo");

	    // Ricavo tutti i Titolari per la struttura passata in input
	    DecTitolTableBean tmpTableBeanTitol = titolariEjb.getDecTitolTableBean(idStrut, false); // TODO:
												    // verificare
	    DecodeMap mapTitol = DecodeMap.Factory.newInstance(tmpTableBeanTitol, "id_titol",
		    "nm_titol");

	    if (sezione.equals(ActionEnums.SezioneCriteriRaggrFasc.FILTRI_CRITERI_RAGGR_FASC)) {
		getForm().getFiltriCriteriRaggrFascicoli().getId_tipo_fascicolo()
			.setDecodeMap(mapTipoFascicolo);
	    } else if (sezione
		    .equals(ActionEnums.SezioneCriteriRaggrFasc.CRITERIO_RAGGR_FASC_DETAIL)) {
		getForm().getCreaCriterioRaggrFascicoli().getNm_tipo_fascicolo()
			.setDecodeMap(mapTipoFascicolo);
	...
	if (getForm().getFiltriCriteriRaggrFascicoli().getId_strut().parse() != null) {
	    checkUniqueStrutInCombo(
		    getForm().getFiltriCriteriRaggrFascicoli().getId_strut().parse(),
		    ActionEnums.SezioneCriteriRaggrFasc.FILTRI_CRITERI_RAGGR_FASC);
	} else {
	    getForm().getFiltriCriteriRaggrFascicoli().getId_tipo_fascicolo()
	...
	getForm().getCreaCriterioRaggrFascicoli().getNi_tempo_scad_chius()
		.setValue("" + numGgScadChius);
    }
	...
	loadDuplicaCrit((getForm().getCriterioRaggrFascicoliList().getTable()).getCurrentRow()
	...
	BaseRow critRB = fascicoliEjb.getDecCriterioRaggrFascRowBean(
		getForm().getCreaCriterioRaggrFascicoli().getId_strut().parse(), nome);
	...
	form.getOggettoDetail().getNm_tipo_oggetto()
		.setValue(SacerLogConstants.TIPO_OGGETTO_CRITERIO_RAGGR_FASC);
	BaseRow riga = (BaseRow) getForm().getCriterioRaggrFascicoliList().getTable()
		.getCurrentRow();
	form.getOggettoDetail().getIdOggetto()
		.setValue(riga.getBigDecimal("id_criterio_raggr_fasc").toString());
     ...
     * fascicoli ("controllo1") sia per conferma su controllo criterio standard ("controllo2"). Con
     * JavaScript attivato il tasto è quello presente nella finestra popup, con JavaScript
     * disattivato invece il tasto (preventivamente allineato tra i componenti) è quello che compare
	...
	if (getSession().getAttribute("salvataggioAttributes") != null) {
		...
		eseguiPrimoStepSalvataggioCriterioRaggrFascicoli((CreaCriterioRaggrFascicoli) sa[0],
			(Object[]) sa[1], (String) sa[2], (BigDecimal) sa[3], (BigDecimal) sa[4],
			((BigDecimal) sa[5]).intValue(), (String) sa[7], (List<BigDecimal>) sa[8]);
	    }
	    if (tipoControllo != null && tipoControllo.equals("controllo2")) {
		eseguiSalvataggioCriterioRaggrFascicoli((CreaCriterioRaggrFascicoli) sa[0],
			(Object[]) sa[1], (String) sa[2], ((BigDecimal) sa[5]).intValue(),
    ...
    public void eseguiPrimoStepSalvataggioCriterioRaggrFascicoli(CreaCriterioRaggrFascicoli filtri,
	    Object[] anniFascicoliValidati, String nome, BigDecimal annoDa, BigDecimal annoA,
	    int status, String standardOld, List<BigDecimal> voceTitolList) throws EMFError {
	// Controllo se il criterio è STANDARD
	String criterioStandardNow = isCriterioRaggrFascicoliStandard(filtri, anniFascicoliValidati,
		voceTitolList) ? "1" : "0";
	...
	if ((idCritRaggrFasc = fascicoliEjb.saveCriterioRaggrFascicoli(param, filtri,
		anniFascicoliValidati, nome, criterioStandard, voceTitolList)) > 0) {
	    ...
	    getMessageBox().addMessage(new Message(MessageLevel.INF,
		    "Criterio di raggruppamento fascicoli salvato con successo"));
	    getMessageBox().setViewMode(ViewMode.plain);
		...
		getForm().getCriterioRaggrFascicoliList().getTable().add()
			.copyFromBaseRow(critRBModificato);
		getForm().getCriterioRaggrFascicoliList().setStatus(Status.update);
		getForm().getTitolariList().setStatus(Status.update);

		// Devo popolare i campi ambiente/ente/struttura versione "casella di testo"...
		getForm().getCreaCriterioRaggrFascicoli().getNm_ambiente().setValue(getForm()
			.getCreaCriterioRaggrFascicoli().getId_ambiente().getDecodedValue());
		getForm().getCreaCriterioRaggrFascicoli().getNm_ente().setValue(
			getForm().getCreaCriterioRaggrFascicoli().getId_ente().getDecodedValue());
		getForm().getCreaCriterioRaggrFascicoli().getNm_strut().setValue(
			getForm().getCreaCriterioRaggrFascicoli().getId_strut().getDecodedValue());
	    } else {
	...
	if (((ComboBox) getForm().getTitolarioDetail().getComponent("id_titol"))
		.getValue() == null) {
	...
	if (idTitol == null) {
	    getMessageBox()
		    .addMessage(new Message(MessageLevel.ERR, "Titolario obbligatorio <br>"));
	    ...
	    // Controllo che non esista per quel criterio di raggruppamento fascicolo uno stesso
		...
		getForm().getTitolariList().getTable().addSortingRule(
			getForm().getTitolariList().getNm_titol().getName(), SortingRule.ASC);
		getForm().getTitolariList().getTable().sort();
	    } else {
		getMessageBox().addMessage(new Message(MessageLevel.ERR,
	...
	if (getForm().getIdFields().getId_strut() != null
		&& getForm().getIdFields().getId_strut().parse() != null
		&& getRequest().getParameter("Id_titol") != null
		&& !getRequest().getParameter("Id_titol").isEmpty()) {
	...
	BaseRow row = titolariEjb
		.getDecVoceTitolRowBean(new BigDecimal(getRequest().getParameter("Id_voce_titol")));
	if (row != null) {

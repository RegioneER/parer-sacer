...
public class ModelliUDAction extends ModelliUDAbstractAction {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (!item.isFormField()) {
                    ...
                    getForm().getModelliXsdUdDetail().getTi_modello_xsd()
                            .setValue(item.getString());
                } else if (item.getFieldName()
                        .equals(getForm().getModelliXsdUdDetail().getFl_default().getName())) {
                    getForm().getModelliXsdUdDetail().getFl_default().setValue(item.getString());
                } else if (item.getFieldName()
                        .equals(getForm().getModelliXsdUdDetail().getCd_xsd().getName())) {
                    getForm().getModelliXsdUdDetail().getCd_xsd().setValue(item.getString());
                } else if (item.getFieldName()
                        .equals(getForm().getModelliXsdUdDetail().getDs_xsd().getName())) {
                    getForm().getModelliXsdUdDetail().getDs_xsd().setValue(item.getString());
                } else if (item.getFieldName()
                        .equals(getForm().getModelliXsdUdDetail().getDt_istituz().getName())) {
                    getForm().getModelliXsdUdDetail().getDt_istituz().setValue(item.getString());
                } else if (item.getFieldName()
                        .equals(getForm().getModelliXsdUdDetail().getDt_soppres().getName())) {
            ...
            if (tmpOperation != null && tmpOperation.getFieldName().contains(NE_DETTAGLIO_CANCEL)) {
                ...
                if (getForm().getModelliXsdUdDetail().getStatus().equals(Status.insert)
                        && tmpFileItem != null && (StringUtils.isBlank(tmpFileItem.getName())
                                || tmpFileItem.getSize() == 0)) {
                    // controllo esistenza del file
                    getMessageBox().addError("Nessun file selezionato");
                }
                // id modello xsd
                BigDecimal idModelloXsdUd = getForm().getModelliXsdUdDetail().getId_modello_xsd_ud()
                        .parse();
                ...
                if (StringUtils.isBlank(flDefault)) {
                ...
                } else if (getForm().getModelliXsdUdDetail().getFl_default().parse()
                    ...
                    getMessageBox()
                            .addError("Modello xsd standard gi\u00E0 presente nell'ambiente");
                }
                ...
                if (dtIstituz == null) {
                    ...
                    getForm().getModelliXsdUdDetail().getDt_istituz()
                ...
                if (dtSoppres == null) {
                    ...
                    getForm().getModelliXsdUdDetail().getDt_soppres()
                ...
                if (getForm().getModelliXsdUdDetail().getStatus().equals(Status.insert)
                        && !dtIstituz.equals(today) && dtIstituz.before(today)) {
                    getMessageBox().addError(
                ...
                DecModelloXsdUdRowBean tmpRow = modelliXsdUdEjb.getDecModelloXsdUd(
                        ...
                        getForm().getModelliXsdUdDetail().getTi_modello_xsd().parse(),
                        CostantiDB.TiUsoModelloXsd.VERS.name(),
                        getForm().getModelliXsdUdDetail().getCd_xsd().parse(), false);
                ...
                if (StringUtils.isNotBlank(tmpFileItem.getName())) {
                    ...
                    if (getForm().getModelliXsdUdDetail().getStatus().equals(Status.update)
                        ...
                        getMessageBox().addError(
                                "Esiste almeno un versamento per questo modello, xsd non aggiornabile");
                    }
                }
                // validate xsd
                validateXsd(clob);
                ...
                if (getMessageBox().isEmpty()) {
                    // save
                    saveModelloXsdUd(idModelloXsdUd, tiModelloXsd, flDefault, cdVersione,
                            dsVersione, dtIstituz, dtSoppres, clob);
                }
            }
            ...
            LOG.error(
                    "Eccezione generica nell'importazione del modello xsd dell'unità documentaria",
                    ex);
            getMessageBox().addError(
    ...
    private void validateXsd(String clob) throws FileUploadException {
        if (StringUtils.isNotBlank(clob)) {
            ...
            try {
                ...
                java.util.logging.Logger.getLogger(ModelliUDAction.class.getName())
    ...
    public void loadDettaglio() throws EMFError {
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
            ...
            if (getTableName().equals(getForm().getModelliXsdUdList().getName())) {
                ...
                BigDecimal idModelloXsdUd = ((DecModelloXsdUdTableBean) getForm()
                        .getModelliXsdUdList().getTable()).getCurrentRow().getIdModelloXsdUd();
    ...
    private void loadDettaglioModelloXsdUd(BigDecimal idModelloXsdUd) throws EMFError {
        ...
        DecModelloXsdUdRowBean modelloXsdUdRowBean = modelliXsdUdEjb
                .getDecModelloXsdUd(idModelloXsdUd);
    ...
    private void initModelloXsdUdDetail() {
        ...
        getForm().getModelliXsdUdDetail().getId_ambiente().setDecodeMap(
                DecodeMap.Factory.newInstance(ambienteTableBean, "id_ambiente", "nm_ambiente"));

        // Imposto la combo "Tipo modello xsd"
        getForm().getModelliXsdUdDetail().getTi_modello_xsd()
                .setDecodeMap(ComboGetter.getMappaTiModelloXsdUd());
        // Imposto la combo "Flag Default"
        getForm().getModelliXsdUdDetail().getFl_default()
    ...
    public void undoDettaglio() throws EMFError {
        if (getLastPublisher().equals(Application.Publisher.MODELLI_XSD_UD_DETAIL)
            ...
            DecModelloXsdUdRowBean currentRow = (DecModelloXsdUdRowBean) getForm()
                    .getModelliXsdUdList().getTable().getCurrentRow();
    ...
    /**
     * Inizializza le combo del DETTAGLIO DI UN MODELLO XSD, ricavando i valori dai filtri di
     * ricerca modelli xsd. Se impostato, restituisce il valore dell'id del filtro relativo
     * all'ambiente
     *
    ...
    private BigDecimal initComboCreaModelloXsdUdFromRicercaModelliXsd() {
        ...
        return StringUtils.isNotBlank(getForm().getFiltriModelliXsdUd().getId_ambiente().getValue())
                ? new BigDecimal(getForm().getFiltriModelliXsdUd().getId_ambiente().getValue())
                : null;
    }
    ...
    private void saveModelloXsdUd(BigDecimal idModelloXsdUd, String tiModelloXsd, String flStandard,
            String cdVersione, String dsVersione, Date dtIstituz, Date dtSoppres, String file)
            throws EMFError {
        getMessageBox().clear();
        ...
        try {
            ...
            if (!getMessageBox().hasError()) {
                ...
                if (getForm().getModelliXsdUdList().getStatus().equals(Status.insert)) {
                    ...
                    if (pk != null) {
                        getForm().getModelliXsdUdDetail().getId_modello_xsd_ud()
                                .setValue(pk.toString());
                    }
    ...
    public void deleteModelliXsdUdList() throws EMFError {
        DecModelloXsdUdRowBean currentRow = (DecModelloXsdUdRowBean) getForm().getModelliXsdUdList()
        ...
        if (!getMessageBox().hasError()) {
            try {
                if (modelliXsdUdEjb
                        .existDecUsoModelloXsdUdAtMostOnce(currentRow.getIdModelloXsdUd())) {
                    getMessageBox().addError(
        ...
        if (!getMessageBox().hasError()
                && getLastPublisher().equals(Application.Publisher.MODELLI_XSD_UD_DETAIL)) {
            goBack();
        } else {
        ...
        try {
            if (publisherName.equals(Application.Publisher.LISTA_MODELLI_XSD_UD_SELECT)) {
                ...
                List<BigDecimal> idAmbientiToFind = getAmbientiToFind(
                        getForm().getFiltriModelliXsdUd());

                // Lista modelli xsd ud
                getForm().getModelliXsdUdList()
                        .setTable(modelliXsdUdEjb.findDecModelloXsdUd(
                                getForm().getFiltriModelliXsdUd(), idAmbientiToFind,
                                CostantiDB.TiUsoModelloXsd.VERS.name(),
                ...
                BigDecimal idModelloXsdUd = currentRow.getIdModelloXsdUd();
                if (idModelloXsdUd != null) {
                    getForm().getModelliXsdUdDetail()
                            .copyFromBean(modelliXsdUdEjb.getDecModelloXsdUd(idModelloXsdUd));
                }
            }
            getForm().getModelliXsdUdDetail().setViewMode();
            // button edit mode
    ...
    private void initComboRicercaModelliXsdUd() {
        ...
        getForm().getFiltriModelliXsdUd().getId_ambiente().setDecodeMap(
                DecodeMap.Factory.newInstance(ambienteTableBean, "id_ambiente", "nm_ambiente"));
        ...
        getForm().getFiltriModelliXsdUd().getTi_modello_xsd()
                .setDecodeMap(ComboGetter.getMappaTiModelloXsdUd());
        getForm().getFiltriModelliXsdUd().getFl_default()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
    }
     ...
     * come sono stati
     *
     * @param filtriModelliXsdUd oggetto di tipo {@link FiltriModelliXsdTipiFascicolo}
     *
     * @return la lista degli ambienti da utilizzare per la ricerca
     *
     * @throws EMFError errore generico
     */
    private List<BigDecimal> getAmbientiToFind(FiltriModelliXsdUd filtriModelliXsdUd)
            throws EMFError {
    ...
    public void ricercaModelliButton() throws EMFError {
        ...
        /*
         * Esegue la post dei filtri compilati e Valida i filtri per verificarne la correttezza
         * sintattica e l'obbligatorietà
         */
        if (filtriModelliXsdUd.postAndValidate(getRequest(), getMessageBox())) {
            /*
             * Ricavo la lista degli ambienti da cercare in base a come è stata impostata nei filtri
             * di ricerca la combo Ambiente
             */
            List<BigDecimal> idAmbientiToFind = getAmbientiToFind(filtriModelliXsdUd);

            // Lista modelli xsd ud
            getForm().getModelliXsdUdList()
                    .setTable(modelliXsdUdEjb.findDecModelloXsdUd(filtriModelliXsdUd,
    ...
    public void logEventi() throws EMFError {
        ...
        form.getOggettoDetail().getNm_tipo_oggetto()
                .setValue(SacerLogConstants.TIPO_OGGETTO_MODELLO_UD);
        DecModelloXsdUdRowBean rb = (DecModelloXsdUdRowBean) getForm().getModelliXsdUdList()
                .getTable().getCurrentRow();
    ...
    public void scaricaXsdButton() throws EMFError {
        ...
        if (getForm().getModelliXsdUdDetail().getId_modello_xsd_ud().parse() != null) {
            nomeTipo = modelliXsdUdEjb
                    .getDecModelloXsdUd(
                            getForm().getModelliXsdUdDetail().getId_modello_xsd_ud().parse())
                    .getTiModelloXsd();
        }
     ...
     *
     */
    private void zipXsdModUd(ZipOutputStream out, String blXsd, String filename)
            throws IOException {

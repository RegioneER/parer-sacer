...
public class EntiConvenzionatiAction extends EntiConvenzionatiAbstractAction {
    ...
    private static final Logger log = LoggerFactory.getLogger(EntiConvenzionatiAction.class);
	...
	if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
		...
		BaseTable ambienteEnteTable = ambienteEjb.getUsrVAbilAmbEnteConvenzTableBean(
		...
		BaseTable enteConvenzTableBean = ambienteEjb.getSIOrgEnteSiamTableBean(
			enteConvenzOrgRowBean.getBigDecimal("id_ambiente_ente_convenz"));
	...
	if (getTableName().equals(getForm().getEnteConvenzOrgList().getName())
		...
		if (!getMessageBox().hasError()) {
		    ...
		    boolean dateContigueConPrecedenteAssociazione = ambienteEjb
			    .checkDateContigue(idStrut, dtIniVal, dtFineVal);

		    if (dateContigueConPrecedenteAssociazione) {
			eseguiSalvataggioAssociazioneEnteStruttura(idStrut, idEnteConvenz, dtIniVal,
				dtFineVal);
		    } else {
    ...
    public void eseguiSalvataggioAssociazioneEnteStruttura(BigDecimal idStrut,
	    BigDecimal idEnteConvenz, Date dtIniVal, Date dtFineVal) throws EMFError {
	try {
		...
		BigDecimal idEnteConvenzOrg = getForm().getEnteConvenzOrg().getId_ente_convenz_org()
			.parse();
		// La modifica, se interessa anche l'ente convenzionato, porta ad una modifica
		// dell'id
		// dell'associazione
		BigDecimal idEnteConvenzOrgNew = ambienteEjb.updateEnteConvenzOrg(param,
			idEnteConvenzOrg, idEnteConvenz, idStrut, dtIniVal, dtFineVal);
	...
	if (getSession().getAttribute("salvataggioAttributes") != null) {
	    ...
	    eseguiSalvataggioAssociazioneEnteStruttura((BigDecimal) sa[0], (BigDecimal) sa[1],
		    (Date) sa[2], (Date) sa[3]);
	}
	forwardToPublisher(Application.Publisher.ENTE_CONVENZIONATO_DETAIL);
    }
	...
	// Ricavo il TableBean relativo agli enti convenzionati: in questa fase vanno selezionati
	// tutti in quanto
	// il controllo di validità verrà eseguito in fase di salvataggio
	// MEV#20463
	BaseTable enteSiamTableBean = ambienteEjb.getSIOrgEnteConvenzAccordoValidoTableBean(
	...
	if (getLastPublisher().equals(Application.Publisher.ENTE_CONVENZIONATO_DETAIL)) {
	    if (!idEnteConvenzOrg
		    .equals(getForm().getEnteConvenzOrg().getId_ente_convenz_org().parse())) {
		getMessageBox().addError(
			"Eccezione imprevista nell'eliminazione dell'associazione all'ente convenzionato");
	    }
	}
	...
	if (Application.Publisher.CREA_STRUTTURA.equalsIgnoreCase(param.getNomePagina())) {
	    ...
	    param.setNomeAzione(
		    SpagoliteLogUtil.getDetailActionNameDelete(form, form.getEnteConvenzOrgList()));
	} else {

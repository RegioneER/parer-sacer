...
public class MonitoraggioFascicoliAction extends MonitoraggioFascicoliAbstractAction {
    ...
    private static final Logger log = LoggerFactory.getLogger(MonitoraggioFascicoliAction.class);
        ...
        try {
            ...
            getForm().getRiepilogoVersamentiFascicoli().getId_ambiente()
                    .setDecodeMap(DecodeMap.Factory.newInstance(tmpTableBeanAmbiente, "id_ambiente",
                            "nm_ambiente"));
            getForm().getRiepilogoVersamentiFascicoli().getId_ente().setDecodeMap(
                    DecodeMap.Factory.newInstance(tmpTableBeanEnte, "id_ente", "nm_ente"));
            getForm().getRiepilogoVersamentiFascicoli().getId_strut().setDecodeMap(
                    DecodeMap.Factory.newInstance(tmpTableBeanStruttura, "id_strut", "nm_strut"));
            getForm().getRiepilogoVersamentiFascicoli().getId_tipo_fascicolo()
    ...
    public void loadListaSessFascicoliErr() throws EMFError {
        ...
        getForm().getFiltriSessioniFascicoli().getTi_stato_ses()
                .setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_stato_ses",
                        FascicoliEjb.statoSessioneFascicoliErrata.values()));
        getForm().getFiltriSessioniFascicoli().getCd_classe_err().setDecodeMap(
                fascicoliEjb.getClasseErrSacerByTipiUsoDecodeMap(Arrays.asList(new String[] {
                        FascicoliEjb.tipoUsoClasseErrore.VERS_FASCICOLO.name(),
    ...
    public void loadDettaglio() throws EMFError {
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
            ...
            if (getTableName().equals(getForm().getFascicoliList().getName())) {
                ...
                MonVLisFascKoTableBean t = (MonVLisFascKoTableBean) getForm().getFascicoliKoList()
                        .getTable();
                MonVLisFascKoRowBean r = t.getCurrentRow();
                // Verifica se utente abilitato al tipo fascicolo
                if (tipoFascicoloHelper.isTipoFascicoloAbilitato(getUser().getIdUtente(),
                        r.getIdStrut(), r.getIdTipoFascicolo(), false)) {
                    getForm().getDettaglioFascVersKo().getNm_ambiente_ente_struttura().setValue(
                            r.getNmAmbiente() + "/" + r.getNmEnte() + "/" + r.getNmStrut());
                    getForm().getDettaglioFascVersKo().getAa_fascicolo()
                            .setValue(r.getAaFascicolo().toPlainString());
                    getForm().getDettaglioFascVersKo().getCd_key_fascicolo()
                            .setValue(r.getCdKeyFascicolo());
                    getForm().getDettaglioFascVersKo().getNm_tipo_fascicolo()
                            .setValue(r.getNmTipoFascicolo());
                    getForm().getDettaglioFascVersKo().getId_ses_ko_first()
                            .setValue(r.getIdSesFascicoloKoFirst().toPlainString());
                    getForm().getDettaglioFascVersKo().getTs_ini_first_ses()
                            .setValue(r.getTsIniFirstSes().toString());
                    getForm().getDettaglioFascVersKo().getId_ses_ko_last()
                            .setValue(r.getIdSesFascicoloKoLast().toPlainString());
                    getForm().getDettaglioFascVersKo().getTs_ini_last_ses()
                            .setValue(r.getTsIniLastSes().toString());
                    getForm().getDettaglioFascVersKo().getCd_err_princ()
                            .setValue(r.getCdErrPrinc());
                    getForm().getDettaglioFascVersKo().getDs_err_princ()
                            .setValue(r.getDsErrPrinc());
                    getForm().getDettaglioFascVersKo().getTi_stato_fascicolo_ko()
                            .setValue(r.getTiStatoFascicoloKo());
                    VrsSesFascicoloKoTableBean tl = fascicoliEjb
                            .ricercaVersamentiFascicoliKo(r.getIdFascicoloKo());
                    getForm().getVersamentiFascicoliKoList().setTable(tl);
                    getForm().getVersamentiFascicoliKoList().getTable().first();
                    getForm().getVersamentiFascicoliKoList().getTable()
                            .setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
                    forwardToPublisher(Application.Publisher.DETT_FASCICOLI_VERS_KO);
                } else {
                    getMessageBox()
                            .addWarning("Utente non abilitato a visualizzare i fascicoli di tipo '"
                                    + r.getNmTipoFascicolo() + "'");
                    forwardToPublisher(getLastPublisher());
                }
                // DETTAGLIO VERSAMENTO FASCICOLO FALLITO
            } else if (getTableName().equals(getForm().getVersamentiFascicoliKoList().getName())) {
                BigDecimal idSesFascicolo = getForm().getVersamentiFascicoliKoList().getTable()
                        .getCurrentRow().getBigDecimal("id_ses_fascicolo_ko");
                DettaglioVersamentoFascicoloKo dett = fascicoliEjb
                        .getDettaglioVersamentoFascicoloKo(idSesFascicolo);
                ...
                if (tab != null) {
                    ...
                    getForm().getErroriVersList().getTable()
                            .setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
                }
    ...
    private void caricaDatiDettSessFascicoloErrato() throws EMFError {
        ...
        try {
            ...
            getForm().getDettaglioSessFascKo().getId_ambiente().setDecodeMap(DecodeMap.Factory
                    .newInstance(tmpTableBeanAmbiente, "id_ambiente", "nm_ambiente"));
            BigDecimal id = Utils.getDecodedBigDecimalFromTablebean(tmpTableBeanAmbiente,
                    "id_ambiente", "nm_ambiente", r.getString("nm_ambiente"));
            ...
            if (id != null) {
                ...
                OrgEnteTableBean tmpTableBeanEnte = ambienteEjb.getEntiAbilitatiNoTemplate(
                        getUser().getIdUtente(), id.longValueExact(), Boolean.TRUE);
                getForm().getDettaglioSessFascKo().getId_ente().setDecodeMap(
                        DecodeMap.Factory.newInstance(tmpTableBeanEnte, "id_ente", "nm_ente"));
                ...
                if (id != null) {
                    ...
                    // Ricavo i valori della combo STRUTTURA
                    OrgStrutTableBean tmpTableBeanStruttura = struttureEjb
                            .getOrgStrutTableBean(getUser().getIdUtente(), id, Boolean.TRUE);
                    getForm().getDettaglioSessFascKo().getId_strut().setDecodeMap(DecodeMap.Factory
                            .newInstance(tmpTableBeanStruttura, "id_strut", "nm_strut"));
                    id = Utils.getDecodedBigDecimalFromTablebean(tmpTableBeanStruttura, "id_strut",
                            "nm_strut", r.getString("nm_strut"));
                    if (id != null) {
                        ...
                        getForm().getDettaglioSessFascKo().getId_tipo_fascicolo()
                                .setDecodeMap(DecodeMap.Factory.newInstance(tmpTableBeanFascicolo,
                                        "id_tipo_fascicolo", "nm_tipo_fascicolo"));
    ...
    public void saveDettaglio() throws EMFError {
        if (getTableName().equals(getForm().getSesFascicoliErrList().getName())) {
            ...
            if (getForm().getDettaglioSessFascKo().validate(getMessageBox())) {
                FascicoliEjb.SalvaDettaSessErrDto dto = fascicoliEjb
                        .salvaDettaglioSessioneFascicoloErr(
                                getForm().getDettaglioSessFascKo().getId_ses_fascicolo_err()
                                        .parse(),
                                ...
                                getForm().getDettaglioSessFascKo().getCd_key_fascicolo()
                                        .getValue());
                if (dto.getMsg() == null) {
                ...
                if (dto.isSessioneCancellata()) {
                    // Rinfresca la lista sessioni errate che non avrà più il record appena
                    // cancellato
    ...
    public void dettaglioOnClick() throws EMFError {
        if (getTableName().equals(getForm().getFascicoliList().getName())) {
            ...
            redirectToPage(Application.Actions.FASCICOLI, form,
                    getForm().getFascicoliList().getName(), getForm().getFascicoliList().getTable(),
                    getNavigationEvent());
        } else if (getTableName().equals(getForm().getFascicoliKoList().getName())) {
    ...
    public void generaRiepilogoVersFascicoliButton() throws EMFError {
        if (getForm().getRiepilogoVersamentiFascicoli().postAndValidate(getRequest(),
                getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiFascicoli().getId_ambiente()
                    .parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiFascicoli().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiFascicoli().getId_strut()
                    .parse();
            BigDecimal idTipoFascicolo = getForm().getRiepilogoVersamentiFascicoli()
    ...
    private void calcolaRiepilogo(BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStruttura,
        ...
        MonVChkCntFascRowBean rowBean = fascicoliEjb.calcolaRiepilogo(
    ...
    public void calcTotFascicoliVersatiButton() throws Throwable {
        if (getForm().getRiepilogoVersamentiFascicoli().postAndValidate(getRequest(),
            ...
            rowBean = fascicoliEjb.calcolaTotFascicoliVersati(rowBean, idUser, idAmbiente, idEnte,
                    idStruttura, idTipoFascicolo);
            getForm().getFascicoliVersati().copyFromBean(rowBean);
            getForm().getVersamentiFalliti().copyFromBean(rowBean);
        }
    }
    ...
    public void calcTotFascicoliVersFallitiButton() throws Throwable {
        if (getForm().getRiepilogoVersamentiFascicoli().postAndValidate(getRequest(),
            ...
            getForm().getFascicoliVersati().copyToBean(rowBean);
            getForm().getVersamentiFalliti().copyToBean(rowBean);
            rowBean = fascicoliEjb.calcolaTotFascicoliFalliti(rowBean, idUser, idAmbiente, idEnte,
                    idStruttura, idTipoFascicolo);
            getForm().getFascicoliVersati().copyFromBean(rowBean);
            getForm().getVersamentiFalliti().copyFromBean(rowBean);
        }
    ...
    public void listaFascicoliVersati() throws EMFError {
        ...
        getForm().getFiltriFascicoli().getId_ente().setDecodeMap(
                getForm().getRiepilogoVersamentiFascicoli().getId_ente().getDecodeMap());
        getForm().getFiltriFascicoli().getId_ente()
                .setValue(getForm().getRiepilogoVersamentiFascicoli().getId_ente().getValue());
        getForm().getFiltriFascicoli().getId_strut().setDecodeMap(
                getForm().getRiepilogoVersamentiFascicoli().getId_strut().getDecodeMap());
        getForm().getFiltriFascicoli().getId_strut()
                .setValue(getForm().getRiepilogoVersamentiFascicoli().getId_strut().getValue());
        getForm().getFiltriFascicoli().getId_tipo_fascicolo().setDecodeMap(
                getForm().getRiepilogoVersamentiFascicoli().getId_tipo_fascicolo().getDecodeMap());
        getForm().getFiltriFascicoli().getId_tipo_fascicolo().setValue(
                getForm().getRiepilogoVersamentiFascicoli().getId_tipo_fascicolo().getValue());
        getForm().getFiltriFascicoli().getFl_ses_fascicolo_ko()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
        getForm().getFiltriFascicoli().getTi_stato_conservazione()
                .setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_stato_conservazione",
                        FascicoliEjb.statoConservazioneFascicoliVersati.values()));
        ...
        case TI_STATO_ATT_MEM:
            getForm().getFiltriFascicoli().getTi_stato_fasc_elenco_vers()
                    .setValue("IN_ATTESA_SCHED");
            break;
        case TI_STATO_IN_ATTESA_SCHED:
            getForm().getFiltriFascicoli().getTi_stato_fasc_elenco_vers()
                    .setValue("NON_SELEZ_SCHED");
            break;
        }
    ...
    public void listaFascicoliVersamentiFalliti() throws EMFError {
        ...
        getForm().getFiltriFascicoli().getId_tipo_fascicolo().setValue(
                getForm().getRiepilogoVersamentiFascicoli().getId_tipo_fascicolo().getValue());
        getForm().getFiltriFascicoli().getTi_stato_ses()
                .setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_stato_ses",
                        FascicoliEjb.statoFascicoliVersFalliti.values()));
        getForm().getFiltriFascicoli().getCd_classe_err()
                .setDecodeMap(fascicoliEjb.getClasseErrSacerDecodeMap(Arrays.asList(new String[] {
                        FascicoliEjb.tipoUsoClasseErrore.VERS_FASCICOLO.name(),
                        FascicoliEjb.tipoUsoClasseErrore.GENERICO.name() })));
    ...
    public void internalRicercaFascicoli() throws EMFError {
        MonitoraggioFascicoliValidator validator = new MonitoraggioFascicoliValidator(
                getMessageBox());
        Date[] dateValidate = validator.validaDate(
                getForm().getFiltriFascicoli().getGiorno_vers_da().parse(),
        ...
        ChiaveBean bean = validator.validaChiavi(
                getForm().getChiaveFascicoloSection().getDescription(),
                getForm().getFiltriFascicoli().getAa_fascicolo(),
                getForm().getFiltriFascicoli().getAa_fascicolo_da(),
        ...
        if (!getMessageBox().hasError()) {
            ...
            BigDecimal idTipoFascicolo = getForm().getFiltriFascicoli().getId_tipo_fascicolo()
                    .parse();
            String statoIndiceAip = getForm().getFiltriFascicoli().getTi_stato_fasc_elenco_vers()
                    .getValue();
            Set<String> statiConservazione = getForm().getFiltriFascicoli()
                    .getTi_stato_conservazione().getValues();
            String flSessioneFascicoloKo = getForm().getFiltriFascicoli().getFl_ses_fascicolo_ko()
                    .getValue();
            MonVLisFascTableBean t = fascicoliEjb.ricercaFascicoliPerMonitoraggio(idUser,
                    idAmbiente, idEnte, idStruttura, idTipoFascicolo, dateValidate,
                    ...
                    statoIndiceAip, statiConservazione, flSessioneFascicoloKo);
    ...
    private void internalRicercaRicercaFascicoliKo() throws EMFError {
        ...
        if (!getMessageBox().hasError()) {
            ...
            if ((cdClasseErr != null && (!cdClasseErr.equals("")))
                    || (cdErr != null && (!cdErr.equals("")))) {
                t = fascicoliEjb.ricercaFascicoliKoPerMonitoraggioErr(idUser, idAmbiente, idEnte,
                ...
                t = fascicoliEjb.ricercaFascicoliKoPerMonitoraggio(idUser, idAmbiente, idEnte,
                        idStruttura, idTipoFascicolo, dateValidate,
                        bean.isSingleValue() ? bean.getAnno() : bean.getAnnoDa(), bean.getAnnoA(),
                        bean.isSingleValue() ? bean.getNumero() : bean.getNumeroDa(),
                        bean.getNumeroA(), statoSessione, cdClasseErr, cdErr);
    ...
    public void scaricaXmlVersButton() throws EMFError {
        downloadFileCommon(getForm().getDettaglioVersamentoKo().getBl_xml_sip().getValue(),
                getForm().getDettaglioVersamentoKo().getBl_xml_rapp_vers().getValue(),
                "versamentoFascicoloFallito",
                getForm().getDettaglioVersamentoKo().getId_ses_fascicolo_ko().parse(), "IndiceSIP",
                "EsitoNegativoVersamento");
    }

    private void downloadFileCommon(String xmlSip, String xmlRisposta, String nomeFileZip,
            BigDecimal idSessione, String nomeFileXmlSip, String nomeFileXmlRisposta)
            throws EMFError {
        ...
        try {
            ...
            try (FileOutputStream tmpOutputStream = new FileOutputStream(zipDaScaricare);
                ...
                String nomeFileXmlRispostaCompleto = nomeFileXmlRisposta + "_" + strIdSessione
                        + ".xml";
    ...
    protected void postLoad() {
        ...
        if (getForm() instanceof MonitoraggioFascicoliForm) {
            getForm().getRiepilogoVersamentiFascicoli().getGeneraRiepilogoVersFascicoliButton()
                    .setEditMode();
            getForm().getCalcolaTotaliButtonList().getCalcTotFascicoliVersFallitiButton()
                    .setEditMode();
            ...
            getForm().getDettaglioVersamentoKo().getScaricaXmlVersButton()
            ...
            getForm().getFiltriSessioniFascicoli().getRicercaSessFascicoliErrateButton()
                    .setEditMode();
            /*
             * // Anche se c'è il jquery che lo cambia lato client lo metto anche lato server per
             * spegnerlo all'inizio String
             * val=getForm().getFiltriSessioniFascicoli().getTi_stato_ses().getValue(); if
             * (val!=null &&
             * val.equals(FascicoliEjb.statoSessioneFascicoliErrati.NON_VERIFICATA.name())) {
             * getForm().getFiltriSessioniFascicoli().getTrasfSessErrateInVersFallitiButton().
             * setEditMode(); } else {
             * getForm().getFiltriSessioniFascicoli().getTrasfSessErrateInVersFallitiButton().
             * setViewMode(); }
             */
            ...
            getForm().getDettaglioSessFascKo().getScaricaXmlVersSessKoButton()
                    .setDisableHourGlass(true);
        }
    }
    ...
    private void ricercaSessFascicoliErrateButtonInternal() throws EMFError {
        MonitoraggioFascicoliValidator validator = new MonitoraggioFascicoliValidator(
                getMessageBox());
        Date[] dateValidate = validator.validaDate(
                getForm().getFiltriSessioniFascicoli().getGiorno_vers_da().parse(),
        ...
        if (!getMessageBox().hasError()) {
            String statoSessione = getForm().getFiltriSessioniFascicoli().getTi_stato_ses()
                    .getValue();
            String cdClasseErr = getForm().getFiltriSessioniFascicoli().getCd_classe_err()
                    .getValue();
            ...
            BaseTable t = fascicoliEjb.ricercaSessFascErrate(
                    ...
                    (dateValidate != null && dateValidate[1] != null) ? dateValidate[1] : null,
                    statoSessione, cdClasseErr, cdErr);
            getForm().getSesFascicoliErrList().setTable(t);
            getForm().getSesFascicoliErrList().getTable()
                    .setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
    ...
    public void tabRapportoVersamentoDettSessFascKoOnClick() throws EMFError {
        getForm().getDettaglioSessFascKoTabs().setCurrentTab(
                getForm().getDettaglioSessFascKoTabs().getRapportoVersamentoDettSessFascKo());
        forwardToPublisher(Application.Publisher.DETT_SESS_FASCICOLO_ERRATO);
    }
    ...
    public void scaricaXmlVersSessKoButton() throws EMFError {
        downloadFileCommon(getForm().getDettaglioSessFascKo().getBl_xml_sip().getValue(),
                getForm().getDettaglioSessFascKo().getBl_xml_rapp_vers().getValue(),
                "versamentoFascicoloFallito",
            ...
            if (idAmbiente != null) {
                ...
                if (tmpTableBeanEnte.size() == 1) {
                    ...
                    OrgStrutTableBean tmpTableBeanStrut = struttureEjb.getOrgStrutTableBean(
                            ...
                            Boolean.TRUE);
                    cbxStruttura.setDecodeMap(DecodeMap.Factory.newInstance(tmpTableBeanStrut,
                            "id_strut", "nm_strut"));
                    if (tmpTableBeanStrut.size() == 1) {
                        ...
                        DecTipoFascicoloTableBean tmpTableBeanFascicolo = tipoFascicoloEjb
                                .getTipiFascicoloAbilitati(getUser().getIdUtente(),
                                        tmpTableBeanStrut.getRow(0).getIdStrut(), false);
                        cbxTipoFascicolo.setDecodeMap(DecodeMap.Factory.newInstance(
                                tmpTableBeanFascicolo, "id_tipo_fascicolo", "nm_tipo_fascicolo"));
                    } else {

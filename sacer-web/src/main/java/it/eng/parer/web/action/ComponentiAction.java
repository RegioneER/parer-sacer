...
public class ComponentiAction extends ComponentiAbstractAction {
    ...
    public void initOnClick() throws EMFError {
        ...
        DecodeMap mappaRappr = DecodeMap.Factory.newInstance(tmpRapprTableBean,
                "nm_tipo_rappr_comp", "nm_tipo_rappr_comp");
        ...
        getForm().getRicComponentiFiltri().getNm_formato_file_vers()
                .setDecodeMap(mappaFormatoFileDoc);
        getForm().getRicComponentiFiltri().getTi_esito_contr_conforme().setDecodeMap(ComboGetter
                .getMappaSortedGenericEnum("stato", VolumeEnums.ControlloConformitaEnum.values()));
        getForm().getRicComponentiFiltri().getTi_esito_verif_firme()
                .setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_esito_verif_firme",
                        VolumeEnums.StatoVerifica.values()));
        getForm().getRicComponentiFiltri().getFl_comp_firmato()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
        getForm().getRicComponentiFiltri().getCd_registro_key_unita_doc()
    ...
    public void ricerca() throws EMFError {
        ...
        if (compfiltri.validate(getMessageBox())) {
            ...
            if (!getMessageBox().hasError()) {
                ...
                // impostati
                String maxResultRicercaComp = configurationHelper.getValoreParamApplicByApplic(
                        CostantiDB.ParametroAppl.MAX_RESULT_RICERCA_COMP);
                AroVRicCompTableBean componentiTableBean = componentiHelper.getAroVRicCompViewBean(
                        getIdStrut(), compfiltri, dateAcquisizioneValidate, tmpTableBeanTipoUD,
                        tmpTableBeanTipoDoc, Integer.parseInt(maxResultRicercaComp));
    ...
    /**
     * Metodo invocato dal bottone omonimo dei filtri ricerca unità documentarie per ripulire i
     * filtri
     *
     * @throws EMFError errore generico
     */
    ...
    /**
     * Metodo invocato sul bottone di dettaglio/modifica di una riga della lista componenti, esegue
     * il caricamento dei dati della riga selezionata per visualizzare il dettaglio
     *
     * @throws EMFError errore generico
     */
    @Override
    public void loadDettaglio() throws EMFError {
        ...
        if (getRequest().getParameter("table").equals(getForm().getComponentiList().getName())) {
            ...
            BigDecimal idComp = getForm().getComponentiList().getTable().getCurrentRow()
                    .getBigDecimal("id_comp_doc");
            ...
            getForm().getComponentiDetail().getUrn_doc()
                    .setValue((String) getSession().getAttribute("UD_URN_DOC"));
            ...
            if (componenteRB.getTiStatoDoc() != null && compVolRB.getIdCompDoc() != null) {
                ...
                getForm().getComponentiDetail().getDt_chius_volume()
                        .setValue(compVolRB.getDtChiusVolume() != null
                                ? compVolRB.getDtChiusVolume().toString()
                                : "");
            ...
            if (dtAnnulDoc.compareTo(cal.getTime()) == 0) {
                ...
                getForm().getComponentiDetail().getDt_annul_doc().setValue(null);
                    ...
                    * Se il documento è annullato e l'unità doc è valida oppure l'unità documentaria
                    * è annullata e la data è diversa da quella di annullamento del documento
                    */ if (dtAnnulUd.compareTo(cal.getTime()) == 0
            ...
            if (componenteRB.getDsAlgoHashFileCalc() != null) {
                ...
                getForm().getComponentiDetail().getHash_calc_personalizzato()
            ...
            if (componenteRB.getDsAlgoHashFile_256() != null) {
                ...
                getForm().getComponentiDetail().getHash_personalizzato()
                        .setDescription(descrizione);
            ...
            if (componenteRB.getTiSupportoComp().equals("RIFERIMENTO")) {
                String trasformatore = componenteRB.getCdRegistroKeyUnitaDocRif() + " - "
                        + componenteRB.getAaKeyUnitaDocRif() + " - "
                        + componenteRB.getCdKeyUnitaDocRif();
            ...
            AroVLisFirmaCompTableBean listFirmaCompTB = componentiHelper
                    .getAroVLisFirmaCompTableBean(idComp, Integer.parseInt(maxResultStandard));
            ...
            AroVLisMarcaCompTableBean listMarcaCompTB = componentiHelper
                    .getAroVLisMarcaCompTableBean(idComp, Integer.parseInt(maxResultStandard));
            ...
            myRecuperoExt.getParametriRecupero()
                    .setIdUnitaDoc(componenteRB.getIdUnitaDoc().longValue());
            ...
            getForm().getComponentiDetail().getScarica_dip_esibizione_comp_doc()
            ...
            // LISTA MARCHE
            // Ottengo l'id marca
            BigDecimal idMarca = ((AroVLisMarcaCompRowBean) getForm().getMarcheList().getTable()
                    .getCurrentRow()).getIdMarcaComp();
            ...
            AroVLisCertifCaMarcaCompTableBean listCertifCaMarcaCompTB = componentiHelper
                    .getAroVLisCertifCaMarcaCompTableBean(idMarca,
                            VFTipoControlloType.CATENA_TRUSTED.name());
            ...
            // LISTA FIRME
            // Ottengo l'id firma
            BigDecimal idFirma = ((AroVLisFirmaCompRowBean) getForm().getFirmeList().getTable()
                    .getCurrentRow()).getIdFirmaComp();
            ...
            if (firmaRB.getTiRifTempUsato() != null) {
                if (firmaRB.getTiRifTempUsato().equals("DATA_FIRMA")) {
                    ...
                    firmaRB.setTiRifTempUsato(
                            firmaRB.getTiRifTempUsato() + " (Data di versamento)");
                } else if (firmaRB.getTiRifTempUsato().equals("MT_VERS_NORMA")) {
                    firmaRB.setTiRifTempUsato(
                            firmaRB.getTiRifTempUsato() + " (Marca temporale versata, a norma)");
                } else if (firmaRB.getTiRifTempUsato().equals("MT_VERS_SEMPLICE")) {
                    firmaRB.setTiRifTempUsato(
                            firmaRB.getTiRifTempUsato() + " (Marca temporale versata, semplice)");
                } else if (firmaRB.getTiRifTempUsato().equals("RIF_TEMP_VERS")) {
                    String rif = componentiHelper
                            .getAroCompDocRowBean((BigDecimal) getSession().getAttribute("idComp"),
                                    getIdStrut())
                            .getDsRifTempVers();
                    if (rif != null) {
                        firmaRB.setTiRifTempUsato(firmaRB.getTiRifTempUsato()
                                + " (Riferimento temporale versato: " + rif + ")");
                    } else {
                        firmaRB.setTiRifTempUsato(
                                firmaRB.getTiRifTempUsato() + " (Riferimento temporale versato)");
                    }
                }
            }
            ...
            AroVLisCertifCaFirmaCompTableBean listCertifCaFirmaCompTB = componentiHelper
                    .getAroVLisCertifCaFirmaCompTableBean(idFirma,
                            VFTipoControlloType.CATENA_TRUSTED.name());
    ...
    /**
     * Trigger invocato alla modifica della combo NM_TIPO_STRUT_DOC per caricare i dati sulla combo
     * NM_TIPO_COMP_DOC
     *
    ...
    /**
     * Metodo invocato sul bottone di dettaglio/modifica di una riga della lista componenti, esegue
     * il caricamento della pagina per visualizzare il dettaglio, in seguito alla loadDettaglio
     *
     * @throws EMFError errore generico
     */
    @Override
    public void dettaglioOnClick() throws EMFError {
        ...
        if (getRequest().getParameter("table").equals(getForm().getComponentiList().getName())) {
            ...
            // LISTA FIRME
            getForm().getFirmeDettaglioTabs()
                    .setCurrentTab(getForm().getFirmeDettaglioTabs().getInfoPrincipaliFirme());
            getForm().getFirmeDettaglioListsTabs().setCurrentTab(
                    getForm().getFirmeDettaglioListsTabs().getListaCertificatiCAFirme());
            // apri le sezioni
            getForm().getDocumento().setLoadOpened(true);
            getForm().getComponente().setLoadOpened(true);
            // Visualizzo le section a seconda dei casi
            if (componentiHelper.isComponenteInElenco(
                    getForm().getFirmeUnitaDocumentarieDetail().getId_comp_doc().parse())) {
                getForm().getFirmaDataVersamento().setHidden(false);
            } else {
                getForm().getFirmaDataVersamento().setHidden(true);
            }

            if (componentiHelper.isComponenteInVolume(
                    getForm().getFirmeUnitaDocumentarieDetail().getId_comp_doc().parse())) {
          ...
          // Componenti
            ...
            BigDecimal idCompDoc = getForm().getComponentiList().getTable().getRow(riga)
                    .getBigDecimal("id_comp_doc");
    ...
    public void tabListaDatiSpecificiCompOnClick() throws EMFError {
        getForm().getComponentiDettaglioListsTabs().setCurrentTab(
                getForm().getComponentiDettaglioListsTabs().getListaDatiSpecificiComp());
        forwardToPublisher(Application.Publisher.COMPONENTI_DETAIL);
    }
    ...
    public void tabListaDatiSpecificiMigrazioneCompOnClick() throws EMFError {
        getForm().getComponentiDettaglioListsTabs().setCurrentTab(
                getForm().getComponentiDettaglioListsTabs().getListaDatiSpecificiMigrazioneComp());
        forwardToPublisher(Application.Publisher.COMPONENTI_DETAIL);
    }
    ...
    public void tabControlloMarcaOnClick() throws EMFError {
        getForm().getMarcheDettaglioTabs()
                .setCurrentTab(getForm().getMarcheDettaglioTabs().getControlloMarca());
        forwardToPublisher(Application.Publisher.MARCHE_DETAIL);
    }

    @Override
    public void tabListaCertificatiCAMarcheOnClick() throws EMFError {
        getForm().getMarcheDettaglioListsTabs().setCurrentTab(
                getForm().getMarcheDettaglioListsTabs().getListaCertificatiCAMarche());
        forwardToPublisher(Application.Publisher.MARCHE_DETAIL);
    }
    ...
    public void tabListaCertificatiCAOCSPMarcheOnClick() throws EMFError {
        getForm().getMarcheDettaglioListsTabs().setCurrentTab(
                getForm().getMarcheDettaglioListsTabs().getListaCertificatiCAOCSPMarche());
        forwardToPublisher(Application.Publisher.MARCHE_DETAIL);
    }
    ...
    public void tabCertificatoFirmatarioOnClick() throws EMFError {
        getForm().getFirmeDettaglioTabs()
                .setCurrentTab(getForm().getFirmeDettaglioTabs().getCertificatoFirmatario());
        forwardToPublisher(Application.Publisher.FIRME_DETAIL);
    }
    ...
    public void tabListaCertificatiCAOCSPFirmeOnClick() throws EMFError {
        getForm().getFirmeDettaglioListsTabs().setCurrentTab(
                getForm().getFirmeDettaglioListsTabs().getListaCertificatiCAOCSPFirme());
        forwardToPublisher(Application.Publisher.FIRME_DETAIL);
    }
    ...
    public void tabControlliFirmaOnClick() throws EMFError {
        getForm().getFirmeDettaglioTabs()
                .setCurrentTab(getForm().getFirmeDettaglioTabs().getControlliFirma());
        forwardToPublisher(Application.Publisher.FIRME_DETAIL);
    }
    ...
    private void scaricaComp(CostantiDB.TipiEntitaRecupero tipoEntitaRecupero) throws EMFError {
        ...
        String tipoSaveFile = udHelper
                .getTipoSaveFile(getForm().getComponentiDetail().getId_tipo_unita_doc().parse());
        ...
        recupero.getChiave().setTipoRegistro(
                getForm().getComponentiDetail().getCd_registro_key_unita_doc().parse());
        recupero.getChiave().setAnno(BigInteger.valueOf(
                getForm().getComponentiDetail().getAa_key_unita_doc().parse().longValue()));
        recupero.getChiave()
                .setNumero(getForm().getComponentiDetail().getCd_key_unita_doc().parse());

        RecuperoWeb recuperoDoc = new RecuperoWeb(recupero, getUser(), idUnitaDoc, idComp,
    ...
    public void scarica_dip_comp() throws EMFError {
        ...
        try {
            ddip = new DownloadDip(getUser(),
                    getForm().getComponentiDetail().getId_unita_doc().parse());

...
public class TestElencoVersamentoHelperServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Collection<Long> retrieveUdVersOrAggInElenco = evHelper.retrieveUdVersOrAggInElencoValidate(idElencoVers);
        ...
        Collection<ComponenteDaVerificare> retrieveCompsToVerify = evHelper.retrieveCompsToVerify(idElencoVers,
                idUnitaDoc);
        ...
        sb.append("<h2>").append("elaboraStrutturaFase2 &gt; elaboraUDFase2 &gt; aggiornaElencoSeNonCiSonoDocAggiunti")
                .append("</h2>");

        Set<ComponenteInElenco> retrieveComponentiInElenco = evHelper.retrieveComponentiInElenco(idUnitaDoc,
                idElencoVers);
        ...
        sb.append("<h2>").append("elaboraStrutturaFase2 &gt; elaboraUDFase2 &gt; aggiornaElencoSeCiSonoDocAggiunti")
                .append("</h2>");
        ...
        if (dataMinimaDocInElenco != null) {
            data = new SimpleDateFormat(WebConstants.DATE_FORMAT_TIMESTAMP_TYPE).format(dataMinimaDocInElenco);
        } else {
        ...
        Set<Long> retrieveDocNonInElenco = evHelper.retrieveDocNonInElenco(idUnitaDoc, new Date(), idElencoVers);
        ...
        Set<ComponenteInElenco> retrieveCompInElenco = evHelper.retrieveCompInElenco(idUnitaDoc, idElencoVers);
    ...
    class PrintAroCompDoc extends Printer<AroCompDoc> {
        public PrintAroCompDoc(AroCompDoc aroCompDoc, StringBuilder buffer) {
            super(Arrays.asList(new AroCompDoc[] { aroCompDoc }), "Retreive componente: ", buffer);
        }

        @Override
        protected String print(AroCompDoc element) {
            return element != null
                    ? "componente con id " + element.getIdCompDoc() + " con urn " + element.getDsUrnCompCalc()
                    : "componente null";
        }
    ...
    class PrintElvElencoVersDaElab extends Printer<ElvElencoVersDaElab> {
        ...
        public PrintElvElencoVersDaElab(Collection<ElvElencoVersDaElab> elements, StringBuilder buffer) {
    ...
    class PrintComponenteDaVerificare extends Printer<ComponenteDaVerificare> {
        ...
        public PrintComponenteDaVerificare(Collection<ComponenteDaVerificare> elements, StringBuilder buffer) {
            super(elements, "Componenti da verifica:", buffer);
        }

        @Override
        protected String print(ComponenteDaVerificare componente) {
            return " id: " + componente.getIdCompDoc() + " data " + componente.getDtCreazione() + " isFirmato "
                    + componente.getFlCompFirmato();
        }
    }
    ...
    class PrintUnitaDocumentariaInElenco extends Printer<UnitaDocumentariaInElenco> {
        ...
        public PrintUnitaDocumentariaInElenco(Collection<UnitaDocumentariaInElenco> elements, StringBuilder buffer) {
            super(elements, "UD in elenco:", buffer);
        }

        @Override
        protected String print(UnitaDocumentariaInElenco ud) {
            return (ud.getIdUnitaDoc() + " id unita doc, solo aggiunti: " + ud.isFlSoloDocAggiunti());
        }

...
public class SceltaOrganizzazioneAction extends SceltaOrganizzazioneAbstractAction {
    ...
    public void process() throws EMFError {
        ...
        if (utente.isUtenteDaAssociare()) {
            ...
            try {
                ...
                getMessageBox()
                        .addError("Impossibile accedere a Sacer. Riprovare ad effettuare il login tra qualche minuto.");
            ...
            if (iamRB != null) {
                ...
                try {
                    ...
                    tmpTableBeanEnte = ambienteEjb.getEntiAbilitatiNoTemplate(utente.getIdUtente(), idAmbienteDefault,
                            Boolean.TRUE);
                    ...
                    logger.error("Errore nel recupero dei valori di default dell'organizzazione: " + ex.getMessage(),
                            ex);
                }
                ...
                try {
                    OrgAmbienteTableBean ambientiTableBean = ambienteEjb.getAmbientiAbilitati(utente.getIdUtente());
                    DecodeMap mappaAmbiente = new DecodeMap();
    ...
    /**
     * Metodo utilizzato per controllare il valore nella combo ambiente quando questo è l'unico presente e settare di
     * conseguenza la combo ente
     *
     * @param idUtente
     *            id utente
     * @param idAmbiente
    ...
    public void checkUniqueAmbienteInCombo(long idUtente, BigDecimal idAmbiente) throws EMFError {
        if (idAmbiente != null) {
            ...
            if (tmpTableBeanEnte.size() == 1) {
                getForm().getStrutture().getId_ente().setValue(tmpTableBeanEnte.getRow(0).getIdEnte().toString());
    ...
    public void checkUniqueEnteInCombo(long idUtente, BigDecimal idEnte) throws EMFError {
        if (idEnte != null) {
            ...
            OrgStrutTableBean tmpTableBeanStrut = struttureEjb.getOrgStrutTableBean(idUtente, idEnte, Boolean.TRUE);
            DecodeMap mappaStrut = new DecodeMap();
            ...
            if (tmpTableBeanStrut.size() == 1) {
                getForm().getStrutture().getId_strut().setValue(tmpTableBeanStrut.getRow(0).getIdStrut().toString());
            }
        }
    }

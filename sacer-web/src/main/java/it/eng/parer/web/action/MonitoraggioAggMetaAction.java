...
public class MonitoraggioAggMetaAction extends MonitoraggioAggMetaAbstractAction {
    ...
    @Override
    public JSONObject triggerFiltriRicercaMonitoraggioAggMetaId_ambienteOnTrigger()
	    throws EMFError {
	getForm().getFiltriRicercaMonitoraggioAggMeta().post(getRequest());
	BigDecimal idAmbiente = getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ambiente()
		.parse();
	...
	if (idAmbiente != null) {
	    ...
	    getForm().getFiltriRicercaMonitoraggioAggMeta().getId_tipo_unita_doc()
		    .setDecodeMap(new DecodeMap());
	    getForm().getFiltriRicercaMonitoraggioAggMeta().getId_registro_unita_doc()
		    .setDecodeMap(new DecodeMap());
	    getForm().getFiltriRicercaMonitoraggioAggMeta().getId_tipo_doc()
		    .setDecodeMap(new DecodeMap());
	} else {
	...
	if (getForm().getFiltriRicercaMonitoraggioAggMeta().postAndValidate(getRequest(),
		getMessageBox())) {
	    BigDecimal idAmbiente = getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ambiente()
		    .parse();
	    BigDecimal idEnte = getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ente()
		    .parse();
	    BigDecimal idStruttura = getForm().getFiltriRicercaMonitoraggioAggMeta().getId_strut()
		    .parse();
	    BigDecimal idTipoUnitaDoc = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getId_tipo_unita_doc().parse();
	    BigDecimal idRegistroUnitaDoc = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getId_registro_unita_doc().parse();
	    BigDecimal idTipoDocPrinc = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getId_tipo_doc().parse();
	    BigDecimal aaKeyUnitaDoc = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getAa_key_unita_doc().parse();
	    BigDecimal aaKeyUnitaDocDa = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getAa_key_unita_doc_da().parse();
	    BigDecimal aaKeyUnitaDocA = getForm().getFiltriRicercaMonitoraggioAggMeta()
		    .getAa_key_unita_doc_a().parse();
		...
		getMessageBox().addError(
			"Inserire l'anno o in alternativa il range di anni, non entrambi");
	    }
		...
		calcolaRiepilogoAggMetaGlobale(idAmbiente, idEnte, idStruttura, aaKeyUnitaDoc,
			aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc, idRegistroUnitaDoc,
			idTipoDocPrinc);
	    }
	}
	...
	getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ente().setDecodeMap(
		DecodeMap.Factory.newInstance(tmpTableBeanEnte, "id_ente", "nm_ente"));
	getForm().getFiltriRicercaMonitoraggioAggMeta().getId_strut().setDecodeMap(
	...
	calcolaRiepilogoAggMetaGlobale(idAmbiente, idEnte, idStruttura, null, null, null, null,
		null, null);
	...
	getForm().getFiltriAggMetaErrati().getTi_stato_ses()
		.setDecodeMap(ComboGetter.getMappaStatoAggiornamento());
	getForm().getFiltriAggMetaErrati().getId_classe_err_sacer().setDecodeMap(
		monitAggMetaEjb.getIdClasseErrSacerDecodeMap(Arrays.asList(new String[] {
			MonitoraggioAggMetaEjb.tipoUsoClasseErrore.VERS_UNITA_DOC.name(),
	...
	if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
		...
		AroVVisUpdUnitaDocRowBean rb = monitAggMetaEjb
		...
		getForm().getAggMetaTabs().setCurrentTab(
			getForm().getAggMetaTabs().getInformazioniPrincipaliAggMeta());
		...
		String sistemaConservazione = configHelper.getValoreParamApplicByApplic(
			CostantiDB.ParametroAppl.NM_SISTEMACONSERVAZIONE);
		...
		getForm().getDettaglioAggMeta().getDs_urn_upd_unita_doc().setValue(
			MessaggiWSFormat.formattaUrnUpdUniDoc(MessaggiWSFormat.formattaBaseUrnUpd(
		...
		AroVLisUpdDocUnitaDocTableBean tDoc = monitAggMetaEjb
		...
		getForm().getDocumentiAggiornatiList().getTable()
		...
		getForm().getComponentiAggiornatiList().getTable()
			.setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
		// Lista aggiornamenti metadati risolti
		AroVLisUpdKoRisoltiTableBean tRis = monitAggMetaEjb
		...
		getForm().getAggMetaRisoltiList().getTable()
			.setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
		// Lista warning rilevati
		AroWarnUpdUnitaDocTableBean tWar = monitAggMetaEjb
			.getAroWarnUpdUnitaDocTableBean(idUpdUnitaDoc);
		getForm().getWarningRilevatiList().setTable(tWar);
		getForm().getWarningRilevatiList().getTable().first();
		getForm().getWarningRilevatiList().getTable()
			.setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
	    } else if (getTableName().equals(getForm().getAggMetaFallitiList().getName())) {
		BaseRow r = ((BaseTable) getForm().getAggMetaFallitiList().getTable())
			.getCurrentRow();
		...
		BaseRow r = ((BaseTable) getForm().getUnitaDocAggMetaFallitiList().getTable())
			.getCurrentRow();
		...
		MonVLisUpdUdErrRowBean r = ((MonVLisUpdUdErrTableBean) getForm()
			.getAggMetaErratiList().getTable()).getCurrentRow();
	...
	getForm().getDettaglioAggMetaFallito().getScaricaXmlAggFallitoButton()
		.setDisableHourGlass(true);

	// Mi posiziono sul tab principale
	getForm().getAggMetaFallitiTabs().setCurrentTab(
		getForm().getAggMetaFallitiTabs().getInformazioniPrincipaliFalliti());
	...
	getForm().getDettaglioUnitaDocAggMetaFallito().getScaricaXmlAggFallitoLastButton()
		.setEditMode();
	getForm().getDettaglioUnitaDocAggMetaFallito().getScaricaXmlAggFallitoLastButton()
		.setDisableHourGlass(true);

	// Mi posiziono sul tab principale
	getForm().getUnitaDocAggMetaFallitiTabs().setCurrentTab(getForm()
		.getUnitaDocAggMetaFallitiTabs().getInformazioniPrincipaliUnitaDocAggFalliti());

	// Formatto gli xml di richiesta e risposta dell'ultimo aggiornamento in modo tale che
	// compaiano on-line in
	...
	// Ulteriori errori
	List<Long> idSesUpdUnitaDocKoList = monitAggMetaEjb
		.ricercaVrsSesUpdUnitaDocKo(rb.getIdUpdUnitaDocKo());
	...
	getForm().getUlterioriErroriUnitaDocAggMetaFallitiList().getTable()
		.setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
	...
	getForm().getDettaglioAggMetaErrato().getTi_stato_ses()
		.setDecodeMap(ComboGetter.getMappaStatoAggiornamento());
	...
	getForm().getDettaglioAggMetaErrato().getScaricaXmlAggErratoButton()
		.setDisableHourGlass(true);
	...
	getForm().getUlterioriErroriErratiList().getTable()
		.setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
    }
	...
	if (xmlVersVuoti) {
	    ...
	    if (!xmls.isEmpty()) {
		XmlPrettyPrintFormatter formatter = new XmlPrettyPrintFormatter();
		getForm().getDettaglioAggMeta().getBl_xml_rich().setValue(formatter
			.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RICHIESTA)));
		getForm().getDettaglioAggMeta().getBl_xml_risp().setValue(
    ...
    /**
     * Nel caso in cui il backend di salvataggio degli XML di versamento aggiornamento metadati
     * fallito sia l'object storage (gestito dal parametro <strong>applicativo</strong>) si possono
     * verificare 2 casi:
     * <ul>
     * <li>gli xml sono <em>ancora</em> sul DB perché non ancora migrati</li>
     * <li>gli xml sono effettivamente sull'object storage</li>
     ...
     * Se si avvera il secondo caso li devo recuperare
     *
     * @param riga MonVVisUdUpdKoRowBean
     */
    private void aggiungiXmlSesAggMdFallitaDaObjectStorageByUpdUd(MonVVisUdUpdKoRowBean riga) {
	boolean xmlSesAggMdKoVuoti = riga.getBlXmlRichLast() == null
		&& riga.getBlXmlRispLast() == null;
	/*
	 * Se gli xml non sono ancora stati migrati, però, sono ancora presenti sulle tabelle
	 */
	if (riga.getIdSesUpdUdKoLast() != null && xmlSesAggMdKoVuoti) {
		...
		getForm().getDettaglioUnitaDocAggMetaFallito().getBl_xml_rich_last()
			.setValue(formatter
				.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RICHIESTA)));
		getForm().getDettaglioUnitaDocAggMetaFallito().getBl_xml_risp_last().setValue(
			formatter.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RISPOSTA)));
	    }
	}
    }
     ...
     * Se si avvera il secondo caso li devo recuperare
     *
     * @param riga MonVVisUpdUdKoRowBean
     */
	...
	if (riga.getIdSesUpdUnitaDocKo() != null && xmlSesAggMdKoVuoti) {
	    ...
	    if (!xmls.isEmpty()) {
		XmlPrettyPrintFormatter formatter = new XmlPrettyPrintFormatter();
		getForm().getDettaglioAggMetaFallito().getBl_xml_rich().setValue(formatter
			.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RICHIESTA)));
		getForm().getDettaglioAggMetaFallito().getBl_xml_risp().setValue(
			formatter.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RISPOSTA)));
	    }
	}
    }
    ...
    /**
     * Nel caso in cui il backend di salvataggio degli XML di versamento aggiornamento metadati
     * errato sia l'object storage (gestito dal parametro <strong>applicativo</strong>) si possono
     * verificare 2 casi:
     * <ul>
     * <li>gli xml sono <em>ancora</em> sul DB perché non ancora migrati</li>
     * <li>gli xml sono effettivamente sull'object storage</li>
     ...
     * Se si avvera il secondo caso li devo recuperare
     *
     * @param riga MonVVisUpdUdErrRowBean
     */
	...
	if (riga.getIdSesUpdUnitaDocErr() != null && xmlSesAggMdErrVuoti) {
	    ...
	    if (!xmls.isEmpty()) {
		XmlPrettyPrintFormatter formatter = new XmlPrettyPrintFormatter();
		getForm().getDettaglioAggMetaErrato().getBl_xml_rich().setValue(formatter
			.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RICHIESTA)));
		getForm().getDettaglioAggMetaErrato().getBl_xml_risp().setValue(
			formatter.prettyPrintWithDOM3LS(xmls.get(CostantiDB.TipiXmlDati.RISPOSTA)));
	    }
	}
    }
	...
	if (getTableName().equals(getForm().getAggMetaFallitiList().getName())) {
	    BigDecimal idSesUpdUnitaDocKo = getForm().getDettaglioAggMetaFallito()
		    .getId_ses_upd_unita_doc_ko().parse();
	    caricaDettaglioAggMetaFallito(idSesUpdUnitaDocKo);
	    forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META_FALLITO);
	} else if (getTableName().equals(getForm().getUnitaDocAggMetaFallitiList().getName())) {
	    BigDecimal idUpdUnitaDocKo = getForm().getDettaglioUnitaDocAggMetaFallito()
		    .getId_upd_unita_doc_ko().parse();
	    caricaDettaglioUnitaDocAggMetaFallito(idUpdUnitaDocKo);
	    forwardToPublisher(Application.Publisher.DETTAGLIO_UNITA_DOC_AGG_META_FALLITO);
	} else if (getTableName().equals(getForm().getAggMetaErratiList().getName())) {
	    BigDecimal idSesUpdUnitaDocErr = getForm().getDettaglioAggMetaErrato()
		    .getId_ses_upd_unita_doc_err().parse();
	    caricaDettaglioAggMetaErrato(idSesUpdUnitaDocErr);
	...
	if (getTableName().equals(getForm().getAggMetaFallitiList().getName())) {
	    try {
		String statoPre = getForm().getDettaglioAggMetaFallito().getTi_stato_ses_upd_ko()
			.parse();
		getForm().getDettaglioAggMetaFallito().post(getRequest());
		if (getForm().getDettaglioAggMetaFallito().validate(getMessageBox())) {
		    String statoPost = getForm().getDettaglioAggMetaFallito()
			    .getTi_stato_ses_upd_ko().parse();
		    monitAggMetaEjb
			    .salvaStatoAggiornamentoFallito(
				    getForm().getDettaglioAggMetaFallito()
					    .getId_ses_upd_unita_doc_ko().parse(),
				    statoPre, statoPost);
		    getMessageBox().addInfo(
			    "Salvataggio modifica stato aggiornamento fallito eseguito con successo!");
    ...
    private void calcolaRiepilogoAggMetaGlobale(BigDecimal idAmbiente, BigDecimal idEnte,
	    BigDecimal idStruttura, BigDecimal aaKeyUnitaDoc, BigDecimal aaKeyUnitaDocDa,
	    BigDecimal aaKeyUnitaDocA, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
	    BigDecimal idTipoDocPrinc) throws EMFError {
	BaseRow rowBean = monitAggMetaEjb.calcolaRiepilogoAggMeta(
		new BigDecimal(getUser().getIdUtente()), idAmbiente, idEnte, idStruttura,
    ...
    private void calcolaRiepilogoAggMetaDataCorrente(BigDecimal idAmbiente, BigDecimal idEnte,
	    BigDecimal idStruttura, BigDecimal aaKeyUnitaDoc, BigDecimal aaKeyUnitaDocDa,
	    BigDecimal aaKeyUnitaDocA, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
	    BigDecimal idTipoDocPrinc) throws EMFError {
	BaseRow rowBean = monitAggMetaEjb.calcolaRiepilogoAggMetaDataCorrente(
		new BigDecimal(getUser().getIdUtente()), idAmbiente, idEnte, idStruttura,
		aaKeyUnitaDoc, aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc, idRegistroUnitaDoc,
		idTipoDocPrinc);
	getForm().getAggiornamentiMetadati().getNi_upd_corr()
		.setValue("" + rowBean.getBigDecimal("ni_upd_corr"));
    ...
    private void calcolaRiepilogoAggMetaFalliti(BigDecimal idAmbiente, BigDecimal idEnte,
	    BigDecimal idStruttura, BigDecimal aaKeyUnitaDoc, BigDecimal aaKeyUnitaDocDa,
	    BigDecimal aaKeyUnitaDocA, BigDecimal idTipoUnitaDoc, BigDecimal idRegistroUnitaDoc,
	    BigDecimal idTipoDocPrinc) throws EMFError {
	BaseRow rowBean = monitAggMetaEjb.calcolaRiepilogoAggMetaFalliti(
		new BigDecimal(getUser().getIdUtente()), idAmbiente, idEnte, idStruttura,
		aaKeyUnitaDoc, aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc, idRegistroUnitaDoc,
		idTipoDocPrinc);
	...
	if (getForm().getFiltriRicercaMonitoraggioAggMeta().postAndValidate(getRequest(),
	    ...
	    if (!getMessageBox().hasError()) {
		calcolaRiepilogoAggMetaDataCorrente(idAmbiente, idEnte, idStruttura, aaKeyUnitaDoc,
			aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc, idRegistroUnitaDoc,
			idTipoDocPrinc);
	    }
	}
	...
	if (getForm().getFiltriRicercaMonitoraggioAggMeta().postAndValidate(getRequest(),
	    ...
	    if (!getMessageBox().hasError()) {
		calcolaRiepilogoAggMetaFalliti(idAmbiente, idEnte, idStruttura, aaKeyUnitaDoc,
			aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc, idRegistroUnitaDoc,
			idTipoDocPrinc);
	    }
	}
	...
	if (getForm().getFiltriRicercaMonitoraggioAggMeta().postAndValidate(getRequest(),
	    ...
	    if (!getMessageBox().hasError()) {
		calcolaRiepilogoUnitaDocAggMetaFalliti(idAmbiente, idEnte, idStruttura,
			aaKeyUnitaDoc, aaKeyUnitaDocDa, aaKeyUnitaDocA, idTipoUnitaDoc,
	...
	getForm().getAggMetaFallitiTabs().setCurrentTab(
		getForm().getAggMetaFallitiTabs().getIndiceSipAggiornamentoFalliti());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META_FALLITO);
    }
	...
	getForm().getAggMetaFallitiTabs().setCurrentTab(
		getForm().getAggMetaFallitiTabs().getEsitoNegativoVersamentoFalliti());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META_FALLITO);
    }
	...
	getForm().getUnitaDocAggMetaFallitiTabs().setCurrentTab(getForm()
		.getUnitaDocAggMetaFallitiTabs().getListaUlterioriErroriUnitaDocAggFalliti());
	forwardToPublisher(Application.Publisher.DETTAGLIO_UNITA_DOC_AGG_META_FALLITO);
    }
    // end MEV#22438
	...
	downloadFileCommon(getForm().getDettaglioAggMetaFallito().getBl_xml_rich().getValue(),
		getForm().getDettaglioAggMetaFallito().getBl_xml_risp().getValue(),
		"AggiornamentoFallito_",
		getForm().getDettaglioAggMetaFallito().getId_ses_upd_unita_doc_ko().parse(),
		"IndiceSIP", "EsitoNegativo");
    }
	...
	downloadFileCommon(
		getForm().getDettaglioUnitaDocAggMetaFallito().getBl_xml_rich_last().getValue(),
		getForm().getDettaglioUnitaDocAggMetaFallito().getBl_xml_risp_last().getValue(),
		"AggiornamentoFallito_",
		getForm().getDettaglioUnitaDocAggMetaFallito().getId_ses_upd_ud_ko_last().parse(),
		"IndiceSIP", "EsitoNegativo");
    }
    // end MEV#22438
	...
	downloadFileCommon(getForm().getDettaglioAggMetaErrato().getBl_xml_rich().getValue(),
		getForm().getDettaglioAggMetaErrato().getBl_xml_risp().getValue(),
		"AggiornamentoErrato_",
		getForm().getDettaglioAggMetaErrato().getId_ses_upd_unita_doc_err().parse(),
		"IndiceSIP", "EsitoNegativo");
    }

    private void downloadFileCommon(String xmlRich, String xmlRisp, String nomeFileZip,
	    BigDecimal idSessione, String nomeFileXmlRich, String nomeFileXmlRisp) throws EMFError {
	...
	try {
	    ...
	    zipDaScaricare = new File(System.getProperty("java.io.tmpdir"),
		    nomeFileZip + strIdSessione + ".zip");
	    ...
	    // MAC 37610: Rimossa la parte di codice che chiudeva i flussi nel blocco finally poiché
	    // il
	...
	if (path != null && filename != null) {
		...
		getResponse().setContentType(
			StringUtils.isBlank(contentType) ? WebConstants.MIME_TYPE_ZIP
				: contentType);
	...
	Date[] dateValidate = validator.validaDate(
		getForm().getFiltriAggMetaErrati().getDt_ini_ses_da().parse(),
	...
	if (!getMessageBox().hasError()) {
	    Set<String> tiStatoSes = getForm().getFiltriAggMetaErrati().getTi_stato_ses()
		    .getValues();
	    BigDecimal idClasseErr = getForm().getFiltriAggMetaErrati().getId_classe_err_sacer()
		    .parse();
	    BigDecimal idErr = getForm().getFiltriAggMetaErrati().getId_err_sacer().parse();

	    MonVLisUpdUdErrTableBean t = monitAggMetaEjb.ricercaAggMetaErrati(dateValidate,
		    tiStatoSes, idClasseErr, idErr);
	...
	getForm().getAggMetaTabs()
		.setCurrentTab(getForm().getAggMetaTabs().getInformazioniPrincipaliAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaTabs()
		.setCurrentTab(getForm().getAggMetaTabs().getInformazioniVersamentoAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaTabs()
		.setCurrentTab(getForm().getAggMetaTabs().getIndiceSipAggiornamentoAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaTabs()
		.setCurrentTab(getForm().getAggMetaTabs().getRapportoVersamentoAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }

    @Override
    public void tabListaDocAggiornatiAggMetaOnClick() throws EMFError {
	getForm().getAggMetaListsTabs()
		.setCurrentTab(getForm().getAggMetaListsTabs().getListaDocAggiornatiAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaListsTabs()
		.setCurrentTab(getForm().getAggMetaListsTabs().getListaCompAggiornatiAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaListsTabs()
		.setCurrentTab(getForm().getAggMetaListsTabs().getListaAggMetaRisoltiAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	getForm().getAggMetaListsTabs()
		.setCurrentTab(getForm().getAggMetaListsTabs().getListaWarnRilevatiAggMeta());
	forwardToPublisher(Application.Publisher.DETTAGLIO_AGG_META);
    }
	...
	String nomeCartella = "SIP_AGGIORNAMENTO_" + registro + "-" + anno + "-" + numero + "-"
		+ prog;

	downloadFileCommon(getForm().getDettaglioAggMeta().getBl_xml_rich().getValue(),
		getForm().getDettaglioAggMeta().getBl_xml_risp().getValue(), nomeCartella, null,
		"IndiceSIP", "RdV");
    }
	...
	if (stato != null && stato.size() == 1 && stato.contains("NON_VERIFICATO")) {
		...
		for (String indexString : indiceScelti) {
			...
			BigDecimal idAgg = getForm().getAggMetaFallitiList().getTable()
				.getRow(index).getBigDecimal("id_ses_upd_unita_doc_ko");
		...
		try {
		    monitAggMetaEjb.salvaStatoAggiornamentoFallito(id, "NON_VERIFICATO",
			    "VERIFICATO");
		    aggiornati++;
		} catch (ParerInternalError ex) {
		    getMessageBox().addError(ex.getMessage());
		    log.error("Errore nel salvataggio dello stato dell'aggiornamento fallito", ex);
		} catch (Exception ex) {
		    getMessageBox().addError(
			    "Errore nel salvataggio dello stato dell'aggiornamento fallito con identificativo "
				    + id);
		    log.error(
			    "Errore nel salvataggio dello stato dell'aggiornamento fallito con identificativo"
				    + id,
	    ...
	    if (aggiornati > 0) {
		getMessageBox().addInfo(
			"Verifica aggiornamenti metadati falliti eseguita con successo per "
				+ aggiornati + " record");
	    } else if (!getMessageBox().hasError()) {
		getMessageBox().addInfo(
			"Nessun record selezionato: non è stato eseguito alcun aggiornamento");
	    }
	    internalRicercaAggMetaFalliti();
	}
	forwardToPublisher(getLastPublisher());
	...
	if (stato != null && stato.size() == 1 && stato.contains("VERIFICATO")) {
	    ...
	    for (BigDecimal id : idAggDaModificare) {
		try {
		    monitAggMetaEjb.salvaStatoAggiornamentoFallito(id, "VERIFICATO",
			    "NON_RISOLUBILE");
		    aggiornati++;
		} catch (ParerInternalError ex) {
		    getMessageBox().addError(ex.getMessage());
	...
	getForm().getFiltriAggMeta().getId_ambiente().setDecodeMap(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ambiente().getDecodeMap());
	getForm().getFiltriAggMeta().getId_ambiente().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ambiente().getValue());
	getForm().getFiltriAggMeta().getId_ente().setDecodeMap(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ente().getDecodeMap());
	getForm().getFiltriAggMeta().getId_ente()
		.setValue(getForm().getFiltriRicercaMonitoraggioAggMeta().getId_ente().getValue());
	getForm().getFiltriAggMeta().getId_strut().setDecodeMap(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_strut().getDecodeMap());
	getForm().getFiltriAggMeta().getId_strut()
		.setValue(getForm().getFiltriRicercaMonitoraggioAggMeta().getId_strut().getValue());
	getForm().getFiltriAggMeta().getId_tipo_unita_doc().setDecodeMap(getForm()
		.getFiltriRicercaMonitoraggioAggMeta().getId_tipo_unita_doc().getDecodeMap());
	getForm().getFiltriAggMeta().getId_tipo_unita_doc().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_tipo_unita_doc().getValue());
	getForm().getFiltriAggMeta().getId_registro_unita_doc().setDecodeMap(getForm()
		.getFiltriRicercaMonitoraggioAggMeta().getId_registro_unita_doc().getDecodeMap());
	getForm().getFiltriAggMeta().getId_registro_unita_doc().setValue(getForm()
		.getFiltriRicercaMonitoraggioAggMeta().getId_registro_unita_doc().getValue());
	getForm().getFiltriAggMeta().getId_tipo_doc().setDecodeMap(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_tipo_doc().getDecodeMap());
	getForm().getFiltriAggMeta().getId_tipo_doc().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getId_tipo_doc().getValue());
	getForm().getFiltriAggMeta().getAa_key_unita_doc().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getAa_key_unita_doc().getValue());
	getForm().getFiltriAggMeta().getAa_key_unita_doc_da().setValue(getForm()
		.getFiltriRicercaMonitoraggioAggMeta().getAa_key_unita_doc_da().getValue());
	getForm().getFiltriAggMeta().getAa_key_unita_doc_a().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getAa_key_unita_doc_a().getValue());
	getForm().getFiltriAggMeta().getTi_stato_upd_elenco_vers()
		.setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_stato_upd_elenco_vers",
			StatoGenerazioneIndiceAip.values()));
	getForm().getFiltriAggMeta().getFl_ses_upd_ko_risolti()
	...
	case TI_STATO_IN_ATTESA_SCHED:
	    getForm().getFiltriAggMeta().getTi_stato_upd_elenco_vers().setValues(new String[] {
		    "IN_ATTESA_SCHED" });
	    break;
	...
	getForm().getFiltriAggMeta().getAa_key_unita_doc_a().setValue(
		getForm().getFiltriRicercaMonitoraggioAggMeta().getAa_key_unita_doc_a().getValue());
	getForm().getFiltriAggMeta().getTi_stato_ses_upd_ko().setDecodeMap(
		ComboGetter.getMappaSortedGenericEnum("ti_stato_ses_upd_ko", Stato.values()));
	getForm().getFiltriAggMeta().getCd_classe_err().setDecodeMap(
		monitAggMetaEjb.getIdClasseErrSacerDecodeMap(Arrays.asList(new String[] {
			MonitoraggioAggMetaEjb.tipoUsoClasseErrore.VERS_UNITA_DOC.name(),
			MonitoraggioAggMetaEjb.tipoUsoClasseErrore.GENERICO.name() })));
	...
	MonitoraggioAggMetaValidator validator = new MonitoraggioAggMetaValidator(getMessageBox());
	Date[] dateValidate = validator.validaDate(
		getForm().getFiltriAggMeta().getDt_ini_ses_da().parse(),
	...
	ChiaveBean bean = validator.validaChiavi(
		getForm().getChiaveAggMetaSection().getDescription(),
	...
	if (!getMessageBox().hasError()) {
	    ...
	    BigDecimal idRegistroUnitadoc = getForm().getFiltriAggMeta().getId_registro_unita_doc()
		    .parse();
	    BigDecimal idTipodoc = getForm().getFiltriAggMeta().getId_tipo_doc().parse();

	    Set<String> statoIndiceAip = getForm().getFiltriAggMeta().getTi_stato_upd_elenco_vers()
		    .getValues();
	    String flSesUpdKoRisolti = getForm().getFiltriAggMeta().getFl_ses_upd_ko_risolti()
		    .getValue();
	    BaseTable t = monitAggMetaEjb.ricercaAggMetaPerMonitoraggio(idUser, idAmbiente, idEnte,
		    ...
		    bean.isSingleValue() ? bean.getNumero() : bean.getNumeroDa(), bean.getNumeroA(),
	...
	if (!getMessageBox().hasError()) {
	    ...
	    BigDecimal idRegistroUnitadoc = getForm().getFiltriAggMeta().getId_registro_unita_doc()
		    .parse();
	    BigDecimal idTipodoc = getForm().getFiltriAggMeta().getId_tipo_doc().parse();
	    Set<String> statoSessione = getForm().getFiltriAggMeta().getTi_stato_ses_upd_ko()
		    .getValues();
	    ...
	    BaseTable t = null;
	    // Ho dovuto differenziare i metodi altrimenti il paginatore faceva casino con le
	    // count(*) su valori
	    // DISTINCT
	    if (idClasseErr != null || idErr != null) {
		t = monitAggMetaEjb.ricercaAggMetaFallitiPerMonitoraggio(idUser, idAmbiente, idEnte,
			idStruttura, idTipoUnitadoc, idRegistroUnitadoc, idTipodoc, dateValidate,
			bean.isSingleValue() ? bean.getAnno() : bean.getAnnoDa(), bean.getAnnoA(),
			bean.isSingleValue() ? bean.getNumero() : bean.getNumeroDa(),
			bean.getNumeroA(), statoSessione, idClasseErr, idErr);
	    } else {
		t = monitAggMetaEjb.ricercaAggMetaFallitiPerMonitoraggioErr(idUser, idAmbiente,
			idEnte, idStruttura, idTipoUnitadoc, idRegistroUnitadoc, idTipodoc,
			dateValidate, bean.isSingleValue() ? bean.getAnno() : bean.getAnnoDa(),
			bean.getAnnoA(),
			bean.isSingleValue() ? bean.getNumero() : bean.getNumeroDa(),
			bean.getNumeroA(), statoSessione, idClasseErr, idErr);
	    }
	    ...
	    getForm().getAggMetaFallitiList().getTable()
		    .setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
		...
		if (statoSessione.contains("NON_VERIFICATO")) {
		    ...
		    getForm().getFiltriAggMeta().getAssegnaNonRisolubAggMetaFallitiButton()
			    .setEditMode();
		    getForm().getFiltriAggMeta().getAssegnaNonVerifAggMetaFallitiButton()
			    .setEditMode();
		} else if (statoSessione.contains("NON_RISOLUBILE")) {
		    getForm().getFiltriAggMeta().getAssegnaVerifAggMetaFallitiButton()
			    .setEditMode();
		}
	    }
	}
	...
	if (!getMessageBox().hasError()) {
	    ...
	    BigDecimal idClasseErr = getForm().getFiltriAggMeta().getCd_classe_err().parse();
	    BigDecimal idErr = getForm().getFiltriAggMeta().getCd_err().parse();
	    BaseTable t = monitAggMetaEjb.ricercaUnitaDocAggMetaFallitiPerMonitoraggio(idUser,
		    idAmbiente, idEnte, idStruttura, idTipoUnitadoc, idRegistroUnitadoc, idTipodoc,
		    dateValidate, bean.isSingleValue() ? bean.getAnno() : bean.getAnnoDa(),
	    ...
	    getForm().getUnitaDocAggMetaFallitiList().getTable()
		    .setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
	...
	getForm().getFiltriAggMeta().getCd_err()
		.setDecodeMap(monitAggMetaEjb.getErrSacerByIdClasseDecodeMap(idClasse));
	return getForm().getFiltriAggMeta().asJSON();
    }
	...
	if (getForm() instanceof MonitoraggioAggMetaForm) {
	    getForm().getFiltriRicercaMonitoraggioAggMeta().getGeneraMonitoraggioAggMetaButton()
		    .setEditMode();
	...
	try {
	    ...
	    getMessageBox()
		    .addError("Errore durante la modifica dell'aggiornamento metadati fallito");
	    forwardToPublisher(getLastPublisher());
	}
    }

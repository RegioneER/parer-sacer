...
public class ElenchiVersamentoAction extends ElenchiVersamentoAbstractAction {
    ...
    private static Logger logger = LoggerFactory.getLogger(ElenchiVersamentoAction.class.getName());
    ...
    private void initComboFiltriComponenti(BigDecimal idStrut) {
        ...
        // Imposto i valori della combo TIPO STRUTTURA DOCUMENTO ricavati dalla tabella DEC_TIPO_STRUT_DOC
        DecTipoStrutDocTableBean tmpTableBeanTipoStrutDoc = tipoStrutDocEjb.getDecTipoStrutDocTableBean(idStrut, false);
        ...
        DecRegistroUnitaDocTableBean tmpTableBeanReg = registroEjb.getRegistriUnitaDocAbilitati(getIdUtenteCorrente(),
                idStrut);
        ...
        getForm().getComponentiFiltri().getTi_esito_contr_conforme().setDecodeMap(
                ComboGetter.getMappaSortedGenericEnum("stato", VolumeEnums.ControlloConformitaEnum.values()));
        getForm().getComponentiFiltri().getTi_esito_verif_firme_vers().setDecodeMap(
    ...
    private void initBottoniPaginaDettaglioElencoVersamento(BigDecimal idElencoVers, String tiStatoElenco) {
        getForm().getDettaglioElenchiVersamentoButtonList().setViewMode();
        getForm().getDettaglioElenchiVersamentoButtonList().getListaOperazioniElencoButton().setHidden(false);
        getForm().getDettaglioElenchiVersamentoButtonList().getListaOperazioniElencoButton().setEditMode();
        ...
        if (evEjb.areUdDocDeletables(idElencoVers)) {
            getForm().getListaComponentiButtonList().getEliminaAppartenenzaUdDocDaElenco().setHidden(false);
            getForm().getListaComponentiButtonList().getEliminaAppartenenzaUdDocDaElenco().setEditMode();
        }
        /* Se upd sono eliminabili dall'elenco di versamento, mostro il bottone */
        if (evEjb.areUpdDeletables(idElencoVers)) {
            getForm().getListaAggiornamentiButtonList().getEliminaAppartenenzaUpdDaElenco().setHidden(false);
            getForm().getListaAggiornamentiButtonList().getEliminaAppartenenzaUpdDaElenco().setEditMode();
        }
        /* Se l'elenco è chiudibile, mostro il bottone */
        if (evEjb.isElencoClosable(idElencoVers)) {
            getForm().getDettaglioElenchiVersamentoButtonList().getChiudiElencoButton().setHidden(false);
            getForm().getDettaglioElenchiVersamentoButtonList().getChiudiElencoButton().setEditMode();
        }
        if (tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.CHIUSO.name())
                ...
                || tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.FIRME_VERIFICATE_DT_VERS.name())
                ...
                || tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.ELENCO_INDICI_AIP_CREATO.name())
                || tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.ELENCO_INDICI_AIP_FIRMA_IN_CORSO.name())
                || tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.ELENCO_INDICI_AIP_FIRMATO.name())
                || tiStatoElenco.equals(ElencoEnums.ElencoStatusEnum.ELENCO_INDICI_AIP_ERR_MARCA.name())
    ...
    private boolean esisteIndiceVersamento(BigDecimal idElencoVers) {
        return evEjb.retrieveFileIndiceElenco(idElencoVers.longValue(), ElencoEnums.FileTypeEnum.INDICE.name()) != null;
    }
    ...
    public void loadListaElenchiVersamentoDaFirmare() throws EMFError {
        ...
        if (idStrut != null && cleanFilter) {
            ...
            if (cleanList) {
                ...
                elenchiValidator
                        .validaTipoValidazione(getForm().getFiltriElenchiDaFirmare().getTi_valid_elenco().parse());
                // Valido i filtri data creazione elenco da - a restituendo le date comprensive di orario
                Date[] dateCreazioneElencoValidate = elenchiValidator.validaDate(
                        ...
                        getForm().getFiltriElenchiDaFirmare().getOre_dt_creazione_elenco_da().parse(),
                        getForm().getFiltriElenchiDaFirmare().getMinuti_dt_creazione_elenco_da().parse(),
                        getForm().getFiltriElenchiDaFirmare().getDt_creazione_elenco_a().parse(),
                        getForm().getFiltriElenchiDaFirmare().getOre_dt_creazione_elenco_a().parse(),
                        getForm().getFiltriElenchiDaFirmare().getMinuti_dt_creazione_elenco_a().parse(),
                        getForm().getFiltriElenchiDaFirmare().getDt_creazione_elenco_da().getHtmlDescription(),
                        getForm().getFiltriElenchiDaFirmare().getDt_creazione_elenco_a().getHtmlDescription());
                if (!getMessageBox().hasError()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (!getForm().getFiltriElenchiDaFirmare().getTi_valid_elenco().parse().contains(tiValidElenco)
                                || (getForm().getFiltriElenchiDaFirmare().getTi_mod_valid_elenco().parse() != null
                                        && !getForm().getFiltriElenchiDaFirmare().getTi_mod_valid_elenco().parse()
                    ...
                    getForm().getElenchiVersamentoDaFirmareList().getTable().addSortingRule(
                            getForm().getElenchiVersamentoDaFirmareList().getDt_creazione_elenco().getName(),
                            SortingRule.ASC);
    ...
    private void initComboRicercaElenchi() throws EMFError {
        ...
        getForm().getFiltriElenchiVersamento().getFl_elenco_firmato()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
        getForm().getFiltriElenchiVersamento().getFl_elenco_fisc().setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
     ...
     * conseguenza le combo Registro unità documentaria e Criterio di Raggruppamento
     *
     * @param idStrut
     *            id struttura
     * @param sezione
    ...
    public void checkUniqueStrutInCombo(BigDecimal idStrut, Enum sezione) throws EMFError {
        if (idStrut != null) {
            // Imposto i valori della mappa CRITERIO_RAGGRUPPAMENTO ricavati dalla tabella DEC_CRITERIO_RAGGR
            DecCriterioRaggrTableBean tmpTableBeanCriteri = criteriRaggruppamentoEjb.getDecCriterioRaggrTableBean(null,
                    null, idStrut, null);
            DecodeMap mappaCriteri = new DecodeMap();
            mappaCriteri.populatedMap(tmpTableBeanCriteri, "nm_criterio_raggr", "nm_criterio_raggr");

            // Setto i valori della mappa TIPO REGISTRO ricavati dalla tabella DEC_REGISTRO_UNITA_DOC
    ...
    public void loadDettaglio() throws EMFError {
        ...
        if (lista != null && azione != null && !azione.equals(NE_DETTAGLIO_INSERT)) {
            ...
            if (lista.equals(getForm().getElenchiVersamentoList().getName())) {
                ...
                BigDecimal idElencoVers = getForm().getElenchiVersamentoDaFirmareList().getTable().getCurrentRow()
                ...
                BigDecimal idElencoVers = getForm().getElenchiIndiciAipDaFirmareList().getTable().getCurrentRow()
                        .getBigDecimal(ElvVLisElencoVersStatoTableDescriptor.COL_ID_ELENCO_VERS);
                dettaglioElencoVersamento(idElencoVers);
            } else if (lista.equals(getForm().getElenchiVersamentoSelezionatiList().getName())) {
                BigDecimal idElencoVers = getForm().getElenchiVersamentoSelezionatiList().getTable().getCurrentRow()
                        .getBigDecimal(ElvVLisElencoVersStatoTableDescriptor.COL_ID_ELENCO_VERS);
                dettaglioElencoVersamento(idElencoVers);
            } else if (lista.equals(getForm().getElenchiIndiciAipSelezionatiList().getName())) {
                BigDecimal idElencoVers = getForm().getElenchiIndiciAipSelezionatiList().getTable().getCurrentRow()
                        .getBigDecimal(ElvVLisElencoVersStatoTableDescriptor.COL_ID_ELENCO_VERS);
                dettaglioElencoVersamento(idElencoVers);
            }
        }
    ...
    public void dettaglioElencoVersamento(BigDecimal idElencoVers) throws EMFError {
        ...
        getForm().getElenchiVersamentoDetail().getTi_gest_elenco().setDecodeMap(
                ComboGetter.getMappaSortedGenericEnum("ti_gest_elenco", ElencoEnums.GestioneElencoEnum.values()));
        getForm().getElenchiVersamentoDetail().getTi_valid_elenco().setDecodeMap(
                ComboGetter.getMappaSortedGenericEnum("ti_valid_elenco", ElvElencoVer.TiValidElenco.values()));
        getForm().getElenchiVersamentoDetail().getTi_mod_valid_elenco().setDecodeMap(
                ComboGetter.getMappaSortedGenericEnum("ti_mod_valid_elenco", ElvElencoVer.TiModValidElenco.values()));
        ...
        getForm().getDettaglioElencoTabs().setCurrentTab(getForm().getDettaglioElencoTabs().getDettaglioElencoTab());
        ...
        if ((getTableName() != null && (getTableName().equals(getForm().getElenchiVersamentoList().getName())
            ...
            ElvVLisUpdUdTableBean listaUpd = aggiornamentiHelper.getElvVLisUpdUdViewBean(idElencoVers,
                    new ElenchiVersamentoForm.ComponentiFiltri());
        ...
        /* Inizializzo le liste fittizie nel caso si voglia visualizzare unità documentarie o documenti */
    ...
    public void dettaglioOnClick() throws EMFError {
        ...
        if (!azione.equals(ListAction.NE_DETTAGLIO_DELETE)) {
            ...
            Set<Object> registriAbilitatiSet = getForm().getComponentiFiltri().getCd_registro_key_unita_doc()
                    .getDecodeMap().keySet();
            Set<Object> tipiUdAbilitateSet = DecodeMap.Factory
                    .newInstance(tipoUnitaDocEjb.getTipiUnitaDocAbilitati(getIdUtenteCorrente(), idStruttura),
                            "id_tipo_unita_doc", "nm_tipo_unita_doc")
                    .keySet();
            Set<Object> tipiDocAbilitatiSet = DecodeMap.Factory
                    .newInstance(tipoDocumentoEjb.getTipiDocAbilitati(getIdUtenteCorrente(), idStruttura),
                            "id_tipo_doc", "nm_tipo_doc")
                    .keySet();
            Set<Object> subStrutAbilitateSet = DecodeMap.Factory
                    .newInstance(subStrutEjb.getOrgSubStrutTableBeanAbilitate(getIdUtenteCorrente(), idStruttura),
            ...
            if (lista.equals(getForm().getElenchiVersamentoList().getName())
            ...
            } /* Dettaglio COMPONENTE */ else if (lista.equals(getForm().getComponentiList().getName())) {
                BigDecimal idUnitaDoc = null;
                /* Verifico se l'utente può accedere al dettaglio componente */
                ElvVListaCompElvRowBean rigaComp = (ElvVListaCompElvRowBean) getForm().getComponentiList().getTable()
                        .getCurrentRow();
                if (registriAbilitatiSet.contains(rigaComp.getCdRegistroKeyUnitaDoc())
                    ...
                    getMessageBox()
                            .addError("Utente non abilitato ad accedere al dettaglio del componente selezionato");
                    ...
                    if (!aroVRicUnitaDocRowBean.getTiStatoConservazione()
                        ...
                        redirectToAction(Application.Actions.COMPONENTI,
                                "?operation=listNavigationOnClick&navigationEvent=" + ListAction.NE_DETTAGLIO_VIEW
                                        + "&table=" + ComponentiForm.ComponentiList.NAME + "&riga="
                ...
                ElvVListaCompElvRowBean rigaComp = ((ElvVListaCompElvRowBean) getForm().getComponentiList().getTable()
                        .getRow(riga));
                if (rigaComp != null && registriAbilitatiSet.contains(rigaComp.getCdRegistroKeyUnitaDoc())
                        && tipiUdAbilitateSet.contains(rigaComp.getIdTipoUnitaDoc())
                        && tipiDocAbilitatiSet.contains(rigaComp.getIdTipoDoc())
                        && subStrutAbilitateSet.contains(rigaComp.getIdSubStrut())) {
                    idUnitaDoc = rigaComp.getIdUnitaDoc();
                ...
                BigDecimal idUnitaDoc = null;
                /* Verifico se l'utente può accedere al dettaglio unità documentaria */
                ElvVLisUpdUdRowBean rigaUpd = ((ElvVLisUpdUdRowBean) getForm().getAggiornamentiList().getTable()
                        .getRow(riga));
                if (rigaUpd != null && registriAbilitatiSet.contains(rigaUpd.getCdRegistroKeyUnitaDoc())) {
            ...
            } /* Dettaglio DOCUMENTO */ else if (lista.equals(getForm().getDocumentiList().getName())) {
                UnitaDocumentarieForm form = new UnitaDocumentarieForm();
                ...
                if (registriAbilitatiSet.contains(rigaComp.getCdRegistroKeyUnitaDoc())
                    ...
                    getMessageBox().addError("Utente non abilitato ad accedere al dettaglio del documento selezionato");
                    forwardToPublisher(getLastPublisher());
                }
                if (idUnitaDoc != null) {
                    AroVRicUnitaDocRowBean aroVRicUnitaDocRowBean = udHelper.getAroVRicUnitaDocRowBean(idUnitaDoc, null,
                    ...
                    if (!aroVRicUnitaDocRowBean.getTiStatoConservazione()
                        ...
                        redirectToAction(Application.Actions.UNITA_DOCUMENTARIE,
                                "?operation=listNavigationOnClick&navigationEvent=" + ListAction.NE_DETTAGLIO_VIEW
                                        + "&table=" + UnitaDocumentarieForm.DocumentiUDList.NAME + "&riga=0",
                                form);
                    } else {
                        getMessageBox().addError(
                                "Operazione non possibile in quanto l'unità documentaria ha stato di conservazione = ANNULLATA");
    ...
    public void updateElenchiVersamentoList() {
        ...
        if (statoElenco.equals(ElencoStatusEnum.CHIUSO.name()) || statoElenco.equals(ElencoStatusEnum.VALIDATO.name())
    ...
    public void deleteElenchiVersamentoList() throws EMFError {
        ...
        if (!evEjb.isElencoDeletable(idElencoVers)) {
            getMessageBox().addError("L'elenco di versamento è diverso da APERTO e da DA_CHIUDERE e da CHIUSO");
    ...
    public void deleteElenchiVersamentoDaFirmareList() throws EMFError {
        ...
        try {
            ...
            getMessageBox().addError("Errore durante l'eliminazione dell'elenco di versamento da firmare");
        } finally {
            String lastPublisher = getLastPublisher();
            if (Application.Publisher.ELENCHI_VERSAMENTO_DETAIL.equals(lastPublisher)) {
                goBack();
    ...
    public void saveDettaglio() throws EMFError {
        ...
        String rowTiValidElenco = getForm().getElenchiVersamentoDetail().getTi_valid_elenco().parse();
        String rowTiModValidElenco = getForm().getElenchiVersamentoDetail().getTi_mod_valid_elenco().parse();
        ...
        String ntIndiceElenco = getForm().getElenchiVersamentoDetail().getNt_indice_elenco().parse();
        String ntElencoChiuso = getForm().getElenchiVersamentoDetail().getNt_elenco_chiuso().parse();
        ...
        String tiModValidElenco = getForm().getElenchiVersamentoDetail().getTi_mod_valid_elenco().parse();
        ...
        if (getForm().getElenchiVersamentoDetail().validate(getMessageBox())) {
            if (!getMessageBox().hasError()) {
                try {
                    evEjb.saveNomeDesNote(getIdUtenteCorrente(), idElencoVers, nmElenco, dsElenco, ntIndiceElenco,
                            ntElencoChiuso, operList, tiGestElenco, tiValidElenco, tiModValidElenco);
                    ...
                    // MAC#21555
                    /*
                     * Ricavo la lista delle strutture da cercare in base a come sono state impostate nei filtri di
                     * ricerca le combo Ambiente/Ente/Struttura
                     */
                    // end MAC#21555
    ...
    public void undoDettaglio() throws EMFError {
        dettaglioElencoVersamento(getForm().getElenchiVersamentoDetail().getId_elenco_vers().parse());
        setElencoVersamentoListAndDetailViewMode();
        forwardToPublisher(Application.Publisher.ELENCHI_VERSAMENTO_DETAIL);
    }
     ...
     *
     * @param publisherName
     *            la pagina cui deve andare a seguito della goBack()
     */
    @Override
    public void reloadAfterGoBack(String publisherName) {
        if (publisherName != null) {
            String lista = getTableName();
            /*
             * Se tornando indietro devo andare nella pagina di Ricerca Elenchi di Versamento rilancio la ricerca in
             * quanto posso aver modificato l'elenco esaminato
             */
            if (publisherName.equals(Application.Publisher.ELENCHI_VERSAMENTO_RICERCA)
                    && (lista != null && lista.equals(getForm().getElenchiVersamentoList().getName()))
                    || (getRequest().getParameter("mainNavTable") != null && getRequest().getParameter("mainNavTable")
    ...
    public JSONObject triggerComponentiFiltriNm_tipo_strut_docOnTrigger() throws EMFError {
        ...
        if (idStrutDoc != null) {
            ...
            DecTipoCompDocTableBean decTipoCompDocTableBean = tipoStrutDocEjb.getDecTipoCompDocTableBean(idStrutDoc,
                    false);
            mappaTipoCompDoc.populatedMap(decTipoCompDocTableBean, "id_tipo_comp_doc", "nm_tipo_comp_doc");
        }
    ...
    public void ricercaElenchiButton() throws EMFError {
        ElenchiVersamentoForm.FiltriElenchiVersamento filtriElenchiVersamento = getForm().getFiltriElenchiVersamento();
        ...
        if (filtriElenchiVersamento.validate(getMessageBox())) {
            ...
            validator.validateDataVolumi(filtriElenchiVersamento.getDt_creazione_elenco_da().parse(),
                    filtriElenchiVersamento.getDt_creazione_elenco_a().parse());
            validator.validateDataVolumi(filtriElenchiVersamento.getDt_validazione_elenco_da().parse(),
                    filtriElenchiVersamento.getDt_validazione_elenco_a().parse());
            validator.validateDataVolumi(filtriElenchiVersamento.getDt_creazione_elenco_ix_aip_da().parse(),
                    filtriElenchiVersamento.getDt_creazione_elenco_ix_aip_a().parse());
            validator.validateDataVolumi(filtriElenchiVersamento.getDt_firma_elenco_ix_aip_da().parse(),
            ...
            validator.validateCodaJMS(chiavi, filtriElenchiVersamento.getHh_stato_elenco_in_coda_jms().parse(),
                    filtriElenchiVersamento.getTi_stato_elenco().parse());
            /* Se la validazione più approfondita non ha riportato errori */
            if (!getMessageBox().hasError()) {
                // MAC#21555
                /*
                 * Ricavo la lista delle strutture da cercare in base a come sono state impostate nei filtri di ricerca
                 * le combo Ambiente/Ente/Struttura
                 */
                // end MAC#21555

                /* Setto i filtri di chiavi unità documentaria impostando gli eventuali valori di default */
                if (chiavi != null && chiavi.length == 5) {
                    filtriElenchiVersamento.getAa_key_unita_doc_da()
                            .setValue(chiavi[1] != null ? ((BigDecimal) chiavi[1]).toString() : null);
                    filtriElenchiVersamento.getAa_key_unita_doc_a()
                ...
                if (chiavi != null) {
                    ...
                    if (!tiStatoElenco.isEmpty()
                        ...
                        getForm().getElenchiVersamentoList().setTable(evEjb.getElvVRicElencoVersByStatoTableBean(
                                getUser().getIdUtente(), filtriElenchiVersamento));
                    } else {
                        getForm().getElenchiVersamentoList().setTable(
                                evEjb.getElvVRicElencoVersTableBean(getUser().getIdUtente(), filtriElenchiVersamento));
                    }
                }
                ...
                /*
                 * Imposto la sezione del filtro chiave unità documentaria aperto dopo la ricerca se ho inserito un
                 * valore nel campo registro
                 */
                if (!getForm().getFiltriElenchiVersamento().getCd_registro_key_unita_doc().getValue().equals("")) {
                    getForm().getChiaveSection().setLoadOpened(true);
                }
            }
        }
    ...
    public void ricercaComp() throws EMFError {
        ...
        if (compfiltri.validate(getMessageBox())) {
            ...
            if (!getMessageBox().hasError()) {
                ...
                getForm().getComponentiList().getTable().first();
                // Carico la lista degli Aggiornamenti
                getForm().getAggiornamentiList().setTable(aggiornamentiHelper.getElvVLisUpdUdViewBean(
                        getForm().getElenchiVersamentoDetail().getId_elenco_vers().parse(), compfiltri));
                getForm().getAggiornamentiList().getTable().setPageSize(10);
                // Workaround in modo che la lista punti al primo record, non all'ultimo
                getForm().getAggiornamentiList().getTable().first();
                // Imposto la sezione del filtro chiave unità documentaria aperto dopo la ricerca se ho inserito un
                // valore nel campo registro
                if (!getForm().getComponentiFiltri().getCd_registro_key_unita_doc().getValue().equals("")) {
                    getForm().getChiaveSection().setLoadOpened(true);
                }
            }
        }
        // Workaround per evitare che il trigger scarichi la pagina HTML anziché visualizzarla sul browser
        forwardToPublisher(Application.Publisher.ELENCHI_VERSAMENTO_DETAIL);
    }
    ...
    /**
     * Bottone di chiusura elenco di versamento. Se presente, significa che sono già soddisfatte le condizioni per la
     * chiusura dell'elenco (ovvero stato APERTO e con almeno una unità documentaria o un documento aggiunto
     * appartenente)
     *
     * @throws Throwable
     *             errore generico
    ...
    public void confermaChiusuraElenco() throws EMFError {
        ...
        try {
            ...
            boolean noIndice = ElvElencoVer.TiValidElenco.NO_INDICE.name().equals(elencoVers.getTiValidElenco());
            if (noIndice) {
                messageOk = "Elenco di versamento chiuso con successo (senza generazione dell'indice).";
            }

            /*
             * Se l'elenco ha tipo validazione = NO_FIRMA e modalità di validazione = AUTOMATICA; se i valori
             * nell'elenco sono nulli leggo i valori dal criterio di raggruppamento.
             */
            ...
            /*
             * Ricarico il dettaglio elenco per caricare i dati modificati e visualizzare i bottoni corretti per il suo
             * nuovo stato
             */
            dettaglioElencoVersamento(idElencoVers);
            forwardToPublisher(Application.Publisher.ELENCHI_VERSAMENTO_DETAIL);
    ...
    public void listaOperazioniElencoButton() throws Throwable {
        ...
        redirectToAction(Application.Actions.MONITORAGGIO,
                "?operation=ricercaOperazioniElenchiDaDettaglioElenco&idElencoPerMon=" + idElencoVers
    ...
    /**
     * Bottone "+" della "Lista elenchi di versamento da firmare" per spostare un elenco da questa lista a quella degli
     * elenchi selezionati pronti per essere firmati
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectElenchiVersamentoDaFirmareList() throws EMFError {
        ...
        int paginaCorrente = getForm().getElenchiVersamentoDaFirmareList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiVersamentoDaFirmareList().getTable().getFirstRowPageIndex();
        ...
        getForm().getElenchiVersamentoSelezionatiList().getTable().addSortingRule(
                getForm().getElenchiVersamentoSelezionatiList().getDt_creazione_elenco().getName(), SortingRule.ASC);
        getForm().getElenchiVersamentoSelezionatiList().getTable().sort();
        forwardToPublisher(Application.Publisher.LISTA_ELENCHI_VERSAMENTO_SELECT);
    }

    /**
     * Bottone "-" della "Lista elenchi di versamento selezionati" per spostare un elenco da questa lista a quella degli
     * elenchi di versamento da firmare
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void selectElenchiVersamentoSelezionatiList() throws EMFError {
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiVersamentoSelezionatiList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiVersamentoSelezionatiList().getTable().getFirstRowPageIndex();
        ...
        int paginaCorrenteVF = getForm().getElenchiVersamentoDaFirmareList().getTable().getCurrentPageIndex();
        int inizioVF = getForm().getElenchiVersamentoDaFirmareList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
        ...
        if (componentiSelezionati != null) {
            for (String comp : componentiSelezionati) {
                if (StringUtils.isNumeric(comp)) {
                    idUnitaDocs.add(((ElvVListaCompElvRowBean) getForm().getComponentiList().getTable()
                            .getRow(Integer.parseInt(comp))).getIdUnitaDoc());
                    idDocsToRemove.add(((ElvVListaCompElvRowBean) getForm().getComponentiList().getTable()
                            .getRow(Integer.parseInt(comp))).getIdDoc());
                }
            }

            if (!idUnitaDocs.isEmpty()) {
                getSession().setAttribute("idDocsToRemove", idDocsToRemove);
                getForm().getDocDaRimuovereList().setTable(udHelper.getAroVLisDocTableBeanByIdDoc(idDocsToRemove));
                getForm().getDocDaRimuovereList().getTable().setPageSize(10);
                getForm().getDocDaRimuovereList().getTable().first();
                getRequest().setAttribute("elenco", getForm().getElenchiVersamentoDetail().getNm_elenco().parse());
    ...
    public void confermaRimozioneDocButton() throws Throwable {
        ...
        BigDecimal idElencoVers = getForm().getElenchiVersamentoDetail().getId_elenco_vers().parse();
        Set<BigDecimal> idDocsToRemove = (Set<BigDecimal>) getSession().getAttribute("idDocsToRemove");
        try {
            evEjb.deleteUdDocFromElencoVersamento(idElencoVers.longValue(), idDocsToRemove, getIdUtenteCorrente());
            ...
            if (evEjb.existNomeElenco(getForm().getElenchiVersamentoDetail().getNm_elenco().parse(),
                ...
                getMessageBox().addInfo("Unità documentarie eliminate con successo! "
                        + "In quanto rimasto privo di componenti, l'elenco di versamento " + idElencoVers
                        + " è stato eliminato!");
        ...
        if (aggiornamentiSelezionati != null) {
            for (String upd : aggiornamentiSelezionati) {
                if (StringUtils.isNumeric(upd)) {
                    idUnitaDocs.add(((ElvVLisUpdUdRowBean) getForm().getAggiornamentiList().getTable()
                            .getRow(Integer.parseInt(upd))).getIdUnitaDoc());
                    idUpdsToRemove.add(((ElvVLisUpdUdRowBean) getForm().getAggiornamentiList().getTable()
                            .getRow(Integer.parseInt(upd))).getIdUpdUnitaDoc());
                }
            }

            if (!idUnitaDocs.isEmpty()) {
                getSession().setAttribute("idUpdsToRemove", idUpdsToRemove);
                getForm().getUpdDaRimuovereList().setTable(udHelper.getElvVLisUpdUdTableBeanByIdUpd(idUpdsToRemove));
    ...
    public void confermaRimozioneUpdButton() throws Throwable {
        /* Ottengo l'idElencoVers */
        BigDecimal idElencoVers = getForm().getElenchiVersamentoDetail().getId_elenco_vers().parse();
        Set<BigDecimal> idUpdsToRemove = (Set<BigDecimal>) getSession().getAttribute("idUpdsToRemove");
        try {
            evEjb.deleteUpdFromElencoVersamento(idElencoVers.longValue(), idUpdsToRemove, getIdUtenteCorrente());
            ...
            if (evEjb.existNomeElenco(getForm().getElenchiVersamentoDetail().getNm_elenco().parse(),
                ...
                getMessageBox().addInfo("Unità documentarie eliminate con successo! "
                        + "In quanto rimasto privo di contenuti, l'elenco di versamento " + idElencoVers
                        + " è stato eliminato!");
                setTableName(getForm().getElenchiVersamentoList().getName());
                goBackTo(Application.Publisher.ELENCHI_VERSAMENTO_RICERCA);
            }
    ...
    private void reloadElenchiVersamentoList(ElenchiVersamentoForm.FiltriElenchiVersamento filtriElenchiVersamento)
            throws EMFError {
        ...
        if (chiavi != null) {
            ...
            // Se non sono presenti i filtri dell'UD
            List<String> tiStatoElenco = filtriElenchiVersamento.getTi_stato_elenco().parse();
            if (!tiStatoElenco.isEmpty() && !tiStatoElenco.contains(ElencoEnums.ElencoStatusEnum.COMPLETATO.name())) {
                // Se il filtro di stato elenco esiste e non contiene COMPLETATO
                getForm().getElenchiVersamentoList().setTable(
                        evEjb.getElvVRicElencoVersByStatoTableBean(getUser().getIdUtente(), filtriElenchiVersamento));
            } else {
                getForm().getElenchiVersamentoList().setTable(
                        evEjb.getElvVRicElencoVersTableBean(getUser().getIdUtente(), filtriElenchiVersamento));
            }
    ...
    public JSONObject triggerFiltriElenchiDaFirmareId_ambienteOnTrigger() throws EMFError {
        triggerAmbienteGenerico(getForm().getFiltriElenchiDaFirmare(), ActionEnums.SezioneElenchi.RICERCA_ELENCHI);
    ...
    public JSONObject triggerFiltriElenchiDaFirmareId_enteOnTrigger() throws EMFError {
        triggerEnteGenerico(getForm().getFiltriElenchiDaFirmare(), ActionEnums.SezioneElenchi.RICERCA_ELENCHI);
        return getForm().getFiltriElenchiDaFirmare().asJSON();
    }

    public Fields triggerAmbienteGenerico(Fields campi, ActionEnums.SezioneElenchi sezione) throws EMFError {
    ...
    public Fields triggerEnteGenerico(Fields campi, ActionEnums.SezioneElenchi sezione) throws EMFError {
        campi.post(getRequest());
    ...
    /**
     * Inizializza i FILTRI DI LISTA ELENCHI DA FIRMARE in base alla struttura con la quale l'utente è loggato
     *
     * @throws EMFError
     *             errore generico
     */
    private void initFiltriElenchiDaFirmare(BigDecimal idStruttura) {
        ...
        getForm().getFiltriElenchiDaFirmare().getRicercaElenchiDaFirmareButton().setDisableHourGlass(true);
    }

    public void ricercaElenchiDaFirmare(FiltriElenchiDaFirmare filtriElenchiDaFirmare) throws EMFError {
    ...
    public void firmaElenchiHsmJs() throws EMFError {
        ...
        char[] passwd = getForm().getFiltriElenchiDaFirmare().getPasswd().parse() != null
                ? getForm().getFiltriElenchiDaFirmare().getPasswd().parse().toCharArray() : null;
        char[] otp = getForm().getFiltriElenchiDaFirmare().getOtp().parse() != null
                ? getForm().getFiltriElenchiDaFirmare().getOtp().parse().toCharArray() : null;
            ...
            for (ElvVLisElencoVersStatoRowBean elencoVista : (ElvVLisElencoVersStatoTableBean) getForm()
                ...
                if (elencoVers.getNiUnitaDocModElenco().longValue() > 0) {
                    ...
                    boolean areAllElenchiNonPresentiFirmati = evEjb.areAllElenchiNonPresentiFirmati(
                            (ElvVLisElencoVersStatoTableBean) getForm().getElenchiVersamentoSelezionatiList()
                                    .getTable(),
    ...
    public void loadListaElenchiIndiciAipDaFirmare() throws EMFError {
        ...
        if (idStrut != null && cleanFilter) {
            ...
            if (cleanList) {
                if (getForm().getFiltriElenchiIndiciAipDaFirmare().validate(getMessageBox())) {
                    if (!getMessageBox().hasError()) {
                        ElenchiVersamentoValidator elenchiValidator = new ElenchiVersamentoValidator(getMessageBox());
                        // Valido i filtri data creazione elenco indici aip da - a restituendo le date comprensive di
                        // orario
                        Date[] dateCreazioneElencoIdxAipValidate = elenchiValidator.validaDate(
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_da()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getOre_dt_creazione_elenco_idx_aip_da()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare()
                                        .getMinuti_dt_creazione_elenco_idx_aip_da().parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_a()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getOre_dt_creazione_elenco_idx_aip_a()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getMinuti_dt_creazione_elenco_idx_aip_a()
                                        .parse(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_da()
                                        .getHtmlDescription(),
                                getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_a()
                        ...
                        if (!getMessageBox().hasError()) {
                            ...
                            getForm().getElenchiIndiciAipDaFirmareList().getTable().addSortingRule(getForm()
                                    .getElenchiIndiciAipDaFirmareList().getDt_creazione_elenco_ix_aip().getName(),
                                    SortingRule.ASC);
        ...
        // Verifico su db la presenza della sessione di firma o di un oggetto future (di una possibile sessione di firma
        // preesistente) in sessione
        if (elencoIndiciAipSignSessionEjb.hasUserActiveSessions(getUser().getIdUtente()) || futureFirma != null) {
            // Se esistono delle sessioni bloccate per quell'utente le sblocco
            if (elencoIndiciAipSignSessionEjb.hasUserBlockedSessions(getUser().getIdUtente())) {
                ...
                getForm().getElenchiIndiciAipDaFirmareButtonList().getFirmaElenchiIndiciAipHsm().setReadonly(false);
                getMessageBox().addInfo("\u00C8 stata sbloccata una sessione di firma bloccata");
                getMessageBox().setViewMode(ViewMode.plain);
            } else {
                getForm().getElenchiIndiciAipDaFirmareButtonList().getFirmaElenchiIndiciAipHsm().setReadonly(true);
                // Sessione di firma attiva
                getMessageBox().addInfo("Sessione di firma attiva");
                getMessageBox().setViewMode(ViewMode.plain);
            }
        ...
        getForm().getElenchiIndiciAipDaFirmareButtonList().getFirmaElenchiIndiciAipHsm().setHidden(false);
    ...
    public void ricercaElenchiIndiciAipDaFirmare() throws Throwable {
        ...
        if (getForm().getFiltriElenchiIndiciAipDaFirmare().validate(getMessageBox())) {
            if (!getMessageBox().hasError()) {
                ElenchiVersamentoValidator elenchiValidator = new ElenchiVersamentoValidator(getMessageBox());
                // Valido i filtri data creazione elenco indici aip da - a restituendo le date comprensive di orario
                Date[] dateCreazioneElencoIdxAipValidate = elenchiValidator.validaDate(
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_da().parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getOre_dt_creazione_elenco_idx_aip_da().parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getMinuti_dt_creazione_elenco_idx_aip_da()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_a().parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getOre_dt_creazione_elenco_idx_aip_a().parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getMinuti_dt_creazione_elenco_idx_aip_a()
                                .parse(),
                        getForm().getFiltriElenchiIndiciAipDaFirmare().getDt_creazione_elenco_idx_aip_da()
                                .getHtmlDescription(),
                ...
                if (!getMessageBox().hasError()) {
                    ...
                    getForm().getElenchiIndiciAipSelezionatiList().getTable().setPageSize(10);
                    getForm().getElenchiIndiciAipSelezionatiList().getTable().addSortingRule(
                            getForm().getElenchiIndiciAipSelezionatiList().getDt_creazione_elenco_ix_aip().getName(),
                            SortingRule.ASC);
                }
            }
        }
    ...
    public void firmaElenchiIndiciAipHsm() throws Throwable {
        ...
        if (!getMessageBox().hasError()) {
            ...
            // ricerca
            // e così verrebbe preso un ambiente errato
            BigDecimal idStrut = ((ElvVLisElencoVersStatoTableBean) getForm().getElenchiIndiciAipSelezionatiList()
                    .getTable()).getRow(0).getIdStrut();
            OrgAmbienteRowBean ambienteRowBean = struttureEjb.getOrgAmbienteRowBeanByIdStrut(idStrut);
            BigDecimal idAmbiente = ambienteRowBean.getIdAmbiente();
            if (idAmbiente != null) {
                // Ricavo il parametro HSM_USERNAME (parametro multiplo dell'ambiente) associato all'utente corrente
    ...
    // Questo metodo può essere unificato a initFiltriElenchiDaFirmare, bisogna aspettare però il rilascio del branch
    ...
    private void initFiltriElenchiIndiciAipDaFirmare(BigDecimal idStruttura) {
        ...
        getForm().getFiltriElenchiIndiciAipDaFirmare().getId_ambiente().setValue(idAmbiente.toString());
        ...
        mappaStrut.populatedMap(tmpTableBeanStruttura, "id_strut", "nm_strut");
        getForm().getFiltriElenchiIndiciAipDaFirmare().getId_strut().setDecodeMap(mappaStrut);
        getForm().getFiltriElenchiIndiciAipDaFirmare().getId_strut().setValue(idStruttura.toString());
    ...
    public void selectElenchiIndiciAipDaFirmareList() throws EMFError {
        /* Ricavo il record interessato della "Lista elenchi di versamento da firmare" */
        ElvVLisElencoVersStatoRowBean row = (ElvVLisElencoVersStatoRowBean) getForm().getElenchiIndiciAipDaFirmareList()
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiIndiciAipDaFirmareList().getTable().getCurrentPageIndex();
    ...
    public void selectElenchiIndiciAipSelezionatiList() throws EMFError {
        ...
        /* "Refresho" la lista senza il record */
        int paginaCorrente = getForm().getElenchiIndiciAipSelezionatiList().getTable().getCurrentPageIndex();
        int inizio = getForm().getElenchiIndiciAipSelezionatiList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
        ...
        int paginaCorrenteVF = getForm().getElenchiIndiciAipDaFirmareList().getTable().getCurrentPageIndex();
        int inizioVF = getForm().getElenchiIndiciAipDaFirmareList().getTable().getFirstRowPageIndex();
        // Rieseguo la query se necessario
    ...
    public void firmaElenchiIndiciAipHsmJs() throws EMFError {
        ...
        char[] passwd = getForm().getFiltriElenchiIndiciAipDaFirmare().getPasswd().parse() != null
                ? getForm().getFiltriElenchiIndiciAipDaFirmare().getPasswd().parse().toCharArray() : null;
        char[] otp = getForm().getFiltriElenchiIndiciAipDaFirmare().getOtp().parse() != null
                ? getForm().getFiltriElenchiIndiciAipDaFirmare().getOtp().parse().toCharArray() : null;
    ...
    public JSONObject triggerFiltriElenchiVersamentoId_ambienteOnTrigger() throws EMFError {
        triggerAmbienteGenerico(getForm().getFiltriElenchiVersamento(), ActionEnums.SezioneElenchi.RICERCA_ELENCHI);
    ...
    public JSONObject triggerFiltriElenchiVersamentoId_enteOnTrigger() throws EMFError {
        triggerEnteGenerico(getForm().getFiltriElenchiVersamento(), ActionEnums.SezioneElenchi.RICERCA_ELENCHI);
        return getForm().getFiltriElenchiVersamento().asJSON();
    }

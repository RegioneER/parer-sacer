...
public class TypeValidator {
     ...
     *
     * @param data_da    data di riferimento da
     * @param ore_da     ore di riferimento da
     * @param minuti_da  minuti di riferimento da
     * @param data_a     data di riferimento a
     * @param ore_a      ore di riferimento a
     * @param minuti_a   minuti di riferimento a
     ...
     *
     * @param size_da dimensione di riferimento da
     * @param size_a  dimensione di riferimento a
     *
     * @throws EMFError errore generico
     */
    public void validaDimensioniKb(BigDecimal size_da, BigDecimal size_a) throws EMFError {
        if (size_da != null && size_a == null) {
            getMessageBox()
                    .addMessage(new Message(Message.MessageLevel.ERR, "Dimensione finale assente"));
        }
        if (size_da != null && size_a != null) {
            if (size_da.compareTo(size_a) > 0) {
                getMessageBox().addMessage(new Message(Message.MessageLevel.ERR,
                        "Dimensione iniziale superiore a dimensione finale"));
            }
        }
    }
     ...
     * @throws EMFError errore generico
     */
    public void validaStruttura(String nmJob, BigDecimal idAmbiente, BigDecimal idEnte,
        ...
        if (nmJob.equals(JobConstants.JobEnum.VERIFICA_VERS_FALLITI.name())
            ...
            if (idAmbiente == null) {
                getMessageBox().addMessage(
                        new Message(Message.MessageLevel.ERR, "Ambiente non impostato"));
            } else if (idEnte == null) {
                getMessageBox()
                        .addMessage(new Message(Message.MessageLevel.ERR, "Ente non impostato"));
            } else if (idStrut == null) {
                getMessageBox().addMessage(
                        new Message(Message.MessageLevel.ERR, "Struttura non impostata"));
            }
        }
    }
    ...
    public ChiaveBean validaChiavi(String type, Input<BigDecimal> input_anno,
        ...
        if (singleValue
                && (annoDa != null || annoA != null || numeroDa != null || numeroA != null)) {
            getMessageBox().addError("Sono stati inseriti valori nella ricerca per chiave " + type
                    + " sia singola che con range");
        } else if (singleValue) {
            result.setAnno(anno);
            ...
            if (annoDa != null || annoA != null) {
                ...
                if (annoDa.compareTo(annoA) > 0) {
                    getMessageBox().addError(
                            "Range di chiavi " + type + ": " + input_annoDa.getHtmlDescription()
            ...
            if (numeroDa != null || numeroA != null) {
                ...
                if (numeroDa.compareTo(numeroA) > 0) {
                    getMessageBox().addError(
                            "Range di chiavi " + type + ": " + input_numeroDa.getHtmlDescription()

...
public class SerieValidator extends TypeValidator {
    ...
    /**
     * Metodo di validazione degli attributi selezionati relativi alle regole di acquisizione file (Dati Profilo, Dati
     * Specifici Tipi Ud, Dati Specifici Tipi Doc). Viene controllato che i numeri d'ordine siano numerici e diversi tra
     * loro.
     *
     ...
     *             errore generico
     */
    public void validaAttributiRegoleAcquisizioneFile(Map<String, Map<String, String>> listaAttributiSelezionati)
            throws EMFError {
        ...
        while (it.hasNext()) {
            ...
            // con il relativo valore (concatenazione dei campi Tipo trasformazione | numero d'ordine |
            // idAttribDatoSpec)
        ...
        if (!getMessageBox().hasError()) {
            if (numeriOrdine.size() != contaCampi) {
                getMessageBox().addError("Attenzione: uno o più campi presentano lo stesso numero d'ordine");
            }
        }
    }
    ...
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Filtri periodo e data versamento entrambi
    // valorizzati!"));
    // } else if (periodo == null && data == null) {
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Attenzione: è necessario valorizzare almeno uno tra i
    ...
    // if (periodo != null
    // && (giorno_vers_da != null || ore_vers_da != null || minuti_vers_da != null || giorno_vers_a != null ||
    // ore_vers_a != null || minuti_vers_a != null)) {
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Campi periodo e data versamento entrambi
    // valorizzati!"));
    ...
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Una sessione può essere definita non risolubile o
    ...
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Data riferimento Da maggiore di Data riferimento A"));
    // }
    // if (dataRifDa.before(dataInizio.getTime())) {
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Data riferimento Da inferiore al 1 gennaio 2000"));
    // }
    // if (dataRifA.after(new Date())) {
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Data riferimento A successiva alla data corrente"));
    // }
    // }
}

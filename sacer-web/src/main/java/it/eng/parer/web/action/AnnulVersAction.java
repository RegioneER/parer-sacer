...
public class AnnulVersAction extends AnnulVersAbstractAction {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AnnulVersAction.class.getName());
    ...
    public void insertDettaglio() throws EMFError {
        if (getTableName().equals(getForm().getRichAnnulVersList().getName())) {
            ...
            getSession().setAttribute(getForm().getCreazioneRichAnnulVers().getFl_immediata().getName(), "0");
            ...
            if (getSession().getAttribute("tiRichAnnulVers") != null) {
                ...
                } else if (((String) getSession().getAttribute("tiRichAnnulVers")).equals("FASCICOLI")) {
            ...
            final BigDecimal idRichAnnulVers = getForm().getRichAnnulVersDetail().getId_rich_annul_vers().parse();
            final String tiRichAnnulVers = getForm().getRichAnnulVersDetail().getTi_rich_annul_vers().parse();
            if (!annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
                ...
                if (tiRichAnnulVers.equals("UNITA_DOC")) {
                    ...
                    unitaDocumentarieForm.getUnitaDocumentariePerRichAnnulVers().getId_rich_annul_vers()
                    ...
                    // Inizializzo la form per andare alla ricerca fascicoli inserendo i valori trovati
                    FascicoliForm fascicoliForm = new FascicoliForm();
                    fascicoliForm.getFascicoliPerRichAnnulVers().getId_rich_annul_vers()
                            .setValue(idRichAnnulVers.toPlainString());

                    redirectToAction(Application.Actions.FASCICOLI, "?operation=fascicoliRicercaSemplice",
                            fascicoliForm);
                }
            }
        }
    ...
    public void loadDettaglio() throws EMFError {
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
            ...
            if (getTableName().equals(getForm().getRichAnnulVersList().getName())) {
                AroVRicRichAnnvrsRowBean row = (AroVRicRichAnnvrsRowBean) getForm().getRichAnnulVersList().getTable()
                        .getCurrentRow();
                ...
                AroVLisStatoRichAnnvrsRowBean currentRow = (AroVLisStatoRichAnnvrsRowBean) getForm().getStatiList()
    ...
    private void loadDettaglioRichiesta(BigDecimal idRichAnnulVers) throws EMFError {
        AroVVisRichAnnvrsRowBean detailRow = annulVersEjb.getAroVVisRichAnnvrsRowBean(idRichAnnulVers);
        getForm().getRichAnnulVersDetail().getFl_forza_annul_combo()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
        getForm().getRichAnnulVersDetail().copyFromBean(detailRow);

        AroVLisStatoRichAnnvrsTableBean listaStati = annulVersEjb.getAroVLisStatoRichAnnvrsTableBean(idRichAnnulVers);
        ...
        AroVLisItemRichAnnvrsTableBean listaItem = annulVersEjb.getAroVLisItemRichAnnvrsTableBean(idRichAnnulVers);
        ...
        if (annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
            ...
            // se nella richiesta non sono presenti item con stato = DA_ANNULLARE_IN_PING ed è presente almeno un item
    ...
    public void saveDettaglio() throws EMFError {
        if (getTableName().equals(getForm().getRichAnnulVersList().getName())) {
            if (getForm().getRichAnnulVersDetail().validate(getMessageBox())) {
                ...
                BigDecimal idRichAnnulVers = getForm().getRichAnnulVersDetail().getId_rich_annul_vers().parse();
                String cdRichAnnulVers = getForm().getRichAnnulVersDetail().getCd_rich_annul_vers().parse();
                String dsRichAnnulVers = getForm().getRichAnnulVersDetail().getDs_rich_annul_vers().parse();
                String ntRichAnnulVers = getForm().getRichAnnulVersDetail().getNt_rich_annul_vers().parse();
                String flForzaAnnul = getForm().getRichAnnulVersDetail().getFl_forza_annul_combo().parse();

                try {
                    annulVersEjb.saveRichAnnulVers(idRichAnnulVers, cdRichAnnulVers, dsRichAnnulVers, ntRichAnnulVers,
                            flForzaAnnul, idStrut);
            ...
            Message validate = getForm().getStatoRichAnnulVersDetail().getDs_nota_rich_annul_vers().validate();
            if (validate == null) {
                BigDecimal idStato = getForm().getStatoRichAnnulVersDetail().getId_stato_rich_annul_vers().parse();
                String dsNota = getForm().getStatoRichAnnulVersDetail().getDs_nota_rich_annul_vers().parse();
                try {
                    if (idStato != null) {
    ...
    public void dettaglioOnClick() throws EMFError {
        if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW) || getNavigationEvent().equals(ListAction.NE_NEXT)
            ...
            if (getTableName().equals(getForm().getRichAnnulVersList().getName())) {
                ...
                AroVLisItemRichAnnvrsRowBean rowBean = (AroVLisItemRichAnnvrsRowBean) getForm().getItemList().getTable()
                        .getCurrentRow();
                if (rowBean.getIdUnitaDoc() != null) {
                    AroVLisItemRichAnnvrsTableBean listaItem = (AroVLisItemRichAnnvrsTableBean) getForm().getItemList()
                            .getTable();
                    ...
                    getMessageBox().addError("Versamento relativo a una unit\u00E0 documentaria inesistente");
                    forwardToPublisher(getLastPublisher());
                }
            } else if (getTableName().equals(getForm().getStatiList().getName())) {
                forwardToPublisher(Application.Publisher.STATO_RICH_ANNUL_VERS_DETAIL);
        ...
        if (isMultipart) {
            ...
            try {
                if (getLastPublisher().equals(Application.Publisher.CREAZIONE_RICH_ANNUL_VERS)) {
                    paramMethods = getForm().getCreazioneRichAnnulVers().postMultipart(getRequest(), size10Mb);
                } else if (getLastPublisher().equals(Application.Publisher.RICH_ANNUL_VERS_DETAIL)) {
                    paramMethods = getForm().getRichAnnulVersDetail().postMultipart(getRequest(), size10Mb);
                }
                if (paramMethods != null) {
                    ...
                    if (paramMethods.length > 1) {
                        ...
                        Method method = AnnulVersAction.class.getMethod(operationMethod, String[].class);
                ...
                logger.error("Errore nell'invocazione del metodo :" + ExceptionUtils.getRootCauseMessage(ex), ex);
    ...
    public void deleteRichAnnulVersList() throws EMFError {
        ...
        if (!annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
            getMessageBox()
                    .addError("La richiesta non \u00E8 eliminabile perch\u00E9 ha stato corrente diverso da APERTA");
        } else {
            // Elimina richiesta
            try {
        ...
        if (!getMessageBox().hasError() && getLastPublisher().equals(Application.Publisher.RICH_ANNUL_VERS_DETAIL)) {
    ...
    public void updateRichAnnulVersList() throws EMFError {
        ...
        if (!annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
            ...
            getMessageBox()
                    .addError("La richiesta non \u00E8 modificabile perch\u00E9 ha stato corrente diverso da APERTA");
            forwardToPublisher(getLastPublisher());
        } else {
            // Modifica richiesta
    ...
    public void deleteItemList() throws EMFError {
        AroVLisItemRichAnnvrsRowBean row = (AroVLisItemRichAnnvrsRowBean) getForm().getItemList().getTable()
                .getCurrentRow();
    ...
    public void updateStatiList() throws EMFError {
        AroVLisStatoRichAnnvrsRowBean currentRow = (AroVLisStatoRichAnnvrsRowBean) getForm().getStatiList().getTable()
                .getCurrentRow();
    ...
    public void creaRichAnnulVers() throws EMFError {
        ...
        try {
            ...
            final String cdRichAnnulVers = getForm().getCreazioneRichAnnulVers().getCd_rich_annul_vers().parse();
            final String dsRichAnnulVers = getForm().getCreazioneRichAnnulVers().getDs_rich_annul_vers().parse();
            final String ntRichAnnulVers = getForm().getCreazioneRichAnnulVers().getNt_rich_annul_vers().parse();
            // // NOTA: essendo il bottone attualmente associato solo alla richieste di annullamento ud, per ora il
            ...
            if (!getMessageBox().hasError() && StringUtils.isBlank(flImmediata)) {
                getMessageBox().addError("Campo '" + getForm().getCreazioneRichAnnulVers().getFl_immediata().getName()
                        + "' obbligatorio");
            }
            ...
            if (!getMessageBox().hasError() && blFile != null) {
                ...
                if (!mime.equals(VerFormatiEnums.CSV_MIME) && !mime.equals(VerFormatiEnums.TEXT_PLAIN_MIME)) {
                    ...
                    if (tiRichAnnulVers.equals("UNITA_DOC") && !annulVersEjb.checkCsvHeaders(fileByteArray)) {
                        getMessageBox().addError(
                                "Il primo record del file non definisce i nomi dei campi previsti (\"REGISTRO\", \"ANNO\" e \"NUMERO\")");
                    }
                    if (tiRichAnnulVers.equals("FASCICOLI") && !annulVersEjb.checkCsvHeadersFasc(fileByteArray)) {
                        getMessageBox().addError(
            ...
            if (!getMessageBox().hasError()) {
                if (annulVersEjb.checkCdRichAnnulVersExisting(cdRichAnnulVers, getUser().getIdOrganizzazioneFoglia())) {
                    getMessageBox().addError(
            ...
            if (!getMessageBox().hasError()) {
                ...
                if (idRichAnnulVers != null && flImmediata.equals("1")) {
                    ...
                    // corrente
                    annulVersEjb.elaboraRichiestaAnnullamento(idRichAnnulVers, getUser().getIdUtente());
                }
                if (idRichAnnulVers != null) {
            ...
            logger.error("Eccezione generica nella creazione della richiesta di annullamento: "
                    + ExceptionUtils.getRootCauseMessage(ex), ex);
            getMessageBox()
                    .addError("Si \u00E8 verificata un'eccezione nella creazione della richiesta di annullamento");
        }
    ...
    private void caricaContenutoPaginaRicercaAnnulVers(String tiRichAnnulVers) throws EMFError {
        getSession().removeAttribute(getForm().getCreazioneRichAnnulVers().getFl_immediata().getName());
        ...
        getForm().getFiltriRicercaRichAnnullVers().getFl_non_annul()
                .setDecodeMap(ComboGetter.getMappaGenericFlagSiNo());
        getForm().getFiltriRicercaRichAnnullVers().getTi_stato_rich_annul_vers_cor().setDecodeMap(ComboGetter
                .getMappaSortedGenericEnum("ti_stato_rich_annul_vers", CostantiDB.StatoRichAnnulVers.values()));
        ...
        if (tiRichAnnulVers.equals("UNITA_DOC")) {
            ...
            getForm().getFiltriRicercaRichAnnullVers().getCd_registro_key_unita_doc().setHidden(false);
            ...
            DecRegistroUnitaDocTableBean tmpTableBeanReg = registroEjb
                    .getRegistriUnitaDocAbilitati(getUser().getIdUtente(), getUser().getIdOrganizzazioneFoglia());
            DecodeMap mappaRegistro = DecodeMap.Factory.newInstance(tmpTableBeanReg, "cd_registro_unita_doc",
                    "cd_registro_unita_doc");
            getForm().getFiltriRicercaRichAnnullVers().getCd_registro_key_unita_doc().setDecodeMap(mappaRegistro);
            ...
            getForm().getFiltriRicercaRichAnnullVers().getCd_registro_key_unita_doc().setHidden(true);
    ...
    public JSONObject triggerFiltriRicercaRichAnnullVersId_ambienteOnTrigger() throws EMFError {
        ...
        if (idAmbiente != null) {
            ...
            if (tmpTableBeanEnte.size() == 1) {
                ...
                getForm().getFiltriRicercaRichAnnullVers().getCd_registro_key_unita_doc().setDecodeMap(new DecodeMap());
            }
        } else {
    ...
    public void ricercaRichAnnulVers() throws EMFError {
        if (getForm().getFiltriRicercaRichAnnullVers().postAndValidate(getRequest(), getMessageBox())) {
            ...
            RicercaRichAnnulVersBean filtri = new RicercaRichAnnulVersBean(getForm().getFiltriRicercaRichAnnullVers());
            ...
            validator.validaOrdineDateOrari(filtri.getDt_creazione_rich_annul_vers_da(),
                    ...
                    getForm().getFiltriRicercaRichAnnullVers().getDt_creazione_rich_annul_vers_da().getName(),
                    getForm().getFiltriRicercaRichAnnullVers().getDt_creazione_rich_annul_vers_a().getName());
            if (!getMessageBox().hasError()) {
                AroVRicRichAnnvrsTableBean table = annulVersEjb.getAroVRicRichAnnvrsTableBean(getUser().getIdUtente(),
                        filtri);
                getForm().getRichAnnulVersList().setTable(table);
                getForm().getRichAnnulVersList().getTable().setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
    ...
    public void rifiutaRichiesta() throws Throwable {
        ...
        if (annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
            ...
            getMessageBox().addError("Modifica non possibile in quanto la richiesta non ha stato APERTA");
        }
    }
    ...
    public void controllaRichiesta() throws Throwable {
        ...
        if (annulVersEjb.checkStatoRichiesta(idRichAnnulVers, CostantiDB.StatoRichAnnulVers.APERTA.name())) {
            ...
            getMessageBox().addError("Controllo non possibile in quanto la richiesta non ha stato APERTA");
        }
        forwardToPublisher(getLastPublisher());
    }
    ...
    private void cambiaStato(BigDecimal idRichAnnulVers, String tiStatoRichAnnulVers) {
        getForm().getCambioStatoRichiesta().getId_rich_annul_vers().setValue(idRichAnnulVers.toPlainString());
        ...
        getForm().getCambioStatoRichiesta().getTi_stato_rich_annul_vers().setValue(tiStatoRichAnnulVers);
        ...
        try {
            if (tiStatoRichAnnulVers.equals(CostantiDB.StatoRichAnnulVers.CHIUSA.name())) {
                ...
                Long numeroItemsNonAnnullabili = annulVersEjb.countItemsInRichAnnulVers(idRichAnnulVers,
    ...
    public void confermaCambioStato() throws EMFError {
        ...
        BigDecimal idRichAnnulVers = getForm().getCambioStatoRichiesta().getId_rich_annul_vers().parse();
        String dsNotaRichAnnulVers = getForm().getCambioStatoRichiesta().getDs_nota_rich_annul_vers().parse();
        String tiStatoRichAnnulVers = getForm().getCambioStatoRichiesta().getTi_stato_rich_annul_vers().parse();
        String tiStatoRichAnnulVersOld = getForm().getRichAnnulVersDetail().getTi_stato_rich_annul_vers().parse();
        try {
            annulVersEjb.cambiaStato(getUser().getIdUtente(), idRichAnnulVers, tiStatoRichAnnulVersOld,
                    tiStatoRichAnnulVers, dsNotaRichAnnulVers);
        } catch (ParerUserError ex) {
            getMessageBox().addError(ex.getDescription());
        }
        goBack();
    ...
    public void uploadFile() throws EMFError {
        try {
            ...
            if (!getMessageBox().hasError()) {
                ...
                // Ricavo l'utente che ha creato la richiesta (il primo stato di essa)
                Long idUtente = annulVersEjb.getUserFirstStateRich(idRichAnnulVers);
                annulVersEjb.saveRichAnnulVers(idRichAnnulVers, fileByteArray, idUtente, tiRichAnnulVers);
                loadDettaglioRichiesta(idRichAnnulVers);
            }
        } catch (IOException ex) {
            logger.error("Eccezione nell'upload del file", ex);
            ...
            logger.error("Eccezione generica nella modifica della richiesta di annullamento: "
                    + ExceptionUtils.getRootCauseMessage(ex), ex);
            getMessageBox()
                    .addError("Si \u00E8 verificata un'eccezione nella modifica della richiesta di annullamento");
        }

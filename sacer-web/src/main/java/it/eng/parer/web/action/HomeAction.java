...
public class HomeAction extends HomeAbstractAction {
    ...
    private static Logger log = LoggerFactory.getLogger(HomeAction.class.getName());
	...
	try {
		...
		int numGiorni = Integer.parseInt(configHelper.getValoreParamApplicByApplic(
			CostantiDB.ParametroAppl.NUM_GIORNI_ESPONI_SCAD_PSW));
	    ...
	    getForm().getContenutoSacerTotaliUdDocComp().getScaricaDisciplinareButton()
		    .setEditMode();
	    getForm().getContenutoSacerTotaliUdDocComp().getScaricaDisciplinareButton()
		    .setDisableHourGlass(true);
	} catch (Exception e) {
	    log.error("Errore nella caricamento della Home Page", e);
	}
	...
	this.getResponse()
		.sendRedirect(configHelper.getValoreParamApplicByApplic(
			CostantiDB.ParametroAppl.URL_MODIFICA_PASSWORD) + "?r=" + retURL + "&h="
			+ hmac + "&s=" + salt);
	...
	String url = configHelper
		.getValoreParamApplicByApplic(CostantiDB.ParametroAppl.URL_RECUP_NEWS);
	...
	String timeoutString = configHelper
		.getValoreParamApplicByApplic(CostantiDB.ParametroAppl.TIMEOUT_RECUP_NEWS);

	RestituzioneNewsApplicazione client = IAMSoapClients
		.restituzioneNewsApplicazioneClient(user, psw, url);
	...
	/* Calcolo i totali */
	BaseRowInterface totali = monitoraggioHelper.getTotaliUdDocCompForHome(idAmbiente, idEnte,
		idStrut, da.getTime(), a.getTime(), getUser().getIdUtente());
	...
	String titolo = " " + organizzazione.get("ENTE") + " - " + organizzazione.get("STRUTTURA")
		+ " al " + ieriString;
	...
	BaseTableInterface totSacer = monitoraggioHelper.getMonTotSacerForHomeTable(idAmbiente,
		idEnte, idStrut, da.getTime(), a.getTime(), getUser().getIdUtente());
	...
	try {
	    ...
	    String nomeFile = "disciplinare_" + getUser().getOrganizzazioneMap().get("STRUTTURA")
		    + "_" + sdf.format(dat) + ".pdf";
	    ...
	    log.error(ERRORE_DOWNLOAD_MESSAGE + ExceptionUtils.getRootCauseMessage(ex), ex);
	...
	if (path != null && filename != null) {
		...
		getResponse().setContentType(
			StringUtils.isBlank(contentType) ? WebConstants.MIME_TYPE_PDF
				: contentType);

...
public class VolumiAction extends VolumiAbstractAction {
    ...
    private static Logger logger = LoggerFactory.getLogger(VolumiAction.class.getName());
    ...
    public void initOnClick() throws EMFError {
        ...
        // Imposto i valori della combo CRITERIO_RAGGRUPPAMENTO ricavati dalla tabella DEC_CRITERIO_RAGGR
        DecCriterioRaggrTableBean tmpTableBeanCriteri = criteriRaggruppamentoEjb.getDecCriterioRaggrTableBean(null,
                null, getIdStrut(), null);
        ...
        getForm().getFiltri().getTi_stato_volume_conserv().setDecodeMap(ComboGetter.getMappaSortedGenericEnum(
                "ti_stato_volume", VolumeEnums.VolStatusEnum.getComboMappaStatoVolRicerca()));
        ...
        getForm().getFiltri().getTi_presenza_firme().setDecodeMap(ComboGetter
                .getMappaSortedGenericEnum("presenza_firme", VolumeEnums.SignatureInfoEnum.getComboPresenzaFirme()));
        getForm().getFiltri().getTi_val_firme().setDecodeMap(ComboGetter.getMappaSortedGenericEnum("validita_firme",
                VolumeEnums.SignatureInfoEnum.getComboValiditaFirme()));
        ...
        /*
         * Rimuovo gli attributi di sessione che potrebbero essere rimasti in caso di navigazione su menu con una
         * operazione in corso
         */
    ...
    public void ricerca() throws EMFError {
        ...
        if (filtri.validate(getMessageBox())) {
            ...
            validator.validateDataVolumi(filtri.getDt_creazione_da().parse(), filtri.getDt_creazione_a().parse());
            Object[] chiavi = validator.validaChiaveUnitaDocVolumi(filtri.getCd_registro_key_unita_doc().parse(),
    ...
    /**
     * Metodo invocato sul bottone di dettaglio/modifica di una riga della lista volumi, esegue il caricamento della
    ...
    public void dettaglioOnClick() throws EMFError {
        if (getRequest().getParameter("table") != null) {
            ...
            Set<Object> regSet = getForm().getComponentiFiltri().getCd_registro_key_unita_doc().getDecodeMap().keySet();
            Set<Object> tipiUdSet = DecodeMap.Factory
                    .newInstance(tipoUnitaDocEjb.getTipiUnitaDocAbilitati(getUser().getIdUtente(),
                            getUser().getIdOrganizzazioneFoglia()), "id_tipo_unita_doc", "nm_tipo_unita_doc")
                    .keySet();
            ...
            if (getRequest().getParameter("table").equals(getForm().getComponentiList().getName())) {
                ...
                VolVListaCompVolRowBean row = (VolVListaCompVolRowBean) getForm().getComponentiList().getTable()
                        .getCurrentRow();
                if (regSet.contains(row.getCdRegistroKeyUnitaDoc()) && tipiUdSet.contains(row.getIdTipoUnitaDoc())) {
            ...
            } else if (getRequest().getParameter("table").equals(getForm().getDocumentoList().getName())) {
                // DOCUMENTO
                UnitaDocumentarieForm form = new UnitaDocumentarieForm();
                AroVLisDocTableBean docTB = new AroVLisDocTableBean();
                Integer riga = Integer.parseInt(getRequest().getParameter("riga"));
                BigDecimal idUnitaDoc = null;
                VolVListaCompVolRowBean rowComp = ((VolVListaCompVolRowBean) getForm().getComponentiList().getTable()
                        .getRow(riga));
    ...
    public void loadDettaglio() throws EMFError {
        ...
        if (getTableName() != null) {
            if (getTableName().equals(getForm().getVolumiList().getName())) {
                // LISTA VOLUMI
                idvol = getForm().getVolumiList().getTable().getCurrentRow().getBigDecimal("id_volume_conserv");
        ...
        if (volDetail) {
            if (!getLastPublisher().equals(Application.Publisher.DOCUMENTI_UNITA_DOCUMENTARIE_DETAIL)) {
    ...
    /**
     * Metodo invocato al salvataggio dei dati nella form di dettaglio volume per la modifica dei campi nome,
     * descrizione e note per indice e volume chiuso
     *
     * @throws EMFError
     *             errore generico
     */
    ...
    public void saveDettaglio() throws EMFError {
        ...
        if (!getMessageBox().hasError()) {
            try {
                volumiHelper.saveNomeDesNote(getUser().getIdUtente(), idvol, nomeVol, descrVol, notaVol, notaVolChiuso);
                undoDettaglio();
    ...
    public void updateVolumiList() throws EMFError {
        String statoVolume = getForm().getVolumiDetail().getTi_stato_volume_conserv().getValue();
        if (statoVolume.equals(VolStatusEnum.CHIUSO.name()) || statoVolume.equals(VolStatusEnum.FIRMATO_NO_MARCA.name())
                || statoVolume.equals(VolStatusEnum.FIRMATO.name())
    ...
    /**
     * Trigger sul filtro nome tipo struttura documento: selezionando un valore della combo box viene popolata la combo
    ...
    public JSONObject triggerComponentiFiltriNm_tipo_strut_docOnTrigger() throws EMFError {
        ...
        if (idTipoStrutDoc != null) {
            ...
            DecTipoCompDocTableBean tmpTableBeanTipoCompDoc = tipoStrutDocEjb.getDecTipoCompDocTableBean(idTipoStrutDoc,
                    false);
            ...
            if (!getMessageBox().hasError()) {
                ...
                getForm().getComponentiList()
                        .setTable(componentiHelper.getVolVListaCompVolViewBean(
                                ...
                                tmpTableBeanTipoUD, tmpTableBeanTipoDoc, tmpSubStrutsTableBean,
                                Integer.parseInt(maxResultCompVol)));
                getForm().getComponentiList().getTable().setPageSize(10);
    ...
    public void ricercaUdSemplice() throws EMFError {
        if (getForm().getVolumiDetail().getDt_scad_chius().parse().after(Calendar.getInstance().getTime())) {
            UnitaDocumentarieForm form = new UnitaDocumentarieForm();
            getSession().setAttribute("addUdToVolume", true);
            redirectToAction(Application.Actions.UNITA_DOCUMENTARIE,
                    "?operation=unitaDocumentarieRicercaSemplice&back=true", form);
        } else {
            getMessageBox().addError("Volume scaduto - Impossibile aggiungere nuove unità documentarie al volume");
            forwardToPublisher(Application.Publisher.VOLUMI_DETAIL);
        }
    }
    ...
    /**
     * Metodo invocato alla pressione del relativo bottone per scaricare in un file zip CRL, certificati e file di
     * conservazione di un volume chiuso o firmato no marca
     *
     * @param idvol
     *            id volume
     * @param statoVol
    ...
    public void downloadProveConservazione(BigDecimal idvol, String statoVol) throws EMFError {
        ...
        try {
            ...
            byte[] indiceConservXml = volumeEjb.retrieveFileByIdVolume(idvol.longValue(), FileTypeEnum.INDICE.name());
            ...
            if (statoVol.equals("FIRMATO_NO_MARCA") || statoVol.equals("FIRMATO")) {
                ...
                byte[] firmaIndiceConservTsrP7m = volumeEjb.retrieveFileByIdVolume(idvol.longValue(),
                        FileTypeEnum.FIRMA.name());
            ...
            getMessageBox().addMessage(
                    new Message(MessageLevel.ERR, "Errore nel recupero dei file delle prove di conservazione"));
    ...
    public void reloadAfterGoBack(String publisherName) {
        if (publisherName.equalsIgnoreCase(Application.Publisher.VOLUMI_DETAIL)
            ...
            // altrimenti se sono tornato indietro dalla lista UD da rimuovere, ne ho probabilmente tolto uno.
            // In entrambi i casi la lista viene ricaricata
            getSession().removeAttribute("addUdToVolume");
            Long idVolume = (Long) getSession().getAttribute("volCorrente");
            if (idVolume != null) {
                try {
                    VolVRicVolumeRowBean vv = volumiHelper.findVolVRicVolume(new BigDecimal(idVolume));
                    // Carico la pagina di dettaglio dei Volumi
                    getForm().getVolumiDetail().copyFromBean(vv);
                    /* Gestione dei tipi dato soggetti alle abilitazioni */
                    DecTipoUnitaDocTableBean tmpTableBeanTipoUD = tipoUnitaDocEjb
                    ...
                    VolVListaCompVolTableBean listaComp = componentiHelper.getVolVListaCompVolViewBean(
                            ...
                            null, tmpTableBeanTipoUD, tmpTableBeanTipoDoc, tmpSubStrutsTableBean,
                            Integer.parseInt(maxResultCompVol));
        ...
        // Se torno indietro dal dettaglio volume, rilancio la ricerca in quanto posso aver modificato il volume
        // esaminato
        if (getLastPublisher().equals(Application.Publisher.VOLUMI_DETAIL)
                && ((getRequest().getParameter("table") != null
                        && getRequest().getParameter("table").equals(getForm().getVolumiList().getName()))
                        || (getRequest().getParameter("mainNavTable") != null && getRequest()
                                .getParameter("mainNavTable").equals(getForm().getVolumiList().getName())))) {
            try {
    ...
    public void listaOperazioniVolume() throws EMFError {
        ...
        redirectToAction(Application.Actions.MONITORAGGIO,
                "?operation=ricercaOperazioniVolumiDaDettaglioVolume&idVolumePerMon=" + idVol + "&idStrutPerMon="
                        + idStrut + "&eseguiForward=true",
                form);
    }
}

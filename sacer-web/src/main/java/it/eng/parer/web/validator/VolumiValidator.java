...
public class VolumiValidator extends TypeValidator {
    ...
    // * @throws EMFError
    // */
    // public void validateChiaveDocVolumi(String registro, BigDecimal anno, String codice) throws EMFError {
    ...
    // getMessageBox().addMessage(new Message(MessageLevel.ERR, "Devono essere impostati il registro e l'anno prima del
    // numero"));
    // }
    // }
    // }
    ...
    public void validateDataVolumi(Date data_da, Date data_a) throws EMFError {
        ...
        if (data_a == null && data_da != null) {
            ...
            if (data_a.before(data_da)) {
                data_a = null;
                getMessageBox().addMessage(new Message(MessageLevel.ERR, "Data di inizio superiore alla data odierna"));
            }
        }
        if (data_da != null && data_a != null) {
            if (data_da.after(data_a)) {
                getMessageBox().addMessage(new Message(MessageLevel.ERR, "Data di inizio superiore a data fine"));
            }
        }
    }

    /**
     * Metodo di validazione dei campi (singoli e range) relativi alla chiave unità documentaria secondo le regole
     * presenti in analisi
     *
     * @param registro
     *            valore registro
    ...
    public Object[] validaChiaveUnitaDocVolumi(String registro, BigDecimal anno, String numero, BigDecimal anno_da,
        ...
        /*
         * Controllo innanzitutto che non siano stati inseriti i filtri sia sulla chiave UD singola sia sulla chiave UD
         * per range
         */
        ...
        if (chiave && range) {
            ...
            if (chiave || registro != null) {
                ...
                // Effettuo i controlli
                if (registro == null
                        && (result[3] != null || result[4] != null || result[1] != null || result[2] != null)) {
                    getMessageBox().addMessage(new Message(MessageLevel.ERR, "Registro non impostato"));
                }
                if (registro != null) {
     ...
     * IN_CODA_JMS_GENERA_INDICE_AIP o IN_CODA_JMS_VERIFICA_FIRME_DT_VERS, se non sono definiti filtri (singoli e range)
     * relativi alla chiave unità documentaria, oppure se è compilato il filtro “Numero ore in coda JMS”
     *
     * @param chiavi
     *            lista chiavi
     * @param numOreInCodaJMS
     *            numero ore in coda
     * @param tiStatoElenco
     *            tipo stato elenco
     * 
     ...
     *             errore generico
     */
    public void validateCodaJMS(Object[] chiavi, BigDecimal numOreInCodaJMS, List<String> tiStatoElenco)
            throws EMFError {
        if ((tiStatoElenco.contains(ElencoEnums.ElencoStatusEnum.IN_CODA_JMS_GENERA_INDICE_AIP.name())
                || tiStatoElenco.contains(ElencoEnums.ElencoStatusEnum.IN_CODA_JMS_VERIFICA_FIRME_DT_VERS.name()))
                && chiavi != null) {
            getMessageBox().addMessage(new Message(MessageLevel.ERR,
        ...
        if (numOreInCodaJMS != null
                && !tiStatoElenco.contains(ElencoEnums.ElencoStatusEnum.IN_CODA_JMS_GENERA_INDICE_AIP.name())
                && !tiStatoElenco.contains(ElencoEnums.ElencoStatusEnum.IN_CODA_JMS_VERIFICA_FIRME_DT_VERS.name())) {
            getMessageBox().addMessage(new Message(MessageLevel.ERR,

...
public class MonitoraggioSinteticoAction extends MonitoraggioSinteticoAbstractAction {
    ...
    public JSONObject triggerRiepilogoVersamentiSinteticoId_ambienteOnTrigger() throws EMFError {
        ...
        BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
        if (idAmbiente != null) {
            ...
            if (tmpTableBeanEnte.size() == 1) {
                ...
                if (tmpTableBeanStrut.size() == 1) {
                    ...
                    getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().setDecodeMap(new DecodeMap());
                }
            } else {
        ...
        if (idEnte != null) {
            ...
            if (tmpTableBeanStrut.size() == 1) {
                ...
                DecTipoUnitaDocTableBean tmpTableBeanTUD = tipoUnitaDocEjb
                        .getTipiUnitaDocAbilitati(getUser().getIdUtente(), tmpTableBeanStrut.getRow(0).getIdStrut());
                getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().setDecodeMap(
                        DecodeMap.Factory.newInstance(tmpTableBeanTUD, "id_tipo_unita_doc", "nm_tipo_unita_doc"));
            } else {
                getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().setDecodeMap(new DecodeMap());
    ...
    public void generaRiepilogoVersSinteticoButton() throws EMFError {
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
            BigDecimal idTipoUnitaDoc = getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().parse();
    ...
    public void reloadAfterGoBack(String publisher) {
        try {
            ...
            logger.error("Errore nel ricaricamento del riepilogo versamenti sintetico", e);
    ...
    public void loadRiepilogoVersamentiSinteticoByStrut() throws EMFError {
        try {
            BigDecimal idStrut = new BigDecimal(
                    getRequest().getParameter("idStrut") != null ? (String) getRequest().getParameter("idStrut") : "0");
            ...
            // Ricavo id struttura, ente ed ambiente attuali
            BigDecimal idStruttura = idStrut.equals(BigDecimal.ZERO) ? getUser().getIdOrganizzazioneFoglia() : idStrut;
            BigDecimal idEnte = monitSintEjb.getEnte(idStruttura);
            BigDecimal idAmbiente = monitSintEjb.getAmbiente(idEnte);
    ...
    private void calcolaRiepilogoSinteticoGlobale(BigDecimal idAmbiente, BigDecimal idEnte, BigDecimal idStruttura,
            BigDecimal idTipoUnitaDoc) throws EMFError {
        Set<String> fieldsSet = (Set<String>) getSession().getAttribute(WebConstants.PARAMETER_SESSION_GET_CNT);
        if (fieldsSet == null) {
            fieldsSet = new HashSet<String>();
            getSession().setAttribute(WebConstants.PARAMETER_SESSION_GET_CNT, fieldsSet);
        }
        LinkedHashMap<String, BaseRowInterface> formMap = monitSintEjb.calcolaRiepilogoSintetico(
                getUser().getIdUtente(), idAmbiente, idEnte, idStruttura, idTipoUnitaDoc, fieldsSet);
        getForm().getUnitaDocVersate().copyFromBean(formMap.get(getForm().getUnitaDocVersate().getName()));
        getForm().getDocAggiunti().copyFromBean(formMap.get(getForm().getDocAggiunti().getName()));
        getForm().getVersamentiFalliti().copyFromBean(formMap.get(getForm().getVersamentiFalliti().getName()));
    ...
    public void listaDocumenti() throws EMFError {
        ...
        form.getRiepilogoVersamenti().getId_ambiente()
                .setDecodeMap(getForm().getRiepilogoVersamentiSintetico().getId_ambiente().getDecodeMap());
        form.getRiepilogoVersamenti().getId_ambiente()
                .setValue(getForm().getRiepilogoVersamentiSintetico().getId_ambiente().getValue());

        form.getRiepilogoVersamenti().getId_ente()
                .setDecodeMap(getForm().getRiepilogoVersamentiSintetico().getId_ente().getDecodeMap());
        form.getRiepilogoVersamenti().getId_ente()
                .setValue(getForm().getRiepilogoVersamentiSintetico().getId_ente().getValue());

        form.getRiepilogoVersamenti().getId_strut()
                .setDecodeMap(getForm().getRiepilogoVersamentiSintetico().getId_strut().getDecodeMap());
        form.getRiepilogoVersamenti().getId_strut()
                .setValue(getForm().getRiepilogoVersamentiSintetico().getId_strut().getValue());

        form.getRiepilogoVersamenti().getId_tipo_unita_doc()
                .setDecodeMap(getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().getDecodeMap());
        form.getRiepilogoVersamenti().getId_tipo_unita_doc()
                .setValue(getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().getValue());
        ...
        if (StringUtils.isNotBlank(stato) && StringUtils.isNotBlank(tiCreazione) && StringUtils.isNotBlank(tipo)) {
    ...
    public void listaDocAnnullati() throws EMFError {
        ...
        if (StringUtils.isNotBlank(stato) && StringUtils.isNotBlank(tipo)) {
            ...
            default:
                // Verifico che lo stato sia definito nell'enum
                VolumeEnums.MonitAnnulStatusEnum status = VolumeEnums.MonitAnnulStatusEnum.valueOf(stato);
    ...
    public void calcTotUdVersateButton() throws EMFError {
        ...
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            ...
            BigDecimal idTipoUnitaDoc = getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().parse();
            calcolaRiepilogoSinteticoGlobale(idAmbiente, idEnte, idStruttura, idTipoUnitaDoc);
            // BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
            // idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.UNITA_DOC_VERSATE, false);
    ...
    public void calcTotAggFallitiButton() throws Throwable {
        setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate.AGGIUNTE_DOC_FALLITE);
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
            ...
            calcolaRiepilogoSinteticoGlobale(idAmbiente, idEnte, idStruttura, idTipoUnitaDoc);
            // BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
            // idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.AGGIUNTE_DOC_FALLITE, false);
    ...
    public void calcTotUdNonVersButton() throws Throwable {
        setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate.UNITA_DOC_VERS_FALLITI);
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
            ...
            calcolaRiepilogoSinteticoGlobale(idAmbiente, idEnte, idStruttura, idTipoUnitaDoc);
            // BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
            // idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.UNITA_DOC_VERS_FALLITI, false);
    ...
    public void calcTotDocNonVersButton() throws Throwable {
        setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate.DOC_AGGIUNTI_VERS_FALLITI);
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
            BigDecimal idTipoUnitaDoc = getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().parse();
            BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
                    idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.DOC_AGGIUNTI_VERS_FALLITI, false);
    ...
    public void calcTotVersUdAnnulButton() throws Throwable {
        ...
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            ...
            BigDecimal idTipoUnitaDoc = getForm().getRiepilogoVersamentiSintetico().getId_tipo_unita_doc().parse();
            BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
                    idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.UNITA_DOC_ANNUL, false);
    ...
    public void setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate field) {
        Set<String> fieldSet = (Set<String>) getSession().getAttribute(WebConstants.PARAMETER_SESSION_GET_CNT);
        if (fieldSet == null) {
    ...
    public void calcTotUdVersateB30Button() throws Throwable {
        setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate.UNITA_DOC_VERSATE_B30);
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
    ...
    public void calcTotAggFallitiB30Button() throws Throwable {
        setRicalcoloSessionAttribute(MonitoraggioSinteticoEjb.fieldSetToPopulate.AGGIUNTE_DOC_FALLITE_B30);
        if (getForm().getRiepilogoVersamentiSintetico().postAndValidate(getRequest(), getMessageBox())) {
            BigDecimal idAmbiente = getForm().getRiepilogoVersamentiSintetico().getId_ambiente().parse();
            BigDecimal idEnte = getForm().getRiepilogoVersamentiSintetico().getId_ente().parse();
            BigDecimal idStruttura = getForm().getRiepilogoVersamentiSintetico().getId_strut().parse();
            ...
            calcolaRiepilogoSinteticoGlobale(idAmbiente, idEnte, idStruttura, idTipoUnitaDoc);
            // BaseRowInterface row = monitSintEjb.calcolaTot(getUser().getIdUtente(), idAmbiente, idEnte, idStruttura,
            // idTipoUnitaDoc, MonitoraggioSinteticoEjb.fieldSetToPopulate.AGGIUNTE_DOC_FALLITE,true);
            // getForm().getAggiunteDocumentiFallite().copyFromBean(row);
        }
    }
}

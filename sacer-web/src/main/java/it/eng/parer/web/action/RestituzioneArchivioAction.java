...
public class RestituzioneArchivioAction extends RestituzioneArchivioAbstractAction {
    ...
    private static final Logger log = LoggerFactory
	    .getLogger(RestituzioneArchivioAction.class.getName());
	...
	if (getNavigationEvent().equals(ListAction.NE_DETTAGLIO_VIEW)
		...
		AroVRicRichRaRowBean row = (AroVRicRichRaRowBean) getForm().getRichRestArchList()
    ...
    private void loadDettaglioRichiesta(BigDecimal idRichRestArch, BigDecimal idStrut)
	    throws EMFError {
	AroRichiestaRaRowBean detailRow = restArchEjb.getAroRichiestaRaRowBean(idRichRestArch,
		idStrut);
	getForm().getRichRestArchDetail().copyFromBean(detailRow);

	AroVLisItemRaTableBean listaItem = restArchEjb.getAroVLisItemRaFmTableBean(idRichRestArch,
		idStrut);
	...
	if (restArchEjb.checkStatoRichiesta(idRichRestArch,
		AroRichiestaTiStato.CALCOLO_AIP_IN_CORSO, AroRichiestaTiStato.IN_ATTESA_ESTRAZIONE,
		AroRichiestaTiStato.ESTRAZIONE_IN_CORSO, AroRichiestaTiStato.ERRORE,
		AroRichiestaTiStato.ESTRATTO, AroRichiestaTiStato.VERIFICATO)) {
	...
	if (restArchEjb.checkStatoRichiesta(idRichRestArch, AroRichiestaTiStato.ERRORE)) {
	    // se nella richiesta non sono presenti item con stato = DA_ELABORARE ed è presente
	    // almeno un item con stato
	    // = ERRORE
		...
		getForm().getRichRestArchDetailButtonList().getRielaboraRichiesta()
			.setHidden(false);
	    }
	}
    }
	...
	try {
		...
		RicercaRichRestArchBean filtri = new RicercaRichRestArchBean(
			getForm().getFiltriRicercaRichRestArch());
		if (!getMessageBox().hasError()) {
		    AroVRicRichRaTableBean table = restArchEjb
			    .getAroVRicRichRaTableBean(getUser().getIdUtente(), filtri);
	...
	if (!getMessageBox().hasError()) {
	    if (idStrut == null) {
		getMessageBox().addError(
			"Per poter creare la richiesta \u00E8 necessario selezionare una struttura");
	    }
	}
	...
	if (!getMessageBox().hasError()) {
		...
		if (((String) getSession().getAttribute("tiRichRestArch")).equals("UNITA_DOC")) {
		    getForm().getCreazioneRichRestArch().getTi_rich_rest_arch()
			    .setValue("UNITA_DOC");
		...
		} else if (((String) getSession().getAttribute("tiRichRestArch"))
			.equals("FASCICOLI")) {
		    getForm().getCreazioneRichRestArch().getTi_rich_rest_arch()
			    .setValue("FASCICOLI");
		}
	...
	try {
	    ...
	    log.error(
		    "Eccezione generica nella creazione della richiesta di restituzione archivio: "
	...
	if (getSession().getAttribute("attributiAssociazioniScadute") != null) {
		...
		log.error("Errore nel salvataggio della richiesta di restituzione archivio", ex);
		getMessageBox().addError(
			"Errore nel salvataggio della richiesta di restituzione archivio");
	    } catch (Exception ex) {
    ...
    public void eseguiCreazioneRichiestaRestituzioneArchivio(BigDecimal idStrut,
	    String tiRichRestArch) throws ParerUserError, Exception {
	// Gestisco le richieste scadute
	restArchEjb.elaboraRichRestArchExpired(idStrut);
	// Salvo la richiesta di restituzione archivio
	Long idRichRestArch = restArchEjb.saveRichRestArch(getUser().getIdUtente(), idStrut,
		tiRichRestArch);
	//
	if (tiRichRestArch.equals("UNITA_DOC")) {
		...
		OrgAmbienteRowBean ambienteRB = struttureEjb
			.getOrgAmbienteRowBean(enteRB.getIdAmbiente());
	...
	if (!getMessageBox().hasError()) {
	    if (idRichRestArch != null) {
		calcoloEstrazioneAsync.creaRichiestaEstrazioneAsync(idRichRestArch,
			idStrut.longValue());
	    }
	    if (idRichRestArch != null) {
	...
	getForm().getFiltriRicercaRichRestArch().getTi_stato_rich_rest_arch_cor()
		.setDecodeMap(ComboGetter.getMappaSortedGenericEnum("ti_stato_rich_rest_arch",
			AroRichiestaTiStato.values()));
	...
	if (getForm().getFiltriRicercaRichRestArch().postAndValidate(getRequest(),
		getMessageBox())) {
	    eseguiRicercaRichRestArch();
	}
	forwardToPublisher(Application.Publisher.RICERCA_RICH_REST_ARCH);
	...
	if (!getMessageBox().hasError()) {
		...
		AroRichiestaRaRowBean richiestaRB = restArchEjb
			.getAroRichiestaRaRowBean(idRichiestaRa);
		OrgStrutRowBean strutRB = struttureEjb.getOrgStrutRowBean(richiestaRB.getIdStrut());
		OrgEnteRowBean enteRB = struttureEjb.getOrgEnteRowBean(strutRB.getIdEnte());
		BaseRow enteSiamRB = ambienteEjb
			.getSIOrgEnteSiamRowBean(strutRB.getIdEnteConvenz());
	    ...
	    table.addSortingRule(
		    SortingRule.getDescending(AroVRicRichRaTableDescriptor.COL_TS_INIZIO));
	    table.sort();
	    getForm().getRichRestArchList().setTable(table);
	    getForm().getRichRestArchList().getTable().setPageSize(WebConstants.DEFAULT_PAGE_SIZE);
	    getForm().getRichRestArchList().getTable().first();
	...
	if (restArchEjb.checkStatoRichiesta(idRichRestArch,
		AroRichiestaTiStato.CALCOLO_AIP_IN_CORSO, AroRichiestaTiStato.IN_ATTESA_ESTRAZIONE,
		AroRichiestaTiStato.ESTRAZIONE_IN_CORSO, AroRichiestaTiStato.ESTRATTO,
		AroRichiestaTiStato.ERRORE, AroRichiestaTiStato.VERIFICATO)) {
	    cambiaStato(idRichRestArch, AroRichiestaTiStato.ANNULLATO);
	} else {
	    getMessageBox()
		    .addInfo("Modifica non possibile in quanto la richiesta ha cambiato lo stato");
	...
	final BigDecimal idRichRestArch = getForm().getRichRestArchDetail().getId_richiesta_ra()
		.parse();
	if (restArchEjb.checkStatoRichiesta(idRichRestArch, AroRichiestaTiStato.ERRORE)) {
	    try {
		...
		restArchEjb.cambiaStato(getUser().getIdUtente(), idRichRestArch,
			AroRichiestaTiStato.ERRORE.name(),
	    ...
	    getMessageBox()
		    .addError("Controllo non possibile in quanto la richiesta non ha stato ERRORE");
	}
	forwardToPublisher(getLastPublisher());
    }
	...
	BigDecimal idRichRestArch = row.getIdRichiestaRa();
	if (restArchEjb.checkStatoRichiesta(idRichRestArch, AroRichiestaTiStato.ESTRATTO)) {
	    // se nella richiesta non sono presenti item con stato = ERRORE o con stato =
	    // DA_ELABORARE
	    ...
	    getMessageBox().addError(
		    "Modifica non possibile in quanto la richiesta non ha stato ESTRATTO");
	}
    }
	...
	if (restArchEjb.checkStatoRichiesta(idRichRestArch, AroRichiestaTiStato.VERIFICATO)) {
	    ...
	    getMessageBox().addError(
		    "Modifica non possibile in quanto la richiesta non ha stato VERIFICATO");
	}
    }

    private void cambiaStato(BigDecimal idRichRestArch, AroRichiestaTiStato tiStatoRichRestArch) {
	getForm().getCambioStatoRichiesta().getId_rich_rest_arch()
		.setValue(idRichRestArch.toPlainString());
	...
	getForm().getCambioStatoRichiesta().getTi_stato_rich_rest_arch()
		.setValue(tiStatoRichRestArch.name());
	...
	try {
	    ...
	    AroVRicRichRaTableBean struttureInteressate = restArchEjb
		    .getAroVRicRichRaTableBean(idRichRestArch);

	    String msg = "Confermando l'operazione lo stato della richiesta sarà modificato in "
		    + tiStatoRichRestArch + " su tutte le strutture dell'ente convenzionato "
		    + nmEnteConvenz + ":";
	...
	BigDecimal idRichRestArch = getForm().getCambioStatoRichiesta().getId_rich_rest_arch()
		.parse();
	String dsNotaRichRestArch = getForm().getCambioStatoRichiesta().getDs_nota_rich_rest_arch()
		.parse();
	String tiStatoRichRestArch = getForm().getCambioStatoRichiesta()
		.getTi_stato_rich_rest_arch().parse();

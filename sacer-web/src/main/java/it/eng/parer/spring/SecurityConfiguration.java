...
public class SecurityConfiguration extends ParerSecurityConfiguration {
    ...
    SecurityFilterChain app(HttpSecurity http,
        ...
        http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/logout").permitAll()
                ...
                .requestMatchers("/*.html").authenticated().requestMatchers("/*.json").authenticated()
                .requestMatchers("/VerificaFirme").authenticated().requestMatchers("/rest/docounter").authenticated()
        ...
        http.headers().permissionsPolicy(pol -> pol.policy(System.getProperty("http.sec.header.permissions-policy",
